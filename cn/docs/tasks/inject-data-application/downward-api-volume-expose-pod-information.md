...
此页面描述如何使用DownwardAPIVolumeFile把自己的信息呈现给pod中运行的容器。DownwardAPIVolumeFile可以呈现pod的字段和容器字段。
...
第一个元素指定Pod的`metadata.labels`字段的值保存在名为`labels`的文件中。
第二个元素自定Pod的`annotations`字段的值保存在名为`annotations`的文件中。
...
下面这些信息可以通过environment variables和DownwardAPIVolumeFiles提供给容器：
...
一个例子是有一个应用采用一个特殊周知的环境变量来保存一个唯一标识。一种可能是给应用增加处理层，但这样是冗余和易出错的，而且它违反了低耦合的目标。
更好的选择是使用Pod名称作为标识，把Pod名称注入这个环境变量中。
{% endcapture %}
