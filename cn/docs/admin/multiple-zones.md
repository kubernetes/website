...
Kubernetes 1.2支持将集群运行在多个故障域中。
(GCE 中称其为 "区"， AWS 中称其为 "可用区"，这里我们也称其为 "区")。
...
完整的集群联邦能够将运行在不同区或云供应商（或本地数据中心）的集群集中管理。
...
这种分发是性能优先（best-effort）的，所以如果集群在各个区之间是异构的
(比如，各区间的节点数量不同，节点类型不同，pod的资源需求不同等)甚至可能使得pod的跨区域分发完全失效。
如果需要的话，尽量使用同质的区(节点数量和节点类型相同)来减少区域之间分配不均匀的可能。

当卷被创建时， `PersistentVolumeLabel`准入控制器会自动为其添加区域的标签。
调度器 (通过 `VolumeZonePredicate` 算法) 会确保声明使用该卷的pod被调度到该卷对应的区域，
因为卷是不支持跨区挂载的。
...
* 我们假设不同的区域间在网络上离得很近，所以我们不做任何的区域感知路由。 特别是，通过服务的信息传输可能跨区域 (即使该服务后端pod的其中一些运行在与客户端相同的区域中)，这可能导致额外的延迟和损耗。

* 卷的区域亲和性只对 `PersistentVolume`有效，如果你在pod的spec中直接指定一个EBS的卷（例如）则不会生效。
...
* 目前有状态应用的卷动态创建时的跨区域分配，与pod的亲和性/反亲和性不兼容。

* 有状态应用的名称包含破折号 ("-")时，可能影响到卷在区域间的均匀分布。

* 当为deployment或pod指定多个PVC时，要求其存储类型处于同一区域内，否则，相应的卷需要在一个区域中静态配置。 另一种方式是使用有状态应用（StatefulSet），这可以确保对应的卷在同一区域中。


## 演练

接下来我们将介绍如何在 GCE 和 AWS 上创建和使用多区域的集群。 为此，你需要创建一个完整的集群 
...
接下来我们复用已有的管理节点，在另一个区域 (us-central1-b or us-west-2b) 添加节点。
再次执行 kube-up， 通过指定 `KUBE_USE_EXISTING_MASTER=true`
kube-up 虽然仍会创建新的管理节点，但是会复用之前创建的。
...
再次检查节点，3个新增的节点已经启动，并被标记为us-central1-b：
...
使用动态创建功能创建一个卷 (卷的区域亲和性只支持PersistentVolumes)：
...
**注意：** Kubernetes 1.3以上的版本中可以将PVC分发到配置的区域中去，在1.2版本中， 动态卷只能创建在管理节点所在的区域内(即这里的 us-central1-a / us-west-2a)；相关issue
...
现在我们将创建应用PVC的pod。
因为 GCE 的PD存储 / AWS 的EBS 卷 不支持跨区域attach，
...
副本管理器或服务的pod被自动创建在了不同的区域。  首先，让我们在第三个区域内启动节点：
...
创建 guestbook-go 用例， 它包含一个副本数为3个RC，运行一个简单的网络应用：
...
负载平衡器跨集群中的所有区域； guestbook-go 用例包含一个
负载均衡服务的例子：
