...
用户通过 `kubectl`，客户端库，或者通过发送REST请求[访问API](/docs/user-guide/accessing-the-cluster)。 用户（自然人）和[Kubernetes服务账户](/docs/tasks/configure-pod-container/configure-service-account/) 都可以进行API访问的授权。
请求到达API后会经过几个阶段，具体说明如图：
...
包含该API服务器证书的根证书，当被指定后会用来代替系统默认根证书。 当用户使用 `kube-up.sh` 创建集群时，该证书通常会被自动写入用户的`$USER/.kube/config`。  如果集群中存在多个用户，则创建者需要与其他用户共享证书。

## 认证

一旦 TLS 连接建立，HTTP请求就进入到了认证的步骤。即图中的步骤 **1** 。
集群创建脚本或集群管理组件会为API服务器配置一个或多个认证模块。
更具体的认证相关的描述详见 [这里](/docs/admin/authentication/)。

认证步骤的输入是整个HTTP请求，但这里通常只是校验请求头和/或客户端证书。

认证模块包括客户端证书，密码和一般的Token，
Bootstrap Tokens，以及JWT Tokens (用于服务账户)。

可以同时制定多种认证模块，在这种情况下，每个模块会依次尝试认证，
直到其中一个认证成功。

在 GCE 平台中，客户端证书，密码和一般的Token，Bootstrap Tokens，以及JWT Tokens同时被启用。

如果请求认证失败，则请求被拒绝，返回401状态码。
如果认证成功，则被认证为具体的 `username`，该用户名可供随后的步骤中使用。一些认证模块还提供了用户的组成员关系，另一些则没有。

尽管Kubernetes使用 "用户名" 来进行访问控制和请求记录，但它实际上并没有 `user` 对象，也不对用户名称或其他相关信息进行存储。
