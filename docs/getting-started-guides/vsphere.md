...
### Getting started with the vSphere
...
**[ Step 5 ]** Create the vSphere cloud config file. Cloud config template can be found [here](https://github.com/kubernetes/kubernetes-anywhere/blob/master/phase1/vsphere/vsphere.conf)
...
[Global]
        ...
        port = "443" #Default 443 for vCenter
...
* `vm-uuid` is the VM Instance UUID of virtual machine. `vm-uuid` can be set to empty (vm-uuid = ""). if set to empty, this will be retrieved from /sys/class/dmi/id/product_serial file on virtual machine (requires root access).
     ...
     * `vm-uuid` needs to be set in the following format
     
        423D7ADC-F7A9-F629-8454-CE9615C810F1
     ...
     * `vm-uuid` can be retrieved from Node Virtual machines using following command. This will be different on each node VM.
        ...
        ```
 * `datastore` is the default datastore used for provisioning volumes using storage classes. If datastore is located in storage folder or datastore is member of datastore cluster, make sure to specify full datastore path.
     ...
     * For datastore located in the storage folder, specify datastore as mentioned below
       ```
       datastore = "storagefolder/datastore2"
       ```
       Note: Make sure vSphere Cloud Provider user has Read Privilege set on the datastore cluster or storage folder to be able to find datastore.
