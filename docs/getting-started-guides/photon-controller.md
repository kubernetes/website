...
```
    go get github.com/vmware/photon-controller-cli
```
...
```
curl --remote-name-all https://bintray.com/artifact/download/photon-controller/public/OS/debian/debian-8.2.vmdk
```
...
If you prefer, you can  use the provided `cluster/photon-controller/setup-prereqs.sh`
script to create these. Assuming the IP address of your Photon
Controller is 192.0.2.2 (change as appropriate) and the downloaded image is in
...
use hte password there. (Note that the output has been trimmed: the
...
1. Configure your service with the NodePort option. For example, this
...
2. Create a new standalone VM (or VMs, for high availability) to act
