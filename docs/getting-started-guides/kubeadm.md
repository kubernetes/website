...
This guide covers the current version of kubeadm.  The previous version is still available but is a little tricky to get to.  Also note that command line options and other configuration items have changed.

As root, run the following.  This is very similar the regular instructions except for pinning the versions.  Note that, due to some unfortunate version strings, kubeadm isn't indexed in the repos and the packages must be downloaded and installed directly.
