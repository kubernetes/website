...
After a cluster has been created you will have a collection of system pods running in the `kube-system` namespace that support monitoring, logging and DNS resolution for names of Kubernetes services:
...
If for any reason the image in this pod is killed off and then restarted by Kubernetes or pod was evicted from the node, logs for the container are lost. Let's check this. First let's delete the currently running counter.
