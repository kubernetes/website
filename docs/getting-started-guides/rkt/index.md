...
* You will need [kubelet](/docs/setup/scratch/#kubelet) installed on the node, and it's recommended that you run [kube-proxy](/docs/setup/scratch/#kube-proxy) on all nodes. This document describes how to set the parameters for kubelet so that it uses rkt as the runtime. 
