...
1. Three machines running Ubuntu 16.04+, CentOS 7 or HypriotOS v1.0.1+ for the masters. 
1. Three machines running Ubuntu 16.04+, CentOS 7 or HypriotOS v1.0.1+ for the workers. 
1. (optional) At least three machines if you intend to host etcd on dedicated nodes (see information below).
...
1. First install etcd binaries like so:
    ...
    It's worth noting that most distributions already have a version of etcd installed, so you will be replacing the system default.
...
1. Copy the contents of `/etc/kubernetes/pki/ca.pem` and create this file manually on `master1` and `master2`.
