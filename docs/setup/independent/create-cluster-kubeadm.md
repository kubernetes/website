...
1. The `HostPort` and `HostIP` functionality is available depending on your Pod Network
   provider. Please contact the author of the Pod Network solution to find out whether
   `HostPort` and `HostIP` functionality is available.
...
To help diagnose what happened, you use `kubectl describe pod` or `kubectl logs` like this:
...
The `kube-dns` Pod being in the `Pending` state is normal, you have to install a Pod network
before it will get deployed fully.

If your Pod Network provider's Pods/workloads are failing even after you have deployed them
to your cluster, please contact the respective communities first. If you think there is an
issue with kubeadm itself, open an issue in the [kubeadm issue tracker](https://github.com/kubernetes/kubeadm/issues)
