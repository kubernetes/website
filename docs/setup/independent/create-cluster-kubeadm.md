...
```
**Note:** you must save `kubeadm join --token <token> <master-ip>:<master-port> --discovery-token-ca-cert-hash sha256:<hash>`in your local, it's so important once you need join machine to cluster.
...
```
 ...
 - More information about `flannel`, please see [here](https://github.com/coreos/flannel).
