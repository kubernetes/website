...
**Note:** You need to choose a Pod Network Plugin in the next step. Depending on what
...
**Note:** In order for Network Policy to work correctly, you need to pass `--pod-network-cidr=192.168.0.0/16` to `kubeadm init`

**Note:** Calico works on `amd64` only.
...
**Note:** For Canal to work correctly, `--pod-network-cidr=10.244.0.0/16` has to be passed to `kubeadm init`.

**Note:** Canal works on `amd64` only.
...
**Note:** flannel works on `amd64`, `arm`, `arm64` and `ppc64le`, but for it to work on an other platform than
...
{% assign tab_names = "Default,Calico,Canal,Flannel,Weave Net" | split: ',' | compact %}
{% assign tab_contents = site.emptyArray | push: default_tab | push: calico | push: canal | push: flannel | push: weave_net %}
