...
`kubectl run` allows you to generate following resources (using `--generator` flag):
...
is chosen.  This also means that combining any of above flags with `--generator` will
not change the generator at any point.
