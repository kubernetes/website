...
run on particular nodes. There are several ways to do this, but they all use
[label selectors](/docs/user-guide/labels/) to make the selection.
Generally this is unnecessary, as the scheduler will take care of things for you, but you may want to do so in certain circumstances
like to ensure that your pod ends up on a machine with an SSD attached to it.
