...
taints and tolerations is like a filter: start
with all of a node's taints, then ignore the ones for which the pod has a matching toleration; the
remaining unignored taints have the indicated effects on the pod. In particular,
...
* if there is at least one unignored taint with effect `NoExecute` then the pod will be evicted from
the node (if it is already running on the node or is scheduled onto the node).
...
hardware off of those nodes, so as to leave room for later-arriving pods that do need the
...
[admission controller](https://kubernetes.io/docs/admin/admission-controllers/)) that
uses some characteristic(s) of the pod to determine that it should be allowed to use
the special nodes and hence add the toleration. To ensure that the pods that need
 ...
 * pods that toleate the taint with a specified `tolerationSeconds` remain
...
bound to node for a long time in the case of network partition, in the hope
...
[here](https://kubernetes.io/docs/admin/node/#node-controller)) is maintained
