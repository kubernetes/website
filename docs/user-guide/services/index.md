...
A special case of service without selectors is an ExternalName service.
It does not define any ports or endpoints. Rather, it serves as a way
to return an alias to an external service residing outside the cluster.
...
Every node in a Kubernetes cluster runs a `kube-proxy`.  This application is
