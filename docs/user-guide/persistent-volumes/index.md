...
* `zones`: a comma separated list of AWS zone(s). If neither zone nor zones is specified, a random zone in the same region as controller-manager will be chosen. Note: zone and zones parameters must not be used
at the same time.
