...
Currently, there are three types of application health checks that you can choose from:
   ...
   * HTTP Health Checks - The Kubelet will call a web hook.  If it returns between 200 and 399, it is considered success, failure otherwise. See health check examples [here](/docs/tasks/configure-pod-container/configure-liveness-readiness-probes/).
   * Container Exec - The Kubelet will execute a command inside your container.  If it exits with status 0 it will be considered a success. See health check examples [here](/docs/tasks/configure-pod-container/configure-liveness-readiness-probes/).
