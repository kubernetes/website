...
{% capture overview %}
This page show how to configure and enable the ip-masq-agent. 
{% endcapture %}
...
The ip-masq-agent configures iptables rules to hide a pod's IP address behind the cluster nodes's IP address. This is typically done when sending traffic to destinations outside the cluster's pod [CIDR](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing) range.
