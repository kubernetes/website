...
etcd, recommended to run as a cluster of odd members, is a leader-based distributed system. The leader needs to periodically send heartbeats on time to all followers to keep the cluster stable. Performance and stability of the cluster is sensitive to network and disk IO. Any resource starvation can lead to heartbeat timeout, causing instability of the cluster. An unstable etcd indicates that no leader is elected. Under such circumstances, a cluster cannot make any changes to its current state, which implies no new pods can be scheduled. Because keeping stable etcd clusters is critical to the stability of Kubernetes clusters, it's recommended to run them on dedicated machines or isolated environments for [guaranteed resource requirements]([https://github.com/coreos/etcd/blob/master/Documentation/op-guide/hardware.md#hardware-recommendations).
...
This section covers instruction on starting a Kubernetes API server with an etcd cluster in the deployment.
...
For durability and high availability, run etcd as a multi-node cluster in production and take its backup periodically. A five-member cluster is recommended in production. For more information, see [FAQ Documentation ](https://github.com/coreos/etcd/blob/master/Documentation/faq.md#what-is-failure-tolerance).
...
After configuring secure communication, restrict the access of etcd cluster to only Kubernetes API server. Use TLS authentication to do so.
...
**Note**: etcd authentication is not currently supported by Kubernetes. For more information, see [Issues](https://github.com/kubernetes/kubernetes/issues/23398).
...
If the majority of etcd members are permanently failed, the etcd cluster is considered failed. In this scenario, Kubernetes cannot make any changes to its current state. Although the scheduled pods might  continue to run, no new pods can be scheduled. In such cases, recover the etcd cluster and potentially reconfigure Kubernetes API server to fix the issue.
