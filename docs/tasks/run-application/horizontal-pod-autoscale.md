...
Starting from v1.6, a cluster operator is enabled to mitigate this problem by tuning
the global HPA settings exposed as flags for the `kube-controller-manager` component:

- `--horizontal-pod-autoscaler-downscale-delay`: The value for this optin is a
  duration that specifies for how long the autoscaler has to wait before another
...
could be complaints that the Horizontal Pod Autoscaler is not responsive at workload
