...
Generate a CSR yaml and send it to the apiserver by running the following
command:
...
1. The subject of the CSR controls the private key used to sign the CSR. This
   ...
   In the above example, this step would be to verify that the pod is allowed
   to participate in the requested service.
2. The subject of the CSR is authorized to act in the requested context. This
   ...
   above example, this step would be to verify that the pod controls the
