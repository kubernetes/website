...
**Note**: Although this flag does have an affect currently (it's needed for the
kube-dns Deployment manifest and the API Server's serving certificate), you will
need to modify the arguments to the kubelets in the cluster for it to work fully.
Specifying DNS parameters using this flag only is not enough. Rewriting the
kubelet's CLI arguments is out of scope for kubeadm as it should be agnostic
to how you run the kubelet. However, making all kubelets in the cluster pick up
information dynamically via the API _is_ in scope and is a
[planned feature](https://github.com/kubernetes/kubeadm/issues/28) for current
releases.
