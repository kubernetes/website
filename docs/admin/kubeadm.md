...
## Using kubeadm with a config file

It's possible to configure kubeadm with a config file instead of command line flags, and the more advanced modes of kubeadm will only be
available from as configuration file options. Here is a demonstration of what the config file looks like.

Keep in mind that this is only the alpha version and this API syntax will _definitely_ change.
The master configuration that can be passed to `kubeadm init --config ./myconfigfile.yaml`
...
The node configuration that can be passed to `kubeadm join --config ./myconfigfile.yaml`
...
1.  Generate a token.  This token must have the form  `<6 character string>.<16 character string>`.
    ...
    Here is how you can do it easily with kubeadm:
...
These enviroment variables are a short-term solution, eventually they will be integrated in the kubeadm config file.
