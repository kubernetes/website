...
self-hosted cluster cannot currently recover from a simultaneous reboot of all
master nodes. This and other limitations are expected to be resolved before
self-hosting graduates from alpha.
...
instead. This may or may not be appropriate for your environment.
...
In summary, `kubeadm init --feature-gates=SelfHosting=true` works as follows:
 ...
 1. Kubeadm creates static pod YAML files in `/etc/kubernetes/manifests/`.
