...
or [Canal](https://github.com/tigera/canal/tree/master/k8s-install/kubeadm), solutions
that aren't compatible with this setting.
If you are using a compatible cloud provider or [flannel](https://github.com/coreos/flannel/blob/master/Documentation/kube-flannel.yml),
you can specify a subnet to use for each pod on the cluster with the `--pod-network-cidr` flag.
This should be a minimum of a /16 so that kubeadm is able to assign /24 subnets
to each node in the cluster. For flannel, you should set `--pod-network-cidr` to `10.244.0.0/16`
if you're using the flannel manifest linked above.
...
If you already have kubeadm installed and want to upgrade, run `apt-get update && apt-get upgrade` or `yum update` to get the latest version of kubeadm.
 ...
 - Second release between v1.4 and v1.5: `v1.5.0-alpha.2.384+9980291736556e`
 ...
 - Initial v1.4 release: `v1.5.0-alpha.0.1534+gcf7301f`
