...
1. kubeadm generates a token that additional nodes can use to register
   ...
   [kubeadm token](kubeadm-token.md).
...
This means you can, for example, copy an existing CA into `/etc/kubernetes/pki/ca.crt`
and `/etc/kubernetes/pki/ca.key` and kubeadm will use this CA for signing the rest 
of the certs.

Only for the CA certificate, it is possible to provide the `ca.crt` file but not the 
`ca.key` file; if all other certificates and kubeconfig files already are in place kubeadm
...
* `--bootstrap-kubeconfig=/etc/kubernetes/bootstrap-kubelet.conf` path to a kubeconfig 
   ...
   On success, a kubeconfig file r is written to the path specified by --kubeconfig. 
