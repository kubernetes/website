...
create selectively static pod file.
...
Use following command to add master Label and Taint to the node.
...
**Note:** You have to run all commands separately to replicate the `kubeadm init` behaviour. 
{: .note}
...
You can use this command to upload the configuration of your cluster, or use [kubeadm config](kubeadm-config.md) as well.
...
You can install all the available abbons with `all` sub command, or 
