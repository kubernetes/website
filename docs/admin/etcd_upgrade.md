...
1. The etcd cluster contains **only a single node**
1. For etcd clusters which contain multiple nodes, the upgrade proceedure requires shutting down the 
   etcd cluster during upgrade. During the time the etcd cluster is shutdown the Kubernetes API Server
   will be read only.

Deviations from the assumptions are untested by continuous integration and upgrades may create undesirable
consequences. Additional information about operating an etcd cluster is available 
[from the etcd maintainers](https://github.com/coreos/etcd/tree/master/Documentation).
