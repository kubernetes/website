...
| `--oidc-username-prefix` | Prefix prepended to username claims to prevent clashes with existing names (such as `system:` users). Setting the value `oidc:` would create usernames like `oidc:jane.doe` | `oidc:` | No |
| `--oidc-groups-claim` | JWT claim to use as the user's group. If the claim is present it must be an array of strings. | groups | No |
| `--oidc-groups-prefix` | Prefix prepended to group claims to prevent clashes with existing names (such as `system:` groups). Setting the value `oidc:` would create group names like `oidc:engineering` and `oidc:infra`. | `oidc:` | No |
| `--oidc-ca-file` | The path to the certificate for the CA that signed your identity provider's web certificate.  Defaults to the host's root CAs. | `/etc/kubernetes/ssl/kc-ca.pem` | No |

If `--oidc-username-prefix` isn't provided and `--oidc-username-claim` is set to
a claim other than `email`, the provider URL is used as a default prefix to prevent
clashes. For example, if the provider URL is `https://accounts.google.com` and the
username claim maps to `jane`, the plugin will authenticate the user as:

```
https://accounts.google.com#jane
```

Username prefixing can be disabled using the special value `-`.
