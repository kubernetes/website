...
can be dynamically created and managed.  There is a controller (TokenCleaner)
built into the Controller Manager that will delete tokens as they expire.
...
controller is enabled by enabling the `tokencleaner` controller on the
Controller Manager.  This is done via something like
