...
When bootstrapping, admin credentials should be placed in the `system:masters`
group, for example by creating a client cert with `/O=system:masters`. This
gives those credentials full access to the API and allows the admin to then
setup bindings for other users.
