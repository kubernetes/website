...
* [Create a Deployment to rollout a ReplicaSet. The ReplicaSet creates Pods in the background. Check the status of the rollout to see if it succeeds or not.](#creating-a-deployment)
* [Later, declare the new state of the Pods you want to run by updating the PodTemplateSpec of the Deployment. A new ReplicaSet is created and the Deployment manages moving the Pods from the old to the new ReplicaSet in a controlled rate. Each new ReplicaSet that is created, updates the revision of the Deployment.](#updating-a-deployment)
* [Rollback to an earlier Deployment revision if the current state of the Deployment will not be stable. Each rollback updates the revision of the Deployment.](#rolling-back-a-deployment)
* [Scale up the Deployment to facilitate more load.](#scaling-a-deployment)
* [Pause the Deployment to apply multiple fixes to its PodTemplateSpec and then resume it to start a new rollout.](#pausing-and-resuming-a-deployment)
* [Use the status of the Deployment as an indicator that a rollout has stuck](#deployment-status)
* [Clean up older ReplicaSets that you don't need anymore](#clean-up-policy)
...
controllers that have overlapping selectors, those controllers will fight with each other's and won't behave
correctly.
...
apply multiple fixes in between pausing and resuming without triggering unecessary rollouts.
...
For general information about working with config files, see [deploying applications](/docs/user-guide/deploying-applications),
[configuring containers](/docs/user-guide/configuring-containers), and [using kubectl to manage resources](/docs/user-guide/working-with-resources) documents.
...
`.spec.selector` is an optional field that specifies a [label selector](/docs/user-guide/labels/#label-selectors)
...
a Pod is considered ready, see [Container Probes](/docs/user-guide/pod-states/#container-probes).
...
Deployment will not trigger new rollouts as long as it is paused. Defaults to false, a Deployment is not paused
