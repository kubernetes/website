...
be down from `08:29:00` to `08:42:00`, the job will no start.
Set a longer `startingDeadlineSeconds` if starting later is better than not
starting at all.

The job is responsible for retrying pods, parallelism among pods it creates,
and determining the success or failure of the set of pods.
A cron job does not examine pods at all.
