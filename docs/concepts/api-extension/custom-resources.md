...
| Subresources | Add extra operations other than CRUD, such as "scale" or "exec". <br>Allows systems like like HorizontalPodAutoscaler and PodDisruptionBudget interact with your new resource. <br>Finer-grained access control: user writes spec section, controller writes status section. <br>Allows incrementing object Generation on custom resource data mutation (requires separate spec and status sections in the resource) | No but planned | Yes, any Subresource |
