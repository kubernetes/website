...
![Pre CCM Kube Arch](pre-ccm-arch.png)
...
![CCM Kube Arch](post-ccm-arch.png)
...
Work is in progress by the cloud provider working group (wg-cloud-provider) to enable PersistentVolume support using CCM - [kubernetes/kubernetes#52371](https://github.com/kubernetes/kubernetes/pull/52371).


Functions of CCM
------------------

CCM inherits its functions from cloud provider dependent components of Kubernetes. This section is structured based on the components from which CCM inherits its functions.
...
The Route Controller is responsible for configuring routes in the cloud appropriately so that containers on different nodes in the Kubernetes cluster can communicate with each other. The route controller is only applicable for GCE clusters.
...
PVL controller was created specifically for the CCM, i.e. it did not exist before CCM was created. This was done to move the PV labelling logic in kube-apiserver (it was an admission controller) to the CCM. It does not run on the KCM.

### 2. Kubelet

The Node Controller contains the cloud dependent functionality of Kubelet. Prior to the introduction of the CCM, the Kubelet was responsible for initializing a node with cloud specific details such as IP addresses, region/zone labels and instance type information. The introduction of CCM has moved this initialization operation from the Kubelet into the CCM. 
...
The Persistent Volume Labels controller moves the cloud dependent functionality of Kube APIServer to CCM as described in the above sections.
...
The implementation of the four shared controllers highlighted above and some scaffolding along with the shared cloudprovider interface will stay in core, but cloud provider-specific implementations will be built out of core, implementing the interfaces in core.
