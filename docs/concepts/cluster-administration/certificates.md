...
1.  Generate server certificate and key.
    ...
    after which the certificate will expire.
...
**cfssl** is another tool that can be used for certificate generation.
...
When using self-signed CA certificates rather than one signed by public third-party
CA authorities like VeriSign, a client node may refuse to recognize it as a valid
CA certificate. For a non-production deployment or one that runs behind a company
firewall, you can distribute the CA certificate to all clients and refresh the
local list for valid certificates.
