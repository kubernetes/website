...
description: >-
  ...
  Konsep Ingress memungkinkan Anda memetakan traffic ke backend yang berbeda berdasarkan aturan yang Anda definisikan
...
* Cluster: Sekumpulan Node yang menjalankan aplikasi terkontainerisasi yang dikelola oleh Kubernetes.
Untuk contoh ini, dan dalam sebagian besar deployment Kubernetes yang umum, node di dalam cluster tidak menjadi bagian dari jaringan cluster itu sendiri.
...
{{% tab name="Cluster" %}}
Ruang lingkup default untuk parameter IngressClass adalah seluruh cluster.Jika Anda mengatur field .spec.parameters dan tidak mengatur .spec.parameters.scope, atau jika Anda mengatur .spec.parameters.scope ke Cluster, maka IngressClass merujuk ke resource berskop cluster. Kind (dikombinasikan dengan apiGroup) dari parameter merujuk ke API berskop cluster (kemungkinan custom resource), dan name dari parameter mengidentifikasi resource berskop cluster tertentu untuk API tersebut.
For example:
...
Jika Anda mengatur *field* `.spec.parameters` dan mengatur `.spec.parameters.scope` ke `Namespace`, maka IngressClass merujuk ke *resource* berskop *namespace*. Anda juga harus mengatur *field* `namespace` di dalam `.spec.parameters` ke *namespace* yang memuat parameter yang ingin Anda gunakan.
...
Anda dapat menandai IngressClass tertentu sebagai *default* untuk *cluster* Anda. Mengatur anotasi `ingressclass.kubernetes.io/is-default-class` ke `true` pada *resource* IngressClass akan memastikan bahwa Ingress baru tanpa *field* `ingressClassName` yang ditentukan akan ditetapkan IngressClass *default* ini.

{{< caution >}}
Jika Anda memiliki lebih dari satu IngressClass yang ditandai sebagai *default* untuk *cluster* Anda, *admission controller* mencegah pembuatan objek Ingress baru yang tidak memiliki `ingressClassName` yang ditentukan. Anda dapat menyelesaikan masalah ini dengan memastikan bahwa paling banyak 1 IngressClass ditandai sebagai *default* di *cluster* Anda.
{{< /caution >}}
...
*Virtual host* berbasis nama mendukung pengalihan *traffic* HTTP ke beberapa nama *host* pada alamat IP yang sama.
...
Mengacu pada *secret* ini dalam Ingress memberi tahu *Ingress controller* untuk mengamankan saluran dari *client* ke *load balancer* menggunakan TLS. Anda perlu memastikan *secret* TLS yang Anda buat berasal dari sertifikat yang memuat *Common Name* (CN), juga dikenal sebagai *Fully Qualified Domain Name* (FQDN) untuk `https-example.foo.com`.
...
Teknik untuk menyebarkan traffic di seluruh domain kegagalan berbeda-beda antara penyedia layanan cloud.
