...
1. Buat sebuah Deployment yang menjalankan Redis:
    ...
    redis-leader-765d459796-258hz   1/1       Running   0          50s
    ```
    ...
    redis-leader-765d459796   1         1         1         1m
    ```
...
2. Buat sebuah Service untuk mengekspos Redis di jaringan:
    ...
    redis-leader   ClusterIP   10.0.0.213   <none>        6379/TCP   27s
    ```

3. Periksa apakah server Redis berjalan di Pod, dan mendengarkan porta 6379:

    ```shell
    # Ubah redis-leader-765d459796-258hz menjadi nama Pod
    kubectl get pod redis-leader-765d459796-258hz --template='{{(index (index .spec.containers 0).ports 0).containerPort}}{{"\n"}}'
    ```
...
1.  `kubectl port-forward` memungkinkan penggunaan nama sumber daya, seperti sebuah nama Pod, untuk memilih Pod yang sesuai untuk melakukan penerusan porta.
    ...
    ```shell
    # Ubah redis-leader-765d459796-258hz menjadi nama Pod
    kubectl port-forward redis-leader-765d459796-258hz 7000:6379
    ```

    yang sama seperti

    ```shell
    kubectl port-forward pods/redis-leader-765d459796-258hz 7000:6379
    ```

    atau

    ```shell
    kubectl port-forward deployment/redis-leader 7000:6379
    ```

    atau

    ```shell
    kubectl port-forward replicaset/redis-leader 7000:6379
    ```

    atau

    ```shell
    kubectl port-forward service/redis-leader 7000:6379
    ```
