...
feature:
  ...
  description: >
    Kubernetes führt schrittweise Änderungen an Ihrer Anwendung oder ihrer Konfiguration durch und überwacht gleichzeitig den Anwendungsstatus, um sicherzustellen, dass nicht alle Instanzen gleichzeitig gelöscht werden. Wenn etwas schief geht, macht Kubernetes die Änderung für Sie rückgängig. Nutzen Sie das wachsende Ökosystem von Implementierungslösungen.
...
Ein _Deployment_ Controller bietet deklarative Updates für [Pods](/docs/concepts/workloads/pods/pod/) und
[ReplicaSets](/docs/concepts/workloads/controllers/replicaset/).

Sie beschreiben einen _Wunschzustand_ in einem Deployment-Objekt, und der Deployment-Controller ändert den tatsächlichen Status mit einer kontrollierten Rate in den gewünschten Status. Sie können Deployments definieren, um neue ReplicaSets zu erstellen oder vorhandene Deployments zu entfernen und all ihre Ressourcen mit neuen Deployments zu übernehmen.

{{< note >}}
Sie sollten keine ReplicaSets verwalten, die einem Deployment gehören. Alle Anwendungsfälle sollten durch Manipulation des Deployment-Objekts abgedeckt werden. Erwägen Sie, ein Problem im Kubernetes-Hauptrepository zu öffnen, wenn Ihr Anwendungsfall im Folgenden nicht behandelt wird.
{{< /note >}}
...
Im Folgenden sind typische Anwendungsfälle für Deployments aufgeführt:

* [Ein Deployment zum Rollout eines ReplicaSet erstellen](#creating-a-deployment). Das ReplicaSet erstellt Pods im Hintergrund. Überprüfen Sie den Status des Rollouts, um zu sehen, ob er erfolgreich ist oder nicht.
* [Den neuen Zustand der Pods deklarieren](#updating-a-deployment) durch Aktualisieren der PodTemplateSpec des Deployments. Ein neues ReplicaSet wird erstellt, und das Deployment verwaltet das Verschieben der Pods vom alten ReplicaSet zum neuen mit einer kontrollierten Geschwindigkeit. Mit jedem neuen ReplicaSet wird die Version des Deployments aktualisiert.
* [Rollback auf eine frühere Version des Deployments](#rolling-back-a-deployment )wenn der aktuelle Stand des Deployments nicht stabil ist. Bei jedem Rollback wird die Version des Deployments aktualisiert.
* [Skalieren Sie das Deployment, um mehr Last zu bewerkstellingen](#scaling-a-deployment).
* [Das Deployment pausieren](#pausing-and-resuming-a-deployment) um mehrere Fixes auf PodTemplateSpec anzuwenden und dann fortzusetzen, um ein neues Rollout zu starten.
* [Verwenden Sie den Status des Deployments](#deployment-status) als Indikator dafür, dass ein Rollout festgefahren ist.
* [Bereinigen Sie ältere ReplicaSets](#clean-up-policy), die Sie nicht mehr benötigen.

## Erstellen eines Deployments

Das folgende Beispiel zeigt eine Bereitstellung. Es erstellt ein ReplicaSet, um drei 'nginx'-Pods aufzurufen:

{{< codenew file="controllers/nginx-deployment.yaml" >}}

In diesem Beispiel:

* Wird ein Deployment mit dem Namen `nginx-deployment` erstellt, das durch das Feld `.metadata.name` gekennzeichnet ist.
* Das Deployment erstellt drei replizierte Pods, die durch das Feld `replicas` gekennzeichnet sind.
* Das `selector`-Feld definiert, wie das Deployment herausfindet, welche Pods verwaltet werden sollen. In diesem Fall wählen Sie einfach ein Label aus, das in der Pod-Vorlage definiert ist (`app: nginx`). Allerdings sind komplexere Auswahlregeln möglich, solange die Pod-Vorlage selbst die Regel erfüllt.

  {{< note >}}
  `matchLabels` ist eine Karte von {key,value} Paaren. Ein einzelner {key,value} in der matchLabels-Map entspricht einem Element von `matchExpressions`, dessen Schlüsselfeld "key" ist, der Operator "In" ist und das Werte-Array nur "value" enthält. Die Anforderungen sind ANDiert.
  {{< /note >}}

* Das Feld `template` enthält die folgenden Unterfelder:
  * Die Pods tragen die Bezeichnung `app: nginx` im Feld `labels`.
  * Die Spezifikation der Pod-Vorlage oder das Feld `.template.spec` zeigt an, dass die Pods einen Container `nginx` ausführen, der das Image `nginx` [Docker Hub](https://hub.docker.com/) in der  Version 1.7.9. ausführt.

  * Erstellen Sie einen Container und benennen Sie ihn `nginx` über das `name`-Feld.
  * Führen Sie das `nginx`-Image in der Version 1.7.9 aus.
  * Öffnen Sie Port `80`, damit der Container Datenverkehr senden und annehmen kann.

Führen Sie den folgenden Befehl aus, um dieses Deployment zu erstellen:
...
{{< note >}}
Sie können das Flag `--record angeben`, um den Befehl zu schreiben, der in der Ressourcenanmerkung `kubernetes.io/change-cause` ausgeführt wird.
Es ist nützlich für die zukünftige Introspektion, z.B. um die Befehle zu sehen, die in jeder Deployment Revision ausgeführt werden.
{{< /note >}}

Führen Sie als nächstes `kubectl get deployments` aus. Die Ausgabe ähnelt der folgenden:
...
Wenn Sie die Deployments in Ihrem Cluster überprüfen, werden die folgenden Felder angezeigt:

* `NAME` listet die Namen der Bereitstellungen im Cluster auf.
* `DESIRED` zeigt die gewünschte Anzahl von _replicas_ der Anwendung an, die Sie beim Erstellen des Deployments definieren. Dies ist der _gewünschte Zustand_.
* `CURRENT` zeigt an, wie viele Replikate gerade ausgeführt werden.
* `UP-TO-DATE` zeigt die Anzahl der Replikate an, die aktualisiert wurden, um den gewünschten Status zu erreichen.
* `AVAILABLE` zeigt an, wie viele Replikate der Anwendung Ihren Benutzern zur Verfügung stehen.
* `AGE` zeigt die Zeitspanne an, in der die Anwendung ausgeführt wurde.

Beachten Sie, dass die Werte in den einzelnen Feldern den Werten in der Deployment-Spezifikation entsprechen:

* Die Anzahl der gewünschten Replikate ist 3 gemäß dem Feld `.spec.replicas`.
* Die Anzahl der aktuellen Replikate ist 0 gemäß dem Feld `.status.replicas`.
* Die Anzahl der aktuellen Replikate ist 0 gemäß dem Feld `.status.updatedReplicas`.
* Die Anzahl der verfügbaren Replikate ist 0 gemäß dem Feld `.status.availableReplicas`.

Um den Rollout-Status der Bereitstellung anzuzeigen, führen Sie "kubectl Rollout-Status deployment.v1.apps/nginx-deployment" aus. Dieser Befehl gibt die folgende Ausgabe zurück:
...
Führen Sie den Befehl `kubectl get deployments` einige Sekunden später erneut aus:
...
Beachten Sie, dass das Deployment alle drei Replikate erstellt hat und alle Replikate aktuell (sie enthalten die neueste Pod-Vorlage) und verfügbar sind (der Pod-Status ist mindestens für den Wert des Feldes `.spec.minReadySeconds` des Deployments bereit).

Um das durch die Bereitstellung erstellte ReplicaSet (`rs`) zu sehen, führen Sie `kubectl get rs` aus:
...
Beachten Sie, dass der Name des ReplicaSet immer als `[DEPLOYMENT-NAME]-[RANDOM-STRING]` formatiert ist. 
Die zufällige Zeichenfolge wird zufällig generiert und verwendet den Pod-Template-Hash als Startwert.

Führen Sie `kubectl get pods --show-labels` aus, um die für jeden Pod automatisch generierten Labels anzuzeigen. Die folgende Ausgabe wird zurückgegeben:
...
Das erstellte ReplicaSet stellt sicher, dass immer drei `nginx`-Pods laufen.

{{< note >}}
Sie müssen in einem Deployment einen entsprechenden Selektor und Pod-Template-Label angeben (in diesem Fall `app: nginx`). Überschneiden Sie keine Bezeichnungen oder Selektoren mit anderen Controllern (einschließlich anderer Deployments und StatefulSets). Kubernetes hindert Sie nicht daran, sich zu überschneiden, 
und wenn mehrere Controller überlappende Selektoren haben, können diese Controller Konflikte verursachen und sich unerwartet verhalten.
{{< /note >}}

### Pod-template-hash Label

{{< note >}}
Ändern Sie dieses Label nicht.
{{< /note >}}
