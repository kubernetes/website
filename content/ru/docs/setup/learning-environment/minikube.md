...
4.  Под (Pod) `hello-minikube` теперь запущен, но нужно подождать, пока он начнёт функционировать, прежде чем пытаться обращаться к нему.
  ...
  Если в столбце вывода `STATUS` выводиться `ContainerCreating`, значит под все еще создается:
...
#### Использование локальных образом путём повторного использования демона Docker
