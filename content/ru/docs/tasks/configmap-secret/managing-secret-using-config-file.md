...
Поле `stringData` существует для удобства и позволяет указывать те же данные
в виде незакодированных строк.
Ключи полей `data` и `stringData` должны состоять из буквенно-цифровых символов,
`-`, `_` или `.`.

В следующем примере в Secret с помощью поля `data` записываются две строки.
