...
1. `filter`: これらのプラグインは、スケジューリングポリシーにおけるPredicatesに相当し、Podを実行不可能なノードを除外するために使用されます。フィルターは設定された順序で呼び出されます。すべてのフィルターを通過するノードがない場合、Podはunschedulableとしてマークされます。
