...
バージョン1.28では、組み込みサイドカーコンテナの終了時の挙動が異なっていました。
サイドカーコンテナの動作を、その古い挙動に合わせることは推奨されていません。
ただし、起動順序のみが懸念事項である場合は、上記で述べた対象を、「フィーチャーゲートを有効化した状態で、バージョン1.28を実行しているノード」と読み替えることができます。
