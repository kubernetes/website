...
このページでは、kubeadmで作成したKubernetesクラスターをバージョン{{< skew currentVersionAddMinor -1 >}}.xから{{< skew currentVersion >}}.xに、または{{< skew currentVersion >}}.xから{{< skew currentVersion >}}.y(y > x)にアップグレードする方法を説明します。
...
1. アップグレードプランを確認します。
   ...
   {{< note >}}
   `kubeadm upgrade`はこのノードで管理している証明書の更新も自動で行います。
   証明書更新を無効にするには`--certificate-renewal=false`フラグを使用できます。
   証明書管理の詳細は[kubeadmによる証明書管理](/docs/tasks/administer-cluster/kubeadm/kubeadm-certs)を参照してください。
   {{</ note >}}
...
1. アップグレードするバージョンを選び、適切なコマンドを実行します。
   ...
   アドオンのアップグレードは、最後のコントロールプレーンインスタンスがアップグレードされた後に行われます。
   {{</ note >}}
