---
title: Ciliumを利用してネットワークポリシーの設定
content_type: task
weight: 30
---
...
Ciliumに簡単に慣れるには、[Cilium Kubernetes Getting Started Guide](https://docs.cilium.io/en/stable/gettingstarted/k8s-install-default/)に従って、minikube上でCiliumの基本的なDaemonSetインストールを行うことができます。
...
本番環境にCiliumをデプロイする際の詳細な手順については、次を参照してください。
...
Ciliumを使用してクラスターをデプロイすると、`kube-system` Namespaceに複数のPodが追加されます。このPodの一覧を確認するには、次を実行します。
...
ぜひお楽しみください。ご質問があれば、[Cilium Slackチャンネル](https://slack.cilium.io/)までお問い合わせください。
