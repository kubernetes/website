...
どの作業に取り組んでいるかにより、`etcdctl`ツールまたは`etcdutl`ツールが必要になります(両方必要になる場合もあります)。
...
`etcdutl`に関する詳細情報については、[etcdリカバリのドキュメント](https://etcd.io/docs/v3.5/op-guide/recovery/)を参照してください。
