...
`LimitRange`は、適用するデフォルト値の一貫性をチェック**しません**。これは、`LimitRange`によって設定された_制限_のデフォルト値が、クライアントがAPIサーバーに送信するspecでコンテナに指定された_リクエスト_値よりも小さい可能性があることを意味します。その場合、最終的なPodはスケジュール可能になりません。
