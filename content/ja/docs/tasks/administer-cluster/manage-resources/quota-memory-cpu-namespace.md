---
title: NamespaceのメモリおよびCPUクォータを設定する
content_type: task
weight: 50
description: >-
...
{{< note >}}
[インプレースPodのリサイズ](/docs/tasks/configure-pod-container/resize-container-resources/)を使用する際、ResourceQuotaの適用はリサイズ後の値に適用されます。
リサイズによって名前空間のクォータ制限を超過する場合、そのリサイズは拒否され、Podのリソースは変更されません。
{{< /note >}}
