...
コントローラーには以下が含まれます。
  ...
  * Jobコントローラー：単発タスクを表すJobオブジェクトを監視し、そのタスクを完了まで実行するPodを作成します。
  * エンドポイントスライスコントローラー：エンドポイントスライスオブジェクトを注入します(つまり、ServiceとPodを紐付けます)。
  * サービスアカウントコントローラー：新規の名前空間に対して、デフォルトサービスアカウントを作成します。
...
[クラスターレベルのロギング](/ja/docs/tasks/debug/debug-cluster/resource-usage-monitoring/)メカニズムは、コンテナのログを、検索／参照インターフェイスを備えた中央ログストアに保存します。
