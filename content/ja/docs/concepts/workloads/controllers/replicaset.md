...
ReplicaSetはローリングアップデートを直接サポートしないため、ユーザーのコントロール下においてPodを新しいspecにアップデートしたい場合は、[Deployment](/ja/docs/concepts/workloads/controllers/deployment/)を使用してください。
...
- メトリック値に基づいてアノテーションを更新するなど、頻繁に更新することは、APIサーバー上で大量のPodの更新操作が発生させることになるので、ユーザーは避けたほうがいいです。
{{< /note >}}
...
ReplicaSetは、Node上のいくつかのKubeletのようなエージェントに対して、ローカルのコンテナ再起動を移譲します。
