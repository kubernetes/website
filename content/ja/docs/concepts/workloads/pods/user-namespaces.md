...
このページでは、KubernetesのPodにおけるユーザー名前空間について説明します。
...
{{< note >}}
いくつかのOCIランタイムにおいては、LinuxのPodでユーザー名前空間を利用するのに必要なサポートが含まれていません。
マネージドKubernetesを利用している場合、あるいはOCIランタイムをパッケージとしてダウンロードしてセットアップした場合には、クラスタ内のノードがユーザー名前空間をサポートしない可能性があります。
{{< /note >}}
...
この分離によってホストで「起こせること」を制限でき、例えばコンテナ内プロセスのホストへのエスケープを処理する場合に、この制限が働きます。
...
コンテナブレークアウトのケースでは、コンテナがrootで稼働している場合にユーザー名前空間を使用していないと、ノードのroot権限が取得されます。
また、ケーパビリティがコンテナに付与されていた場合、そのケーパビリティはホスト上でも有効です。
ユーザー名前空間を利用している場合には、これらの事象は成立しません。
...
PodとホストのUID/GIDが重複していなければ、Podができることは限られているからです。
(PodのUID/GIDはホストのファイル所有者やグループと合致しません)。
...
* PodのUIDの範囲が開始するsubordinate UIDの起点が65536の倍数に設定されていて、かつ65536以上であること( **必須** )。
言い換えると、0-65535の範囲をPodのIDとして使うことはできません。
偶発的にインセキュアな設定がなされることを防ぐために、kubeletはこの制約を強制します。

* subordinate ID の個数が65536の倍数であること(必須)。

* subordinate IDの個数は最低でも`65536 x <最大Pod数>`であること(必須)。
`<最大Pod数>`
  はノードで稼働できるPodの数の最大値を示します。

* ユーザーIDとグループIDとして同じ個数を割り当てること(必須)。
他のユーザーがグループIDの範囲と合致しないユーザーID範囲を利用することは問題ありません。

* 割り当てられたUID/GID範囲は他の割当と重複しないこと(推奨)。

* subordinateの設定が単一行でなされること(必須)。
複数のUID/GID範囲を定義することはできません。
...
#   name:firstID:count
# この例における意味
# - firstID は 65536 (とりうる最小の値)
# - count は 110 (デフォルトの制限値) * 65536
...
フィーチャーゲートを有効化した上でユーザー名前空間を使うPodを作成する場合、_Baseline_ ないしは _Restricted_ Podセキュリティ基準のセキュリティコンテキストが強制されていても、以下のフィールドによる制約がなされません。
