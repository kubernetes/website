...
Conditionは、オブジェクトが表す対象の実際の状態の一部を示すマーカーです。
...
これらのフェーズが完了すると、kubeletは({{< glossary_tooltip text="Container Runtime Interface (CRI)" term_id="cri" >}}を使用して)コンテナランタイムと連携し、ランタイムサンドボックスをセットアップしてPodのネットワークを構成します。
...
- [コンテナのプローブ](/docs/concepts/configuration/liveness-readiness-startup-probes/)と、それらがPodのReadinessにどう影響するかについて学ぶ。
