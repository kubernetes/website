...
継続的に実行され、複製されたアプリケーションができたので、ネットワーク上で公開できます。
ネットワーキングに対するKubernetesのアプローチについて説明する前に、ネットワーキングをDockerと連携させる「通常の」方法と比較する価値があります。
...
Lクラウドロードバランサーを使用するようにサービスを再作成しましょう。
