...
4. ノードが応答しなくなった場合、ノードがクラウドから削除されているかを確認する。クラウドからノードが削除されていた場合、KubernetesからNode objectを削除する
...
ルートコントローラーは、Node objectの作成を待ち受け、ルートを適切に設定します。Node objectについて、get権限が必要です。
...
サービスコントローラーは、Service objectの作成、更新、削除イベントを待ち受け、その後、サービスのエンドポイントを適切に設定します。
