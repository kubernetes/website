...
* [Windows Serverを実行するマスターノードとワーカーノード](/docs/tasks/administer-cluster/kubeadm/adding-windows-nodes)を含むKubernetesクラスターを作成します
* Kubernetes上にServiceとワークロードを作成してデプロイすることは、LinuxコンテナとWindowsコンテナ共に、ほぼ同じように動作することに注意してください。クラスターとのインターフェースとなる[Kubectlコマンド](/docs/reference/kubectl/)も同じです。Windowsコンテナをすぐに体験できる例を以下セクションに用意しています。
...
1. デプロイが成功したことを確認します。検証するために行うこと:
    ...
    * サービスディスカバリ、Kubernetesの[default DNS suffix](/docs/concepts/services-networking/dns-pod-service/#services)と共にService名に`curl`します
