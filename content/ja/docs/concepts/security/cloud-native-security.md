...
1. 内部の脅威であっても、攻撃対象となる範囲を最小限に抑える[ゼロトラスト](https://glossary.cncf.io/zero-trust-architecture/)のようなアーキテクチャを採用する。
...
1. ソフトウェアのディストリビューションが、ソフトウェアのソースに対する信頼チェーンを使用して、転送中の暗号化を行うようにします。
1. 利用可能になった更新に対応するための依存関係の更新プロセスを採用し、それに従います。
1. サプライチェーンを保証するために、デジタル証明書などの検証メカニズムを使用します。
1. セキュリティリスクを通知するためのフィードや他のメカニズムにをサブスクライブします。
...
これらの2つの側面、分離と集約は、ランタイムセキュリティとのトレードオフがあり、適切なバランスを見つける必要あることを意味します。
...
1. コンテナ化されたワークロードを実行するために、特別に設計されたオペレーティングシステムをノード上で実行します。
   ...
   コンテナ固有のオペレーティングシステムは、システムコンポーネントを分離し、コンテナエスケープの攻撃対象領域を減らすのに役立ちます。
