...
大規模ワークロードのスケジューリングを効率的に行うために適応されたカスタムスケジューラーは数多く存在しますが、特にユースケースが増加しているAI時代においては、こうしたワークロードのスケジューリングがKubernetesユーザーにとってどれほど一般的で重要であるかを考えると、大規模ワークロードのスケジューリングを `kube-scheduler` がネイティブにサポートする時が来ています。
...
スケジューラー内で Opportunistic に動作し、同一のスケジューリング要件（コンテナイメージ、リソース要求、アフィニティなど）を持つPodを識別します。
