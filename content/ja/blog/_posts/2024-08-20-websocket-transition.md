...
Kubernetesは、発信元のHTTPリクエストに特定のアップグレードヘッダーを追加することで、HTTP接続をストリーミング接続にアップグレードします。
...
この対話シェルにおけるSTDIN、STDOUT、STDERRデータ(ターミナルリサイズデータやプロセス終了コードデータも含む)は、このアップグレードされた接続を通じてストリーミングされます。
