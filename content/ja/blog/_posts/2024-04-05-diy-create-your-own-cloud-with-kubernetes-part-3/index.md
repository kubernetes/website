...
Cluster API内には、いくつかのタイプのプロバイダーがあります。
主なものは次のとおりです。
...
- **安定性**: 複雑な多層デプロイメント方式を排除することで、アーキテクチャを簡素化できます。
...
これは、物理サーバーか仮想マシンかにかかわらず、ノードのグループを記述するものです。
...
そうすると、インストールは直接そのクラスターに対して実行されます。
...
Cloud Controller Manager(CCM)の主な役割は、Kubernetes をクラウドインフラストラクチャプロバイダーの環境(この場合は、テナントKubernetesのすべてのワーカーがプロビジョニングされている管理Kubernetesクラスター)と統合することです。
CCMが実行するタスクは次のとおりです。
...
このコンポーネントは、ポッドへのボリュームのマウントとマウント解除を担当し、クラスターノードで直接発生するプロセスとの緊密な統合が必要です。

KubeVirt CSIドライバーは、ボリュームの注文のためのプロキシとして機能します。
