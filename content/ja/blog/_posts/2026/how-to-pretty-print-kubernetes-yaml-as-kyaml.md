---
layout: blog
title: "Kubernetes YAMLをKYAMLとして(出力を)見やすく整形する方法、そしてなぜそうしたいのか"
...
標準YAMLでは、`NO`は文字列型の`"NO"`ではなく、真偽値の`false`として解釈されます。
...
YAMLはこれを、多くの人が使う従来の**ブロックスタイル**とは対照的に、**フロースタイル**と呼びます。
...
## YAMLをKYAMLとして(出力を)見やすく整形するには {#how-to-pretty-print-yaml-as-kyaml}
...
```bash
# Kubernetes 1.35+ (ベータ; デフォルトで機能が有効になっているが、まだ-o kyaml CLIパラメーターは必要とする)
