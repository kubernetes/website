...
そのうち、16件がGAに昇格し、23件がベータに昇格し、27件がアルファとして導入され、1件が非推奨化/削除されます。

## リリースのテーマとロゴ {#release-theme-and-logo}

{{< figure src="k8s-v1.37.svg" alt="Kubernetes v1.37 ガルワールのロゴ: リングアル(ringaal)に着想を得た編み込みのフレームが、雪を頂いたヒマラヤの峰々、段々畑、デオダーの木々、曲がりくねった川、1.37と記された山小屋、色とりどりの旗、ヒマラヤモナル、そして中心にKubernetesの舵輪シンボルを持つ赤いシャクナゲ(buransh)の花を取り囲んでいる" class="release-logo" >}}
...
一部のPodがスケジューリングされる一方で、残りはリソース不足のためPendingのままになる、という事態が起こりえます。
