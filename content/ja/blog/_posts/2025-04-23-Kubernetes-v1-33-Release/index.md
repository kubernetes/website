...
そのうち18がGAに昇格し、20がベータ版に移行し、24がアルファ版に導入され、2つが非推奨または撤回されています。
...
これはv1.27でアルファ版としてリリースされ、v1.33でベータ版に昇格しました。
...
v1.24でベータ版としてリリースされた後、_Volume Populators_はv1.33でGAに昇格しました。
...
_これはv1.33リリース後にベータ版となった改善点の一部です。_
...
v1.14で最初に導入されたDSRのサポートは、[KEP-5100: Support for Direct Service Return (DSR) and overlay networking in Windows kube-proxy](https://kep.k8s.io/5100)の一環としてSIG Windowsによりベータ版に昇格しました。
...
v1.32で導入されたDRAによるネットワークインターフェースデータの標準化された報告がv1.33でベータ版に昇格しました。
...
v1.32でアルファ版として導入された非同期プリエンプションがv1.33でベータ版に昇格しました。
...
X.509トラストアンカー(ルート証明書)を保持するために設計されたクラスタースコープリソースであるClusterTrustBundleがv1.33でベータ版に昇格しました。
...
v1.31で導入されたこの機能はv1.33でベータ版に昇格し、現在はデフォルトで有効になっています。
...
v1.31で導入されたPodでOpen Container Initiative(OCI)イメージをボリュームとして使用する機能のサポートがベータ版に昇格しました。
...
このKEPは2016年後半に最初に提案され、複数の改訂を経て、v1.25でアルファリリース、v1.30で初期ベータ(デフォルトでは無効)となり、v1.33の一部としてデフォルトで有効なベータ版に移行しました。
...
v1.12でアルファ版として導入され、v1.31でデフォルト無効のベータ版だった`procMount`オプションが、v1.33でデフォルト有効のベータ版に移行しました。
...
_これはv1.33リリース後にアルファ版となった改善点の一部です。_
...
Kubernetes v1.33で(アルファ版の)フィーチャーゲートである`ContainerStopSignals`を有効にすることで、Pod仕様内で直接カスタム停止シグナルを定義できるようになりました。
...
以下はv1.33で導入されたすべてのアルファ版のDRAのフィーチャーゲートです:
...
元の実装はv1.26でアルファ版として導入されましたが、予期せぬ`containerd`の挙動に直面し、代替ソリューションが存在したため、Kubernetesプロジェクトは関連するKEPを取り下げることを決定しました。
