...
특정 타입(`bootstrap.kubernetes.io/token`)의 시크릿(Secret)으로 정의된다.
...
인증에 사용되는 시크릿을 노출하지 않고 토큰을 참조할 때 사용한다.
...
각각의 유효한 토큰은 `kube-system` 네임스페이스의 시크릿에 의해 뒷받침된다.
전체 설계 문서는
...
사용자는 JWS 규칙(base64로 인코딩한 뒤 끝에 붙는 `=` 를 제거)에 따라
...
손상된 클라이언트가 서명에 의존해 TLS 신뢰를 부트스트랩하는
다른 클라이언트에 중간자 공격(man-in-the-middle)을 수행할 수 있기 때문이다.
{{< /warning >}}
