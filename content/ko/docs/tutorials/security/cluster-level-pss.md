---
title: 파드 시큐리티 스탠다드를 클러스터 수준에서 적용하기
...
파드 시큐리티(Pod Security)는 새로운 파드가 생성될 때 쿠버네티스
[파드 시큐리티 스탠다드(Pod Security Standards)](/docs/concepts/security/pod-security-standards/)에 대한 검사를
수행하는 어드미션 컨트롤러이다. v1.25에서 GA된 기능이다.
이 튜토리얼에서는 `baseline` 파드 시큐리티 
스탠다드를 클러스터 수준에서 강제(enforce)하여 클러스터의 모든 네임스페이스에 표준 구성을
적용하는 방법을 보여준다.

파드 시큐리티 스탠다드를 특정 네임스페이스에 적용하려면, 
[파드 시큐리티 스탠다드를 네임스페이스 수준에 적용하기](/docs/tutorials/security/ns-level-pss/)를 참고한다.

실행 중인 쿠버네티스 버전이 v{{< skew currentVersion >}}이 아니라면,
해당 버전의 문서를 확인한다.
...
내장 [파드 시큐리티 스탠다드](/docs/concepts/security/pod-security-standards/)를
`enforce`, `audit`, `warn` 모드로 적용할 수 있다.
...
그러나 `baseline` 및 `restricted` 스탠다드에서는
특히 `kube-system` 네임스페이스에서 경고가 발생한다.

## 모드, 버전 및 스탠다드 설정하기

이 섹션에서는, 다음 파드 시큐리티 스탠다드를 `latest` 버전으로 적용한다.

* `baseline` 스탠다드는 `enforce` 모드로 적용
* `restricted` 스탠다드는 `warn` 및 `audit` 모드로 적용
...
1. API 서버가 클러스터 생성 과정에서 이 파일을 사용하도록 구성한다.
   ...
   - role: control-plane
     ...
     - hostPath: /tmp/pss
       ...
       # see https://kubernetes.io/docs/concepts/storage/volumes/#마운트-전파-propagation
       # default None
       propagation: None
   EOF
   ```
