...
설치 안내는 [minikube start](https://minikube.sigs.k8s.io/docs/start/)를 참고한다.
{{< /note >}}
...
이 튜토리얼은 이미 `minikube`를 설정했다고 가정한다.
설치 방법은 [minikube start](https://minikube.sigs.k8s.io/docs/start/)의 __Step 1, Installation__ 을 참고한다.
{{< note >}}
...
{{< note >}}
`dashboard` 명령어는 대시보드 추가 기능을 활성화하고 기본 웹 브라우저에서 프록시를 연다.
대시보드에서 Deployment, Service와 같은 쿠버네티스 리소스를 생성할 수 있다.

브라우저를 터미널에서 직접 호출하지 않고 웹 대시보드에 접속하기 위한 URL을 얻는 방법을 알아보려면, "경로 복사 및 붙여넣기" 탭을 참고한다.
...
{{% /tab %}}
{{% tab name="경로 복사 및 붙여넣기" %}}

minikube가 자동으로 웹 브라우저를 열지 않게 하려면, `dashboard` 서브커맨드를 
`--url` 플래그와 함께 실행하면 된다. `minikube`가 선호하는 브라우저에서 열 수 있는 URL을 출력한다.
...
6. 파드 내 컨테이너의 애플리케이션 로그를 확인한다 (파드 이름을 `kubectl get pods` 명령어로 얻은 이름으로 교체한다).
    ...
    {{< note >}}    
    `kubectl logs` 명령어의 `hello-node-5f76cf6ccf-br9b5`를 `kubectl get pods` 명령어 출력에서 얻은 파드 이름으로 교체한다.
    {{< /note >}}
...
{{< warning >}}
agnhost 컨테이너에는 디버깅에 유용한 `/shell` 엔드포인트가 있지만, 이는 공용 인터넷에 노출하기에 위험하다. 이 컨테이너를 인터넷에 연결된 클러스터나 프로덕션 클러스터에서 실행하지 않는다.
{{< /warning >}}
...
3. 다음 명령어를 실행한다
    ...
    ```
    사용 중인 앱을 띄우고 앱의 응답을 보여주는 브라우저 창이 열린다.
...
만약 minikube를 다시 사용하여 쿠버네티스에 대해 더 배우고 싶다면, 삭제할 필요는 없다.
