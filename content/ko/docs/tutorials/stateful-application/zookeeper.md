...
각 노드에 최소 2개 CPU에 4 GiB 메모리를 갖는 노드가 최소한 4개인 클러스터가 필요하다. 이 튜토리얼에서 클러스터 노드를 통제(cordon)하고 비우게(drain) 할 것이다. **이것은 클러스터를 종료하고 노드에서 모든 파드를 퇴출(evict)하는 것이요 모든 파드는 임시적으로 언스케쥴된다는 의미이다.** 이 튜토리얼을 위해 전용 클러스터를 이용하거나 다른 테넌트에 간섭을 주지 않는 것는 혼란이 발생하지 않도록 해야합니다.


이 튜토리얼은 클러스터가 동적으로 퍼시스턴트볼륨을 프로비저닝하도록 설정했음을 가정한다.
그렇게 설정되어 있지 않다면
...
이는 `zk-hs` 헤듸스 서비스, `zk-cs` 서비스,
`zk-pdb` 포드디스룹션버짓과 `zk` 스테이트풀셋을 생성한다.
...
[`kubectl get`](/docs/reference/generated/kubectl/kubectl-commands/#get) 사용하여
스테이트풀셋 컨트롤러가 스테이트풀셋 파드를 생성하는지 지켜 보자.

```shell
kubectl get pods -w -l app=zk
```

`zk-2` 파드가 Running and Ready 상태가 되면, `CTRL-C`를 눌러 kubectl 을 종료하자.
...
익명 네트워크에서 리더 선출을 위한 종료 알고리즘이 없기에 Zab은 리더 선출을 위해 명시적인 맴버 구성을 해야 한다. 앙상블의 각 서버는 고유 식별자를 가지고 모든 서버는 식별자 전역 집합을 알아야 하며 각 식별자는 네트워크 주소에 연관되어야 한다.
...
스테이트풀셋 컨트롤러는 각 순번 인덱스에 기초한 각 파드에 고유한 호스트네임을 부여한다. 
각 호스트네임은 `<스테이트풀셋 이름>-<순번 인덱스>` 형식을 취한다.
`zk` 스테이트풀셋의 `replicas` 필드는 `3`으로 설정되었기 때문에 그 스테이트풀셋 컨트롤러는 3개의 파드의 호스트네임을 `zk-0`, `zk-1`, `zk-2`로 정한다.
...
`zk` 스테이트풀셋에 각 포드의 FQDN을 얻기 위해 다음 명령어를 이용하자.
```shell
...
`zk-0`이 완전히 종료되면 `CTRL-C`를 이용해 kubectl 을 종료하자.
...
`zookeeper.yaml` 메니페스트를 다시 적용하자.

```shell
kubectl apply -f https://k8s.io/examples/application/zookeeper/zookeeper.yaml
```

`zk` 스테이트풀셋 오브젝트를 생성하지만 메니페스트에 다른 API 오브젝트는 이미 존재하므로 수정되지 않는다.

스테이트풀셋 컨트롤러가 스테트풀셋의 파드를 재생성하는 것을 지켜보자.

```shell
kubectl get pods -w -l app=zk
```

`zk-2` 파드가 Running과 Ready가 되면 `CTRL-C`를 이용하여 kubectl 을 종료한다.
...
`zk` 스테이트풀셋의 `spec`에 `volumeClaimTemplates` 필드는 각 파드에 프로비전될 퍼시스턴트 볼륨을 지정한다.
...
`스테이트풀셋`의 파드를 재생성할 때에 파드의 퍼시스턴트볼륨도 재마운트한다.
...
[Elasticsearch 와 Kibana](/docs/tasks/debug-application-cluster/logging-elasticsearch-kibana/)를 지원한다.
...
감독 프로세스로 외부 유틸리티를 사용하는 대신 쿠버네티스를 귀하의 애플리케이션의
와치독으로서 사용해야 한다.

### 앙상블 관리하기

`zk` `스테이트풀셋`은 `RollingUpdate` 업데이트 전략을 이용하도록 구성했다.

`kubectl patch`로 서버에 할당된 `cpu` 수량을 갱신할 수 있다.
...
이것은 파드를 순번에 역순으로 한번에 하나씩 종료시키고, 새 구성으로 재생성한다. 이는 롤링업데이트 동안에 쿼럼을 유지하도록 보장한다.
...
[재시작정책](/docs/user-guide/pod-states/#restartpolicy)은
쿠버네티스가 파드 내에 컨테이너의 진입점에서 프로세스 실패를 어떻게 다루는지 제어한다.
`스테이트풀셋`의 파드에서 오직 적절한 `재시작정책`는 Always 이며
...
다른 터미널에서 다음 명령어로 `zk` `스테이트풀셋`의 파드를 지켜보자.
...
애플리케이션 프로세가 건강하지 않으며 재시작해야 한다고
쿠버네티스에게 알리도록 활성도검사를 이용해야 한다.

`zk` `스테이트풀셋`에 파드 `template`에 활성도검사를 지정했다.
``
...
`zk` `스테이트풀셋`에 모든 파든ㄴ 다른 노드에 배포되었다.
...
이는 `zk` `스테이트풀셋`의 파드에 `파드안티어피니티`를 지정했기 때문이다.
...
[`kubectl cordon`](/docs/reference/generated/kubectl/kubectl-commands/#cordon)을 이용하여 클러스터 내에 4개 노드를 제외하고 다른 모든 노드를 통제해보자.

```shell
kubectl cordon <node-name>
```

`zk-pdb` `포드디스룹션버짓`을 살펴보고자 이 명령어를 이용하자.

```shell
kubectl get pdb zk-pdb
```

`max-unavailable` 필드는 쿠버네티스가 `zk` `스테이트풀셋`의 많아야 1개의 파드는
...
클러스터에 4 개 노드가 있기 때문에 `kubectl drain` 은 성공했고
`zk-0`을 다른 노드로 재스케쥴링했다.
...
서비스는 여전히 가용한데 이는 `파드디스룹션버짓`이 존중되기 떄문이다.
...
`zk-1` 은 이 노드에서 재스케쥴된다. `zk-1`이 Running과 Ready가 될때까지 기다리자.
...
`zk-2`가 재스케쥴되도록 두번째 노드의 통제를 풀어보자.
...
`kubectl drain`을 `포드디스룹션버짓`과 결합하면 유지보수중에도 서비스를 가용하게 할 수 있다. drain으로 노드를 통제하고 유지보수를 위해 노드를 오프라인하기 전에 파드를 추출하기 위해 사용한다면 서비스는 혼란 예산을 표기한 서비스는 그 예산이 존중은 존중될 것이다. 파드가 즉각적으로 재스케쥴 할 수 있도록 항상 중요 서비스를 위한 추가 용량을 할당해야 한다.
{{% /capture %}}
