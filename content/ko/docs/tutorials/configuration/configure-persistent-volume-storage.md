...
아직 단일 노드 클러스터를 가지고 있지 않다면, [Minikube](https://minikube.sigs.k8s.io/docs/)를
사용하여 클러스터 하나를 생성할 수 있다.

* [퍼시스턴트볼륨](/ko/docs/concepts/storage/persistent-volumes/)의
관련 자료에 익숙해지도록 한다.
...
Google Compute Engine 영구 디스크, NFS 공유 또는 Amazon Elastic
...
지정한다. 또한 설정에서 볼륨 크기를 10 기비바이트로 지정하고 단일 노드가
읽기-쓰기 모드로 볼륨을 마운트할 수 있는 `ReadWriteOnce` 접근 모드를 지정한다. 여기서는
퍼시스턴트볼륨의 [스토리지클래스 이름](/ko/docs/concepts/storage/persistent-volumes/#클래스)을
`manual` 로 정의하며, 이는 퍼시스턴트볼륨클레임의 요청을
이 퍼시스턴트볼륨에 바인딩하는 데 사용한다.

{{< note >}}
이 예제에서는 간단하게 설명하기 위해 `ReadWriteOnce` 접근 모드를 사용한다. 운영
환경에서는 쿠버네티스 프로젝트가 그 대신 `ReadWriteOncePod` 접근 모드를 사용할 것을
권장한다.
{{< /note >}}
...
사용하여 물리적인 스토리지를 요청한다. 이 예제에서, 사용자는 한 번에 최대
하나의 노드에 대해 읽기-쓰기 접근을 지원하며 최소 3 기비바이트의 볼륨을 요청하는
퍼시스턴트볼륨클레임을 생성한다.
...
퍼시스턴트볼륨을 지정하지는 않는다는 것을 유념하자. 파드의 관점에서 볼 때,
클레임은 볼륨이다.
...
퍼시스턴트볼륨과 퍼시스턴트볼륨클레임을 생성하고 볼륨을 컨테이너의 한 경로에
마운트하는 방법을 알아보았다. 이제 동일한 퍼시스턴트볼륨을 컨테이너의 서로 다른
두 경로에 마운트하는 방법을 살펴보자. 다음은 그 예시이다.

{{% code_sample file="pods/storage/pv-duplicate.yaml" %}}

여기서 각 필드의 의미는 다음과 같다.

- `subPath`: 마운트한 퍼시스턴트볼륨의 특정 파일이나 디렉터리를 컨테이너의
  서로 다른 경로에 노출할 수 있게 한다. 이 예제에서는 다음과 같이 사용한다.
  - `subPath: html`은 `html` 디렉터리를 마운트한다.
  - `subPath: nginx.conf`는 `nginx.conf` 파일 하나를 마운트한다.

첫 번째 `subPath`가 `html`이므로 노드의 `/mnt/data/` 안에 `html` 디렉터리를
생성해야 한다.

두 번째 `subPath`인 `nginx.conf`는 `/mnt/data/` 디렉터리 안의 파일을 사용한다는
의미이다. 다른 디렉터리를 만들 필요는 없다.

nginx 컨테이너에는 다음과 같이 두 개의 볼륨 마운트가 생성된다.

- 정적 웹사이트를 위한 `/usr/share/nginx/html`
- 기본 구성 파일을 위한 `/etc/nginx/nginx.conf`

### 노드의 index.html 파일을 새 디렉터리로 옮기기

여기서 `index.html` 파일은 [사용자 노드에 index.html 파일 생성하기](#사용자-노드에-indexhtml-파일-생성하기) 절에서 생성한 파일을 의미한다.

클러스터의 단일 노드에 연결되는 셸을 연다. 셸을 여는 방법은 클러스터 설정에 따라 달라진다.
예를 들어 Minikube를 사용하는 경우 `minikube ssh`를 입력하여 노드에 연결되는 셸을 열 수 있다.

`/mnt/data/html` 디렉터리를 생성한다.
...
`index.html`을 해당 디렉터리로 옮긴다.

```shell
# index.html을 현재 위치에서 html 하위 디렉터리로 옮긴다
sudo mv /mnt/data/index.html /mnt/data/html/index.html
```

### 새 nginx.conf 파일 생성하기

{{% code_sample file="pods/storage/nginx.conf" %}}

이 파일은 기본 `nginx.conf` 파일을 수정한 버전이다. 기본 `keepalive_timeout` 값을
`60`으로 변경했다.
...
여기서는 기존 퍼시스턴트볼륨과 퍼시스턴트볼륨클레임을 사용하는 파드를 생성한다.
다만 이 파드는 `nginx.conf` 파일과 `html` 디렉터리만 컨테이너에 마운트한다.
...
파드에서 실행 중인 컨테이너에 셸로 접근한다.

```shell
kubectl exec -it test -- /bin/bash
```

셸에서 nginx가 hostPath 볼륨의 `index.html` 파일을
제공하는지 확인한다.

```shell
# 이전 단계에서 "kubectl exec"를 실행하여 연 루트 셸 안에서
# 다음 세 명령을 실행해야 한다
apt update
apt install curl
curl http://localhost/
...
출력에는 사용자가 hostPath 볼륨의 `index.html` 파일에 작성한 텍스트가
표시된다.
...
셸에서 nginx가 hostPath 볼륨의 `nginx.conf` 파일을 사용하는지도
확인한다.
...
출력에는 사용자가 hostPath 볼륨의 `nginx.conf` 파일에서 수정한 내용이
표시된다.
...
이 메시지들이 표시되면 퍼시스턴트볼륨클레임의 스토리지에서 특정 파일과 디렉터리를
사용하도록 파드를 성공적으로 구성한 것이다.
...
클러스터 노드에 대한 셸이 열려 있지 않다면
이전과 같은 방법으로 새 셸을 연다.

노드의 셸에서 생성한 파일과 디렉터리를 제거한다.
...
퍼시스턴트볼륨에 GID로 어노테이션을 달 수 있다. 그 뒤에, 퍼시스턴트볼륨을
...
파드의 모든 컨테이너에 적용된다. 퍼시스턴트볼륨의
어노테이션에서 왔든 파드의 명세에서 왔든, 모든 GID는 각 컨테이너에서 실행되는 첫 번째
프로세스에 적용된다.
