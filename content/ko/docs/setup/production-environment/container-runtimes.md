...
기본적으로 리눅스 커널은 인터페이스 간에 IPv4 패킷이 라우팅되는 것을 허용하지 않는다.
대부분의 쿠버네티스 클러스터 네트워킹 구현은 이 설정을 필요할 때 변경하지만,
일부 구현은 관리자가 직접 설정해 주기를 기대할 수 있다.
(일부 구현은 다른 sysctl 파라미터 설정이나 커널 모듈 로드를 기대할 수도 있으므로,
사용 중인 네트워크 구현의 문서를 확인한다.)
...
{{< note >}}
v1.22 이후부터는 kubeadm으로 클러스터를 생성할 때 사용자가
`KubeletConfiguration` 아래의 `cgroupDriver` 필드를 설정하지 않으면,
kubeadm이 기본값으로 `systemd`를 사용한다.
{{< /note >}}
...
컨테이너 런타임이 `RuntimeConfig` CRI RPC를 지원하면,
kubelet은 런타임으로부터 적절한 cgroup 드라이버를 자동으로 감지하고
kubelet 설정 안의 `cgroupDriver` 값을 무시한다.

하지만 오래된 컨테이너 런타임 버전(특히 containerd 1.y 이하)은
`RuntimeConfig` CRI RPC를 지원하지 않으며,
이 질의에 올바르게 응답하지 못할 수 있다. 이 경우 kubelet은
자체 `--cgroup-driver` 플래그 값을 사용하도록 되돌아간다.

쿠버네티스 1.36에서는 이 폴백 동작이 제거되며,
오래된 containerd 버전은 최신 kubelet과 함께 동작하지 않게 된다.
...
CRI API의 v1만 지원한다. 컨테이너 런타임이 v1 API를 지원하지 않으면,
...
초기 클러스터 설치 후 또는 CNI 설치 후 컨테이너 크래시 루프가 발생한다면,
...
쿠버네티스 v1.28에서는 cgroup 드라이버 자동 감지를 알파 기능으로 활성화할 수 있다.
자세한 내용은 [systemd cgroup 드라이버](#systemd-cgroup-driver)를 참조한다.
...
2. [`cri-dockerd`](https://mirantis.github.io/cri-dockerd/usage/install)의 설치 문서에 따라
  `cri-dockerd`를 설치한다.
