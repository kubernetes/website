...
{{< note >}}
v1.22 이후부터는 kubeadm으로 클러스터를 생성할 때 사용자가 `KubeletConfiguration` 아래의
...
컨테이너 런타임이 `RuntimeConfig` CRI RPC를 지원하면, kubelet은 런타임으로부터
...
2. [`cri-dockerd`](https://mirantis.github.io/cri-dockerd/usage/install)의 설치 문서에 따라 `cri-dockerd`를 설치한다.
