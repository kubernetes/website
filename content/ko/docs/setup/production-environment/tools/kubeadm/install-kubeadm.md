...
3. 적절한 쿠버네티스 `apt` 리포지터리를 추가한다. 이 저장소에는 오직 {{< skew currentVersion >}} 버전에 대한 패키지만 있음에 유의한다. 다른 쿠버네티스 마이너 버전을 설치하려면 URL에서 쿠버네티스 마이너 버전을 원하는 버전으로 변경해야 한다 (설치하려는 쿠버네티스 버전에 맞는 문서를 읽고있는지도 확인해야 한다).
...
2. 쿠버네티스 `yum` 리포지터리를 추가한다. 쿠버네티스를 업그레이드 하려면 특별한 절차가 필요하기 때문에,
   ...
   다른 쿠버네티스 마이너 버전을 설치하려면 URL에서 쿠버네티스 마이너 버전을 원하는 버전으로 변경해야 한다 (설치하려는 쿠버네티스 버전에 맞는 문서를 읽고있는지도 확인해야 한다).
