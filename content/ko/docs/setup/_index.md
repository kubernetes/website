...
{{< glossary_tooltip text="kube-apiserver" term_id="kube-apiserver" >}}나 {{< glossary_tooltip text="kube-proxy" term_id="kube-proxy" >}}와 같은 몇몇 [쿠버네티스 컴포넌트](/docs/concepts/overview/components/)들은
...
쿠버네티스 클러스터를 직접 관리하고 싶지 않다면, [인증된 플랫폼](/docs/setup/production-environment/turnkey-solutions/)과
...
[학습 환경](/docs/setup/learning-environment/)을 참고한다.

## 프로덕션 환경

[프로덕션 환경](/docs/setup/production-environment/)을 위한
...
[kubeadm](/docs/setup/production-environment/tools/kubeadm/)이다.

## {{% heading "whatsnext" %}}

- [쿠버네티스를 다운로드](/releases/download/)한다.
- `kubectl`을 포함한 [도구를 설치](/docs/tasks/tools/)한다.
- 새로운 클러스터에 사용할 [컨테이너 런타임](/docs/setup/production-environment/container-runtimes/)을 선택한다.
- 클러스터 구성의 [모범 사례](/docs/setup/best-practices/)를 확인한다.
...
- [윈도우 노드를 포함하는 클러스터 구성하기](/docs/concepts/windows/)를 살펴본다.
