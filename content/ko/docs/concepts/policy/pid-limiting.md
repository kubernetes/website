...
프로세스 ID(PID)는 노드에서 기본이 되는 리소스이다. 다른 종류의 리소스 상한(limit)에 도달하지 않고도 
태스크의 상한에 도달하기는 매우 쉬우며, 이는 호스트 머신의 
불안정성을 야기할 수 있다.
...
추가적으로, 파드가 동일한 노드의 다른 워크로드에 미치는 영향을 제한하려면 
...
CPU나 메모리처럼 PID도 오버 커밋할 수 있지만, 몇 가지 추가 위험이 따른다.
...
예약할 수 있다. 이는 파드와 해당 컨테이너 외부의 운영 체제 및 그 밖의 구성 요소에서 
사용하기 위해 CPU, 메모리 또는 기타 리소스를 예약하는 방식과 
유사하다.

PID 제한은 [컴퓨팅 리소스](/docs/concepts/configuration/manage-resources-containers/)
요청 및 제한에서 중요한 개념이다.
...
축출 신호에 대한 [리소스 부족 처리를 구성](/docs/concepts/scheduling-eviction/node-pressure-eviction/)할 
수 있다.
...
어떻게 구성되었는지에 따라,
파드가 재스케줄링(rescheduling)될 수도 있고 그렇지 않을 수도 있다. 그러나 제한이 올바르게 설정되었다면,
...
- [컨테이너에 대한 리소스 관리](/docs/concepts/configuration/manage-resources-containers/)를 읽는다.
- [리소스 부족 처리 구성](/docs/concepts/scheduling-eviction/node-pressure-eviction/)을 배운다.
