...
1. 수신 및 송신 트래픽에 대해 "default" 네임스페이스에서 "role=db"인 파드를 격리한다(아직 격리되지 않은 경우).
2. (수신 규칙)은 "role=db" 레이블을 사용하는 "default" 네임스페이스의 모든 파드에 대해서 TCP 포트 6397로의 연결을 허용한다. 수신을 허용 대상은 다음과 같다.
...
__podSelector__: `NetworkPolicy` 을 통해서, 수신 소스 또는 송신 목적지로 허용되야 하는 동일한 네임스페이스에 있는 특정 파드들을 선택한다.
...
__ipBlock__: 수신 소스 또는 송신 대상으로 허용할 IP CIDR 범위를 선택한다. 파드 IP는 임시적이고 예측할 수 없기에 클러스터 외부 IP이어야 한다.

클러스터 수신 및 송신 매커니즘은 종종 패킷의 소스 또는 대상 IP의 재작성을
