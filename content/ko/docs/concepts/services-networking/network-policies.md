...
네트워크 정책은 충돌하지 않으며, 추가된다. 만약 모든 정책 또는 정책이 파드를 선택하면 해당 정책의 인그레스(수신)/이그레스(송신) 규칙을 통합하여 허용되는 범위로 파드가 제한된다. 따라서 평가 순서는 정책 결과에 영향을 미치지 않는다.
...
### 기본적으로 모든 인그레스와 모든 이그레스 트래픽의 거부
