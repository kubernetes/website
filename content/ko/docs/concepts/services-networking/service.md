...
쿠버네티스에서 서비스는 익숙하지 않은 서비스 디스커버리 메커니즘을 사용하기
위해 기존 애플리케이션을 수정할 필요가 없도록 하는 것이다.
클라우드-네이티브 환경에 맞게 설계된 코드이든, 컨테이너화된 오래된 애플리케이션이든
파드에서 실행할 수 있다. 클라이언트가 상호작용할 수 있도록, 서비스를 사용하여 해당
파드 집합을 네트워크에서 사용할 수 있게 한다.
...
생성되고 삭제된다. 파드는 비영구적 리소스이다(개별 파드가 신뢰할 수
...
쿠버네티스의 일부인 서비스 API는 파드 그룹을 네트워크로 노출하는 데 도움이 되는
추상화 개념이다. 각 서비스 오브젝트는 엔드포인트의 논리적
집합과 (일반적으로 이러한 엔드포인트는 파드이다), 해당 파드에 접근하는 방법에 대한 정책을 정의한다.
...
인그레스는 서비스 유형이 아니지만, 클러스터의 진입점 역할을 한다. 인그레스를
사용하면 라우팅 규칙을 단일 리소스로 통합할 수 있으므로, 클러스터 내에서 개별적으로
실행되는 워크로드의 여러 컴포넌트를 단일 리스너 뒤에서 노출할 수
있다.

쿠버네티스용 [게이트웨이(Gateway)](https://gateway-api.sigs.k8s.io/#what-is-the-gateway-api)
API는 인그레스와 서비스를 넘어서는 추가 기능을 제공한다. 게이트웨이는 클러스터에
추가할 수 있으며, {{< glossary_tooltip term_id="CustomResourceDefinition" text="커스텀리소스데피니션" >}}을
사용하여 구현되는 익스텐션(extension) API 집합이다 -
이후 이를 사용하여 클러스터에서 실행 중인 네트워크 서비스에 대한 접근을 구성할 수 있다.
...
`kubectl`과 같은 도구를 사용하여 이러한 API 호출을 대신 수행한다.
...
이 매니페스트를 적용하면 기본 ClusterIP [서비스 유형](#publishing-services-service-types)을
...
서비스 오브젝트의 이름은 유효한
[RFC 1123 레이블 이름](/docs/concepts/overview/working-with-objects/names/#dns-label-names)이어야 한다.
...
## 서비스 유형  {#publishing-services-service-types}

애플리케이션의 일부(예: 프론트엔드)에 대하여, 클러스터 외부에서 접근할 수 있는
외부 IP 주소로 서비스를 노출하고
싶을 수 있다.

쿠버네티스 서비스 유형을 사용하면 원하는 서비스 종류를 지정할 수 있다.
...
이 기본 서비스 유형은 클러스터가 해당 목적을 위해 예약한 IP 주소 풀에서
IP 주소를 할당한다.

서비스의 다른 몇몇 유형은 `ClusterIP` 유형을 기반으로
만들어진다.
...
기본적으로, LoadBalancer 유형의 서비스에서 둘 이상의 포트가 정의된 경우, 모든 
포트는 동일한 프로토콜을 가져야 하며 프로토콜은 클라우드 공급자가
지원하는 프로토콜이어야 한다.

`MixedProtocolLBService` 기능 게이트(v1.24에서 kube-apiserver에 대해 기본적으로 활성화되어 있음)는 
둘 이상의 포트가 정의되어 있는 경우에 로드 밸런서 유형의 서비스에 대해 서로 다른 프로토콜을 사용할 수 있도록 해 준다.
...
기본적으로 `spec.allocateLoadBalancerNodePorts`는 `true`이며 LoadBalancer 유형의 서비스는 계속해서 노드 포트를 할당할 것이다. 
...
ExternalName 유형의 서비스는 `my-service`나 `cassandra`와 같은 일반적인 셀렉터가 아니라,
...
* `ExternalName`을 제외한 모든 서비스 유형에 대해, 서비스의 준비된 
...
서비스를 정의할 때, 모든 [서비스 유형](#publishing-services-service-types)에
