...
(`type: ClusterIP`인 서비스를 통해) 클러스터-범위의 가상 IP 주소를 가질 수 있다.
...
가장 좋은 예시는 클러스터의 DNS 서비스이다. 느슨한 관례로 일부 쿠버네티스 설치 도구들은
...
하지만 이전에도 설명했듯이, IP 주소 10.96.0.10은 예약된 상태가 아니다. 만약 다른 서비스들이
동적 할당보다 먼저 또는 동적 할당과 동시에 생성된다면, 해당 서비스들이 이 IP를 할당받을 가능성이 있다.
따라서, 충돌 에러로 인해 DNS 서비스를 생성하지 못할 것이다.

## 어떻게 하면 서비스 클러스터IP 충돌을 피할 수 있는가? {#avoid-ClusterIP-conflict}

쿠버네티스에 구현된 할당 전략은
서비스에 클러스터IP를 할당할 때의 충돌 위험을 줄여준다.
