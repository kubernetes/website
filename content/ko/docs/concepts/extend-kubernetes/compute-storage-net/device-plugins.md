...
#
# 이 파드에는 2개의 hardware-vendor.example/foo 장치가 필요하며
...
장치 플러그인은 kubelet 재시작을 감지할 것으로 예상되면 새로운
kubelet 인스턴스에 자신을 다시 등록한다. 현재의 구현에서, 새 kubelet 인스턴스는 시작될 때
...
변경될 수 있다. 프로젝트로서, 쿠버네티스는 장치 플러그인 개발자에게 다음과 같은 것들을 권장한다.
...
노드에서 사용 중인 장치 셋을 검색하고 메트릭을 연관지어질 컨테이너를 설명하는
메타데이터를 얻을 수 있어야 한다. 장치 모니터링 에이전트에 의해 노출된
[프로메테우스(Prometheus)](https://prometheus.io/) 지표는
[쿠버네티스 Instrumentation 가이드라인](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-instrumentation/instrumentation.md)를 따라
