...
weight: 55
description: >
  쿠버네티스에서 배포할 수 있는 가장 작은 컴퓨트 오브젝트인 파드와, 이를 실행하는 데 도움이 되는 하이-레벨(higher-level) 추상화 이해
no_list: true
card:
  title: 워크로드와 파드
  name: concepts
...
[_파드_](/docs/concepts/workloads/pods) 집합 내에서 실행한다.
...
그러나, 작업이 훨씬 쉽도록 각 파드를 직접 관리할 필요는 없다.
...
* [디플로이먼트(Deployment)](/docs/concepts/workloads/controllers/deployment/) 및 [레플리카셋(ReplicaSet)](/docs/concepts/workloads/controllers/replicaset/)
  (레거시 리소스
...
* [스테이트풀셋(StatefulSet)](/docs/concepts/workloads/controllers/statefulset/)은
  ...
  [퍼시스턴트 볼륨(PersistentVolume)](/docs/concepts/storage/persistent-volumes/)을 연계하는 스테이트풀셋을 실행할 수 있다.
  전체적인 회복력 향상을 위해서, 스테이트풀셋의 파드에서 동작 중인 코드는 동일한 스테이트풀셋의
  다른 파드로 데이터를 복제할 수 있다.
* [데몬셋(DaemonSet)](/docs/concepts/workloads/controllers/daemonset/)은 노드에 국한된 
  기능을 제공하는 파드를 정의한다.
  ...
  [클러스터 네트워킹(cluster networking)](/docs/concepts/cluster-administration/networking/#how-to-implement-the-kubernetes-network-model)을 실행하는 플러그인), 
  노드 관리를 도와줄 수도 있으며, 
  실행 중인 컨테이너 플랫폼을 향상시키는 선택적 동작을 제공할 수도 있다.
* [잡(Job)](/docs/concepts/workloads/controllers/job/) 및 
...
리소스도 찾을 수 있다.
[커스텀 리소스 데피니션](/docs/concepts/extend-kubernetes/api-extension/custom-resources/)을 사용하면,
쿠버네티스 코어에서 제공하지 않는 특별한 동작을 원하는 경우 제 3자의 워크로드 리소스를
추가할 수 있다. 예를 들어, 사용자 애플리케이션을 위한 파드의 그룹을 실행하되
_모든_ 파드가 가용하지 않으면 작업을 멈추고 싶다면(아마도 높은 처리량의 분산 처리를 하는 상황 같은),
...
디플로이먼트(Deployment)나 잡(Job)과 같은 표준 워크로드 리소스가 파드의 라이프사이클을 관리하지만,
파드 그룹을 하나의 단위로 취급해야 하는 복잡한 스케줄링 요구사항이 있을 수 있다.

[워크로드 API](/docs/concepts/workloads/workload-api/)를 사용하면 파드 그룹을 정의하고
[갱 스케줄링(gang scheduling)](/docs/concepts/scheduling-eviction/gang-scheduling/)과 같은
고급 스케줄링 정책을 적용할 수 있다.
이는 "전부 아니면 전무(all-or-nothing)" 배치가 필요한 배치 처리 및 머신러닝 워크로드에
특히 유용하다.
...
[구성](/docs/concepts/configuration/)을 참고하길 바란다.

다음은 쿠버네티스가 애플리케이션의 파드를 어떻게 관리하는지를 알 수 있게 해주는
두 가지 개념이다.
* [가비지(Garbage) 수집](/docs/concepts/architecture/garbage-collection/)은 _소유하는 리소스_ 가
  제거된 후 클러스터에서 오브젝트를 정리한다.
* [_time-to-live after finished_ 컨트롤러](/docs/concepts/workloads/controllers/ttlafterfinished/)는
  잡이 완료된 이후에 정의된 시간이 경과되면 잡을 제거한다.

일단 애플리케이션이 실행되면, 인터넷에서 [서비스](/docs/concepts/services-networking/service/)로
사용하거나, 웹 애플리케이션의 경우에만
[인그레스(Ingress)](/docs/concepts/services-networking/ingress)를 이용하여 사용할 수 있다.
