...
description: >-
  디플로이먼트는 애플리케이션 워크로드를 실행하기 위해 여러 파드 집합을 관리하며, 보통 상태를 유지하지 않는(stateless) 애플리케이션을 대상으로 한다.
...
도달한 **후**에 초기화는 시작된다.
`.spec.revisionHistoryLimit` 값을 0으로 설정하더라도, 롤아웃이 발생하면 Kubernetes가 
