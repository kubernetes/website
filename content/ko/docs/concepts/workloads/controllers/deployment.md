...
feature:
  title: 자동화된 롤아웃과 롤백
  description: >
    쿠버네티스는 애플리케이션 또는 애플리케이션의 설정 변경시 점진적으로 롤아웃하는 동시에 애플리케이션을 모니터링해서 모든 인스턴스가 동시에 종료되지 않도록 보장한다. 만약 어떤 문제가 발생하면 쿠버네티스는 변경사항을 롤백한다. 성장하는 디플로이먼트 솔루션 생태계를 이용한다.
...
{{< note >}}
디플로이먼트가 소유하는 레플리카셋은 관리하지 말아야 한다. 다음 유스케이스에 포함되지 않는 경우 쿠버네티스 리포지터리에 문제 열기를 고려해야 한다.
{{< /note >}}
...
* [레플리카셋을 롤아웃 할 디플로이먼트 생성](#디플로이먼트-생성). 레플리카셋은 백그라운드에서 파드를 생성한다. 롤아웃 상태를 체크해서 성공여부를 확인한다.
* 디플로이먼트의 PodTemplateSpec을 업데이트해서 [파드의 새로운 상태를 선언한다](#디플로이먼트-업데이트). 새 레플리카셋이 생성되면, 디플로이먼트는 파드를 기존 레플리카셋에서 새로운 레플리카셋으로 속도를 제어하며 이동하는 것을 관리한다. 각각의 새로운 레플리카셋은 디플로이먼트의 수정 버전으로 업데이트한다.
* 만약 디플로이먼트의 현재 상태가 안정적이지 않은 경우 [디플로이먼트의 이전 버전으로 롤백](#디플로이먼트-롤백)한다. 각 롤백은 디플로이먼트의 수정 버전을 업데이트한다.
* [더 많은 로드를 위해 디플로이먼트의 스케일 업](#디플로이먼트-스케일링).
* [디플로이먼트 일시 중지](#디플로이먼트의-일시-중지와-재개)로 PodTemplateSpec에 여러 수정 사항을 적용하고, 새로운 롤아웃의 시작을 재개한다.
* 롤아웃이 고착된 지표를 나타내는 [디플로이먼트 상태 이용](#디플로이먼트-상태).
* 더 이상 필요 없는 [이전 레플리카셋 정리](#정책-초기화)

## 디플로이먼트 생성

다음은 디플로이먼트의 예시이다. 예시는 3개의 `nginx` 파드를 불러오기위해 레플리카셋을 생성한다.
...
* `replicas` 필드에 따라 디플로이먼트는 3개의 라플리카 파드를 생성한다.
* `selector` 필드는 디플로이먼트가 관리할 파드를 찾는 방법을 정의한다.
  이 사례에서는 간단하게 파드 템플릿에 정의된 레이블(`app: nginx`)을 선택한다.
  그러나 파드 템플릿 자체가 규칙을 만족하는 
  한 보다 정교한 규칙 셀렉션이 가능하다.
    {{< note >}}
    `matchLabels` 필드는 {key,value}의 쌍으로 매핑되어있다. `matchLabels` 에 매핑된 
    단일 {key,value}은 `matchExpressions` 의 요소에 해당하며, 키 필드는 "key" 연사자는 "In", 
    그리고 값 배열에는 "value"만 포함되어 있다.
    `matchLabels` 과 `matchExpressions` 의 모든 요건은 일치시키위해 충족되어야 한다.
    {{< /note >}}
...
* `template` 필드에는 다음 하위 필드가 포함되어있다.
  ...
  * 컨테이너 1개를 생성하고, `name` 필드를 사용해서 `nginx` 이름을 붙인다.
  ...
  1. 다음 명령어를 실행해서 디플로이먼트를 생성한다.
      ...
      {{< note >}}
      `--record` 플래그를 지정해서 실행된 명령을 `kubernetes.io/change-cause` 리소스 주석에 작성할 수 있다. 이것은 향후 자기 성찰에 유용하다.
      예를 들어 각 디플로이먼트 수정 버전에서 실행된 명령을 본다.
      {{< /note >}}
  ...
  2. `kubectl get deployments` 을 실행해서 디플로이먼트가 생성되었는지 확인한다. 만약 디플로이먼트가 여전히 생성중이면 다음과 유사하게 출력된다.
    ...
    클러스터에서 디플로이먼트를 점검할 때 다음 필드가 표시된다.
      ...
      * `CURRENT` 는 현재 실행중인 레플리카의 수를 표시한다.
      * `UP-TO-DATE` 는 의도한 상태를 얻기위해 업데이트 된 레플리카의 수를 표시한다.
      * `AVAILABLE` 은 사용자가 사용가능한 애플리케이션 레플리카의 수를 표시한다.
  ...
  4. 몇 초 후 `kubectl get deployments` 를 다시 실행한다. 다음과 유사하게 출력된다.
    ...
    ```
    디플로이먼트에서 3개의 레플리카가 생성되었고, 모든 레플리카는 최신 상태(최신 파드 템플릿에 포함)이며 사용가능한 것을 알 수 있다.

  5. 디플로이먼트로 생성된 레플리카셋(`rs`)를 보려면 `kubectl get rs` 를 실행한다. 다음과 유사하게 출력된다.
    ```shell
    NAME                          DESIRED   CURRENT   READY   AGE
    nginx-deployment-75675f5897   3         3         3       18s
    ```
    레플리카셋의 이름은 항상 `[DEPLOYMENT-NAME]-[RANDOM-STRING]` 형식으로 된 것을 알 수 있다. 무작위 문자열은
    	무작위로 생성되며, Pod-template-hash를 시드로 사용한다.
  ...
  {{< note >}}
  디플로이먼트에서 파드 템플릿 레이블과 적절한 셀렉터를 명시해야한다(이 예시에서는 `app: nginx`).
  레이블 또는 셀렉터를 다른 컨트롤러(다른 디플로이먼트와 스테이트풀 셋 포함)와 겹치지 않아야 한다. 쿠버네티스는 겹치는 것을 막지 않으며, 만약 다중 컨트롤러가 겹치는 셀렉터를 가지는 경우 해당 컨트롤러의 충돌 또는 예기치 않은 동작을 할 수 있다.
  {{< /note >}}
...
`pod-template-hash` 레이블은 디플로이먼트가 생성 또는 채택한 모든 레플리케셋에 디플로이먼트 컨트롤러에 의해 추가된다.
...
{{< note >}}
디플로이먼트의 파드 템플릿(즉, `.spec.template`)이 변경된 경에만 디플로이먼트의 롤아웃이 트리거 된다. 
예를 들어 템플릿의 렝블이나 컨테이너 이미지가 업데이트된 경우이다. 디플로이먼트의 스케일링과 같은 다른 업데이트는 롤아웃 트리거하지 말아야 합니다.
{{< /note >}}

다음 단계에 따라 디플로이먼트를 업데이트한다.

1. nginx 파드를 이용중인 `nginx:1.9.1` 이미지에서 `nginx:1.7.9` 로 업데이트 한다.
...
새 레플리카셋을 최대 3개의 레플리카로 스케일업, 이전 레플리카셋을 0개의 레플리카로 스케일다운한다.
...
* `get pods` 를 실행하면 새 파드만 표시된다.
    ...
    의도한 파드의 수 중 25% 이상이 되도록 보장한다(최대 25% 이상 불가).

    또한 디플로이먼트는 의도한 수의 파드 이상으로 파드가 생성되도록 보장한다.
    기본적으로 의도한 파드의 수의 최대 25%가 되도록 보장한다(최대 25% 까지).

    예를 들어, 위 디플로이먼트를 자세히 살펴보면 먼저 새로운 파드를 생성한 다음 
    이전 파드를 삭제하고, 새로운 파드를 만든 것을 볼 수 있다. 새로은 파드가 나올 때까지 이전 파드를 죽이지 않으며, 
    이전 파드가 충분히 죽기 전까지 새로운 파드를 만들지 않는다.
...
* 디플로이먼트의 세부 정보 가져오기
  ...
  Pod Template:
    ...
    ```
    여기에서 처음 디플로이먼트를 생성할 때 레플리카셋(nginx-deployment-2035384211)을 생성해서 
    3개의 레플리카를 직접 스케일 업한 것을 볼 수 있다.
    디플로이먼트를 배포할 때 새 레플리카셋(nginx-deployment-1564180365)을 생성하고, 1개로 스케일업한 다음 
    이전 레플리카셋을 2개로 스케일다운해서 최소 2개의 파드를 사용할 수 있고, 최대 4개의 파드가 생성됬다.
    이후 지속해서 같은 롤링 업데이트 계획으로 새 레플리카셋은 스케일 업하고 이전 레플리카셋은 스케일 다운한다.
    마지막으로 새로운 레플리카셋에 3개의 사용가능한 레플리카가 있으며, 이전 레플리카셋은 0개로 스케일다운된다.

### 롤오버(일명 다중 업데이트 진행 중)

디플로이먼트 컨트롤러가 각 시간마다 새로운 디플로이먼트를 관찰하면 레플리카셋이 
필요한 파드를 생성해서 가져온다. 만약 디플로이먼트가 업데이트되면, 기존 레플리카셋에서 
`.spec.selector` 레이블과 일치하는 파드를 컨트롤 하지만, `.spec.template` 과 템플릿이 불일치하면 스케일다운이 된다.
결국 새로운 레플리카셋은 `.spec.replicas` 으로 스케일되고, 모든 기존 레플리카 셋은 0개로 스케일된다.

만약 기존에 롤아웃이 진행중에 디플로이먼트를 업데이트하는 경우 디플로이먼트가 업데이트에 따라 새 레플리카셋을 생성하고, 
스케일 업하기 시작한다. 그리고 이전에 스케일업 하던 레플리카셋에 롤오버 한다.
 --이것은 기존 레플리카셋 목록에 추가하고 스케일다운을 할 것이다.

예를 들어 가령 디플로이먼트로 `nginx:1.7.9` 레플리카를 5개 생성한다. 
하지만 `nginx:1.7.9` 레플리카 3개가 생성되었을 때 디플로이먼트를 업데이트해서 `nginx:1.9.1` 
레플리카 5개를 생성성하도록 업데이트를 한다. 이 경우 디플로이먼트는 즉시 생성된 3개의 
`nginx:1.7.9` 파드 3개를 죽이기 시작하고 `nginx:1.9.1` 파드를 생성하기 시작한다. 
이것은 코스가 변경되기 전에 `nginx:1.7.9` 레플리카 5개가 
생성되는 것을 기다리지 않는다.

### 레이블 셀렉터 업데이트

일반적으로 레이블 셀렉터를 업데이는 하는 것은 권장하지 않으며 셀렉터를 미리 계획하는 것을 권장한다.
...
* 생성된 파드를 보면, 새로은 레플리카셋에 생성된 1개의 파드가 이미지 풀 루프에서 고착된 것을 볼 수 있다.
    ...
    이는 지정한 롤링업데이트의 파라미터 (구체적으로 `maxUnavailable`)에 따라 달라진다.
...
사용자 또는 오토스케일러가 롤아웃 중에 있는 디플로이먼트 롤링업데이트를 스케일링 하는 경우(진행중 또는 일시 중지 중), 
...
* 이미지 업데이트는 레플리카셋 nginx-deployment-1989198191 로 새로운 롤 아웃이 시작하지만, 
...
* 그 다음 디플로이먼트에 대한 새로운 스케일링 요청이 함께 따라온다. 오토스케일러는 디플로이먼트 레플리카는 15로 증가시킨다.
디플로이먼트 컨트롤러는 새로운 5개의 레플리카의 추가할 위치를 결정해야 한다.
만약 비례적 스케일링을 사용하지 않으면 5개 모두 새 레플리카셋에 추가된다.
비례적 스케일링으로 추가 레플리카를 모든 레플리카셋에 걸쳐 분산할 수 있다.
비율이 높을수록 가장 많은 레플리카가 있는 레플리카셋으로 이동하고, 비율이 적을 수록 적은 레플리카가 있는 레플리카 셋으로 이동한다.
남은 것들은 대부분의 레플리카가 있는 레플리카셋에 추가된다. 0개의 레플리카가 있는 레플리카셋은 스케일 업 되지 않는다.

위의 예시에서 기존 레플리카셋에 3개의 레플리카가 추가되고, 2개의 레플리카는 새 레플리카에 추가된다.
결국 롤아웃 프로세스는 새 레플리카가 정상이라고 가정하면 모든 레플리카를 새 레플리카셋으로 이동한다. 
이를 확인하려면 다음을 실행한다.
...
이렇게 하면 불필요한 롤아웃을 트리거하지 않고 일시 중지와 재개 사이에 여러 수정사항을 적용할 수 있다.
...
* 예를 들어 사용할 리소스를 업데이트하는 것처럼 원하는만큼 업데이트할 수 있다.
    ```shell
...
### 디플로이먼트 진행 중(Progressing Deployment)
...
디플로이먼트의 진행이 정지되었음을 나타내는(디플로이먼트 상태에서) 디플로이먼트 컨트롤러가 
대기하는 시간(초)를 나타낸다.

다음 `kubectl` 명령어로 `progressDeadlineSeconds` 를 설정해해서 컨트롤러 보고서가 
10분 후 디플로이먼트에 대한 진행 상태가 부족하도록 한다.
...
{{< note >}}
쿠버네티스는 `Reason=ProgressDeadlineExceeded` 과 같은 컨디션 상태 조건을 
보고하는 것 왜에 정지된 디플로이먼트에 대해 조치를 취하지 않는다. 더 높은 수준의 오케스트레이터는 이를 활용할 수 있으며, 
...
디플로이먼트를 스케일 다운하거나, 실행중인 다른 컨트롤러를 스케일 다운하거나, 
...
롤아웃 도중에에 진행중 이거나, 성공적으로 완료되었으며, 진행 중 최소한으로 필요한 새로운 레플리카를 이용가능하다는 것이다.
...
컨테이너 구성하기 그리고 [kubectl을 사용해서 리소스 관리하기](/ko/docs/concepts/overview/working-with-objects/object-management/) 문서를 참조하라.
...
API 버전 `apps/v1` 에서는 `.spec.selector` 와 `.metadata.labels` 이 설정되지 않으면 `.spec.template.metadata.labels` 은 기본 설정되지 않는다. 그래서 이것들은 명시적으로 설정되어야 한다. 또한 `apps/v1` 에서는 디플로이먼트를 생성한 후에는 `.spec.selector` 이 변경되지 않는 점을 참조하라.
...
`.spec.strategy` 는 이전파드를 새로운 파드로 대체하는 전략을 명시한다.
`.spec.strategy.type` 은 "재생성" 또는 "롤링업데이트"가 될 수 있다.
"롤링업데이트"가 기본값이다.

#### 디플로이먼트 재생성

기존의 모든 파드는 `.spec.strategy.type==Recreate` 일면 새파드가 생성되기 전에 죽는다.
...
`.spec.progressDeadlineSeconds` 는 디플로어민트가 표면적으로 `Type=Progressing`, `Status=False`의 
상태 그리고 리소스가 `Reason=ProgressDeadlineExceeded` 상태로 [진행 실패](#디플로이먼트-실패)를 보고하기 전에 
디플로이먼트가 진행되는 것을 대기하는 시간(초)를 명시하는 선택적 필드이다.
...
용할 수 있도록 준비되어야 하는 최소 시간(초)를 지정하는 선택적 필드이다.
