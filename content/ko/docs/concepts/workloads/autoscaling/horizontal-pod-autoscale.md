...
더 많은 리소스(예: 메모리 또는 CPU)를 할당하는 것)과는 다르다.
...
HorizontalPodAutoscaler API 리소스는 컨트롤러의 행동을 결정한다.
...
각 주기마다, 컨트롤러 매니저는
각 HorizontalPodAutoscaler 정의에 지정된 메트릭에 대해 리소스 사용률을 질의한다. 컨트롤러 매니저는
`scaleTargetRef`에 의해 정의된 타겟 리소스를 찾고 나서,
타겟 리소스의 `.spec.selector` 레이블을 보고 파드를 선택하며,
리소스 메트릭 API(파드 단위 리소스 메트릭용) 또는
커스텀 메트릭 API(그 외 모든 메트릭용)로부터 메트릭을 수집한다.

- 파드 단위 리소스 메트릭(예: CPU)의 경우 컨트롤러는 HorizontalPodAutoscaler가
  대상으로 하는 각 파드에 대한 리소스 메트릭 API에서 메트릭을 가져온다.
  그런 다음, 목표 사용률 값이 설정되면, 컨트롤러는 각 파드의
  컨테이너에 대한 동등한 [리소스 요청](/docs/concepts/configuration/manage-resources-containers/#요청-및-제한)을
  퍼센트 단위로 하여 사용률 값을
  ...
  알고리즘의 작동 방식에 대한 자세한 내용은 아래 [알고리즘 세부 정보](#알고리즘-세부-정보) 섹션을 참고한다.
...
Metrics Server는 별도로 실행해야 한다. 리소스 메트릭에 대한 추가 정보는
[Metrics Server](/docs/tasks/debug/debug-cluster/resource-metrics-pipeline/#metrics-server)를 참고한다.
...
그리고 몇 개의 파드가 [`Ready`](/docs/concepts/workloads/pods/pod-lifecycle/#파드의-컨디션)인지도 고려한다.
파드 단위 리소스 메트릭의 경우, 삭제 타임스탬프가 설정된 모든 파드(삭제
타임스탬프가 있는 오브젝트는 셧다운/삭제가 진행 중임을 뜻한다)는 무시되며,
실패한 모든 파드는 버려진다.
외부 메트릭과 오브젝트 메트릭의 경우, 레플리카 수는 `Running` 상태이면서
`Ready` 상태인 파드의 수를 기준으로 한다. 종료 중이지만 여전히 `Ready` 상태인
파드도 그 합계에 계속 포함된다.

특정 파드에 메트릭이 누락된 경우, 나중을 위해 처리를 미뤄두는데, 이와
같이 누락된 메트릭이 있는 파드는 최종 스케일 양을 조정하는 데 사용된다.

CPU를 기준으로 스케일링할 때, 아직 `Ready` 상태가 되지 않았거나(여전히
초기화 중이거나 비정상 상태일 수 있음) *또는* 해당 파드의 최신 메트릭 포인트가
`Ready` 상태가 되기 전의 것이라면, 그 파드도 마찬가지로 제외된다.

기술적 제약으로 인해, HorizontalPodAutoscaler 컨트롤러는
특정 CPU 메트릭을 제외할지 판단할 때, 파드가 처음으로 준비 상태가 되는
시점을 정확하게 알 수 없다. 대신, 파드가 준비되지 않은 상태이고
시작 이후 짧은, 구성 가능한 시간 창 내에 준비 상태로
전환되었다면, 해당 파드를 "아직 준비되지 않음(not yet ready)"으로 간주한다.
이 값은 `--horizontal-pod-autoscaler-initial-readiness-delay`
커맨드 라인 옵션으로 설정되며, 기본값은 30초이다.
파드가 준비 상태가 되면, 파드가 시작된 이후 보다 긴,
구성 가능한 시간 내에 발생한 준비 상태로의 전환을 첫 번째 전환으로
간주한다. 이 값은
`--horizontal-pod-autoscaler-cpu-initialization-period` 커맨드 라인 옵션으로 설정되며
기본값은 5분이다.

그런 다음 \\( 현재메트릭값 \over 원하는메트릭값 \\) 기본 스케일 비율은
위에서 제외되거나 버려지지 않고 남은 파드를 사용하여 계산된다.
...
컨트롤러가 스케일링을 건너뛴다.
그렇지 않으면, 새로 계산한 사용률을 이용하여 파드 수 변경 결정을 내린다.
...
큰 값이 선택된다. 이러한 메트릭 중 하나라도 원하는
레플리카 수로 변환할 수 없고(예: 메트릭 API에서 메트릭을
가져오는 중 오류 발생), 가져올 수 있는 메트릭이 스케일 다운을
제안하는 경우 스케일링을 건너뛴다. 이는 하나 이상의 메트릭이
현재 값보다 높은 `desiredReplicas`를 제공하는 경우
HPA가 여전히 스케일 업할 수 있음을 의미한다.

마지막으로, HPA가 목표를 스케일하기 직전에 스케일 권장 사항이
기록된다. 컨트롤러는 구성 가능한 창(window) 내에서 가장 높은 권장
사항을 선택하도록 해당 창 내의 모든 권장 사항을 고려한다. 이 값은
`--horizontal-pod-autoscaler-downscale-stabilization` 커맨드 라인 옵션을 사용하여 설정할 수 있고, 기본값은 5분이다.
즉, 스케일 다운이 점진적으로 발생하여 급격히 변동하는 메트릭 값의
영향을 완만하게 한다.

## 파드 준비성(readiness)과 오토스케일링 메트릭

HorizontalPodAutoscaler(HPA) 컨트롤러에는 시작 중인 파드에서 CPU 메트릭을 수집하는 방식에 영향을 주는 두 가지 커맨드 라인 옵션이 있다.

1. `--horizontal-pod-autoscaler-cpu-initialization-period` (기본값: 5분)

  이 옵션은 파드가 시작된 후 **CPU 사용량이 무시**되는 시간 윈도우를 정의한다. 단, 다음 조건을 모두 만족하면 무시하지 않는다.
    - 파드가 `Ready` 상태**이고**
    - 메트릭 샘플이 파드가 `Ready` 상태인 기간 동안에만 수집된 경우.

  이 커맨드 라인 옵션은 초기화 중인 파드(예: Java 애플리케이션 워밍업)의 **잘못된 높은 CPU 사용량**이 HPA 스케일링 결정에 반영되는 것을 방지하는 데 도움이 된다.

1. `--horizontal-pod-autoscaler-initial-readiness-delay` (기본값: 30초)

  이 옵션은 파드가 시작된 후 짧은 지연 기간을 정의하며, 이 기간 동안 HPA 컨트롤러는 현재 `Unready` 상태인 파드를 **이전에 잠깐 `Ready`로 전환된 적이 있더라도** 여전히 초기화 중인 것으로 처리한다.

  이 옵션의 목적은 다음과 같다.
    - 시작 중에 `Ready`와 `Unready` 사이를 빠르게 변동하는 파드를 포함하지 않도록 방지.
    - HPA가 메트릭을 유효한 것으로 간주하기 전에 초기 준비 신호의 안정성을 보장.


이러한 커맨드 라인 옵션은 클러스터 전체에서만 설정할 수 있다.

### 파드 준비성의 주요 동작 {#pod-readiness-key-behaviors}

- 파드가 `Ready` 상태이고 `Ready`를 유지하면, 지연 기간 내에서도 메트릭에 기여하는 것으로 간주될 수 있다.
- 파드가 `Ready`와 `Unready` 사이를 빠르게 전환하면, 안정적으로 `Ready`인 것으로 간주될 때까지 메트릭이 무시된다.

### 파드 준비성에 대한 모범 사례 {#pod-readiness-good-practices}

- 높은 CPU 사용량이 지나갈 때까지 통과하지 않는 `startupProbe`를 구성하거나,
- `initialDelaySeconds`를 사용하여 CPU 스파이크가 가라앉은 **이후에만** `readinessProbe`가 `Ready`를 보고하도록 한다.

그리고 이상적으로는 `--horizontal-pod-autoscaler-cpu-initialization-period`를 **시작 기간을 포함하도록** 설정한다.
...
[DNS 서브도메인 이름](/docs/concepts/overview/working-with-objects/names/#dns-서브도메인-이름)인지 확인해야 한다.
...
하나의 디플로이먼트에 하나의 HorizontalPodAutoscaler를 연결한다.
...
스테이트풀셋이 자신의 파드 집합을 직접 관리한다(레플리카셋과
같은 중간 리소스가 없다).
...
지정해야 한다. 이것은 리소스 사용률을 결정하는 데 사용되며 HPA 컨트롤러가 대상을
스케일 업 또는 스케일 다운하는 데 사용한다. 리소스 사용률 기반 스케일링을 사용하려면 다음과 같은 메트릭 소스를
지정해야 한다.
...
예를 들어 웹 애플리케이션과 로깅을 제공하는 사이드카 컨테이너가 있는 경우 사이드카 컨테이너와 해당 리소스 사용을 무시하고
...
참조한다. 컨테이너 리소스를 오토스케일링에 사용하려면 다음과 같이
메트릭 소스를 정의한다.
...
효율적으로 유지할 수 있다. 컨테이너를 정의하는 리소스(예: 디플로이먼트)를
...
`autoscaling/v2` API 버전을 사용하는 경우, HorizontalPodAutoscaler는 스케일링에 사용할 복수의 메트릭을
설정할 수 있다. 이 경우 HorizontalPodAutoscaler 컨트롤러가 각 메트릭을 확인하고
해당 단일 메트릭에 대한 새로운 스케일링 크기를 제안한다. HorizontalPodAutoscaler는 새롭게 제안된 스케일링 크기 중 가장 큰 값을
선택하여 워크로드 사이즈를 조정한다(이 값이 이전에 설정한
총 최대값(overall maximum)보다는 크지 않을 때에만).
...
- 해당 API 등록:
  ...
  - 리소스 메트릭의 경우, 이것은 일반적으로
    [metrics-server](https://github.com/kubernetes-sigs/metrics-server)가 제공하는 `metrics.k8s.io` [API](/docs/reference/external-api/metrics.v1beta1/)이다.
    클러스터 애드온으로 실행할 수 있다.

  - 사용자 정의 메트릭의 경우, 이것은 `custom.metrics.k8s.io` [API](/docs/reference/external-api/custom-metrics.v1beta2/)이다.
    메트릭 솔루션 공급 업체에서 제공하는 "어댑터" API 서버에서 제공한다.
    사용 가능한 쿠버네티스 메트릭 어댑터가 있는지 확인하려면 사용하고자 하는 메트릭 파이프라인을 확인한다.

  - 외부 메트릭의 경우, 이것은 `external.metrics.k8s.io` [API](/docs/reference/external-api/external-metrics.v1beta1/)이다.
    위에 제공된 사용자 정의 메트릭 어댑터에서 제공될 수 있다.
...
어떻게 사용하는지에 대한 예시는
[커스텀 메트릭을 사용하는 작업 과정](/docs/tasks/run-application/horizontal-pod-autoscale-walkthrough/#다양한-메트릭-및-사용자-정의-메트릭에-기반한-오토스케일링)과
[외부 메트릭을 사용하는 작업 과정](/docs/tasks/run-application/horizontal-pod-autoscale-walkthrough/#쿠버네티스-오브젝트와-관련이-없는-메트릭을-기반한-오토스케일링)을 참조한다.
...
`scaleUp` 및/또는 `scaleDown`을 설정하여 지정할 수 있다.

스케일링 정책을 이용하여 스케일링 시 레플리카 수 변화 속도를 조절할 수 있다.
또한 [흔들림](#flapping)을 방지하기 위한 두 가지 설정이 있다. 레플리카 수를 매끄럽게 조절하기 위한
_안정화 윈도우_ 를 명시할 수 있으며, 지정된 임계값 이하의 사소한 메트릭 변동을 무시하기 위한
허용 오차(tolerance)를 설정할 수도 있다.
...
`periodSeconds`에 설정할 수 있는 최대값은 1800(30분)이다.
첫 번째 정책 _(Pods)_ 은 1분 내에 최대 4개의 레플리카를 스케일 다운할 수 있도록 허용한다. 두 번째 정책
_(Percent)_ 은 현재 레플리카의 최대 10%를 1분 내에 스케일 다운할 수 있도록 허용한다.
...
미만으로 떨어지면 첫 번째 폴리시 _(Pods)_ 가 적용되고 한번에
4개의 레플리카가 줄어든다.

스케일링 방향에 대해 `selectPolicy` 필드를 지정하여 폴리시 선택을 변경할 수 있다.
값을 `Min`으로 설정하면 레플리카 수를 가장 적게 변경하는 폴리시가 선택된다.
...
워크로드 스케일의 원치 않는 변화를 방지하기 위해 이 안정화 윈도우를 활용한다.
...
`tolerance` 필드는 메트릭 변동에 대한 임계값을 구성하여,
그 값보다 작은 변화에 대해 오토스케일러가 스케일링하지 않도록 한다.

이 허용 오차는 원하는 메트릭 값 주변의 변동량으로 정의되며,
이 범위 내에서는 스케일링이 발생하지 않는다. 예를 들어, 목표 메모리 사용량이 100MiB이고
스케일 업 허용 오차가 5%로 구성된 HorizontalPodAutoscaler를 고려해 보자.

```yaml
behavior:
  scaleUp:
    tolerance: 0.05 # 스케일 업에 대한 5% 허용 오차
```

이 구성에서, HPA 알고리즘은 메모리 사용량이 105MiB(즉, 목표값보다 5% 높음)보다 높은
경우에만 스케일 업을 고려한다.

이 필드를 설정하지 않으면, HPA는 클러스터 전체 기본 허용 오차인 10%를 적용한다.
이 기본값은 [kube-controller-manager](/docs/reference/command-line-tools-reference/kube-controller-manager/)의
`--horizontal-pod-autoscaler-tolerance` 커맨드 라인 인자를 사용하여 스케일 업과
스케일 다운 모두에 대해 업데이트할 수 있다. (쿠버네티스 API를 사용하여
이 기본값을 구성할 수는 없다.)
...
behavior:
  ...
  scaleUp:
    ...
    selectPolicy: Max
```
스케일 다운의 경우 안정화 윈도우는 _300_ 초(또는 제공된 경우
`--horizontal-pod-autoscaler-downscale-stabilization` 커맨드 라인 옵션의 값)이다. 스케일링 다운에서는 현재
...
또한 HorizontalPodAutoscaler 오브젝트를 생성할 수 있는 `kubectl autoscale`이라는 특별한 명령이 있다.
예를 들어 `kubectl autoscale rs foo --min=2 --max=5 --cpu=80%`를
실행하면 레플리카셋 *foo* 에 대한 오토스케일러가 생성되고, 목표 CPU 사용률은 `80%`,
그리고 2와 5 사이의 레플리카 개수로 설정된다.

## 암시적 점검 모드(maintenance-mode) 비활성화

HPA 구성 자체를 변경할 필요 없이 대상에 대한
...
이는 바람직하지 않을 수 있고, HPA가 활성화된 경우 스래싱(thrashing) 또는 흔들림(flapping) 동작을 유발하여 문제가 될 수 있다.

`spec.replicas` 값을 제거하면 1회성으로 파드 숫자가 줄어들 수 있는데,
이는 이 키의 기본값이 1이기 때문이다(레퍼런스:
[디플로이먼트 레플리카](/docs/concepts/workloads/controllers/deployment#레플리카)).
...
클러스터에 오토스케일링을 구성한다면, 적절한 수의 노드를 실행하도록
[노드 오토스케일링](/docs/concepts/cluster-administration/node-autoscaling/) 사용도
고려해 볼 수 있다.
[_수직_ 파드 오토스케일링](/docs/concepts/workloads/autoscaling/vertical-pod-autoscale/)에 대해서도 더 자세히 읽어볼 수 있다.
...
- HorizontalPodAutoscaler에 대한 [API 레퍼런스](/docs/reference/kubernetes-api/workload-resources/horizontal-pod-autoscaler-v2/)를 확인한다.
