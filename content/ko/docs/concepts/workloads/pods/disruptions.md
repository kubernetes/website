...
파드는 무언가가(사람 또는 컨트롤러) 파괴하거나 
불가피한 하드웨어 오류 또는 시스템 소프트웨어 오류가 아니면 사라지지 않는다.

우리는 불가피한 상황을 애플리케이션의 *불가피한 장애*로 부른다.
예시 :

- 노드를 지원하는 실제 컴퓨터의 하드웨어 오류
...
클러스터에 자발적인 장애를 일으킬 수 있는 어떤 원인이 있는지 
클러스터 관리자에게 문의하거나 클라우드 공급자에게 문의하고, 배포 문서를 참조해서 확인해야 한다.
만약 자발적인 장애를 일으킬 수 있는 원인이 없다면 Pod Disruption Budget의 생성을 넘길 수 있다.
...
예를 들어 노드 소프트웨어의 업데이트를 출시하는 경우 자발적 장애가 발생할 수 있다.
또한 클러스터(노드)에서 오토스케일링을 구현하면 임의의 조각모음 및 소형 노드에서 자발적 장애가 발생할 수 있다.
클러스터 관리자 또는 호스팅 공급자는 예측 가능한 자발적 장애 수준에 대해 문서화해야 한다.

쿠버네티스는 자주 발생하는 자발적 중단을 위해 고가용성 애플리케이션을
실행 할 수 있는 기능을 제공한다. 
...
해당 도구는 머신에 존재하는 모든 파드를 제거하려고 시도한다.
제거 요청은 일시적으로 거부될 수 있으며, 도구는 모든 파드가 종료되거나 
실행 시간이 초과할 때까지 주기적으로 모든 실패된 요청을 다시 시도한다.

PDB는 애플리케이션이 필요로 하는 레플리카의 수에 비례해서 이용가능한 레플리카의 개수를 지정한다.
예를 들어 `.spec.replicas: 5`로 디플로이먼트가 되는 경우 실행 시간동안 5개의 파드가 지원해야 한다.
...
PDB는 [불가피한 장애](#voluntary-and-involuntary-disruptions)가 발생하는 것을 막을 수는 없지만,
버짓에 영향을 준다.
...
어느 순간 파드가 종료되고, 클러스터는 다음과 같이 보이게 된다.
...
쿠버네티스가 장애가 발생할 수 있는 비율을 어떻게 변화시키는지 
다음의 사례를 통해 알 수 있다.
