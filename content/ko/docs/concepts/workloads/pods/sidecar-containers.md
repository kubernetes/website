...
주 애플리케이션 컨테이너의 기능을 향상시키거나 확장시키는데 사용된다.
...
시작된 후에도 계속 실행된다. 이는
`.spec.initContainers`의 파드 내에 하나 이상의 항목이 있을 때 중요하다. 사이드카 스타일의 초기화 컨테이너가 실행 중일 때 (kubelet이
해당 초기화 컨테이너의 `started `상태를 true로 설정함), kubelet은 순차적인 `.spec.initContainers` 목록에서
다음 초기화 컨테이너를 시작한다.
이 상태는 컨테이너 내에 프로세스가 실행 중이고 시작 프로브가 정의되지 않았거나,
`startupProbe`가 성공한 결과로 true가 된다.

다음은 사이드카가 있는 두 컨테이너를 포함하는 배포의 예시이다:
...
사이드카 컨테이너는 주 애플리케이션 컨테이너와 동시에 실행된다. 그들은 파드의 수명 주기
내내 활성화되며,
주 컨테이너와 독립적으로 시작 및 중지될 수 있다. [초기화 컨테이너](/ko/docs/concepts/workloads/pods/init-containers/)와 달리,
사이드카 컨테이너는 수명 주기를 제어하기 위해 [프로브](/ko/docs/concepts/workloads/pods/pod-lifecycle/#프로브-종류)를 지원한다.
...
{{< comment >}}
이 섹션은 [초기화 컨테이너](/ko/docs/concepts/workloads/pods/init-containers/)에도 존재합니다.
이 섹션을 편집하는 경우, 두 곳 모두 변경하십시오.
{{< /comment >}}
...
[파드 오버헤드](/ko/docs/concepts/scheduling-eviction/pod-overhead/)와 다음 중 더 높은 값의 합이다:
  * 모든 비초기화 컨테이너(앱 및 사이드카 컨테이너)의 
  리소스에 대한 요청량/한도의 합계
  * 리소스에 대한 유효한 초기화 요청량/한도
* 스케줄링은 유효한 요청/한도에 따라 이루어진다. 즉,
  초기화 컨테이너는 파드의 수명 동안 사용되지 않는 초기화를 위한 리소스를
  예약할 수 있다.
* 파드의 *유효한 QoS 계층* 에서 QoS (서비스의 품질) 계층은 모든 초기화 컨테이너들과
  앱 컨테이너들의 QoS 계층과 같다.

쿼터 및 한도는 유효한 파드의 요청량 및 한도에 따라
적용된다.

파드 레벨 제어 그룹 (cgroups)은 유효한 파드 요청량 및 한도룰 기반으로 한다.
이는 스케줄러와 같다.
