...
부가적인 서비스나 기능을 제공하여 기본 _앱 컨테이너_의 기능을 향상시키거나 확장하는 데 사용되며,
...
쿠버네티스는 사이드카 컨테이너를 
[초기화 컨테이너](/ko/docs/concepts/workloads/pods/init-containers/)의 특별한 유형으로 구현한다. 사이드카 컨테이너는
...
쿠버네티스 관점에서, 사이드카 컨테이너의 정상 종료는 그리 중요하지 않다.
다른 컨테이너가 할당된 모든 정상 종료 시간을 사용하면, 
사이드카 컨테이너는 정상 종료할 시간이 있기 전에 `SIGTERM` 신호를 받고, 이어서 `SIGKILL` 신호를 받는다.
따라서 종료 코드가 `0`(`0`은 성공적인 종료를 나타냄)이 아니더라도, 이는 파드 종료 시 사이드카 컨테이너에 있어 정상적이며,
...
사이드카 컨테이너는 라이프사이클을 제어하기 위한 [프로브(Probe)](/ko/docs/concepts/workloads/pods/pod-lifecycle/#types-of-probe)를 지원한다.
