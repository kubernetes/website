...
이 특징은 1.6에서 베타를 빠져나왔다. 초기화 컨테이너는 앱 `컨테이너들` 배열과 나란히 
파드 스펙에 명시될 수 있다. 베타 어노테이션의 값은 여전히 존중되며 파드 스펙 필드 값을 덮어쓴다. 
하지만, 베타 어노테이션은 1.6과 1.7에서 데프러케이트(deprecated)되었다.
...
반복적으로 재시작한다. 그러나, 만약 파드가 `재시작 정책`을 절대 하지 않음(Never)으로 설정한다면, 파드는 재시작되지 않는다.

컨테이너를 초기화 컨테이너로 지정하기 위해서는, 파드 스펙에 앱 `컨테이너들` 배열과 나란히 
`초기화 컨테이너들` 필드를 
[컨테이너](/docs/reference/generated/kubernetes-api/{{< param "version" >}}/#container-v1-core)
타입 오브젝트들의 JSON 배열로서 추가한다.
초기화 컨테이너의 상태는 `.상태.초기화 컨테이너 상태들` 필드를 
통해서 컨테이너 상태 배열로 반환된다 (`.상태.컨테이너 상태들`과 
유사하게). 
...
1.8 또는 더 높은 버전에서 사용되어야 한다. 초기화에 대한 선언은 `스펙`으로 옮겨졌다. 
...
실패된다면, 초기화 컨테이너는 파드의 `재시작 정책`에 따라서 재시도 된다. 다만, 
파드의 `재시작 정책`이 항상(Always)으로 설정된 경우, 해당 초기화 컨테이너는 
`재시작 정책`을 실패 시(OnFailure)로 사용한다.
...
`준비성 프로브`가 사용되는 것을 금지한다. 초기화 컨테이너가 완료 상태와 준비성을 
구분해서 정의할 수 없기 때문이다. 이것은 유효성 검사 중에 시행된다.

초기화 컨테이너들이 실패를 영원히 지속하는 상황을 방지하기 위해서 
파드의 `활성 데드라인 초(activeDeadlineSeconds)`와 컨테이너의 `활성화 프로브`를 
사용한다.
...
* 파드 내의 모든 컨테이너들이, 재시작을 강제하는 `재시작 정책`이 항상으로 설정되어 있는, 
  동안 종료되었다. 그리고 초기화 컨테이너의 완료 기록이 가비지 수집 
  때문에 유실되었다.

## 지원 및 호환성

Api서버 버전 1.6.0 또는 더 높은 버전으로 구성된 클러스터는 `.스펙.초기화 컨테이너` 
필드를 사용하여 초기화 컨테이너를 지원한다. 이전 버전들은 초기화 컨테이너를 알파 또는 
베타 어노테이션을 사용하여 지원한다. `.스펙.초기화 컨테이너` 필드는 알파 또는 베타 
...
Api서버 및 Kubelet 버전 1.8.0 이상에서는, 데프러케이트된 어노테이션을 
`.스펙.초기화 컨테이너` 필드로 변환하는 것이 필요한, 알파 및 베타 어노테이션의 지원이 중단되었다.
