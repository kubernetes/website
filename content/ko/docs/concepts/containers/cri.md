...
{{< glossary_tooltip text="파드(Pod)" term_id="pod" >}}와 컨테이너를 
실행할 수 있도록, 
클러스터의 각 노드에는 동작 중인 
...
에러를 보고한다. 컨테이너 런타임이 업그레이드되어 gRPC 재다이얼이 필요한 경우, 
연결이 성공하려면 런타임이 반드시 CRI `v1` API를 지원해야 한다. 
이는 컨테이너 런타임이 올바르게 설정된 후 
kubelet을 재시작해야할 수 있다.

## {{% heading "whatsnext" %}}

- CRI [프로토콜 정의](https://github.com/kubernetes/cri-api/blob/v0.33.1/pkg/apis/runtime/v1/api.proto)를 자세히 알아보자.
