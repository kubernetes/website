...
(바이너리와 혼동되지 않는) 클라우드 컨트롤러 매니저 (CCM) 개념은 본래 클라우드 특유의 밴더 코드와 쿠버네티스 코어가 상호 독립적으로 진화할 수 있도록 해주기 위해 생성되었다. 클라우드 컨트롤러 매니저는 쿠버네티스 컨트롤러 매니저, API 서버, 그리고 스케줄러와 같은 다른 마스터 컴포넌트와 함께 동작된다. 또한 쿠버네티스 위에서 동작하는 경우에는, 쿠버네티스 애드온으로서 구동된다.
...
이 문서는 클라우드 컨트롤러 매니저 이면상의 개념들을 논의하고 그것과 연관되어진 기능들에 대한 세부적인 사항들을 제시한다.
...
CCM은 클라우드와 함께 통합함에 있어 단일 포인트를 생성하기 위해 이전의 세 가지 컴포넌트들로부터 모든 클라우드 종속적인 로직을 통합한다. CCM과 함께하는 새로운 아키텍처는 다음과 같다.
...
{{< note >}}
볼륨 컨트롤러는 의도적으로 CCM의 일부가 되지 않도록 선택되었다. 연관된 복잡성 때문에 그리고 밴더 특유의 볼륨 로직 개념을 일반화 하기 위한 기존의 노력때문에, 볼륨 컨트롤러는 CCM으로 이전되지 않도록 결정되었다.
{{< /note >}}
...
라우트 컨트롤러는 클라우드에서 적합하게 경로를 구성하는 책임을 가지며 쿠버네티스 클러스터 내 상이한 노드 상의 컨테이너들이 상호 소통할 수 있도록 해준다. 라우트 컨트롤러는 오직 구글 컴퓨트 엔진 클러스터에서만 적용가능 하다.
...
PersistentVolumeLabels 컨트롤러는 생성되는 시점에, AWS EBS/GCE PD 볼륨에 대한 레이블을 적용한다. 이것은 사용자로 하여금 이들 볼륨에 대한 수동적 레이블 설정을 불필요하게 만들어 준다.

이들 볼륨은 오직 그것들이 속한 지역/영역 내에서만 동작되도록 제한되어지므로 파드 스케줄에 필수적이다. 이들 볼륨을 이용하는 모든 파드는 동일한 지역/영역 내에서 스케줄 되어져야 한다.  

PersistentVolumeLabels 컨트롤러는 특별하게 CCM을 위해 생성되었다. 즉 CCM이 생성되기 전까지는 존재하지 않는다. 쿠버네티스 API 서버 내 (승인 컨트롤러 였다.) PV 레이블된 로직을 CCM으로 이전하기 위해 행해졌다. KCM 상에서 동작하지 않는다.  
...
이 새로운 모델에서, kubelet은 클라우드 특유의 정보 없이 노드를 초기화 해준다. 그러나, CCM 이 클라우드 특유의 정보를 가지고 노드를 초기화 할때까지 노드가 스케줄 안되도록 해주는 것은 바로 새로 생성된 노드에 taint를 추가하는 것으로 한다. 그러고 나서 이 taint를 제거한다.  
...
클라우드 컨트롤러 매니저는 어떠한 클라우드에서든지 플러그 인 되어 이행될 수 있도록 Go 인터페이스를 이용한다. 특히, [여기](https://github.com/kubernetes/cloud-provider/blob/9b77dc1c384685cb732b3025ed5689dd597a5971/cloud.go#L42-L62)에 정의된 CloudProvider 인터페이스를 이용한다.

위에서 강조되었던 4개의 공유 컨트롤러의 이행, 그리고 공유 cloudprovider 인터페이스와 더불어 일부 골격은 쿠버네티스 코어 내에 유지될 것이다. 클라우드 제공사업자 특유의 이행은 코어의 외부에 탑재되어 코어 내에 정의된 인터페이스를 이행할 것이다.
...
CCM의 코어에 대한 이행은 이벤트를 create 하고, 보안 작업을 보장하기 위한 접근을 요하며, ServiceAccount를 create 하기 위한 접근을 요한다.
