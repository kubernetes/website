...
클라우드 컨트롤러 매니저는 컨트롤 플레인에서 복제된 여러 프로세스로 구성되어 실행된다.
...
1. 노드가 배포되는 리전 및 가용 리소스(CPU, 메모리 등)와 같은 클라우드별 정보로 노드 오브젝트에 어노테이션을 추가하고 레이블을 설정한다.
...
안전한 작동을 보장하기 위해 서비스어카운트를 생성해야 한다.
