...
컨트롤 플레인의 컴포넌트는 클러스터에 대한 전역적인 결정(스케줄링 등)뿐만 아니라,
디플로이먼트의 `{{< glossary_tooltip text="레플리카" term_id="replica" >}}` 필드가 충족되지 않을 때
{{< glossary_tooltip text="파드" term_id="pod">}}를
새로 시작하는 것과 같이 클러스터의 이벤트를 감지하고 대응한다.
...
클라우드 컨트롤러 매니저는 오직 클라우드 제공자에 특화된 컨트롤러만 실행한다.
...
수평 확장(하나 이상의 복제본을 실행)을 할 수 있다.

다음과 같은 컨트롤러는 클라우드 제공자 의존성을 가질 수 있다.

- 노드 컨트롤러(Node controller): 노드가 응답을 멈춘 뒤 클라우드에서 해당 노드가
삭제되었는지를 판단하기 위해 클라우드 제공자를 확인한다.
- 라우트 컨트롤러(Route controller): 클라우드 인프라스트럭처 기반에서 라우트를 설정한다.
- 서비스 컨트롤러(Service controller): 클라우드 제공자의 로드 밸런서를 생성, 업데이트, 삭제한다.

---

## 노드 컴포넌트

노드 컴포넌트는 모든 노드에서 실행되며, 실행중인 파드를 유지하고 쿠버네티스 런타임 환경을 제공한다.
...
다른 애드온들은 반드시 필요하지 않지만, 모든 쿠버네티스 클러스터에는 [클러스터 DNS](/ko/docs/concepts/services-networking/dns-pod-service/)가
있어야 한다. 많은 예제가 이를 기반으로 하기 때문이다.
...
[클러스터 수준 로깅](/ko/docs/concepts/cluster-administration/logging/) 매커니즘은
컨테이너 로그를 중앙 로그 저장소에 저장하고 이를 검색/탐색할 수 있는 인터페이스를 제공한다.
...
쿠버네티스의 핵심 컴포넌트는 일관되게 유지되지만, 그것이 배포되고 관리되는 방식은
...
셀프 호스팅(Self-hosted)
: 컨트롤 플레인이 쿠버네티스 클러스터 자체 내에서 파드로 실행되며, 디플로이먼트(Deployments)와
  스테이트풀셋(StatefulSets) 또는 다른 쿠버네티스 리소스에 의해 관리된다.

매니지드 쿠버네티스 서비스(Managed Kubernetes services)
: 클라우드 제공자는 종종 컨트롤 플레인을 추상화하여, 자사 서비스 제공의 일부로 컴포넌트를 관리해 준다.
...
- 클라우드 제공자는 클라우드 컨트롤러 매니저를 사용하여 쿠버네티스와 깊게 통합할 수 있다.
...
- [클라우드 컨트롤러 매니저](/ko/docs/concepts/architecture/cloud-controller/)를 사용한 클라우드 제공자 통합.
