...
로봇과 자동화에서 _컨트롤 루프_ 는 
시스템 상태를 조절하는 종료되지 않는 루프이다.

여기 컨트롤 루프의 한 예시가 있다: 실내 온도 조절기.

사용자가 온도를 설정하면, 온도 조절기는 사용자가 
*의도한 상태* 를 알려준다. 실제 실내 온도는 
*현재 상태* 이다. 온도 조절기는 장비를 작동하거나 종료해서 
...
컨트롤러는 자체로 작업을 수행할 수 있다. 보다 일반적으로, 
쿠버네티스에서는 컨트롤러가 
{{< glossary_tooltip text="API 서버" term_id="kube-apiserver" >}} 로
유용한 부수적인 효과가 있는 메시지를 발송한다. 사용자는 이 예시를 아래에서 볼수 있다.

{{< comment >}}
네임스페이스 컨트롤러와 같은 일부 내장된 컨트롤러는 사양을 가지지 않는 
오브젝트에 대해 작동한다. 이에 대해 내용을 간단히 하기 위해서 이 페이지에서는 
자세한 설명을 생략한다.
{{< /comment >}}
...
내장 컨트롤러의 예시이다. 내장 컨트롤러는 클러스터 API서버와 
상호 작용하며 상태를 관리한다.
...
잡 컨트롤러가 새 작업을 볼 때, 클러스터 어딘가에서 
노드 셋의 kubelet이 작업을 수행하기 위해 올바른 
수의 파드를 실행하고 있는지 확인한다.
...
다른 컴포넌트는 새 정보
(예약 및 실행해야 하는 새 파드)에 작용되며, 
결국 작업이 완료된다.

새 잡을 생성한 이후에는, 의도한 상태는 해당 잡을 완료하는 것이다.
잡 컨트롤러는 현재 상태를 의도한 상태에 가깝게 
만들며, 사용자가 원하는 잡을 수행하기위해 파드를 생성해서 
잡이 완료에 가까워 지도록 한다.

또한, 컨트롤러는 오브젝트의 설정을 업데이트 한다.
예시: 한 작업이 종료된 잡은 잡 컨트롤러가 
잡 오브젝트를 업데이트해서 `Finished` 로 표시한다.
...
있는지 확인하는 경우, 해당 컨트롤러는 필요할때 새 노드를 설정할 
현재 클러스터 외부의 어떤 무언가를 필요로 한다.
...
쿠버네티스는 시스템에 대한 클라우드-네이티브 뷰를 제공하고, 지속적인
변화를 처리할 수 있다.

어떤 지점에서 작업과 같은 것이 발생할 때마다 클러스터가 
변경 될 수 있으며 컨트롤 루프가 자동으로 오류를 수정한다. 이는 잠재적으로,
클러스터가 안정적인 상태에 도달하지 못하는 것을 의미한다.

클러스터의 컨트롤러가 실행 중이고 유용한 변경을 수행할 수 있는 한,
전체 상태가 안정적이거나 안정적이지 않은지는 중요하지 않다.

## 디자인

쿠버네티스의 디자인 원리는 각각의 클러스터 상태의 특정 측면을 
관리하는 많은 컨트롤러를 사용한다. 가장 일반적으로, 특별히 컨트롤 루프
(컨트롤러)는 의도한 상태로 한 종류의 리소스를 사용하고, 의도한 상태로 
만들기 위해 다른 종류의 리소스를 관리한다.

연결되어 있는 모놀리틱(monolithic) 컨트롤 루프 셋보다, 
간단한 컨트롤러를 사용하는 것이 유용하다. 컨트롤러는 실패할 수 있으므로, 쿠버네티스는 이를 
허용하도록 디자인되었다.

예를 들어, 잡(Job)용 컨트롤러는 잡 오브젝트(새 작업을 
발견하기 위해)와 파드 오브젝트(잡을 실행하고, 완료된 시기를 
확인하기 위해)를 추적한다. 이 경우 다른 작업을 생성하고, 반면에 잡 
컨트롤러는 파드를 생성한다.
...
예를 들어, 디플로이먼트와 잡을 가지고 있다. 이 두가지 모두 파드를 생성한다.
...
내부에서 실행되는 내장된 컨틀로러 셋이 있다. 이
내장 컨트롤러는 중요한 핵심 동작을 제공한다.

디플로이먼트 컨트롤러와 잡 컨트롤러는 쿠버네티스의 
자체("내장" 컨트롤러)로 제공하는 컨트롤러 중 예시이다.
...
소유하고 있는 컨트롤러로 파드 셋을 실행하거나, 
또는 외부에서 쿠버네티스를 실행할 수 있다. 가장 적합한 것은 특정 컨트롤러의 기능에
따라 달라진다.
...
* [쿠버네티스 오브젝트](/ko/docs/concepts/#쿠버네티스-오브젝트)의 몇가지 기본 사항을 알아보자.
* [쿠버네티스 API](/ko/docs/concepts/overview/kubernetes-api/)에 대해 더 배워 보자.
* 만약 자신만의 컨트롤러를 작성하기 원한다면, 쿠버네티스 확장의 [확장 패턴](/docs/concepts/extend-kubernetes/extend-cluster/#extension-patterns)을 본다.
{{% /capture %}}
