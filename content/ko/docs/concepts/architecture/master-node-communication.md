...
그 목적은 사용자로 하여금 untrusted network(외부 네트워크) 상에서
(또는 클라우드 제공사업자 환경에서 완전히 공인 IP로) 동작되어질 수 있는
...
사용가능토록 하여 안전한 HTTPS 포트(443)를 통해 원격 연결에 대해 서비스 리슨하도록 구성되어 진다.
...
예를 들어, 기본 GKE 배포의 경우, kubelet에 제공되어지는 클라이언트 자격증명은
...
apiserver 에 접속하려는 파드는 서비스 계정에 영향력을 발휘함으로써 안전하게
...
그것이 불가능한 경우, 외부 또는 공인 네트워크에 대한 연결을 피하고 싶다면,
...
[Kubelet 인증 그리고/또는 인가](/docs/admin/kubelet-authentication-authorization/)가 활성화 되어져야만 한다.
...
이러한 연결들은 외부 그리고/또는 공인 네트워크에서 동작하기에
**현재로서는 안전하지 않다**.
