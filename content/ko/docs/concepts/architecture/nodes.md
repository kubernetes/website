...
- `--node-ip` - (선택 사항) 노드의 IP 주소를 쉼표로 구분하여 나열한다.
  각 주소 집합에 대해 하나의 주소만 지정할 수 있다.
  예를 들어 단일 스택 IPv4 클러스터의 경우,
  이 값을 kubelet이 노드에 사용해야 하는 IPv4 주소로 설정한다.
  이중 스택 클러스터 실행에 대한 자세한 내용은
  [IPv4/IPv6 이중 스택 구성](/ko/docs/concepts/services-networking/dual-stack/#ipv4-ipv6-이중-스택-구성)을 참조한다.

  이 인자를 제공하지 않으면 kubelet은 노드의 IPv4 주소가 있는 경우에 기본 IPv4 주소를 사용한다.
  노드에 IPv4 주소가 없으면 kubelet은 노드의 기본 IPv6 주소를 사용한다.
...
할당된 새 레이블에 대해 테인트될 수 있는 반면 해당 파드와 호환되지 않는 다른 파드는
...
[테인트](/ko/docs/concepts/scheduling-eviction/taint-and-toleration/)를 생성한다.
예를 들어 준비 상태의 `status`가 kube-controller-manager의 `NodeMonitorGracePeriod`(기본값 40초)
보다 오랫동안 `Unknown` 또는 `False` 상태로 유지되는 경우
`Unknown` 상태의 `node.kubernetes.io/unreachable` 테인트
또는 `False` 상태의 `node.kubernetes.io/not-ready` 테인트가 노드에 추가된다.

이러한 테인트는 스케줄러가 노드에 파드를 할당할 때 노드의 테인트를 고려하기 때문에 보류 중인 파드에 영향을 미친다.
노드에 예약된 기존 파드는 `NoExecute` 테인트 적용으로 인해 축출될 수 있다.
파드에 특정 테인트가 있더라도,
노드에서 예약되고 계속 실행할 수 있도록 하는 {{< glossary_tooltip text="톨러레이션" term_id="toleration" >}} 또한 가질 수 있다.

자세한 내용은 [테인트 기반 축출](/ko/docs/concepts/scheduling-eviction/taint-and-toleration/#테인트-기반-축출)과
[컨디션별 노드 테인트하기](/ko/docs/concepts/scheduling-eviction/taint-and-toleration/#컨디션-condition-을-기준으로-노드-테인트하기)를
참조한다.
...
* `kube-node-lease`
  ...
  내의 [리스(Lease)](/ko/docs/concepts/architecture/leases/)
...
일반 [파드 종료 프로세스](/ko/docs/concepts/workloads/pods/pod-lifecycle/#pod-termination)를 따르도록 한다.
노드 종료 중에 kubelet은 새 파드(이 파드가 이미 노드에 바인딩되어 있더라도)를
받아들이지 않는다.
...
systemd가 노드 종료를 감지하거나 통보하면 kubelet은 노드의 상태를 `NotReady`로 설정하고,
`reason`을 `"node is shutting down"`로 설정한다.
kube-scheduler는 이 상태를 준수하여 영향 받는 노드에 파드를 예약하지 않는다.
다른 서드 파티 스케줄러도 동일한 로직을 따라야 한다.
이는 새 파드가 노드에 예약되지 않으므로 시작되지 않는다는 것을 의미한다.

kubelet **또한** 진행 중인 노드 종료가 감지된 경우 `PodAdmission` 단계에서 파드를 거부하므로,
`node.kubernetes.io/not-ready:NoSchedule`에 대한
{{< glossary_tooltip text="톨러레이션" term_id="toleration" >}}이
 있는 파드 또한 노드에서 시작되지 않는다.

kubelet이 API를 통해 노드 상태를 설정하는 동시에
kubelet은 로컬에서 실행 중인 모든 Pod도 종료하기 시작한다.
...
시스템 또는 관리자가 가능한 경우에
수동으로 노드 종료를 취소한 경우가 있다.
두 경우 모두 노드가 `Ready` 상태로 되돌아간다.
그러나 이미 종료 프로세스를 시작한 파드는
kubelet에 의해 복원되지 않으므로 다시 예약해야 한다.
...
[kubelet 환경 설정](/docs/reference/config-api/kubelet-config.v1beta1/) 안의
...
명령이 kubelet에서 사용하는 억제 잠금 메커니즘을 트리거하지 않거나 사용자 오류(예: ShutdownGracePeriod와 ShutdownGracePeriodCriticalPods가 적절하게 구성되지 않은 경우)로 인해
kubelet의 Node Shutdown Manager에서 노드 종료 작업이 감지되지 않을 수 있다.
자세한 내용은 위 섹션의 [그레이스풀 노드 셧다운](#graceful-node-shutdown)를 참조한다.
노드가 종료되었지만 kubelet의 Node Shutdown Manager에 탐지되지 않으면,
{{< glossary_tooltip text="스테이트풀셋" term_id="statefulset" >}}의 일부인 파드가 종료 상태로 종료 노드에 고착되어 새로이 실행하는 노드로 이동할 수 없다.

이는 종료 노드의 kubelet이 파드를 삭제할 수 없기 때문에
스테이트풀셋은 새로운 노드에 동일한 이름의 파드를 생성할 수 없기 때문이다.
파드에서 사용되는 볼륨이 있는 경우,
VolumeAttachments가 원래의 종료중인 노드에서 삭제되지 않아
이 볼륨을 사용하는 새로이 실행하는 노드에 연결할 수 없다.
결과적으로,
스테이트풀셋에서 실행되는 애플리케이션은 올바른 동작을 할 수 없다.
원본 종료 노드가 나타나면,
kubelet에 의해 파드가 삭제되고 실행 중인 다른 노드에 새로운 파드가 생성된다.
원본 종료 노드가 나타나지 않으면,
이러한 파드는 영원히 종료 노드에서 종료 상태로 고착된다.

위의 상황을 줄이기 위하여, 사용자는 수동으로 테인트 `node.kubernetes.io/out-of-service`와 함께 `NoExecute` 또는 `NoSchedule`를 추가하여
노드가 서비스 중단임을 표시하는 효과를 줄 수 있다.
{{< glossary_tooltip text="kube-controller-manager" term_id="kube-controller-manager" >}}에서
`NodeOutOfServiceVolumeDetach`[기능 게이트](/ko/docs/reference/command-line-tools-reference/feature-gates/)가 사용되고,
이 테인트와 함께 사용 중단 상태로 표시된 노드가 매칭되는 톨러레이션이 없으면,
파드는 강제로 삭제되고 노드에서 종료되는 파드에 대한 볼륨 분리 작업이 즉시 진행된다.
이 사용으로 서비스 중단 노드의 파드는 다른 노드에서 신속하게 복구할 수 있다.

논 그레이스풀 셧다운의 경우, 파드는 두 단계로 종료된다.

1. `out-of-service` 톨러레이션과 일치하지 않는 파드를 강제 삭제한다.
2. 이러한 파드에 대해 볼륨 분리 작업이 즉시 실행된다.

{{< note >}}
- `node.kubernetes.io/out-of-service` 테인트를 추가하기 전에,
  노드가 이미 종료되었거나 재시작 중이 아닌 전원이 꺼져있는 상태인지 확인해야 한다.
- 사용자는 파드가 새로운 노드로 이동된 후
  수동으로 사용 중지 서비스 테인트를 제거해야 하고,
  사용자가 처음에 테인트를 추가했기 때문에
  종료 노드가 복구되었는지 확인해야 한다.
{{< /note >}}
...
[구성 설정](/docs/reference/config-api/kubelet-config.v1beta1/)에서 `failSwapOn`가
false로 지정되어야 한다.
...
아래에 대하여 자세히 알아보기
* 노드를 구성하는 [컴포넌트](/ko/docs/concepts/overview/components/#노드-컴포넌트)
* [노드에 대한 API 정의](/docs/reference/generated/kubernetes-api/{{< param "version" >}}/#node-v1-core)
* 아키텍처 디자인 문서의 [노드](https://git.k8s.io/design-proposals-archive/architecture/architecture.md#the-kubernetes-node) 섹션
* [테인트와 톨러레이션](/ko/docs/concepts/scheduling-eviction/taint-and-toleration/)
* [노드 리소스 매니저](/ko/docs/concepts/policy/node-resource-managers/).
* [윈도우 노드의 자원 관리](/ko/docs/concepts/configuration/windows-resource-management/).
