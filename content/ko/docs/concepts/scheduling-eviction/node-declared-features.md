...
Kubernetes 노드는 새로 추가되었거나 기능 게이트가 적용된 특정 기능의 사용 가능 여부를 보고하기 위해 선언된 기능을 사용합니다. 컨트롤 플레인 구성 요소는 이 정보를 활용해 더 나은 결정을 내립니다. kube-scheduler는 NodeDeclaredFeatures 플러그인을 통해 파드가 요구하는 기능을 명시적으로 지원하는 노드에만 파드가 배치되도록 보장합니다. 또한 NodeDeclaredFeatureValidator 어드미션 컨트롤러는 노드의 선언된 기능을 기준으로 파드 업데이트를 검증합니다.
