...
스케줄러 우회 없이 특정 노드에 파드를 배정하기 위해서는 [노드 어피니티](#노드-어피니티-node-affinity) 또는 [노드 셀렉터](#노드셀렉터nodeselector-nodeselector)를 사용할 수 있다.
{{</ note >}}
...
{{<note>}}
`Gt`와 `Lt` 오퍼레이터는 정수가 아닌 값과는 작동하지 않는다.
