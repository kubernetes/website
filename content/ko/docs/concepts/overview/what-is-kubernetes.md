...
가상화를 사용하면 물리 서버에서 리소스를 보다 효율적으로 활용할 수 있으며, 쉽게 애플리케이션을 추가하거나 업데이트할 수 있고 하드웨어 비용을 절감할 수 있어 더 나은 확장성을 제공한다. 가상화를 통해 물리 리소스의 집합을 일회용 가상 머신 클러스터로 제공할 수 있다.
...
쿠버네티스에는 많은 기능이 있어 점점 인기를 끌고 있다. 다음과 같다.
...
컨테이너화된 작업을 실행하는데 사용할 수 있는 쿠버네티스 클러스터 노드를 제공할 수 있다. 각 컨테이너가 필요로 하는 CPU와 메모리 (RAM)를 쿠버네티스에게 알려줄 수 있다. 쿠버네티스는 리소스를 최대한 사용할 수 있도록 컨테이너를 노드에 배치할 수 있다.
