...
**이름은 동일한 리소스의 모든 [API 버전](/docs/concepts/overview/kubernetes-api/#api-groups-and-versioning)에서 
...
리소스 생성 요청에 `generateName`이 `name` 대신에 제공되면 서버가 이름을 생성한다.
`generateName`을 사용하면, 제공된 값은 이름의 접두사로 사용되고, 서버는 생성된 접미사를 여기에 
붙인다. 이름이 생성되더라도 기존 이름과 충돌하여 HTTP 409 응답이 발생할 수 있다. 이는
쿠버네티스 v1.31 이상의 버전에서 서버가 HTTP 409 응답을 반환하기 전에 고유한 이름을 
최대 8번까지 생성하려고 시도하므로 이러한 문제 발생할 가능성이 훨씬 낮아졌다.
