...
- 커맨드를 익히기가 단순, 용이하고 기억하기 쉽다.
- 클러스터를 변경하려면 커맨드가 하나의 단계만 필요하다.

오브젝트 구성과 비교한 단점은

- 커맨드가 변경 검토 프로세스와 통합되지 않는다.
- 커맨드가 변경에 관한 감사 추적을 제공하지 않는다.
- 커맨드가 활성 동작 중인 경우를 제외하고는 레코드의 소스를 제공하지 않는다.
- 커맨드가 새로운 오브젝트 생성을 위한 템플릿을 제공하지 않는다.
...
{{< warning >}}
명령형 `replace` 커맨드는 기존 명세를 새로 제공된 명세로 바꾸고 
구성 파일에서 누락된 오브젝트의 모든 변경 사항을 삭제한다. 
이 방법은 명세가 구성 파일과는 별개로 업데이트되는 리소스 유형에는 
사용하지 말아야한다. 
...
- 오브젝트 구성을 Git과 같은 소스 컨트롤 시스템에 보관할 수 있다.
...
이는 `replace` API 작업을 사용하여 전체 객체 구성을 바꾸는 대신 
`patch` API 작업을 사용하여 인지된 차이만 
작성하는 것이 가능하다.
{{< /note >}}
...
- 선언형 오브젝트 구성은 디렉토리에서 작업하고 오브젝트 별 작업 유형(생성, 패치, 삭제)을 자동으로 감지하는 데 더 나은 지원을 제공한다.

명령형 오브젝트 구성과 비교한 단점은

- 선언형 오브젝트 구성은 예상치 못한 결과를 디버깅하고 이해하기가 더 어렵다.
- diff를 사용한 부분 업데이트는 복잡한 병합 및 패치 작업을 만든다.
