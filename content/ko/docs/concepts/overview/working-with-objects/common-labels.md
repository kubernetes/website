...
레이블을 최대한 활용하려면 모든 리스소 오브젝트에 적용해야 한다.
...
애플리케이션은 때에따라 쿠버네티스 클러스터의 동일한 네임스페이스에 한번 또는 그 이상 설치 할 수 있다.
예를 들어 워드프레스는 다른 드프레스가 설치되어있는 웹사이트에 한번 한번 또는 그 이상 설치할 수 있다.
...
모든 애플리케이션 인스턴스는 고유한 이늠을 가져야 한다.
...
Helm을 이용해서 설치된 데이터베이스(MySQL)을 사용하는 웹 애플리케이션(WordPress)를 좀더 복잡한 애플리케이션을 보자.
다음 식별자는 이애플리케이션을 배포하는데 사용하는 오브젝트의 시작을 보여준다.

WordPress를 배포하는데 다음과 같이 `Deployment`를 시작한다.
...
MySQL은 MySQL이 속한 것과, 상위 애플리케이션에 대한 메타데이터가 `StateFilSet`에 포함되어 노출된다.
...
MySQL을 `StatefulSet`과 `Service`를 이용하면 상위 애플리케이션은 MySQL과 WordPress 모두에 대한 정보가 포함된 것을 알게 된다.
