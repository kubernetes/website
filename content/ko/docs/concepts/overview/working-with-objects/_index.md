...
*쿠버네티스 오브젝트*는 쿠버네티스 시스템의 영구 엔티티이다. 쿠버네티스는 이러한
엔티티를 사용하여 클러스터의 상태를 나타낸다. 구체적으로, 다음을 설명할 수 있다.

* 실행 중인 컨테이너화된 애플리케이션 (그리고 어떤 노드인지)
...
쿠버네티스 시스템에 클러스터의 워크로드가 어떤 모습이어야 하는지를 알려주는 것이며, 이것을
클러스터의 *바라는 상태(desired state)* 라고 한다.

쿠버네티스 객체를 생성, 수정 또는 삭제하려면 [Kubernetes API](/docs/concepts/overview/kubernetes-api/)를
사용해야 한다. `kubectl` 명령줄 인터페이스를 사용하면, 
예를 들어, CLI가 필요한 쿠버네티스 API 호출을 대신 수행한다. 또한 
[클라이언트 라이브러리](/docs/reference/using-api/client-libraries/) 중 하나를 이용해 
...
스펙을 읽고 원하는 애플리케이션 인스턴스 세 개를 시작하며, 
스펙에 맞게 상태를 업데이트 한다. 인스턴스 중 하나라도 실패하면
(상태 변경), 쿠버네티스 시스템은
스펙과 상태 차이에 대응하여 수정 작업을 수행한다. 이 경우에는 
대체 인스턴스를 시작한다.

오브젝트 스펙, 상태, 그리고 메타데이터에 대한 자세한 내용은 
[Kubernetes API Conventions](https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md)을 참고한다.

### 쿠버네티스 오브젝트 설명

쿠버네티스에서 오브젝트를 생성할 때는 원하는 상태를 설명하는 
객체 사양과 객체에 대한 기본 정보 (예: 이름)을 제공해야 한다. 쿠버네티스 API를 
...
다음은 쿠버네티스 배포에 필요한 필드와 객체 사양을 보여주는 매니페스트 
예시이다.
...
* `apiVersion` - 이 객체를 생성하는 데 사용하는 쿠버네티스 API 버전 
* `kind` - 생성하려는 객체 종류 
* `metadata` - `name` 문자열, `UID` 및 선택적인 `namespace`을 포함하여 객체를 고유하게 식별하는 데 도움이 되는 데이터
* `spec` - 객체에 대해 원하는 상태 

오브젝트 `spec`의 정확한 형식은 모든 쿠버네티스 객체마다 다르며, 해당 
객체에 고유한 중첩 필드를 포함한다. [쿠버네티스 API 참조](/docs/reference/kubernetes-api/)를 통해
쿠버네티스를 사용하여 생성할 수 있는 모든 오브젝트의 사양 형식을 찾을 수 있다.

예를 들어, 파드 API 레퍼런스의 
[`spec` 필드](/docs/reference/kubernetes-api/workload-resources/pod-v1/#PodSpec)을 참조한다.
각 파드에 대해, `.spec` 필드는 파드와 원하는 상태 (예: 해당 파드 내 
각 컨테이너의 컨테이너 이미지 이름)을 지정한다.
객체 사양의 또 다른 예로는 스테이트풀셋(StatefulSet) API의 
...
[템플릿](/docs/concepts/workloads/pods/#pod-templates)이 있다. 이 템플릿은 스테이트풀셋 컨트롤러가 스테이트풀셋 사양을 충족하기 위해 
생성할 파드를 설명한다. 
다양한 유형의 오브젝트는 서로 다른 `.status`을 가진다. 다시 말하자면, API 참조 페이지에서는
해당 `.status` 필드의 구조와 각 유형의 객체에 대한 내용을 자세히 설명한다.

{{< note >}}
YAML 구성 파일 작성에 대한 추가 정보는 [구성 모범 사례](/docs/concepts/configuration/overview/)를 
참조한다. 
{{< /note >}} 

## 서버 측 필드 유효성 검사

쿠버네티스 v1.25 부터, API 서버는 객체에서 인식되지 않거나 중복된 필드를 감지하는 서버 측 
...
쿠버네티스를 처음 사용하는 경우 다음 내용을 자세히 읽어보세요.

* 쿠버네티스의 가장 중요한 기본 오브젝트인 [파드](/ko/docs/concepts/workloads/pods/).
* [디플로이먼트](/docs/concepts/workloads/controllers/deployment/) 오브젝트.
* 쿠버네티스 [컨트롤러](/docs/concepts/architecture/controller/).
* [kubectl](/ko/docs/reference/kubectl/) 과 [kubectl commands](/docs/reference/generated/kubectl/kubectl-commands).

[쿠버네티스 오브젝트 관리](/docs/concepts/overview/working-with-objects/object-management/)는
`kubectl` 를 오브젝트 관리하기 위해 사용하는 방법에 대해 설명한다.
아직 없다면, [install kubectl](/docs/tasks/tools/#kubectl)으로 설치한다.

전반적으로 쿠버네티스 API에 대해 배우고 싶다며, 아래를 방문한다:

* [쿠버네티스 API 개요](/ko/docs/reference/using-api/)

쿠버네티스의 객체에 대해 자세히 알아보려면 이 섹션의 다른 페이지를 읽어보세요. 
