...
다른 주요 멀티 테넌시 형태로 서비스형소프트웨어(SaaS) 사업자가 
...
클러스터 레벨의 오브젝트에 대한 접근 권한을 인가하기 위해 사용되는 유사한 오브젝트도 존재하지만, 이는 멀티 테넌트 클러스터에서는 비교적 유용성이 떨어진다.
...
공유하는 것이다. 여기에는 네임스페이스 레이블 관리, 네임스페이스 라이프사이클, 
