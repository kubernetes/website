...
클러스터 사용자 및 워크로드가 자신의 역할을 수행하기 위해 필요한 리소스에 대해서만
...
각 클러스터마다 경우가 다르겠지만, 여기서 적용해 볼 수 있는 보편적 규칙은 다음과 같다.
 ...
 - 와일드카드(wildcard)를 사용한 권한 지정을 할 시에는, 특히 모든 리소스에 대해서는 가능하면 지양한다. 
   ...
   와일드카드를 이용한 권한 지정은 현재 클러스터에 존재하는 모든 오브젝트 타입뿐만 아니라
   앞으로 생성될 모든 오브젝트 타입에 대해서도 권한을 부여한다.
 - 운영자는 `cluster-admin` 계정이 필수로 요구되지 않을시에는 사용을 지양한다. 
   ...
   혼합하여 사용하면 클러스터 리소스를 실수로 수정하는 일을 방지할 수 있다.
...
접근 권한을 암묵적으로 부여한다. 또한 파드는 모든 [서비스어카운트](/docs/reference/access-authn-authz/service-accounts-admin/)로 
실행할 수 있기 때문에, 워크로드 생성 권한을 부여하면 
해당 네임스페이스에 있는 모든 서비스어카운트의 API 접근 수준도 암묵적으로 
부여된다.
...
어떤 사용자 또는 애플리케이션이 임의의 퍼시스턴트볼륨을 생성할 수 있다면, 이 권한에는
`hostPath` 볼륨 생성이 포함되며, 이는 파드가 해당 노드의 호스트 파일시스템에
접근할 수 있게 됨을 의미한다. 이러한 권한을 부여하는 것은 보안 위험이 된다.

호스트 파일시스템에 제한 없이 접근할 수 있는 컨테이너는 다른 컨테이너의 데이터를 읽거나
Kubelet과 같은 시스템 서비스의 자격 증명을 악용하는 등 다양한 방법으로 권한을 에스컬레이션할 수 있다.

다음 대상에만 퍼시스턴트볼륨 오브젝트 생성 권한을 허용해야 한다.

- 해당 권한이 업무상 필요하며 신뢰할 수 있는 사용자(클러스터 운영자)
- 자동 프로비저닝이 설정된 퍼시스턴트볼륨클레임(PersistentVolumeClaim)을 기반으로
  퍼시스턴트볼륨을 생성하는 쿠버네티스 컨트롤 플레인 컴포넌트.
  이는 일반적으로 쿠버네티스 제공자 또는 운영자가 CSI 드라이버를 설치할 때 구성된다.

퍼시스턴트 스토리지에 대한 접근이 필요한 경우, 신뢰할 수 있는 관리자가 퍼시스턴트볼륨을
생성하고, 제한된 권한을 가진 사용자는 퍼시스턴트볼륨클레임을 통해 해당 스토리지에 접근해야 한다.

### 노드의 `proxy` 하위 리소스에 대한 접근

`nodes/proxy` 하위 리소스에 대한 접근 권한을 가지는 사용자는 Kubelet API에 대한 권한도 가지며,
이는 권한을 가지고 있는 노드에 위치한 모든 파드에서 명령어를 실행할 수 있음을 의미한다.
이러한 접근은 감사 로깅 및 어드미션 컨트롤을 우회하므로,
해당 리소스에 대한 권한을 부여할 때에는 주의가 필요하다.
...
서명자가 `kubernetes.io/kube-apiserver-client`인 새로운 클라이언트 인증서를 생성할 수 있게 되며 이를 통해 사용자는 클러스터에 인증할 수 있게 된다.
해당 클라이언트 인증서는 임의의 이름을 가지게 되며, 이에 중복된 쿠버네티스 시스템 컴포넌트도 포함된다.
...
네임스페이스 오브젝트에 대해 **patch** 작업을 수행할 수 있는 사용자는(해당 권한을 가진 롤에 대한 네임스페이스 범위 롤바인딩을 통해) 해당
네임스페이스의 레이블을 수정할 수 있다. 파드 시큐리티 어드미션을 사용하는 클러스터에서는, 사용자가 관리자가 의도한 것보다
더 허용적인 정책으로 네임스페이스를 구성할 수 있게 된다.
네트워크폴리시(NetworkPolicy)를 사용하는 클러스터에서는, 사용자가 레이블을 설정하여 관리자가 의도하지 않은
서비스에 간접적으로 접근할 수 있게 될 수 있다.

## 쿠버네티스 RBAC - 서비스 거부 위험 {#denial-of-service-risks}

### 오브젝트 생성 서비스 거부 {#object-creation-dos}

클러스터 내 오브젝트 생성 권한을 가지는 사용자는 
[쿠버네티스가 사용하는 etcd는 OOM 공격에 취약](https://github.com/kubernetes/kubernetes/issues/107325)에서 언급되었듯이,
오브젝트의 크기 또는 개수로 인해 서비스 거부 상태를 초래할 만큼 큰 오브젝트를 생성할 수 있다.
