...
* 관리자는 네임스페이스에 임시 스토리지 리소스 쿼터를 설정해야 한다.
* 사용자는 파드 명세에 임시 스토리지 리소스의 제한값을 지정해야 한다.
...
이 저장 위치는 kubelet이 아니라 컨테이너 런타임에서 설정해야 한다.
...
컨테이너-레벨 격리의 경우, 컨테이너의 쓰기 가능한 레이어와 로그
사용량이 스토리지 제한을 초과하면, kubelet은 파드를 축출하도록 표시한다.

파드-레벨 격리의 경우, kubelet은 해당 파드의
컨테이너에 대한 제한을 합하여 전체 파드 스토리지 제한을 계산한다.
...
임시 로컬 스토리지에 대해 지원되는 [구성](#configurations)을 참조한다.
{{< /caution >}}
...
사용자(또는 컨테이너)가 `emptyDir` 볼륨 안에 파일을 생성하고,
어떤 프로세스가 그 파일을 연 다음, 파일이 열려 있는 동안 파일을 삭제하면,
삭제된 파일의 inode는 사용자가 해당 파일을 닫을 때까지 유지되지만
kubelet은 해당 공간을 사용 중인 것으로 분류하지 않는다.
...
{{< note >}}
프로젝트 쿼터를 사용하면 스토리지 사용량을 모니터링할 수 있지만, 한도를 강제하지는 않는다.
{{< /note >}}
