...
이 페이지는 쿠버네티스 `퍼시스턴트 볼륨`의 현재 상태에 대해 설명한다. [볼륨](/docs/concepts/storage/volumes/)에 먼저 익숙해질 것을 권장한다. 
...
스토리지 관리와 계산 인스턴스를 관리하는 건 별개의 문제다. `PersistentVolume` 서브시스템은 사용자와 관리자가 어떻게 스토리지가 제공되고 소비되는지 몰라도 되게끔 추상화된 API를 제공한다. 이렇게 하기 위해서, 두가지 새로운 API 리소스을 도입한다: `PersistentVolume`와 `PersistentVolumeClaim`.

`PersistentVolume`(PV)는 클러스터의 스토리지 하나이다. 관리자가 직접 관리할 수도, [Storage Classes](/docs/concepts/storage/storage-classes/)가 자동으로 관리할 수도 있다. 클러스터 안에서 노드가 하나의 클러스터 리소스인 것처럼 똑같이 하나의 클러스터 리소스이다. PV는 `Volume`처럼 볼륨 플러그인이지만, PV를 사용하는 파드와 독립적인 라이프사이클를 갖는다. 이 API 객체는 NFS, iSCSI, 클라우드 프로바이더의 스토리지 시스템 등 스토리지의 세부 구현에 맞게 대응한다. 

`PersistentVolumeClaim`(PVC)는 스토리지에 대한 유저의 요청이다. PVC는 파드와 비슷하다. 파드가 노드 리소스을 소비하듯, PVC는 PV 리소스을 소비한다. 파드가 특정 수준의 리소스(CPU와 Memory)를 요구하듯, PVC는 특정 크기, 접근 모드를 요청한다 (예로, 한 번 마운트되고 읽기/쓰기 또는 여러번 마운트 되고 읽기 전용).

PVC가 사용자가 추상화된 스토리지 리소스를 사용하게 해주지만, 사용자가 필요로 하는 PV의 속성은 변할 수 있다. 다른 문제에는 다른 퍼포먼스의 스토리지가 필요한 것처럼 말이다. 클러스터 관리자는 단순히 크기와 접근 모드 뿐만 아니라 다른 부분에서도 다양한 `PersistentVolumes`를, 사용자가 디테일한 구현에 
신경쓰지 않게 하면서, 제공할 수 있어야 한다. 이러한 요구를 충족시키는 것이 `StorageClass` 리소스이다.

[과정이 자세히 설명되어 있는 예시](/docs/tasks/configure-pod-container/configure-persistent-volume-storage/)를 참고해보자.
