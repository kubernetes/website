...
API 오브젝트를 기반으로 한다. 클러스터 관리자는 볼륨을 프로비저닝하는
...
정의하고 노출할 수 있다. 또한 이 디자인을 통해 최종 사용자는
스토리지 프로비저닝 방식의 복잡성과 뉘앙스에 대해 걱정할 필요가 없다. 하지만,
여전히 여러 스토리지 옵션들을 선택할 수 있다.

자세한 내용은 [스토리지 클래스](/docs/concepts/storage/storage-classes/) 개념을 참고한다.
...
다음 매니페스트는 표준 디스크와 같은 퍼시스턴트 디스크를 프로비저닝하는
스토리지클래스 "slow"를 만든다.
...
다음 매니페스트는 SSD와 같은 퍼시스턴트 디스크를 프로비저닝하는
스토리지클래스 "fast"를 만든다.
...
일치해야 한다. ([동적 프로비저닝 활성화하기](#동적-프로비저닝-활성화하기)를 참고)

예를 들어 "fast" 스토리지클래스를 선택하려면 다음과
...
프로비저닝된다. 클레임이 삭제되면 볼륨이 삭제된다.

## 기본 동작

스토리지클래스를 지정하지 않은 경우 모든 클레임이 동적으로
프로비저닝되도록 클러스터에서 동적 프로비저닝을 활성화할 수 있다. 클러스터 관리자는
이 방법으로 활성화할 수 있다.

- 하나의 `StorageClass` 오브젝트를 *기본(default)*으로 표시한다.
- API 서버에서 [`DefaultStorageClass` 어드미션 컨트롤러](/docs/reference/access-authn-authz/admission-controllers/#defaultstorageclass)를
  사용하도록 설정한다.

관리자는 [`storageclass.kubernetes.io/is-default-class` 어노테이션](/docs/reference/labels-annotations-taints/#storageclass-kubernetes-io-is-default-class)을
추가하여 특정 `StorageClass`를 기본으로 표시할 수 있다.
기본 `StorageClass`가 클러스터에 존재하고 사용자가
`storageClassName`을 지정하지 않은 `PersistentVolumeClaim`을
작성하면, `DefaultStorageClass` 어드미션 컨트롤러가 기본
스토리지클래스를 가리키는 `storageClassName` 필드를 자동으로 추가한다.
...
[다중 영역](/docs/setup/best-practices/multiple-zones/) 클러스터에서 파드는 한 지역 내
여러 영역에 걸쳐 분산될 수 있다. 파드가 스케줄링된 영역에서 단일 영역 스토리지 백엔드를
프로비저닝해야 한다. [볼륨 바인딩 모드](/docs/concepts/storage/storage-classes/#볼륨-바인딩-모드)를
설정해서 수행할 수 있다.
