...
쿠버네티스 API의 쿠버네티스 오브젝트 상태를 메트릭으로 노출할 수 있다.  
...
## 예시: kube-state-metrics 메트릭을 사용하여 클러스터 상태 조회하기 {#example-kube-state-metrics-query-1}

kube-state-metrics에서 생성한 메트릭 시리즈(metric series)는 조회에 사용할 수 있어 클러스터에 대한 추가적인 정보(insights)를 얻는데 도움이 된다.

프로메테우스 또는 그와 같은 쿼리 언어를 사용하는 도구를 사용하는 경우, 아래 PromQL 쿼리는 아직 준비되지 않은 파드의 수를 반환한다.
...
프로메테우스 또는 그와 같은 알림 규칙 언어(alert rule language)를 사용하는 도구를 사용하는 경우, 다음 알람은 `Terminating` 상태가 5분 이상 지속된 파드를 감지할 수 있다.
