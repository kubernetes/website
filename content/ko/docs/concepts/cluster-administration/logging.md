...
이러한 로그 스트림에 직접 엑세스할 수 있도록 허용한다. `stream` 쿼리 문자열으로 스트림 이름 (`Stdout` 또는 `Stderr`)을 
...
컨테이너화된 애플리케이션의 `stdout(표준 출력)` 및 `stderr(표준 에러)` 스트림에 의해 
...
두 개의 kubelet 파라미터 
`containerLogMaxSize`(기본값 10Mi)와 `containerLogMaxSize`(기본값 5)를 구성할 수 있다. 
...
[kubelet 구성 파일](/docs/tasks/administer-cluster/kubelet-config-file/)을 사용하여 
...
특정 프로세스가 이 경로를 암묵적으로 assume할 수 있다는 점은 유의해야 한다.
...
컨테이너는 로그를 stdout과 stderr로 출력하며, 합의된 형식은 없다. 노드-레벨 에이전트는 
