...
생성하거나 수정할 수 있다.
예를 들면, 디플로이먼트를 스케일하거나, 롤링 업데이트를 시작하거나, 파드를 재시작하거나
...
또한, 대시보드가 있는 클러스터에서 기본적으로 `kube-system`
[네임스페이스](/docs/tasks/administer-cluster/namespaces/)에서 기본적으로 동작 중인 시스템 애플리케이션(예를 들면 대시보드 자신)을 볼 수 있다.
...
- **앱 이름** (필수): 애플리케이션 이름.
  ...
  소문자, 숫자 및 대쉬(-)만을 포함해야 한다. 길이는 24자로 제한된다.
  처음과 끝의 스페이스는 무시된다.
...
클러스터에 정의된 쿠버네티스 오브젝트가 있으면, 대시보드는 초기화된 뷰를 제공한다.
기본적으로 _기본_ 네임스페이스의 오브젝트만이 보이는데,
