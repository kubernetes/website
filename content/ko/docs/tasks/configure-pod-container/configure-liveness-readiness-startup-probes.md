...
이런 상태에서 컨테이너를 재시작하면 버그가 있어도 애플리케이션의 가용성을 높일 수 있다. 
...
파드의 `Ready`[조건](/ko/docs/concepts/workloads/pods/pod-lifecycle/#pod-conditions)이 `true`일 때 해당 파드는 준비된 것으로 간주된다. 
파드가 준비되지 않은 경우 서비스 로드 밸런서에서 제거된다.

파드의 `Ready` 조건이 `false`가 되는 경우는 다음과 같다.

- 노드의 `Ready` 조건이 `true`가 아닐 때
- 파드의 `readinessGates` 중 하나가 `false`일 때
- 컨테이너 중 적어도 하나가 준비되지 않았을 때
...
이 실습에서는 `registry.k8s.io/busybox:1.27.2` 이미지를 기반으로 하는 컨테이너를 실행하는 파드를 생성한다. 
다음은 파드의 구성 파일이다.
...
10초 후에 파드 이벤트를 확인하여 liveness 프로브가 실패하고 컨테이너가 재시작되었는지 확인한다.
...
startup 프로브가 계속 실패하면 컨테이너는 300초 후에 종료되고 파드의 `restartPolicy`에 따라 처리된다.
...
HTTP 및 TCP readiness 프로브 구성도 liveness 프로브와 동일하게 유지된다.
