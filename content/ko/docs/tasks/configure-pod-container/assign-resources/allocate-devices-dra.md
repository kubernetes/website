...
{{< glossary_tooltip text="리소스클레임(ResourceClaim)" term_id="resourceclaim" >}} 및
{{< glossary_tooltip text="리소스클레임템플릿(ResourceClaimTemplate)" term_id="resourceclaimtemplate" >}}과
같은 DRA 용어를 숙지한다.
자세한 정보는
...
워크로드 운영자는 리소스클레임 또는 리소스클레임템플릿을 생성하여
워크로드에 대한 디바이스를 _클레임_ 할 수 있다. 워크로드를 배포하면,
...
## 클레임할 디바이스 확인하기 {#identify-devices}
...
특정 디바이스 속성을 필터링함으로써 디바이스를 클레임할 수 있다.
...
## 리소스 클레임하기 {#claim-resources}

{{< glossary_tooltip text="리소스클레임(ResourceClaim)" term_id="resourceclaim" >}}을 사용하여
DeviceClass에서 리소스를 요청할 수 있다.
리소스클레임을 생성하려면, 다음 중 하나를 수행한다.

* 여러 파드가 동일한 디바이스에 대한 접근을 공유하도록 하거나,
  파드의 수명을 넘어 클레임이 존재하도록 하려면
  리소스클레임을 수동으로 생성한다.
* 쿠버네티스가 파드별 리소스클레임을 생성하고 관리하도록 하려면
  {{< glossary_tooltip text="리소스클레임템플릿(ResourceClaimTemplate)" term_id="resourceclaimtemplate" >}}을
  사용한다. 모든 파드가 유사한 구성의 별도 디바이스에
  접근하도록 하려면 리소스클레임템플릿을 생성한다. 예를 들어,
  [병렬 실행](/docs/concepts/workloads/controllers/job/#parallel-jobs)을
  사용하는 잡(Job)에서 파드에 대한 동시 디바이스
  접근이 필요할 수 있다.

파드에서 특정 리소스클레임을 직접 참조하는 경우, 해당 리소스클레임이
클러스터에 이미 존재해야 한다. 참조된 리소스클레임이 존재하지 않으면,
리소스클레임이 생성될 때까지 파드는 보류(pending) 상태로 유지된다.
파드에서 자동 생성된 리소스클레임을 참조할 수 있지만,
자동 생성된 리소스클레임은 생성을 트리거한 파드의
수명에 바인딩되므로 권장되지 않는다.

리소스를 클레임하는 워크로드를 생성하려면, 다음 옵션 중 하나를 선택한다.
...
모두 일치하는 디바이스를 요청하는 리소스클레임템플릿을 생성한다.

  * `driver.example.com/type` 속성의 값이
    `gpu`인 디바이스.
  * 용량이 `64Gi`인 디바이스.

리소스클레임템플릿을 생성하려면, 다음 명령을 실행한다.
...
이 매니페스트는 `example-device-class` DeviceClass에서 다음 두 파라미터에
모두 일치하는 디바이스를 요청하는 리소스클레임을 생성한다.

  * `driver.example.com/type` 속성의 값이
    `gpu`인 디바이스.
  * 용량이 `64Gi`인 디바이스.

리소스클레임을 생성하려면, 다음 명령을 실행한다.
...
리소스클레임 또는 리소스클레임템플릿을 지정한다. 그런 다음, 해당 파드의
...
1. 다음 예시 잡을 확인한다.
   ...
   * `separate-gpu-claim`이라는 리소스클레임템플릿과
     `shared-gpu-claim`이라는 리소스클레임을 컨테이너에서 사용할 수 있도록 한다.
   * 다음 컨테이너를 실행한다.
       * `container0`은 `separate-gpu-claim`
         리소스클레임템플릿의 디바이스를 요청한다.
       * `container1`과 `container2`는 `shared-gpu-claim`
         리소스클레임의 디바이스에 대한 접근을 공유한다.
