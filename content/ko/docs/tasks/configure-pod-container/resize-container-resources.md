---
title: 컨테이너에 할당된 CPU 및 메모리 리소스 리사이즈
content_type: task
weight: 30
...
인플레이스(In-place) 파드 리사이즈를 사용하면 실행 중인 파드 내에서 컨테이너의 CPU/메모리 할당량을 변경할 수 있으며,
애플리케이션 중단을 피할 수 있다. 파드 리소스 리사이즈에 대한 프로세스는 [파드에 할당된 CPU 및 메모리 리소스 리사이즈](/docs/tasks/configure-pod-container/resize-pod-resources)에서 다룬다.
...
* **리사이즈 트리거(Triggering a Resize):** 파드 명세에서 원하는 `requests`와
  `limits`를 업데이트하여 리사이즈를 요청할 수 있다.
...
kubelet은 리사이즈 요청의 상태를 나타내기 위해 파드의 상태 조건을 업데이트한다.

* `type: PodResizePending`: kubelet이 요청을 즉시 승인할 수 없다.
  `message` 필드에서 이유에 대한 설명을 제공한다.
    * `reason: Infeasible`: 요청된 리사이즈가 현재 노드에서 불가능하다(예: 노드가 가진 것보다 더 많은 리소스 요청).
    * `reason: Deferred`: 요청된 리사이즈가
    현재는 불가능하지만, 나중에 가능할 수 있다
    (예: 다른 파드가 제거된 경우).
    kubelet이 리사이즈를 재시도한다.
* `type: PodResizeInProgress`: kubelet이 리사이즈를 승인하고 리소스를
  할당했지만, 변경사항이 아직 적용 중이다.
  이는 보통 짧은 시간이지만 리소스 유형과 런타임 동작에 따라 더 오래 걸릴 수 있다.
  실행 중 오류는 `message` 필드에 (`reason: Error`와 함께) 보고된다.

### Kubelet의 지연된 리사이즈 재시도 방식

요청된 리사이즈가 _Deferred_ 으로 표시되면, kubelet은 주기적으로 리사이즈를 재시도한다.
예를 들어, 다른 파드가 제거되거나 스케일 다운되는 경우에 해당한다. 지연된 리사이즈가
여러 개인 경우, 다음 우선순위에 따라 재시도를 한다.

* 우선순위가 높은 파드(PriorityClass 기준)의 리사이즈 요청이 먼저 재시도된다.
* 두 파드의 우선순위가 같은 경우, Guaranteed 파드의 리사이즈가 Burstable 파드의 리사이즈보다 먼저 재시도된다.
...
* 최상위 `status.observedGeneration` 필드는 kubelet이 승인한 최신 파드 사양에 해당하는 `metadata.generation`을 보여준다. 이 필드를 사용하여 kubelet이 처리한 가장 최근의 리사이즈 요청을 확인할 수 있다.
* `PodResizeInProgress` 상태에서 `conditions[].observedGeneration` 필드는 현재 진행 중인 리사이즈가 시작되었을 때의 파드 사양에서 `metadata.generation`을 나타낸다.
* `PodResizePending` 상태에서 `conditions[].observedGeneration` 필드는 보류 중인 리사이즈에 대한 할당이 마지막으로 시도되었을 때의 파드 사양에서 `metadata.generation`을 나타낸다.

## 컨테이너 리사이즈 정책

컨테이너 명세에서 `resizePolicy`를 설정하여 크기를 조정할 때 컨테이너를
재시작해야 하는지 여부를 제어할 수 있다.
이를 통해 리소스 유형(CPU 또는 메모리)에 따른 세밀한 제어가 가능하다.
...
그런 파드에서는 재시작이 필요한 리사이즈 정책을 구성할 수 없다.
{{< /note >}}
...
쿠버네티스 {{< skew currentVersion >}}에서 파드 리소스의 인플레이스 리사이즈에는 다음 제한사항이 있다.

* **리소스 유형:** CPU와 메모리 리소스만 크기를 조정할 수 있다.
* **메모리 감소:** 메모리 리사이즈 후 재시작 정책이 `NotRequired`(또는 지정되지 않음)인 경우, kubelet은 메모리 제한을 줄일 때
  oom-kill을 방지하기 위해 최선을 다하지만, 어떠한 보장도 제공하지 않는다.
  컨테이너의 메모리 제한을 줄이기 전에 메모리 사용량이 요청된 제한을 초과하면 리사이즈가 건너뛰어지고
...
* **운영 체제:** 윈도우 파드는 인플레이스 리사이즈를 지원하지 않는다.
...
* **스왑:** [스왑 메모리](/docs/concepts/architecture/nodes/#swap-memory)를 활용하는 파드는 메모리에 대한
  `resizePolicy`가 `RestartContainer`가 아닌 경우 메모리 요청량을 리사이즈할 수 없다.
...
## 예시 1: 재시작 없는 CPU 리사이즈

먼저, 인플레이스 CPU 리사이즈와 재시작이 필요한 메모리 리사이즈를 위해 설계된 파드를 생성한다.
...
# kubectl -n qos-example apply -f <수정된-매니페스트> --subresource resize --server-side
```
...
* `status.containerStatuses[0].resources`도 `cpu: 800m`를 보여주며, 노드에서 리사이즈가 성공했음을 나타낸다.
* `status.containerStatuses[0].restartCount`는 `0`으로 유지된다. CPU `resizePolicy`가 `NotRequired`였기 때문이다.

## 예시 2: 재시작이 있는 메모리 리사이즈
...
* 중요한 것은, 실행 불가능한 리사이즈가 kubelet에 의해 적용되지 않았기 때문에 `status.containerStatuses[0].resources`는
...
네임스페이스를 삭제한다. 이렇게 하면 이 작업을 위해 생성한 모든 파드를 삭제한다.
...
* [컨테이너 및 파드 메모리 리소스 할당](/docs/tasks/configure-pod-container/assign-memory-resource/)
...
* [네임스페이스에 대한 기본 메모리 요청량과 제한량 구성](/docs/tasks/administer-cluster/manage-resources/memory-default-namespace/)

* [네임스페이스에 대한 기본 CPU 요청량과 제한량 구성](/docs/tasks/administer-cluster/manage-resources/cpu-default-namespace/)

* [네임스페이스에 대한 최소 및 최대 메모리 제약 조건 구성](/docs/tasks/administer-cluster/manage-resources/memory-constraint-namespace/)

* [네임스페이스에 대한 최소 및 최대 CPU 제약 조건 구성](/docs/tasks/administer-cluster/manage-resources/cpu-constraint-namespace/)
