...
이 페이지는 {{<glossary_tooltip term_id="daemonset" text="데몬셋(DaemonSet)">}}의 일부로써, 특정 {{<glossary_tooltip term_id="node" text="노드">}}에서만 {{<glossary_tooltip term_id="pod" text="파드">}}를 실행하는 방법을 보여준다.
...
{{<glossary_tooltip term_id="daemonset" text="데몬셋">}}을 실행할려고 하는데, 로컬 솔리드 스테이트 드라이브(SSD) 
...
노드에 레이블을 지정하면 컨트롤 플레인(특히, 데몬셋 컨트롤러)이 자동으로 해당 노드에서 
새로운 데몬 파드를 실행하도록 트리거된다..
