...
출력 결과는 quota-object-example 네임스페이스에서 퍼시스턴트볼륨클레임은 최대 1개, LoadBalancer 타입 서비스는 최대 2개, 그리고 NodePort 타입 서비스는
허용되지 않음을 보여준다.
...
출력 결과는 네임스페이스의 쿼터를 초과하기 때문에 두 번째 퍼시스턴트볼륨클레임이 생성되지 않았음을 보여준다.
...
다음은 쿼터로 제한할 수 있는 API 리소스를 식별하는 문자열이다.
