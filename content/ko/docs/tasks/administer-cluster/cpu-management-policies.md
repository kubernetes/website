...
* [`none`](#none-정책-구성)은 기본 정책이다.
* [`static`](#static-정책-구성)은 특정 리소스 특성을 가진 파드가 노드에서 더 높은 CPU 어피니티와
...
`--node-status-update-frequency`와 동일한 기간이 기본값으로 사용된다.
...
이 기능 게이트들은 개별 옵션에 기능 게이트를 추가하는 것이 너무 번거롭기 때문에
옵션 그룹 단위로 보호한다.
...
{{< note >}}
노드에서 온라인 CPU 집합이 변경되면, 노드를 드레인하고 kubelet 루트 디렉터리에 있는
상태 파일 `cpu_manager_state`를 삭제하여 CPU 매니저를 수동으로 재설정해야 한다.
{{< /note >}}
...
켜고 끌 수 있다.
* `CPUManagerPolicyBetaOptions`는 기본적으로 활성화되어 있다. 베타 수준 옵션을 숨기려면 이 기능 게이트를 비활성화한다.
* `CPUManagerPolicyAlphaOptions`는 기본적으로 비활성화되어 있다. 알파 수준 옵션을 표시하려면 이 기능 게이트를 활성화한다.
...
옵션을 함께 사용하면, kubelet이 시작에 실패하며 로그에 오류 설명이
표시된다.
...
* [파드 수준 리소스 매니저](/docs/concepts/resource-management/resource-managers/#pod-level-resource-managers)에 대해 알아본다.
