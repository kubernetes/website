...
내용은 [Reviewing changes](/ko/docs/contribute/review/)를 참고한다.
...
- PR 기여자들이 [CLA](https://github.com/kubernetes/community/blob/master/CLA.md)에 서명했는지 확인한다.
  - 아직 CLA에 서명하지 않은 기여자들에게 CLA에 서명을 요청하기 위해 [이](https://github.com/zparnold/k8s-docs-pr-botherer) 
    스크립트를 사용한다.
...
- 또한 [참조 문서 생성시](https://github.com/kubernetes-sigs/reference-docs) 코드에 대한 풀 리퀘스트가 있는지 확인하고, 
  직접 리뷰하거나 (필요하다면) 도움을 요청한다.
- 매일 새로 올라오는 이슈를 심사하고 태그를 지정하는 
  [issue wrangler](/ko/docs/contribute/participate/issue-wrangler/)를 보조한다.
  [이슈 심사와 분류](/ko/docs/contribute/review/for-approvers/#triage-and-categorize-issues)를 
  참고하여 SIG Docs가 메타데이터를 사용하는 방법에 대한 지침을 확인한다.

{{< note >}}
PR 랭글러 업무는 현지화PR (영문이 아닌 PR)에는 적용되지 않는다.
현지화 팀은 해당 언어 PR 리뷰를 위한 자체적인 프로세스와 팀을 보유하고 있다.
하지만 현지화 PR의 레이블이 정확하게 지정되었는지 확인하거나,
언어와 무관한 작은 PR(링크 업데이트 등)을 리뷰하거나,
또는 오랫동안 방치된 PR
(6개월보다 더 이전에 생성되었고 한 달 이상 업데이트가 없는 PR)의 리뷰어나 기여자를 태그한다.
{{< /note >}}
...
- [메인 브랜치 이외의 브랜치에 대한 PR](https://github.com/kubernetes/website/pulls?q=is%3Aopen+is%3Apr+label%3Alanguage%2Fen+-base%3Amain): 
  `dev-` 브랜치에 대한 것일 경우, 곧 출시될 예정인 릴리스이다. `/assign @<meister's_github-username>` 을 사용하여 
...
- 다른 사람들은 특정 주간에 배정된 PR 랭글러의 섀도류 기여를 요청하기 위해 [#sig-docs Slack channel](https://kubernetes.slack.com/messages/sig-docs)
