...
- 품질 및 [스타일](/docs/contribute/style/style-guide/)과 
  ...
  - 가장 작은 PR(`size/XS`)부터 시작하고, 가장 큰(`size/XXL`) PR까지 리뷰한다. 
    가능한 한 많은 PR을 리뷰한다.
- PR 기여자들이 [CLA](https://github.com/kubernetes/community/blob/main/CLA.md)에 서명했는지 확인한다.
...
- 제안된 변경 사항에 대한 피드백을 제공하고 다른 SIG의 멤버에게 기술 리뷰를 요청한다.
  ...
  - 또한, PR에 `@kubernetes/<sig>-pr-reviews` 라는 코멘트를 남겨 [SIG](https://github.com/kubernetes/community/blob/main/sig-list.md)가 
    리뷰하도록 태그할 수 있다.
