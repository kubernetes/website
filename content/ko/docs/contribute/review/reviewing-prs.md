...
- 쿠버네티스 문서화 커뮤니티의 다양한
  [역할과 책임](/docs/contribute/participate/#역할과-책임)을
  이해한다.
...
- 언어나 문법에 명백한 오류가 있는가? 무언가를 표현하는 더 좋은 방법이 있는가?
  ...
  - 문서의 경로를 _이동_한 PR이 있는지 주의한다.
    ...
    보통 기여자에게 _이동된_ 문서에서 발견될 수 있는 모든 문법이나 철자 오류를 수정하도록 요청하는 것은 지양한다.
...
어떤 기준으로 게시물이 evergreen 인지 판단하는지에 대해서도 숙지해야한다.
...
일부 블로그 게시물은 반드시 유지 관리되어야 하며, 우리는 항상 릴리스 공지를 항상 evergreen 으로 표시한다. 이 점을 어떻게 검토해야 할지 확실하지 않은 경우 다른 기여자에게 문의한다.
...
마크다운 소스가 올바른 [페이지 콘텐츠 유형](/docs/contribute/style/page-content-types/) 및 / 또는 `레이아웃`을 사용하고 있는지 확인한다.
