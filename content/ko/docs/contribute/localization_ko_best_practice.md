...
### 리뷰 반영 과정에서는 추가 커밋을 작성하여 변경 사항을 쉽게 확인할 수 있도록 한다.
...
### Github Profile Name은 가급적 영문으로 한다.

- `kubernetes/website` 저장소는 한국어뿐만 아니라 여러 언어가 함께 사용되는 저장소이기 때문이다.

---

### 이슈, 풀 리퀘스트, 커밋 메시지는 영문으로 작성한다.
...
### 영어 원문과 유사한 위치에서 개행하여 전체 라인 수를 동일하게 유지한다.
...
### 영문 병기는 기본적으로 문서에서 해당 용어가 처음 등장할 때 한 번만 표기해도 된다.
