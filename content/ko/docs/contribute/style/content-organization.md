...
이를 바탕으로, 페이지나 섹션을 위로 올리고 싶다면, 페이지의 프론트 매터(front matter)에 weight 값을 설정한다.
...
파일의 프론트 매터에 `main_menu` 플래그가 설정된 `docs/` 아래의 섹션들로 구성된다.
...
섹션이나 페이지를 목록에 표시하고 싶지 않다면, 프론트 매터에서 `toc_hide` 플래그를 `true`로 설정한다.
...
또 다른 널리 사용되는 예로 `includes` 번들이 있다. 이 번들은 프론트 매터에 
...
* 번들에 포함된 모든 파일은 Hugo에서 `Resources`라고 부르는 대상이며,
  프론트 매터(YAML 파일 등)을 지원하지 않는 경우에도
