...
- 쿠버네티스 조직의 **맴버** 는 쿠버네티스 프로젝트에 시간과 노력을 투자 한 기여자이다. 일반적으로 승인되는 변경이 되는 풀 리퀘스트를 연다. 맴버십 기준은 [커뮤니티 맴버십](https://github.com/kubernetes/community/blob/master/community-membership.md)을 참조한다.
...
- SIG Docs의 **승인자** 는 프로젝트에 대한 지속적인 헌신을 보여준 
  ...
  릴리즈 조정과 같은 SIG Docs 승인자의 일부 의무는
  상당한 시간을 전념을 필요로 한다.
...
- [슬랙](http://slack.k8s.io/) 또는 [SIG docs 메일링 리스트](https://groups.google.com/forum/#!forum/kubernetes-sig-docs)에 개선 할 아이디어를 제시한다.
...
품질을 높히기 위해 풀 리퀘스트를 SIG Docs 승인자가 병헙하는 것을 제한한다.
작동 방식은 다음과 같다.

- 풀 리퀘스트에 `lgtm` 과 `approve` 레이블이 있고, `hold` 레이블이 없고,
  모든 테스트가 통과되면 풀 리퀘스트는 자동적으로 병헙된다.
...
- SIG Docs 승인자들만이 코멘트에 `/approve` 를
  추가해서 풀 리퀘스트를 병헙할 수 있다. 일부 승인자들은
