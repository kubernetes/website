...
폭넓은 인기와 규모를 막론한 수많은 기업들의 광범위한 사용, 그리고 메인테이너들의 거듭된 도움 요청에도 불구하고, 인그레스 NGINX 프로젝트는 그토록 절실히 필요로 했던 기여자들을 끝내 확보하지 못했다. Datadog 내부 조사에 따르면 현재 클라우드 네이티브 환경의 약 50%가 이 도구에 의존하고 있으나, 지난 몇 년 동안 이 프로젝트는 개인 시간을 들여 작업하는 단 한두 명에 의해서만 유지보수되어 왔다. 우리와 사용자 모두가 안전하다고 여길 수준으로 도구를 유지보수할 인력이 충분하지 않은 상황에서, 책임 있는 선택은 프로젝트를 단계적으로 종료하고 [게이트웨이 API](https://gateway-api.sigs.k8s.io/guides/getting-started/)와 같은 현대적인 대안에 다시 역량을 집중하는 것이다.
...
우리는 이 변화의 규모와, 이 문제를 방치할 경우 상당수의 쿠버네티스 사용자에게 닥칠 수 있는 심각한 위험을 다시 한번 강조하기 위해 공동으로 이 성명을 발표한다. 지금 즉시 클러스터를 점검해야 한다. 인그레스 NGINX에 의존하고 있다면, 반드시 마이그레이션 계획을 세우기 시작해야 한다.
