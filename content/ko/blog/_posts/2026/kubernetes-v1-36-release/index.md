...
이번 릴리스에는 [사용 중단 및 제거](#deprecations-removals-and-community-updates)도 있으니 꼭 확인하시기 바랍니다.
...
우리는 계절이 바뀌고 산 위의 빛이 달라지는 때에 찾아온 쿠버네티스 v1.36과 함께 2026년을 시작합니다. ハル(_Haru_)는 여러 의미를 담은 일본어 소리입니다. 그중에서도 우리가 특히 주목한 의미는 春(봄), 晴れ(_hare_, 맑은 하늘), 遥か(_haruka_, 아득한, 먼)입니다. 계절, 하늘, 지평선. 이 세 가지는 이어지는 이야기 곳곳에 담겨 있습니다.
...
필드를 베타로 승격하여 이 기능을 확장합니다. 이 필드는 특수 하드웨어 전반의 상태를
한곳에서 보고할 수 있게 합니다.
...
스케줄 가능한 최소 파드 수를 요구하는 방식으로 [갱 스케줄링](/docs/concepts/scheduling-eviction/gang-scheduling/)을 지원했습니다.
v1.36은 여기에서 더 나아가 전체 그룹을 원자적으로 평가하는 새로운 PodGroup 스케줄링 주기를 도입합니다.
그룹의 모든 파드가 함께 바인딩되거나, 아무 파드도 바인딩되지 않습니다.
...
크래시 일관성 복구 지점을 바탕으로 워크로드를 복구할 수 있게 하는 것입니다.

이 작업은 SIG Storage가 주도한 [KEP \#3476](https://kep.k8s.io/3476)의 일환으로 진행되었습니다.

### 변경 가능한 볼륨 연결 한도

쿠버네티스 v1.36에서 _변경 가능한 `CSINode` 할당 가능_ 기능이 스테이블로 승격되었습니다.
...
### ServiceAccount 토큰 외부 서명 API {#api-for-external-signing-of-service-account-tokens}

쿠버네티스 v1.36에서 서비스 어카운트용 _외부 ServiceAccount 토큰 서명자_ 기능이 스테이블로 승격되어,
...
v1.36에서 GA가 되면서 외부 ServiceAccount 토큰 서명은
...
쿠버네티스 v1.36에서 노드 리소스 관리와 관찰성이 더 정밀해졌습니다.
...
쿠버네티스 컨트롤러에서 오래된 상태(staleness)는 많은 컨트롤러에 영향을 주고 컨트롤러 동작에 미묘한 영향을 줄 수 있는 문제입니다.
...
컨트롤러 동작에 대한 더 나은 관찰성을 제공하는 새 기능이 포함되었음을 알리게 되어 기쁩니다.
...
역사적으로 전부 아니면 전무였던 메커니즘을 실제로 최소 권한 원칙을 따를 수 있는 방식으로 강화합니다.
...
관찰성과 디버깅 워크플로우에 안전하게 사용할 수 있습니다.
...
### 혼합 버전 프록시(일명 _알 수 없는 버전 상호운용성 프록시_) {#mixed-version-proxy}
...
커널 제어가 파드 요청과 한도에 더 잘 맞도록 하고, 같은 노드를 공유하는 워크로드의 간섭과 스래싱을 줄입니다.
이번 반복에서는 kubelet이 memory.high와 memory.min을 설정하는 방식도 개선하고, 라이브락을 피하기 위한 메트릭과 보호 장치를 추가하며,
...
장치별 메타데이터나 가용성에 대한 관찰성도 제한적이었습니다.
쿠버네티스 v1.36은 워크로드를 위한 네이티브 ResourceClaim 지원과
DRA의 유연성을 CPU 관리에 제공하기 위한 DRA 네이티브 리소스를 포함하여, 알파 단계의 DRA 개선 사항을 대거 도입합니다.

이제 사용자는 [다운워드 API](/docs/concepts/workloads/pods/downward-api/)를 활용해 복잡한 리소스 속성을 컨테이너에 직접 노출하고,
...
새로운 단계에 도달했습니다. 기존 Prometheus 히스토그램은 정적이고 미리 정의된 버킷에 의존해
...
가장 예측하기 어려운 워크로드 급증 중에도 정확성을 유지하는 정밀한 히트맵을 제공합니다.
...
생태계의 안전과 보안을 우선하기 위해 Kubernetes SIG Network와 Security Response Committee는
...
쿠버네티스 v1.36은 클러스터를 감사하기에 이상적인 릴리스입니다. 자세한 내용은 [SELinux Volume Label Changes goes GA (and likely implications in v1.37)](/blog/2026/04/22/breaking-changes-in-selinux-volume-labeling/) 블로그를 확인합니다.

이 개선 사항에 대한 자세한 내용은 [KEP-1710: Speed up recursive SELinux label change](https://kep.k8s.io/1710)를 참고합니다.
...
- [Service Account 토큰 외부 서명 API](https://kep.k8s.io/740)
...
- [DRA: Dynamic Resource Allocation의 리소스를 포함하도록 PodResources 확장](https://kep.k8s.io/3695)
...
[KEP-5707: Deprecate service.spec.externalIPs](https://kep.k8s.io/5707)를 읽어봅니다.
...
이 제거에 대한 자세한 내용은 [KEP-5040: Remove gitRepo volume driver](https://kep.k8s.io/5040)를 참고합니다.
