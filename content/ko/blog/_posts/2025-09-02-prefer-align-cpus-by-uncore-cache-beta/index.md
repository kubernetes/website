...
translator: >
  [Jaehan Byun (Supergate)](https://github.com/jaehanbyun), [Ian Y. Choi (AWS)](https://github.com/ianychoi)
---

Kubernetes v1.32에서 `prefer-align-cpus-by-uncorecache`라는 새로운 CPU Manager Static Policy 옵션이 알파 기능으로 처음 도입되었고, Kubernetes v1.34에서는 **베타** 단계로 승격되었습니다.
이 옵션은 분할된 언코어 캐시 아키텍처를 사용하는 프로세서에서 실행되는 특정 워크로드의 성능을 최적화하도록 설계되었습니다.
...
![기본 정적 정책과 prefer-align-cpus-by-uncorecache 적용 시의 워크로드 CPU 할당 예시를 보여주는 다이어그램](./cache-align-diagram.png)

기본적으로 쿠버네티스는 언코어 캐시 토폴로지를 고려하지 않으며, _packed 방식_ 으로 CPU를 할당합니다. 이 경우 컨테이너 1과 컨테이너 2가 같은 언코어 캐시(예: Uncore Cache 0)를 공유하면서 Noisy Neighbor 현상(캐시 경합)이 발생할 수 있습니다. 또한 컨테이너 2는 CPU가 두 캐시에 걸쳐 분산되어 cross-cache latency가 추가로 발생할 수 있습니다.

`prefer-align-cpus-by-uncorecache`를 활성화하면 각 컨테이너가 서로 다른 캐시로 격리되어 배치됩니다. 그 결과 컨테이너 간 캐시 경합이 제거되고, 사용 중인 CPU들의 캐시 지연도 최소화됩니다.

## 사용 사례

일반적인 사용 사례에는 vRAN, 모바일 패킷 코어, 방화벽 같은 통신 애플리케이션이 포함될 수 있습니다. 다만 `prefer-align-cpus-by-uncorecache`가 제공하는 최적화는 워크로드에 따라 달라질 수 있다는 점을 유의해야 합니다. 예를 들어, 더 많은 언코어 캐시를 활용할수록 메모리 대역폭 접근이 증가하기 때문에, 메모리 대역폭에 제약받는 애플리케이션의 경우 언코어 캐시 정렬에서 이점을 얻지 못할 수 있습니다.
