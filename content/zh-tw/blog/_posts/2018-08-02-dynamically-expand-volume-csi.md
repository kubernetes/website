...
_Kubernetes 本身有一個非常強大的存儲子系統，涵蓋了相當廣泛的用例。而當我們計劃使用 Kubernetes 構建產品級關係型數據庫平臺時，我們面臨一個巨大的挑戰：提供存儲。本文介紹瞭如何擴展最新的 Container Storage Interface 0.2.0 和與 Kubernetes 集成，並演示了捲動態擴容的基本方面。_
...
他們期待着能用開源解決方案重新設計已經存在的整體應用程序，這些應用程序已經在虛擬化基礎架構或裸機上運行了幾年。
...
考慮到可擴展性和技術成熟程度，Kubernetes 和 Docker 排在我們選擇列表的首位。但是將整體應用程序遷移到類似於 Kubernetes 之類的分佈式容器編排平臺上很具有挑戰性，其中關係數據庫對於遷移來說至關重要。
...
關於關係數據庫，我們應該注意存儲。Kubernetes 本身內部有一個非常強大的存儲子系統。它非常有用，涵蓋了相當廣泛的用例。當我們計劃在生產環境中使用 Kubernetes 運行關係型數據庫時，我們面臨一個巨大挑戰：提供存儲。目前，仍有一些基本功能尚未實現。特別是，卷的動態擴容。這聽起來很無聊，但在除創建，刪除，安裝和卸載之類的操作外，它是非常必要的。
...
爲了啓用此功能，我們應該將特性開關 `ExpandPersistentVolumes` 設置爲 true 並打開 `PersistentVolumeClaimResize` 准入插件。 一旦啓用了 `PersistentVolumeClaimResize`，則其對應的 `allowVolumeExpansion` 字段設置爲 true 的存儲類將允許調整大小。
...
不幸的是，即使基礎存儲提供者具有此功能，也無法通過容器存儲接口（CSI）和 Kubernetes 動態擴展卷。
...
爲了更好地瞭解我們將要做什麼，我們首先需要知道什麼是容器存儲接口。當前，Kubernetes 中已經存在的存儲子系統仍然存在一些問題。 存儲驅動程序代碼在 Kubernetes 核心存儲庫中維護，這很難測試。 但是除此之外，Kubernetes 還需要授予存儲供應商許可，以將代碼簽入 Kubernetes 核心存儲庫。 理想情況下，這些應在外部實施。
...
CSI 旨在定義行業標準，該標準將使支持 CSI 的存儲提供商能夠在支持 CSI 的容器編排系統中使用。
...
更多詳細信息，請訪問： https://github.com/container-storage-interface/spec/blob/master/spec.md
...
除了擴展的 CSI 規範之外，Kubernetes 中的 `csiPlugin` 接口還應該實現 `expandablePlugin`。`csiPlugin` 接口將擴展代表 `ExpanderController` 的 `PersistentVolumeClaim`。
...
最後，爲了抽象化實現的複雜性，我們應該將單獨的存儲提供程序管理邏輯硬編碼爲以下功能，這些功能在 CSI 規範中已明確定義：
...
我們注意到中間的讀數顯示在批量插入期間 MySQL 數據文件的大小緩慢增加。 同時，底部讀數顯示文件系統在大約20分鐘內擴展了兩次，從 300 GiB 擴展到 400 GiB，然後擴展到 500 GiB。 同時，上半部分顯示，擴展卷的整個過程立即完成，幾乎不會影響 MySQL QPS。
...
不管運行什麼基礎結構應用程序，數據庫始終是關鍵資源。擁有更高級的存儲子系統以完全支持數據庫需求至關重要。這將有助於推動雲原生技術的更廣泛採用。
