...
_Kubernetes 本身有一個非常強大的儲存子系統，涵蓋了相當廣泛的用例。而當我們計劃使用 Kubernetes 構建產品級關係型資料庫平臺時，我們面臨一個巨大的挑戰：提供儲存。本文介紹瞭如何擴展最新的 Container Storage Interface 0.2.0 和與 Kubernetes 集成，並演示了捲動態擴容的基本方面。_
...
他們期待着能用開源解決方案重新設計已經存在的整體應用程式，這些應用程式已經在虛擬化基礎架構或裸機上運行了幾年。
...
考慮到可擴展性和技術成熟程度，Kubernetes 和 Docker 排在我們選擇列表的首位。但是將整體應用程式遷移到類似於 Kubernetes 之類的分佈式容器編排平臺上很具有挑戰性，其中關係資料庫對於遷移來說至關重要。
...
關於關係資料庫，我們應該注意儲存。Kubernetes 本身內部有一個非常強大的儲存子系統。它非常有用，涵蓋了相當廣泛的用例。當我們計劃在生產環境中使用 Kubernetes 運行關係型資料庫時，我們面臨一個巨大挑戰：提供儲存。目前，仍有一些基本功能尚未實現。特別是，卷的動態擴容。這聽起來很無聊，但在除創建，刪除，安裝和卸載之類的操作外，它是非常必要的。
...
爲了啓用此功能，我們應該將特性開關 `ExpandPersistentVolumes` 設置爲 true 並打開 `PersistentVolumeClaimResize` 准入插件。 一旦啓用了 `PersistentVolumeClaimResize`，則其對應的 `allowVolumeExpansion` 字段設置爲 true 的儲存類將允許調整大小。
...
不幸的是，即使基礎儲存提供者具有此功能，也無法通過容器儲存介面（CSI）和 Kubernetes 動態擴展卷。
...
本文將給出 CSI 的簡化視圖，然後逐步介紹如何在現有 CSI 和 Kubernetes 上引入新的擴展卷功能。最後，本文將演示如何動態擴展卷容量。
...
爲了更好地瞭解我們將要做什麼，我們首先需要知道什麼是容器儲存介面。當前，Kubernetes 中已經存在的儲存子系統仍然存在一些問題。 儲存驅動程式代碼在 Kubernetes 核心儲存庫中維護，這很難測試。 但是除此之外，Kubernetes 還需要授予儲存供應商許可，以將代碼簽入 Kubernetes 核心儲存庫。 理想情況下，這些應在外部實施。
...
CSI 旨在定義行業標準，該標準將使支持 CSI 的儲存提供商能夠在支持 CSI 的容器編排系統中使用。
...
該圖描述了一種與 CSI 集成的高級 Kubernetes 原型：
...
* 引入了三個新的外部組件以解耦 Kubernetes 和儲存提供程式邏輯
...
爲了實現在 Kubernetes 上擴展卷的功能，我們應該擴展幾個組件，包括 CSI 規範，“in-tree” 卷插件，external-provisioner 和 external-attacher。
...
除了擴展的 CSI 規範之外，Kubernetes 中的 `csiPlugin` 介面還應該實現 `expandablePlugin`。`csiPlugin` 介面將擴展代表 `ExpanderController` 的 `PersistentVolumeClaim`。
...
最後，爲了抽象化實現的複雜性，我們應該將單獨的儲存提供程式管理邏輯硬編碼爲以下功能，這些功能在 CSI 規範中已明確定義：
...
* 創建 PVC `qcfs-pvc`，它將由儲存類 `csi-qcfs` 動態設定
...
* 創建 MySQL 5.7 實例以使用 PVC `qcfs-pvc`
* 爲了反映完全相同的生產級別方案，實際上有兩種不同類型的工作負載，包括：
     * 批量插入使 MySQL 消耗更多的檔案系統容量
     * 浪湧查詢請求
...
Prometheus 和 Grafana 的集成使我們可以可視化相應的關鍵指標。

![prometheus grafana](/images/blog/2018-08-02-dynamically-expand-volume-csi/prometheus-grafana.png)

我們注意到中間的讀數顯示在批量插入期間 MySQL 資料檔案的大小緩慢增加。 同時，底部讀數顯示檔案系統在大約20分鐘內擴展了兩次，從 300 GiB 擴展到 400 GiB，然後擴展到 500 GiB。 同時，上半部分顯示，擴展卷的整個過程立即完成，幾乎不會影響 MySQL QPS。
...
不管運行什麼基礎結構應用程式，資料庫始終是關鍵資源。擁有更高級的儲存子系統以完全支持資料庫需求至關重要。這將有助於推動雲原生技術的更廣泛採用。
