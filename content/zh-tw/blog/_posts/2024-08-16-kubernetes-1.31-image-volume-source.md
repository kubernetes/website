...
雖然此項目在過去設計爲滿足微服務架構，但現在是時候聽聽最終使用者的聲音並引入更側重於 AI/ML 的特性了。
...
映像檔卷源（[KEP-4639](https://kep.k8s.io/4639)）。
此特性允許使用者在 Pod 中指定一個映像檔引用作爲卷，並在容器內將其作爲卷掛載進行復用：
...
-->
但就長期而言，作爲此項目的最終使用者的你要負責爲這一新特性的其他重要使用場景給出規劃。
[SIG Node](https://github.com/kubernetes/community/blob/54a67f5/sig-node/README.md)
...
你需要在 [API 伺服器](/zh-cn/docs/reference/command-line-tools-reference/kube-apiserver)以及
...
- `Never`：kubelet 從不拉取引用，只使用本地映像檔或工件。如果引用不存在，容器創建將失敗。
...
容器運行時拉取映像檔（或工件），將其掛載到容器中，並最終使其可被直接使用。
在實現中有很多細節，這些細節與 kubelet 現有的映像檔拉取行爲密切相關。例如：
...
- [在 Pod 中使用映像檔卷](/zh-cn/docs/tasks/configure-pod-container/image-volumes)
