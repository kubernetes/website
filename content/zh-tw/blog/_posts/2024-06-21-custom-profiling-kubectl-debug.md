---
layout: blog
title: "Kubernetes 1.31：kubectl debug 中的自定義模板化配置特性已進入 Beta 階段"
date: 2024-08-22
...
-->
有很多方法可以對集羣中的 Pod 和節點進行故障排查，而 `kubectl debug` 是最簡單、使用最廣泛、最突出的方法之一。
它提供了一組靜態配置，每個配置適用於不同類型的角色。
例如，從網絡管理員的視角來看，調試節點應該像這樣簡單：
...
-->
另一方面，靜態配置也存在固有的剛性，對某些 Pod 所產生的影響與其易用性是相悖的。
因爲各種類型的 Pod（或節點）都有其特定的需求，不幸的是，有些問題僅通過靜態配置是無法調試的。
...
此外，如果用戶需要將 `REQUIRED_ENV_VAR` 環境變量修改爲其他不同值來進行高級故障排查，
當前並沒有機制能夠實現這一需求。
...
自定義模板化配置使用 `--custom` 標誌提供的一項新特性，在 `kubectl debug` 中引入以提供可擴展性。
...
不受控制的擴展性會損害可用性。因此，某些字段（例如命令、鏡像、生命週期、卷設備和容器名稱）不允許進行自定義模版化配置。
...
這些方面最大的交集是 Pod 內的容器規約，因此自定義模版化配置僅支持修改使用 `containers` 下定義的字段。
這導致了一個限制，如果用戶需要修改 Pod 規約中的其他字段，則不受支持。
