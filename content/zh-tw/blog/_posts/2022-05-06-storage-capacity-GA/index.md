...
-->
在 Kubernetes v1.24 版本中，[儲存容量](/zh-cn/docs/concepts/storage/storage-capacity/)跟蹤已經成爲一項正式發佈的功能。
...
儲存容量跟蹤允許 CSI 驅動程式發佈有關剩餘容量的資訊。當 Pod 仍然有需要設定的卷時，
kube-scheduler 使用該資訊爲 Pod 選擇合適的節點。
...
capacity left.
-->
如果沒有這些資訊，Pod 可能會被卡住，而不會被調度到合適節點，這是因爲 kube-scheduler
...
different node.
-->
因爲 CSI 驅動程式發佈的這些儲存容量資訊在被使用的時候可能已經不是最新的資訊了，
所以最終選擇的節點無法正常工作的情況仍然可能會發生。
卷設定通過通知調度程式需要在其他節點上重試來恢復。
...
如果 Pod 使用兩個卷並且只能製備其中一個，那麼所有將來的調度決策都受到已經制備的卷的限制。
...
此原型旨在與任意 CSI 驅動程式協同工作，但這種靈活性使其難以設定並減慢了擴展操作：
因爲自動擴展程式無法模擬卷製備操作，它一次只能將叢集擴展一個節點，這是此方案的不足之處。
...
爲此，需要更好地瞭解哪些本地儲存 CSI 驅動程式與叢集自動縮放結合使用。如果這會引出新的 KEP，
...
-->
非常感謝爲此功能做出貢獻或提供反饋的 [SIG Scheduling](https://github.com/kubernetes/community/tree/master/sig-scheduling)、
