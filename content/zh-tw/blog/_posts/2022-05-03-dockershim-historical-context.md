...
儘管如此，它已經工作了很長時間，以至於今天的許多用戶都比這個決定更新，
...
用戶希望選擇最適合他們的運行時。因此，Kubernetes 需要一種方法來允許集羣操作員靈活地使用他們選擇的任何運行時。
...
已發佈以支持這種靈活性。 CRI 的引入對項目和用戶來說都很棒，但它確實引入了一個問題：Docker Engine
...
允許集羣操作員繼續使用 Docker Engine 作爲他們的容器運行時基本上不間斷。
...
Docker作爲一家公司意味着什麼，Docker 構建的容器鏡像是否仍然可以運行，以及
...
對於想要嘗試其他運行時（如 containerd 或 cri-o）的用戶，
...
-->
我們後來[調查了社區](https://kubernetes.io/blog/2021/11/12/are-you-ready-for-dockershim-removal/)[發現還有很多用戶有疑問和顧慮](/zh-cn/blog/2022/01/07/kubernetes-is-moving-on-from-dockershim)。 
作爲回應，Kubernetes 維護人員和 CNCF 承諾通過擴展文檔和其他程序來解決這些問題。
事實上，這篇博文是這個計劃的一部分。隨着如此多的最終用戶成功遷移到其他運行時，以及改進的文檔，
我們相信每個人現在都爲遷移鋪平了道路。
