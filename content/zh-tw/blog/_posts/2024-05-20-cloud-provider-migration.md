...
除了實現我們最初的目標之外，我們還通過刪除大約 150 萬行代碼，將核心組件的可執行檔案大小減少了大約 40%，
極大簡化了 Kubernetes。
...
1. **kubelet 憑證提供程式插件**（[KEP-2133](https://github.com/kubernetes/enhancements/tree/master/keps/sig-node/2133-kubelet-credential-providers)）
1. **儲存遷移以使用 [CSI](https://github.com/container-storage-interface/spec?tab=readme-ov-file#container-storage-interface-csi-specification-)**（[KEP-625](https://github.com/kubernetes/enhancements/blob/master/keps/sig-storage/625-csi-migration/README.md)）
...
這個基本組件負責通過施加元資料標籤來初始化節點。所施加的元資料標籤標示節點運行所在的雲區域和可用區，
以及只有雲驅動知道的 IP 地址。
...
-->
要進一步瞭解相關資訊，請閱讀 Kubernetes 文檔中的[雲控制器管理器](/zh-cn/docs/concepts/architecture/cloud-controller/)。
...
kubelet 憑據提供程式插件的開發是爲了取代 kubelet 的內置功能，用於動態獲取用於託管在
...
-->
憑據驅動插件機制於 2019 年推出，爲 kubelet 提供了一個通用擴展點用於執行插件的可執行檔案，
...
-->
要了解更多資訊，請閱讀[用於認證映像檔拉取的 kubelet 憑據提供程式](/zh-cn/docs/concepts/containers/images/#kubelet-credential-provider)。
...
容器儲存介面（Container Storage Interface，CSI）是一種控制平面標準，用於管理 Kubernetes
和其他容器編排系統中的塊和檔案儲存系統，已在 1.13 中進入正式發佈狀態。
它的設計目標是用可在 Kubernetes 叢集中 Pod 內運行的驅動程式替換直接內置於 Kubernetes 中的樹內卷插件。
這些驅動程式通過 Kubernetes API 與 kube-controller-manager 儲存控制器通信，並通過本地 gRPC 端點與 kubelet 進行通信。
現在，所有主要雲和儲存供應商一起提供了 100 多個 CSI 驅動，使 Kubernetes 中運行有狀態工作負載成爲現實。
...
這使我們能夠將所有儲存操作重定向到 CSI 驅動程式，爲我們在不刪除 API 的情況下刪除內置卷插件的代碼鋪平了道路。
...
該博客包含與我們所作的變更相關的詳細資訊，並提供了有關如何遷移到外部驅動的指導。
