...
-->
在 Kubernetes 1.24 中，gRPC 探針（probe）功能進入了 beta 階段，預設情況下可用。
現在，你可以爲 gRPC 應用程式設定啓動、活躍和就緒探測，而無需公開任何 HTTP 端點，
也不需要可執行檔案。Kubernetes 可以通過 gRPC 直接連接到你的工作負載並查詢其狀態。
...
讓管理你的工作負載的系統檢查應用程式是否健康、啓動是否正常，以及應用程式是否認爲自己可以接收流量，是很有用的。
在添加 gRPC 探針支持之前，Kubernetes 已經允許你通過從容器映像檔內部運行可執行檔案、發出 HTTP
...
-->
對於大多數應用程式來說，這些檢查就足夠了。如果你的應用程式提供了用於運行狀況（或準備就緒）檢查的
...
的 [3,626 個 Dockerfile 檔案](https://github.com/search?l=Dockerfile&q=grpc_health_probe&type=code)和
[6,621 個 yaml 檔案](https://github.com/search?l=YAML&q=grpc_health_probe&type=Code)（在撰寫本文時）。
...
因爲這是一個 alpha 特性，所以在 1.23 版中預設是禁用的。
...
與涉及 `grpc_health_probe` 可執行檔案的解決辦法相比，原生支持的健康探針有許多好處。
...
-->
有了原生 gRPC 支持，你不需要在映像檔中下載和攜帶 `10MB` 的額外可執行檔案。
Exec 探針通常比 gRPC 調用慢，因爲它們需要實例化一個新進程來運行可執行檔案。
...
-->
由於特性門控 `GRPCContainerProbe` 在 1.24 版本中是預設啓用的，因此許多供應商支持此功能開箱即用。
...
-->
如果清單檔案名爲 `test.yaml`，你可以用以下命令創建 Pod，並檢查它的狀態。如輸出片段所示，Pod 將處於就緒狀態。
...
像 gRPC 探針支持這樣的特性是一個小的改進，它將使許多應用程式開發人員的生活更容易，應用程式更有彈性。
