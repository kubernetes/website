...
2014 年 6 月 6 日，我檢查了 Kubernetes 公共代碼庫的[第一次 commit](https://github.com/kubernetes/kubernetes/commit/2c4b3a562ce34cddc3f8218a2c4d11c7310e6d56) 。許多人會認爲這是故事開始的地方。這難道不是一切開始的地方嗎？但這的確不能把整個過程說清楚。
...
第一次 commit 涉及的人員衆多，自那以後 Kubernetes 的成功歸功於更大的開發者陣容。

Kubernetes 建立在過去十年曾經在 Google 的 Borg 集羣管理系統中驗證過的思路之上。而 Borg 本身也是 Google 和其他公司早期努力的結果。

具體而言，Kubernetes 最初是從 Brendan Burns 的一些原型開始，結合我和 Craig McLuckie 正在進行的工作，以更好地將 Google 內部實踐與 Google Cloud 的經驗相結合。 Brendan，Craig 和我真的希望人們使用它，所以我們建議將這個原型構建爲一個開源項目，將 Borg 的最佳創意帶給大家。

在我們所有人同意後，就開始着手構建這個系統了。我們採用了 Brendan 的原型（Java 語言），用 Go 語言重寫了它，並且以上述核心思想去構建該系統。到這個時候，團隊已經成長爲包括 Ville Aikas，Tim Hockin，Brian Grant，Dawn Chen 和 Daniel Smith。一旦我們有了一些工作需求，有人必須承擔一些脫敏的工作，以便爲公開發布做好準備。這個角色最終由我承擔。當時，我不知道這件事情的重要性，我創建了一個新的倉庫，把代碼搬過來，然後進行了檢查。所以在我第一次提交 public commit 之前，就有工作已經啓動了。

那時 Kubernetes 的版本只是現在版本的簡單雛形。核心概念已經有了，但非常原始。例如，Pods 被稱爲 Tasks，這在我們推廣前一天就被替換。2014年6月10日 Eric Brewe 在第一屆 DockerCon 上的演講中正式發佈了 Kubernetes。你可以在此處觀看該視頻：
...
但是，無論多麼原始，這小小的一步足以激起一個開始強大而且變得更強大的社區的興趣。在過去的四年裏，Kubernetes 已經超出了我們所有人的期望。我們對 Kubernetes 社區的所有人員表示感謝。該項目所取得的成功不僅基於代碼和技術，還基於一羣出色的人聚集在一起所做的有意義的事情。Sarah Novotny 策劃的一套 [Kubernetes 價值觀](https://github.com/kubernetes/steering/blob/master/values.md)是以上最好的表現形式。
