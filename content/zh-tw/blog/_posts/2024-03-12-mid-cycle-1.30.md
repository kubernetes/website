...
這種方法對於集羣自動縮放器（CA）或任何需要爲一組 Pod 做決策的高級控制器（例如作業調度器）都會帶來問題。
...
-->
Kubernetes 的 SIG Node 小組還將根據最終用戶、貢獻者和更廣泛的 Kubernetes 社區的反饋更新文檔，
以幫助你瞭解如何使用經過修訂的實現。
...
-->
### 支持 Pod 運行在用戶命名空間 ([KEP-127](https://kep.k8s.io/127))
...
-->
[用戶命名空間](/zh-cn/docs/concepts/workloads/pods/user-namespaces) 是一個僅在 Linux 上可用的特性，它更好地隔離 Pod，
...
在 Kubernetes 1.30 中，對用戶命名空間的支持正在遷移到 beta，並且現在支持帶有和不帶有卷的 Pod，自定義 UID/GID 範圍等等！

<!--
### Structured authorization configuration ([KEP-3221](https://kep.k8s.io/3221))
-->
### 結構化鑑權配置([KEP-3221](https://kep.k8s.io/3221))
...
file is modified.
-->
對[結構化鑑權配置](/zh-cn/docs/reference/access-authn-authz/authorization/#configuring-the-api-server-using-an-authorization-config-file)的支持正在晉級到 Beta 版本，並將默認啓用。
這個特性支持創建具有明確參數定義的多個 Webhook 所構成的鑑權鏈；這些 Webhook 按特定順序驗證請求，
並允許進行細粒度的控制，例如在失敗時明確拒絕。
配置文件方法甚至允許你指定 [CEL](/zh-cn/docs/reference/using-api/cel/) 規則，以在將請求分派到 Webhook 之前對其進行預過濾，幫助你防止不必要的調用。
當配置文件被修改時，API 服務器還會自動重新加載鑑權鏈。
...
docs](/docs/reference/access-authn-authz/authorization/#configuring-the-api-server-using-an-authorization-config-file).
-->
你必須使用 `--authorization-config` 命令行參數指定鑑權配置的路徑。
如果你想繼續使用命令行標誌而不是配置文件，命令行方式沒有變化。
要訪問新的 Webhook 功能，例如多 Webhook 支持、失敗策略和預過濾規則，需要切換到將選項放在 `--authorization-config` 文件中。
從 Kubernetes 1.30 開始，配置文件格式約定是 beta 級別的，只需要指定 `--authorization-config`，因爲特性門控默認啓用。
...
HorizontalPodAutoscaler 的這一新行爲允許你根據各個容器的資源使用情況而不是 Pod 的聚合資源使用情況來配置自動伸縮。
...
-->
將 CEL 引入到 Kubernetes 的准入控制後，集羣管理員就具有了制定複雜規則的能力，
這些規則可以根據集羣的期望狀態和策略來評估 API 請求的內容，而無需使用基於 Webhook 的訪問控制器。
這種控制水平對於維護集羣操作的完整性、安全性和效率至關重要，使 Kubernetes 環境更加健壯，更適應各種用例和需求。
