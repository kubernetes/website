---
layout: blog
title:  '在 Kubernetes 上對 gRPC 服務器進行健康檢查'
date: 2018-10-01
...
瞭解更多信息，請參閱[配置存活探針、就緒探針和啓動探針](/zh-cn/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#define-a-grpc-liveness-probe)。
...
--->
[gRPC](https://grpc.io) 將成爲本地雲微服務間進行通信的通用語言。如果您現在將 gRPC 應用程序部署到 Kubernetes，您可能會想要了解配置健康檢查的最佳方法。在本文中，我們將介紹 [grpc-health-probe](https://github.com/grpc-ecosystem/grpc-health-probe/)，這是 Kubernetes 原生的健康檢查 gRPC 應用程序的方法。
...
--->
Kubernetes 原本 [不支持](https://github.com/kubernetes/kubernetes/issues/21493) gRPC 健康檢查。gRPC 的開發人員在 Kubernetes 中部署時可以採用以下三種方法：
...
2.  **tcpSocket probe：** 打開 gRPC 服務器的 Socket 是沒有意義的，因爲它無法讀取響應主體。
...
爲了使上述 "exec probe" 方法標準化，我們需要：

- 可以在任何 gRPC 服務器中輕鬆實現的 **標準** 健康檢查 "協議" 。
...
--->
如果您在 gRPC 應用程序中 [實現](https://github.com/grpc/grpc/blob/v1.15.0/src/proto/grpc/health/v1/health.proto) 此健康檢查協議，那麼可以使用標準或通用工具調用 `Check()` 方法來確定服務器狀態。
...
--->
使用此工具，您可以在所有 gRPC 應用程序中使用相同的健康檢查配置。這種方法有以下要求：
...
3.  [配置](https://github.com/grpc-ecosystem/grpc-health-probe/tree/1329d682b4232c102600b5e7886df8ffdcaf9e26#example-grpc-health-checking-on-kubernetes) Kubernetes 的 "exec" 檢查模塊來調用容器中的 "grpc_health_probe" 工具。

<!--
In this case, executing "grpc_health_probe" will call your gRPC server over
`localhost`, since they are in the same pod.
--->
在這種情況下，執行 "grpc_health_probe" 將通過 `localhost` 調用您的 gRPC 服務器，因爲它們位於同一個容器中。
...
**grpc-health-probe** 項目仍處於初期階段，需要您的反饋。它支持多種功能，例如與 TLS 服務器通信和配置延時連接/RPC。
...
--->
如果您最近要在 Kubernetes 上運行 gRPC 服務器，請嘗試使用 gRPC Health Protocol，並在您的 Deployment 中嘗試 grpc-health-probe，然後 [進行反饋](https://github.com/grpc-ecosystem/grpc-health-probe/)。
...
- 文檔： [Kubernetes 存活和就緒探針](/docs/tasks/configure-pod-container/configure-liveness-readiness-probes/)
