...
--->
在這種情況下，執行 "grpc_health_probe" 將通過 `localhost` 調用您的 gRPC 伺服器，因爲它們位於同一個容器中。
...
**grpc-health-probe** 項目仍處於初期階段，需要您的反饋。它支持多種功能，例如與 TLS 伺服器通信和設定延時連接/RPC。
...
--->
如果您最近要在 Kubernetes 上運行 gRPC 伺服器，請嘗試使用 gRPC Health Protocol，並在您的 Deployment 中嘗試 grpc-health-probe，然後 [進行反饋](https://github.com/grpc-ecosystem/grpc-health-probe/)。
