...
-->
當你使用 Kubernetes 中的資源管理能力時，例如設定 
...
-->
預設情況下，許多最新版本的 Linux 發行版已切換到 cgroup v2，
...
* API 中單個統一的層次結構設計
* 爲容器提供更安全的子樹委派能力
* [壓力阻塞資訊](https://www.kernel.org/doc/html/latest/accounting/psi.html)等新功能
* 增強的資源分配管理和跨多個資源的隔離
  * 統一覈算不同類型的內存分配（網路和內核內存等）
  * 考慮非即時資源更改，例如頁面緩存回寫
...
許多 Linux 發行版預設切換到 cgroup v2； 
你可能會在下次更新控制平面和節點的 Linux 版本時開始使用它！

推薦使用預設使用 cgroup v2 的 Linux 發行版。 
...
* kubelet 和容器運行時設定爲使用 [systemd cgroup 驅動程式](/zh-cn/docs/setup/production-environment/container-runtimes#systemd-cgroup-driver)
...
[systemd cgroup 驅動程式](/zh-cn/docs/setup/production-environment/container-runtimes#systemd-cgroup-driver)，
以便系統上只有一個 cgroup 管理員。要設定 kubelet 和容器運行時以使用該驅動程式，
請參閱 [systemd cgroup 驅動程式文檔](/zh-cn/docs/setup/production-environment/container-runtimes#systemd-cgroup-driver)。
...
kubelet 應該會自動適應而無需任何額外的設定。

在大多數情況下，除非你的使用者直接訪問 cgroup 檔案系統，
否則當你切換到使用 cgroup v2 時，不會感知到使用者體驗有什麼不同。
...
-->
如果你在節點上或從容器內直接訪問 cgroup 檔案系統的應用程式，
你必須更新應用程式以使用 cgroup v2 API 而不是 cgroup v1 API。

你可能需要更新到 cgroup v2 的場景包括：

* 如果你運行依賴於 cgroup 檔案系統的第三方監控和安全代理，請將代理更新到支持 cgroup v2 的版本。
* 如果你將 [cAdvisor](https://github.com/google/cadvisor) 作爲獨立的 DaemonSet 運行以監控 Pod 和容器，
  請將其更新到 v0.43.0 或更高版本。
* 如果你使用 JDK 部署 Java 應用程式，首選使用[完全支持 cgroup v2](https://bugs.openjdk.org/browse/JDK-8230305)
