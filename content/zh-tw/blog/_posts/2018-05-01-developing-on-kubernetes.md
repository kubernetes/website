...
[Minikube](/docs/getting-started-guides/minikube/) 在更加喜歡於本地 VM 上運行 Kubernetes 的開發人員中，非常受歡迎。不久前，Docker 的 [Mac](https://docs.docker.com/docker-for-mac/kubernetes/) 版和 [Windows](https://docs.docker.com/docker-for-windows/kubernetes/) 版，都試驗性地開始自帶 Kubernetes（需要下載 “edge” 安裝包）。在兩者之間，以下原因也許會促使您選擇 Minikube 而不是 Docker 桌面版：
...
既然您已經瞭解到運行環境的基本選項，那麼我們就接着討論如何迭代式的開發並部署您的應用。
...
請注意這很棘手，因爲即使在成熟定型的技術中做選擇，比如說在 JSON、YAML、XML、REST、gRPC 或者 SOAP 之間做選擇，很大程度也取決於您的背景、喜好以及公司環境。在 Kubernetes 生態系統內比較各種工具就更加困難，因爲技術發展太快，幾乎每週都有新工具面市；舉個例子，僅在準備這篇博客的期間，[Gitkube](https://gitkube.sh/) 和 [Watchpod](https://github.com/MinikubeAddon/watchpod) 相繼出品。爲了進一步覆蓋到這些新的，以及一些相關的已推出的工具，例如 [Weave Flux](https://github.com/weaveworks/flux) 和 OpenShift 的 [S2I](https://docs.openshift.com/container-platform/3.9/creating_images/s2i.html)，我們計劃再寫一篇跟進的博客。
...
【1】：此處疑爲 0.11.0，因爲 0.12.0 已經支持本地構建，見下一條
...
一旦兩個部署建好並且 pod 開始運行，我們轉發 `stock-con` 服務以供本地讀取（另開一個終端窗口）：
...
這樣，通過定期查詢 `healthz` 端點，我們就應該能夠從本地機器上讀取 `stock-con` 服務，查詢命令如下（在一個單獨的終端窗口）：
...
現在，改動 `ksync/stock-con` 目錄中的代碼，例如改動 [`service.js` 中定義的 `/healthz` 端點代碼](https://github.com/kubernauts/dok-example-us/blob/2334ee8fb11f8813370122bd46285cf45bdd4c48/stock-con/service.js#L52)，在其 JSON 形式的響應中新添一個字段並觀察 pod 如何更新以及 `curl localhost：9898/healthz` 命令的輸出發生何種變化。總的來說，您最後應該看到類似的內容：
...
一旦兩個部署建好並且 pod 開始運行，我們轉發 `stock-con` 服務以供本地讀取（另開一個終端窗口）並檢查 `healthz` 端點的響應：
...
請注意，最初我們在執行 `skaffold dev` 時發生身份驗證錯誤，爲避免此錯誤需要安裝[問題322](https://github.com/GoogleContainerTools/skaffold/issues/322) 中所述的修復。本質上，需要將 `〜/.docker/config.json` 的內容改爲：
...
現在我們準備好開始開發了。爲此，在 `stock-con/` 目錄中執行以下命令：
...
上面的命令將觸發 `stock-con` 圖像的構建和部署。一旦 `stock-con` 部署的 pod 開始運行，我們再次轉發 `stock-con` 服務以供本地讀取（在單獨的終端窗口中）並檢查 `healthz` 端點的響應：
...
現在，如果您修改一下 `stock-con` 目錄中的代碼，例如 [`service.js` 中定義的 `/healthz` 端點代碼](https://github.com/kubernauts/dok-example-us/blob/2334ee8fb11f8813370122bd46285cf45bdd4c48/stock-con/service.js#L52)，在其 JSON 形式的響應中添加一個字段，您應該看到 Skaffold 可以檢測到代碼改動並創建新圖像以及部署它。您的屏幕看起來應該類似這樣：
