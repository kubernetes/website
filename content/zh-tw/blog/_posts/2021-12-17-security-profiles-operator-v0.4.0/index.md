...
管理 SELinux 策略（相當於使用通常在單個伺服器上調用的 `semodule` ）不是由 SPO 本身完成的，
而是由另一個名爲 selinuxd 的容器完成，以提供更好的隔離。此版本將所使用的 selinuxd
容器映像檔從個人倉庫遷移到位於[我們團隊的 quay.io 倉庫](https://quay.io/organization/security-profiles-operator)下的映像檔。
...
同時我們提供了一個額外的 `spo-metrics-client` 叢集角色（和綁定）以從叢集內檢索指標。 
