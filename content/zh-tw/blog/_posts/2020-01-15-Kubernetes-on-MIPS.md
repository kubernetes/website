...
-->
[Kubernetes](https://kubernetes.io) 官方目前支持衆多 CPU 架構諸如 x86, arm/arm64, ppc64le, s390x 等。然而目前還不支持 MIPS 架構，始終是一個遺憾。隨着雲原生技術的廣泛應用，MIPS 架構下的用戶始終對 Kubernetes on MIPS 有着迫切的需求。
...
-->
多年來，爲了豐富開源社區的生態，我們一直致力於在 MIPS 架構下適配 Kubernetes。隨着 MIPS CPU 的不斷迭代優化和性能的提升，我們在 mips64el 平臺上取得了一些突破性的進展。
...
-->
多年來，我們一直積極投入 Kubernetes 社區，在 Kubernetes 技術應用和優化方面具備了豐富的經驗。最近，我們在研發過程中嘗試將 Kubernetes 適配到 MIPS 架構平臺，並取得了階段性成果。成功完成了 Kubernetes 以及相關組件的遷移適配，不僅搭建出穩定高可用的 MIPS 集羣，同時完成了 Kubernetes v1.16.2 版本的一致性測試。
...
-->
幾乎所有的 Kubernetes 相關的雲原生組件都沒有提供 MIPS 版本的安裝包或鏡像，在 MIPS 平臺上部署 Kubernetes 的前提是自行編譯構建出全部所需組件。這些組件主要包括：
...
-->
得益於 Golang 優秀的設計以及對於 MIPS 平臺的良好支持，極大地簡化了上述雲原生組件的編譯過程。首先，我們在 mips64el 平臺編譯出了最新穩定的 golang, 然後通過源碼構建的方式編譯完成了上述大部分組件。
...
-->
在編譯過程中，我們不可避免地遇到了很多平臺兼容性的問題，比如關於 golang 系統調用 (syscall) 的兼容性問題, syscall.Stat_t 32 位 與 64 位類型轉換，EpollEvent 修正位缺失等等。
...
-->
構建 K8S-MIPS 組件主要使用了交叉編譯技術。構建過程包括集成 QEMU 工具來實現 MIPS CPU 指令的轉換。同時修改 Kubernetes 和 E2E 鏡像的構建腳本，構建了 Hyperkube 和 MIPS 架構的 E2E 測試鏡像。
...
-->
成功構建出以上組件後，我們使用工具完成 Kubernetes 集羣的搭建，比如 kubespray、kubeadm 等。
...
-->
驗證 K8S-MIP 集羣穩定性和可用性最簡單直接的方式是運行 Kubernetes 的 [一致性測試](https://github.com/kubernetes/kubernetes/blob/v1.16.2/cluster/images/conformance/README.md)。

<!--
Conformance is a standalone container to launch Kubernetes end-to-end tests for conformance testing.
-->
一致性測試是一個獨立的容器，用於啓動 Kubernetes 端到端的一致性測試。
...
-->
當執行一致性測試時，測試程序會啓動許多 Pod 進行各種端到端的行爲測試，這些 Pod 使用的鏡像源碼大部分來自於 `kubernetes/test/images` 目錄下，構建的鏡像位於 `gcr.io/kubernetes-e2e-test-images/`。由於鏡像倉庫中目前並不存在 MIPS 架構的鏡像，我們要想運行 E2E 測試，必須首先構建出測試所需的全部鏡像。
...
-->
第一步是找到測試所需的所有鏡像。我們可以執行 `sonobuoy images-p e2e` 命令來列出所有鏡像，或者我們可以在 [/test/utils/image/manifest.go](https://github.com/kubernetes/kubernetes/blob/master/test/utils/image/manifest.go) 中找到這些鏡像。儘管 Kubernetes 官方提供了完整的 Makefile 和 shell 腳本，爲構建測試映像提供了命令，但是仍然有許多與體系結構相關的問題未能解決，比如基礎映像和依賴包的不兼容問題。因此，我們無法通過直接執行這些構建命令來製作 mips64el 架構鏡像。
...
-->
多數測試鏡像都是使用 golang 編寫，然後編譯出二進制文件，並基於相應的 Dockerfile 製作出鏡像。這些鏡像對我們來說可以輕鬆地製作出來。但是需要注意一點：測試鏡像默認使用的基礎鏡像大多是 alpine, 目前 [Alpine](https://www.alpinelinux.org/) 官方並不支持 mips64el 架構，我們暫時未能自己製作出 mips64el 版本的 alpine 礎鏡像，只能將基礎鏡像替換爲我們目前已有的 mips64el 基礎鏡像，比如 debian-stretch,fedora, ubuntu 等。替換基礎鏡像的同時也需要替換安裝依賴包的命令，甚至依賴包的版本等。
...
-->
有些測試所需鏡像的源碼並不在 `kubernetes/test/images` 下,比如 `gcr.io/google-samples/gb-frontend:v6` 等，沒有明確的文檔說明這類鏡像來自於何方，最終還是在 [github.com/GoogleCloudPlatform/kubernetes-engine-samples](github.com/GoogleCloudPlatform/kubernetes-engine-samples) 這個倉庫找到了原始的鏡像源代碼。但是很快我們遇到了新的問題，爲了製作這些鏡像，還要製作它依賴的基礎鏡像，甚至基礎鏡像的基礎鏡像，比如 `php:5-apache`、`redis`、`perl` 等等。
...
-->
經過漫長龐雜的的鏡像重製工作，我們完成了總計約 40 個鏡像的製作 ，包括測試鏡像以及直接和間接依賴的基礎鏡像。
最終我們將所有鏡像在集羣內準備妥當，並確保測試用例內所有 Pod 的鏡像拉取策略設置爲 `imagePullPolicy: ifNotPresent`。
...
- `registry.k8s.io/etcd:3.3.15` (鏡像自發布以來已更改（以前使用的倉庫爲 "k8s.gcr.io"))
- `registry.k8s.io/pause:3.1` (鏡像自發布以來已更改（以前使用的倉庫爲 "k8s.gcr.io"))
...
-->
我們手動構建了 K8S-MIPS 組件以及執行了 E2E 測試，驗證了 Kubernetes on MIPS 的可行性，極大的增強了我們對於推進 Kubernetes 支持 MIPS 架構的信心。
...
-->
後續，我們將積極地向社區貢獻我們的工作經驗以及成果，提交 PR 以及 Patch For MIPS 等， 希望能夠有更多的來自社區的力量加入進來，共同推進 Kubernetes for MIPS 的進程。
...
-->
- 貢獻構建 E2E 測試鏡像代碼
- 貢獻構建 MIPS 版本 hyperkube 代碼
- 貢獻構建 MIPS 版本 kubeadm 等集羣部署工具
