...
-->
[Kubernetes](https://kubernetes.io) 官方目前支持衆多 CPU 架構諸如 x86, arm/arm64, ppc64le, s390x 等。然而目前還不支持 MIPS 架構，始終是一個遺憾。隨着雲原生技術的廣泛應用，MIPS 架構下的使用者始終對 Kubernetes on MIPS 有着迫切的需求。
...
-->
多年來，我們一直積極投入 Kubernetes 社區，在 Kubernetes 技術應用和優化方面具備了豐富的經驗。最近，我們在研發過程中嘗試將 Kubernetes 適配到 MIPS 架構平臺，並取得了階段性成果。成功完成了 Kubernetes 以及相關組件的遷移適配，不僅搭建出穩定高可用的 MIPS 叢集，同時完成了 Kubernetes v1.16.2 版本的一致性測試。
...
-->
幾乎所有的 Kubernetes 相關的雲原生組件都沒有提供 MIPS 版本的安裝包或映像檔，在 MIPS 平臺上部署 Kubernetes 的前提是自行編譯構建出全部所需組件。這些組件主要包括：
...
-->
構建 K8S-MIPS 組件主要使用了交叉編譯技術。構建過程包括集成 QEMU 工具來實現 MIPS CPU 指令的轉換。同時修改 Kubernetes 和 E2E 映像檔的構建腳本，構建了 Hyperkube 和 MIPS 架構的 E2E 測試映像檔。
...
-->
成功構建出以上組件後，我們使用工具完成 Kubernetes 叢集的搭建，比如 kubespray、kubeadm 等。
...
-->
驗證 K8S-MIP 叢集穩定性和可用性最簡單直接的方式是運行 Kubernetes 的 [一致性測試](https://github.com/kubernetes/kubernetes/blob/v1.16.2/cluster/images/conformance/README.md)。
...
-->
第一步是找到測試所需的所有映像檔。我們可以執行 `sonobuoy images-p e2e` 命令來列出所有映像檔，或者我們可以在 [/test/utils/image/manifest.go](https://github.com/kubernetes/kubernetes/blob/master/test/utils/image/manifest.go) 中找到這些映像檔。儘管 Kubernetes 官方提供了完整的 Makefile 和 shell 腳本，爲構建測試映像提供了命令，但是仍然有許多與體系結構相關的問題未能解決，比如基礎映像和依賴包的不兼容問題。因此，我們無法通過直接執行這些構建命令來製作 mips64el 架構映像檔。
...
-->
有些測試所需映像檔的源碼並不在 `kubernetes/test/images` 下,比如 `gcr.io/google-samples/gb-frontend:v6` 等，沒有明確的文檔說明這類映像檔來自於何方，最終還是在 [github.com/GoogleCloudPlatform/kubernetes-engine-samples](github.com/GoogleCloudPlatform/kubernetes-engine-samples) 這個倉庫找到了原始的映像檔源代碼。但是很快我們遇到了新的問題，爲了製作這些映像檔，還要製作它依賴的基礎映像檔，甚至基礎映像檔的基礎映像檔，比如 `php:5-apache`、`redis`、`perl` 等等。
...
-->
經過漫長龐雜的的映像檔重製工作，我們完成了總計約 40 個映像檔的製作 ，包括測試映像檔以及直接和間接依賴的基礎映像檔。
最終我們將所有映像檔在叢集內準備妥當，並確保測試用例內所有 Pod 的映像檔拉取策略設置爲 `imagePullPolicy: ifNotPresent`。
...
- `registry.k8s.io/etcd:3.3.15` (映像檔自發布以來已更改（以前使用的倉庫爲 "k8s.gcr.io"))
- `registry.k8s.io/pause:3.1` (映像檔自發布以來已更改（以前使用的倉庫爲 "k8s.gcr.io"))
