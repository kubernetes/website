...
雖然 Kubernetes 從 v1.18 版本開始就支持純 IPv6 叢集，但當時還無法支持 IPv4 遷移到 IPv6。
...
因此，使用兩個 IP 協議族意味着需爲每個 IP 協議族創建一個 Service。在 1.20 版本中對使用者體驗進行簡化，
...
所以不再需要運行重複的 Service，在兩個 IP 協議族中各運行一個。你可以在兩個 IP 協議族中分配叢集 IP 地址。
...
-->
從 1.20 版本開始，重新實現的雙協議棧服務處於 Alpha 階段，無論叢集是否設定了啓用雙協議棧的特性標誌，
...
如果你想要設定雙協議棧網路，這一能力總是存在的。
你可以將叢集網路設置爲 IPv4 單協議棧 、IPv6 單協議棧或 IPV4/IPV6 雙協議棧 。
...
在[雙協議棧服務設定](/zh-cn/docs/concepts/services-networking/dual-stack/#dual-stack-service-configuration-scenarios)
文檔中的示例列出了可能出現的各種場景.
...
[kubeadm 創建雙協議棧叢集](/zh-cn/docs/setup/production-environment/tools/kubeadm/dual-stack-support/),
在本地嘗試用 [KIND 創建雙協議棧叢集](https://kind.sigs.k8s.io/docs/user/configuration/#ip-family)，
還可以將雙協議棧叢集部署到雲上（在查閱 CNI 或 kubenet 可用性的文檔之後）
...
-->
許多 Kubernetes 貢獻者爲雙協議棧網路做出了貢獻。感謝所有貢獻了代碼、經驗報告、文檔、代碼審查以及其他工作的人。
