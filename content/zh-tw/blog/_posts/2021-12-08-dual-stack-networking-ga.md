---
layout: blog
title: 'Kubernetes 1.23：IPv4/IPv6 雙協議棧網絡達到 GA'
date: 2021-12-08
...
-->
“Kubernetes 何時支持 IPv6？” 自從 k8s v1.9 版本中首次添加對 IPv6 的 alpha 支持以來，這個問題的討論越來越頻繁。
雖然 Kubernetes 從 v1.18 版本開始就支持純 IPv6 集羣，但當時還無法支持 IPv4 遷移到 IPv6。
[IPv4/IPv6 雙協議棧網絡](https://github.com/kubernetes/enhancements/tree/master/keps/sig-network/563-dual-stack/)
在 Kubernetes v1.23 版本中進入正式發佈（GA）階段。

讓我們來看看雙協議棧網絡對你來說意味着什麼？
...
因此，使用兩個 IP 協議族意味着需爲每個 IP 協議族創建一個 Service。在 1.20 版本中對用戶體驗進行簡化，
重新實現了 Service 以支持兩個 IP 協議族，這意味着一個 Service 就可以處理 IPv4 和 IPv6 協議。
...
-->
Service API 現在有了支持雙協議棧的新字段，取代了單一的 ipFamily 字段。
* 你可以通過將 `ipFamilyPolicy` 字段設置爲 `SingleStack`、`PreferDualStack` 或
`RequireDualStack` 來設置 IP 協議族。Service 可以在單協議棧和雙協議棧之間進行轉換(在某些限制內)。
* 設置 `ipFamilies` 爲指定的協議族列表，可用來設置使用協議族的順序。
* 'clusterIPs' 的能力在涵蓋了之前的 'clusterIP'的情況下，還允許設置多個 IP 地址。
所以不再需要運行重複的 Service，在兩個 IP 協議族中各運行一個。你可以在兩個 IP 協議族中分配集羣 IP 地址。
...
-->
請注意，Pods 也是雙協議棧的。對於一個給定的 Pod，不可能在同一協議族中設置多個 IP 地址。
...
-->
從 1.20 版本開始，重新實現的雙協議棧服務處於 Alpha 階段，無論集羣是否配置了啓用雙協議棧的特性標誌，
Kubernetes 的底層網絡都已經包括了雙協議棧。
...
如果你想要配置雙協議棧網絡，這一能力總是存在的。
你可以將集羣網絡設置爲 IPv4 單協議棧 、IPv6 單協議棧或 IPV4/IPV6 雙協議棧 。
...
-->
雖然 Service 是根據你的配置設置的，但 Pod 默認是由 CNI 插件設置的。
...
-->
雖然現在上游 Kubernetes 支持[雙協議棧網絡](/zh-cn/docs/concepts/services-networking/dual-stack/)
作爲 GA 或穩定特性，但每個提供商對雙協議棧 Kubernetes 的支持可能會有所不同。節點需要提供可路由的 IPv4/IPv6 網絡接口。
Pod 需要是雙協議棧的。[網絡插件](/zh-cn/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/)
是用來爲 Pod 分配 IP 地址的，所以集羣需要支持雙協議棧的網絡插件。一些容器網絡接口（CNI）插件支持雙協議棧，例如 kubenet。
...
-->
支持雙協議棧的生態系統在不斷壯大；你可以使用
[kubeadm 創建雙協議棧集羣](/zh-cn/docs/setup/production-environment/tools/kubeadm/dual-stack-support/),
在本地嘗試用 [KIND 創建雙協議棧集羣](https://kind.sigs.k8s.io/docs/user/configuration/#ip-family)，
還可以將雙協議棧集羣部署到雲上（在查閱 CNI 或 kubenet 可用性的文檔之後）
...
-->
SIG-Network 希望從雙協議棧網絡的社區體驗中學習，以瞭解更多不斷變化的需求和你的用例信息。
...
說明了該 SIG 的重點領域。[雙協議棧 API 服務器](https://github.com/kubernetes/enhancements/issues/2438)
是一個考慮貢獻的方向。
...
是一個友好、熱情的場所，你可以與社區聯繫並分享你的想法。期待你的加入！
...
-->
許多 Kubernetes 貢獻者爲雙協議棧網絡做出了貢獻。感謝所有貢獻了代碼、經驗報告、文檔、代碼審查以及其他工作的人。
Bridget Kromhout 在 [Kubernetes的雙協議棧網絡](https://containerjournal.com/features/dual-stack-networking-in-kubernetes/)
中詳細介紹了這項社區工作。Tim Hockin 和 Khaled (Kal) Henidak 在 2019 年的 KubeCon 大會演講
（[Kubernetes 通往 IPv4/IPv6 雙協議棧的漫漫長路](https://www.youtube.com/watch?v=o-oMegdZcg4)）
和 Lachlan Evenson 在 2021 年演講（[我們來啦，Kubernetes 雙協議棧網絡](https://www.youtube.com/watch?v=o-oMegdZcg4)）
中討論了雙協議棧的發展旅程，耗時 5 年和海量代碼。
