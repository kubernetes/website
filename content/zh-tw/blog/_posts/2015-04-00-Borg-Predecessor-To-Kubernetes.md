...
-->
十多年來，谷歌一直在生產中運行容器化工作負載。
無論是像網絡前端和有狀態服務器之類的工作，像 [Bigtable](http://research.google.com/archive/bigtable.html) 和
...
Google 的幾乎一切都是以容器的方式運行的。今天，我們揭開了 Borg 的面紗，Google 傳聞已久的面向容器的內部集羣管理系統，並在學術計算機系統會議 [Eurosys](http://eurosys2015.labri.fr/) 上發佈了詳細信息。你可以在 [此處](https://research.google.com/pubs/pub43438.html) 找到論文。
...
在 Google 的很多從事 Kubernetes 的開發人員以前都是 Borg 項目的開發人員。
我們在 Kubernetes 中結合了 Borg 的最佳創意，並試圖解決用戶多年來在 Borg 中發現的一些痛點。
...
Pod 是 Kubernetes 中調度的單位。
它是一個或多個容器在其中運行的資源封裝。
保證屬於同一 Pod 的容器可以一起調度到同一臺計算機上，並且可以通過本地卷共享狀態。
...
Borg 中 alloc 的常見用法包括運行 Web 服務器，該服務器生成日誌，一起部署一個輕量級日誌收集進程，
該進程將日誌發送到集羣文件系統（和 fluentd 或 logstash 沒什麼不同 ）；
運行 Web 服務器，該 Web 服務器從磁盤目錄提供數據，
該磁盤目錄由從集羣文件系統讀取數據併爲 Web 服務器準備/暫存的進程填充（與內容管理系統沒什麼不同）；
並與存儲分片一起運行用戶定義的處理功能。
<!--
...
-->
Pod 不僅支持這些用例，而且還提供類似於在單個 VM 中運行多個進程的環境 -- Kubernetes 用戶可以在 Pod 中部署多個位於同一地點的協作過程，而不必放棄一個應用程序一個容器的部署模型。
...
儘管 Borg 的主要角色是管理任務和計算機的生命週期，但是在 Borg 上運行的應用程序還可以從許多其它集羣服務中受益，包括命名和負載均衡。
Kubernetes 使用服務抽象支持命名和負載均衡：帶名字的服務，會映射到由標籤選擇器定義的一組動態 Pod 集（請參閱下一節）。
集羣中的任何容器都可以使用服務名稱鏈接到服務。
<!--
...
-->
在幕後，Kubernetes 會自動在與標籤選擇器匹配到 Pod 之間對與服務的連接進行負載均衡，並跟蹤 Pod 在哪裏運行，由於故障，它們會隨着時間的推移而重新安排。
...
3) [標籤](/zh-cn/docs/concepts/overview/working-with-objects/labels/)。
Borg 中的容器通常是一組相同或幾乎相同的容器中的一個副本，該容器對應於 Internet 服務的一層（例如 Google Maps 的前端）或批處理作業的工人（例如 MapReduce）。
該集合稱爲 Job ，每個副本稱爲任務。
儘管 Job 是一個非常有用的抽象，但它可能是有限的。
例如，用戶經常希望將其整個服務（由許多 Job 組成）作爲一個實體進行管理，或者統一管理其服務的幾個相關實例，例如單獨的 Canary 和穩定的發行版。
另一方面，用戶經常希望推理和控制 Job 中的任務子集 --最常見的示例是在滾動更新期間，此時作業的不同子集需要具有不同的配置。
...
-->
通過使用標籤組織 Pod ，Kubernetes 比 Borg 支持更靈活的集合，標籤是用戶附加到 Pod（實際上是系統中的任何對象）的任意鍵/值對。
用戶可以通過在其 Pod 上使用 “job:\<jobname\>” 標籤來創建與 Borg Jobs 等效的分組，但是他們還可以使用其他標籤來標記服務名稱，服務實例（生產，登臺，測試）以及一般而言，其 pod 的任何子集。
標籤查詢（稱爲“標籤選擇器”）用於選擇操作應用於哪一組 Pod 。
結合起來，標籤和[複製控制器](/zh-cn/docs/concepts/workloads/controllers/replicationcontroller/) 允許非常靈活的更新語義，以及跨等效項的操作 Borg Jobs。
...
雖然這意味着 Borg 可以使用普通網絡，但是它給基礎結構和應用程序開發人員帶來了許多負擔：Borg 必須將端口作爲資源進行調度；任務必須預先聲明它們需要多少個端口，並將要使用的端口作爲啓動參數；Borglet（節點代理）必須強制端口隔離；命名和 RPC 系統必須處理端口以及 IP 地址。
...
-->
多虧了軟件定義的覆蓋網絡，例如 [flannel](https://coreos.com/blog/introducing-rudder/) 或內置於[公有云](https://cloud.google.com/compute/docs/networking)網絡的出現，Kubernetes 能夠爲每個 Pod 提供服務併爲其提供自己的 IP 地址。
...
-->
隨着基於容器的微服務架構的日益普及，Google 從內部運行此類系統所汲取的經驗教訓已引起外部 DevOps 社區越來越多的興趣。
通過揭示集羣管理器 Borg 的一些內部工作原理，並將下一代集羣管理器構建爲一個開源項目（Kubernetes）和一個公開可用的託管服務（[Google Container Engine](http://cloud.google.com/container-engine)），我們希望這些課程可以使 Google 之外的廣大社區受益，並推動容器調度和集羣管理方面的最新技術發展。
