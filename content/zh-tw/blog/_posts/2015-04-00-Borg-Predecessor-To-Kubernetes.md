...
例如，使用者經常希望將其整個服務（由許多 Job 組成）作爲一個實體進行管理，或者統一管理其服務的幾個相關實例，例如單獨的 Canary 和穩定的發行版。
另一方面，使用者經常希望推理和控制 Job 中的任務子集 --最常見的示例是在滾動更新期間，此時作業的不同子集需要具有不同的設定。
...
-->
通過使用標籤組織 Pod ，Kubernetes 比 Borg 支持更靈活的集合，標籤是使用者附加到 Pod（實際上是系統中的任何對象）的任意鍵/值對。
使用者可以通過在其 Pod 上使用 “job:\<jobname\>” 標籤來創建與 Borg Jobs 等效的分組，但是他們還可以使用其他標籤來標記服務名稱，服務實例（生產，登臺，測試）以及一般而言，其 pod 的任何子集。
...
通過揭示叢集管理器 Borg 的一些內部工作原理，並將下一代叢集管理器構建爲一個開源項目（Kubernetes）和一個公開可用的託管服務（[Google Container Engine](http://cloud.google.com/container-engine)），我們希望這些課程可以使 Google 之外的廣大社區受益，並推動容器調度和叢集管理方面的最新技術發展。
