...
在谷歌，我們依賴 Linux 容器應用程序去運行我們的核心基礎架構。所有服務，從搜索引擎到Gmail服務，都運行在容器中。事實上，我們非常喜歡容器，甚至我們的谷歌雲計算引擎虛擬機也運行在容器上！由於容器對於我們的業務至關重要，我們已經與社區合作開發許多基本的容器技術（從 cgroups 到 Docker 的 LibContainer）,甚至決定去構建谷歌的下一代開源容器調度技術，Kubernetes。
...
在 Kubernetes 項目進行一年後，在 OSCON 上發佈 V1 版本的前夕，我們很高興的宣佈Kubernetes 的主要貢獻者 Mesosphere 組織了有史以來第一次正規的以企業爲中心的 Kubernetes 培訓會議。首屆會議將於 6 月 20 日在波特蘭的 OSCON 舉辦，由來自 Mesosphere 的 Zed Shaw 和 Michael Hausenblas 演講。[Pre-registration](https://mesosphere.com/training/kubernetes/) 對於優先註冊者是免費的，但名額有限，立刻行動吧！
...
這個爲期一天的課程將包涵使用 Kubernetes 構建和部署容器化應用程序的基礎知識。它將通過完整的流程引導與參會者創建一個 Kubernetes 的應用程序體系結構，創建和設定 Docker 映像檔，並把它們部署到 Kubernetes 叢集上。使用者還將瞭解在我們的谷歌容器引擎和 Mesosphere 的數據中心操作系統上部署 Kubernetes 應用程序和服務的基礎知識。
...
即將推出的 Kubernetes bootcamp 將是學習如何應用 Kubernetes 解決長期部署和應用程序管理問題的一個好途徑。相對於我們所預期的，來自於廣泛社區的衆多培訓項目而言，這只是其中一個。
