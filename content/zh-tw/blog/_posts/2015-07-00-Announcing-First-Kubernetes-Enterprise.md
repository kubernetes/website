...
這個爲期一天的課程將包涵使用 Kubernetes 構建和部署容器化應用程序的基礎知識。它將通過完整的流程引導與參會者創建一個 Kubernetes 的應用程序體系結構，創建和配置 Docker 鏡像，並把它們部署到 Kubernetes 集羣上。用戶還將瞭解在我們的谷歌容器引擎和 Mesosphere 的數據中心操作系統上部署 Kubernetes 應用程序和服務的基礎知識。
