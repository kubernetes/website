...
有關確保叢集已準備好進行此刪除的更多資訊，請參閱[本指南](/zh-cn/blog/2022/03/31/ready-for-dockershim-removal/)。
...
[新的 beta API 預設不會在叢集中啓用](https://github.com/kubernetes/enhancements/issues/3136)。
預設情況下，現有 Beta API 和及其更新版本將繼續被啓用。
...
發佈工件的簽名和驗證是[提高 Kubernetes 發佈過程的軟體供應鏈安全性](https://github.com/kubernetes/enhancements/issues/3027)
的一部分。
...
[儲存容量跟蹤](https://github.com/kubernetes/enhancements/issues/1472)支持通過
[CSIStorageCapacity 對象](/zh-cn/docs/concepts/storage/storage-capacity/#api)公開當前可用的儲存容量，
...
目前正在進行[遷移樹內儲存插件的內部組件](https://github.com/kubernetes/enhancements/issues/625)工作，
...
已進入測試版，預設可用。現在，你可以在 Kubernetes 中爲你的 gRPC
應用程式原生地[設定啓動、存活和就緒性探測](/zh-cn/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#configure-probes)，
而無需暴露 HTTP 端點或使用額外的可執行檔案。
...
的支持已升級到 Beta。這允許 kubelet 使用 exec 插件動態檢索容器映像檔倉庫的憑據，而不是將憑據儲存在節點的檔案系統上。
...
-->
當 CNI 插件尚未升級和/或 CNI 設定版本未在 CNI 設定檔案中聲明時，在 containerd v1.6.0–v1.6.3
...
Kubernetes 和容器儲存介面 (CSI) 的卷快照和恢復功能，提供標準化的 API 設計 (CRD) 並添加了對 CSI 卷驅動程式的
PV 快照/恢復支持，在 v1.20 中升級至 GA。VolumeSnapshot v1beta1 在 v1.20 中被棄用，現在不受支持。
有關詳細資訊，請參閱 [KEP-177: CSI 快照](https://git.k8s.io/enhancements/keps/sig-storage/177-volume-snapshot#kep-177-csi-snapshot)
...
-->
* [容器儲存介面（CSI）卷擴展](https://github.com/kubernetes/enhancements/issues/284)
...
* [儲存容量跟蹤](https://github.com/kubernetes/enhancements/issues/1472)
* [OpenStack Cinder In-Tree 到 CSI 驅動程式遷移](https://github.com/kubernetes/enhancements/issues/1489)
* [Azure 磁盤樹到 CSI 驅動程式遷移](https://github.com/kubernetes/enhancements/issues/1490)
...
* [預設關閉 Beta API](https://github.com/kubernetes/enhancements/issues/3136)
...
在我們的[發行說明](https://git.k8s.io/kubernetes/CHANGELOG/CHANGELOG-1.24.md) 中查看 Kubernetes 1.24 版本的完整詳細資訊。
...
是一款爲數百萬人服務的應用程式。每個人都是我們天空中的一顆星星，幫助我們規劃路線。
<!--
...
* KubeCon + CloudNativeCon Europe 2022 於 2022 年 5 月 16 日至 20 日在西班牙巴倫西亞舉行！
  你可以在[活動網站](https://events.linuxfoundation.org/kubecon-cloudnativecon-europe/)上找到有關會議和註冊的更多資訊。
...
* [Linux 基金會](https://www.linuxfoundation.org/)和[雲原生計算基金會](https://www.cncf.io/) (CNCF)
  ...
  爲參與者提供設計、構建和部署雲原生應用程式的知識和技能。查看[公告](https://www.cncf.io/announcements/2022/03/15/new-cloud-native-developer-bootcamp-provides-a-clear-path-to-cloud-native-careers/)以瞭解更多資訊。
...
彙總了許多與 Kubernetes 和各種子項目的速度相關的有趣資料點。這包括從個人貢獻到做出貢獻的公司數量的所有內容，
...
瞭解此版本的主要功能以及棄用和刪除，以幫助規劃升級。有關更多資訊和註冊，
...
* 在 [Kubernetes Contributors](https://www.kubernetes.dev/) 網站上了解有關爲 Kubernetes 做出貢獻的更多資訊
...
* 在[博客](/zh-cn/blog/)上閱讀有關 Kubernetes 正在發生的事情的更多資訊
