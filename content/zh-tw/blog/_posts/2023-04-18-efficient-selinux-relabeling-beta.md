...
-->
然後，容器運行時以遞歸的方式更改 Pod 容器可見的所有檔案上的 SELinux 標籤。
如果捲上有許多檔案，這一過程可能會非常耗時，尤其是當卷位於遠程檔案系統上時。
...
因此可能逃離容器邊界的進程將無法訪問主機上任何其他容器的資料。
...
這種掛載將在確定時間內完成，容器運行時不需要遞歸地重新標記其上的任何檔案。
...
3. Pod 必須在其 [Pod 安全上下文](/zh-cn/docs/reference/kubernetes-api/workload-resources/pod-v1/#security-context)中至少設置
   ...
   否則 Kubernetes 將從操作系統預設值（通常是 `system_u`、`system_r` 和 `container_t`）中讀取預設的 `user`、`role` 和 `type`。
...
-->
4. 儲存卷必須是[訪問模式](/zh-cn/docs/concepts/storage/persistent-volumes/#access-modes) 
   ...
   因爲整個卷已掛載，並且大多數檔案系統不支持使用不同的 SELinux 標籤多次掛載同一個卷。
...
-->
5. 卷插件或負責卷的 CSI 驅動程式支持使用 SELinux 掛載選項進行掛載。

   這些樹內卷插件支持使用 SELinux 掛載選項進行掛載：`fc`、`iscsi` 和 `rbd`。

   支持使用 SELinux 掛載選項掛載的 CSI 驅動程式必須通過設置 `seLinuxMount`
   字段在其 [CSIDriver](/zh-cn/docs/reference/kubernetes-api/config-and-storage-resources/csi-driver-v1/) 實例中聲明這一點。

   由其他未設置 `seLinuxMount: true` 的卷插件或 CSI 驅動程式管理的卷將由容器運行時遞歸地重新標記。
...
-->
當滿足所有上述條件時，kubelet 會將 `-o context=<SELinux label>` 掛載選項傳遞給卷插件或 CSI 驅動程式。
CSI 驅動程式提供者必須確保其 CSI 驅動程式支持此安裝選項，並且如有必要，CSI 驅動程式要附加
`-o context` 所需的其他安裝選項。
...
-->
在 CSIDriver 實例中設置 `seLinuxMount: true` 之前，CSI 驅動程式提供者需要在啓用 SELinux
的環境中測試其 CSI 驅動程式。
