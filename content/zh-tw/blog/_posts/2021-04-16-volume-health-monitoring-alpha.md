...
-->
CSI 卷健康監控允許 CSI 驅動程序檢測來自底層存儲系統的異常卷狀況，並將其作爲 PVC 或 Pod 上的事件報送。
...
-->
監控捲和使用卷健康信息報送事件的 Kubernetes 組件包括：
...
-->
* Kubelet 除了收集現有的卷統計信息外，還將觀察該節點上 PVC 的卷健康狀況。
...
-->
要使用卷健康功能，首先確保你使用的 CSI 驅動程序支持此功能。
請參閱此 [CSI 驅動程序文檔](https://kubernetes-csi.github.io/docs/drivers.html)以瞭解哪些 CSI 驅動程序支持此功能。
...
-->
如果 CSI 驅動程序支持控制器端的卷健康監控功能，則有關異常卷條件的事件將記錄在 PVC 上。
...
-->
如果 CSI 驅動程序支持節點端的卷健康監控功能，則有關異常卷條件的事件將使用 PVC 記錄在 Pod 上。

<!--
## As a storage vendor, how do I add support for volume health to my CSI driver?
-->
## 作爲存儲供應商，如何向 CSI 驅動程序添加對卷健康的支持？
...
-->
有關詳細信息，請參閱 [CSI 規約](https://github.com/container-storage-interface/spec/blob/master/spec.md)
...
-->
[CSI 主機路徑驅動程序](https://github.com/kubernetes-csi/csi-driver-host-path)中有一個卷健康的示例實現。
...
或者具有 `GET_VOLUME` 和 `VOLUME_CONDITION` 能力的 CSI 驅動程序才支持外部控制器中的卷健康監控。
...
-->
要從控制器端實現卷健康功能，CSI 驅動程序**必須**添加對新控制器功能的支持。
...
-->
如果 CSI 驅動程序支持 `LIST_VOLUMES` 和 `VOLUME_CONDITION` 控制器功能，它**必須**實現控制器 RPC `ListVolumes` 並在響應中報送卷狀況。
...
-->
如果 CSI 驅動程序支持 `GET_VOLUME` 和 `VOLUME_CONDITION` 控制器功能，它**必須**實現控制器 PRC `ControllerGetVolume` 並在響應中報送卷狀況。
...
-->
如果 CSI 驅動程序支持 `LIST_VOLUMES`、`GET_VOLUME` 和 `VOLUME_CONDITION` 控制器功能，則外部健康監視控制器將僅調用 `ListVolumes` CSI RPC。
...
-->
如果檢測到異常的卷條件，
Kubelet 會調用 `NodeGetVolumeStats` CSI RPC 並報送 VolumeConditionAbnormal 事件以及 Pod 上的信息。
只有具有 `VOLUME_CONDITION` 節點功能的 CSI 驅動程序才支持 Kubelet 中的卷健康監控。

<!--
To implement the volume health feature from the node side, a CSI driver **must** add support for the new node capabilities.
-->
要從節點端實現卷健康功能，CSI 驅動程序**必須**添加對新節點功能的支持。
...
-->
如果 CSI 驅動程序支持 `VOLUME_CONDITION` 節點能力，它**必須**在節點 RPC `NodeGetVoumeStats` 中報送卷狀況。
...
-->
我們還在探索如何在 Kubernetes 中使用卷健康信息進行編程檢測和自動協調。
...
-->
[容器存儲接口文檔](https://kubernetes-csi.github.io/docs/)中還有關於卷健康檢查的更多詳細信息。
...
-->
那些有興趣參與 CSI 或 Kubernetes 存儲系統任何部分的設計和開發的人，
