---
layout: blog
title: "一個編排高可用應用的 Kubernetes 自定義調度器"
date: 2020-12-21
...
-->
任何系統的擴展都有兩個主要組成部分。
1. 增加或刪除系統將運行的基礎架構，以及
2. 確保系統知道如何處理自身額外實例的添加和刪除。
...
-->
然而，Kubernetes 的調度器並不像我們一開始預期的那樣 _保證_ 上面的分佈。
...
* Kubernetes [自動跨區分配 Pod](/zh-cn/docs/setup/best-practices/multiple-zones/#pods-are-spread-across-zones) 的能力
* 一個有 _n_ 個副本的 StatefulSet，當 Pod 被部署時，它們會按照 `{0...n-1}` 的順序依次創建。
...
並讓 CRDB-1 處於未調度狀態，因爲持久性卷只在其創建時所處的區域內可用。
...
### 3. 編寫一個自定義的 Kubernetes 調度器

感謝 [Kelsey Hightower](https://github.com/kelseyhightower/scheduler) 的例子和 
[Banzai Cloud](https://banzaicloud.com/blog/k8s-custom-scheduler/) 的博文，我們決定投入進去，編寫自己的[自定義 Kubernetes 調度器](/zh-cn/docs/tasks/extend-kubernetes/configure-multiple-schedulers/)。
