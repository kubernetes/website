---
layout: blog
title: "Kubernetes 1.30：對 Pod 使用使用者命名空間的支持進階至 Beta"
date: 2024-04-22
slug: userns-beta
author: >
...
例如，一個典型的 Kubernetes Pod 運行在一個網路命名空間中可以隔離網路身份，運行在一個 PID 命名空間中可以隔離進程。

Linux 有一個以前一直未被容器化應用所支持的命名空間是[使用者命名空間](https://man7.org/linux/man-pages/man7/user_namespaces.7.html)。
這個命名空間允許我們將容器內使用的使用者標識符和組標識符（UID 和 GID）與主機上的標識符隔離開來。
...
但我們與主機的交互僅限於非特權使用者可以執行的操作。這對於限制容器逃逸的影響非常有用。
...
are moving to beta!
-->
在 Kubernetes v1.25 中，我們僅爲無狀態 Pod 引入了對使用者命名空間的支持。
...
-->
## 什麼是使用者命名空間？   {#what-is-a-user-namespace}
...
benefits:
-->
使用者命名空間是一個 Linux 特性，它將容器的 UID 和 GID 與主機上的隔離開來。
...
-->
{{< figure src="/images/blog/2024-04-22-userns-beta/userns-ids.png" alt="此圖顯示了 ID 0-65535 爲主機預留，Pod 使用更大的 ID" title="使用者命名空間 ID 分配" >}}
...
-->
如果不使用使用者命名空間，容器逃逸時以 root 運行的容器在節點上將具有 root 特權。
如果某些權能授權給了此容器，這些權能在主機上也會有效。
如果使用使用者命名空間，就不會是這種情況（當然，除非有漏洞 🙂）。
...
在 Kubernetes 1.30 中，除了將使用者命名空間進階至 Beta，參與此特性的貢獻者們還：
...
* 爲 Kubernetes 添加了一種強制執行的方式讓運行時支持使用者命名空間所需的所有特性。
  如果不支持這些特性，Kubernetes 在嘗試創建具有使用者命名空間的 Pod 時，會顯示一個明確的錯誤。
  在 1.30 之前，如果容器運行時不支持使用者命名空間，Pod 可能會在沒有使用者命名空間的情況下被創建。
...
-->
你可以查閱有關使用者命名空間的[文檔](/zh-cn/docs/concepts/workloads/pods/user-namespaces/#set-up-a-node-to-support-user-namespaces)，
瞭解如何設定映射的自定義範圍。
...
演示了此漏洞在沒有使用者命名空間時的工作方式，而在使用使用者命名空間後 **得到了緩解**。
...
-->
{{< youtube id="07y5bl5UDdA" title="通過啓用使用者命名空間支持來在 Kubernetes 上緩解 CVE-2024-21626" class="youtube-quote-sm" >}}
...
請注意，你還需要 [crun][crun] 1.9 或更高版本。如果你使用的是 CRI-O 和 [runc][runc]，則仍然不支持使用者命名空間。

containerd 對此特性的支持目前鎖定爲 [containerd][containerd] 2.0，同樣 crun 也有適用的版本要求。
如果你使用的是 containerd 和 runc，則仍然不支持使用者命名空間。
...
-->
請注意，正如在 Kubernetes 1.25 和 1.26 中實現的那樣，containerd 1.7 增加了對使用者命名空間的**實驗性**支持。
我們曾在 Kubernetes 1.27 中進行了重新設計，所以容器運行時需要做一些變更。
而 containerd 1.7 並未包含這些變更，所以它僅在 Kubernetes 1.25 和 1.26 中支持使用使用者命名空間。
