---
title: “SIG-Networking：1.3 版本引入 Kubernetes 網絡策略 API”
date: 2016-04-18
...
Network-SIG 小組今天的帖子描述了 1.3 版中的網絡策略 API-安全，隔離和多租戶策略。_
...
-->
自去年下半年以來，[Kubernetes SIG-Networking](https://kubernetes.slack.com/messages/sig-network/) 一直在定期開會，致力於將網絡策略引入 Kubernetes，我們開始看到這個努力的結果。
...
-->
許多用戶面臨的一個問題是，Kubernetes 的開放訪問網絡策略不適用於需要對訪問容器或服務的流量進行更精確控制的應用程序。
...
其中“安全組”成員的流量由網絡策略或訪問控制列表（ ACL ）定義，並由網絡數據包過濾器實施。
...
-->
SIG-Networking 開始這項工作時的第一步是辯識需要特定網絡隔離以增強安全性的
...
-->
快速支持此實驗性 API 的最簡單方法是對 API 服務器的 ThirdPartyResource 擴展，今天在 Kubernetes 1.2 中就可以實現。
...
-->
第三方網絡控制器現在可以在這些端點上進行偵聽，並在創建，修改或刪除資源時根據需要做出反應。
_注意：在即將發佈的 Kubernetes 1.3 版本中-當網絡政策 API 以 beta 形式發佈時-無需創建如上所示的 ThirdPartyResource API 端點。_
...
-->
默認情況下，網絡隔離處於關閉狀態，以便所有Pod都能正常通信。
但是，重要的是要知道，啓用網絡隔離後，所有命名空間中所有容器的所有流量都會被阻止，這意味着啓用隔離將改變容器的行爲


<!--
Network isolation is enabled by defining the _network-isolation_ annotation on namespaces as shown below:
-->
通過在名稱空間上定義 _network-isolation_ 註解來啓用網絡隔離，如下所示：
...
-->
啓用網絡隔離後，**必須應用**顯式網絡策略才能啓用 Pod 通信。
...
-->
現今，[Romana](http://romana.io/), [OpenShift](https://www.openshift.com/), [OpenContrail](http://www.opencontrail.org/) 和 [Calico](http://projectcalico.org/) 支持應用於名稱空間和容器的網絡策略。
思科和 VMware 也在努力實施。
...
-->
每個解決方案都有其自己的特定實現細節。
今天，他們依靠某種形式的主機執行機制，但是將來的實現也可以構建爲在虛擬機管理程序上，甚至直接由網絡本身應用策略構建。&nbsp;
...
當發生需要配置策略的事件時，偵聽器將識別出更改，並且控制器將通過配置接口並應用策略來做出響應。&nbsp;
下圖顯示了 API 偵聽器和策略控制器，它通過主機代理在本地應用網絡策略來響應更新。
主機上的網絡接口由主機上的 CNI 插件配置（未顯示）。
...
-->
如果您由於網絡隔離和/或安全性問題而一直拒絕使用 Kubernetes 開發應用程序，那麼這些新的網絡策略將爲您提供所需的控制功能大有幫助。
無需等到 Kubernetes 1.3，因爲網絡策略現在作爲實驗性 API 可用，已啓用 ThirdPartyResource。

<!--
If you’re interested in Kubernetes and networking, there are several ways to participate - join us at:
-->
如果您對 Kubernetes 和網絡感興趣，有幾種參與方式-請通過以下方式加入我們：
...
<!--
- Our [Kubernetes 網絡特別興趣小組](https://groups.google.com/forum/#!forum/kubernetes-sig-network) email list&nbsp;
-->
...
-->
網絡“特殊興趣小組”，每兩週一次，在太平洋時間下午 3 點（15：00）在[SIG-Networking 環聊](https://zoom.us/j/5806599998)開會。&nbsp;
