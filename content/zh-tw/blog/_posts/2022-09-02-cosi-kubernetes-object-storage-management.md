---
layout: blog
title: "COSI 簡介：使用 Kubernetes API 管理對象儲存"
date: 2022-09-02
...
-->
本文介紹了容器對象儲存介面 (COSI)，它是在 Kubernetes 中製備和使用對象儲存的一個標準。
它是 Kubernetes v1.25 中的一個 Alpha 功能。

檔案和塊儲存通過 [Container Storage Interface](https://kubernetes.io/blog/2019/01/15/container-storage-interface-ga/) (CSI)
被視爲 Kubernetes 生態系統中的一等公民。
使用 CSI 卷的工作負載可以享受跨供應商和跨 Kubernetes 叢集的可移植性優勢，
而無需更改應用程式清單。對象儲存不存在等效標準。

近年來，對象儲存作爲檔案系統和塊設備的替代儲存形式越來越受歡迎。
對象儲存範式促進了計算和儲存的分解，這是通過網路而不是本地提供資料來完成的。
...
-->
COSI 旨在標準化對象儲存的使用，以提供以下好處：

* Kubernetes 原生 - 使用 Kubernetes API 來製備、設定和管理 Bucket
* 自助服務 - 明確劃分管理和運營 (DevOps)，爲 DevOps 人員賦予自助服務能力
* 可移植性 - 通過跨 Kubernetes 叢集和跨對象儲存供應商的可移植性實現供應商中立性

**跨供應商的可移植性只有在兩家供應商都支持通用資料路徑 API 時纔有可能。
...
每個 Kubernetes 叢集都需要一個控制器管理器實例。即使叢集中使用了多個對象儲存提供程式，也只需要一個。

COSI 邊車充當 COSI API 請求和供應商特定 COSI 驅動程式之間的轉換器。
該組件使用供應商驅動程式應滿足的標準化 gRPC 協議。

COSI 驅動程式是供應商特定組件，它接收來自 sidecar 的請求並調用適當的供應商 API 以創建 Bucket、 
管理其生命週期及對它們的訪問。
...
COSI 介面 以 Bucket 爲中心，因爲 Bucket 是對象儲存的抽象單元。COSI 定義了三個旨在管理它們的 Kubernetes API
...
BucketClass 在檔案/塊設備世界中對應的是 StorageClass。

由於對象儲存始終通過網路進行身份驗證，因此需要訪問憑證才能訪問 Bucket。
BucketAccess 和 BucketAccessClass 這兩個 API 用於表示訪問憑證和身份驗證策略。
有關這些 API 的更多資訊可以在官方 COSI 提案中找到 - https://github.com/kubernetes/enhancements/tree/master/keps/sig-storage/1979-object-storage-support
...
例如，叢集操作員可以使用管理策略將最大預置容量限制爲 100GB，並且允許開發人員創建 Bucket 並將資料儲存到該限制。
...
跨叢集可移植性允許在一個叢集中設定的 Bucket 在另一個叢集中可用。這僅在對象儲存後端本身可以從兩個叢集訪問時纔有效。

跨提供商可移植性是指允許組織或團隊無縫地從一個對象儲存提供商遷移到另一個對象儲存提供商，
而無需更改應用程式定義（PodTemplates、StatefulSets、Deployment 等）。這只有在源和目標提供者使用相同的資料時纔有可能。

**COSI 不處理資料遷移，因爲它超出了其範圍。如果提供者之間的移植也需要遷移資料，則需要採取其他措施來確保資料可用性。**
...
我們期待很多供應商加入編寫 COSI 驅動程式並與 COSI 兼容！

我們希望爲 COSI Bucket 添加更多身份驗證機制，我們正在設計高級儲存桶共享原語、多叢集儲存桶管理等等。
未來有很多偉大的想法和機會！
