...
採用了全新版本（[v1beta4](/zh-cn/docs/reference/config-api/kubeadm-config.v1beta4/)）的配置文件格式。
之前 v1beta3 格式的配置現已正式棄用，這意味着儘管之前的格式仍然受支持，但你應遷移到 v1beta4 並停止使用已棄用的格式。
對 v1beta3 配置的支持將在至少 3 次 Kubernetes 次要版本發佈後被移除。
...
-->
在本文中，我將介紹關鍵的變更；我將解釋 kubeadm v1beta4 配置格式，以及如何從 v1beta3 遷移到 v1beta4。

你可以參閱 v1beta4 配置格式的參考文檔：
...
-->
- 增加了兩個新的配置元素：ResetConfiguration 和 UpgradeConfiguration
...
- ResetConfiguration API 類型現在在 v1beta4 中得到支持。用戶可以通過將 `--config` 文件傳遞給 `kubeadm reset` 來重置節點。
- `dryRun` 模式現在在 InitConfiguration 和 JoinConfiguration 中可配置。
<!--
...
- 添加了 `ClusterConfiguration.encryptionAlgorithm`，可用於設置此集羣的密鑰和證書所使用的非對稱加密算法。
...
- 添加了 `ClusterConfiguration.dns.disabled` 和 `ClusterConfiguration.proxy.disabled`，
  可用於在集羣初始化期間禁用 CoreDNS 和 kube-proxy 插件。
  在集羣創建期間跳過相關插件階段將把相同的字段設置爲 `true`。
<!--
...
這些變更簡化了使用 kubeadm 的工具的配置，並提高了 kubeadm 本身的可擴展性。
...
此命令讀取使用舊格式的現有配置文件，並寫入一個使用當前格式的新文件。
...
-->
如果你有興趣參與 kubeadm 配置的後續討論，可以通過多種方式與 kubeadm 或
