...
SK：SIG Node 是一個垂直 SIG，負責支持 Pod 和主機資源之間受控互動的組件。我們管理被調度到節點上的 Pod 的生命週期。
這個 SIG 的重點是支持廣泛的工作負載類型，包括具有硬件特性或性能敏感要求的工作負載。同時保持節點上 Pod 之間的隔離邊界，以及 Pod 和主機的隔離邊界。
這個 SIG 維護了相當多的組件，並有許多外部依賴（如容器運行時間或操作系統功能），這使得我們處理起來十分複雜。但我們戰勝了這種複雜度，旨在不斷提高節點的可靠性。
...
EH：有兩種 SIG：橫向和垂直。橫向 SIG 關注 Kubernetes 中每個組件的特定功能：例如，SIG Security 考慮 Kubernetes 中每個組件的安全方面，或者 SIG Instrumentation 關注 Kubernetes 中每個組件的日誌、度量、跟蹤和事件。
這樣的 SIG 並不太會擁有大量的代碼。
...
-->
### CI 子項目是如何開始的？這是專門針對 SIG Node 的嗎？它對 SIG 有什麼幫助？

SK：該子項目是在其中一個版本因關鍵測試的大量測試失敗而受阻後開始跟進的。
這些測試並不是一下子就開始下降的，而是持續的缺乏關注導致了測試質量的緩慢下降。
SIG Node 一直將質量和可靠性放在首位，組建這個子項目是強調這一優先事項的一種方式。
...
-->
### 作爲 issue 和 PR 數量第三大的 SIG，你們 SIG 是如何兼顧這麼多工作的？

EH：這歸功於有組織性。當我在 2021 年 1 月增加對 SIG 的貢獻時，我發現自己被大量的 PR 和 issue 淹沒了，不知道該從哪裏開始。
我們已經在 CI 子項目板上跟蹤與測試有關的 issue 和 PR 請求，但這缺少了很多 bug 修復和功能工作。
因此，我開始爲我們剩餘的 PR 建立一個分流板，這使我能夠根據狀態和採取的行動對其進行分類，併爲其他貢獻者記錄它的用途。
...
SIG Node 維護着許多組件，其中許多組件都依賴於外部項目或操作系統功能。這使得入職過程相當冗長和苛刻。
但如果你願意接受挑戰，總有一個地方適合你，也有一羣人支持你。
<!--
...
EH：在 SIG Node 的起步工作可能是令人生畏的，因爲有太多的工作要做，我們的 SIG 會議非常大，而且很難找到一個開始的地方。

我總是鼓勵新手貢獻者在他們已經有一些投入的方向上更進一步。
在 SIG Node 中，這可能意味着自願幫助修復一個隻影響到你個人的 bug，或者按優先級去分流你關心的 bug。

爲了儘快瞭解任何開源代碼庫，你可以採取兩種策略：從深入探索一個特定的問題開始，然後根據需要擴展你的知識邊緣，或者單純地儘可能多的審查 issues 和變更請求，以瞭解更高層次的組件工作方式。
最終，如果你想成爲一名 Node reviewer 或 approver，兩件事是不可避免的。

[Davanum Srinivas](https://twitter.com/dims) 和我各自舉辦了一次小組輔導，以幫助教導新手貢獻者成爲 Node reviewer 的技能，如果有興趣，我們可以努力尋找一個導師來舉辦另一次會議。
我也鼓勵新手貢獻者參加我們的 Node CI 子項目會議：它的聽衆較少，而且我們不記錄分流會議，所以它可以是一個比較溫和的方式來開始 SIG 之旅。
<!--
...
最後，我們一直在尋找關於如何最好地運行你的工作負載的反饋。來解釋一下它的具體情況，以及 SIG Node 組件中的哪些功能可能有助於更好地運行它。
<!--
...
EH：在過去的一年裏，SIG Node 採用了一些新的流程來幫助管理我們的功能開發和 Kubernetes 增強提議，其他 SIG 也向我們尋求在管理大型工作負載方面的靈感。
...
加入 SIG 的會議，瞭解最新的研究成果，未來一年的計劃是什麼，以及如何作爲貢獻者參與到上游的 Node 團隊中!