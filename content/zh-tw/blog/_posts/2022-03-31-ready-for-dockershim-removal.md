...
如果你是集羣操作員，則該指南包含你在此版本中需要了解的實際情況。
另外還包括你需要做些什麼來確保你的集羣不會崩潰！
...
請注意，使用 Docker Desktop 構建應用程序容器並不算是集羣對 Docker 有依賴。
Docker 創建的容器鏡像符合 [Open Container Initiative (OCI)](https://opencontainers.org/) 規範，
而 OCI 是 Linux 基金會的一種治理架構，負責圍繞容器格式和運行時定義行業標準。 
這些鏡像可以在 Kubernetes 支持的任何容器運行時上正常工作。
...
但如果你的集羣中有任何自定義的節點，你要確保它們不需要被更新。
...
-->
無論你是在管理自己的集羣還是使用雲服務提供商管理的 Kubernetes 服務，
...
-->
如果你的 Kubernetes 集羣對 Docker Engine 有依賴，
...
-->
是的。只要你深入瞭解此版本所做的變更和你自己集羣的詳細信息，
並確保與你的開發團隊進行清晰的溝通，它的不確定性就會降到最低。 
你可能需要對集羣、應用程序代碼或腳本進行一些更改，但所有這些要求都已經有說明指導。 
