...
諸如 Secret 和證書之類的憑據時，它必須通過儲存提供者的身份認證才能訪問這些憑據。
...
-->
當前，有兩種不是那麼理想的方法來實現這一目的，要麼通過授予 CSI 驅動程式使用 TokenRequest API 的權限，要麼直接從主機檔案系統中讀取令牌。
...
-->
- 令牌的受衆預設爲 kube-apiserver
- 該令牌不能保證可用（例如，`AutomountServiceAccountToken=false`）
- 該方法不適用於以與 Pod 不同的（非 root 使用者）使用者身份運行的 CSI 驅動程式。請參見
...
-->
Kubernetes 1.20 引入了一個內測功能 `CSIServiceAccountToken` 以改善安全狀況。這項新功能允許 CSI 驅動程式接收 Pod 的[綁定服務帳戶令牌](https://github.com/kubernetes/enhancements/blob/master/keps/sig-auth/1205-bound-service-account-tokens/README.md)。
...
如果 CSI 驅動程式要代表其爲掛載卷的 Pod 訪問 GCP API，則可以使用 Pod 的服務帳戶令牌來
[交換 GCP 令牌](https://cloud.google.com/iam/docs/reference/sts/rest)。啓用功能 `CSIServiceAccountToken` 後，
可通過 `NodePublishVolume` RPC 調用中的捲上下文來訪問 Pod 的服務帳戶令牌。例如：通過 [Secret 儲存 CSI 驅動](https://github.com/GoogleCloudPlatform/secrets-store-csi-driver-provider-gcp)
...
對於使用 Vault 作爲資源提供者的 CSI 驅動程式，它們需要將 Pod 的服務帳戶提供給 Vault。
...
爲了使諸如證書之類的短時卷保持有效，CSI 驅動程式可以在其 `CSIDriver` 對象中指定 `RequiresRepublish=true`，
