---
layout: blog
title: 'Kubernetes 1.20：CSI 驅動程式中的 Pod 身份假扮和短時卷'
date: 2020-12-18
...
-->
通常，當 [CSI](https://github.com/container-storage-interface/spec/blob/baa71a34651e5ee6cb983b39c03097d7aa384278/spec.md) 驅動程式掛載
諸如 Secret 和證書之類的憑據時，它必須透過儲存提供者的身份認證才能訪問這些憑據。
然而，對這些憑據的訪問是根據 Pod 的身份而不是 CSI 驅動程式的身份來控制的。 
因此，CSI 驅動程式需要某種方法來取得 Pod 的服務帳戶令牌。
...
-->
當前，有兩種不是那麼理想的方法來實現這一目的，要麼透過授予 CSI 驅動程式使用 TokenRequest API 的許可權，要麼直接從主機檔案系統中讀取令牌。
...
-->
- 違反最少特權原則
- 每個 CSI 驅動程式都需要重新實現獲取 Pod 的服務帳戶令牌的邏輯
...
-->
- 令牌的受眾預設為 kube-apiserver
- 該令牌不能保證可用（例如，`AutomountServiceAccountToken=false`）
- 該方法不適用於以與 Pod 不同的（非 root 使用者）使用者身份執行的 CSI 驅動程式。請參見
  [服務帳戶令牌的檔案許可權部分](https://github.com/kubernetes/enhancements/blob/f40c24a5da09390bd521be535b38a4dbab09380c/keps/sig-storage/20180515-svcacct-token-volumes.md#file-permission)
...
-->
Kubernetes 1.20 引入了一個內測功能 `CSIServiceAccountToken` 以改善安全狀況。這項新功能允許 CSI 驅動程式接收 Pod 的[繫結服務帳戶令牌](https://github.com/kubernetes/enhancements/blob/master/keps/sig-auth/1205-bound-service-account-tokens/README.md)。

<!--
This feature also provides a knob to re-publish volumes so that short-lived volumes can be refreshed.
-->
此功能還提供了一個重新發布卷的能力，以便可以重新整理短時卷。
...
-->
使用 [Workload Identity](https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity)，Kubernetes 服務帳戶可以在訪問 Google Cloud API 時驗證為 Google 服務帳戶。
如果 CSI 驅動程式要代表其為掛載卷的 Pod 訪問 GCP API，則可以使用 Pod 的服務帳戶令牌來
[交換 GCP 令牌](https://cloud.google.com/iam/docs/reference/sts/rest)。啟用功能 `CSIServiceAccountToken` 後，
可透過 `NodePublishVolume` RPC 呼叫中的捲上下文來訪問 Pod 的服務帳戶令牌。例如：透過 [Secret 儲存 CSI 驅動](https://github.com/GoogleCloudPlatform/secrets-store-csi-driver-provider-gcp)
...
如果使用者將 [Kubernetes 作為身份驗證方法](https://www.vaultproject.io/docs/auth/kubernetes)配置，
則 Vault 使用 `TokenReview` API 來驗證 Kubernetes 服務帳戶令牌。
對於使用 Vault 作為資源提供者的 CSI 驅動程式，它們需要將 Pod 的服務帳戶提供給 Vault。
例如，[Secret 儲存 CSI 驅動](https://github.com/hashicorp/secrets-store-csi-driver-provider-vault)和
...
為了使諸如證書之類的短時卷保持有效，CSI 驅動程式可以在其 `CSIDriver` 物件中指定 `RequiresRepublish=true`，
以使 kubelet 定期針對已掛載的卷呼叫 `NodePublishVolume`。
這些重新發布操作使 CSI 驅動程式可以確保卷內容是最新的。
...
此功能是 Alpha 版，預計將在 1.21 版中移至 Beta 版。 請參閱以下 KEP 和 CSI 文件中的更多內容：
...
-->
- [KEP-1855: CSI 驅動程式的服務帳戶令牌](https://github.com/kubernetes/enhancements/blob/master/keps/sig-storage/1855-csi-driver-service-account-token/README.md)
...
-->
隨時歡迎您提供反饋!
- SIG-Auth [定期開會](https://github.com/kubernetes/community/tree/master/sig-auth#meetings)，可以透過 [Slack 和郵件列表](https://github.com/kubernetes/community/tree/master/sig-auth#contact)加入
- SIG-Storage [定期開會](https://github.com/kubernetes/community/tree/master/sig-storage#meetings)，可以透過 [Slack 和郵件列表](https://github.com/kubernetes/community/tree/master/sig-storage#contact)加入
