...
與大多數 POSIX 系統不同，這裏的根/節點文件系統不是 `/`，而是 `/var/lib/kubelet` 所在的磁盤。
...
Pod 和容器的某些操作可能需要臨時或瞬態的本地存儲。
臨時存儲的生命週期短於 Pod 的生命週期，且臨時存儲不能被多個 Pod 共享。
...
默認情況下，Kubernetes 將每個運行容器的日誌存儲爲 `/var/log` 中的文件。
...
Kubernetes 將自動檢測容器文件系統是否與節點文件系統分離。
