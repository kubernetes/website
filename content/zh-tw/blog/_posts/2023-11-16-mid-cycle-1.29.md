...
-->
## Kubernetes 社區自治軟體包倉庫相關說明

在 2023 年年初，Kubernetes 項目[引入了](/zh-cn/blog/2023/08/15/pkgs-k8s-io-introduction/) `pkgs.k8s.io`,
這是 Debian 和 RPM 軟體包所用的社區自治軟體包倉庫。這些社區自治的軟體包倉庫取代了先前由 Google 管理的倉庫
...
對於 Kubernetes v1.29，預設特性門控 `DisableCloudProviders` 和 `DisableKubeletCloudCredentialProviders`
...
爲了做好準備，你可以編輯現有的清單（Manifest）並重寫客戶端軟體，以使用自 v1.26 起可用的
...
棄用資訊是在 Kubernetes 發佈說明（Release Notes）中公佈的。你可以在以下版本的發佈說明中看到待棄用的公告：
