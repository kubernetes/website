...
Kubernetes 項目爲了託管其容器鏡像，使用社區自治的一個名爲 registry.k8s.io 的鏡像倉庫。
從最近的 3 月份起，所有流向 k8s.gcr.io 舊倉庫的請求開始被重定向到 registry.k8s.io。
已棄用的 k8s.gcr.io 倉庫最終將被淘汰。有關這一變更的細節或若想查看你是否受到影響，參閱
[k8s.gcr.io 重定向到 registry.k8s.io - 用戶須知](/zh-cn/blog/2023/03/10/image-registry-redirect/)。
...
都將被設置爲 `true`。這個變更將要求當前正在使用內部雲驅動集成（Azure、GCE 或 vSphere）的用戶啓用外部雲控制器管理器，
