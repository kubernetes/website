---
layout: blog
title: 'SIG Node CI 子項目慶祝測試改進兩週年'
date: 2022-02-16
...
保證 SIG 節點上游代碼的可靠性是一項持續的工作，需要許多貢獻者在幕後付出大量努力。
Kubernetes、基礎操作系統、容器運行時和測試基礎架構的頻繁發佈，導致了一個複雜的矩陣，
需要關注和穩定的投資來“保持燈火通明”。2020 年 5 月，Kubernetes Node 特殊興趣小組
（“SIG Node”）爲節點相關代碼和測試組織了一個新的持續集成（CI）子項目。自成立以來，SIG Node CI
子項目每週舉行一次會議，即使一整個小時通常也不足以完成對所有缺陷、測試相關的 PR 和問題的分類，
並討論組內所有相關的正在進行的工作。
...
在過去兩年中，我們修復了阻塞合併和阻塞發佈的測試，由於減少了測試缺陷，縮短了合併 Kubernetes 
貢獻者的拉取請求的時間。通過我們的努力，任務通過率由開始時 42% 提高至穩定大於 90% 。我們已經解決了 144 個測試失敗問題，
並在 kubernetes/kubernetes 中合併了 176 個拉取請求。
我們還幫助子項目參與者提升了 Kubernetes 貢獻者的等級，新增了 3 名組織成員、6 名評審員和 2 名審批員。
...
Node CI 子項目是一個可入門的第一站，幫助新參與者開始使用 SIG Node。對於新貢獻者來說，
解決影響較大的缺陷和測試修復的門檻很低，儘管貢獻者要攀登整個貢獻者階梯還有很長的路要走：
爲該團隊培養了兩個新的審批人花了一年多的時間。爲 Kubernetes 節點及其測試基礎設施提供動力的所有
不同組件的複雜性要求開發人員在很長一段時間內進行持續投資，
以深入瞭解整個系統，從宏觀到微觀。
...
SIG 中的子項目如何形成、運行和工作並不總是顯而易見的。每一個都是其背後的 SIG 所獨有的，
並根據該小組打算支持的項目量身定製。作爲一個歡迎了許多第一次 SIG Node 貢獻者的團隊，
我們想分享過去兩年的一些細節和成就，幫助揭開我們內部工作的神祕面紗，並慶祝我們所有專注貢獻者的辛勤工作！
...
在啓動會議上，我們討論了應該首先集中精力修復哪些測試，並討論了阻塞合併和阻塞發佈的測試，
其中許多測試由於基礎設施問題或錯誤的測試代碼而失敗。

<!--The subproject launched weekly hour-long meetings to discuss ongoing work
discussion and triage.-->
該子項目每週召開一小時的會議，討論正在進行的工作會談和分類。
...
因其貢獻而被公認爲 SIG Node CI 小組的評審員，爲該子項目的早期階段提供了重要幫助。
...
***2020 年 8 月*** 所有的阻塞合併和阻塞發佈的測試都通過了，伴有一些邏輯問題。
...
***2020 年 10 月*** Amim Knabben 因對子項目的貢獻成爲 Kubernetes 組織成員。
...
***2021 年 1 月*** 隨着健全的預提交和關鍵定期工作的通過，子項目討論了清理其餘定期測試並確保其順利通過的目標。

<!--Elana Hashman joined the subproject, stepping up to help lead it after
Victor's departure.-->
Elana Hashman 加入了這個子項目，在 Victor 離開後幫助領導該項目。

<!--***February 2021.*** Artyom Lukianov becomes a Kubernetes org member for his
contributions to the subproject.-->
***2021 年 2 月*** Artyom Lukianov 因其對子項目的貢獻成爲 Kubernetes 組織成員。
...
***2021 年 8 月*** 在 SIG Node 成功運行 [bug scrub](https://groups.google.com/g/kubernetes-dev/c/w2ghO4ihje0/m/VeEql1LJBAAJ)
...
子項目負責人 Elana Hashman 和 Sergey Kanzhelev 都被認爲是所有節點測試代碼的審批人，由 SIG node 和 SIG Testing 支持。
...
該子項目設定了一個目標，即在 1.23 發佈日期之前讓串行任務完全通過。

<!--Mike Miranda becomes a Kubernetes org member for his contributions
to the subproject.-->
Mike Miranda 因其對子項目的貢獻成爲 Kubernetes 組織成員。
...
***2021 年 11 月*** 在整個 2021 年， SIG Node 沒有合併或發佈的測試失敗。
過去版本中的許多古怪測試都已從阻止發佈的儀表板中刪除，因爲它們已被完全清理。

<!--Danielle Lancashire was recognized as a reviewer for SIG Node's subgroup, test code.-->
Danielle Lancashire 被公認爲 SIG Node 子組測試代碼的評審員。
...
到 1.23 版本凍結時，最後一次系列測試已修復，作業順利通過。
...
[![宣佈系列測試爲綠色](serial-tests-green.png)](https://kubernetes.slack.com/archives/C0BP8PW9G/p1638211041322900)
...
1.23 版本在測試質量和 CI 信號方面得到了特別的關注。SIG Node CI 子項目很自豪能夠爲這樣一個高質量的發佈做出貢獻，
...
[![Slack 大聲宣佈發佈的版本大多是綠色的](release-mostly-green.png)](https://kubernetes.slack.com/archives/C92G08FGD/p1637175755023200)

<!--***December 2021.*** An estimated 90% of test jobs were passing at the time of
the 1.23 release (up from 42% in August 2020).-->
***2021 年 12 月*** 在 1.23 版本發佈時，估計有 90% 的測試工作通過了測試（2020 年 8 月爲 42%）。
...
by the 1.24 release.-->
Dockershim 代碼已從 Kubernetes 中刪除。這影響了 SIG Node 近一半的測試作業，
SIG Node CI 子項目反應迅速，並重新確定了所有測試的目標。
SIG Node 是第一個完成 dockershim 外測試遷移的 SIG ，爲其他受影響的 SIG 提供了示例。
絕大多數新工作在引入時都已通過，無需進一步修復。
從 Kubernetes 中[將 dockershim 除名的工作](https://k8s.io/dockershim) 正在進行中。
隨着我們發現 dockershim 對 dockershim 的依賴性越來越大，dockershim 的刪除仍然存在一些問題，
...
我們還在改進測試的可調試性和去剝落方面不斷取得進展。

如果你對此感興趣，我們很樂意您能加入我們！
在調試測試失敗中有很多東西需要學習，它將幫助你熟悉 SIG Node 維護的代碼。
...
[KubeCon + CloudNativeCon Europe 2021](https://kccnceu2021.sched.com/event/iE8E/kubernetes-sig-node-intro-and-deep-dive-elana-hashman-red-hat-sergey-kanzhelev-google) 和
[KubeCon + CloudNative North America 2021](https://kccncna2021.sched.com/event/lV9D/kubenetes-sig-node-intro-and-deep-dive-elana-hashman-derek-carr-red-hat-sergey-kanzhelev-dawn-chen-google?iframe=no&w=100%&sidebar=yes&bg=no)。
加入我們的使命，保持 kubelet 和其他 SIG Node 組件的可靠性，確保順順利利發佈！
