...
在 1.0 版本發佈之前，Kubernetes 已經有了一個非常基礎的網路插件形式-大約在引入 Docker’s [libnetwork](https://github.com/docker/libnetwork) 和 Container Network Model ([CNM](https://github.com/docker/libnetwork/blob/master/docs/design.md)) 的時候。與 libnetwork 不同，Kubernetes 插件系統仍然保留它的 'alpha' 名稱。現在 Docker 的網路插件支持已經發布並得到支持，我們發現一個明顯的問題是 Kubernetes 尚未採用它。畢竟，供應商幾乎肯定會爲 Docker 編寫插件-我們最好還是用相同的驅動程序，對吧？
...
首先，Docker 的網路驅動程序設計中存在一些基本假設，這些假設會給我們帶來問題。
...
Docker 有一個“本地”和“全局”驅動程序的概念。本地驅動程序（例如 "bridge" ）以機器爲中心，不進行任何跨節點協調。全局驅動程序（例如 "overlay" ）依賴於 [libkv](https://github.com/docker/libkv) （一個鍵值存儲抽象庫）來協調跨機器。這個鍵值存儲是另一個插件接口，並且是非常低級的（鍵和值，沒有其他含義）。 要在 Kubernetes 叢集中運行類似 Docker's overlay 驅動程序，我們要麼需要叢集管理員來運行 [consul](https://github.com/hashicorp/consul), [etcd](https://github.com/coreos/etcd) 或 [zookeeper](https://zookeeper.apache.org/) 的整個不同實例 (see [multi-host networking](https://docs.docker.com/engine/userguide/networking/get-started-overlay/)) 否則我們必須提供我們自己的 libkv 實現，那被 Kubernetes 支持。
...
後者聽起來很有吸引力，並且我們嘗試實現它，但 libkv 接口是非常低級的，並且架構在內部定義爲 Docker 。我們必須直接暴露我們的底層鍵值存儲，或者提供鍵值語義（在我們的結構化API之上，它本身是在鍵值系統上實現的）。對於性能，可伸縮性和安全性原因，這些都不是很有吸引力。最終結果是，當使用 Docker 網路的目標是簡化事情時，整個系統將顯得更加複雜。
...
對於願意並且能夠運行必需的基礎架構以滿足 Docker 全局驅動程序並自己設定 Docker 的使用者， Docker 網路應該“正常工作。” Kubernetes 不會妨礙這樣的設置，無論項目的方向如何，該選項都應該可用。但是對於默認安裝，實際的結論是這對使用者來說是一個不應有的負擔，因此我們不能使用 Docker 的全局驅動程序（包括 "overlay" ），這消除了使用 Docker 插件的很多價值。
...
我們調查了爲 Docker [編寫 "bridge" CNM驅動程序](https://groups.google.com/g/kubernetes-sig-network/c/5MWRPxsURUw) 並運行 CNI 驅動程序。事實證明這非常複雜。首先， CNM 和 CNI 模型非常不同，因此沒有一種“方法”協調一致。 我們仍然有上面討論的全球與本地和鍵值問題。假設這個驅動程序會聲明自己是本地的，我們必須從 Kubernetes 獲取有關邏輯網路的信息。
...
不幸的是， Docker 驅動程序很難映射到像 Kubernetes 這樣的其他控制平面。具體來說，驅動程序不會被告知連接容器的網路名稱 - 只是 Docker 內部分配的 ID 。這使得驅動程序很難映射回另一個系統中存在的任何網路概念。
...
出於所有這些原因，我們選擇投資 CNI 作爲 Kubernetes 插件模型。這會有一些不幸的副作用。它們中的大多數都相對較小（例如， `docker inspect` 不會顯示 IP 地址），特別是由 `docker run` 啓動的容器可能無法與 Kubernetes 啓動的容器通信，如果網路集成商想要與 Kubernetes 完全集成，則必須提供 CNI 驅動程序。但另一方面， Kubernetes 將變得更簡單，更靈活，早期引入的許多醜陋的（例如設定 Docker 使用我們的網橋）將會消失。
