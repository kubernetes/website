...
在 1.0 版本發佈之前，Kubernetes 已經有了一個非常基礎的網絡插件形式-大約在引入 Docker’s [libnetwork](https://github.com/docker/libnetwork) 和 Container Network Model ([CNM](https://github.com/docker/libnetwork/blob/master/docs/design.md)) 的時候。與 libnetwork 不同，Kubernetes 插件系統仍然保留它的 'alpha' 名稱。現在 Docker 的網絡插件支持已經發布並得到支持，我們發現一個明顯的問題是 Kubernetes 尚未採用它。畢竟，供應商幾乎肯定會爲 Docker 編寫插件-我們最好還是用相同的驅動程序，對吧？
...
在進一步說明之前，重要的是記住 Kubernetes 是一個支持多種容器運行時的系統， Docker 只是其中之一。配置網絡只是每一個運行時的一個方面，所以當人們問起“ Kubernetes 會支持CNM嗎？”，他們真正的意思是“ Kubernetes 會支持 Docker 運行時的 CNM 驅動嗎？”如果我們能夠跨運行時實現通用的網絡支持會很棒，但這不是一個明確的目標。
...
首先，Docker 的網絡驅動程序設計中存在一些基本假設，這些假設會給我們帶來問題。
...
Docker 有一個“本地”和“全局”驅動程序的概念。本地驅動程序（例如 "bridge" ）以機器爲中心，不進行任何跨節點協調。全局驅動程序（例如 "overlay" ）依賴於 [libkv](https://github.com/docker/libkv) （一個鍵值存儲抽象庫）來協調跨機器。這個鍵值存儲是另一個插件接口，並且是非常低級的（鍵和值，沒有其他含義）。 要在 Kubernetes 集羣中運行類似 Docker's overlay 驅動程序，我們要麼需要集羣管理員來運行 [consul](https://github.com/hashicorp/consul), [etcd](https://github.com/coreos/etcd) 或 [zookeeper](https://zookeeper.apache.org/) 的整個不同實例 (see [multi-host networking](https://docs.docker.com/engine/userguide/networking/get-started-overlay/)) 否則我們必須提供我們自己的 libkv 實現，那被 Kubernetes 支持。
...
後者聽起來很有吸引力，並且我們嘗試實現它，但 libkv 接口是非常低級的，並且架構在內部定義爲 Docker 。我們必須直接暴露我們的底層鍵值存儲，或者提供鍵值語義（在我們的結構化API之上，它本身是在鍵值系統上實現的）。對於性能，可伸縮性和安全性原因，這些都不是很有吸引力。最終結果是，當使用 Docker 網絡的目標是簡化事情時，整個系統將顯得更加複雜。
...
對於願意並且能夠運行必需的基礎架構以滿足 Docker 全局驅動程序並自己配置 Docker 的用戶， Docker 網絡應該“正常工作。” Kubernetes 不會妨礙這樣的設置，無論項目的方向如何，該選項都應該可用。但是對於默認安裝，實際的結論是這對用戶來說是一個不應有的負擔，因此我們不能使用 Docker 的全局驅動程序（包括 "overlay" ），這消除了使用 Docker 插件的很多價值。
...
Docker 的網絡模型做出了許多對 Kubernetes 無效的假設。在 docker 1.8 和 1.9 版本中，它包含一個從根本上有缺陷的“發現”實現，導致容器中的 `/etc/hosts` 文件損壞 ([docker #17190](https://github.com/docker/docker/issues/17190)) - 並且這不容易被關閉。在 1.10 版本中，Docker 計劃 [捆綁一個新的DNS服務器](https://github.com/docker/docker/issues/17195)，目前還不清楚是否可以關閉它。容器級命名不是 Kubernetes 的正確抽象 - 我們已經有了自己的服務命名，發現和綁定概念，並且我們已經有了自己的 DNS 模式和服務器（基於完善的 [SkyDNS](https://github.com/skynetservices/skydns) ）。捆綁的解決方案不足以滿足我們的需求，但不能禁用。
...
另一方面， CNI 在哲學上與 Kubernetes 更加一致。它比 CNM 簡單得多，不需要守護進程，並且至少有合理的跨平臺（ CoreOS 的 [rkt](https://coreos.com/rkt/docs/) 容器運行時支持它）。跨平臺意味着有機會啓用跨運行時（例如 Docker ， Rocket ， Hyper ）運行相同的網絡配置。 它遵循 UNIX 的理念，即做好一件事。
...
此外，包裝 CNI 插件並生成更加個性化的 CNI 插件是微不足道的 - 它可以通過簡單的 shell 腳本完成。 CNM 在這方面要複雜得多。這使得 CNI 對於快速開發和迭代是有吸引力的選擇。早期的原型已經證明，可以將 kubelet 中幾乎 100％ 的當前硬編碼網絡邏輯彈出到插件中。
...
我們調查了爲 Docker [編寫 "bridge" CNM驅動程序](https://groups.google.com/g/kubernetes-sig-network/c/5MWRPxsURUw) 並運行 CNI 驅動程序。事實證明這非常複雜。首先， CNM 和 CNI 模型非常不同，因此沒有一種“方法”協調一致。 我們仍然有上面討論的全球與本地和鍵值問題。假設這個驅動程序會聲明自己是本地的，我們必須從 Kubernetes 獲取有關邏輯網絡的信息。
...
不幸的是， Docker 驅動程序很難映射到像 Kubernetes 這樣的其他控制平面。具體來說，驅動程序不會被告知連接容器的網絡名稱 - 只是 Docker 內部分配的 ID 。這使得驅動程序很難映射回另一個系統中存在的任何網絡概念。
...
出於所有這些原因，我們選擇投資 CNI 作爲 Kubernetes 插件模型。這會有一些不幸的副作用。它們中的大多數都相對較小（例如， `docker inspect` 不會顯示 IP 地址），特別是由 `docker run` 啓動的容器可能無法與 Kubernetes 啓動的容器通信，如果網絡集成商想要與 Kubernetes 完全集成，則必須提供 CNI 驅動程序。但另一方面， Kubernetes 將變得更簡單，更靈活，早期引入的許多醜陋的（例如配置 Docker 使用我們的網橋）將會消失。
