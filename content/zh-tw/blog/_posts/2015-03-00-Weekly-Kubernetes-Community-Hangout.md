---
title: "Kubernetes 社區每週聚會筆記 - 2015 年 3 月 27 日"
date: 2015-03-28
...
-->
    ...
    * 作爲代理與主機建立連接，找出 pod 所在的節點，代理與 kubelet 的連接，這一點很有趣。通過 nsenter。

    * 使用 SPDY 通過 HTTP 進行多路複用流式傳輸

    * 還有互動模式：

    * 假設第一個容器，可以使用 -c $CONTAINER 一個特定的。

    * 如果在容器中預先安裝了 gdb，則可以交互地將其附加到正在運行的進程中

        * backtrace、symbol tbles、print 等。  使用gdb可以做的大多數事情。

    * 也可以用精心製作的參數在上面運行 rsync 或者在容器內設置 sshd。
...
* 如果能在博客上發表這方面的文章就太好了
* wheezy 中的 nginx 版本太舊，無法支持所需的主代理功能
...
2\. Clayton: 我們的社區組織在哪裏，例如 kubernetes UI 組件？

* google-containers-ui IRC 頻道，郵件列表。
* Tim: google-containers 前綴是歷史的，應該只做 "kubernetes-ui"
