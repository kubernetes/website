...
以及如何避免它們的建議。
無論你只是剛開始嘗試 Kubernetes，還是已經在管理生產集羣，
...
-->
**背景**：
在 Kubernetes 中，資源請求和限制對於高效的集羣管理至關重要。
...
用戶過早訪問了它，遇到了奇怪的超時，我花了幾個小時才找到問題。
...
-->
有關爲容器配置存活態、就緒態和啓動探針的全面說明，
請參閱 Kubernetes 官方文檔中的[配置存活、就緒和啓動探針](/zh-cn/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/)。
...
- **將日誌與 Prometheus 指標對應起來**：與應用日誌同時跟蹤集羣級數據。
...
可能顯著不同時，常會發生這種情況。
如果略過定製這一步驟，針對一個環境優化的配置可能會導致負載在另一個環境下不穩定、
性能差或暴露安全漏洞。
...
- 使用環境覆蓋層或 [kustomize](https://kustomize.io/) 來維護共享基礎，
  同時爲每個環境定製資源請求、副本數或配置。
- 將環境特定的配置提取到 ConfigMap 和/或 Secret 中。
  你可以使用專門的工具如 [Sealed Secrets](https://github.com/bitnami-labs/sealed-secrets) 來管理機密數據。
- 爲生產環境中的擴縮需求做規劃。
  你的開發集羣可能只需要最少的 CPU/內存，但生產環境可能需要顯著更多。
...
-->
**常見誤區**：在集羣中遺留未使用或過時的資源——例如 Deployment、Service、ConfigMap 或 PersistentVolumeClaim。
這種情況經常發生，因爲 Kubernetes 不會自動刪除資源，除非明確指示；
同時系統也沒有內建機制來追蹤資源的歸屬或過期時間。
隨着時間推移，這些被遺忘的對象可能不斷累積，
佔用集羣資源、增加雲成本，並造成運維上的混亂，
...
- **爲所有資源添加標籤**：使用用途或所有者標籤。
  這樣，你可以輕鬆查詢不再需要的資源。
- **定期審計集羣**：運行 `kubectl get all -n <namespace>` 查看實際運行的內容，
  並確認它們都是合法的。
...
-->
## 6. 過早深入複雜的網絡配置 {#6-diving-too-deep-into-networking-too-soon}
...
-->
**常見誤區**：在完全理解 Kubernetes 原生網絡原語之前引入高級網絡解決方案——
如服務網格、自定義 CNI 插件或多集羣通信。
這通常發生在團隊使用外部工具實現流量路由、可觀測性或 mTLS 等功能，
而沒有首先掌握核心 Kubernetes 網絡的工作原理：
...
- 確保理解集羣內的流量流向、服務發現機制以及 DNS 的配置方式。
...
-->
**常見誤區**：以不安全的方式配置部署工作負載，例如以 root 用戶運行容器、使用 `latest` 鏡像標籤、
禁用安全上下文（security context），或分配過於寬泛的 RBAC 角色（如 `cluster-admin`）。
這些做法之所以普遍存在，是因爲 Kubernetes 默認並不會強制實施嚴格的安全策略——
該平臺在設計上追求靈活性而非強約束性。如果未顯式配置安全策略，集羣可能面臨容器逃逸、
未經授權的權限提升或由於未固定鏡像導致的意外生產變更等風險。
...
- 將鏡像固定到特定版本（不要再使用 `:latest`！）。這有助於你瞭解實際部署的內容。
