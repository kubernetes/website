...
-->
自 Kubernetes 誕生之初，持久資料以及如何解決有狀態應用程式的需求一直是一個重要的話題。
對無狀態部署的支持是很自然的、從一開始就存在的，並引起了人們的關注，變得衆所周知。
從早期開始，我們也致力於更好地支持有狀態應用程式，每個版本都增加了可以在 Kubernetes 上運行的範圍。
...
-->
消息隊列、資料庫、叢集檔案系統：這些是具有不同儲存要求的解決方案的一些示例，
如今這些解決方案越來越多地部署在 Kubernetes 中。
處理來自許多不同供應商的臨時和持久儲存（本地或遠程、檔案或塊），同時考慮如何提供使用者期望的所需彈性和資料一致性，
...
-->
**Xing Yang (XY)**：我是 VMware 的技術主管，從事雲原生儲存方面的工作。我也是 SIG Storage 的聯合主席。
...
並提醒人們發佈截止日期等。有關 SIG 的更多資訊，請查閱[社區頁面](https://github.com/kubernetes/community/tree/master/sig-storage)。
...
[K8s 資料保護工作組](https://github.com/kubernetes/community/blob/master/wg-data-protection/README.md)。
SIG Storage 擁有或共同擁有資料保護工作組正在討論的功能特性。
...
**FSM**：儲存是很多模塊的基礎組件，尤其是 Kubernetes：你認爲 Kubernetes 在儲存管理方面的具體挑戰是什麼?
...
還有 Kubelet 可以將 PVC 掛載到 Pod 上。當然，CSI 驅動程式也參與其中。
...
這並不是特定於儲存的。CRD 是一種擴展 Kubernetes 功能的好方法，同時又不會向 Kubernetes 核心本身添加太多代碼。
...
因此，這對儲存供應商來說就成了一個問題：現在部署這些非驅動程式特定的通用組件成爲他們的責任。
如果客戶需要使用多個儲存系統，且部署多個 CSI 驅動，可能會導致衝突。

<!--
**FSM**: Not only the complexity of a single storage system, you have to consider how they will be used together in Kubernetes?
-->
**FSM**：不僅要考慮單個儲存系統的複雜性，還要考慮它們在 Kubernetes 中如何一起使用？
...
-->
**XY**：是的，有許多不同的儲存系統可以爲 Kubernetes 中的容器提供儲存。它們的工作方式不同。找到適合所有人的解決方案是具有挑戰性的。
...
-->
**FSM**：Kubernetes 中的儲存還涉及與外部解決方案的交互，可能比 Kubernetes 的其他部分更多。
...
-->
**XY**：是的，這絕對是具有挑戰性的。最初 Kubernetes 儲存具有 in-tree 卷插件介面。
多家儲存供應商實現了 in-tree 介面，並在 Kubernetes 核心代碼庫中擁有卷插件。這引起了很多問題。
如果卷插件中存在錯誤，它會影響整個 Kubernetes 代碼庫。所有卷插件必須與 Kubernetes 一起發佈。
如果儲存供應商需要修復其插件中的錯誤或希望與他們自己的產品版本保持一致，這是不靈活的。
...
-->
**XY**：沒錯，接下來就是[容器儲存介面](https://kubernetes-csi.github.io/docs/)（CSI）。
這是一個試圖設計通用儲存介面的行業標準，以便儲存供應商可以編寫一個插件並讓它在一系列容器編排系統（CO）中工作。
現在 Kubernetes 是主要的 CO，但是在 CSI 剛開始的時候，除了 Kubernetes 之外，還有 Docker、Mesos、Cloud Foundry。
CSI 驅動程式是 out-of-tree 的，因此可以按照自己的節奏進行錯誤修復和發佈。
...
-->
**FSM**：將驅動程式從 Kubernetes 主倉移到 CSI 中是一項重要的改進。
...
-->
**XY**： CSI 介面是對 in-tree 卷插件介面的改進，但是仍然存在挑戰。有很多儲存系統。
目前在 [CSI 驅動程式文檔中列出了 100 多個 CSI 驅動程式](https://kubernetes-csi.github.io/docs/drivers.html)。
這些儲存系統也非常多樣化。因此，很難設計一個適用於所有人的通用 API。
我們在 CSI 驅動層面引入了功能，但當同一驅動設定的卷具有不同的行爲時，我們也會面臨挑戰。
前幾天我們剛剛開會討論每種卷 CSI 驅動程式功能。
當同一個驅動程式同時支持塊卷和檔案卷時，我們在區分某些 CSI 驅動程式功能時遇到了問題。
我們將召開後續會議來討論這個問題。
...
們可以看到管道中有一些與儲存相關的 [KEPs](https://bit.ly/k8s125-enhancements)。
你是否認爲這個版本對 SIG 特別重要？
...
* [CSI 遷移](https://github.com/kubernetes/enhancements/tree/master/keps/sig-storage/625-csi-migration)
  ...
  CSI 驅動程式，並最終刪除 in-tree 卷插件。在 1.25 版本中，有 7 個 KEP 與 CSI 遷移有關。
  有一個核心 KEP 用於通用的 CSI 遷移功能。它的目標是在 1.25 版本中達到 GA。
  GCE PD 和 AWS EBS 的 CSI 遷移以 GA 爲目標。vSphere 的 CSI 遷移的目標是在預設情況下啓用特性門控，
  在 1.25 版本中達到 Beta。Ceph RBD 和 PortWorx 的目標是達到 Beta，預設關閉特性門控。
...
-->
* 我要強調的第二個是 [COSI，容器對象儲存介面](https://github.com/kubernetes-sigs/container-object-storage-interface-spec)。
  這是 SIG Storage 下的一個子項目。COSI 提出對象儲存 Kubernetes API 來支持 Kubernetes 工作負載的對象儲存操作的編排。
  它還爲對象儲存提供商引入了 gRPC 介面，以編寫驅動程式來設定儲存桶。COSI 團隊已經在這個項目上工作兩年多了。
...
* 我要提到的另一個功能是 [CSI 臨時卷](https://github.com/kubernetes/enhancements/issues/596)支持。
  ...
  例如設定、Secrets、身份、變量或類似資訊。這最初是在 1.15 版本中作爲一個 Alpha 功能引入的，現在它的目標是在 1.25 版本中達到 GA。
...
-->
**XY**：CSI 遷移絕對是 SIG 投入大量精力的領域之一，並且現在已經進行了多個版本。它還涉及來自多個雲提供商和儲存供應商的工作。
...
它有很多關於如何開始的資訊。[SIG 年度報告](https://github.com/kubernetes/community/blob/master/sig-storage/annual-report-2021.md)告訴你我們每年做了什麼。
查看貢獻指南。它有一些演示的鏈接，可以幫助你熟悉 Kubernetes 儲存概念。
