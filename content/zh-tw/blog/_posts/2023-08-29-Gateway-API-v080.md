...
這導致在 Gateway API **一致性設定文件**的工作重新啓動，如 [GEP-1709] 中所述。
...
the `Mesh` profile.
-->
一致性設定文件的基本思想是，我們可以定義 Gateway API 的子集，並允許實現選擇（並記錄）他們符合哪些子集。
GAMMA 正在添加一個名爲 `Mesh` 的新設定文件，其描述在 [GEP-1686] 中，僅檢查由 GAMMA 定義的網格功能。
此時，Kuma 2.3+、Linkerd 2.14+ 和 Istio 1.16+ 都符合 `Mesh` 設定文件的標準。
...
- [GEP-1686] 在 [GEP-1709] 的工作基礎上，爲聲明符合 Gateway API 的服務網格定義了一個一致性設定文件。
