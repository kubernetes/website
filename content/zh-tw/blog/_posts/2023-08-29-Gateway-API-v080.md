...
而不需要 Gateway API 的用戶重新學習他們瞭解的有關 API 的一切。
...
你可以在 [Gateway API Mesh 路由文檔][mesh-routing]和 [GEP-1426] 中找到所有詳細信息，
...
Ingress 控制器處理的，A/B 測試可以發生在應用程序的調用圖中的任何位置。
...
這導致在 Gateway API **一致性配置文件**的工作重新啓動，如 [GEP-1709] 中所述。
...
the `Mesh` profile.
-->
一致性配置文件的基本思想是，我們可以定義 Gateway API 的子集，並允許實現選擇（並記錄）他們符合哪些子集。
GAMMA 正在添加一個名爲 `Mesh` 的新配置文件，其描述在 [GEP-1686] 中，僅檢查由 GAMMA 定義的網格功能。
此時，Kuma 2.3+、Linkerd 2.14+ 和 Istio 1.16+ 都符合 `Mesh` 配置文件的標準。
...
CRD 中的信息的 [CEL 驗證][cel]。取決於你使用的 Kubernetes 版本，這一轉換的影響有些不同：
...
因此，Gateway API 不再支持這些版本，不幸的是，在這些集羣版本中無法安裝 Gateway API v0.8.0，
...
有關此更改以及此版本中包含的所有其他內容的更多信息，請參見 [v0.8.0 發佈說明][v0.8.0 release notes]。
...
因爲這是一個基於 CRD 的 API，所以你可以在任何 Kubernetes 1.23+ 集羣上安裝最新版本。
...
- [GEP-1686] 在 [GEP-1709] 的工作基礎上，爲聲明符合 Gateway API 的服務網格定義了一個一致性配置文件。
