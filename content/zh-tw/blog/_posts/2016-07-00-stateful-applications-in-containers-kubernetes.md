...
專注於有狀態應用程序和聯邦叢集是我對 1.3 如此興奮的兩個原因。
Kubernetes 對有狀態應用程序（例如 Cassandra、Kafka 和 MongoDB）的支持至關重要。
重要服務依賴於數據庫、鍵值存儲、消息隊列等。
此外，隨着應用程序的發展爲全球數百萬使用者提供服務，僅依靠一個數據中心或容器叢集將無法正常工作。
聯邦叢集允許使用者跨多個叢集和數據中心部署應用程序，以實現規模和彈性。
...
Pet Set 在每個數據庫副本的啓動階段進行排序（例如），以確保有序的主/從設定。
...
我們的[第一個實現](https://github.com/kubernetes/kubernetes/pull/13580)使 Kubernetes 調度程序意識到每個叢集節點本地的網路和存儲資源。
...
我們還正在研究有關通過 Kubernetes 跨叢集聯邦服務改善應用程序放置，遷移和跨容器叢集故障轉移的建議。
