---
layout: blog
title: '使用 Microk8s 在 Linux 上本地運行 Kubernetes'
date: 2019-11-26
...
-->
這種差異有其優點和缺點。在這裏，我們將討論一些有趣的區別，並且基於 VM 的方法和非 VM 方法的好處。第一個因素是跨平臺的移植性。雖然 Minikube VM 可以跨操作系統移植——它不僅支持 Linux，還支持 Windows、macOS、甚至 FreeBSD，但 Microk8s 需要 Linux，而且只在[那些支持 snaps](https://snapcraft.io/docs/installing-snapd) 的發行版上。支持大多數流行的 Linux 發行版。
<!--
...
-->
最後，項目似乎遵循了不同的發佈節奏和策略。Microk8s 和 snaps 通常提供[渠道](https://snapcraft.io/docs/channels)允許你使用測試版和發佈 KUbernetes 新版本的候選版本，同樣也提供先前穩定版本。Microk8s 通常幾乎立刻發佈 Kubernetes 上游的穩定版本。
<!--
...
一個[支持 snaps](https://snapcraft.io/docs/installing-snapd) 的 Linux 發行版是被需要的。這篇指南，我們將會用支持 snaps 且即開即用的 Ubuntu 18.04 LTS。如果你對運行在 Windows 或者 Mac 上的 MicroK8s 感興趣，你應該檢查[多通道](https://multipass.run)，安裝一個快速的 Ubuntu VM，作爲在你的系統上運行虛擬機 Ubuntu 的官方方式。
<!--
...
-->
## 使用 microk8s
使用 MicrosK8s 就像和安裝它一樣便捷。MicroK8s 本身包括一個 `kubectl` 庫，該庫可以通過執行 `microk8s.kubectl` 命令去訪問。例如：
```shell
...
-->
當使用前綴 `microk8s.kubectl` 時，允許在沒有影響的情況下並行地安裝另一個系統級的 kubectl，你可以便捷地使用 `snap alias` 命令擺脫它：
```shell
