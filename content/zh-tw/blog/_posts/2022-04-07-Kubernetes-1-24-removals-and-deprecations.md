...
-->
已經有一些文檔指南，提供了關於從 dockershim 遷移到與 Kubernetes 直接兼容的容器運行時的有用資訊。
...
-->
有關 Kubernetes 爲何不再使用 dockershim 的更多資訊，
...
* [動態 kubelet 設定](https://github.com/kubernetes/enhancements/issues/281): `DynamicKubeletConfig`
  ...
  更多詳細資訊，請參閱[“移除動態 kubelet 設定” 的 KEP](https://github.com/kubernetes/enhancements/issues/281)。
<!--
...
* [動態日誌清洗](https://github.com/kubernetes/kubernetes/pull/107207)：實驗性的動態日誌清洗功能已被棄用，
  ...
  以防止各種類型的敏感資訊通過日誌泄漏。有關更多資訊和替代方法，請參閱
...
* [從 kubelet 中移除 Dockershim](https://github.com/kubernetes/enhancements/issues/2221)：Docker
  的容器運行時介面(CRI)（即 Dockershim）目前是 kubelet 代碼中內置的容器運行時。它在 1.20 版本中已被棄用。
...
-->
* [Pod 調度的儲存容量追蹤](https://github.com/kubernetes/enhancements/issues/1472)：CSIStorageCapacity API
  支持通過 CSIStorageCapacity 對象暴露當前可用的儲存容量，並增強了使用帶有延遲綁定的 CSI 卷的 Pod 的調度。
  CSIStorageCapacity API 自 1.24 版本起提供穩定版本。升級到穩定版的 API 將棄用 v1beta1 CSIStorageCapacity API。
  更多資訊請參見 [Pod 調度儲存容量約束 KEP](https://github.com/kubernetes/enhancements/tree/master/keps/sig-storage/1472-storage-capacity-tracking)。
<!--
...
* [kubeadm 控制面節點上不再存在 `master` 標籤](https://github.com/kubernetes/kubernetes/pull/107533)。
  ...
  只會添加 'node-role.kubernetes.io/control-plane' 標籤。更多資訊請參考
...
* [VolumeSnapshot v1beta1 CRD 在 1.24 版本中將被移除](https://github.com/kubernetes/enhancements/issues/177)。
  ...
  的卷快照和恢復功能，在 1.20 版本中進入測試版。該功能提供標準化 API 設計 (CRD ) 併爲 CSI 卷驅動程式添加了 PV 快照/恢復支持，
  VolumeSnapshot v1beta1 在 1.21 版本中已被棄用，現在不受支持。更多資訊請參考
...
例如 Pod 或執行 Docker 命令的第三方工具或 Docker 設定檔案中的私有映像檔庫。
...
關於 API 遷移過程的更多資訊可以在[已棄用 API 的遷移指南](/zh-cn/docs/reference/using-api/deprecation-guide/)中找到。
按照[安裝 `kubectl convert` 插件](/zh-cn/docs/tasks/tools/install-kubectl-linux/#install-kubectl-convert-plugin)
文檔下載並安裝 `kubectl-convert` 二進制檔案。
...
有關棄用和移除過程的資訊，請查看 Kubernetes 官方[棄用策略](/zh-cn/docs/reference/using-api/deprecation-policy/#deprecating-parts-of-the-api)文檔。
