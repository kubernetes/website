...
保護 Kubernetes 集羣時首先要考慮的事情之一是加密靜態的 etcd 數據。
KMS 爲供應商提供了一個接口，以便利用存儲在外部密鑰服務中的密鑰來執行此加密。
...
KMS 加密驅動使用信封加密方式來加密 etcd 中的數據，使用數據加密密鑰（DEK）對數據進行加密。
DEK 使用在遠程 KMS 中存儲和管理的密鑰加密密鑰（KEK）進行加密。
使用 KMS v1，每次加密都會生成一個新的 DEK。
使用 KMS v2，只有在服務器啓動時且 KMS 插件通知 API 服務器發生 KEK 輪換時纔會生成新的 DEK。
...
-->
如果你運行的是虛擬機（VM）節點，其中啓用此特性的節點使用了 VM 的狀態存儲，
則不得使用 KMS v2。

對於 KMS v2，API 服務器使用帶有 12 字節隨機數（8 字節原子計數器和 4 字節隨機數據）的
...
在 KMS v2 中，我們對 KMS 加密提供程序的性能進行了重大改進。對於 KMS v1，
每次加密都會生成一個新的 DEK。這意味着對於每個寫入請求，API 服務器都會調用
KMS 插件以使用遠程 KEK 加密 DEK。爲避免每個讀取請求都會調用 KMS 插件，
API 服務器必須緩存 DEK。當 API 服務器重新啓動時，
它必須根據緩存大小爲 etcd 存儲中的每個 DEK 調用 KMS 插件來填充緩存。
這對 API 服務器來說是一個很大的開銷。使用 KMS v2，API 服務器在啓動時生成一個 DEK 並將其緩存。
API 服務器還調用 KMS 插件以使用遠程 KEK 加密 DEK。這是啓動時和 KEK 輪換時的一次性調用。
在此之後，API 服務器使用緩存的 DEK 來加密資源。這樣做減少了對 KMS 插件的調用次數，
並改善了 API 服務器請求的整體延遲。
...
服務器加密資源所花費的時間。使用的指標是
[`apiserver_storage_transformation_duration_seconds`](https://kubernetes.io/zh-cn/docs/reference/instrumentation/metrics/)。
對於 KMS v1，測試在具有 2 個節點的託管 Kubernetes v1.25 集羣上運行。
測試期間集羣上沒有額外的負載。對於 KMS v2，
測試是在具有以下[集羣配置](https://github.com/kubernetes/kubernetes/blob/release-1.27/test/e2e/testing-manifests/auth/encrypt/kind.yaml)的
...
-->
你可以通過閱讀[使用 KMS 驅動進行數據加密](/zh-cn/docs/tasks/administer-cluster/kms-provider/)，
