...
-->
有關 `KubeletConfiguration` 的更多詳細信息，請參閱[本節](#configure-kubelets-using-kubeadm)。
...
- 當前，kubelet 無法自動檢測容器運行時使用的 cgroup 驅動程序，
  但是值 `--cgroup-driver` 必須與容器運行時使用的 cgroup 驅動程序匹配，以確保 kubelet 的健康運行狀況。
...
-->
當 `/etc/kubernetes/kubelet.conf` 文件被寫入後，kubelet 就完成了 TLS 引導過程。
kubeadm 在完成 TLS 引導過程後將刪除 `/etc/kubernetes/bootstrap-kubelet.conf` 文件。
...
-->
此文件指定由 kubeadm 爲 kubelet 管理的所有文件的默認位置。
...
-->
## Kubernetes 可執行文件和軟件包內容    {#kubernetes-binaries-and-package-contents}

<!--
The DEB and RPM packages shipped with the Kubernetes releases are:
-->
Kubernetes 版本對應的 DEB 和 RPM 軟件包是：
...
| 軟件包名稱   | 描述        |
|--------------|-------------|
| `kubeadm`    | 給 kubelet 安裝 `/usr/bin/kubeadm` CLI 工具和 [kubelet 的 systemd drop-in 文件](#the-kubelet-drop-in-file-for-systemd)。 |
| `kubelet`    | 安裝 `/usr/bin/kubelet` 可執行文件。 |
| `kubectl`    | 安裝 `/usr/bin/kubectl` 可執行文件。 |
| `cri-tools` | 從 [cri-tools git 倉庫](https://github.com/kubernetes-sigs/cri-tools)中安裝 `/usr/bin/crictl` 可執行文件。 |
| `kubernetes-cni` | 從 [plugins git 倉庫](https://github.com/containernetworking/plugins)中安裝 `/opt/cni/bin` 可執行文件。|
