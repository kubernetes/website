...
如果您使用 [kubeadm](/docs/reference/setup-tools/kubeadm/) 安裝 Kubernetes，
...
* API 伺服器端點的伺服器憑證
* etcd 伺服器的伺服器憑證
* 每個 kubelet 的[伺服器憑證](/docs/reference/access-authn-authz/kubelet-tls-bootstrapping/#client-and-serving-certificates)
  （每個{{< glossary_tooltip text="節點" term_id="node" >}}上都會執行 kubelet）
* [前端代理伺服器（front-proxy）](/docs/tasks/extend-kubernetes/configure-aggregation-layer/)伺服器端憑證（選用）
...
* [前端代理伺服器](/docs/tasks/extend-kubernetes/configure-aggregation-layer/)的用戶端憑證（選用）
...
-->
只有在您執行 kube-proxy 來支援[擴展 API 伺服器](/docs/tasks/extend-kubernetes/setup-extension-api-server/)的情況下，
才需要前端代理的憑證。
{{< /note >}}
...
關於建立自有憑證授權機構的詳細資訊，請參閱[憑證](/docs/tasks/administer-cluster/certificates/)。
若要深入了解如何管理憑證，請參閱[使用 kubeadm 管理憑證](/docs/tasks/administer-cluster/kubeadm/kubeadm-certs/)。
...
| front-proxy-ca.crt,key | kubernetes-front-proxy-ca | 用於[前端代理](/docs/tasks/extend-kubernetes/configure-aggregation-layer/) |
...
例如 [kubeadm](/docs/reference/setup-tools/kubeadm/) 所使用的負載平衡器固定 IP 與/或 DNS 名稱，
...
這在 [CertificateSigningRequest](/docs/reference/kubernetes-api/authentication-resources/certificate-signing-request-v1#CertificateSigningRequest) 類型的 `.spec.usages` 欄位中也有詳細說明：
...
-->
憑證應放置在建議的路徑中（就像 [kubeadm](/docs/reference/setup-tools/kubeadm/) 所用的）。
...
想了解更多細節，請參閱[節點授權](/docs/reference/access-authn-authz/node/)。
{{< /note >}}
