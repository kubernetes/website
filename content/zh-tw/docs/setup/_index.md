...
安裝 Kubernetes 時，請根據以下條件選擇安裝類型：易於維護、安全性、可控制性、可用資源以及操作和管理 Kubernetes 集羣所需的專業知識。
...
-->
你可以[下載 Kubernetes](/zh-cn/releases/download/)，在本地機器、雲或你自己的數據中心上部署 Kubernetes 集羣。

諸如 {{< glossary_tooltip text="kube-apiserver" term_id="kube-apiserver" >}} 或
{{< glossary_tooltip text="kube-proxy" term_id="kube-proxy" >}}
等某些 [Kubernetes 組件](/zh-cn/docs/concepts/overview/components/)可以在集羣中以[容器鏡像](/zh-cn/releases/download/#container-images)部署。

**建議**儘可能將 Kubernetes 組件作爲容器鏡像運行，並且讓 Kubernetes 管理這些組件。
但是運行容器的相關組件 —— 尤其是 kubelet，不在此列。

如果你不想自己管理 Kubernetes 集羣，則可以選擇託管服務，包括[經過認證的平臺](/zh-cn/docs/setup/production-environment/turnkey-solutions/)。
...
或生態系統中的工具在本地計算機上設置 Kubernetes 集羣。
...
請考慮要自己管理 Kubernetes 集羣（或相關抽象）的哪些方面，將哪些託付給提供商。

對於你自己管理的集羣，官方支持的用於部署 Kubernetes 的工具是
...
- 爲新集羣選擇[容器運行時](/zh-cn/docs/setup/production-environment/container-runtimes/)
- 瞭解集羣設置的[最佳實踐](/zh-cn/docs/setup/best-practices/)

Kubernetes 的設計是讓其{{< glossary_tooltip term_id="control-plane" text="控制平面" >}}在 Linux 上運行的。
在集羣中，你可以在 Linux 或其他操作系統（包括 Windows）上運行應用程序。

- 學習[配置包含 Windows 節點的集羣](/zh-cn/docs/concepts/windows/)
