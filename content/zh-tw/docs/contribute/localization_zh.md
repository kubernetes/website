...
本節詳述文檔中文本地化過程中須注意的事項。
這裏列舉的內容包含了**中文本地化小組**早期給出的指導性建議和後續實踐過程中積累的經驗。
在閱讀、貢獻、評閱中文本地化文檔的過程中，如果對本文的指南有任何改進建議，
...
1. Followed by a long step with code snippets and notes ...
   this is a really long item
-->
2. 這裏是第二個編號，但需要顯式給出數字，不能沿用英文編號。
   縮進內容同上，3 個空格。
   即使有三個反引號的代碼段或者短代碼，都按 3 個空格縮進。
...
- `title`、`description` 的內容要翻譯，其他字段一般不必（甚至不可）翻譯。
- `reviewers` 部分要刪除，不然中文譯文會轉給英文作者來審閱。

#### 短代碼（shortcode）處理   {#shortcode}

通過 HTML 註釋的短代碼仍會被運行，因此需要額外小心。建議處理方式：
...
{{< note >}}
現行風格與之前風格有些不同，這是因爲較新的 Hugo 版本已經能夠正確處理短代碼中的註釋段落。
保持註釋掉的英文與譯文都在短代碼內更便於維護。
{{< /note >}}

### 博客譯者署名    {#blog-translators-signature}

翻譯一篇博客需要花費大量的時間和精力，添加署名是對譯者工作的認可，
也有利於激勵貢獻者同步英文博客，提升博客質量。
如要添加譯者署名，可在作者下面一行添加譯者相關內容。例如：
...
#### 資源名稱或字段不譯   {#resource-name-or-fields}

根據英文原文寫作風格約定【也在持續修訂改進】，對 Kubernetes 中的 API
資源均按其規範中所給的大小寫形式書寫，例如：英文中會使用 Deployment 而不是
deployment 來表示名爲 "Deployment" 的 API 資源類型和對象實例。

對這類詞語，一般不應翻譯。

{{< note >}}
英文原文在這方面並不嚴謹，譯者或中文譯文的評閱者要非常留心。
比如 Secret 資源，很多時候被誤寫爲 secret。
這時在本地化版本中一定不能譯爲“祕密”，以免與原文的語義不符。
{{< /note >}}
...
如果超級鏈接的目標是 Kubernetes 網站之外的純英文網頁，鏈接中的內容**可以**不翻譯。
例如：
...
{{< note >}}
注意，這裏的 `installation` 與 `參閱` 之間留白，因爲解析後屬於中英文混排的情況。
{{< /note >}}

### 標點符號   {#punctuations}

1. 譯文中標點符號要使用全角字符，除非以下兩種情況：
...
由於整個文檔站點會隨着 Kubernetes 項目的開發進展而演化，英文版本的網站內容會不斷更新。
...
英文 Markdown 中的各級標題會自動生成錨點，以便從其他頁面中鏈接。
在譯爲中文後，相應的鏈接必然會失效。爲防止這類問題，
建議在翻譯各級標題時，使用英文方式顯式給出鏈接錨點。例如：
...
此類問題對於概念部分的頁面最爲突出，需要格外注意。

### 中文鏈接目標   {#link-to-zh-pages}

由於大部分頁面已經完成中文本地化，這意味着很多鏈接可以使用中文版本作爲目標。
例如：
...
...
-->
更多的信息可參考[卷](/zh-cn/docs/concepts/storage/)頁面。
```
...
- 譯文 Markdown 中不要使用長行，應適當斷行。
  ...
  * 不要在兩個中文字符中間斷行，因爲這樣會造成中文字符中間顯示一個多餘空格，
    如果句子太長，可以從中文與非中文符號之間斷行
  * 超級鏈接文字一般較長，可獨立成行

- 英文原文中可能通過 `_text_` 或者 `*text*` 的形式用斜體突出部分字句。
  考慮到中文斜體字非常不美觀，在譯文中應改爲 `**譯文**` 形式，
  即用雙引號語法生成加粗字句，實現突出顯示效果。

{{< warning >}}
我們注意到有些貢獻者可能使用了某種自動化工具，在 Markdown 英文原文中自動添加空格。
雖然這些工具可一定程度提高效率，仍然需要提請作者注意，某些工具所作的轉換可能是不對的，
例如將 `foo=bar` 轉換爲 `foo = bar`、將 `），另一些文字` 轉換爲 `） ，另一些文字` 等等，
甚至將超級鏈接中的半角井號（`#`）轉換爲全角，導致鏈接失效。
{{< /warning >}}

## 特殊詞彙   {#special-words}

英文中 "you" 翻譯成 "你"，不必翻譯爲 "您" 以表現尊敬或謙卑。
...
```
列舉所有 pods，查看其創建時間 ...       [No]
列舉所有 Pod，查看其創建時間 ...        [Yes]
```

**第一次**使用首字母縮寫時，應標註其全稱和中文譯文。例如：

```
你可以創建一個 Pod 干擾預算（Pod Disruption Budget，PDB）來解決這一問題。
所謂 PDB 實際上是 ...
```
...
```
鏡像策略（Image Policy）用來控制集羣可拉取的鏡像倉庫（Image Registry）源。
```
...
- authorize，授權或鑑權
  - 將權限授予某主體時，譯爲“授權”
  - 根據某些參數/條件來確定具有哪些權限時，譯爲“鑑權”
- authorizer，鑑權器
- autoscale，自動擴縮容
- bearer token，持有者令牌
- capabilities
  * 當泛指某主體執行某操作的能力時，可直譯爲“能力”
  * 當特指 Linux 操作系統上的[權限控制](http://man7.org/linux/man-pages/man7/capabilities.7.html)機制時，譯爲“權能字”
- certificate authority，證書機構
- certificate，證書
- claim，申領
- cloud provider
  * 當用來指代下層雲服務的提供廠商時，譯爲“雲服務供應商”
  * 當特指 Kubernetes 中對不同雲平臺的支持時，可酌情譯爲“雲驅動”
- cluster，集羣
- condition
  * 大多數上下文中，可譯爲“條件”
  * 在討論 Kubernetes 資源的 condition 時，應譯爲“狀況”
- control loop，控制迴路
