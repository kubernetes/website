...
每種頁面內容類型都有一些使用 Markdown 註釋和 HTML 標題定義的章節。
你可以使用 `heading` 短代碼將內容標題添加到你的頁面中。
註釋和標題有助於維護對應頁面內容類型的結構組織。
...
-->
要在內容頁面中創建通用的標題，可以使用 `heading` 短代碼加上標題字符串。
...
例如，要創建一個 `whatsnext` 標題，添加 `heading` 短代碼並指定 "whatsnext" 字符串：
...
-->
其中的 `overview` 和 `body` 章節在概念頁面中顯示爲註釋。
你可以使用 `heading` 短代碼向頁面添加 `wahtsnext` 節。

在爲每個章節撰寫內容時，遵從一些規定：

- 使用二級和三級標題（H2、H3）來組織內容；
- 在 `overview` 節中，使用一段文字概括當前主題的上下文；
- 在 `body` 節中，詳細解釋對應概念；
- 對於 `whatsnext` 節，提供一個項目符號列表（最多 5 個），幫助讀者進一步學習掌握概念。

[註解](/zh-cn/docs/concepts/overview/working-with-objects/annotations/)頁面是一個已經上線的概念頁面的例子。
...
-->
其中的 `overview`、`steps` 和 `discussion` 節在任務頁面中顯示爲註釋。
你可以使用 `heading` 短代碼添加 `prerequisites` 和 `whatsnext` 小節。

在每個小節內撰寫內容時注意以下規定：

- 最低使用二級標題（H2，標題行前帶兩個 `#` 字符）。每個小節都會由模板自動給出標題。
...
已上線的任務主題示例之一是[使用 HTTP 代理訪問 Kubernetes API](/zh-cn/docs/tasks/extend-kubernetes/http-proxy-access-api/)。
...
-->
教程頁面的 `overview`、`objectives` 和 `lessoncontent` 小節顯示爲註釋形式。
你可以使用 `heading` 短代碼根據需要添加 `prerequisites`、`cleanup` 和
`whatsnext` 小節。

在每個小節中編寫內容時，請注意以下規定：

- 最低使用二級標題（H2，標題前面有兩個 `#` 字符）。模板會自動爲每個小節設置標題。
- 在 `overview` 節中，用一個段落爲整個主題設定語境；
- 在 `prerequisites` 節中，儘可能使用項目符號列表。
...
已發佈的教程主題的一個例子是
[使用 Deployment 運行無狀態應用](/zh-cn/docs/tasks/run-application/run-stateless-application-deployment/).
...
組件工具的參考頁面給出的是某個 Kubernetes 組件工具的描述和參數選項輸出。
每個頁面都是使用組件工具命令基於腳本生成的。
...
| synopsis（用法）|
| options（選項） |
| options from parent commands（從父命令集成的選項） |
| examples（示例）|
| seealso（參考）|
