...
1. 在做出變更前運行一系列的預檢項來驗證系統狀態。一些檢查項目僅僅觸發警告，
   其它的則會被視爲錯誤並且退出 kubeadm，除非問題得到解決或者用戶指定了
...
-->
2. 生成一個自簽名的 CA 證書來爲集羣中的每一個組件建立身份標識。
   用戶可以通過將其放入 `--cert-dir` 配置的證書目錄中（默認爲 `/etc/kubernetes/pki`）
   來提供他們自己的 CA 證書以及/或者密鑰。
   API 服務器證書將爲所有 `--apiserver-cert-extra-sans` 參數值提供附加的 SAN 條目，必要時將其小寫。
...
3. 將 kubeconfig 文件寫入 `/etc/kubernetes/` 目錄以便 kubelet、控制器管理器和調度器連接到
   API 服務器，它們每一個都有自己的身份標識。再編寫額外的 kubeconfig 文件，將 kubeadm
   作爲管理實體（`admin.conf`）和可以繞過 RBAC 的超級管理員用戶（`super-admin.conf`）。
...
-->
4. 爲 API 服務器、控制器管理器和調度器生成靜態 Pod 的清單文件。假使沒有提供一個外部的 etcd
...
7. 爲了使得節點能夠遵照[啓動引導令牌](/zh-cn/docs/reference/access-authn-authz/bootstrap-tokens/)和
   ...
   這兩份文檔中描述的機制加入到集羣中，kubeadm 會執行所有的必要配置：

   - 創建一個 ConfigMap 提供添加集羣節點所需的信息，併爲該 ConfigMap 設置相關的 RBAC 訪問規則。
...
-->
8. 通過 API 服務器安裝一個 DNS 服務器 (CoreDNS) 和 kube-proxy 附加組件。
   在 Kubernetes v1.11 和更高版本中，CoreDNS 是默認的 DNS 服務器。
   請注意，儘管已部署 DNS 服務器，但直到安裝 CNI 時才調度它。
...
-->
通過一份配置文件而不是使用命令行參數來配置 `kubeadm init` 命令是可能的，
但是一些更加高級的功能只能夠通過配置文件設定。
這份配置文件通過 `--config` 選項參數指定的，
...
如果你的配置沒有使用最新版本，**推薦**使用
[kubeadm config migrate](/zh-cn/docs/reference/setup-tools/kubeadm/kubeadm-config/)
命令進行遷移。

關於配置的字段和用法的更多信息，你可以訪問 [API 參考頁面](/zh-cn/docs/reference/config-api/kubeadm-config.v1beta4/)。
...
-->
kubeadm 支持一組獨有的特性門控，只能在 `kubeadm init` 創建集羣期間使用。
這些特性可以控制集羣的行爲。特性門控會在畢業到 GA 後被移除。
...
或者你可以在用 `--config`
傳遞[配置文件](/zh-cn/docs/reference/config-api/kubeadm-config.v1beta4/#kubeadm-k8s-io-v1beta4-ClusterConfiguration)時添加條目到
`featureGates` 字段中。
...
: 啓用此特性門控後，當加入新的控制平面節點時，
  kubeadm 將配置 kubelet 連接到本地 kube-apiserver。
...
: 啓用此特性門控後，kubeadm 將使用 `/var/lib/kubelet/instance-config.yaml` 文件讀寫每個節點的 CRI 套接字，
  ...
  這個新的文件將作爲實例配置補丁被應用，之後纔會應用其他通過 `--patches` 標誌設置的用戶管理的補丁。
  這個新的文件僅包含源自
...
: 啓用此特性門控後，kubeadm 將等待控制平面節點上的所有控制平面組件
  ...
  -->
  如果你在 kubeadm 配置中指定自定義的 `ADDRESS` 或 `PORT`，kubeadm 將使用這些定製的值。
...
-->
`PublicKeysECDSA`
: 可用於創建一個使用 ECDSA 證書而非默認 RSA 算法的集羣。
  支持用 `kubeadm certs renew` 更新現有 ECDSA 證書，
  但你不能在集羣運行期間或升級期間切換 RSA 和 ECDSA 算法。
...
-->
`RootlessControlPlane`
: 設置此標誌來配置 kubeadm 所部署的控制平面組件中的靜態 Pod 容器
  `kube-apiserver`、`kube-controller-manager`、`kube-scheduler` 和 `etcd`
  以非 root 用戶身份運行。如果未設置該標誌，則這些組件以 root 身份運行。
...
-->
`IPv6DualStack`
: 在 IP 雙棧特性處於開發過程中時，此標誌有助於配置組件的雙棧支持。有關 Kubernetes
...
-->
`UnversionedKubeletConfigMap`
: 此標誌控制 kubeadm 存儲 kubelet 配置數據的 {{<glossary_tooltip text="ConfigMap" term_id="configmap" >}} 的名稱。
...
kubeadm 配置中有關 kube-proxy 的說明請查看：
...
-->
### 向控制平面組件傳遞自定義的命令行參數 {#control-plane-flags}

有關向控制平面組件傳遞命令行參數的說明請查看：

- [控制平面命令行參數](/zh-cn/docs/setup/production-environment/tools/kubeadm/control-plane-flags/)
...
要在沒有互聯網連接的情況下運行 kubeadm，你必須提前拉取所需的控制平面鏡像。

你可以使用 `kubeadm config images` 子命令列出並拉取鏡像：
...
-->
你可以通過 `--config` 把 [kubeadm 配置文件](#config-file) 傳遞給上述命令來控制
...
-->
kubeadm 需要的所有默認 `registry.k8s.io` 鏡像都支持多種硬件體系結構。
...
-->
### 使用自定義的鏡像 {#custom-images}

默認情況下，kubeadm 會從 `registry.k8s.io` 倉庫拉取鏡像。如果請求的 Kubernetes 版本是 CI 標籤
...
-->
你可以通過使用[帶有配置文件的 kubeadm](#config-file) 來重寫此操作。
允許的自定義功能有：

* 提供影響鏡像版本的 `kubernetesVersion`。
...
-->
由於向後兼容的原因，使用 `imageRepository` 所指定的定製鏡像庫可能與默認的
`registry.k8s.io` 鏡像路徑不同。例如，某鏡像的子路徑可能是 `registry.k8s.io/subpath/image`，
...
-->
確保將鏡像推送到 kubeadm 可以使用的自定義倉庫的路徑中，你必須：
...
-->
* 使用 `kubeadm config images {list|pull}` 從 `registry.k8s.io` 的默認路徑中拉取鏡像。
* 將鏡像推送到 `kubeadm config images list --config=config.yaml` 的路徑，
...
-->
#### 定製沙箱（pause）鏡像  {#custom-pause-image}

如果需要爲這些組件設置定製的鏡像，
你需要在你的{{< glossary_tooltip text="容器運行時" term_id="container-runtime" >}}中完成一些配置。
...
-->
### 將控制平面證書上傳到集羣  {#uploading-control-plane-certificates-to-the-cluster}

通過將參數 `--upload-certs` 添加到 `kubeadm init`，你可以將控制平面證書臨時上傳到集羣中的 Secret。
...
-->
在使用 `--config`
傳遞[配置文件](/zh-cn/docs/reference/config-api/kubeadm-config.v1beta4/)時，
...
DEB/RPM package.
-->
### 管理 kubeadm 爲 kubelet 提供的 systemd 配置文件 {#kubelet-drop-in}

`kubeadm` 包自帶了關於 `systemd` 如何運行 `kubelet` 的配置文件。
請注意 `kubeadm` 客戶端命令行工具永遠不會修改這份 `systemd` 配置文件。
這份 `systemd` 配置文件屬於 kubeadm DEB/RPM 包。
...
-->
2. 使用這個令牌同時啓動控制平面節點和工作節點。這些節點一旦運行起來應該就會互相尋找對方並且形成集羣。
...
-->
一旦集羣啓動起來，你就可以從控制平面節點的 `/etc/kubernetes/admin.conf` 文件獲取管理憑證，
並使用這個憑證同集羣通信。

一旦集羣啓動起來，你就可以從控制平面節點中的 `/etc/kubernetes/admin.conf`
文件獲取管理憑證或通過[爲其他用戶生成的 kubeconfig 文件](/zh-cn/docs/tasks/administer-cluster/kubeadm/kubeadm-certs#kubeconfig-additional-users)與集羣通信。
...
-->
注意這種搭建集羣的方式在安全保證上會有一些寬鬆，因爲這種方式不允許使用
