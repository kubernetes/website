...
-->
`kubeadm init phase` 能確保調用引導過程的原子步驟。
...
-->
使用以下階段來給作爲控制平面的節點打標籤（label）和記錄污點（taint）。
...
phases.
-->
使用以下階段在 TLS 引導後更新與 kubelet 相關的設置。
你可以使用 `all` 子命令來運行所有 `kubelet-finalize` 階段。
...
-->
你可以使用 `all` 子命令安裝所有可用的插件，或者有選擇性地安裝它們。
