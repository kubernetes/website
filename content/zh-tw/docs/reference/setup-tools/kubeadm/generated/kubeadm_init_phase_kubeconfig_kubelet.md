...
-->
為 kubelet 生成一個 kubeconfig 檔案，*僅僅*用於叢集引導目的
...
生成 kubelet 要使用的 kubeconfig 檔案，並將其儲存到 kubelet.conf 檔案。
...
請注意，該操作目的是*僅*應用於引導叢集。在控制平面啟動之後，應該從 CSR API 請求所有 kubelet 憑據。
