...
打印所選資源的詳細描述，包括事件或控制器等相關資源。
你可以通過名稱選擇單個對象，選擇該類型的所有對象，提供名稱前綴或標籤選擇算符。例如：
...
-->
這條命令將首先檢查與 TYPE 和 NAME_PREFIX 的精確匹配。
如果不存在這樣的資源，它將輸出名稱以 NAME_PREFIX 爲前綴的所有資源的細節。

使用 "kubectl api-resources" 獲取受支持資源的完整列表。
...
# 描述在 "pod.json" 中通過類別和名稱標識的 Pod
kubectl describe -f pod.json
  
# 描述所有 Pod
kubectl describe pods
  
# 描述帶標籤 name=myLabel 的 Pod
kubectl describe pods -l name=myLabel
  
# 描述由 “frontend” 副本控制器管理的所有 Pod
# （副本控制器所創建的 Pod 在 Pod 名稱中帶有此副本控制器的名稱作爲前綴）
kubectl describe pods frontend
```
...
<tr>
<td colspan="2">--show-events&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default:-->預設值：true</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">
<p>
