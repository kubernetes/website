...
-->
設置 last-applied-configuration 註解使之與某檔案內容相匹配。
這會導致 last-applied-configuration 被更新，就像運行了 `kubectl apply -f <file>` 一樣，
但是不會更新對象的任何其他部分。
...
-->
```shell
# 設置資源的 last-applied-configuration，使之與某文件內容相同
kubectl apply set-last-applied -f deploy.yaml
  
# 針對目錄中的每一個配置文件執行 set-last-applied 操作
kubectl apply set-last-applied -f path/
  
# 設置資源的 last-applied-configuration 註解，使之與某文件內容匹配；如果該註解尚不存在，則會被創建。
kubectl apply set-last-applied -f deploy.yaml --create-annotation=true
```
...
-->
如果當前的對象沒有 'last-applied-configuration' 註解，將該註解會被創建。
</p></td>
</tr>
...
-->
包含 last-applied-configuration 註解的檔案的檔案名、目錄或 URL 的列表。
</p></td>
</tr>
...
-->
當指定 `-o=go-template` 、`-o=go-template-file` 時使用的模板字符串或模板檔案路徑。
