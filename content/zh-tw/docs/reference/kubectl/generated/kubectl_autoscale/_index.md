...
-->
創建一個自動擴縮器，自動選擇並設置在 Kubernetes 叢集中運行的 Pod 數量。
此命令將先嚐試使用 `autoscaling/v2` API，如果發生錯誤，它將回退到 `autoscaling/v1` API。

按名稱查找 Deployment、ReplicaSet、StatefulSet 或 ReplicationController，
並創建使用給定資源作爲參照指標的自動擴縮器。
自動擴縮器可以根據需要自動增加或減少系統內部署的 Pod 數量。
...
-->
```shell
# 自動擴縮名爲 "foo" 的 Deployment，其 Pod 數量在 2 到 10 之間，
# 未指定目標 CPU 利用率，因此將使用默認的自動擴縮策略
kubectl autoscale deployment foo --min=2 --max=10
  
# 自動擴縮名爲 "foo" 的 ReplicationController，其 Pod 數量在 1 到 5 之間，
# 目標 CPU 利用率爲 80%
kubectl autoscale rc foo --max=5 --cpu-percent=80

# 自動擴縮名爲 "bar" 的 Deployment，其 Pod 數量在 3 到 6 之間，
# 目標平均 CPU 爲 500m，內存爲 200Mi
kubectl autoscale deployment bar --min=3 --max=6 --cpu=500m --memory=200Mi
  
# 自動擴縮名爲 "bar" 的 Deployment，其 Pod 數量在 2 到 8 之間，
# 目標 CPU 利用率爲 60%，內存利用率爲 70%
kubectl autoscale deployment bar --min=3 --max=6 --cpu=60% --memory=70%
```
...
<tr>
<td colspan="2">--allow-missing-template-keys&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default: true-->預設值：true</td>
</tr>
<tr>
...
- 當以百分比指定時（例如 `"70%"` 表示請求 70% 的 CPU），目標爲平均利用率。
- 當以數值指定時（例如 `"500m"` 表示 500 milliCPU），目標爲平均值。
- 未帶單位的數值會被視爲 milliCPU 單位（例如 `"500"` 等同於 `"500m"`）。
</p></td>
</tr>
...
<tr>
<td colspan="2">--field-manager string&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default: "kubectl-autoscale"-->預設值："kubectl-autoscale"</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">
<p>
...
<tr>
<td colspan="2">--max int32&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default: -1-->預設值：-1</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">
<p>
...
- 當以百分比指定時（例如 `"60%"` 表示請求 60% 內存），目標爲平均利用率。  
- 當以數值指定時（例如 `"200Mi"` 表示 200 MiB，`"1Gi"` 表示 1 GiB），目標爲平均值。  
- 未帶單位的數值會被視爲以 MiB 爲單位（例如 `"200"` 等同於 `"200Mi"`）。  
</p></td>
</tr>

<tr>
<td colspan="2">--min int32&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default: -1-->預設值：-1</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">
<p>
...
-->
自動擴縮器可以設置的 Pod 數量下限。如果未指定或爲負數，伺服器將應用預設值。
</p></td>
</tr>
