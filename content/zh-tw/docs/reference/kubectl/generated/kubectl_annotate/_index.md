...
所有 Kubernetes 對象都支持以註解（Annotation）的形式爲對象儲存額外的資料。
註解是一些鍵/值對，可以比標籤的資料量更大，可以包含諸如結構化 JSON 這類任意字符串值。
各種工具和系統擴展可以使用註解來儲存自己的資料。

嘗試設置已存在的註解的操作將會失敗，除非設置了 --overwrite 選項。
如果 --resource-version 被指定且與伺服器上當前資源版本不匹配，命令將會失敗。

使用 "kubectl api-resources" 獲取可支持的資源完整列表。
...
-->
```shell
# 使用註解 'description' 和值 'my frontend' 更新 Pod 'foo'
# 如果同一註解被設置多次，則只使用最後一個值
kubectl annotate pods foo description='my frontend'

# 更新在 "pod.json" 中指定 type 和 name 的 Pod
kubectl annotate -f pod.json description='my frontend'

# 更新 Pod 'foo'，設置註解 'description' 和值 'my frontend running nginx'，覆蓋其當前值
kubectl annotate --overwrite pods foo description='my frontend running nginx'

# 更新命名空間中的所有 Pod
kubectl annotate pods --all description='my frontend running nginx'

# 僅在資源版本仍爲 1 時更新 Pod 'foo'
kubectl annotate pods foo description='my frontend running nginx' --resource-version=1

# 通過刪除名爲 'description' 的註解（如果存在）來更新 Pod 'foo'
# 不需要 --overwrite 標誌
kubectl annotate pods foo description-
```
...
<tr>
<td colspan="2">--dry-run string[="unchanged"]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default: "none"-->預設值："none"</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;"><p>
<!--
...
<tr>
<td colspan="2">--field-manager string&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default: "kubectl-annotate"-->預設值："kubectl-annotate"</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;"><p>
<!--
...
-->
如果爲 true，則允許註解被覆蓋，否則拒絕覆蓋現有註解的更新。
</p></td>
</tr>
...
-->
如果非空，則只有在所給值是對象的當前資源版本時，註解更新纔會成功。僅在指定單個資源時有效。
</p></td>
</tr>
