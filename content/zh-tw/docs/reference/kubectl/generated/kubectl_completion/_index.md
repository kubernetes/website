...
-->
輸出指定 shell（bash、zsh、fish 或 powershell）的 shell 補全代碼。
必須評估 shell 代碼才能提供 kubectl 命令的交互式補全，這可以通過從 .bash_profile 中獲取它來完成。
...
- 對於 Windows：
 ...
 -->
 zsh 使用者注意事項：[1] zsh 補全僅支持 zsh &gt;= 5.2 版本。
...
-->
```shell
# 使用 homebrew 在 macOS 上安裝 bash-completion
## 如果運行 macOS 附帶的 Bash 3.2
brew install bash-completion
## 或者，如果運行的是 Bash 4.1+
brew install bash-completion@2
## 如果通過 homebrew 安裝了 kubectl，Shell 補全應該能夠立即起作用
## 如果你通過其他方式安裝，則可能需要將新的補全數據添加到補全目錄中
kubectl completion bash > $(brew --prefix)/etc/bash_completion.d/kubectl


# 在 Linux 上安裝 bash 補全
## 如果 Linux 上未安裝 bash-completion，請通過發行版的包管理器安裝 "bash-completion" 包。
## 將 bash 的 kubectl 補全代碼加載到當前 shell 中
source <(kubectl completion bash)
## 將 bash 補全代碼寫入文件並從 .bash_profile 中引用之
kubectl completion bash > ~/.kube/completion.bash.inc
printf "
# kubectl shell completion
source '$HOME/.kube/completion.bash.inc'
...
# 將 zsh[1] 的 kubectl 補全代碼加載到當前 shell 中
source <(kubectl completion zsh)
# 將 zsh[1] 的 kubectl 補全代碼設置爲在啓動時自動加載
kubectl completion zsh > "${fpath[1]}/_kubectl"


# 將 fish[2] 的 kubectl 補全代碼加載到當前 shell 中
kubectl completion fish | source
# 要爲每個會話都加載補全代碼，請執行一次如下命令：
kubectl completion fish > ~/.config/fish/completions/kubectl.fish

# 將 powershell 的 kubectl 補全代碼加載到當前 shell 中
kubectl completion powershell | Out-String | Invoke-Expression
# 設置 powershell 的 kubectl 補全代碼在啓動時運行
## 將補全代碼保存到腳本並在配置文件中執行
kubectl completion powershell > $HOME\.kube\completion.ps1
Add-Content $PROFILE "$HOME\.kube\completion.ps1"
## 執行配置文件中的補全代碼
