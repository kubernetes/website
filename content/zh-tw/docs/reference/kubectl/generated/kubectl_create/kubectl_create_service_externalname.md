...
-->
創建指定名稱的 ExternalName Service。

ExternalName Service 引用外部 DNS 地址，而不僅僅是 Pod，
這類 Service 允許應用作者引用平臺外、其他叢集或本地存在的服務。
...
-->
```shell
# 新建一個名爲 my-ns 的 ExternalName Service
kubectl create service externalname my-ns --external-name bar.com
```
