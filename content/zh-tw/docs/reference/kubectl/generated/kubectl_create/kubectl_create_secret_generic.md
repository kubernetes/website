...
-->
當基於文件創建 Secret 時，鍵將默認爲文件的基本名稱，值將默認爲文件內容。
如果基本名稱是無效的鍵，或者你希望選擇自己的鍵，你可以指定一個替代鍵。

當基於目錄創建 Secret 時，目錄中每個基本名稱爲有效鍵的文件都將被打包到 Secret 中。
除常規文件外的所有目錄條目（例如子目錄、符號鏈接、設備、管道等）都將被忽略。
...
-->
指定文件路徑以讀取 key=val 對的行來創建一個 Secret。
</p></td>
</tr>
...
-->
鍵可以通過其文件路徑被指定，這種情況將爲它們分配一個默認名稱；
鍵也可以選擇通過某個名稱和文件路徑被指定，這種情況將使用給定的名稱。
指定一個目錄將遍歷目錄中所有已命名的且是有效 Secret 鍵的文件。
</p></td>
</tr>
