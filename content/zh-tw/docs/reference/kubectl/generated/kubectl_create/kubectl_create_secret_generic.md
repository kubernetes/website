...
-->
基於檔案、目錄或指定的文字值創建 Secret。

單個 Secret 可以包含一個或多個鍵值對。
...
-->
當基於檔案創建 Secret 時，鍵將預設爲檔案的基本名稱，值將預設爲檔案內容。
如果基本名稱是無效的鍵，或者你希望選擇自己的鍵，你可以指定一個替代鍵。

當基於目錄創建 Secret 時，目錄中每個基本名稱爲有效鍵的檔案都將被打包到 Secret 中。
除常規檔案外的所有目錄條目（例如子目錄、符號鏈接、設備、管道等）都將被忽略。
...
-->
```shell
# 新建一個名爲 my-secret 的 Secret，其鍵爲文件夾 bar 中的每個文件
kubectl create secret generic my-secret --from-file=path/to/bar
  
# 使用指定的鍵而不是磁盤上的文件名來新建一個名爲 my-secret 的 Secret
kubectl create secret generic my-secret --from-file=ssh-privatekey=path/to/id_rsa --from-file=ssh-publickey=path/to/id_rsa.pub
  
# 使用 key1=supersecret 和 key2=topsecret 新建一個名爲 my-secret 的 Secret
kubectl create secret generic my-secret --from-literal=key1=supersecret --from-literal=key2=topsecret
  
# 組合使用文件和文字值新建一個名爲 my-secret 的 Secret
kubectl create secret generic my-secret --from-file=ssh-privatekey=path/to/id_rsa --from-literal=passphrase=topsecret
  
# 使用 env 文件新建一個名爲 my-secret 的 Secret
kubectl create secret generic my-secret --from-env-file=path/to/foo.env --from-env-file=path/to/bar.env
```
...
-->
指定檔案路徑以讀取 key=val 對的行來創建一個 Secret。
</p></td>
</tr>
...
-->
鍵可以通過其檔案路徑被指定，這種情況將爲它們分配一個預設名稱；
鍵也可以選擇通過某個名稱和檔案路徑被指定，這種情況將使用給定的名稱。
指定一個目錄將遍歷目錄中所有已命名的且是有效 Secret 鍵的檔案。
</p></td>
</tr>
...
-->
指定鍵和文字值以插入到 Secret 中（例如 mykey=somevalue）。
</p></td>
</tr>
