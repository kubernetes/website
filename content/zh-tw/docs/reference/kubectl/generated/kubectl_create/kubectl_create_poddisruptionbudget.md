...
```
-->
創建具有指定名稱、選擇算符和預期最少可用 Pod 個數的 Pod 干擾預算。
...
-->
```shell
# 創建一個名爲 my-pdb 的 Pod 干擾預算，它將選擇所有帶有 app=rails 標籤的 Pod
# 並要求至少有一個 Pod 在任何時候都是可用的
kubectl create poddisruptionbudget my-pdb --selector=app=rails --min-available=1
  
# 創建一個名爲 my-pdb 的 Pod 干擾預算，它將選擇所有帶有 app=nginx 標籤的 Pod
# 並要求在任何時候所選 Pod 中至少有一半是可用的
kubectl create pdb my-pdb --selector=app=nginx --min-available=50%
```
