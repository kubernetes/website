...
-->
```shell
# 使用 cluster-admin 集羣角色爲 user1、user2 和 group1 創建一個集羣角色綁定
kubectl create clusterrolebinding cluster-admin --clusterrole=cluster-admin --user=user1 --user=user2 --group=group1
```
...
-->
特定 ClusterRoleBinding 應引用的 ClusterRole。
</p></td>
</tr>
...
-->
要綁定到 ClusterRole 的組。此標誌可以被重複使用以添加多個組。
</p></td>
</tr>
...
-->
要綁定到 ClusterRole 的服務賬戶，格式爲 <namespace>:<name>。此標誌可以被重複使用以添加多個服務賬戶。
</p></td>
</tr>
...
-->
要綁定到 ClusterRole 的使用者名。此標誌可以被重複使用以添加多個使用者。
</p></td>
</tr>
