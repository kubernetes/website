...
-->
基於檔案、目錄或指定的文字值創建 ConfigMap。

一個 ConfigMap 可以包含一個或多個鍵/值對。

當你基於檔案創建 ConfigMap 時，鍵預設爲檔案的基本名稱，值預設爲檔案內容。
如果基本名稱是無效的鍵，你可以指定一個替代鍵。

當基於目錄創建 ConfigMap 時，目錄中每個基本名稱是有效鍵的檔案都會被打包到 ConfigMap 中。
除常規檔案之外的所有目錄條目都會被忽略（例如子目錄、符號鏈接、設備、管道等）。
...
-->
```shell
  # 基於 bar 文件夾新建一個名爲 my-config 的 ConfigMap
  kubectl create configmap my-config --from-file=path/to/bar
  
  # 新建一個名爲 my-config 的 ConfigMap，使用指定的鍵而不是磁盤上的文件基本名稱
  kubectl create configmap my-config --from-file=key1=/path/to/bar/file1.txt --from-file=key2=/path/to/bar/file2.txt
  
  # 新建一個名爲 my-config 的 ConfigMap，包含 key1=config1 和 key2=config2
  kubectl create configmap my-config --from-literal=key1=config1 --from-literal=key2=config2
  
  # 從文件中的 key=value 對新建一個名爲 my-config 的 ConfigMap
  kubectl create configmap my-config --from-file=path/to/bar
  
  # 從 env 文件新建一個名爲 my-config 的 ConfigMap
  kubectl create configmap my-config --from-env-file=path/to/foo.env --from-env-file=path/to/bar.env
```
...
-->
指定檔案的路徑以讀取 key=val 對的那些行來創建 ConfigMap。
</p></td>
</tr>
...
-->
鍵檔案可以使用其檔案路徑來指定，在這種情況下，檔案的基本名稱將用作 ConfigMap 的鍵。
另外，鍵檔案也可以選擇使用鍵和檔案路徑來指定，在這種情況下，將使用指定的鍵。
指定一個目錄將遍歷此目錄中所有被命名的檔案（其基本名稱爲有效的 ConfigMap 鍵）。
</p></td>
</tr>
...
-->
指定鍵和文字值以插入到 ConfigMap 中（例如 mykey=somevalue）。
</p></td>
</tr>
