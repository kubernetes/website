...
-->
基於文件、目錄或指定的文字值創建 ConfigMap。

一個 ConfigMap 可以包含一個或多個鍵/值對。

當你基於文件創建 ConfigMap 時，鍵默認爲文件的基本名稱，值默認爲文件內容。
如果基本名稱是無效的鍵，你可以指定一個替代鍵。

當基於目錄創建 ConfigMap 時，目錄中每個基本名稱是有效鍵的文件都會被打包到 ConfigMap 中。
除常規文件之外的所有目錄條目都會被忽略（例如子目錄、符號鏈接、設備、管道等）。
...
-->
指定文件的路徑以讀取 key=val 對的那些行來創建 ConfigMap。
</p></td>
</tr>
...
-->
鍵文件可以使用其文件路徑來指定，在這種情況下，文件的基本名稱將用作 ConfigMap 的鍵。
另外，鍵文件也可以選擇使用鍵和文件路徑來指定，在這種情況下，將使用指定的鍵。
指定一個目錄將遍歷此目錄中所有被命名的文件（其基本名稱爲有效的 ConfigMap 鍵）。
</p></td>
</tr>
