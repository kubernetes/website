...
```shell
kubectl create rolebinding NAME --clusterrole=NAME|--role=NAME [--user=用戶名] [--group=組名] [--serviceaccount=命名空間:服務賬戶名] [--dry-run=server|client|none]
```
...
-->
```shell
# 使用 admin 集羣角色爲 user1、user2 和 group1 創建角色綁定
kubectl create rolebinding admin --clusterrole=admin --user=user1 --user=user2 --group=group1
  
# 使用 admin 角色爲服務賬戶 monitoring:sa-dev 創建角色綁定
kubectl create rolebinding admin-binding --role=admin --serviceaccount=monitoring:sa-dev
```
...
-->
指定 RoleBinding 應引用的 ClusterRole。
</p></td>
</tr>
...
-->
要綁定到角色的服務賬戶，格式爲 `<namespace>:<name>`。此標誌可以重複使用以添加多個服務賬戶。
</p></td>
</tr>
