...
當使用 Docker 命令行推送鏡像時，你可以通過運行以下命令向給定的倉庫進行身份認證：
...
在創建應用時，你可能有一個 Docker 倉庫要求進行身份認證。爲了讓節點代表你拉取鏡像，這些節點必須有憑據。
