...
支持 JSON 和 YAML 格式。如果要替換某個現有資源，則必須提供完整的資源規約。
你可以通過以下方式獲取資源規約：
...
-->
```shell
# 使用 pod.json 中的數據替換 Pod
kubectl replace -f ./pod.json
  
# 基於傳遞到標準輸入中的 JSON 替換 Pod
cat pod.json | kubectl replace -f -
  
# 將單容器 Pod 的鏡像版本（標籤）更新爲 v4
kubectl get pod mypod -o yaml | sed 's/\(image: myimage\):.*$/\1:v4/' | kubectl replace -f -
...
<tr>
<td colspan="2">--field-manager string&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default:-->默認值："kubectl-replace"</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;"><p>
<!--
...
-->
標誌值爲以 PUT 方式發送到伺服器上的原始 URI。使用 kubeconfig 文件中指定的傳輸方式。
</p></td>
</tr>
