...
-->
使用策略合併補丁、JSON 合併補丁或 JSON 補丁來更新某資源的字段。
...
-->
```shell
# 使用策略合併補丁部分更新節點，指定補丁爲 JSON 格式
kubectl patch node k8s-node-1 -p '{"spec":{"unschedulable":true}}'
  
# 使用策略合併補丁部分更新節點，指定補丁爲 YAML 格式
kubectl patch node k8s-node-1 -p $'spec:\n unschedulable: true'
  
# 使用策略合併補丁部分更新以在 "node.json" 中所指定類別和名稱標識的節點
kubectl patch -f node.json -p '{"spec":{"unschedulable":true}}'
  
# 更新容器的鏡像；spec.containers[*].name 是必需的，因爲它是合併鍵
kubectl patch pod valid-pod -p '{"spec":{"containers":[{"name":"kubernetes-serve-hostname","image":"new image"}]}}'

# 使用帶有位置數組的 JSON 補丁更新容器的鏡像
kubectl patch pod valid-pod --type='json' -p='[{"op": "replace", "path": "/spec/containers/0/image", "value":"new image"}]'
  
# 使用合併補丁通過 “scale” 子資源更新 Deployment 的副本
...
<tr>
<td colspan="2">--field-manager string&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default:-->默認值："kubectl-patch"</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;"><p>
<!--
...
-->
如果爲真，patch 操作將作用於文件內容，而不是服務器端資源。
</p></td>
</tr>
...
<tr>
<td colspan="2">--type string&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default:-->默認值："strategic"</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">
<p>
...
-->
提供的補丁類型；可以是 json、merge、strategic 之一。
</p></td>
</tr>
