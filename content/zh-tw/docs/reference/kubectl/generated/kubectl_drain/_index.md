...
如果 API 服務器支持 [Eviction](/zh-cn/docs/concepts/workloads/pods/disruptions/)，
則 "drain" 會驅逐 Pod。否則，它將使用常規的 DELETE 來刪除 Pod。
"drain" 會驅逐或刪除除鏡像 Pod（無法通過 API 服務器刪除）之外的所有 Pod。
...
如果有 Pod 既不是鏡像 Pod，也不是由 ReplicationController、ReplicaSet、DaemonSet、StatefulSet 或 Job 管理，
