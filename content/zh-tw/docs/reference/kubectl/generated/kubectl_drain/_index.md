...
給定節點將被標記爲不可調度，以防止新的 Pod 調度到此。
如果 API 伺服器支持 [Eviction](/zh-cn/docs/concepts/workloads/pods/disruptions/)，
則 "drain" 會驅逐 Pod。否則，它將使用常規的 DELETE 來刪除 Pod。
"drain" 會驅逐或刪除除映像檔 Pod（無法通過 API 伺服器刪除）之外的所有 Pod。
如果存在 DaemonSet 管理的 Pod，若沒有 `--ignore-daemonsets`，"drain" 將不會繼續執行，
無論如何，"drain" 操作都不會刪除 Daemonset 所管理的任何 Pod，因爲這些 Pod 會立即被 DaemonSet 控制器替換，
如果有 Pod 既不是映像檔 Pod，也不是由 ReplicationController、ReplicaSet、DaemonSet、StatefulSet 或 Job 管理，
則 "drain" 不會刪除此 Pod，除非你使用 --force。如果一個或多個 Pod 的管理資源丟失，--force 還將允許繼續刪除。

"drain" 等待體面終止。在命令完成之前，不應在機器上進行操作。

當你準備好將節點重新投入使用時，請使用 `kubectl uncordon`，這將使節點再次可調度。
...
-->
```shell
# 騰空節點 "foo"，即使上面有不受 ReplicationController、ReplicaSet、Job、DaemonSet
# 或 StatefulSet 管理的 Pod
kubectl drain foo --force

# 與上條命令類似，但如果存在不受 ReplicationController、ReplicaSet、Job、DaemonSet
# 或 StatefulSet 管理的 Pod，則中止，並使用 15 分鐘的寬限期
kubectl drain foo --grace-period=900
```
...
-->
即使存在使用 emptyDir（騰空節點時將被刪除的本地數據）的 Pod，也要繼續。
</p>
</td>
</tr>
...
-->
強制使用刪除操作來進行節點騰空，即使系統支持驅逐操作。
這種設置將繞過檢查 PodDisruptionBudget 約束，請謹慎使用。
</p>
</td>
</tr>
...
-->
即使存在未聲明控制器的 Pod，也要繼續。
</p>
</td>
</tr>

<tr>
<td colspan="2">--grace-period int&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default: -1-->默認值：-1</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">
<p>
...
-->
給予每個 Pod 體面終止的時間段長度（以秒爲單位）。如果爲負，則將使用 Pod 中指定的默認值。
</p>
</td>
</tr>
...
-->
如果 Pod 的 DeletionTimestamp 比當前時間早 N 秒，那麼跳過等待該 Pod 的過程。
秒數必須大於 0 才能跳過等待。
</p>
</td>
</tr>
