...
打印一張包含與事件相關的最重要資訊的表格。
你可以請求某個命名空間的事件、所有命名空間的事件，或者僅過濾出與指定資源相關的事件。
...
-->
```shell
# 列舉 default 命名空間中近期的事件
kubectl events
  
# 列舉所有命名空間中近期的事件
kubectl events --all-namespaces
  
# 列舉指定 Pod 的近期事件，然後等待更多事件，並在出現新事件時列舉出來
kubectl events --for pod/web-pod-13je7 --watch
  
# 以 YAML 格式列舉近期的事件
kubectl events -oyaml
  
# 僅列舉類別爲 “Warning” 或 “Normal” 的近期事件
kubectl events --types=Warning,Normal
```
...
-->
* [kubectl](../kubectl/) - kubectl 控制 Kubernetes 叢集管理
