...
-->
爲某個資源設置選擇算符。請注意，
如果資源在 `set selector` 調用之前已有選擇算符，則新的選擇算符將覆蓋舊的選擇算符。

選擇算符必須以字母或數字開頭，可以包含字母、數字、連字符、點和下劃線，最長爲 63 個字符。
如果指定了 --resource-version，則更新將使用此資源版本，否則將使用現有的資源版本。
注意：目前只能在 Service 對象上設置選擇算符。
...
-->
```shell
# 在創建 Deployment/Service 對之前設置標籤和選擇算符
kubectl create service clusterip my-svc --clusterip="None" -o yaml --dry-run=client | kubectl set selector --local -f - 'environment=qa' -o yaml | kubectl create -f -
kubectl create deployment my-dep -o yaml --dry-run=client | kubectl label --local -f - environment=qa -o yaml | kubectl create -f -
```
...
<tr>
<td colspan="2">-R, --recursive&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default:-->默認值：true</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;"><p>
<!--
...
-->
如果非空，則只有在所給值是對象的當前資源版本時，選擇算符更新纔會成功。僅在指定單個資源時有效。
</p>
</td>
</tr>
