...
-->
爲定義 Pod 模板的任一資源指定計算資源要求（CPU、內存）。
如果 Pod 被成功調度，將保證獲得所請求的資源量，但可以在某一瞬間達到其指定的限制值。

對於每類計算資源，如果只指定限制值而省略請求值，則請求值將被預設設置爲限制值。

可能的資源包括（不區分大小寫）：使用 "kubectl api-resources" 查看受支持資源的完整列表。
...
-->
```shell
# 將 Deployment nginx 中容器 nginx 的 CPU 限制設置爲 "200m"， 將內存限制設置爲 "512Mi"
kubectl set resources deployment nginx -c=nginx --limits=cpu=200m,memory=512Mi
  
# 爲 nginx 中的所有容器設置資源請求和限制
kubectl set resources deployment nginx --limits=cpu=200m,memory=512Mi --requests=cpu=100m,memory=256Mi
  
# 移除 nginx 中容器對資源的資源請求
kubectl set resources deployment nginx --limits=cpu=0,memory=0 --requests=cpu=0,memory=0
  
# 打印基於本地清單更新 nginx 容器限制的結果（以 YAML 格式），不向伺服器發送請求
kubectl set resources -f path/to/file.yaml --limits=cpu=200m,memory=512Mi --local -o yaml
```
...
<tr>
<td colspan="2">-c, --containers string&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default:-->預設值："*"</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">
<p>
...
-->
在所選 Pod 模板中要更改的容器名稱，預設會選擇所有容器 - 可以使用通配符。
</p></td>
</tr>
...
<tr>
<td colspan="2">--field-manager string&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--Default:-->預設值："kubectl-set"</td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">
<p>
...
-->
檔案名、目錄或檔案 URL 的列表，用於標識要從伺服器獲取的資源。
</p></td>
</tr>
...
-->
指定容器的資源請求。例如，“cpu=100m,memory=256Mi”。
請注意，伺服器端組件可能會根據伺服器設定（例如 LimitRange）分配請求。
</p></td>
</tr>
...
-->
如果爲真，`set resources` 將不會與 API 伺服器通信，而是在本地運行。
</p></td>
</tr>
...
-->
如果爲真，則不檢查伺服器證書的有效性。這將使你的 HTTPS 連接不安全。
</p></td>
</tr>
