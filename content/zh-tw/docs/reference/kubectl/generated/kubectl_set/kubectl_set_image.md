...
-->
```shell
# 將 Deployment 的 nginx 容器鏡像設置爲 “nginx:1.9.1”，並將其 busybox 容器鏡像設置爲 “busybox”
kubectl set image deployment/nginx busybox=busybox nginx=nginx:1.9.1
  
# 更新所有 Deployment 和副本控制器的 nginx 容器鏡像爲 “nginx:1.9.1”
kubectl set image deployments,rc nginx=nginx:1.9.1 --all
  
# 更新 DaemonSet abc 的所有容器鏡像爲 "nginx:1.9.1"
kubectl set image daemonset abc *=nginx:1.9.1
  
# 使用本地文件更新 nginx 容器鏡像，並以 YAML 格式打印結果，但不向伺服器發出請求
kubectl set image -f path/to/file.yaml nginx=nginx:1.9.1 --local -o yaml
```
...
-->
如果爲 true，`set image` 將不會與 API 伺服器通信，而是在本地運行。
</p></td>
</tr>
