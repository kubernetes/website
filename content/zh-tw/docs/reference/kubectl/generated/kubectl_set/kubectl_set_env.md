...
添加、更新或移除（在副本控制器或 Deployment 配置中的）一個或多個 Pod 模板中的容器環境變量定義。
...
-->
如果在命令行上設置了 "--env -"，則可以使用標準的 env 語法從標準輸入中讀取環境變量。
...
# 以 YAML 格式輸出修改後的 Deployment，但不更改服務器上的對象
kubectl set env deployment/sample-build STORAGE_DIR=/data -o yaml
...
# 從磁盤上的 Deployment 定義中移除環境變量 ENV 並更新服務器上的 Deployment 配置
kubectl set env -f deploy.json ENV-
  
# 將某些本地 Shell 環境變量設置到服務器上的 Deployment 配置中
env | grep RAILS_ | kubectl set env -e - deployment/registry
```
...
-->
如果爲真，`set env` 將不會與 API 服務器通信，而是在本地運行。
</p></td>
</tr>
