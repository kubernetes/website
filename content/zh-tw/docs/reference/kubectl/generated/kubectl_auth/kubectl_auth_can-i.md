...
- VERB 指的是邏輯上的 Kubernetes API 動詞，如 `get`、`list`、`watch`、`delete` 等。
- TYPE 指的是 Kubernetes 中的一種資源類型，快捷表示和資源組都可被解析。
- NONRESOURCEURL 是以 `/` 開頭的部分 URL。
- NAME 是特定 Kubernetes 資源的名稱，此命令可與身份僞裝功能完美搭配，請參閱 --as 全局標誌。
...
-->
```shell
# 檢查是否可以在任意命名空間中創建 Pod
kubectl auth can-i create pods --all-namespaces
  
# 檢查是否可以列出當前命名空間中的 Deployment
kubectl auth can-i list deployments.apps
  
# 檢查命名空間 "dev" 的服務帳戶 "foo" 是否可以列出命名空間 "prod" 下的 Pod。
# 你必須有權限使用全局選項 "--as" 所涉及的身份僞裝功能。
kubectl auth can-i list pods --as=system:serviceaccount:dev:foo -n prod
  
# 檢查我是否可以在當前命名空間中執行所有操作（"*" 表示全部）
kubectl auth can-i '*' '*'

# 檢查是否可以在命名空間 "foo" 中獲取名爲 "bar" 的 Job
kubectl auth can-i list jobs.batch/bar -n foo
