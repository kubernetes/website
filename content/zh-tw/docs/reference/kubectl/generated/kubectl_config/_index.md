...
-->
使用 "kubectl config set current-context my-context" 等子命令修改 kubeconfig 檔案。

加載順序遵循以下規則：

1. 如果設置了 --kubeconfig 標誌，則僅加載該檔案。該標誌只能設置一次，並且不會發生合併。
2. 如果設置了 $KUBECONFIG 環境變量，則將其用作路徑列表（系統的正常路徑分隔規則），這些路徑會被合併。
   當某個值被修改時，也會在定義這部分內容的檔案中修改此值。當某個值被創建時，也會在存在的第一個檔案中創建此值。
   如果鏈中不存在檔案，則它會創建列表中的最後一個檔案。
3. 否則，將使用 `${HOME}/.kube/config`，並且不會發生合併。
...
* [kubectl config current-context](kubectl_config_current-context/) - 顯示當前上下文
* [kubectl config delete-cluster](kubectl_config_delete-cluster/) - 從 kubeconfig 中刪除指定叢集資訊
* [kubectl config delete-context](kubectl_config_delete-context/) - 從 kubeconfig 中刪除指定的上下文資訊
* [kubectl config delete-user](kubectl_config_delete-user/) - 從 kubeconfig 中刪除指定的使用者資訊
* [kubectl config get-clusters](kubectl_config_get-clusters/) - 顯示 kubeconfig 中指定叢集的資訊
* [kubectl config get-contexts](kubectl_config_get-contexts/) - 顯示 kubeconfig 中指定上下文的資訊
* [kubectl config get-users](kubectl_config_get-users/) - 顯示 kubeconfig 中指定使用者的資訊
* [kubectl config rename-context](kubectl_config_rename-context/) - 對 kubeconfig 檔案的上下文重命名
* [kubectl config set](kubectl_config_set/) - 在 kubeconfig 檔案中設置單個值
* [kubectl config set-cluster](kubectl_config_set-cluster/) - 在 kubeconfig 中設置叢集條目
* [kubectl config set-context](kubectl_config_set-context/) - 在 kubeconfig 中設置上下文條目
* [kubectl config set-credentials](kubectl_config_set-credentials/) - 在 kubeconfig 中設置使用者條目
* [kubectl config unset](kubectl_config_unset/) - 取消設置 kubeconfig 檔案中的單個值
* [kubectl config use-context](kubectl_config_use-context/) - 設置在 kubeconfig 檔案中的當前上下文
* [kubectl config view](kubectl_config_view/) - 顯示合併的 kubeconfig 設置或指定的 kubeconfig 檔案
