...
-->
設置 kubeconfig 文件中的單個值。

- PROPERTY_NAME 是一個以點分隔的名稱，其中每個元素代表一個屬性名稱或一個鍵名。鍵名不得包含點。
- PROPERTY_VALUE 是要設置的值。除非使用 `--set-raw-bytes` 標誌，否則二進制字段（例如 `certificate-authority-data` ）必須爲經過 base64 編碼的字符串。
...
-->
```shell
# 將 my-cluster 集羣的 server 字段設置爲 https://1.2.3.4
kubectl config set clusters.my-cluster.server https://1.2.3.4
  
# 設置 my-cluster 集羣的 certificate-authority-data 字段
kubectl config set clusters.my-cluster.certificate-authority-data $(echo "cert_data_here" | base64 -i -)
  
# 將 my-context 上下文中的 cluster 字段設置爲 my-cluster
kubectl config set contexts.my-context.cluster my-cluster
  
# 使用 --set-raw-bytes 選項設置 cluster-admin 用戶中的 client-key-data 字段
...
-->
寫入 []byte 格式的 PROPERTY_VALUE 時，直接寫入給定的字符串，無需 base64 解碼。
</p></td>
</tr>
