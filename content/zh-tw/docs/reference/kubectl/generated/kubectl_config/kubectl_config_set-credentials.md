...
-->
在 kubeconfig 中設置使用者條目。

- 指定已存在的屬性名稱將把新字段值與現有值合併。
  - 客戶端證書標誌：--client-certificate=certfile --client-key=keyfile
  - 持有者令牌標誌：--token=bearer_token
  - 基本身份驗證標誌：--username=basic_user --password=basic_password
- 持有者令牌和基本身份驗證是互斥的（不可同時使用）。
...
-->
```shell
# 僅設置 "cluster-admin" 條目上的 "client-key" 字段，不觸及其他值
kubectl config set-credentials cluster-admin --client-key=~/.kube/admin.key
  
# 爲 "cluster-admin" 條目設置基本身份驗證
kubectl config set-credentials cluster-admin --username=admin --password=uXFGweU9l35qcif
  
# 在 "cluster-admin" 條目中嵌入客戶端證書數據
kubectl config set-credentials cluster-admin --client-certificate=~/.kube/admin.crt --embed-certs=true
  
# 爲 "cluster-admin" 條目啓用 Google Compute Platform 身份認證提供程序
kubectl config set-credentials cluster-admin --auth-provider=gcp
  
# 使用附加參數爲 "cluster-admin" 條目啓用 OpenID Connect 身份認證提供程序
kubectl config set-credentials cluster-admin --auth-provider=oidc --auth-provider-arg=client-id=foo --auth-provider-arg=client-secret=bar
  
# 刪除 "cluster-admin" 條目的 OpenID Connect 身份驗證提供程序的 "client-secret" 配置值
kubectl config set-credentials cluster-admin --auth-provider=oidc --auth-provider-arg=client-secret-
  
# 爲 "cluster-admin" 條目啓用新的 exec 認證插件
kubectl config set-credentials cluster-admin --exec-command=/path/to/the/executable --exec-api-version=client.authentication.k8s.io/v1beta1
  
# 爲 "cluster-admin" 條目啓用新的、帶交互模式的 exec 認證插件
kubectl config set-credentials cluster-admin --exec-command=/path/to/the/executable --exec-api-version=client.authentication.k8s.io/v1beta1 --exec-interactive-mode=Never
  
# 爲 "cluster-admin" 條目定義新的 exec 認證插件參數
kubectl config set-credentials cluster-admin --exec-arg=arg1 --exec-arg=arg2
  
# 爲 "cluster-admin" 條目創建或更新 exec 認證插件環境變量
kubectl config set-credentials cluster-admin --exec-env=key1=val1 --exec-env=key2=val2
  
# 刪除 "cluster-admin" 條目的 exec 認證插件環境變量
kubectl config set-credentials cluster-admin --exec-env=var-to-remove-
```
...
-->
在 kubeconfig 中嵌入使用者條目的客戶端證書/密鑰。
</p></td>
</tr>
...
-->
kubeconfig 中使用者條目的 exec 憑據插件的 API 版本。
</p></td>
</tr>
...
-->
kubeconfig 中使用者條目的 exec 憑據插件命令的新參數。
</p></td>
</tr>
...
-->
kubeconfig 中使用者條目的 exec 憑據插件命令。
</p></td>
</tr>
...
-->
kubeconfig 中使用者條目的 exec 憑據插件的交互模式。
</p></td>
</tr>
...
-->
提供給 kubeconfig 中使用者條目所使用的 exec 憑據插件的叢集資訊。
</p></td>
</tr>
