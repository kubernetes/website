...
- edit 命令允許你直接編輯可通過命令列工具檢索的任何 API 資源。
  ...
  如果未定義，將使用預設的 shell，在 Linux 中爲 "/bin/bash"，在 Windows 中爲 "cmd"。

- 你可以編輯多個對象，但一次只會應用一個更改。該命令接受檔案名和命令列參數，儘管你指向的檔案必須是以前保存的資源版本。

- edit 是通過用於獲取資源的 API 版本完成的。要編輯特定 API 版本的資源，請完全限定資源、版本和組。

- 預設格式爲 YAML。要以 JSON 格式進行編輯，請指定 "-o json"。

- `--windows-line-endings` 標誌可用於強制 Windows 行結束，否則將使用操作系統的預設值。

- 如果更新時發生錯誤，將在磁盤上創建一個臨時檔案，其中包含未應用的更改。
