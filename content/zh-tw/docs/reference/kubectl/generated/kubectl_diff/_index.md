...
-->
對比當前在線配置與通過文件名或標準輸入所指定的配置之間的差異，並顯示如果應用配置後將會如何變化。

輸出始終是 YAML。

KUBECTL_EXTERNAL_DIFF 環境變量可用於選擇你自己的 diff 命令。
用戶也可以使用帶參數的外部命令，例如：
...
注意：如果使用 KUBECTL_EXTERNAL_DIFF，則需要遵循該約定。
...
-->
在與當前版本進行差異比較時並行處理的對象的數量。
數量越多，速度越快，但在短時間內會消耗更多的內存、I/O 和 CPU 等資源。
</p></td>
</tr>
...
-->
包含 diff 操作所用配置的文件名、目錄或指向文件的 URL。
</p></td>
</tr>
...
-->
以遞歸方式處理以 -f 或 --filename 指定的目錄。當你希望管理在同一目錄中組織的相關清單時，這很有用。
</p></td>
</tr>
...
-->
如果爲 true，應用操作將在服務器上運行，而不是在客戶端運行。
</p></td>
</tr>
