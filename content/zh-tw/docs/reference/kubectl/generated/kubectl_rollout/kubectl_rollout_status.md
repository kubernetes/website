...
- 默認情況下，"rollout status" 將監視最新的上線狀態，直到它完成。
  如果你不想等待 rollout 完成，則可以使用 --watch=false。請注意，如果中間開始了新的上線動作，
  則 "rollout status" 將繼續監視最新修訂版本。如果你想 watch 特定的修訂版本並在它被另一個修訂覆蓋時中止，
  請使用 --revision=N，其中 N 是你需要監視的修訂版本。
...
-->
結束監視之前等待的時間長度，零表示永不結束。任何其他值都應包含相應的時間單位（例如 1s、2m、3h）。
</p></td>
</tr>
