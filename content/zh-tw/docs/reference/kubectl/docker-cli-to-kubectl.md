...
-->
你可以使用 Kubernetes 命令列工具 `kubectl` 與 API 伺服器進行互動。如果你熟悉 Docker 命令列工具，
...
-->
要執行 nginx 部署並將其暴露，請參見 [kubectl create deployment](/docs/reference/generated/kubectl/kubectl-commands#-em-deployment-em-)
...
-->
`kubectl` 命令列印建立或突變資源的型別和名稱，然後可以在後續命令中使用。部署後，你可以公開新服務。
{{< /note >}}
...
-->
在 kubectl 命令中，我們建立了一個 [Deployment](/zh-cn/docs/concepts/workloads/controllers/deployment/)，
這將保證有 N 個執行 nginx 的 Pod（N 代表 spec 中宣告的 replica 數，預設為 1）。
我們還建立了一個 [service](/zh-cn/docs/concepts/services-networking/service/)，其選擇器與容器標籤匹配。
檢視[使用服務訪問叢集中的應用程式](/zh-cn/docs/tasks/access-application-cluster/service-access-application-cluster) 獲取更多資訊。
...
-->
預設情況下映象會在後臺執行，與 `docker run -d ...` 類似，如果你想在前臺執行，
使用 [`kubectl run`](/docs/reference/generated/kubectl/kubectl-commands/#run) 在前臺執行 Pod:
...
-->
與 `docker run ...` 不同的是，如果指定了 `--attach`，我們將連線到 `stdin`，`stdout` 和 `stderr`，
而不能控制具體連線到哪個輸出流（`docker -a ...`）。要從容器中退出，可以輸入 Ctrl + P，然後按 Ctrl + Q。
...
-->
因為我們使用 Deployment 啟動了容器，如果你終止連線到的程序（例如 `ctrl-c`），容器將會重啟，
這跟 `docker run -it` 不同。如果想銷燬該 Deployment（和它的 Pod），
你需要執行 `kubectl delete deployment <name>`。
...
-->
如何列出哪些正在執行？檢視 [kubectl get](/docs/reference/generated/kubectl/kubectl-commands/#get)。
...
-->
如何連線到已經執行在容器中的程序？
檢視 [kubectl attach](/docs/reference/generated/kubectl/kubectl-commands/#attach)。
...
-->
如何在容器中執行命令？檢視 [kubectl exec](/docs/reference/generated/kubectl/kubectl-commands/#exec)。
...
-->
更多資訊請檢視[獲取執行中容器的 Shell 環境](/zh-cn/docs/tasks/debug/debug-application/get-shell-running-container/)。
...
-->
如何檢視執行中程序的 stdout/stderr？檢視 [kubectl logs](/docs/reference/generated/kubectl/kubectl-commands/#logs)。
...
-->
現在是時候提一下 Pod 和容器之間的細微差別了；預設情況下如果 Pod 中的程序退出 Pod 也不會終止，
相反它將會重啟該程序。這類似於 docker run 時的 `--restart=always` 選項，這是主要差別。
在 docker 中，程序的每個呼叫的輸出都是被連線起來的，但是對於 Kubernetes，每個呼叫都是分開的。
要檢視以前在 Kubernetes 中執行的輸出，請執行以下操作：
...
-->
檢視[日誌架構](/zh-cn/docs/concepts/cluster-administration/logging/)獲取更多資訊。
...
-->
如何停止和刪除執行中的程序？檢視 [kubectl delete](/docs/reference/generated/kubectl/kubectl-commands/#delete)。
...
如果我們直接刪除 Pod，Deployment 將會重新建立該 Pod。
{{< /note >}}
...
-->
在 kubectl 中沒有對 `docker login` 的直接模擬。如果你有興趣在私有映象倉庫中使用 Kubernetes，
請參閱[使用私有映象倉庫](/zh-cn/docs/concepts/containers/images/#using-a-private-registry)。
...
-->
如何檢視客戶端和服務端的版本？檢視 [kubectl version](/docs/reference/generated/kubectl/kubectl-commands/#version)。
...
-->
如何獲取有關環境和配置的各種資訊？檢視 [kubectl cluster-info](/docs/reference/generated/kubectl/kubectl-commands/#cluster-info)。
