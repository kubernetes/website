...
-->
特性門控 `VolumeCapacityPriority` 在 v1.32 中用於支持靜態製備的儲存。
從 v1.33 開始，新的特性門控 `StorageCapacityScoring` 取代了舊的 `VolumeCapacityPriority` 特性門控，
並增加了對動態製備儲存的支持。當 `StorageCapacityScoring` 被啓用時，
kube-scheduler 中的 VolumeBinding 插件得到了擴展，可以基於每個節點上的儲存容量對 Node 進行評分。
此特性適用於已支持[儲存容量](/zh-cn/docs/concepts/storage/storage-capacity/) 的
CSI 卷，包括由 CSI 驅動所支持的本地儲存。
