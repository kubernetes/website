...
如果你將通過檢查點操作生成的容器數據移動到能夠恢復該容器的一臺計算機，
所恢復的容器將從之前檢查點操作執行的時間點繼續運行。
你也可以檢視所保存的數據，前提是你擁有這類操作的合適工具。
...
這裏的內容包括一切私密數據和可能用於加密的密鑰。
底層 CRI 實現（該節點上的容器運行時）應創建只有 `root` 用戶可以訪問的檢查點存檔。
...
還會請求將該檢查點存檔存儲到其根目錄（由 `--root-dir` 定義）下的 `checkpoints` 子目錄中。
這個目錄默認爲 `/var/lib/kubelet/checkpoints`。
...
的一種實現來讀取。存檔文件的內容取決於底層 CRI 實現（該節點的容器運行時）。
...
- **timeout** (**查詢參數**)：integer
  ...
  等待檢查點創建完成的超時時間（單位爲秒）。
  如果超時值爲零或未設定，將使用默認的 {{<glossary_tooltip term_id="cri" text="CRI">}} 超時時間值。
...
-->
TODO：一旦 CRI 實現具有檢查點/恢復能力，就會添加有關返回碼的更多信息。
