...
-->
許多特性依賴於特定的內核功能，並且有最低的內核版本要求。
然而，單純依賴內核版本號可能不足以滿足某些操作系統發行版，
因爲像 RHEL、Ubuntu 和 SUSE 等發行版的維護者們通常會將選定的特性反向移植到較舊的內核版本（保留較舊的內核版本）。
...
以下 sysctl 具有最低的內核版本要求，
並在[安全集](/zh-cn/docs/tasks/administer-cluster/sysctl-cluster/#safe-and-unsafe-sysctls)中得到了支持：
...
-->
- `net.ipv4.ip_local_reserved_ports`（自 Kubernetes 1.27 起，需要內核 3.16+）。
- `net.ipv4.tcp_keepalive_time`（自 Kubernetes 1.29 起，需要內核 4.5+）。
- `net.ipv4.tcp_fin_timeout`（自 Kubernetes 1.29 起，需要內核 4.6+）。
- `net.ipv4.tcp_keepalive_intvl`（自 Kubernetes 1.29 起，需要內核 4.5+）。
- `net.ipv4.tcp_keepalive_probes`（自 Kubernetes 1.29 起，需要內核 4.5+）。
- `net.ipv4.tcp_syncookies`（自內核 4.6+ 添加了命名空間作用域）。
- `net.ipv4.tcp_rmem`（自 Kubernetes 1.32，需要內核 4.15+）。
- `net.ipv4.tcp_wmem`（自 Kubernetes 1.32，需要內核 4.15+）。
- `net.ipv4.vs.conn_reuse_mode`（用於 `ipvs` 代理模式，需要內核 4.1+）。
...
-->
## v2 控制組   {#version2-control-groups}
...
-->
## 其他內核要求   {#requirements-other}

某些特性可能依賴於新的內核功能並具有特定的內核要求：
...
-->
## Linux 內核長期維護   {#linux-kernel-long-term-maintenance}

你可以在 [kernel.org](https://www.kernel.org/category/releases.html) 找到活動的內核版本。

通常會提供多個 __長期維護__ 內核版本，用於將 Bug 修復反向移植到較舊的內核樹。
特別是對於較舊的樹，只有重要的 Bug 修復纔會被應用到此類內核，這些內核通常不會頻繁發佈新版本。
請參閱 Linux 內核網站，瞭解 _Longterm_ 類別中的[發佈列表](https://www.kernel.org/category/releases.html)。
...
-->
- 查閱 [sysctls](/zh-cn/docs/tasks/administer-cluster/sysctl-cluster/) 以獲取更多細節。
- 允許在 [nftables 模式](/zh-cn/docs/reference/networking/virtual-ips/#proxy-mode-nftables)下運行 kube-proxy。
- 參閱 [cgroups v2](/zh-cn/docs/concepts/architecture/cgroups/)。
