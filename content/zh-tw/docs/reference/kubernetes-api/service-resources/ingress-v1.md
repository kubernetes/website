...
description: "Ingress 是允許入站連接到達後端定義的端點的規則集合。"
title: "Ingress"
weight: 4
---
<!--
...
- **defaultBackend** (<a href="{{< ref "../service-resources/ingress-v1#IngressBackend" >}}">IngressBackend</a>)
  ...
  defaultBackend 是負責處理與任何規則都不匹配的請求的後端。
  如果未指定 rules，則必須指定 defaultBackend。
  如果未設置 defaultBackend，則不符合任何 rules 的請求的處理將由 Ingress 控制器決定。
...
- **ingressClassName** (string)
  ...
  新創建的 Ingress 資源應該優先選擇使用該字段。但是，即使註解已被正式棄用，
  出於向後兼容性的原因，Ingress 控制器仍應能夠處理該註解（如果存在）。
...
- **rules** ([]IngressRule)
  ...
  *IngressRule represents the rules mapping the paths under a specified host to the related backend services. Incoming requests are first evaluated for a host match, then routed to the backend associated with the matching IngressRuleValue.*
  ...
  <a name="IngressRule"></a>
  **IngressRule 表示將指定主機下的路徑映射到相關後端服務的規則。
  傳入請求首先評估主機匹配，然後路由到與匹配的 IngressRuleValue 關聯的後端。**
  ...
  - **rules.host** (string)
    ...
    這兩種情況在未來都可能發生變化。入站請求在通過 IngressRuleValue 處理之前先進行 host 匹配。
    如果主機未指定，Ingress 將根據指定的 IngressRuleValue 規則路由所有流量。
    ...
    1. 如果 host 是精確匹配的，則如果 http `Host` 標頭等於 host 值，則請求與此規則匹配。
    2. 如果 host 是用通配符給出的，那麼如果 HTTP `Host` 標頭與通配符規則的後綴（刪除第一個標籤）相同，
       則請求與此規則匹配。
  ...
  - **rules.http** (HTTPIngressRuleValue)
    ...
    *HTTPIngressRuleValue is a list of http selectors pointing to backends. In the example: http://<host>/<path>?<searchpart> -> backend where where parts of the url correspond to RFC 3986, this resource will be used to match against everything after the last '/' and before the first '?' or '#'.*
    ...
    <a name="HTTPIngressRuleValue"></a>
    **HTTPIngressRuleValue 是指向後端的 http 選擇算符列表。例如 `http://<host>/<path>?<searchpart> -> 後端`，
    其中 `url` 的部分對應於 RFC 3986，此資源將用於匹配最後一個 “/” 之後和第一個 “?” 之前的所有內容或 “#”。**
    ...
    - **rules.http.paths** ([]HTTPIngressPath)，必需
      ...
      **Atomic: 將在合併期間被替換**
      ...
      paths 是一個將請求映射到後端的路徑集合。

      <a name="HTTPIngressPath"></a>
      **HTTPIngressPath 將路徑與後端關聯。與路徑匹配的傳入 URL 將轉發到後端。**
      ...
      - **rules.http.paths.pathType** (string)，必需
        ...
        * ImplementationSpecific: Interpretation of the Path matching is up to
        ...
        pathType 決定如何解釋 path 匹配。pathType 可以是以下值之一：

        * `Exact`：與 URL 路徑完全匹配。

        * `Prefix`：根據按 “/” 拆分的 URL 路徑前綴進行匹配。
          匹配是按路徑元素逐個元素完成。路徑元素引用的是路徑中由“/”分隔符拆分的標籤列表。
          如果每個 p 都是請求路徑 p 的元素前綴，則請求與路徑 p 匹配。
          請注意，如果路徑的最後一個元素是請求路徑中的最後一個元素的子字符串，則匹配不成功
          （例如 `/foo/bar` 匹配 `/foo/bar/baz`，但不匹配 `/foo/barbaz`）。

        * ImplementationSpecific：路徑匹配的解釋取決於 IngressClass。
          實現可以將其視爲單獨的路徑類型，也可以將其視爲前綴或確切的路徑類型。
          實現需要支持所有路徑類型。
      ...
      - **rules.http.paths.path** (string)
        ...
        path 要與傳入請求的路徑進行匹配。
        目前，它可以包含 RFC 3986 定義的 URL 的常規 “路徑” 部分所不允許的字符。
        路徑必須以 “/” 開頭，並且在 pathType 值爲 “Exact” 或 “Prefix” 時必須存在。
...
- **tls** ([]IngressTLS)
  ...
  **IngressTLS 描述與 Ingress 相關的傳輸層安全性。**
  ...
  - **tls.secretName** (string)
    ...
    secretName 是用於終止端口 443 上 TLS 通信的 Secret 的名稱。
    字段是可選的，以允許僅基於 SNI 主機名的 TLS 路由。
    如果偵聽器中的 SNI 主機與入口規則使用的 “Host” 標頭字段衝突，則 SNI 主機用於終止，Host 標頭的值用於路由。
...
- **resource** (<a href="{{< ref "../common-definitions/typed-local-object-reference#TypedLocalObjectReference" >}}">TypedLocalObjectReference</a>)
  ...
  resource 是對 Ingress 對象所在命名空間中另一個 Kubernetes 資源的引用。
  如果指定了 resource，則不得指定 service.name 和 service.port。
  此字段是一個與 `service` 互斥的設置。
...
- **service** (IngressServiceBackend)
  ...
  *IngressServiceBackend references a Kubernetes Service as a Backend.*
  ...
  service 引用一個 Service 作爲後端。此字段是一個與 `resource` 互斥的設置。

  <a name="IngressServiceBackend"></a>
  **IngressServiceBackend 引用一個 Kubernetes Service 作爲後端。**
  ...
  - **service.name** (string)，必需
    ...
    name 是引用的服務。服務必須與 Ingress 對象位於同一命名空間中。
  ...
  - **service.port** (ServiceBackendPort)
    ...
    所引用的服務的端口。IngressServiceBackend 需要端口名或端口號。
...
IngressStatus describe the current state of the Ingress.
-->
IngressStatus 描述 Ingress 的當前狀態。
  ...
  - **loadBalancer.ingress** ([]IngressLoadBalancerIngress)
    ...
    **IngressLoadBalancerIngress 表示負載均衡器入口點的狀態。**
    ...
    - **loadBalancer.ingress.hostname** (string)
      ...
      hostname 是爲基於 DNS 的負載平衡器入口點所設置的主機名。
    ...
    - **loadBalancer.ingress.ports** ([]IngressPortStatus)
      ...
      **IngressPortStatus 表示服務端口的錯誤情況**
      ...
      - **loadBalancer.ingress.ports.protocol** (string)，必需
        ...
        protocol 是入棧端口的協議。支持的值爲：“TCP”、“UDP”、“SCTP”。
