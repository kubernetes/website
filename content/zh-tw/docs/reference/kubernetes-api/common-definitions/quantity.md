...
（注意 <suffix> 可能爲空，例如 <decimalSI> 的 "" 情形。）
...
（國際單位制度；查閱： http://physics.nist.gov/cuu/Units/binary.html）

<decimalSI>       ::= m | "" | k | M | G | T | P | E

（注意，1024 = 1ki 但 1000 = 1k；我沒有選擇大寫。）
...
更大或更精確的數字將被截斷或向上取整（例如：0.1m 將向上取整爲 1m）。
如果將來我們需要更大或更小的數量，可能會擴展。

當從字符串解析數量時，它將記住它具有的後綴類型，並且在序列化時將再次使用相同類型。
...
-->
在序列化之前，數量將以“規範形式”放置。這意味着指數或者後綴將被向上或向下調整（尾數相應增加或減少），並確保：
...
這種格式旨在使得很難在不撰寫某種特殊處理代碼的情況下使用這些數字，進而希望實現者也使用定點實現。
