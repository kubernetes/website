...
- **webhooks** ([]ValidatingWebhook)
  ...
  **Map：name 鍵的唯一值將在合併期間保留**
  
  webhooks 是 Webhook 以及受影響的資源和操作的列表。

  <a name="ValidatingWebhook"></a>
  **ValidatingWebhook 描述了一個准入 Webhook 及其適用的資源和操作。**
    ...
    - **webhooks.clientConfig.service** (ServiceReference)
      ...
      **ServiceReference 持有對 Service.legacy.k8s.io 的引用**
      ...
      - **webhooks.clientConfig.service.path** (string)
        ...
        `path` 是一個可選的 URL 路徑，它將發送任何請求到此服務。
  ...
  - **webhooks.sideEffects** (string)，必需
   ...
   sideEffects 說明此 Webhook 是否有副作用。可接受的值爲：None、NoneOnDryRun（通過 v1beta1 創建的 Webhook 也可以指定 Some 或 Unknown）。
  ...
  **MatchCondition 表示將請求發送到 Webhook 之前必須滿足的條件。**
  ...
  - **webhooks.matchPolicy** (string)
    ...
    - Equivalent: 如果針對的資源包含在 “rules” 中，即使是通過另一個 API 組或版本，也視作匹配請求。 
    例如，如果可以通過 apps/v1、apps/v1beta1 和 extensions/v1beta1 修改 deployments 資源，
  ...
  - **webhooks.namespaceSelector** (<a href="{{< ref "../common-definitions/label-selector#LabelSelector" >}}">LabelSelector</a>)
    ...
    -->
    相反，如果你只想在命名空間與 “prod” 或 “staging” 的 “environment” 相關聯的對象上運行 Webhook；
    你可以按如下方式設置 selector:
    ```
    "namespaceSelector": {
    "matchExpressions": [
...
#### HTTP Request
-->
### `list` 列出或觀察 ValidatingWebhookConfiguration 類型的對象
...
#### HTTP Request
-->
### `create` 創建一個 ValidatingWebhookConfiguration
