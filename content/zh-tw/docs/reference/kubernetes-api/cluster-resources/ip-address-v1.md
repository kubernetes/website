...
description: "IPAddress 表示單個 IP 族的單個 IP。"
title: "IPAddress"
weight: 4
---
<!--
...
-->
IPAddress 表示單個 IP 族的單個 IP。此對象旨在供操作 IP 地址的 API 使用。
此對象由 Service 核心 API 用於分配 IP 地址。
IP 地址可以用不同的格式表示，爲了保證 IP 地址的唯一性，此對象的名稱是格式規範的 IP 地址。
IPv4 地址由點分隔的四個十進制數字組成，前導零可省略；IPv6 地址按照 RFC 5952 的定義來表示。
有效值：192.168.1.5、2001:db8::1 或 2001:db8:aaaa:bbbb:cccc:dddd:eeee:1。
無效值：10.01.2.3 或 2001:db8:0:0:0::1。
...
- **parentRef** (ParentReference)，必需
  ...
  parentRef 引用掛接 IPAddress 的資源。IPAddress 必須引用一個父對象。

  <a name="ParentReference"></a>
  **ParentReference 描述指向父對象的引用。**
...
#### HTTP Request
-->
### `deletecollection` 刪除 IPAddress 的集合
