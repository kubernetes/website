...
- **roleRef** (RoleRef)，必需
  ...
  RoleRef 只能引用全局命名空間中的 ClusterRole。
...
- **subjects** ([]Subject)
  ...
  **原子性：將在合併期間被替換**
  ...
  Subjects 包含角色所適用的對象的引用。
  
  <a name="Subject"></a>
  **Subject 包含對角色綁定所適用的對象或使用者標識的引用。其中可以包含直接 API 對象的引用或非對象（如使用者名和組名）的值。**
  <!--
  ...
  - **subjects.kind** (string)，必需
    ...
    被引用的對象的類別。這個 API 組定義的值是 `User`、`Group` 和 `ServiceAccount`。
  ...
  - **subjects.namespace** (string)
    ...
    被引用對象的命名空間。
    如果對象類別是 "User" 或 "Group" 等非命名空間作用域的對象且該值不爲空，
...
#### HTTP Request
-->
### `list` 列出或觀測類別爲 ClusterRoleBinding 的對象
