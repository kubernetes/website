...
<p>包 v1beta2 定義 kubeadm 配置檔案格式的 v1beta2 版本。
此版本改進了 v1beta1 的格式，修復了一些小問題並添加了一些新的欄位。</p>

<p>從 v1beta1 版本以來的變更列表：</p>
...
-->
<li>&quot;certificateKey&quot; 欄位被新增到 InitConfiguration 和 JoinConfiguration 中。</li>
<li>&quot;ignorePreflightErrors&quot; 欄位被新增到 NodeRegistrationOptions 中。</li>
<li>JSON 標籤 &quot;omitempty&quot; 在合適的情況下被用到更多的位置。</li>
<li>&quot;taints&quot; 欄位（在 NodeRegistrationOptions）的 JSON 標籤 &quot;omitempty&quot; 被去除。</li>
</ul>

<!--
<p>See the Kubernetes 1.15 changelog for further details.</p>
-->
<p>參閱 Kubernetes 1.15 的變更記錄以瞭解詳細資訊。</p>
...
<p>請使用 kubeadm v1.15.x 的 &quot;kubeadm config migrate&quot; 命令將 v1beta1
版本的配置檔案轉換為 v1beta2。
（從更老版本的 kubeadm 配置檔案遷移需要使用更老版本的 kubeadm。例如：</p>
<ul>
<li>kubeadm v1.11 版本可以用來從 v1alpha1 遷移到 v1alpha2 版本；kubeadm v1.12
可用來將 v1alpha2 翻譯為 v1alpha3。</li>
<li>kubeadm v1.13 或 v1.14 可以用來將 v1alpha3 遷移到 v1beta1。</li>
</ul>
）
...
-->
<p>儘管如此，kubeadm v1.15.x 會支援讀取 v1beta1 版本的 kubeadm 配置檔案格式。</p>
...
kubeadm 配置檔案中定義的某些配置選項也可以作為命令列標誌來使用，
不過這種方法所支援的都是一些最常見的、最簡單的使用場景。</p>
...
<p>如果使用者所提供的配置型別並非你所執行的操作需要的，
kubeadm 會忽略這些配置型別並列印警告資訊。</p>
...
-->
<p>型別 InitConfiguration 用來配置執行時設定，就 kubeadm init 命令而言，
包括啟動引導令牌以及所有與 kubeadm 所在節點相關的設定，包括：</p>

<ul>
<li><code>nodeRegistration</code>：其中包含與向叢集註冊新節點相關的欄位；
使用這個型別來定製節點名稱、要使用的 CRI 套接字或者其他僅對當前節點起作用的設定
（例如節點 IP 地址）。</li>
<li><code>apiServer</code>：代表的是要部署到此節點上的 API 伺服器示例的端點；
...
-->
<p>型別 <code>ClusterConfiguration</code> 用來定製叢集範圍的設定，具體包括以下設定：</p>

<ul>
<li><code>networking</code>：其中包含叢集的網路拓撲配置。使用這一部分可以定製 Pod
的子網或者 Service 的子網。</li>
...
-->
<li><code>etcd</code>：etcd 資料庫的配置。例如使用這個部分可以定製本地 etcd 或者配置
API 伺服器使用一個外部的 etcd 叢集。</li>
<li><code>kube-apiserver</code>、<code>kube-scheduler</code>、<code>kube-controller-manager</code>
配置：這些部分可以透過新增定製的設定或者過載 kubeadm 的預設設定來定製控制面元件。</li>
</ul>
...
-->
<p>KubeProxyConfiguration 型別用來更改傳遞給在叢集中部署的 kube-proxy 例項的配置。
...
-->
<p>JoinConfiguration 型別用來配置執行時設定，就 <code>kubeadm join</code>
而言包括用來訪問叢集資訊的發現方法，以及所有特定於 kubeadm 執行所在節點的設定，
包括：</p>
...
<td>
   ...
   -->
   <code>networking</code> 欄位包含叢集的網路拓撲配置。
</td>
</tr>
...
取值可以是一個合法的 IP 地址或者 RFC-1123 形式的 DNS 子域名，二者均可以帶一個可選的
TCP 埠號。
...
-->
<ul>
  <li>在一個包含不止一個控制面例項的叢集中，
該欄位應該設定為放置在控制面例項之前的外部負載均衡器的地址。</li>
  <li>在帶有強制性節點回收的環境中，<code>controlPlaneEndpoint</code>
可以用來為控制面設定一個穩定的 DNS。</li>
</ul>
</td>
</tr>
...
when this set to true, and at some point it may become ignored.</p>
   -->
   <p><code>useHyperKubeImage</code> 控制是否使用 hyperkube 來作為Kubernetes
元件，而不是一個個獨立的映象。
已啟用：由於 <code>hyperkube</code> 自身已被棄用，此欄位也被啟用。
將被從將來的 kubeadm 配置版本中移除，kubeadm 在此欄位設定為 true
時會列印多個警告資訊，並且在一些其他位置忽略此欄位設定。</p>
</td>
</tr>
...
-->
<p>ClusterStatus 包含叢集資訊。ClusterStatus 會被儲存在叢集中 kubeadm-config
ConfigMap 中，之後在新的控制面例項新增到叢集或者現有控制面例項離開叢集時被更新。</p>
<table class="table">
<thead><tr><th width="30%"><!--Field-->欄位</th><th><!--Description-->描述</th></tr></thead>
<tbody>
...
The key of the map is the IP of the host's default interface.</p>
   -->
   <p><code>apiEndpoints</code> 為當前叢集中可用的 API 端點，每個控制面例項
（API 伺服器）對應一個表項。
對映的鍵名為主機預設介面的 IP 地址。</p>
</td>
</tr>
</tbody>
</table>
...
information.</p>
-->
<p>InitConfiguration 包含一組特定於 &quot;kubeadm init&quot; 的執行時元素。</p>
...
這裡的資訊不會被上傳到 kubeadm 在叢集中儲存的 ConfigMap 中，部分原因是由於資訊本身比較敏感。</p>
</td>
</tr>
...
fails you may set the desired value here.</p>
   -->
   <p><code>localAPIEndpoint</code> 所代表的的是在此控制面節點上要部署的 API 伺服器的端點。
在高可用（HA）配置中，此欄位與 <code>ClusterConfiguration.controlPlaneEndpoint</code>
的取值不同：後者代表的是整個叢集的全域性端點，該端點上的請求會被負載均衡到每個 API 伺服器。
...
Defaults to &quot;/etc/kubernetes/pki/ca.crt&quot;.</p>
   -->
   <p><code>caCertPath</code> 是指向 SSL 證書機構的路徑，
該證書包用來加密節點與控制面之間的通訊。預設值為
...
bootstrap process.</p>
   -->
   <p><code>discovery</code> 設定 TLS 引導過程中 kubelet 要使用的選項。</p>
</td>
</tr>
...
on the joining node. If nil, no additional control plane instance will be deployed.</p>
   -->
   <p><code>controlPlane</code> 定義要在正被加入到叢集中的節點上部署的額外控制面例項。
此欄位為 null 時，不會再上面部署額外的控制面例項。</p>
</td>
</tr>
</tbody>
</table>
...
<td>
   ...
   -->
   <p><code>advertiseAddress</code> 設定 API 伺服器要公佈的 IP 地址。</p>
</td>
</tr>
...
Defaults to 6443.</p>
   -->
   <p><code>bindPort</code> 設定 API 伺服器要繫結到的安全埠。預設值為 6443。</p>
</td>
</tr>
</tbody>
</table>
...
-->
（<code>ControlPlaneComponent</code> 結構的欄位被嵌入到此型別中）
...
to appear.</p>
   -->
   <p><code>timeoutForControlPlane</code> 用來控制我們等待 API 伺服器開始執行的超時時間。</p>
</td>
</tr>
</tbody>
</table>
...
for, so other administrators can know its purpose.</p-->
   <p><code>description</code> 設定一個對人友好的訊息，
說明為什麼此令牌會存在以及其目標用途，這樣其他管理員能夠知道其目的。</p>
</td>
</tr>
...
<code>expires</code> and <code>ttl</code> are mutually exclusive.</p-->
   <p><code>ttl</code> 定義此令牌的宣告週期。預設為 '24h'。
...
dynamically at runtime based on the <code>ttl</code>. <code>expires</code> and <code>ttl</code> are mutually exclusive.</p-->
   <p><code>expires</code> 設定此令牌過期的時間戳。
預設為在執行時基於<code>ttl</code>來決定。
<code>expires</code>和<code>ttl</code>是互斥的。</p>
</td>
</tr>
...
for establishing bidirectional trust, but that can be changed here.</p-->
   <p><code>usages</code> 描述此令牌的可能使用方式。預設情況下，
令牌可用於建立雙向的信任關係；不過這裡可以改變預設用途。</p>
</td>
</tr>
...
information will be fetched.
   -->
   <code>apiServerEndpoint</code> 為 API 伺服器的 IP 地址或者域名，從該端點可以獲得叢集資訊。
</p>
</td>
</tr>
...
impersonate the control-plane.</p>
   -->
   <p><code>unsafeSkipCAVerification</code> 允許在使用基於令牌的服務發現時不使用
...
<p>BootstrapTokenString 形式為 'abcdef.abcdef0123456789' 的一個令牌，
...
-->
ControlPlaneComponent 中包含對叢集中所有控制面元件都適用的設定。
</p>
...
<p>
   ...
   component.
   -->
   <code>extraVolumes</code> 是一組額外被掛載到控制面元件中的主機卷。
</p>
</td>
</tr>
</tbody>
...
<td>
   ...
   -->
   <p><code>type</code> 定義要使用的 DNS 外掛型別。</p>
</td>
</tr>
...
-->
<p>DNSAddOnType 定義的是用來辨識 DNS 外掛型別的字串。</p>
...
-->
Discovery 設定 TLS 啟動引導過程中 kubelet 要使用的配置選項。
</p>
...
<code>bootstrapToken</code> and <code>file</code> are mutually exclusive.
   -->
   <code> 用來設定一個檔案或者 URL 路徑，指向一個 kubeconfig 檔案；
該配置檔案中包含叢集資訊。
<code>bootstrapToken</code> 與 <code>file</code> 是互斥的。
</td>
</tr>
...
如果設定了 <code>bootstrapToken</code>，則此欄位預設值為 <code>.bootstrapToken.token</code>，
不過可以被過載。
如果設定了 <code>file</code>，此欄位<B>必須被設定</B>，以防 kubeconfig
檔案中不包含其他身份認證資訊。
</p>
</td>
</tr>
...
<td>
   ...
   <p><code>name</code> 欄位為卷在 Pod 模板中的名稱。</p>
</td>
</tr>
...
<td>
   ...
   <p><code>mountPath</code> 是 hostPath 在 Pod 內掛載的路徑。</p>
</td>
</tr>
...
<td>
   ...
   <p><code>pathType</code> 是 hostPath 的型別。</p>
</td>
</tr>
</tbody>
</table>
...
<p>JoinControlPlane 包含在正在加入叢集的節點上要部署的額外的控制面元件的設定。</p>
...
deployed on this node.</p>
   -->
   <p><code>localAPIEndpoint</code> 代表的是將在此節點上部署的 API 伺服器例項的端點。</p>
</td>
</tr>
<tr><td><code>certificateKey</code> <B><!--[Required]-->[必需]</B><br/>
<code>string</code>
...
pod 中執行 etcd。對映中的每一個鍵對應命令列上的一個標誌引數，只是去掉了前置的連字元。</p>
</td>
</tr>
...
server signing certificate.</p>
   -->
   <p><code>serverCertSANs</code> 為 etcd 伺服器的簽名證書設定額外的主體替代名
...
signing certificate.</p>
   -->
   <p><code>peerCertSANs</code> 為 etcd 的對等端簽名證書設定額外的主體替代名
（Subject Alternative Names，SAN）。</p>
</td>
</tr>
</tbody>
...
此資訊會被以註解的方式新增到 Node API 物件之上，用於後續用途。</p>
</td>
</tr>
...
這裡的引數會透過 kubeadm 在執行時寫入的、由 kubelet 來讀取的環境檔案來傳遞給 kubelet 命令列。
...
the current node is registered.</p>
   -->
   <p><code>ignorePreflightErrors</code> 提供一組在當前節點被註冊時可以忽略掉的預檢錯誤。</p>
</td>
</tr>
</tbody>
</table>
