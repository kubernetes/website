...
-->
<code>authorizers</code> 是用於針對請求進行鑑權的鑑權組件的有序列表。
這類似於 <code>--authorization-modes</code> kube-apiserver 標誌。
必須至少包含一個元素。
</p>
</td>
</tr>
...
<td>
   ...
   -->
   <code>resources</code> 是一個包含資源及其對應加密驅動的列表。
   </p>
</td>
</tr>
</tbody>
...
<td>
   ...
   -->
   <code>keys</code> 是一個用於創建 AES 轉換器的密鑰列表。
   對於 AES-CBC，每個密鑰的長度必須是 32 字節；
   對於 AES-GCM，每個密鑰的長度可以是 16、24 或 32 字節。
   </p>
</td>
</tr>
</tbody>
...
It must match the registered admission plugin name.
  -->
  <code>name</code> 是准入控制器的名稱。它必須與所註冊的准入插件名稱匹配。
  </p>
</td>
</tr>
...
<td>
   ...
   -->
   <code>name</code> 是要使用的 KMS 插件的名稱。
   </p>
</td>
</tr>
...
<td>
   ...
   -->
   <code>secret</code> 是實際的密鑰，以 base64 編碼。
   </p>
</td>
</tr>
</tbody>
...
eg: '&ast;.' will encrypt all resources in the core group (such as pods, configmaps, etc).
   -->
   <code>resources</code> 是一個需要加密的 Kubernetes 資源列表。
   資源名稱來源於組/版本/資源的 “<code>resource</code>” 或 “<code>resource.group</code>”。
   例如，pandas.awesome.bears.example 是一個自定義資源，其 “group” 爲 awesome.bears.example，
   “resource” 爲 pandas。使用 “&ast;.&ast;” 以加密所有資源，使用 “&ast;.&lt;group&gt;” 以加密特定組中的所有資源。
   例如，“&ast;.awesome.bears.example” 將加密 “awesome.bears.example” 組中的所有資源。
   再比如，“&ast;.” 將加密核心組中的所有資源（如 Pod、ConfigMap 等）。
   </p>
</td>
</tr>
...
<td>
   ...
   -->
   <code>providers</code> 是從磁盤讀取資源和寫入資源到磁盤要使用的轉換器的列表。
...
<td>
   ...
   -->
   <code>keys</code> 是一個用於創建 Secretbox 轉換器的密鑰列表。每個密鑰的長度必須爲 32 字節。
   </p>
</td>
</tr>
</tbody>
...
Valid values:
-->
控制當 Webhook 請求未能完成、返回格式錯誤的響應或在評估 matchConditions 時發生錯誤的情況下鑑權決策。
有效值：
</p>
<ul>
<li>
...
-->
NoOpinion：繼續處理後續的鑑權組件以查看它們中是否有一個允許請求。
</li>
<li>
<!--
...
-->
<code>connectionInfo</code> 定義了我們如何與 Webhook 進行通信。
</p>
</td>
</tr>
...
-->
<code>matchConditions</code> 是請求發送到此 Webhook 之前必須滿足的一系列條件。
空的 <code>matchConditions</code> 列表匹配所有請求。
最多允許 64 個匹配條件。
</p>
<p>
<!--
...
-->
<li>如果至少一個 <code>matchConditions</code> 元素的計算結果爲 FALSE，則跳過此 Webhook。</li>
<li>如果所有 <code>matchConditions</code> 元素的計算結果都爲 TRUE，則調用此 Webhook。</li>
<li>如果至少一個 <code>matchConditions</code> 元素的計算結果爲出錯（但沒有 FALSE）：
<ul>
<li>如果 <code>failurePolicy</code> 爲 Deny，則 Webhook 拒絕請求。</li>
<li>如果 <code>failurePolicy</code> 爲 NoOpinion，則忽略錯誤並跳過此 Webhook。</li>
</ul>
</li>
</ol>
</td>
...
-->
<code>expression</code> 表示將由 CEL 求值的表達式。求值結果必須是布爾值。
CEL 表達式可以訪問 v1 版本中的 SubjectAccessReview 的內容。
如果請求變量中指定的 <code>subjectAccessReviewVersion</code> 是 v1beta1，
則在對 CEL 表達式求值之前，其內容會被轉換爲 v1 版本。
</p>
<ul>
<!--
...
-->
關於 CEL 的文檔：https://kubernetes.io/zh-cn/docs/reference/using-api/cel/
</p>
</td>
</tr>
</tbody>
