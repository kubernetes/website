...
<table>
<caption><!-- Examples of CEL expressions and the purpose of each -->CEL 表達式例子和每個表達式的用途</caption>
<thead>
<tr>
  <th><!-- Rule -->規則</th>
  <th><!-- Purpose -->用途</th>
...
<tr>
  ...
  <td><!-- Validate that the three fields defining replicas are ordered appropriately -->驗證定義副本的三個字段被正確排序</td>
</tr>
<tr>
  <td><tt>'Available' in self.stateCounts</tt></td>
  <td><!-- Validate that an entry with the 'Available' key exists in a map -->驗證映射中存在主鍵爲 'Available' 的條目</td>
</tr>
<tr>
  <td><tt>(self.list1.size() == 0) != (self.list2.size() == 0)</tt></td>
  <td><!-- Validate that one of two lists is non-empty, but not both -->驗證兩個列表中有一個非空，但不是兩個都非空</td>
</tr>
<tr>
  ...
  驗證 listMap 條目的 'value' 字段，其主鍵字段 'name' 是 'MY_ENV'</td>
</tr>
<tr>
  ...
  驗證 'expired' 日期在 'create' 日期加上 'ttl' 持續時間之後</td>
</tr>
<tr>
  <td><tt>self.health.startsWith('ok')</tt></td>
  <td><!-- Validate a 'health' string field has the prefix 'ok' -->驗證 'health' 字符串字段具有前綴 'ok'</td>
</tr>
<tr>
...
<tr>
  ...
  驗證 'details' 映射是由 'names' listSet 中的各項鍵入的</td>
</tr>
<tr>
  <td><tt>self.details.all(key, key.matches('^[a-zA-Z]*$'))</tt></td>
  <td><!-- Validate the keys of the 'details' map -->驗證 'details' 映射的主鍵</td>
</tr>
<tr>
  <td><tt>self.details.all(key, self.details[key].matches('^[a-zA-Z]*$'))</tt></td>
  <td><!-- Validate the values of the 'details' map -->驗證 'details' 映射的值</td>
</tr>
</tbody>
</table>
...
<thead>
<tr>
  <th><!-- CEL option, library or language feature -->CEL 選項、庫或語言特性</th>
  <th><!-- Included -->包含的內容</th>
...
<tr>
  ...
  <a href="https://github.com/google/cel-spec/blob/v0.7.0/doc/langdef.md#macros">標準宏</a></td>
...
<tr>
  ...
  <a href="https://github.com/google/cel-spec/blob/master/doc/langdef.md#list-of-standard-definitions">標準函數</a></td>
  <td><!-- See
...
<tr>
  <td>
    <a href="https://pkg.go.dev/github.com/google/cel-go@v0.17.4/cel#HomogeneousAggregateLiterals">
      <!-- Homogeneous Aggregate Literals -->同質聚合字面量
    </a>
  </td>
  <td>-</td>
  <td><!-- All Kubernetes versions -->所有 Kubernetes 版本</td>
...
<tr>
  <td>
    <a href="https://pkg.go.dev/github.com/google/cel-go@v0.17.4/cel#EagerlyValidateDeclarations">
      <!-- Eagerly Validate Declarations -->迫切驗證聲明
    </a>
  </td>
  <td>-</td>
  <td><!-- All Kubernetes versions -->所有 Kubernetes 版本</td>
...
<tr>
  ...
  <td> -->
  <a href="https://pkg.go.dev/github.com/google/cel-go/ext#Strings">擴展字符串庫</a>，v1</td>
  <td>
...
<tr>
  <td><!-- Kubernetes list library -->Kubernetes 列表庫</td>
  <td><!-- See
...
<tr>
  <td><!-- Kubernetes regex library -->Kubernetes 正則表達式庫</td>
  <td>See
    <a href="#kubernetes-regex-library">
      <!-- Kubernetes regex library -->Kubernetes 正則表達式庫
    </a>
  </td>
  <td><!-- All Kubernetes versions -->所有 Kubernetes 版本</td>
</tr>
<tr>
  <td><!-- Kubernetes URL library -->Kubernetes URL 庫</td>
  <td>See
    <a href="#kubernetes-url-library">
      <!-- Kubernetes URL library -->Kubernetes URL 庫
    </a>
  </td>
  <td><!-- All Kubernetes versions -->所有 Kubernetes 版本</td>
</tr>
<tr>
  <td><!-- Kubernetes authorizer library -->Kubernetes 鑑權組件庫</td>
  <td><!-- See
...
<tr>
  <td><!-- Kubernetes quantity library -->Kubernetes 數量庫</td>
  <td><!-- See
...
<tr>
  <td><!-- CEL optional types -->CEL 可選類型</td>
  <td><!-- See
...
-->
## Kubernetes CEL 庫   {#kubernetes-cel-libraries}

除了 CEL 社區庫之外，Kubernetes 還包括在 Kubernetes 中使用 CEL 時所有可用的 CEL 庫。
...
comparable types.
-->
### Kubernetes 列表庫   {#kubernetes-list-library}

列表庫包括 `indexOf` 和 `lastIndexOf`，這兩個函數的功能類似於同名的字符串函數。
這些函數返回提供的元素在列表中的第一個或最後一個位置索引。

列表庫還包括 `min`、`max` 和 `sum`。
`sum` 可以用於所有數字類型以及持續時間類型。
`min` 和 `max` 可用於所有可比較的類型。
...
Examples:
-->
`isSorted` 也作爲一個便捷的函數提供，並且支持所有可比較的類型。

例如：

<table>
<caption><!-- Examples of CEL expressions using list library functions -->使用列表庫函數的 CEL 表達式例子</caption>
<thead>
<tr>
  <td><!-- CEL Expression -->CEL 表達式</td>
  <td><!-- Purpose -->用途</td>
</tr>
</thead>
<tbody>
<tr>
  ...
  <td><!-- Verify that a list of names is kept in alphabetical order -->驗證名稱列表是否按字母順序排列</td>
</tr>
<tr>
  <td><tt>items.map(x, x.weight).sum() == 1.0</tt></td>
  <td><!-- Verify that the "weights" of a list of objects sum to 1.0 -->驗證對象列表的 “weight” 總和爲 1.0</td>
</tr>
<tr>
  <td><tt>lowPriorities.map(x, x.priority).max() &lt; highPriorities.map(x, x.priority).min()</tt></td>
  <td><!-- Verify that two sets of priorities do not overlap -->驗證兩組優先級不重疊</td>
</tr>
<tr>
  <td><tt>names.indexOf('should-be-first') == 1</tt></td>
  <td><!-- Require that the first name in a list if a specific value -->如果是特定值，則使用列表中的第一個名稱</td>
</tr>
</tbody>
</table>
...
Examples:
-->
### Kubernetes 正則表達式庫   {#kubernete-regex-library}

除了 CEL 標準庫提供的 `matches` 函數外，正則表達式庫還提供了 `find` 和 `findAll`，
使得更多種類的正則表達式運算成爲可能。

例如：

<table>
<caption><!-- Examples of CEL expressions using regex library functions -->使用正則表達式庫函數的 CEL 表達式例子</caption>
<thead>
<tr>
  <td><!-- CEL Expression -->CEL 表達式</td>
  <td><!-- Purpose -->用途</td>
...
<tr>
  ...
  <td><!-- Find the first number in a string -->找到字符串中的第一個數字</td>
</tr>
<tr>
  <td><tt>"1, 2, 3, 4".findAll('[0-9]+').map(x, int(x)).sum() &lt; 100</tt></td>
  <td><!-- Verify that the numbers in a string sum to less than 100 -->驗證字符串中的數字之和小於 100</td>
</tr>
</tbody>
</table>
...
-->
### Kubernetes URL 庫   {#kubernetes-url-library}

爲了更輕鬆、更安全地處理 URL，添加了以下函數：

- `isURL(string)` 按照
  [Go 的 net/url](https://pkg.go.dev/net/url#URL)
  檢查字符串是否是一個有效的 URL。該字符串必須是一個絕對 URL。
- `url(string) URL` 將字符串轉換爲 URL，如果字符串不是一個有效的 URL，則返回錯誤。
...
Examples:
-->
一旦通過 `url` 函數解析，所得到的 URL 對象就具有
`getScheme`、`getHost`、`getHostname`、`getPort`、`getEscapedPath` 和 `getQuery` 訪問器。

例如：

<table>
<caption><!-- Examples of CEL expressions using URL library functions -->使用 URL 庫函數的 CEL 表達式例子</caption>
<thead>
<tr>
  <td><!-- CEL Expression -->CEL 表達式</td>
  <td><!-- Purpose -->用途</td>
...
<tr>
  ...
  <td><!-- Gets the 'example.com:80' host part of the URL -->獲取 URL 的 'example.com:80' 主機部分</td>
</tr>
<tr>
...
-->
1. 指定要檢查的組和資源：`Authorizer.group(string).resource(string) ResourceCheck`
2. 可以調用以下任意組合的構建器函數（Builder Function），以進一步縮小鑑權檢查範圍。
   注意這些函數將返回接收者的類型，並且可以串接起來：
   - `ResourceCheck.subresource(string) ResourceCheck`
   - `ResourceCheck.namespace(string) ResourceCheck`
   - `ResourceCheck.name(string) ResourceCheck`
3. 調用 `ResourceCheck.check(verb string) Decision` 來執行鑑權檢查。
4. 調用 `allowed() bool` 或 `reason() string` 來查驗鑑權檢查的結果。
...
1. 調用 `PathCheck.check(httpVerb string) Decision` 來執行鑑權檢查。
1. 調用 `allowed() bool` 或 `reason() string` 來查驗鑑權檢查的結果。
...
<table>
<caption><!-- Examples of CEL expressions using URL library functions -->使用 URL 庫函數的 CEL 表達式示例</caption>
<thead>
<tr>
  <th><!-- CEL Expression -->CEL 表達式</th>
  <th><!-- Purpose -->用途</th>
</tr>
</thead>
<tbody>
...
<tr>
  ...
  <td><!-- Checks if the service account is authorized to delete deployments. -->檢查服務賬號是否有權限刪除 Deployment。</td>
</tr>
</tbody>
</table>
...
-->
啓用 Alpha 級別的 `AuthorizeWithSelectors` 特性後，字段和標籤選擇算符可以被添加到鑑權檢查中。

<table>
<caption><!-- Examples of CEL expressions using selector authorization functions -->使用選擇算符鑑權函數的 CEL 表達式示例</caption>
<thead>
<tr>
  <th><!-- CEL Expression -->CEL 表達式</th>
  <th><!-- Purpose -->用途</th>
...
-->
### Kubernetes 數量庫   {#kubernetes-quantity-library}

Kubernetes 1.28 添加了對數量字符串（例如 1.5G、512k、20Mi）的操作支持。
...
-->
- `isQuantity(string)` 根據 [Kubernetes 的 resource.Quantity](https://pkg.go.dev/k8s.io/apimachinery/pkg/api/resource#Quantity)，
  檢查字符串是否是有效的 Quantity。
- `quantity(string) Quantity` 將字符串轉換爲 Quantity，如果字符串不是有效的數量，則會報錯。

<!--
Once parsed via the `quantity` function, the resulting Quantity object has the
following library of member functions:
-->
一旦通過 `quantity` 函數解析，得到的 Quantity 對象將具有以下成員函數庫：

<table>
<caption><!-- Available member functions of a Quantity -->Quantity 的可用成員函數</caption>
<thead>
<tr>
  <th><!-- Member Function -->成員函數</th>
...
<tr>
  ...
  僅當 asInteger 可以被安全調用且不出錯時，才返回 true</td>
</tr>
<tr>
  ...
  <td>
    ...
    將當前值作爲 int64 的表示返回，如果轉換會導致溢出或精度丟失，則會報錯
  </td>
</tr>
<tr>
  ...
  <td>
    ...
    返回數量的 float64 表示，可能會丟失精度。如果數量的值超出了 float64 的範圍，則返回 +Inf/-Inf
    </td>
</tr>
<tr>
  <td><tt>sign()</tt></td>
  ...
  <td>
    ...
    <tt>0</tt> if it is zero. -->如果數量爲正，則返回 1；如果數量爲負，則返回 -1；如果數量爲零，則返回 0
  </td>
</tr>
<tr>
  <td><tt>add(&lt;Quantity&gt;)</tt></td>
  <td>Quantity</td>
  <td><!-- Returns sum of two quantities -->返回兩個數量的和</td>
</tr>
<tr>
  <td><tt>add(&lt;int&gt;)</tt></td>
  <td>Quantity</td>
  <td><!-- Returns sum of quantity and an integer -->返回數量和整數的和</td>
  <td>
<tr>
  <td><tt>sub(&lt;Quantity&gt;)</tt></td>
  <td>Quantity</td>
  <td><!-- Returns difference between two quantities -->返回兩個數量的差</td>
</tr>
<tr>
  <td><tt>sub(&lt;int&gt;)</tt></td>
  <td>Quantity</td>
  <td><!-- Returns difference between a quantity and an integer -->返回數量減去整數的差</td>
</tr>
<tr>
  <td><tt>isLessThan(&lt;Quantity&gt;)</tt></td>
  <td>bool</td>
  <td><!-- Returns true if and only if the receiver is less than the operand -->如果接收值小於操作數，則返回 true</td>
</tr>
<tr>
  <td><tt>isGreaterThan(&lt;Quantity&gt;)</tt></td>
  <td>bool</td>
  <td><!-- Returns true if and only if the receiver is greater than the operand -->如果接收值大於操作數，則返回 true</td>
</tr>
<tr>
  ...
  <td>
    ...
    將接收值與操作數進行比較，如果它們相等，則返回 0；如果接收值大於操作數，則返回 1；如果接收值小於操作數，則返回 -1
  </td>
</tr>
</tbody>
</table>
...
<tr>
  ...
  <td><!-- Test if conversion to integer would throw an error -->測試轉換爲整數是否會報錯</td>
</tr>
<tr>
  <td><tt>quantity("50k").asInteger()</tt></td>
  <td><!-- Precise conversion to integer -->精確轉換爲整數</td>
</tr>
<tr>
  <td><tt>quantity("9999999999999999999999999999999999999G").asApproximateFloat()</tt></td>
  <td><!-- Lossy conversion to float -->鬆散轉換爲浮點數</td>
</tr>
<tr>
  <td><tt>quantity("50k").add(quantity("20k"))</tt></td>
  <td><!-- Add two quantities -->兩個數量相加</td>
</tr>
<tr>
  <td><tt>quantity("50k").sub(20000)</tt></td>
  <td><!-- Subtract an integer from a quantity -->從數量中減去整數</td>
</tr>
<tr>
  <td><tt>quantity("50k").add(20).sub(quantity("100k")).sub(-50000)</tt></td>
  <td><!-- Chain adding and subtracting integers and quantities -->鏈式相加和減去整數和數量</td>
</tr>
<tr>
  <td><tt>quantity("200M").compareTo(quantity("0.2G"))</tt></td>
  <td><!-- Compare two quantities -->比較兩個數量</td>
</tr>
<tr>
  <td><tt>quantity("150Mi").isGreaterThan(quantity("100Mi"))</tt></td>
  <td><!-- Test if a quantity is greater than the receiver -->測試數量是否大於接收值</td>
</tr>
<tr>
  <td><tt>quantity("50M").isLessThan(quantity("100M"))</tt></td>
  <td><!-- Test if a quantity is less than the receiver -->測試數量是否小於接收值</td>
</tr>
</tbody>
</table>
...
CEL 是一種[逐漸類型化的語言](https://github.com/google/cel-spec/blob/master/doc/langdef.md#gradual-type-checking)。

一些 Kubernetes API 字段包含完全經過類型檢查的 CEL 表達式。
例如，[CustomResourceDefinition 驗證規則](/zh-cn/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#validation-rules)就是完全經過類型檢查的。
...
-->
一些 Kubernetes API 字段包含部分經過類型檢查的 CEL 表達式。
部分經過類型檢查的表達式是指一些變量是靜態類型，而另一些變量是動態類型的表達式。
例如在 [ValidatingAdmissionPolicy](/zh-cn/docs/reference/access-authn-authz/validating-admission-policy/)
的 CEL 表達式中，`request` 變量是有類型的，但 `object` 變量是動態類型的。
因此，包含 `request.namex` 的表達式將無法通過類型檢查，因爲 `namex` 字段未定義。
然而，即使對於 `object` 所引用的資源種類沒有定義 `namex` 字段，
`object.namex` 也會通過類型檢查，因爲 `object` 是動態類型。
...
value. For example:
-->
在 CEL 中，`has()` 宏可用於檢查動態類型變量的字段是否可訪問，然後再嘗試訪問該字段的值。
例如：
...
-->
## 類型系統集成   {#type-system-integration}

<table>
<caption><!-- Table showing the relationship between OpenAPIv3 types and CEL types -->
表格顯示了 OpenAPIv3 類型和 CEL 類型之間的關係</caption>
<thead>
<tr>
  <th><!-- OpenAPIv3 type -->OpenAPIv3 類型</th>
  <th><!-- CEL type -->CEL 類型</th>
</tr>
</thead>
<tbody>
<tr>
  <td><!-- 'object' with Properties -->設置了 properties 的 'object'</td>
  <td>
    <!-- object / "message type"
    (<tt>type(&lt;object&gt;)</tt> evaluates to
     <tt>selfType&lt;uniqueNumber&gt;.path.to.object.from.self</tt>) -->
    object / "message type"（<tt>type(<object>)<tt> 評估爲 <tt>selfType<uniqueNumber>.path.to.object.from.self<tt>）
  </td>
</tr>
<tr>
  <td><!-- 'object' with <tt>additionalProperties</tt> -->設置了 <tt>additionalProperties</tt> 的 'object'</td>
  <td>map</td>
</tr>
<tr>
  <td><!-- 'object' with <tt>x-kubernetes-embedded-type</tt> -->
  設置了 <tt>x-kubernetes-embedded-type</tt> 的 'object'</td>
  <td>
...
<tr>
  ...
  <td>
    ...
    int 或 string 的並集，<tt>self.intOrString < 100 || self.intOrString == '50%'<tt>
    對於 50 和 "50%" 都評估爲 true
  </td>
</tr>
<tr>
  <td>'array'</td>
...
<tr>
  <td><!-- 'array' with <tt>x-kubernetes-list-type=map</tt> -->設置了 <tt>x-kubernetes-list-type=map</tt> 的 'array'</td>
  <td><!-- list with map based Equality & unique key guarantees -->list，具有基於 Equality 和唯一鍵保證的 map</td>
</tr>
<tr>
  <td><!-- 'array' with <tt>x-kubernetes-list-type=set</tt> -->設置了 <tt>x-kubernetes-list-type=set</tt> 的 'array'</td>
  <td><!-- list with set based Equality & unique entry guarantees -->list，具有基於 Equality 和唯一條目保證的 set</td>
</tr>
<tr>
  <td>'boolean'</td>
  <td>boolean</td>
...
<tr>
  <td><!-- <i>no equivalent</i> --><i>非等價</i></td>
  <td>uint (64)</td>
</tr>
<tr>
  <td>'null'</td>
...
<tr>
  <td><!-- 'string' with format=byte (base64 encoded) -->設置了 format=byte 的 'string'（以 base64 編碼）</td>
  <td>bytes</td>
</tr>
<tr>
  <td><!-- 'string' with format=date -->設置了 format=date 的 'string'</td>
  <td>timestamp (<tt>google.protobuf.Timestamp</tt>)</td>
</tr>
<tr>
  <td><!-- 'string' with format=datetime -->設置了 format=datetime 的 'string'</td>
  <td>timestamp (<tt>google.protobuf.Timestamp</tt>)</td>
</tr>
<tr>
  <td><!-- 'string' with format=duration -->設置了 format=duration 的 'string'</td>
...
-->
另見：[CEL 類型](https://github.com/google/cel-spec/blob/v0.6.0/doc/langdef.md#values)、
...
list type:
-->
`x-kubernetes-list-type` 爲 `set` 或 `map` 的數組進行相等比較時會忽略元素順序。
例如，如果這些數組代表 Kubernetes 的 `set` 值，則 `[1, 2] == [2, 1]`。

使用 `x-kubernetes-list-type` 的數組進行串接時，使用 list 類型的語義：
...
-->
`set`
: `X + Y` 執行並集操作，保留 `X` 中所有元素的數組位置，
  將 `Y` 中非交集元素追加到 `X` 中，保留它們的部分順序。

`map`
: `X + Y` 執行合併操作，保留 `X` 中所有鍵的數組位置，
  但是當 `X` 和 `Y` 的鍵集相交時，將 `Y` 中的值覆蓋 `X` 中的值。
  將 `Y` 中非交集鍵的元素附加到 `X` 中，保留它們的部分順序。
...
僅形如 `[a-zA-Z_.-/][a-zA-Z0-9_.-/]*` 的 Kubernetes 資源屬性名可以從 CEL 中訪問。
當在表達式中訪問可訪問的屬性名時，會根據以下規則進行轉義：

<table>
<caption><!-- Table of CEL identifier escaping rules -->CEL 標識符轉義規則表</caption>
<thead>
<tr>
  <th><!-- escape sequence -->轉義序列</th>
  <th><!-- property name equivalent -->等價的屬性名</th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
  ...
  <td>
    <a href="https://github.com/google/cel-spec/blob/v0.6.0/doc/langdef.md#syntax">
      <!-- CEL <b>RESERVED</b> keyword -->CEL <b>保留的</b>關鍵字
    </a>
  </td>
</tr>
</tbody>
...
-->
當你需要轉義 CEL 的任一 **保留的** 關鍵字時，你需要使用下劃線轉義來完全匹配屬性名
（例如，`sprint` 這個單詞中的 `int` 不會被轉義，也不需要被轉義）。

轉義示例：

<table>
<caption><!-- Examples escaped CEL identifiers -->轉義的 CEL 標識符例子</caption>
<thead>
<tr>
  <th><!-- property name -->屬性名稱</th>
  <th><!-- rule with escaped property name -->帶有轉義的屬性名稱的規則</th>
</tr>
</thead>
<tbody>
<tr>
...
-->
CEL 的許多核心運算具有固定成本。例如比較（例如 `<`）這類最簡單的運算成本爲 1。
有些運算具有更高的固定成本，例如列表字面聲明具有 40 個成本單位的固定基礎成本。
...
-->
調用本地代碼實現的函數時，基於運算的時間複雜度估算其成本。
舉例而言：`match` 和 `find` 這類使用正則表達式的運算使用
`length(regexString)*length(inputString)` 的近似成本進行估算。
這個近似的成本反映了 Go 的 RE2 實現的最壞情況的時間複雜度。
...
-->
### 運行時成本預算   {#runtime-cost-budget}

所有由 Kubernetes 評估的 CEL 表達式都受到運行時成本預算的限制。
運行時成本預算是通過在解釋 CEL 表達式時增加成本單元計數器來計算實際 CPU 利用率的估算值。
如果 CEL 解釋器執行的指令太多，將超出運行時成本預算，表達式的執行將停止，並將出現錯誤。
...
-->
一些 Kubernetes 資源定義了額外的運行時成本預算，用於限制多個表達式的執行。
如果所有表達式的成本總和超過預算，表達式的執行將停止，並將出現錯誤。
例如，自定義資源的驗證具有針對驗證自定義資源所評估的所有
[驗證規則](/zh-cn/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#validation-rules)的
**每個驗證** 運行時成本預算。
