...
-->
你可以通過編寫設定檔案，並將其路徑傳給 `kube-scheduler` 的命令列參數，定製 `kube-scheduler` 的行爲。
...
-->
通過調度設定檔案，你可以設定 {{< glossary_tooltip text="kube-scheduler" term_id="kube-scheduler" >}} 在不同階段的調度行爲。
...
-->
你可以設定同一 `kube-scheduler` 實例使用[多個設定檔案](#multiple-profiles)。
...
1. `preFilter`: These plugins are used to pre-process or check information
   ...
   unschedulable.
 -->
2. `preFilter`：這些插件用於在過濾之前預處理或檢查 Pod 或叢集的資訊。
   它們可以將 Pod 標記爲不可調度。
<!--
...
1. `reserve`: This is an informational extension point that notifies plugins
   ...
   `Reserve`.
-->
7. `reserve`：這是一個資訊擴展點，當資源已經預留給 Pod 時，會通知插件。
   這些插件還實現了 `Unreserve` 介面，在 `Reserve` 期間或之後出現故障時調用。
<!--
...
-->
11. `postBind`：這是一個資訊擴展點，在 Pod 綁定了節點之後調用。
<!--
...
-->
對每個擴展點，你可以禁用[預設插件](#scheduling-plugins)或者是啓用自己的插件，例如：
...
desired.
-->
你可以在 `disabled` 數組中使用 `*` 禁用該擴展點的所有預設插件。
...
-->
### 多設定檔案 {#multiple-profiles}

你可以設定 `kube-scheduler` 運行多個設定檔案。
每個設定檔案都有一個關聯的調度器名稱，並且可以在其擴展點中設定一組不同的插件。
...
disabled.
-->
使用下面的設定樣例，調度器將運行兩個設定檔案：一個使用預設插件，另一個禁用所有打分插件。
...
-->
對於那些希望根據特定設定檔案來進行調度的 Pod，可以在 `.spec.schedulerName` 字段指定相應的調度器名稱。
...
-->
預設情況下，將創建一個調度器名爲 `default-scheduler` 的設定檔案。
這個設定檔案包括上面描述的所有預設插件。
聲明多個設定檔案時，每個設定檔案中調度器名稱必須唯一。
...
因此，應該存在一個調度器名爲 `default-scheduler` 的設定檔案來調度這些 Pod。
...
-->
所有設定檔案必須在 `queueSort` 擴展點使用相同的插件，並具有相同的設定參數（如果適用）。
...
-->
從 `kubescheduler.config.k8s.io/v1beta3` 開始，設定檔案設定中有一個附加字段
`multiPoint`，它允許跨多個擴展點輕鬆啓用或禁用插件。
`multiPoint` 設定的目的是簡化使用者和管理員在使用自定義設定檔案時所需的設定。
...
要爲其所有可用的擴展點啓用 `MyPlugin`，設定檔案設定如下所示：
...
這適用於禁用預設插件、非預設插件或使用通配符 (`'*'`) 來禁用所有插件。
...
-->
從 `kubescheduler.config.k8s.io/v1beta3` 開始，所有[預設插件](#scheduling-plugins)都通過 `MultiPoint` 在內部啓用。
但是，仍然可以使用單獨的擴展點來靈活地重新設定預設值（例如排序和分數權重）。
...
-->
在這個例子中，沒有必要在 `MultiPoint` 中明確指定插件，因爲它們是預設插件。
...
-->
## 調度程式設定遷移   {#scheduler-configuration-migrations}
...
* 在 v1beta2 設定版本中，你可以爲 `NodeResourcesFit` 插件使用新的 score 擴展。
  ...
  則可以改用 `NodeResourcesFit`（預設啓用）並添加一個 `pluginConfig` 和 `scoreStrategy`，類似於：
...
-->
* 在 v1beta2 設定檔案中啓用的插件優先於該插件的預設設定。
...
-->
* 調度器插件 `SelectorSpread` 被移除，改爲使用 `PodTopologySpread` 插件（預設啓用）來實現類似的行爲。
