...
1. 爲每個 kubelet 創建密鑰和證書；強烈建議爲每個 kubelet 使用獨一無二的、
   CN 取值與衆不同的密鑰和證書
1. 使用 CA 密鑰對 kubelet 證書籤名
1. 將 kubelet 密鑰和簽名的證書發佈到 kubelet 運行所在的特定節點上
...
-->
### 啓動引導初始化   {#bootstrap-initialization}
...
* 能夠識別對客戶端證書進行簽名的 CA
* 能夠對啓動引導的 kubelet 執行身份認證，並將其置入 `system:bootstrappers` 組
* 能夠對啓動引導的 kubelet 執行鑑權操作，允許其創建證書籤名請求（CSR）
...
如果還沒有設置，要爲 kube-apiserver 命令添加 `--client-ca-file=FILENAME`
標誌來啓用客戶端證書認證，在標誌中引用一個包含用來簽名的證書的證書機構包，
...
-->
儘管所有身份認證策略都可以用來對 kubelet 的初始啓動憑據來執行認證，
...
-->
啓動引導令牌是一種對 kubelet 進行身份認證的方法，相對簡單且容易管理，
且不需要在啓動 kube-apiserver 時設置額外的標誌。
...
-->
關於創建 Secret 的進一步細節可訪問[這裏](/zh-cn/docs/reference/access-authn-authz/bootstrap-tokens/)。

如果你希望使用啓動引導令牌，你必須在 kube-apiserver 上使用下面的標誌啓用之：
...
-->
### 授權 kubelet 創建 CSR    {#authorize-kubelet-to-create-csr}

現在啓動引導節點被 **身份認證** 爲 `system:bootstrappers` 組的成員，
它需要被 **授權** 創建證書籤名請求（CSR）並在證書被簽名之後將其取回。
...
* 能夠訪問你之前所創建並分發的 “Kubernetes CA 密鑰和證書”
* 啓用 CSR 簽名
...
爲了對 CSR 進行批覆，你需要告訴控制器管理器批覆這些 CSR 是可接受的。
這是通過將 RBAC 訪問權限授予正確的組來實現的。
...
* `nodeclient`：如果節點在爲節點創建新的證書，則該節點還沒有證書。
  該節點使用前文所列的令牌之一來執行身份認證，因此是 `system:bootstrappers` 組的成員。
* `selfnodeclient`：如果節點在對證書執行續期操作，則該節點已經擁有一個證書。
  節點持續使用現有的證書將自己認證爲 `system:nodes` 組的成員。
...
-->
要允許 kubelet 請求並接收新的證書，可以創建一個 `ClusterRoleBinding`
將啓動引導節點所處的組 `system:bootstrappers` 綁定到爲其賦予訪問權限的 `ClusterRole`
...
-->
要允許 kubelet 對其客戶端證書執行續期操作，可以創建一個 `ClusterRoleBinding`
將正常工作的節點所處的組 `system:nodes` 綁定到爲其授予訪問許可的 `ClusterRole`
...
-->
### 客戶端和服務證書   {#client-and-serving-certificates}

前文所述的內容都與 kubelet **客戶端**證書相關，尤其是 kubelet 用來向
...
-->
kubelet 也可以使用 **服務（Serving）** 證書。kubelet 自身向外提供一個 HTTPS 末端，
包含若干功能特性。要保證這些末端的安全性，kubelet 可以執行以下操作之一：
...
1. 請求中包含服務證書用法（確保 `spec.usages` 中包含 `server auth`，可選地也可包含
   `digital signature` 和 `key encipherment`，且不包含其它用法）
1. 僅包含隸屬於請求節點的 IP 和 DNS 的 `subjectAltNames`，沒有 URI 和 Email
   形式的 `subjectAltNames`（解析 `spec.request` 中的 x509 證書籤名請求可以檢查
   `subjectAltNames`）
{{< /note >}}
...
-->
## 其它身份認證組件   {#other-authenticating-components}

本文所描述的所有 TLS 啓動引導內容都與 kubelet 相關。不過，其它組件也可能需要直接與
kube-apiserver 直接通信。容易想到的是 kube-proxy，同樣隸屬於
Kubernetes 的節點組件並且運行在所有節點之上，不過也可能包含一些其它負責監控或者聯網的組件。
...
-->
與 kubelet 類似，這些其它組件也需要一種向 kube-apiserver 認證身份的方法。
你可以用幾種方法來生成這類憑據：
