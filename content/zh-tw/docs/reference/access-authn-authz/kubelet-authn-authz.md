...
-->
默認情況下，未被已配置的其他身份認證方法拒絕的對 kubelet 的 HTTPS 端點的請求會被視爲匿名請求，
並被賦予 `system:anonymous` 用戶名和 `system:unauthenticated` 組。
...
* 帶 `--kubelet-client-certificate` 和 `--kubelet-client-key` 標誌啓動 API 服務器
* 有關更多詳細信息，請參見
  [API 服務器身份認證文檔](/zh-cn/docs/reference/access-authn-authz/authentication/#x509-client-certificates)
...
-->
* 確保在 API 服務器中啓用了 `authentication.k8s.io/v1beta1` API 組
* 帶 `--authentication-token-webhook` 和 `--kubeconfig` 標誌啓動 kubelet
* kubelet 調用已配置的 API 服務器上的 `TokenReview` API，以根據持有者令牌確定用戶信息
...
-->
* 啓用了匿名身份認證，但是應限制匿名用戶調用 kubelet API 的能力
* 啓用了持有者令牌認證，但應限制任意 API 用戶（如服務賬號）調用 kubelet API 的能力
* 啓用了客戶端證書身份認證，但僅應允許已配置的 CA 簽名的某些客戶端證書使用 kubelet API

<!--
To subdivide access to the kubelet API, delegate authorization to the API server:
-->
要細分對 kubelet API 的訪問權限，請將鑑權委派給 API 服務器：
...
-->
* 確保在 API 服務器中啓用了 `authorization.k8s.io/v1beta1` API 組
* 帶 `--authorization-mode=Webhook` 和 `--kubeconfig` 標誌啓動 kubelet
* kubelet 調用已配置的 API 服務器上的 `SubjectAccessReview` API，
  以確定每個請求是否得到鑑權
...
-->
kubelet 使用與 API
服務器相同的[請求屬性](/zh-cn/docs/reference/access-authn-authz/authorization/#review-your-request-attributes)方法對
API 請求執行鑑權。
...
-->
在此模式下運行時，請確保傳遞給 API 服務器的由 `--kubelet-client-certificate` 和
`--kubelet-client-key` 標誌標識的用戶具有以下屬性的鑑權：
...
-->
當特性門控 `KubeletFineGrainedAuthz` 被啓用時，請確保經傳遞給 API 服務器的
`--kubelet-client-certificate` 和 `--kubelet-client-key` 標誌所鑑別的用戶被授權了以下屬性：
