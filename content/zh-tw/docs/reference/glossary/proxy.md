...
-->
客戶端與代理進行交互；代理將客戶端的數據複製到實際服務器；實際服務器回覆代理；代理將實際服務器的回覆發送給客戶端。
...
-->
[kube-proxy](/zh-cn/docs/reference/command-line-tools-reference/kube-proxy/) 是集羣中每個節點上運行的網絡代理，實現了部分 Kubernetes {{< glossary_tooltip term_id="service">}} 概念。
...
-->
你可以將 kube-proxy 作爲普通的用戶態代理服務運行。
