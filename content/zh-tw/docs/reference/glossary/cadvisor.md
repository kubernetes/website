...
-->
cAdvisor (Container Advisor) 爲容器用戶提供對其運行中的{{< glossary_tooltip text="容器" term_id="container" >}}
...
-->
cAdvisor 是一個守護進程，負責收集、聚合、處理並輸出運行中容器的信息。
具體而言，針對每個容器，該進程記錄容器的資源隔離參數、歷史資源用量、完整歷史資源用量和網絡統計的直方圖。
這些數據可以按容器或按機器層面輸出。
