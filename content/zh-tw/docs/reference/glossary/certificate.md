...
short_description: >
  證書是個安全加密文件，用來確認對 Kubernetes 叢集訪問的合法性。
...
<!--
 A cryptographically secure file used to validate access to the Kubernetes cluster.
-->
證書是個安全加密文件（cryptographically secure file），用來確認對 Kubernetes 叢集訪問的合法性。
...
-->
證書（Certificate）可以讓 Kubernetes 叢集中運行的應用程序安全的訪問 Kubernetes API。
證書可以確認客戶端是否被允許訪問 API。
