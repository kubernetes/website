...
short_description: >
  容器環境變量提供了 name=value 形式的、運行容器化應用所必須的一些重要資訊。
...
-->
容器環境變量爲運行中的容器化應用提供必要的資訊，
同時還提供與{{< glossary_tooltip text="容器" term_id="container" >}}相關的重要細節資訊，
例如：檔案系統資訊、容器自身的資訊以及其他像服務端點（Service endpoints）這樣的叢集資源資訊。
