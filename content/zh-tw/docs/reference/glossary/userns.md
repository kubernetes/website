...
short_description: >
  一種爲非特權用戶模擬超級用戶特權的 Linux 內核功能特性。
...
-->
用來模擬 root 用戶的內核功能特性。用來支持“Rootless 容器”。
...
-->
用戶名字空間（User Namespace）是一種 Linux 內核功能特性，允許非 root 用戶
模擬超級用戶（"root"）的特權，例如用來運行容器卻不必成爲容器之外的超級用戶。

<!--
User namespace is effective for mitigating damage of potential container break-out attacks.
-->
用戶名字空間對於緩解因潛在的容器逃逸攻擊而言是有效的。
...
of the term.
-->
在用戶名字空間語境中，名字空間是 Linux 內核的功能特性而不是 Kubernetes 意義上的
