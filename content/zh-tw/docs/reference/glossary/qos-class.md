...
short_description: >
  QoS 類（Quality of Service Class）爲 Kubernetes 提供了一種將叢集中的 Pod 分爲幾個類並做出有關調度和驅逐決策的方法。
...
<!--
 QoS Class (Quality of Service Class) provides a way for Kubernetes to classify Pods within the cluster into several classes and make decisions about scheduling and eviction.
-->
 QoS Class（Quality of Service Class）爲 Kubernetes 提供了一種將叢集中的 Pod
...
-->
Pod 的 QoS 類是基於 Pod 在創建時設定的計算資源請求和限制。QoS 類用於制定有關 Pod 調度和逐出的決策。
