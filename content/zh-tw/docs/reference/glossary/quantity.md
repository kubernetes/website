...
full_link:
short_description: >
  使用全數字來表示較小數值或使用 SI 後綴表示較大數值的表示法。
...
-->
使用全數字來表示較小數值或使用 [SI](https://zh.wikipedia.org/wiki/International_System_of_Units) 後綴表示較大數值的表示法。
...
例如，數字 `1.5` 表示爲 `1500m`，
而數字 `1000` 表示爲 `1k`，`1000000` 表示爲 `1M`。 
你還可以指定二進制表示法後綴；數字 2048 可以寫成 `2Ki`。

公認的十進制（10 的冪數）單位是 `m`（milli）、`k`（kilo，有意小寫）、
`M`（mega）、`G`（giga）、`T`（terra）、`P`（peta）、`E`（exa）。

公認的二進制（2 的冪數）單位是 `Ki` (kibi)、`Mi` (mebi)、`Gi` (gibi)、
`Ti` (tebi)、 `Pi` (pebi)、 `Ei` (exbi)。
