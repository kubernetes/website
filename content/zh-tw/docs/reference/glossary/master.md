...
-->
該術語仍被一些配置工具使用，如 {{< glossary_tooltip text="kubeadm" term_id="kubeadm" >}} 以及託管的服務，爲 {{< glossary_tooltip text="節點（nodes）" term_id="node" >}} 添加 `kubernetes.io/role` 的 {{< glossary_tooltip text="標籤（label）" term_id="label" >}}，以及管理控制平面 Pod 的調度。