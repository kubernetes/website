...
short_description: >
  一個帶有命名空間的鍵，告訴 Kubernetes 等到特定的條件被滿足後，
  再完全刪除被標記爲刪除的資源。
aka: 
tags:
- fundamental
...
-->
Finalizer 是帶有命名空間的鍵，告訴 Kubernetes 等到特定的條件被滿足後，
再完全刪除被標記爲刪除的{{< glossary_tooltip text="資源" term_id="api-resource" >}}。
Finalizer 提醒{{<glossary_tooltip text="控制器" term_id="controller">}}清理被刪除的對象擁有的資源。
...
-->
當你告訴 Kubernetes 刪除一個指定了 Finalizer 的對象時，
Kubernetes API 通過填充 `.metadata.deletionTimestamp` 來標記要刪除的對象，
並返回 `202` 狀態碼（HTTP "已接受"）使其進入只讀狀態。
此時控制平面或其他組件會採取 Finalizer 所定義的行動，
而目標對象仍然處於終止中（Terminating）的狀態。
這些行動完成後，控制器會刪除目標對象相關的 Finalizer。
當 `metadata.finalizers` 字段爲空時，Kubernetes 認爲刪除已完成並刪除對象。
...
例如，你可以定義一個 Finalizer，在刪除目標資源前清理相關
{{< glossary_tooltip text="API 資源" term_id="api-resource" >}}或基礎設施。
