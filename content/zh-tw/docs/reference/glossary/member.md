...
-->
 ...
 K8s 社區中持續活躍的{{< glossary_tooltip text="貢獻者（contributor）" term_id="contributor" >}}。
...
可以將問題單（issue）和 PR 指派給成員（Member），成員（Member）也可以通過 GitHub 小組加入 {{< glossary_tooltip text="特別興趣小組 (SIGs)" term_id="sig" >}}。針對成員（Member）所提交的 PR，系統自動運行提交前測試。成員（Member）應該是持續活躍的社區貢獻者。
