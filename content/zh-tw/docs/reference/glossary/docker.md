...
short_description: >
  Docker 是一種可以提供操作系統級別虛擬化（也稱作容器）的軟體技術。
...
（也稱作{{< glossary_tooltip text="容器" term_id="container" >}}）的軟體技術。
...
-->
Docker 使用了 Linux 內核中的資源隔離特性（如 cgroup 和內核命名空間）以及支持聯合檔案系統（如 OverlayFS 和其他），
