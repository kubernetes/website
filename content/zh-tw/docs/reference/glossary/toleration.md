...
short_description: >
  容忍度是一種核心對象，包含三個必需的屬性：key、value 和 effect。
  容忍度允許將 Pod 調度到具有對應污點的節點或節點組上。
...
-->
容忍度是一種核心對象，包含三個必需的屬性：key、value 和 effect。容忍度允許將 Pod
調度到具有對應{{< glossary_tooltip text="污點" term_id="taint" >}}的節點或節點組上。
...
-->
容忍度和{{< glossary_tooltip text="污點" term_id="taint" >}}共同作用可以確保不會將 Pod
調度在不適合的節點上。在同一 {{< glossary_tooltip text="Pod" term_id="pod" >}}
上可以設置一個或者多個容忍度。
容忍度表示在包含對應{{< glossary_tooltip text="污點" term_id="taint" >}}的節點或節點組上調度
{{< glossary_tooltip text="Pod" term_id="pod" >}}是允許的（但並非必需）。
