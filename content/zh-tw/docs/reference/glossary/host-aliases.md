...
short_description: >
  主機別名 (HostAliases) 是一組 IP 地址和主機名的映射，用於注入到 Pod 內的 hosts 檔案。
...
-->
 主機別名 (HostAliases) 是一組 IP 地址和主機名的映射，用於注入到 {{< glossary_tooltip text="Pod" term_id="pod" >}} 內的 hosts 檔案。
...
是一個包含主機名和 IP 地址的可選列表，設定後將被注入到 Pod 內的 hosts 檔案中。
