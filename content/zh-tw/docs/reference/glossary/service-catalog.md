---
title: 服務目錄（Service Catalog）
id: service-catalog
date: 2018-04-12
full_link: 
short_description: >
  服務目錄是一種擴展 API，它能讓 Kubernetes 集羣中運行的應用易於使用外部託管的軟件服務，例如雲供應商提供的數據倉庫服務。
  
aka: 
tags:
- extension
---

<!--
title: Service Catalog
id: service-catalog
date: 2018-04-12
full_link: 
short_description: >
  A former extension API that enables applications running in Kubernetes clusters to easily use external managed software offerings, such as a datastore service offered by a cloud provider.

aka: 
tags:
- extension
-->

<!--
 A former extension API that enables applications running in Kubernetes clusters to easily use external managed software offerings, such as a datastore service offered by a cloud provider.
-->
服務目錄是一種過去曾經存在的擴展 API，它能讓 Kubernetes 集羣中運行的應用易於使用外部託管的軟件服務，例如雲供應商提供的數據倉庫服務。

<!--more--> 

<!--
It provided a way to list, provision, and bind with external {{< glossary_tooltip text="Managed Services" term_id="managed-service" >}} without needing detailed knowledge about how those services would be created or managed.
-->
服務目錄可以檢索、供應並綁定外部{{< glossary_tooltip text="託管服務（Managed Services）" term_id="managed-service" >}}，
而無需知道那些服務具體是怎樣創建和託管的。
