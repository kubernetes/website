...
-->
本頁面向你展示如何在節點上加載 AppArmor 設定檔案並在 Pod 中強制應用這些設定檔案。
要了解有關 Kubernetes 如何使用 AppArmor 限制 Pod 的更多資訊，請參閱
...
-->
1. AppArmor 內核模塊已啓用 —— 要使 Linux 內核強制執行 AppArmor 設定檔案，
   必須安裝並且啓動 AppArmor 內核模塊。預設情況下，有幾個發行版支持該模塊，
...
-->
3. 設定檔案已加載 —— 通過指定每個容器應使用的 AppArmor 設定檔案，
   AppArmor 會被應用到 Pod 上。如果所指定的設定檔案未加載到內核，
   kubelet 將拒絕 Pod。
   通過檢查 `/sys/kernel/security/apparmor/profiles` 檔案，
   可以查看節點加載了哪些設定檔案。例如:
   ...
   有關在節點上加載設定檔案的詳細資訊，請參見[使用設定檔案設置節點](#setting-up-nodes-with-profiles)。
...
-->
AppArmor 設定檔案可以在 Pod 級別或容器級別指定。容器
AppArmor 設定檔案優先於 Pod 設定檔案。
...
-->
* `RuntimeDefault` 使用運行時的預設設定檔案
* `Localhost` 使用主機上加載的設定檔案（見下文）
...
-->
有關 AppArmor 設定檔案 API 的完整詳細資訊，請參閱[指定 AppArmor 限制](#specifying-apparmor-confinement)。
...
-->
要驗證是否應用了設定檔案，
你可以通過檢查容器根進程的進程屬性來檢查該進程是否正在使用正確的設定檔案運行：
...
-->
你還可以通過檢查容器的 proc attr，直接驗證容器的根進程是否以正確的設定檔案運行：
...
-->
首先，將要使用的設定檔案加載到節點上，該設定檔案阻止所有檔案寫入操作：
...
-->
由於不知道 Pod 將被調度到哪裏，該設定檔案需要加載到所有節點上。
在本例中，你可以使用 SSH 來安裝設定檔案，
但是在[使用設定檔案設置節點](#setting-up-nodes-with-profiles)中討論了其他方法。
...
-->
接下來，運行一個帶有拒絕寫入設定檔案的簡單 “Hello AppArmor” Pod：
...
-->
你可以通過檢查其 `/proc/1/attr/current` 來驗證容器是否確實使用該設定檔案運行：
...
-->
最後，你可以看到，如果你通過寫入檔案來違反設定檔案會發生什麼：
...
-->
### 使用設定檔案設置節點 {#setting-up-nodes-with-profiles}
...
-->
Kubernetes {{< skew currentVersion >}} 目前不提供任何本地機制來將 AppArmor 設定檔案加載到節點上。
...
-->
調度程式不知道哪些設定檔案加載到哪個節點上，因此必須將全套設定檔案加載到每個節點上。
另一種方法是爲節點上的每個設定檔案（或設定檔案類）添加節點標籤，
...
更多詳細資訊參見 [AppArmor 失敗](https://gitlab.com/apparmor/apparmor/wikis/AppArmor_Failures)。
...
-->
### 安全上下文中的 AppArmor 設定檔案   {#appArmorProfile}
...
如果在 Pod 級別設置設定檔案，該設定將被用作 Pod 中所有容器（包括 Init、Sidecar 和臨時容器）的預設設定檔案。
如果同時設置了 Pod 和容器 AppArmor 設定檔案，則將使用容器的設定檔案。

AppArmor 設定檔案有 2 個字段：
...
-->
`type` **（必需）** - 指示將應用哪種 AppArmor 設定檔案。有效選項是：

`Localhost`
: 節點上預加載的設定檔案（由 `localhostProfile` 指定）。
...
-->
`localhostProfile` - 在節點上加載的、應被使用的設定檔案的名稱。
該設定檔案必須在節點上預先設定才能工作。
...
-->
* [Apparmor 設定檔案語言快速指南](https://gitlab.com/apparmor/apparmor/wikis/QuickProfileLanguage)
