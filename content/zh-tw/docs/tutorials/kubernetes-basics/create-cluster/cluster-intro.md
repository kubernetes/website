...
可編排在計算機叢集內和跨計算機叢集的應用容器的部署（調度）和執行。**
{{% /alert %}}
...
-->
**Kubernetes 協調一個高可用計算機叢集，每個計算機互相連接之後作爲同一個工作單元運行。**
Kubernetes 中的抽象允許你將容器化的應用部署到叢集，而無需將它們綁定到某個特定的獨立計算機。
爲了使用這種新的部署模型，需要以將應用與單個主機解耦的方式打包：它們需要被容器化。
與過去的那種應用直接以包的方式深度與主機集成的部署模型相比，容器化應用更靈活、更可用。
**Kubernetes 以更高效的方式跨叢集自動分佈和調度應用容器。**
...
**控制面負責管理整個叢集。**
控制面協調叢集中的所有活動，例如調度應用、維護應用的期望狀態、對應用擴容以及將新的更新上線等等。
...
-->
**節點是一個虛擬機或者物理機，它在 Kubernetes 叢集中充當工作機器的角色。**
...
處理生產級流量的 Kubernetes 叢集至少應具有三個節點，因爲如果只有一個節點，出現故障時其對應的
...
與控制面通信。終端使用者也可以使用 Kubernetes API 與叢集交互。
...
-->
Kubernetes 既可以部署在物理機上也可以部署在虛擬機上。你可以使用 Minikube 開始部署 Kubernetes 叢集。
Minikube 是一種輕量級的 Kubernetes 實現，可在本地計算機上創建 VM 並部署僅包含一個節點的簡單叢集。
Minikube 可用於 Linux、macOS 和 Windows 系統。Minikube CLI 提供了用於引導叢集工作的多種操作，
包括啓動、停止、查看狀態和刪除。
...
* 瞭解更多關於[叢集架構](/zh-cn/docs/concepts/architecture/)方面的知識。
