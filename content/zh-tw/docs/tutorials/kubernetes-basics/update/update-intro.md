...
-->
用戶希望應用程序始終可用，而開發人員則需要每天多次部署它們的新版本。
...
在更新期間僅將流量負載均衡到可用的 Pod。可用的 Pod 是指對應用的用戶可用的實例。
...
-->
要查看應用程序當前的鏡像版本，可以運行 `describe pods` 子命令，
然後查找 `Image` 字段：
...
-->
要將應用程序的鏡像版本更新爲 v2，可以使用 `set image` 子命令，
後面跟着 Deployment 名稱和新版本的鏡像：
...
-->
此命令通知 Deployment 爲應用程序使用不同的鏡像，並啓動滾動更新。
...
subcommand:
-->
在受影響的 Pod 的輸出中，注意其 `Events` 部分包含 `v10` 版本的鏡像在倉庫中不存在的信息。
...
-->
`rollout undo` 命令會恢復 Deployment 到先前的已知狀態（`v2` 的鏡像）。
...
-->
要檢查正在運行的 Pod 上部署的鏡像，請使用 `describe pods` 子命令：
