...
-->
這篇文檔基於[設定 Pod 以使用 ConfigMap](/zh-cn/docs/tasks/configure-pod-container/configure-pod-configmap/)
這個任務，提供了一個使用 ConfigMap 來設定 Redis 的真實案例。
...
* 理解[設定 Pod 以使用 ConfigMap](/zh-cn/docs/tasks/configure-pod-container/configure-pod-configmap/)。
...
-->
使用 `kubectl exec` 進入 pod，運行 `redis-cli` 工具檢查當前設定：
...
-->
現在，向 `example-redis-config` ConfigMap 添加一些設定：
...
-->
通過 `kubectl exec` 使用 `redis-cli` 再次檢查 Redis Pod，查看是否已應用設定：
...
-->
設定值未更改，因爲需要重新啓動 Pod 才能從關聯的 ConfigMap 中獲取更新的值。
讓我們刪除並重新創建 Pod：
