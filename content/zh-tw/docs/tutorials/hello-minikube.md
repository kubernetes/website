...
-->
您還需要安裝 `kubectl`。
請參閱[安裝工具](/docs/tasks/tools/#kubectl)以取得安裝說明。
...
Kubernetes [*Pod*](/docs/concepts/workloads/pods/) 是由一個或多個容器組成的群組，
以便集中管理並共用網路資源。本教學中的 Pod 只有一個容器。Kubernetes
[*Deployment*](/docs/concepts/workloads/controllers/deployment/) 會檢查 Pod 的健康狀態，
...
有關 `kubectl` 指令的更多資訊，請參閱 [kubectl 概述](/docs/reference/kubectl/)。
{{< /note >}}
...
您必須將 Pod 公開為 Kubernetes [*Service*](/docs/concepts/services-networking/service/)。
...
-->
* 教學：_[使用 kubectl 在 Kubernetes 上部署您的第一個應用程式](/docs/tutorials/kubernetes-basics/deploy-app/deploy-intro/)_。
* 深入了解 [Deployment 物件](/docs/concepts/workloads/controllers/deployment/)。
* 深入了解[部署應用程式](/docs/tasks/run-application/run-stateless-application-deployment/)。
* 深入了解 [Service 物件](/docs/concepts/services-networking/service/)。
