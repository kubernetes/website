...
1. 將憑據保存到文件：
   ...
   -->
   `-n` 標誌用來確保生成文件的文末沒有多餘的換行符。這很重要，因爲當 `kubectl`
   讀取文件並將內容編碼爲 base64 字符串時，額外的換行符也會被編碼。
   你不需要對文件中包含的字符串中的特殊字符進行轉義。
...
2. 在 `kubectl` 命令中傳遞文件路徑：
   ...
   using `--from-file=[key=]source`. For example:
    -->
   默認鍵名爲文件名。你也可以通過 `--from-file=[key=]source` 設置鍵名，例如：
...
-->
`kubectl get` 和 `kubectl describe` 命令默認不顯示 `Secret` 的內容。
這是爲了防止 `Secret` 被意外暴露或存儲在終端日誌中。
...
2. 解碼 `password` 數據:
   ...
   decode commands.
   -->
   這是一個出於文檔編制目的的示例。實際上，該方法可能會導致包含編碼數據的命令存儲在
...
-->
這將打開默認編輯器，並允許你更新 `data` 字段中的 base64 編碼的 Secret 值，示例如下：
