...
從 Pod 內部訪問 API 時，定位 API 服務器和向服務器認證身份的操作與外部客戶端場景不同。
...
-->
在以上場景中，客戶端庫都使用 Pod 的服務賬號憑據來與 API 服務器安全地通信。
...
服務器生成一個 HTTPS URL。
API 服務器的集羣內地址也發佈到 `default` 命名空間中名爲 `kubernetes` 的 Service 中，
從而 Pod 可以引用 `kubernetes.default.svc` 作爲本地 API 服務器的 DNS 名稱。
...
-->
Kubernetes 不保證 API 服務器具有主機名 `kubernetes.default.svc` 的有效證書；
...
通過將認證令牌直接發送到 API 服務器，也可以避免運行 kubectl proxy 命令。
內部的證書機制能夠爲連接提供保護。
