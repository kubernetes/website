...
在當前證書即將過期時，
將自動生成新的祕鑰，並從 Kubernetes API 申請新的證書。 一旦新的證書可用，它將被用於與
Kubernetes API 間的連接認證。
...
## 啓用客戶端證書輪換
 ...
 `kubelet` 進程接收 `--rotate-certificates` 參數，該參數決定 kubelet 在當前使用的
證書即將到期時，是否會自動申請新的證書。
...
-->
`kube-controller-manager` 進程接收 `--cluster-signing-duration` 參數
（在 1.19 版本之前爲 `--experimental-cluster-signing-duration`），用來
控制簽發證書的有效期限。
...
證書的有效期限由 `--cluster-signing-duration` 參數指定，簽署的證書會被附加到證書籤名請求中。
...
會從 Kubernetes API 取回簽署的證書，並將其寫入磁盤。 然後它會更新與 Kubernetes API
