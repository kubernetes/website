...
-->
本文展示如何使用 kubectl 來列出集羣中所有運行 Pod 的容器的鏡像
...
-->
在本練習中，你將使用 kubectl 來獲取集羣中運行的所有 Pod，並格式化輸出來提取每個 Pod 中的容器列表。
...
-->
## 列出所有命名空間下的所有容器鏡像   {#list-all-container-images-in-all-namespaces}

- 使用 `kubectl get pods --all-namespaces` 獲取所有命名空間下的所有 Pod
- 使用 `-o jsonpath={.items[*].spec['initContainers', 'containers'][*].image}` 來格式化輸出，以僅包含容器鏡像名稱。
...
-->
## 按 Pod 列出容器鏡像   {#list-container-images-by-pod}
...
-->
## 列出以標籤過濾後的 Pod 的所有容器鏡像   {#list-container-images-filtering-by-pod-label}
...
-->
## 列出以命名空間過濾後的 Pod 的所有容器鏡像   {#list-container-images-filtering-by-pod-namespace}
...
-->
## 使用 go-template 代替 jsonpath 來獲取容器鏡像   {#list-container-images-using-a-go-template-instead-of-jsonpath}
