...
- 通過公網 IP 訪問服務
  ...
  - 取決於你的叢集環境，你可以僅把 Service 暴露在你的企業網路環境中，也可以將其暴露在
...
典型情況下，kube-system 名字空間中會啓動叢集的幾個服務。
...
（使用 Elasticsearch）啓用了叢集層面的日誌。如果提供合適的憑據，可以通過
...
-->
{{< note >}}
請參閱[使用 Kubernetes API 訪問叢集](/zh-cn/docs/tasks/administer-cluster/access-cluster-api/#accessing-the-kubernetes-api)
...
-->
#### 通過 Web 瀏覽器訪問叢集中運行的服務    {#uusing-web-browsers-to-access-services-running-on-the-cluster}

你或許能夠將 API 伺服器代理的 URL 放入瀏覽器的地址欄，然而：
...
- Web 伺服器通常不能傳遞令牌，所以你可能需要使用基本（密碼）認證。
  API 伺服器可以設定爲接受基本認證，但你的叢集可能並沒有這樣設定。
