...
-->
在配置文件中，你可以看到 Pod 有一個共享卷，名爲 `shared-data`。

配置文件中的第一個容器運行了一個 nginx 服務器。共享卷的掛載路徑是 `/usr/share/nginx/html`。
第二個容器是基於 debian 鏡像的，有一個 `/pod-data` 的掛載路徑。第二個容器運行了下面的命令然後終止。
...
-->
回憶一下，debian 容器在 nginx 的根目錄下創建了 `index.html` 文件。
使用 `curl` 向 nginx 服務器發送一個 GET 請求：
...
輔助應用的典型例子是數據抽取，數據推送和代理。輔助應用和主應用經常需要相互通信。
就如這個練習所示，通信通常是通過共享文件系統完成的，或者，也通過迴環網絡接口 localhost 完成。
舉個網絡接口的例子，web 服務器帶有一個協助程序用於拉取 Git 倉庫的更新。
...
* 參見[配置 Pod 使用捲來存儲數據](/zh-cn/docs/tasks/configure-pod-container/configure-volume-storage/)
