...
<!--
在 https://swimlanes.io 生成的泳道，其源碼如下：
...
note:
7.聚合的 apiserver 通過 SubjectAccessReview 請求 kube-apiserver 鑑權
...
-->
### Kubernetes Apiserver 認證和授權   {#kubernetes-apiserver-authentication-and-authorization}

由擴展 apiserver 服務的對 API 路徑的請求以與所有 API 請求相同的方式開始：
...
Kubernetes apiserver 現在準備將請求發送到擴展 apiserver。
...
-->
### Kubernetes Apiserver 代理請求   {#kubernetes-apiserver-proxies-the-request}

Kubernetes apiserver 現在將請求發送或代理到註冊以處理該請求的擴展 apiserver。
爲此，它需要了解幾件事：
...
2. 連接必須使用客戶端證書，該客戶端證書的 CN 是 `--requestheader-allowed-names` 中列出的證書之一。
...
-->
你可以將此選項設置爲空白，即爲`--requestheader-allowed-names=""`。
這將向擴展 apiserver 指示**任何** CN 都是可接受的。
{{< /note >}}
...
1. 使用它們向擴展 apiserver 認證。
2. 在 `kube-system` 命名空間中創建一個名爲
   `extension-apiserver-authentication` 的 ConfigMap，
   它將在其中放置 CA 證書和允許的 CN。
   反過來，擴展 apiserver 可以檢索這些內容以驗證請求。

請注意，Kubernetes apiserver 使用相同的客戶端證書對所有擴展 apiserver 認證。
它不會爲每個擴展 apiserver 創建一個客戶端證書，而是創建一個證書作爲
Kubernetes apiserver 認證。所有擴展 apiserver 請求都重複使用相同的請求。
...
-->
### 擴展 Apiserver 認證請求    {#extension-apiserver-authenticates-the-request}

擴展 apiserver 在收到來自 Kubernetes apiserver 的代理請求後，
必須驗證該請求確實確實來自有效的身份驗證代理，
該認證代理由 Kubernetes apiserver 履行。擴展 apiserver 通過以下方式對其認證：
...
-->
如果以上均通過，則該請求是來自合法認證代理（在本例中爲 Kubernetes apiserver）
的有效代理請求。
...
-->
### 擴展 Apiserver 對請求鑑權   {#extensions-apiserver-authorizes-the-request}

擴展 apiserver 現在可以驗證從標頭檢索的`user/group`是否有權執行給定請求。
通過向 Kubernetes apiserver 發送標準
...
-->
### 擴展 Apiserver 執行   {#enable-kubernetes-apiserver-flags}

如果 `SubjectAccessReview` 通過，則擴展 apiserver 執行請求。

## 啓用 Kubernetes Apiserver 標誌

通過以下 `kube-apiserver` 標誌啓用聚合層。
你的服務提供商可能已經爲你完成了這些工作：
...
但是，如果兩者都使用同一個 CA，則通常會通過 `--client-ca-file`
...
spec:
  ...
  groupPriorityMinimum: <APIService 對應組的優先級, 參考 API 文檔>
  versionPriority: <版本在組中的優先排序, 參考 API 文檔>
  service:
...
-->
APIService
對象的名稱必須是合法的[路徑片段名稱](/zh-cn/docs/concepts/overview/working-with-objects/names#path-segment-names)。
...
一旦 Kubernetes apiserver 確定應將請求發送到擴展 apiserver，
它需要知道如何調用它。
