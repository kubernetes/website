---
title: 使用 CustomResourceDefinition 擴展 Kubernetes API
content_type: task
...
本頁展示如何使用
[CustomResourceDefinition](/docs/reference/generated/kubernetes-api/{{< param "version" >}}/#customresourcedefinition-v1-apiextensions-k8s-io)
將[定製資源（Custom Resource）](/zh-cn/docs/concepts/extend-kubernetes/api-extension/custom-resources/)
...
spec:
  ...
  names:
    ...
    singular: crontab
    # kind 通常是單數形式的駝峯命名（CamelCased）形式。你的資源清單會使用這一形式。
    kind: CronTab
...
resource to show up.
-->
此端點 URL 自此可以用來創建和管理定製對象。對象的 `kind`
...
如果你將下面的 YAML 保存到 `my-crontab.yaml`：
...
-->
使用 kubectl 時，資源名稱是大小寫不敏感的，而且你既可以使用 CRD
中所定義的單數形式或複數形式，也可以使用其短名稱：
...
-->
如果你在以後創建相同的 CustomResourceDefinition 時，該 CRD 會是一個空的結構。
...
1. 爲對象根（root）設置一個非空的 type 值（藉由 OpenAPI 中的 `type`），對每個
   object 節點的每個字段（藉由 OpenAPI 中的 `properties` 或 `additionalProperties`）以及
   array 節點的每個條目（藉由 OpenAPI 中的 `items`）也要設置非空的 type 值，
   除非：
   * 節點包含屬性 `x-kubernetes-int-or-string: true`
   * 節點包含屬性 `x-kubernetes-preserve-unknown-fields: true`
2. 對於 object 的每個字段或 array 中的每個條目，如果其定義中包含 `allOf`、`anyOf`、`oneOf`
   或 `not`，則模式也要指定這些邏輯組合之外的字段或條目（試比較例 1 和例 2）。
3. 在 `allOf`、`anyOf`、`oneOf` 或 `not` 上下文內不設置 `description`、`type`、`default`、
   `additionalProperties` 或者 `nullable`。此規則的例外是
   `x-kubernetes-int-or-string` 的兩種模式（見下文）。
4. 如果 `metadata` 被設置，則只允許對 `metadata.name` 和 `metadata.generateName` 設置約束。
...
可能沒有結構化的模式定義，因此其 `spec.preserveUnknownFields` 可能爲 `true`。

對於使用 `apiextensions.k8s.io/v1beta1` 且將 `spec.preserveUnknownFields` 設置爲 `true`
...
-->
爲了與 `apiextensions.k8s.io/v1` 兼容，將你的定製資源定義更新爲：

1. 使用結構化的 OpenAPI 模式。
2. `spec.preserveUnknownFields` 設置爲 `false`。
...
-->
注意其中的字段 `someRandomField` 已經被剪裁掉。
...
-->
通過這樣設置，JSON 中只能設置 `object` 類型的值。

對於所指定的每個屬性（或 `additionalProperties`），剪裁會再次被啓用。
...
-->
這意味着所指定的 `spec` 對象中的 `something` 字段被剪裁掉，而其外部的內容都被保留。
...
-->
在以上兩種規約中，整數值和字符串值都會被認爲是合法的。

在[合法性檢查模式定義的發佈時](#publish-validation-schema-in-openapi)，
`x-kubernetes-int-or-string: true` 會被展開爲上述兩種模式之一。
...
項目中 `runtime.RawExtension` 所定義的那樣）
可以保存完整的 Kubernetes 對象，也就是，其中會包含 `apiVersion` 和 `kind`
字段。

通過 `x-kubernetes-embedded-resource: true` 來設定這些嵌套對象的規約
（無論是完全無限制還是部分指定都可以）是可能的。例如：
...
-->
由於字段上設置了 `x-kubernetes-preserve-unknown-fields: true`，
其中的內容不會被剪裁。不過，在這個語境中，`x-kubernetes-preserve-unknown-fields: true`
的使用是可選的。

設置了 `x-kubernetes-embedded-resource: true` 之後，`apiVersion`、`kind` 和
...
-->
**Finalizer** 能夠讓控制器實現異步的刪除前（Pre-delete）回調。
與內置對象類似，定製對象也支持 Finalizer。

你可以像下面一樣爲定製對象添加 Finalizer：
...
-->
自定義 Finalizer 的標識符包含一個域名、一個正向斜線和 finalizer 的名稱。
任何控制器都可以在任何對象的 finalizer 列表中添加新的 finalizer。

對帶有 Finalizer 的對象的第一個刪除請求會爲其 `metadata.deletionTimestamp`
設置一個值，但不會真的刪除對象。一旦此值被設置，`finalizers` 列表中的表項只能被移除。
在列表中仍然包含 finalizer 時，無法強制刪除對應的對象。

當 `metadata.deletionTimestamp` 字段被設置時，監視該對象的各個控制器會執行它們所能處理的
finalizer，並在完成處理之後將其從列表中移除。
...
一旦 finalizers 列表爲空時，就意味着所有 finalizer 都被執行過，
Kubernetes 會最終刪除該資源，
...
定製資源是通過
[OpenAPI v3.0 模式定義](https://github.com/OAI/OpenAPI-Specification/blob/3.0.0/versions/3.0.0.md#schema-object)，
來執行合法性檢查的，當啓用[驗證規則特性](#validation-rules)時，通過 `x-kubernetes-validations` 驗證，
你可以通過使用[准入控制 Webhook](/zh-cn/docs/reference/access-authn-authz/admission-controllers/#validatingadmissionwebhook)
來添加額外的合法性檢查邏輯。
...
-->
- 字段 `uniqueItems` 不可設置爲 `true`
- 字段 `additionalProperties` 不可設置爲 `false`
- 字段 `additionalProperties` 與 `properties` 互斥，不可同時使用
...
-->
當[驗證規則特性](#validation-rules)被啓用並且 CustomResourceDefinition
模式是一個[結構化的模式定義](#specifying-a-structural-schema)時，
`x-kubernetes-validations`
擴展可以使用[通用表達式語言（CEL）](https://github.com/google/cel-spec)表達式來驗證定製資源。
...
-->
關於對某些 CustomResourceDefinition 特性所必需的限制，
可參見[結構化的模式定義](#specifying-a-structural-schema)小節。
...
-->
模式定義是在 CustomResourceDefinition 中設置的。在下面的例子中，
CustomResourceDefinition 對定製對象執行以下合法性檢查：
...
If you save the following YAML to `my-crontab.yaml`:
-->
對於一個創建 CronTab 類別對象的定製對象的請求而言，如果其字段中包含非法值，則該請求會被拒絕。
在下面的例子中，定製對象中包含帶非法值的字段：
...
-->
儘管大多數放在 CRD 的 OpenAPIV3 模式定義中的驗證都支持逐步升級，仍存在一些例外。
Kubernetes {{< skew currentVersion >}} 下實現的驗證逐步升級不支持下面所列舉的 OpenAPIV3 模式檢查，
如果檢查時發現違例，會和以往一樣拋出錯誤：
...
- `x-kubernetes-validations`
  ...
  在 Kubernetes 1.28 中，CRD [驗證規則](#validation-rules)被逐步升級所忽略。
  從 Kubernetes 1.29 中 Alpha 2 開始，`x-kubernetes-validations` 僅在不引用 `oldSelf` 時纔會進行調整。

  轉換規則（Transition Rules）永遠不會被逐步升級機制處理：只有那些不使用
  `oldSelf` 的規則引發的錯誤會在其值未更改時自動按逐步升級機制處理。

  要爲 CEL 表達式編寫自定義棘輪邏輯，請查看 [optionalOldSelf](#field-optional-oldself)。
...
- `x-kubernetes-list-type`
  ...
  更改子模式的列表類型引發的錯誤不會被逐步升級機制處理。
  例如，在具有重複項的列表上添加 `set` 一定會出錯。

- `x-kubernetes-list-map-keys`

  由於更改列表模式定義的映射鍵而引起的錯誤將不會被逐步升級機制處理。
...
- `required`
  ...
  由於更改必需字段列表而引起的錯誤將不會被逐步升級處理。

- `properties`

  添加、移除、修改屬性的名稱不會被逐步升級處理，但如果屬性名稱保持不變，
  如果更改各屬性的模式定義和子模式定義中的合法性檢查規則，可能會被逐步升級機制處理。
...
- `additionalProperties`
  ...
  移除先前指定的 `additionalProperties` 合法性檢查時，不會被逐步升級機制處理。
...
-->
驗證規則使用[通用表達式語言（CEL）](https://github.com/google/cel-spec)來驗證定製資源的值。
驗證規則使用 `x-kubernetes-validations` 擴展包含在 `CustomResourceDefinition` 模式定義中。

<!--
The Rule is scoped to the location of the `x-kubernetes-validations` extension in the schema.
And `self` variable in the CEL expression is bound to the scoped value.
-->
規則的作用域是模式定義中 `x-kubernetes-validations` 擴展所在的位置。
CEL 表達式中的 `self` 變量被綁定到限定作用域的取值。

<!--
All validation rules are scoped to the current object: no cross-object or stateful validation
rules are supported.
-->
所有驗證規則都是針對當前對象的：不支持跨對象或有狀態的驗證規則。
...
-->
你可以在 [CEL Playground](https://playcel.undistro.io) 中快速測試這些 CEL 表達式。
{{< /note >}}
...
-->
當 CRD 被創建/更新時，驗證規則被編譯。
如果驗證規則的編譯失敗，CRD 的創建/更新請求將失敗。
編譯過程也包括類型檢查。
...
- `no_matching_overload`：此函數沒有參數類型的重載。

  例如，像 `self == true` 這樣的規則對一個整數類型的字段將得到錯誤：
...
- `no_such_field`：不包含所需的字段。
  ...
  例如，針對一個不存在的字段，像 `self.nonExistingField > 0` 這樣的規則將返回錯誤：
...
| 規則 | 目的 |
| --- | --- |
| `self.minReplicas <= self.replicas && self.replicas <= self.maxReplicas` | 驗證定義副本數的三個字段大小順序是否正確 |
| `'Available' in self.stateCounts` | 驗證映射中是否存在鍵名爲 `Available`的條目 |
| `(size(self.list1) == 0) != (size(self.list2) == 0)` | 檢查兩個列表之一是非空的，但不是二者都非空 |
| <code>!('MY_KEY' in self.map1) &#124;&#124; self['MY_KEY'].matches('^[a-zA-Z]*$')</code> | 如果某個特定的鍵在映射中，驗證映射中對應鍵的取值 |
| `self.envars.filter(e, e.name = 'MY_ENV').all(e, e.value.matches('^[a-zA-Z]*$')` | 驗證一個 listMap 中主鍵 'name' 爲 'MY_ENV' 的表項的取值 |
| `has(self.expired) && self.created + self.ttl < self.expired` | 驗證 'Expired' 日期是否晚於 'Create' 日期加上 'ttl' 時長 |
| `self.health.startsWith('ok')` | 驗證 'health' 字符串字段有前綴 'ok' |
| `self.widgets.exists(w, w.key == 'x' && w.foo < 10)` | 驗證鍵爲 'x' 的 listMap 項的 'foo' 屬性是否小於 10 |
| `type(self) == string ? self == '100%' : self == 1000` | 在 int 型和 string 型兩種情況下驗證 int-or-string 字段 |
| `self.metadata.name.startsWith(self.prefix)` | 驗證對象的名稱是否以另一個字段值爲前綴 |
| `self.set1.all(e, !(e in self.set2))` | 驗證兩個 listSet 是否不相交 |
| `size(self.names) == size(self.details) && self.names.all(n, n in self.details)` | 驗證 'details' 映射中的 'names' 來自於 listSet |
| `size(self.clusters.filter(c, c.name == self.primary)) == 1` | 驗證 'primary' 屬性在 'clusters' listMap 中出現一次且只有一次 |

參考：[CEL 中支持的求值](https://github.com/google/cel-spec/blob/v0.6.0/doc/langdef.md#evaluation)
...
- If the Rule is scoped to the root of a resource, it may make field selection into any fields
  ...
  same expression:
-->
- 如果規則的作用域是某資源的根，則它可以對 CRD 的 OpenAPIv3 模式表達式中聲明的任何字段進行字段選擇，
  以及 `apiVersion`、`kind`、`metadata.name` 和 `metadata.generateName`。
  這包括在同一表達式中對 `spec` 和 `status` 的字段進行選擇：
...
-->
- 如果規則的作用域是具有屬性的對象，那麼可以通過 `self.field` 對該對象的可訪問屬性進行字段選擇，
  而字段存在與否可以通過 `has(self.field)` 來檢查。
  在 CEL 表達式中，Null 值的字段被視爲不存在的字段。
...
-->
- 如果規則的作用域是一個帶有 additionalProperties 的對象（即 map），那麼 map 的值
  可以通過 `self[mapKey]` 訪問，map 的包含性可以通過 `mapKey in self` 檢查，
  map 中的所有條目可以通過 CEL 宏和函數如 `self.all(...)` 訪問。
...
-->
- 如果規則的作用域是 array，則 array 的元素可以通過 `self[i]` 訪問，也可以通過宏和函數訪問。
...
| 規則作用域字段類型     | 規則示例             |
| -----------------------| -----------------------|
| 根對象            | `self.status.actual <= self.spec.maxDesired`|
| 對象映射         | `self.components['Widget'].priority < 10`|
...
- 屬性模式爲"未知類型（Unknown Type）"的對象屬性。一個"未知類型"被遞歸定義爲：

  - 一個沒有類型的模式，`x-kubernetes-preserve-unknown-fields` 設置爲 true。
  - 一個數組，其中項目模式爲"未知類型"
  - 一個 additionalProperties 模式爲"未知類型"的對象
...
-->
`set` 或 `map` 的 `x-Kubernetes-list-type` 的數組的等值比較會忽略元素順序，即 `[1，2] == [2，1]`。
使用 `x-kubernetes-list-type` 對數組進行串聯時，使用 List 類型的語義：

- `set`：`X + Y` 執行一個並集操作，其中 `X` 中所有元素的數組位置被保留，
  `Y` 中不相交的元素被追加，保留其部分順序。

- `map`：`X + Y`執行合併，其中 `X` 中所有鍵的數組位置被保留，
...
-->
以下是 OpenAPIV3 和 CEL 類型之間的聲明類型映射：
...
| OpenAPIv3 類型 | CEL 類型 |
| ------------- | ------- |
| 帶有 Properties 的對象 | 對象 / "消息類型" |
| 帶有 AdditionalProperties 的對象 | map |
| 帶有 x-kubernetes-embedded-type 的對象 | 對象 / "消息類型"，'apiVersion'、'kind'、'metadata.name' 和 'metadata.generateName' 都隱式包含在模式中 |
| 帶有 x-kubernetes-preserve-unknown-fields 的對象 | 對象 / "消息類型"，未知字段無法從 CEL 表達式中訪問 |
| x-kubernetes-int-or-string | 可能是整數或字符串的動態對象，可以用 `type(value)` 來檢查類型 |
| 數組 | list |
| 帶有 x-kubernetes-list-type=map 的數組 | 列表，基於集合等值和唯一鍵名保證的 map 組成 |
| 帶有 x-kubernetes-list-type=set 的數組 | 列表，基於集合等值和唯一鍵名保證的 set 組成 |
| 布爾值 | boolean |
| 數字 (各種格式) | double |
| 整數 (各種格式) | int (64) |
| 'null' | null_type |
| 字符串 | string |
| 帶有 format=byte （base64 編碼）字符串 | bytes |
| 帶有 format=date 字符串 | timestamp (google.protobuf.Timestamp) |
| 帶有 format=datetime 字符串 | timestamp (google.protobuf.Timestamp) |
| 帶有 format=duration 字符串 | duration (google.protobuf.Duration) |
...
-->
參考：[CEL 類型](https://github.com/google/cel-spec/blob/v0.6.0/doc/langdef.md#values)、
[OpenAPI 類型](https://swagger.io/specification/#data-types)、
...
-->
請記住，CEL 字符串連接（`+` 運算符）不會自動轉換爲字符串。
如果你有一個非字符串標量，請使用 `string(<value>)` 函數將標量轉換爲字符串，如上例所示。
...
breaks.
-->
`messageExpression` 必須計算爲一個字符串，並且在編寫 CRD 時進行檢查。
請注意，可以在同一個規則上設置 `message` 和 `messageExpression`，如果兩者都存在，則將使用 `messageExpression`。
但是，如果 `messageExpression` 計算出錯，則將使用 `message` 中定義的字符串，而 `messageExpression` 的錯誤將被打印到日誌。
如果在 `messageExpression` 中定義的 CEL 表達式產生一個空字符串或包含換行符的字符串，也會發生這種回退。
...
-->
`messageExpression` 是一個 CEL 表達式，
因此[驗證函數的資源使用](#resource-use-by-validation-functions)中所列出的限制也適用於它。
如果在 `messageExpression` 執行期間由於資源限制而導致計算停止，則不會繼續處理其他合法性檢查規則。

`messageExpression` 設置是可選的。
...
如果你要設置一個靜態消息，可以提供 `message` 而不是 `messageExpression`。
如果合法性檢查失敗，則 `message` 的值將被用作不透明的錯誤字符串。
...
你可以在 `validation` 中添加一個機器可讀的驗證失敗原因，以便在請求未通過此驗證規則時返回。
...
-->
`fieldPath` 值必須是相對 JSON 路徑，且限定爲此 `x-kubernetes-validations` 擴展在模式定義中的位置。
此外，它應該指向模式定義中的一個現有字段。例如，當驗證檢查 `testMap` 映射下的特定屬性 `foo` 時，
...
通常，在對象創建期間或在更新中引入新值時，如果無法確定 `oldSelf`，則不會處理轉換規則。
...
-->
如果 `optionalOldSelf` 設置爲 true，則一定會處理轉換規則，並且 `oldSelf` 的類型會被更改爲
CEL [`Optional`](https://pkg.go.dev/github.com/google/cel-go/cel#OptionalTypes) 類型。
...
| CEL                                     | 描述 |
|-----------------------------------------|------|
| <code>self.foo == "foo" &#124;&#124; (oldSelf.hasValue() && oldSelf.value().foo != "foo")</code> | 逐步升級規則。一旦將值設置爲 "foo"，它必須保持爲 foo。但如果在引入 "foo" 約束之前它已存在，則可以使用所有值 |
| <code>[oldSelf.orValue(""), self].all(x, ["OldCase1", "OldCase2"].exists(case, x == case)) &#124;&#124; ["NewCase1", "NewCase2"].exists(case, self == case) &#124;&#124; ["NewCase"].has(self)</code> | "如果 oldSelf 使用了已移除的枚舉值，則逐步升級驗證" |
| <code>oldSelf.optMap(o, o.size()).orValue(0) < 4 &#124;&#124; self.size() >= 4</code> 對新增的最小映射或列表大小進行逐步升級驗證 |

<!--
#### Validation functions {#available-validation-functions}
-->
#### 驗證函數   {#available-validation-functions}
...
- CEL 標準函數，在[標準定義列表](https://github.com/google/cel-spec/blob/v0.7.0/doc/langdef.md#list-of-standard-definitions)中定義
- CEL 標準[宏](https://github.com/google/cel-spec/blob/v0.7.0/doc/langdef.md#macros)
- CEL [擴展字符串函數庫](https://pkg.go.dev/github.com/google/cel-go@v0.11.2/ext#Strings)
- Kubernetes [CEL 擴展庫](https://pkg.go.dev/k8s.io/apiextensions-apiserver@v0.24.0/pkg/apiserver/schema/cel/library#pkg-functions)
...
-->
包含引用標識符 `oldSself` 的表達式的規則被隱式視爲 **轉換規則（Transition Rule）**。
...
-->
- 舊的值和新的值都存在。仍然可以通過在父節點上放置轉換規則來檢查值是否已被添加或移除。
  轉換規則從不應用於定製資源創建。當被放置在可選字段上時，轉換規則將不適用於設置或取消設置該字段的更新操作。
...
-->
- 被轉換規則驗證的模式節點的路徑必須解析到一個在舊對象和新對象之間具有可比性的節點。
  例如，列表項和它們的後代（`spec.foo[10].bar`）不一定能在現有對象和後來對同一對象的更新之間產生關聯。
...
如果所有 `array` 父模式都是 `x-kubernetes-list-type=map`類型的，那麼該模式的一部分就是可關聯的；
任何 `set` 或者 `atomic` 數組父模式都不支持確定性地將 `self` 與 `oldSelf` 關聯起來。
...
| 如果之前的值爲 X，則新值只能爲 A 或 B，不能爲 Y 或 Z | `oldSelf != 'X' \|\| self in ['A', 'B']`
...
-->
#### 驗證函數的資源使用  {#resource-use-by-validation-functions}
...
-->
運行時也使用類似的系統來觀察解釋器的行動。如果解釋器執行了太多的指令，規則的執行將被停止，並且會產生一個錯誤。
<!--
...
如果在創建時估計其規則的總和超過了這個限制，那麼也會發生驗證錯誤。
...
-->
例如，一個斷言 `self.foo == 1` 的規則本身不存在因爲資源預算組驗證而導致被拒絕的風險。
<!--
...
-->
但是，如果 `foo` 是一個字符串，而你定義了一個驗證規則 `self.foo.contains("someString")`，
...
-->
因此，通過 `maxItems`，`maxProperties` 和 `maxLength` 進行限制被認爲是最佳實踐，
...
-->
這個拒絕會發生是因爲 `self.all` 意味着對 `foo` 中的每一個字符串調用 `contains()`，
而這又會檢查給定的字符串是否包含 `'a string'`。如果沒有限制，這是一個非常昂貴的規則。
...
-->
如果在一個列表內部的一個列表有一個使用 `self.all` 的驗證規則，那就會比具有相同規則的非嵌套列表的成本高得多。
一個在非嵌套列表中被允許的規則可能需要在兩個嵌套列表中設置較低的限制才能被允許。
例如，即使沒有設置限制，下面的規則也是允許的：
...
-->
但是同樣的規則在下面的模式中（添加了一個嵌套數組）產生了一個驗證錯誤：
...
-->
這是因爲 `foo` 的每一項本身就是一個數組，而每一個子數組依次調用 `self.all`。
在使用驗證規則的地方，儘可能避免嵌套的列表和字典。
...
-->
像下面這樣創建一個爲 `foo`、`bar` 和 `baz` 設置空值的對象時：
...
-->
### 以 OpenAPI 形式發佈合法性檢查模式  {#publish-validation-schema-in-openapi}
...
爲了與 OpenAPI V2 兼容，OpenAPI v3 驗證模式會對 OpenAPI v2 模式進行有損轉換。
該模式顯示在 [OpenAPI v2 規範](/zh-cn/docs/concepts/overview/kubernetes-api/#openapi-and-swagger-definitions)中的
`definitions` 和` paths` 字段中。
OpenAPI v3 合法性檢查模式定義會被轉換爲 OpenAPI v2 模式定義，並出現在
[OpenAPI v2 規範](/zh-cn/docs/concepts/overview/kubernetes-api/#openapi-and-swagger-definitions)的
`definitions` 和 `paths` 字段中。
...
-->
1. 以下字段會被移除，因爲它們在 OpenAPI v2 中不支持。

   - 字段 `allOf`、`anyOf`、`oneOf` 和 `not` 會被刪除

2. 如果設置了 `nullable: true`，我們會丟棄 `type`、`nullable`、`items` 和 `properties`
   OpenAPI v2 無法表達 Nullable。爲了避免 kubectl 拒絕正常的對象，這一轉換是必要的。
...
每個列都包含一個 `priority`（優先級）字段。當前，優先級用來區分標準視圖（Standard
View）和寬視圖（Wide View）（使用 `-o wide` 標誌）中顯示的列：
...
types are correct.
-->
如果 CustomResource 中的值與列中指定的類型不匹配，該值會被忽略。
你可以通過 CustomResource 的合法性檢查來確保取值類型是正確的。
...
-->
列的 `format` 字段控制 `kubectl` 打印對應取值時採用的風格。
...
selectors.
-->
### 字段選擇算符  {#field-selectors}

[字段選擇算符](/zh-cn/docs/concepts/overview/working-with-objects/field-selectors/)允許客戶端根據一個或多個資源字段的值選擇自定義資源。

所有自定義資源都支持 `metadata.name` 和 `metadata.namespace` 字段選擇器。
...
的 `spec.versions[*].selectableFields` 字段中時，也可以與字段選擇器一起使用。

<!--
#### Selectable fields for custom resources {#crd-selectable-fields}
-->
#### 自定義資源的可選字段    {#crd-selectable-fields}
...
-->
通過編輯 `shirt-resources.yaml` 定義一些 Shirt，例如：
...
-->
獲取藍色 shirt（檢索 `color` 爲 `blue` shirt）：
...
-->
僅獲取 `color` 爲 `green`、`size` 爲 `M` 的資源：
...
定製資源支持 `/status` 和 `/scale` 子資源。

通過在 CustomResourceDefinition 中定義 `status` 和 `scale`，
可以有選擇地啓用這些子資源。
...
當啓用了 status 子資源時，對應定製資源的 `/status` 子資源會被暴露出來。

- status 和 spec 內容分別用定製資源內的 `.status` 和 `.spec` JSON 路徑來表達；
- 對 `/status` 子資源的 `PUT` 請求要求使用定製資源對象作爲其輸入，但會忽略
  status 之外的所有內容。
- 對 `/status` 子資源的 `PUT` 請求僅對定製資源的 status 內容進行合法性檢查。
- 對定製資源的 `PUT`、`POST`、`PATCH` 請求會忽略 status 內容的改變。
- 對所有變更請求，除非改變是針對 `.metadata` 或 `.status` 的，`.metadata.generation`
  的取值都會增加。
...
當啓用了 scale 子資源時，定製資源的 `/scale` 子資源就被暴露出來。
針對 `/scale` 所發送的對象是 `autoscaling/v1.Scale`。

爲了啓用 scale 子資源，CustomResourceDefinition 定義了以下字段：
...
And create it:
-->
之後創建此 CustomResourceDefinition：
...
-->
那麼會創建新的、命名空間作用域的 RESTful  API 端點：
...
例如下面的命令將前面創建的定製資源的 `.spec.replicas` 設置爲 5：
...
Save the following YAML to `my-crontab.yaml`:
-->
創建了 CustomResourceDefinition 對象之後，你可以創建定製對象。

將下面的 YAML 保存到 `my-crontab.yaml` 中：
...
* 參閱支持 CustomResourceDefinition 的[多個版本](/zh-cn/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definition-versioning/)
