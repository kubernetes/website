...
title: 重新配置 Kubernetes 默認的 ServiceCIDR
content_type: task
---
<!--
reviewers:
...
to a cluster.
-->
本文介紹如何重新配置集羣中分配的默認 Service IP 範圍。
...
for Services.
-->
## 重新配置 Kubernetes 默認 ServiceCIDR   {#kubernetes-default-service-cidr-reconfiguration}

本文解釋瞭如何管理 Kubernetes 集羣中的 Service IP 地址範圍，這也會影響集羣針對不同 Service 所支持的 IP 協議族。
...
-->
自 Kubernetes 1.33 起，爲集羣所配置的 Service IP 協議族會通過名爲 `kubernetes` 的 ServiceCIDR 對象反映。
Kubernetes 的 ServiceCIDR 由第一個啓動的 kube-apiserver 實例根據其 `--service-cluster-ip-range`
參數配置創建。爲了確保集羣行爲一致，所有 kube-apiserver 實例必須使用相同的
`--service-cluster-ip-range` 配置，其取值需與默認的 kubernetes ServiceCIDR 對象保持一致。
...
-->
### Kubernetes ServiceCIDR 重新配置類別   {#kubernetes-service-cidr-reconfiguration-categories}

我們可以將 ServiceCIDR 的重新配置分爲以下幾種情形：
...
-->
* **擴展現有的 ServiceCIDR：**
  這可以通過添加新的 ServiceCIDR 對象動態完成，無需重新配置 kube-apiserver。
...
* **保留主 ServiceCIDR 的單棧到雙棧轉換：**
  這意味着引入次要 IP 協議族（IPv6 到僅 IPv4 集羣，或 IPv4 到僅 IPv6 集羣），
  同時保留原 IP 協議族作爲主協議族。
  這需要更新 kube-apiserver 配置，並相應修改需要處理這個附加 IP 協議族的各個集羣組件。
  這些組件包括但不限於 kube-proxy、CNI 或網絡插件、服務網格實現和 DNS 服務。
...
* **保留主 ServiceCIDR 的雙棧到單棧轉換：**
  這意味着從雙棧集羣中移除次要 IP 協議族，恢復爲單一 IP 協議族，同時保留原主 IP 協議族。
  除了重新配置這些組件以匹配新的 IP 協議族外，你還可能需要處理那些顯式使用被移除
  IP 協議族的 Service。
...
* **變更主 ServiceCIDR 的任何情形：**
  ...
  如果主 IP 協議族也發生變化，則更爲複雜，可能需要修改多個集羣組件（如 kubelet、網絡插件等）
  以匹配新的主 IP 協議族。
...
重新配置默認 ServiceCIDR 需要集羣運維人員、管理員或管理集羣生命週期的軟件執行一系列手動步驟。
這些通常包括：
...
-->
2. **重新配置**網絡組件：這一步至關重要，具體步驟取決於正在使用的聯網組件。
   這可能包括更新配置文件、重啓代理 Pod，或更新組件以處理新的 ServiceCIDR 和期望的 Pod IP 協議族配置。
   典型組件可以是 Kubernetes Service（例如 kube-proxy）的實現、
   所配置的網絡插件以及服務網格控制器和 DNS 服務器等潛在的其他聯網組件，
   以確保它們能夠正確處理流量並使用新的 IP 協議族配置來執行服務發現。
<!--
...
-->
3. **管理現有 Service：**
   如果某些 Service 所使用的 IP 不在新配置的範圍內，則需處理這些服務。
   你可以選擇重新創建（會造成停機並重新分配 IP），或者採取更復雜的重新配置策略。
4. **重新創建 Kubernetes 內部服務：**
   如果主 IP 協議族發生變化，或者被替換爲另一個不同的網絡，
...
-->
### 示例重新配置步驟   {#illustrative-reconfiguration-steps}

以下步驟描述了受控的重新配置過程，重點是完全替換默認 ServiceCIDR 並重新創建 `kubernetes.default` Service：
...
3. 引入新的 ServiceCIDR，作爲重新配置的臨時目標。
...
6. 使用配置的新 ServiceCIDR 重新啓動 kube-apiserver，並關閉舊實例。
