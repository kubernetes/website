...
-->
本頁展示瞭如何配置密鑰管理服務（Key Management Service，KMS）驅動和插件以啓用 Secret 數據加密。
...
如果你使用的控制平面組件早於 Kubernetes v1.29，請查看集羣所運行的 Kubernetes 版本文檔中的相應頁面。
...
- 如果你選擇了 KMS API v1 來支持早於 v1.27 版本的集羣，或者你有一個僅支持 KMS v1 的舊版 KMS 插件，
...
* 對於 1.29 及更高版本，KMS 的 v1 實現默認處於禁用狀態。
  要啓用此特性，設置 `--feature-gates=KMSv1=true` 以配置 KMS v1 驅動。
* 你的集羣必須使用 etcd v3 或更高版本
...
API 服務器使用**密鑰派生函數**根據祕密的種子數結合一些隨機數據生成一次性的數據加密密鑰。
種子會在 KEK 輪換時進行輪換
...
這個 KMS 插件作爲一個 gRPC 服務器被部署在 Kubernetes 控制平面的相同主機上，負責與遠端 KMS 的通信。
...
-->
## 配置 KMS 驅動   {#configuring-the-kms-provider}

爲了在 API 服務器上配置 KMS 驅動，在加密配置文件中的 `providers` 數組中加入一個類型爲 `kms`
的驅動，並設置下列屬性：
...
* `endpoint`：gRPC 服務器（KMS 插件）的監聽地址。該端點是一個 UNIX 域套接字。
...
-->
KMS v2 不支持 `cachesize` 屬性。一旦服務器通過調用 KMS 解密了數據加密密鑰（DEK），
所有的 DEK 將會以明文形式被緩存。一旦被緩存，DEK 可以無限期地用於解密操作，而無需再次調用 KMS。

<!--
See [Understanding the encryption at rest configuration.](/docs/tasks/administer-cluster/encrypt-data)
-->
參見[理解靜態配置加密](/zh-cn/docs/tasks/administer-cluster/encrypt-data)。
...
爲實現一個 KMS 插件，你可以開發一個新的插件 gRPC 服務器或啓用一個由你的雲服務驅動提供的 KMS 插件。
...
-->
### 開發 KMS 插件 gRPC 服務器   {#developing-a-kms-plugin-grpc-server}

你可以使用 Go 語言的存根文件開發 KMS 插件 gRPC 服務器。
對於其他語言，你可以用 proto 文件創建可以用於開發 gRPC 服務器代碼的存根文件。
...
* 協議：UNIX 域套接字 (`unix`)
  ...
  該插件被實現爲一個在 UNIX 域套接字上偵聽的 gRPC 服務器。
  該插件部署時應在文件系統上創建一個文件來運行 gRPC UNIX 域套接字連接。
  API 服務器（gRPC 客戶端）配置了 KMS 驅動（gRPC 服務器）UNIX 域套接字端點，以便與其通信。
  通過以 `/@` 開頭的端點，可以使用一個抽象的 Linux 套接字，即 `unix:///@foo`。
  使用這種類型的套接字時必須小心，因爲它們沒有 ACL 的概念（與傳統的基於文件的套接字不同）。
  然而，這些套接字遵從 Linux 網絡命名空間約束，因此只能由同一 Pod 中的容器進行訪問，除非使用了主機網絡。
...
* KMS 插件版本：`v2`
  ...
  under constant load.
  -->
  當一切健康時，API 服務器大約每分鐘輪詢一次 `Status` 過程調用，
...
* 加密
  ...
  當 API 服務器運行正常時，並不會高頻執行 `EncryptRequest` 過程調用。
...
* 解密
  ...
  -->
  在啓動時，API 服務器可能會執行數千個 `DecryptRequest` 過程調用以填充其監視緩存。
...
* 理解 `key_id` 和密鑰輪換
  ...
  它可能會在 API 服務器的常規操作期間記錄，因此不得包含任何私有數據。
  建議插件實現使用哈希來避免泄漏任何數據。
  ...
  -->
  API 服務器認爲從 `Status` 過程調用返回的 `key_id` 是權威性的。因此，此值的更改表示遠程 KEK 已更改，
  ...
  -->
  由於 API 服務器大約每分鐘輪詢一次 `Status`，因此 `key_id` 輪換並不立即發生。
  此外，API 服務器在三分鐘內以最近一個有效狀態爲準。因此，
  如果用戶想採取被動方法進行存儲遷移（即等待），則必須安排遷移在遠程 KEK 輪換後的 `3 + N + M` 分鐘內發生
  （其中 `N` 表示插件觀察 `key_id` 更改所需的時間，`M` 是允許處理配置更改的緩衝區時間 - 建議至少使用 5 分鐘）。
  請注意，執行 KEK 輪換不需要進行 API 服務器重啓。
...
* 協議：UNIX 域套接字 (`unix`)
  ...
  通過以 `/@` 開頭的端點，可以使用一個抽象的 Linux 套接字，即 `unix:///@foo`。
  使用這種類型的套接字時必須小心，因爲它們沒有 ACL 的概念（與傳統的基於文件的套接字不同）。
  然而，這些套接字遵從 Linux 網絡命名空間，因此只能由同一 Pod 中的容器進行訪問，除非使用了主機網絡。
...
所有的配置數據，包括 KMS 插件用於與遠程 KMS 通信的認證憑據，都由 KMS 插件獨立地存儲和管理。
...
確保 KMS 插件與 Kubernetes API 服務器運行在同一主機上。
...
1. 使用適合於 `kms` 驅動的屬性創建一個新的 `EncryptionConfiguration` 文件，以加密
   ...
   如果要加密使用 CustomResourceDefinition 定義的擴展 API，你的集羣必須運行 Kubernetes v1.26 或更高版本。

2. 設置 kube-apiserver 的 `--encryption-provider-config` 參數指向配置文件的位置。
...
3. `--encryption-provider-config-automatic-reload` 布爾參數決定了磁盤內容發生變化時是否應自動
   ...
   通過 `--encryption-provider-config` 設置的文件。這樣可以在不重啓 API 服務器的情況下進行密鑰輪換。
...
只有 KMS v1 驅動正使用且加密配置未被自動重新加載時，才能進行獨立的健康檢查。
...
並且這一端點基於插件在加密配置中的位置確定，例如 `/healthz/kms-provider-0`、`/healthz/kms-provider-1` 等。

這些健康檢查端點路徑是由服務器硬編碼、生成並控制的。
`Individual Healthchecks` 的索引序號對應於 KMS 加密配置被處理的順序。
...
加密所有資源後，應移除 `identity` 提供程序，以防止 API 服務器接受未加密的數據。
...
當靜態加密被正確配置時，資源將在寫入時被加密。
重啓 `kube-apiserver` 後，所有新建或更新的 Secret 或在
`EncryptionConfiguration` 中配置的其他資源類型在存儲時應該已被加密。
要驗證這點，你可以用 `etcdctl` 命令行程序獲取私密數據的內容。
...
-->
2. 用 `etcdctl` 命令行，從 etcd 讀取出 Secret：
...
-->
下列命令讀取所有 Secret 並更新它們以便應用服務器端加密。如果因爲寫入衝突導致錯誤發生，
請重試此命令。對較大的集羣，你可能希望根據命名空間或腳本更新去細分 Secret 內容。
...
-->
1. 在配置文件中加入 `kms` 驅動作爲第一個條目，如下列樣例所示
