...
Kubernetes 的 `dockershim` 組件使得你可以把 Docker 用作 Kubernetes 的
{{< glossary_tooltip text="容器運行時" term_id="container-runtime" >}}。
在 Kubernetes v1.24 版本中，內建組件 `dockershim` 被移除。
...
-->
即使你是通過 Docker 創建的應用容器，也不妨礙你在其他任何容器運行時上運行這些容器。
這種使用 Docker 的方式並不構成對 Docker 作爲一個容器運行時的依賴。
...
-->
當用了別的容器運行時之後，Docker 命令可能不工作，或者產生意外的輸出。
...
-->
切換到 Containerd 容器運行時可以消除掉中間環節。
所有相同的容器都可由 Containerd 這類容器運行時來運行。
但是現在，由於直接用容器運行時調度容器，它們對 Docker 是不可見的。
...
-->
如果你在用 Kubernetes 運行工作負載，最好通過 Kubernetes API 停止容器，
而不是通過容器運行時來停止它們（此建議適用於所有容器運行時，不僅僅是針對 Docker）。
{{< /note >}}
...
-->
Kubelet `/metrics/cadvisor` 端點提供 Prometheus 指標，
如 [Kubernetes 系統組件指標](/zh-cn/docs/concepts/cluster-administration/system-metrics/) 中所述。
如果你安裝了一個依賴該端點的指標收集器，你可能會看到以下問題：
...
-->
1. 找到名稱格式爲 `vX.Y.Z-containerd-cri` 的最新
...
2. 按照 [cAdvisor Kubernetes Daemonset](https://github.com/google/cadvisor/tree/master/deploy/kubernetes)
   中的步驟來創建守護進程。
3. 將已安裝的指標收集器指向使用 cAdvisor 的 `/metrics` 端點。
   該端點提供了全套的 [Prometheus 容器指標](https://github.com/google/cadvisor/blob/master/docs/storage/prometheus.md)。
...
- 使用替代的第三方指標收集解決方案。
- 從 Kubelet 摘要 API 收集指標，該 API 在 `/stats/summary` 提供服務。
