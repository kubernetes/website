...
-->
在這個過程中，內存管理器會更新其內部存儲於[節點映射和內存映射][2]中的計數器，
從而管理有保障的內存分配。
...
這是默認的策略，並且不會以任何方式影響內存分配。該策略的行爲好像內存管理器不存在一樣。

`None` 策略返回默認的拓撲提示信息。這種特殊的提示會表明拓撲驅動組件（Hint Provider）
...
對 `Guaranteed` Pod 而言，`Static` 內存管理器策略會返回拓撲提示信息，
該信息與內存分配有保障的 NUMA 節點集合有關，並且內存管理器還通過更新內部的[節點映射][2]
對象來完成內存預留。

對 `BestEffort` 或 `Burstable` Pod 而言，因爲不存在對有保障的內存資源的請求，
`Static` 內存管理器策略會返回默認的拓撲提示，並且不會通過內部的[節點映射][2]對象來預留內存。
...
節點設置 `--reserved-memory`。實際上，`eviction-hard` 閾值默認爲 `100Mi`，
...
display an error.
-->
默認的硬性驅逐閾值是 100MiB，**不是**零。
請記得在使用 `--reserved-memory` 設置要預留的內存量時，加上這個硬性驅逐閾值。
否則 kubelet 不會啓動內存管理器，而會輸出一個錯誤信息。
{{< /note >}}
...
內存管理器爲每個 `Guaranteed` Pod 向拓撲管理器提供拓撲提示信息。
對於不在 `Guaranteed` QoS 類中的其他 Pod，內存管理器向拓撲管理器提供默認的拓撲提示信息。
...
-->
- Pod 狀態 - 可表明拓撲親和性錯誤
- 系統日誌 - 包含用來調試的有價值的信息，例如，關於所生成的提示信息
- 狀態文件 - 其中包含內存管理器內部狀態的轉儲（包含[節點映射和內存映射][2]）
...
-->
使用 `kubectl describe pod <id>` 或 `kubectl get events` 可以獲得詳細的錯誤信息。
...
內存管理器爲 Pod 所生成的提示信息可以在日誌中找到。
此外，日誌中應該也存在 CPU 管理器所生成的提示信息。
...
-->
拓撲管理器將這些提示信息進行合併，計算得到唯一的最合適的提示數據。
此最佳提示數據也應該出現在日誌中。

最佳提示表明要在哪裏分配所有的資源。拓撲管理器會用當前的策略來測試此數據，
...
`/var/lib/kubelet/memory_manager_state` 的狀態文件：
...
-->
從這個狀態文件，可以推斷 Pod 被同時綁定到兩個 NUMA 節點，即：
...
kubelet 提供了一個 `PodResourceLister` gRPC 服務來啓用對資源和相關元數據的檢測。
通過使用它的
[List gRPC 端點](/zh-cn/docs/concepts/extend-kubernetes/compute-storage-net/device-plugins/#grpc-endpoint-list)，
可以獲得每個容器的預留內存信息，該信息位於 protobuf 協議的 `ContainerMemory` 消息中。
只能針對 Guaranteed QoS 類中的 Pod 來檢索此信息。
