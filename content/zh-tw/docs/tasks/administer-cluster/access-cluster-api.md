...
-->
## 訪問 Kubernetes API   {#accessing-the-kubernetes-api}
...
-->
首次訪問 Kubernetes API 時，請使用 Kubernetes 命令行工具 `kubectl`。
...
-->
要訪問集羣，你需要知道集羣位置並擁有訪問它的憑證。
通常，當你完成[入門指南](/zh-cn/docs/setup/)時，這會自動設置完成，或者由其他人設置好集羣並將憑證和位置提供給你。
...
提供了使用 kubectl 的介紹。完整文檔請見 [kubectl 手冊](/zh-cn/docs/reference/kubectl/)。
...
kubectl 處理對 API 服務器的定位和身份驗證。如果你想通過 http 客戶端（如 `curl`、`wget`
或瀏覽器）直接訪問 REST API，你可以通過多種方式對 API 服務器進行定位和身份驗證：
...
1. 以代理模式運行 kubectl（推薦）。
   推薦使用此方法，因爲它用存儲的 apiserver 位置並使用自簽名證書驗證 API 服務器的標識。
   使用這種方法無法進行中間人（MITM）攻擊。
2. 另外，你可以直接爲 HTTP 客戶端提供位置和身份認證。
   這適用於被代理混淆的客戶端代碼。
   爲防止中間人攻擊，你需要將根證書導入瀏覽器。
...
下列命令使 kubectl 運行在反向代理模式下。它處理 API 服務器的定位和身份認證。
...
-->
然後你可以通過 curl，wget，或瀏覽器瀏覽 API，像這樣：
...
```shell
# 查看所有的集羣，因爲你的 .kubeconfig 文件中可能包含多個上下文
kubectl config view -o jsonpath='{"Cluster name\tServer\n"}{range .clusters[*]}{.name}{"\t"}{.cluster.server}{"\n"}{end}'

# 從上述命令輸出中選擇你要與之交互的集羣的名稱
export CLUSTER_NAME="some_server_name"

# 指向引用該集羣名稱的 API 服務器
APISERVER=$(kubectl config view -o jsonpath="{.clusters[?(@.name==\"$CLUSTER_NAME\")].cluster.server}")

# 創建一個 secret 來保存默認服務賬戶的令牌
kubectl apply -f - <<EOF
apiVersion: v1
kind: Secret
metadata:
...
當 kubectl 訪問集羣時，它使用存儲的根證書和客戶端證書訪問服務器。
（已安裝在 `~/.kube` 目錄下）。
由於集羣認證通常是自簽名的，因此可能需要特殊設置才能讓你的 http 客戶端使用根證書。
...
-->
在一些集羣中，API 服務器不需要身份認證；它運行在本地，或由防火牆保護着。
對此並沒有一個標準。
...
參考[客戶端庫](/zh-cn/docs/reference/using-api/client-libraries/)瞭解如何使用其他語言來訪問 API
以及如何執行身份認證。
...
-->
client-go 定義了自己的 API 對象，因此如果需要，從 client-go 而不是主倉庫導入
...
-->
要使用 [Python 客戶端](https://github.com/kubernetes-client/python)，運行下列命令：
...
-->
Python 客戶端可以使用與 kubectl 命令行工具相同的
[kubeconfig 文件](/zh-cn/docs/concepts/configuration/organize-cluster-access-kubeconfig/)
定位和驗證 API 服務器。參見這個
...
-->
要安裝 [Java 客戶端](https://github.com/kubernetes-client/java)，運行：
...
要使用 [.Net 客戶端](https://github.com/kubernetes-client/csharp)，運行下面的命令：
...
要安裝 [JavaScript 客戶端](https://github.com/kubernetes-client/javascript)，運行下面的命令：
...
JavaScript 客戶端可以使用 kubectl 命令行所使用的
[kubeconfig 文件](/zh-cn/docs/concepts/configuration/organize-cluster-access-kubeconfig/)
以定位 API 服務器並向其認證身份。
...
參考 [https://github.com/kubernetes-client/haskell/releases](https://github.com/kubernetes-client/haskell/releases)
瞭解支持的版本。
...
-->
* [從 Pod 中訪問 Kubernetes API](/zh-cn/docs/tasks/run-application/access-api-from-pod/)
