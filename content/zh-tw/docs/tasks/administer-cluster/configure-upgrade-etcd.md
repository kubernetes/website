---
title: 操作 Kubernetes 中的 etcd 叢集
content_type: task
weight: 270
---
<!--
...
你需要先理解運行 etcd 叢集的一般期望。更多細節參閱 [etcd 文檔](https://etcd.io/docs/)。
...
* etcd 是一個基於主節點（Leader-Based）的分佈式系統。確保主節點定期向所有從節點發送心跳，以保持叢集穩定。

* 你運行的 etcd 叢集成員個數應爲奇數。
...
* 確保不發生資源不足。
  ...
  叢集的性能和穩定性對網路和磁盤 I/O 非常敏感。任何資源匱乏都會導致心跳超時，
  從而導致叢集的不穩定。不穩定的情況表明沒有選出任何主節點。
  在這種情況下，叢集不能對其當前狀態進行任何更改，這意味着不能調度新的 Pod。
...
使用有限的資源運行 etcd 只適合測試目的。在生產環境中部署 etcd，你需要有先進的硬件設定。
...
`etcdctl` 和 `etcdutl` 是用於與 etcd 叢集交互的命令列工具，但它們有不同的用途：

- `etcdctl`：這是通過網路與 etcd 交互的主要命令列客戶端。
  它用於日常操作，比如管理鍵值對、管理叢集、檢查健康狀態等。
...
-->
## 啓動 etcd 叢集    {#starting-etcd-clusters}

本節介紹如何啓動單節點和多節點 etcd 叢集。
...
-->
2. 使用參數 `--etcd-servers=$PRIVATE_IP:2379` 啓動 Kubernetes API 伺服器。
...
-->
例如，考慮運行以下客戶端 URL 的五個成員的 etcd 叢集：`http://$IP1:2379`、
`http://$IP2:2379`、`http://$IP3:2379`、`http://$IP4:2379` 和 `http://$IP5:2379`。
要啓動 Kubernetes API 伺服器：
...
-->
### 使用負載均衡器的多節點 etcd 叢集    {#multi-node-etcd-cluster-with-load-balancer}

要運行負載均衡的 etcd 叢集：

1. 建立一個 etcd 叢集。
2. 在 etcd 叢集前面設定負載均衡器。例如，讓負載均衡器的地址爲 `$LB`。
3. 使用參數 `--etcd-servers=$LB:2379` 啓動 Kubernetes API 伺服器。
...
the URL schema.
-->
若要使用安全對等通信對 etcd 進行設定，請指定參數 `--peer-key-file=peer.key`
...
communication:
-->
類似地，要使用安全客戶端通信對 etcd 進行設定，請指定參數 `--key=k8sclient.key`
...
-->
設定安全通信後，使用 TLS 身份驗證來限制只有 Kubernetes API 伺服器可以訪問 etcd 叢集。
...
當 etcd 設定爲 `--client-cert-auth` 和 TLS 時，它使用系統 CA 或由 `--trusted-ca-file`
...
`--etcd-cafile=ca.cert`.
-->
一旦正確設定了 etcd，只有具有有效證書的客戶端才能訪問它。要讓 Kubernetes API 伺服器訪問，
可以使用參數 `--etcd-certfile=k8sclient.cert`、`--etcd-keyfile=k8sclient.key`
和 `--etcd-cafile=ca.cert` 設定。
...
例如，考慮一個三成員的 etcd 叢集。假定 URL 分別爲：`member1=http://10.0.0.1`、`member2=http://10.0.0.2`
...
2. 執行以下操作之一：
   ...
   1. 如果每個 Kubernetes API 伺服器都設定爲與所有 etcd 成員通信，
      請從 `--etcd-servers` 標誌中移除刪除失敗的成員，然後重新啓動每個 Kubernetes API 伺服器。
   2. 如果每個 Kubernetes API 伺服器都與單個 etcd 成員通信，
      則停止與失敗的 etcd 通信的 Kubernetes API 伺服器。
...
7. 執行以下操作之一：
   ...
   1. 如果每個 Kubernetes API 伺服器都設定爲與所有 etcd 成員通信，
      則將新增的成員添加到 `--etcd-servers` 標誌，然後重新啓動每個 Kubernetes API 伺服器。
   2. 如果每個 Kubernetes API 伺服器都與單個 etcd 成員通信，請啓動在第 2 步中停止的 Kubernetes API 伺服器。
      然後設定 Kubernetes API 伺服器客戶端以再次將請求路由到已停止的 Kubernetes API 伺服器。
      這通常可以通過設定負載均衡器來完成。
...
-->
## 爲 etcd 叢集擴容    {#scaling-out-etcd-clusters}
...
supported scale.
-->
通過交換性能，對 etcd 叢集擴容可以提高可用性。縮放不會提高叢集性能和能力。
一般情況下不要擴大或縮小 etcd 叢集的集合。不要爲 etcd 叢集設定任何自動縮放組。
強烈建議始終在任何官方支持的規模上運行生產 Kubernetes 叢集時使用靜態的五成員 etcd 叢集。
...
-->
如果叢集中正在運行任何 API 伺服器，則不應嘗試還原 etcd 的實例。相反，請按照以下步驟還原 etcd：
...
instead.
-->
如果還原的叢集的訪問 URL 與前一個叢集不同，則必須相應地重新設定 Kubernetes API 伺服器。
在本例中，使用參數 `--etcd-servers=$NEW_ETCD_CLUSTER` 而不是參數 `--etcd-servers=$OLD_ETCD_CLUSTER`
重新啓動 Kubernetes API 伺服器。用相應的 IP 地址替換 `$NEW_ETCD_CLUSTER` 和 `$OLD_ETCD_CLUSTER`。
如果在 etcd 叢集前面使用負載均衡，則可能需要更新負載均衡器。
...
-->
如果大多數 etcd 成員永久失敗，則認爲 etcd 叢集失敗。在這種情況下，Kubernetes 不能對其當前狀態進行任何更改。
雖然已調度的 Pod 可能繼續運行，但新的 Pod 無法調度。在這種情況下，
恢復 etcd 叢集並可能需要重新設定 Kubernetes API 伺服器以修復問題。
...
-->
### 叢集碎片整理   {#cluster-defragmentation}
