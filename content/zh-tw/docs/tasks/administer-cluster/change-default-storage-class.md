...
-->
本文展示瞭如何改變默認的 Storage Class，它用於爲沒有特殊需求的 PersistentVolumeClaims 配置 volumes。
...
取決於安裝模式，你的 Kubernetes 集羣可能和一個被標記爲默認的已有 StorageClass 一起部署。
...
-->
預先安裝的默認 StorageClass 可能不能很好的適應你期望的工作負載；例如，它配置的存儲可能太過昂貴。
如果是這樣的話，你可以改變默認 StorageClass，或者完全禁用它以防止動態配置存儲。
...
-->
刪除默認 StorageClass 可能行不通，因爲它可能會被你集羣中的擴展管理器自動重建。
