...
通過聚合這些信號並在其間建立關聯，你可以獲得整個叢集中控制平面、插件和應用程式的統一視圖。
...
-->
有關詳細資訊和設定選項，請參閱[系統指標指南](/zh-cn/docs/concepts/cluster-administration/system-metrics/)。
...
對於多叢集或多雲可觀測性，分佈式時序資料庫（例如 Thanos 或 Cortex）
可以補充 Prometheus。
...
日誌爲應用程式、Kubernetes 系統組件的內部事件，以及與安全相關的活動（例如審計日誌）提供按時序排列的記錄。
...
-->
容器運行時從標準輸出（`stdout`）和標準錯誤（`stderr`）流捕獲容器化應用程式的輸出。
...
- 在安裝了 `systemd` 的機器上，kubelet 和容器運行時寫入 journald。
  否則，它們寫入 `/var/log` 目錄中的 `.log` 檔案。
- 在容器內運行的系統組件始終寫入 `/var/log` 中的 `.log` 檔案，
  繞過預設的容器日誌記錄機制。
...
-->
儲存在 `/var/log` 下的系統組件和容器日誌需要進行日誌輪轉以防止不受控制的增長。
某些叢集設定腳本預設安裝日誌輪轉；請檢查你的環境並根據需要進行調整。
有關位置、格式和設定選項的詳細資訊，請參閱[系統日誌參考](/zh-cn/docs/concepts/cluster-administration/system-logs/)。
...
該代理尾部跟蹤這些日誌檔案並將日誌條目轉發到某集中式日誌儲存。
...
-->
有關日誌記錄代理和集中式日誌儲存的資訊，請參閱[常見可觀測性工具 - 日誌工具](#logging-tools)。
...
鏈路資料記錄請求如何在 Kubernetes 組件和應用程式之間移動，
將操作之間的延遲、時序和關係聯繫起來。
通過收集鏈路資料，你可以將端到端的請求流可視化、診斷性能問題，
並發現控制平面、插件或應用程式中的瓶頸或意外交互。
...
（OTLP）導出跨度（span）資料，既可以直接通過內置的 gRPC 導出器導出，
...
-->
OpenTelemetry Collector 從組件和應用程式接收跨度資料，
...
-->
有關鏈路收集器和後端的資訊，請參閱[常見可觀測性工具 - 鏈路工具](#tracing-tools)。
...
- [Cortex](https://cortexmetrics.io/) 提供水平可擴展的長期 Prometheus 儲存。
- [Grafana Mimir](https://grafana.com/oss/mimir/) 是一個 Grafana Labs 項目，
  提供多租戶、水平可擴展的 Prometheus 兼容儲存。
- [Prometheus](https://prometheus.io/) 是從 Kubernetes 組件抓取和儲存指標的監控系統。
- [Thanos](https://thanos.io/) 使用全局查詢、下采樣和對象儲存支持擴展 Prometheus。
...
- [Grafana Loki](https://grafana.com/oss/loki/) 以 Prometheus 啓發的基於標籤的格式儲存日誌。
...
- [Grafana Tempo](https://grafana.com/oss/tempo/) 提供可擴展、低成本的分佈式鏈路儲存。
- [Jaeger](https://www.jaegertracing.io/) 捕獲並可視化微服務的分佈式鏈路。
- [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) 接收、處理和導出包括鏈路在內的遙測資料。
