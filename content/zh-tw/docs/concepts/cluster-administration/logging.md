...
-->
應用日誌可以讓你瞭解應用內部的運行狀況。日誌對調試問題和監控叢集活動非常有用。
...
在叢集中，日誌應該具有獨立的儲存，並且其生命週期與節點、Pod 或容器的生命週期相獨立。
這個概念叫[叢集級的日誌](#cluster-level-logging-architectures)。
...
容器運行時對寫入到容器化應用程式的 `stdout` 和 `stderr` 流的所有輸出進行處理和轉發。
...
-->
預設情況下，如果容器重新啓動，kubelet 會保留一個終止的容器及其日誌。
...
kubelet（使用 CRI）將此資訊發送到容器運行時，而運行時則將容器日誌寫到給定位置。
...
-->
你可以使用 [kubelet 設定檔案](/zh-cn/docs/reference/config-api/kubelet-config.v1beta1/)設定兩個
kubelet [設定選項](/zh-cn/docs/reference/config-api/kubelet-config.v1beta1/#kubelet-config-k8s-io-v1beta1-KubeletConfiguration)、
`containerLogMaxSize` （預設 10Mi）和 `containerLogMaxFiles`（預設 5）。
這些設置分別允許你分別設定每個日誌檔案大小的最大值和每個容器允許的最大檔案數。
...
-->
爲了在工作負載生成的日誌量較大的叢集中執行高效的日誌輪換，kubelet
還提供了一種機制，基於可以執行多少併發日誌輪換以及監控和輪換日誌所需要的間隔來調整日誌的輪換方式。
你可以使用 [kubelet 設定檔案](/zh-cn/docs/reference/config-api/kubelet-config.v1beta1/)
設定兩個 kubelet [設定選項](/zh-cn/docs/tasks/administer-cluster/kubelet-config-file/)：
...
節點上的 kubelet 會處理請求並直接從日誌檔案讀取。kubelet 將返回該日誌檔案的內容。
...
-->
只有最新的日誌檔案的內容可以通過 `kubectl logs` 獲得。

例如，如果 Pod 寫入 40 MiB 的日誌，並且 kubelet 在 10 MiB 之後輪換日誌，
則運行 `kubectl logs` 將最多返回 10 MiB 的資料。
{{< /note >}}
...
-->
在使用 systemd 的 Linux 節點上，kubelet 和容器運行時預設寫入 journald。
你要使用 `journalctl` 來閱讀 systemd 日誌；例如：`journalctl -u kubelet`。

如果 systemd 不存在，kubelet 和容器運行時將寫入到 `/var/log` 目錄中的 `.log` 檔案。
...
-->
預設情況下，kubelet 指示你的容器運行時將日誌寫入 `/var/log/pods` 中的目錄。

有關 `kube-log-runner` 的更多資訊，請閱讀[系統日誌](/zh-cn/docs/concepts/cluster-administration/system-logs/#klog)。
...
-->
預設情況下，kubelet 將日誌寫入目錄 `C:\var\logs` 中的檔案（注意這不是 `C:\var\log`）。

儘管 `C:\var\log` 是這些日誌的 Kubernetes 預設位置，
但一些叢集部署工具會將 Windows 節點設置爲將日誌放到 `C:\var\log\kubelet`。
...
但是，kubelet 預設指示你的容器運行時在目錄 `C:\var\log\pods` 中寫入日誌。
...
-->
對於在 Pod 中運行的 Kubernetes 叢集組件，其日誌會寫入 `/var/log` 目錄中的檔案，
相當於繞過預設的日誌機制（組件不會寫入 systemd 日誌）。
你可以使用 Kubernetes 的儲存機制將持久儲存映射到運行該組件的容器中。
...
-->
kubelet 允許將 Pod 日誌目錄從預設的 `/var/log/pods` 更改爲自定義路徑。
可以通過在 kubelet 的設定檔案中設定 `podLogsDir` 參數來進行此調整。
...
-->
需要注意的是，預設位置 `/var/log/pods` 已使用很長一段時間，並且某些進程可能會隱式使用此路徑。
...
否則，如果日誌位於與 `/var` 不同的檔案系統上，kubelet
將不會跟蹤該檔案系統的使用情況。如果檔案系統已滿，則可能會出現問題。
{{< /caution >}}
...
-->
有關 etcd 及其日誌的詳細資訊，請查閱 [etcd 文檔](https://etcd.io/docs/)。
同樣，你可以使用 Kubernetes 的儲存機制將持久儲存映射到運行該組件的容器中。
...
-->
## 叢集級日誌架構   {#cluster-level-logging-architectures}

雖然 Kubernetes 沒有爲叢集級日誌記錄提供原生的解決方案，但你可以考慮幾種常見的方法。
以下是一些選項：
...
-->
你可以通過在每個節點上使用**節點級的日誌記錄代理**來實現叢集級日誌記錄。
...
* 邊車容器運行一個日誌代理，設定該日誌代理以便從應用容器收集日誌。
...
-->
例如，某 Pod 中運行一個容器，且該容器使用兩個不同的格式寫入到兩個不同的日誌檔案。
下面是這個 Pod 的清單：
...
相反，你可以創建兩個邊車容器。每個邊車容器可以從共享卷跟蹤特定的日誌檔案，
並將檔案內容重定向到各自的 `stdout` 流。
...
container approach.
-->
如果你在叢集中安裝了節點級代理，由代理自動獲取上述日誌流，而無需任何進一步的設定。
如果你願意，你可以將代理設定爲根據源容器解析日誌行。

即使對於 CPU 和內存使用率較低的 Pod（CPU 爲幾毫核，內存爲幾兆字節），將日誌寫入一個檔案，
將這些日誌流寫到 `stdout` 也有可能使節點所需的儲存量翻倍。
如果你有一個寫入特定檔案的應用程式，則建議將 `/dev/stdout` 設置爲目標檔案，而不是採用流式邊車容器方法。
...
the agent to parse log lines depending on the source container.
-->
叢集中安裝的節點級代理會自動獲取這些日誌流，而無需進一步設定。
如果你願意，你也可以設定代理程式來解析源容器的日誌行。
...
向檔案寫日誌然後輸出到 `stdout` 流仍然會成倍地增加磁盤使用率。
如果你的應用向單一檔案寫日誌，通常最好設置 `/dev/stdout` 作爲目標路徑，
而不是使用流式的邊車容器方式。
...
-->
如果節點級日誌記錄代理程式對於你的場景來說不夠靈活，
你可以創建一個帶有單獨日誌記錄代理的邊車容器，將代理程式專門設定爲與你的應用程式一起運行。
...
-->
下面是兩個設定檔案，可以用來實現一個帶日誌代理的邊車容器。
第一個檔案包含用來設定 fluentd 的
...
-->
你可以將此示例設定中的 fluentd 替換爲其他日誌代理，從應用容器內的其他來源讀取資料。
{{< /note >}}
...
該 Pod 掛載一個卷，fluentd 可以從這個捲上揀選其設定資料。
...
of Kubernetes.
-->
從各個應用中直接暴露和推送日誌資料的叢集日誌機制已超出 Kubernetes 的範圍。
...
-->
* 閱讀有關 [Kubernetes 系統日誌](/zh-cn/docs/concepts/cluster-administration/system-logs/)的資訊
