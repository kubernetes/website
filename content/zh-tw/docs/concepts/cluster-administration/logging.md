...
在叢集中，日誌應該具有獨立的存儲，並且其生命週期與節點、Pod 或容器的生命週期相獨立。
...
-->
你可以使用 [kubelet 設定文件](/zh-cn/docs/reference/config-api/kubelet-config.v1beta1/)設定兩個
...
這些設置分別允許你分別設定每個日誌文件大小的最大值和每個容器允許的最大文件數。
...
你可以使用 [kubelet 設定文件](/zh-cn/docs/reference/config-api/kubelet-config.v1beta1/)
...
-->
對於在 Pod 中運行的 Kubernetes 叢集組件，其日誌會寫入 `/var/log` 目錄中的文件，
...
可以通過在 kubelet 的設定文件中設定 `podLogsDir` 參數來進行此調整。
...
你可以創建一個帶有單獨日誌記錄代理的邊車容器，將代理程序專門設定爲與你的應用程序一起運行。
...
-->
下面是兩個設定文件，可以用來實現一個帶日誌代理的邊車容器。
第一個文件包含用來設定 fluentd 的
...
-->
你可以將此示例設定中的 fluentd 替換爲其他日誌代理，從應用容器內的其他來源讀取數據。
{{< /note >}}
...
該 Pod 掛載一個卷，fluentd 可以從這個捲上揀選其設定數據。
...
of Kubernetes.
-->
從各個應用中直接暴露和推送日誌數據的叢集日誌機制已超出 Kubernetes 的範圍。
