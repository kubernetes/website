...
title: "工作負載"
weight: 55
description: 理解 Kubernetes 中可部署的最小計算對象 Pod 以及輔助 Pod 運行的上層抽象。
card:
  title: 工作負載與 Pod
  name: concepts
  weight: 60
...
* [Job](/zh-cn/docs/concepts/workloads/controllers/job/) 和
  ...
  來定義只需要執行一次並且執行後即視爲完成的任務。你可以使用
  [CronJob](/zh-cn/docs/concepts/workloads/controllers/cron-jobs/)
  來根據某個排期表來多次運行同一個 Job。
...
-->
在龐大的 Kubernetes 生態系統中，你還可以找到一些提供額外操作的第三方工作負載相關的資源。
通過使用[定製資源定義（CRD）](/zh-cn/docs/concepts/extend-kubernetes/api-extension/custom-resources/)，
...
* [使用 Deployment 運行一個無狀態的應用](/zh-cn/docs/tasks/run-application/run-stateless-application-deployment/)
* 以[單實例](/zh-cn/docs/tasks/run-application/run-single-instance-stateful-application/)或者[多副本集合](/zh-cn/docs/tasks/run-application/run-replicated-stateful-application/)
  的形式運行有狀態的應用；
* [使用 CronJob 運行自動化的任務](/zh-cn/docs/tasks/job/automated-tasks-with-cron-jobs/)
...
-->
一旦你的應用處於運行狀態，你就可能想要以
[Service](/zh-cn/docs/concepts/services-networking/service/)
的形式使之可在互聯網上訪問；或者對於 Web 應用而言，使用
[Ingress](/zh-cn/docs/concepts/services-networking/ingress) 資源將其暴露到互聯網上。
