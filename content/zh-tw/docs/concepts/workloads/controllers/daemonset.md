...
- apiVersion: "apps/v1"
  kind: "DaemonSet"
description: >-
  DaemonSet 定義了提供節點本地設施的 Pod。這些設施可能對於叢集的運行至關重要，例如網路輔助工具，或者作爲 add-on 的一部分。
...
當有節點加入叢集時， 也會爲他們新增一個 Pod 。
當有節點從叢集移除時，這些 Pod 也會被回收。刪除 DaemonSet 將會刪除它創建的所有 Pod。
...
| [`node.kubernetes.io/network-unavailable`](/zh-cn/docs/reference/labels-annotations-taints/#node-kubernetes-io-network-unavailable) | `NoSchedule` | **僅針對請求主機聯網的 DaemonSet Pod 添加此容忍度**，即 Pod 具有 `spec.hostNetwork: true`。這些 DaemonSet Pod 可以被調度到網路不可用的節點上。|
...
例如[叢集聯網](/zh-cn/docs/concepts/cluster-administration/networking/)，
Kubernetes 在節點就緒之前將 DaemonSet Pod 放到節點上會很有幫助。
例如，如果沒有這種特殊的容忍度，因爲網路插件未在節點上運行，所以你可能會在未標記爲就緒的節點上陷入死鎖狀態，
同時因爲該節點還未就緒，所以網路插件不會在該節點上運行。
...
- 爲守護進程和應用所使用的設定語言和工具（如 Pod 模板、`kubectl`）是相同的。
...
例如，[網路插件](/zh-cn/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/)通常包含一個以
DaemonSet 運行的組件。這個 DaemonSet 組件確保它所在的節點的叢集網路正常工作。
