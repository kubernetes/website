...
在節點上運行 Pod 時，Pod 本身佔用大量系統資源。這些是運行 Pod 內容器所需資源之外的資源。
...
相關聯的開銷在[准入](/zh-cn/docs/reference/access-authn-authz/extensible-admission-controllers/#what-are-admission-webhooks)時設置的。
...
在調度 Pod 時，除了考慮容器資源請求的總和外，還要考慮 Pod 開銷。
...
-->
要使用 Pod 開銷，你需要一個定義了 `overhead` 字段的 RuntimeClass。
例如，你可以使用以下 RuntimeClass 定義，其中使用了一個虛擬化容器運行時（在這個例子中，Kata Containers 與 Firecracker 虛擬機監視器結合使用），
每個 Pod 使用大約 120MiB 的虛擬機和寄宿操作系統：
...
```yaml
# 你需要修改這個示例以匹配實際的運行時名稱，
# 以及在你的集羣中運行時在 Pod 層面增加的資源開銷。
apiVersion: node.k8s.io/v1
kind: RuntimeClass
metadata:
  name: kata-fc
...
-->
如果在 Pod 定義中只設置了 `limits`，kubelet 將根據 limits 推斷 `requests`，並將其設置與 limits 相同的值。
{{< /note >}}
...
更新工作負載的 PodSpec 以包含
RuntimeClass 中定義的 `overhead`。如果 PodSpec 中已定義該字段，該 Pod 將會被拒絕。
...
Pod overhead value:
-->
在 RuntimeClass 准入控制器進行修改後，你可以查看更新後的 Pod 開銷值：
```bash
...
`overhead` field are counted.
 -->
如果定義了 [ResourceQuota](/zh-cn/docs/concepts/policy/resource-quotas/), 
則容器請求的總量以及 `overhead` 字段都將計算在內。
...
-->
當 kube-scheduler 決定在哪一個節點調度運行新的 Pod 時，調度器會兼顧該 Pod 的
`overhead` 以及該 Pod 的容器請求總量。在這個示例中，調度器將資源請求和開銷相加，
然後尋找具備 2.25 CPU 和 320 MiB 內存可用的節點。
...
-->
一旦 Pod 被調度到了某個節點， 該節點上的 kubelet 將爲該 Pod 新建一個 
{{< glossary_tooltip text="cgroup" term_id="cgroup" >}}。 底層容器運行時將在這個
Pod 中創建容器。
...
Burstable QoS），kubelet 會爲與該資源（CPU 的 `cpu.cfs_quota_us` 以及內存的
...
PodSpec 中定義的 `overhead` 之和設置 `cpu.shares`。
...
-->
## 驗證 Pod cgroup 限制 {#verify-pod-cgroup-limits}
...
-->
```bash
# 在該 Pod 被調度到的節點上執行這個命令。
# 另外，修改 cgroup 的名稱以匹配爲該 Pod 分配的 cgroup。
...
-->
在 [kube-state-metrics](https://github.com/kubernetes/kube-state-metrics) 中可以通過
