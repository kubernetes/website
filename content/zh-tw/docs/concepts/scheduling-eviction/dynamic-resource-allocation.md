...
-->
DRA 爲叢集中的設備提供了一種靈活的方式來進行分類、請求和使用。 使用 DRA 具有以下好處：
...
* **簡化 Pod 的資源請求**：使用 DRA 後，應用運維人員無需在 Pod 的資源請求中明確指定設備的規格。
  相反，Pod 只需引用一個資源申領，這個申領中的設備設定將會自動應用到該 Pod。
...
### DRA 使用者的類型 {#dra-user-types}
使用 DRA 進行設備分配的工作流程裏通常涉及到以下幾類使用者：
...
* **Cluster admin**: responsible for configuring clusters and nodes,
  ...
  * Optionally, create DeviceClasses that workload operators can use to claim
    devices.
-->
* **叢集管理員**：負責叢集與節點設定、設備掛接、驅動安裝等相關工作。
  若要使用 DRA，叢集管理員需要執行以下操作：
...
要創建 DeviceClass，請參閱[在叢集中安裝 DRA](/zh-cn/docs/tasks/configure-pod-container/assign-resources/set-up-dra-cluster).
...
* **ResourceClaim**： 你希望多個 Pod 對某個特定設備進行共享訪問。你可以創建 ResourceClaim
  並對其生命週期進行手動管理。
* **ResourceClaimTemplate**：你希望 Pod 能夠有對獨立但有相似設定的設備進行獨立訪問。
...
這使得使用者能夠在首選設備不可用時指定工作負載可以使用的替代設備。
...
[DRA 使用者的類型](#dra-user-types)以及 Kubernetes 系統各自的工作流程。

<!--
### Workflow for users {#user-workflow}
-->
### 使用者工作流程{#user-workflow}
...
-->
2. **創建工作負載**：叢集控制面檢查那些引用了 ResourceClaimTemplate 或特定
   ResourceClaim 的工作負載。
   ...
   * 如果這個工作負載使用了一個特定的 ResourceClaim， 那麼 Kubernetes 將會檢查這個
     ResourceClaim 在叢集中是否存在。如果 ResourceClaim 不存在，則 Pod 將不會被部署。
...
-->
3. **過濾 ResourceSlice**：對於任意一個 Pod，Kubernetes 會檢查叢集中的 ResourceSlice
   以找到一個滿足所有以下條件的設備：
...
-->
這種情況也可能發生在 Pod 被調度時調度器中未啓用動態資源分配支持的時候（原因可能是版本偏差、設定、特性門控等）。
...
此後調度器會從該類的設備中選擇匹配的設備。這允許使用者在 Pod
中繼續使用擴展資源請求通過設備插件來申請擴展資源，或是 DRA 設備。
在叢集的單個節點上，同一個擴展資源要麼通過設備插件，要麼通過 DRA 提供。
在同一個叢集內，同一個擴展資源在某些節點上可以由設備插件提供，而在其他節點上由 DRA 提供。
...
-->
另外，使用者可以使用一種特殊的擴展資源來分配設備，而不一定需要顯式創建 ResourceClaim。
...
使用者可以在 ResourceClaim 的 `spec.devices.requests` 中新增的 `capacity` 字段進行設置，
以指定每次分配所需的設備資源容量。
...
-->
下面是一個支持多次分配、並具有可消耗帶寬容量的網路設備的示例。
