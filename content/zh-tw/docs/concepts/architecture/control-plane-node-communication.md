...
provider).
-->
本文列舉控制面節點（確切地說是 {{< glossary_tooltip term_id="kube-apiserver" text="API 服務器" >}}）和
Kubernetes {{< glossary_tooltip text="集羣" term_id="cluster" length="all" >}}之間的通信路徑。
目的是爲了讓用戶能夠自定義他們的安裝，以實現對網絡配置的加固，
使得集羣能夠在不可信的網絡上（或者在一個雲服務商完全公開的 IP 上）運行。
...
所有從節點（或運行於其上的 Pod）發出的 API 調用都終止於 API 服務器。
其它控制面組件都沒有被設計爲可暴露遠程服務。
API 服務器被配置爲在一個安全的 HTTPS 端口（通常爲 443）上監聽遠程連接請求，
...
-->
應該使用集羣的公共根{{< glossary_tooltip text="證書" term_id="certificate" >}}開通節點，
這樣它們就能夠基於有效的客戶端憑據安全地連接 API 服務器。
...
-->
想要連接到 API 服務器的 {{< glossary_tooltip text="Pod" term_id="pod" >}}
可以使用服務賬號安全地進行連接。
當 Pod 被實例化時，Kubernetes 自動把公共根證書和一個有效的持有者令牌注入到 Pod 裏。
`kubernetes` 服務（位於 `default` 名字空間中）配置了一個虛擬 IP 地址，
用於（通過 `{{< glossary_tooltip text="kube-proxy" term_id="kube-proxy" >}}`）轉發請求到
API 服務器的 HTTPS 末端。

控制面組件也通過安全端口與集羣的 API 服務器通信。
...
networks.
-->
這樣，從集羣節點和節點上運行的 Pod 到控制面的連接的缺省操作模式即是安全的，
能夠在不可信的網絡或公網上運行。
...
從控制面（API 服務器）到節點有兩種主要的通信路徑。
第一種是從 API 服務器到集羣中每個節點上運行的
{{< glossary_tooltip text="kubelet" term_id="kubelet" >}} 進程。
第二種是從 API 服務器通過它的**代理**功能連接到任何節點、Pod 或者服務。
...
-->
### API 服務器到 kubelet  {#api-server-to-kubelet}

從 API 服務器到 kubelet 的連接用於：
...
默認情況下，API 服務器不檢查 kubelet 的服務證書。這使得此類連接容易受到中間人攻擊，
在非受信網絡或公開網絡上運行也是 **不安全的**。
...
API 服務器提供一個根證書包，用於 kubelet 的服務證書。

如果無法實現這點，又要求避免在非受信網絡或公共網絡上進行連接，可在 API 服務器和
...
-->
### API 服務器到節點、Pod 和服務  {#api-server-to-nodes-pods-and-services}

從 API 服務器到節點、Pod 或服務的連接默認爲純 HTTP 方式，因此既沒有認證，也沒有加密。
這些連接可通過給 API URL 中的節點、Pod 或服務名稱添加前綴 `https:` 來運行在安全的 HTTPS 連接上。
不過這些連接既不會驗證 HTTPS 末端提供的證書，也不會提供客戶端證書。
因此，雖然連接是加密的，仍無法提供任何完整性保證。
這些連接 **目前還不能安全地** 在非受信網絡或公共網絡上運行。
...
在這種配置下，API 服務器建立一個到集羣中各節點的 SSH 隧道（連接到在 22 端口監聽的 SSH 服務器）
並通過這個隧道傳輸所有到 kubelet、節點、Pod 或服務的請求。
這一隧道保證通信不會被暴露到集羣節點所運行的網絡之外。
...
-->
作爲 SSH 隧道的替代方案，Konnectivity 服務提供 TCP 層的代理，以便支持從控制面到集羣的通信。
Konnectivity 服務包含兩個部分：Konnectivity 服務器和 Konnectivity 代理，
分別運行在控制面網絡和節點網絡中。
Konnectivity 代理建立並維持到 Konnectivity 服務器的網絡連接。
啓用 Konnectivity 服務之後，所有控制面到節點的通信都通過這些連接傳輸。

請瀏覽 [Konnectivity 服務任務](/zh-cn/docs/tasks/extend-kubernetes/setup-konnectivity/)
在你的集羣中配置 Konnectivity 服務。
...
* 進一步瞭解如何[保護集羣](/zh-cn/docs/tasks/administer-cluster/securing-a-cluster/)
...
* [使用端口轉發來訪問集羣中的應用](/zh-cn/docs/tasks/access-application-cluster/port-forward-access-application-cluster/)
