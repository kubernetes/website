...
### 調度器身份認證與鑑權命令行選項   {#scheduler-authentication-authorization-command-line-options}

在設置身份認證配置時，應確保 kube-scheduler 的身份認證配置與 kube-apiserver 的身份認證配置保持一致。
如果任一請求缺少身份認證頭，
則應[通過 kube-apiserver 進行身份認證以保證集羣內的所有身份認證一致](/zh-cn/docs/tasks/extend-kubernetes/configure-aggregation-layer/#original-request-username-and-group)。
...
-->
- `authentication-kubeconfig`：確保提供正確的 kubeconfig 文件，使調度器能從 API 服務器獲取身份認證配置選項。
  這個 kubeconfig 文件應設置嚴格的文件權限以確保安全。
- `authentication-tolerate-lookup-failure`：設置爲 `false`，確保調度器**始終**從 API 服務器查找其身份認證配置。
- `authentication-skip-lookup`：設置爲 `false`，確保調度器**始終**從 API 服務器查找其身份認證配置。
...
- `profiling`：設置爲 `false` 以禁用性能分析端點。性能分析端點可用於調試，
  ...
  提供。在 kube-scheduler 配置中，將 `enableProfiling` 設置爲 `false` 即可禁用性能分析。
...
-->
### 調度器網絡命令行選項   {#scheduler-networking-command-line-options}
...
-->
### 調度器 TLS 命令行選項    {#scheduler-tls-command-line-options}
...
-->
## 自定義調度器的調度配置   {#scheduling-configurations-for-custom-schedulers}

在基於 Kubernetes 調度代碼使用自定義調度器時，
集羣管理員需謹慎使用 `queueSort`、`prefilter`、`filter` 和 `permit`
[擴展點](/zh-cn/docs/reference/scheduling/config/#extension-points)。
這些擴展點控制調度過程的不同階段，配置錯誤可能會影響 kube-scheduler 在集羣中的行爲。
...
- 實現 `permit` 擴展點的插件可能會阻止或延遲 Pod 的綁定。此類插件應由集羣管理員徹底審查。
...
-->
這會創建一個調度器配置文件 `my-custom-scheduler`。每當 Pod 的 `.spec` 中未設置 `.spec.schedulerName` 時，
kube-scheduler 會使用主要配置和默認插件運行該 Pod。如果你定義的 Pod 將 `.spec.schedulerName` 設置爲
`my-custom-scheduler`，kube-scheduler 會運行但使用自定義配置；在該自定義配置中，
...
（或任何其他在你的集羣中不存在的調度器名稱），那麼 Pod 將不會生成任何事件。
