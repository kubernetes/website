...
-->
選擇合適的身份認證機制是確保叢集安全的一個重要方面。
Kubernetes 提供了多種內置機制，
當爲你的叢集選擇最好的身份認證機制時需要謹慎考慮每種機制的優缺點。
...
以簡化使用者管理，避免使用者仍保有對其不再需要的叢集的訪問權限的情況。
...
對系統組件進行身份認證，
例如 Kubelet 對 API 伺服器進行身份認證時。
雖然這種機制也可以用於使用者身份認證，但由於一些限制它可能不太適合在生產中使用：
...
- 客戶端證書無法獨立撤銷。
  ...
  爲了降低這種風險，建議爲使用客戶端證書創建的使用者身份認證憑據設定較短的有效期。
<!--
...
-->
- 如果證書需要被作廢，必須重新爲證書機構設置密鑰，但這樣做可能給叢集帶來可用性風險。
<!--
...
-->
- 使用客戶端證書身份認證需要客戶端直連 API 伺服器而不允許中間存在 TLS 終止節點，
  這一約束可能會使網路架構變得複雜。
<!--
...
-->
[啓動引導令牌](/zh-cn/docs/reference/access-authn-authz/bootstrap-tokens/)用於節點加入叢集，
因爲下列的一些原因，不建議用於使用者身份認證：
...
-->
- 任何叢集使用者，只要能讀取服務賬號令牌定義所在的命名空間中的 Secret，就能看到身份認證令牌。
<!--
...
-->
首先，在代理和 Kubernetes API 伺服器間必須以安全的方式設定 TLS 連接，
從而降低流量劫持或嗅探攻擊的風險。
TLS 連接可以確保代理和 Kubernetes API 伺服器間的通信是安全的。
...
-->
- [使用者認證](/zh-cn/docs/reference/access-authn-authz/authentication/)
