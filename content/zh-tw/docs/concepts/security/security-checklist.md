...
- [ ] 在啓動後 `system:masters` 組不用於用戶或組件的身份驗證。
...
an admin user.
-->
在啓動後，用戶和組件都不應以 `system:masters` 身份向 Kubernetes API 進行身份驗證。
同樣，應避免將任何 kube-controller-manager 以 `system:masters` 運行。
事實上，`system:masters` 應該只用作一個例外機制，而不是管理員用戶。
...
- [ ] 使用的 CNI 插件可支持網絡策略。
- [ ] 對集羣中的所有工作負載應用入站和出站的網絡策略。
- [ ] 落實每個名字空間內的默認網絡策略，覆蓋所有 Pod，拒絕一切訪問。
- [ ] 如果合適，使用服務網格來加密集羣內的所有通信。
...
-->
許多[容器網絡接口（Container Network Interface，CNI）插件](/zh-cn/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/)提供了限制
Pod 可能與之通信的網絡資源的功能。
這種限制通常通過[網絡策略](/zh-cn/docs/concepts/services-networking/network-policies/)來完成，
網絡策略提供了一種名字空間作用域的資源來定義規則。
在每個名字空間中，默認的網絡策略會阻塞所有的出入站流量，並選擇所有 Pod，
...
-->
應該限制外部互聯網對 Kubernetes API 服務器未公開的 API 的訪問。
請小心，因爲許多託管的 Kubernetes 發行版在默認情況下公開了 API 服務器。
當然，你可以使用堡壘機訪問服務器。

對 [kubelet](/zh-cn/docs/reference/command-line-tools-reference/kubelet/) API 的訪問應該受到限制，
並且不公開，當沒有使用 `--config` 參數來設置配置文件時，默認的身份驗證和鑑權設置是過於寬鬆的。
...
- [ ] 對敏感工作負載可以設置 CPU 限制。
- [ ] 對於支持 Seccomp 的節點，可以爲程序啓用合適的系統調用配置文件。
- [ ] 對於支持 AppArmor 或 SELinux 的系統，可以爲程序啓用合適的配置文件。
...
在未指定額外許可的情況下，創建這些資源的權限允許直接不受限制地訪問集羣的可調度節點。
...
它可以將進程的特權沙箱化，來限制從用戶空間發起的對內核的調用。
Kubernetes 允許你將加載到節點上的 Seccomp 配置文件自動應用於你的 Pod 和容器。

Seccomp 通過減少容器內對 Linux 內核的系統調用（System Call）以縮小攻擊面，從而提高工作負載的安全性。
Seccomp 過濾器模式藉助 BPF 創建具體系統調用的允許清單或拒絕清單，名爲配置文件（Profile）。

從 Kubernetes 1.27 開始，你可以將 `RuntimeDefault` 設置爲工作負載的默認 Seccomp 配置。
...
默認 AppArmor 配置文件在支持它的節點上強制執行，或者可以配置自定義配置文件。
與 Seccomp 一樣，AppArmor 也通過配置文件進行配置，
其中每個配置文件要麼在強制（Enforcing）模式下運行，即阻止訪問不允許的資源，要麼在投訴（Complaining）模式下運行，只報告違規行爲。
AppArmor 配置文件是通過註解的方式，以容器爲粒度強制執行的，允許進程獲得剛好合適的權限。
...
following features:
-->
處於不同敏感級別的 Pod，例如，應用程序 Pod 和 Kubernetes API 服務器應該被部署到不同的節點上。
...
設置爲 `false` 來完成整個名字空間範圍的配置，或者也可以單獨在 Pod 層面定製。
...
- [ ] 儘量減少容器鏡像中不必要的內容。
- [ ] 容器鏡像配置爲以非特權用戶身份運行。
- [ ] 對容器鏡像的引用是通過 Sha256 摘要實現的，而不是標籤（tags），
  或者[通過准入控制器](/zh-cn/docs/tasks/administer-cluster/verify-signed-artifacts/#verifying-image-signatures-with-admission-controller)在部署時驗證鏡像的數字簽名來驗證鏡像的來源。
- [ ] 在創建和部署過程中定期掃描容器鏡像，並對已知的漏洞軟件進行修補。
...
-->
容器鏡像應該包含運行其所打包的程序所需要的最少內容。
最好，只使用程序及其依賴項，基於最小的基礎鏡像來構建鏡像。
尤其是，在生產中使用的鏡像不應包含 Shell 或調試工具，
...
-->
構建鏡像時使用 [Dockerfile 中的 `USER`](https://docs.docker.com/develop/develop-images/dockerfile_best-practices/#user)
指令直接開始使用非特權用戶。
[安全上下文（Security Context）](/zh-cn/docs/tasks/configure-pod-container/security-context/#set-the-security-context-for-a-pod)
允許使用 `runAsUser` 和 `runAsGroup` 來指定使用特定的用戶和組來啓動容器鏡像，
即使沒有在鏡像清單文件（Manifest）中指定這些配置信息。
不過，鏡像層中的文件權限設置可能無法做到在不修改鏡像的情況下，使用新的非特權用戶來啓動進程。
...
-->
避免使用鏡像標籤來引用鏡像，尤其是 `latest` 標籤，因爲標籤對應的鏡像可以在倉庫中被輕鬆地修改。
首選使用完整的 `Sha256` 摘要，該摘要對特定鏡像清單文件而言是唯一的。
可以通過 [ImagePolicyWebhook](/zh-cn/docs/reference/access-authn-authz/admission-controllers/#imagepolicywebhook)
強制執行此策略。
鏡像簽名還可以在部署時由[准入控制器自動驗證](/zh-cn/docs/tasks/administer-cluster/verify-signed-artifacts/#verifying-image-signatures-with-admission-controller)，
以驗證其真實性和完整性。
...
Production.
-->
掃描容器鏡像可以防止關鍵性的漏洞隨着容器鏡像一起被部署到集羣中。
鏡像掃描應在將容器鏡像部署到集羣之前完成，通常作爲 CI/CD 流水線中的部署過程的一部分來完成。
鏡像掃描的目的是獲取有關容器鏡像中可能存在的漏洞及其預防措施的信息，
例如使用[公共漏洞評分系統 （Common Vulnerability Scoring System，CVSS）](https://www.first.org/cvss/)評分。
如果鏡像掃描的結果與管道合性規則匹配，則只有經過正確修補的容器鏡像纔會最終進入生產環境。
...
-->
准入控制器可以幫助提高集羣的安全性。
然而，由於它們是對 API 服務器的擴展，其自身可能會帶來風險，
...
-->
下面列出了一些准入控制器，可以考慮用這些控制器來增強集羣和應用的安全狀況。
...
- [保護集羣](/zh-cn/docs/tasks/administer-cluster/securing-a-cluster/)提供如何保護集羣免受意外或惡意訪問的信息。
- [集羣多租戶指南](/zh-cn/docs/concepts/security/multi-tenancy/)提供有關多租戶的配置選項建議和最佳實踐。
