...
of a cluster.
-->
使用 kubeconfig 檔案來組織有關叢集、使用者、命名空間和身份認證機制的資訊。
`kubectl` 命令列工具使用 kubeconfig 檔案來查找選擇叢集所需的資訊，並與叢集的 API 伺服器進行通信。
...
-->
用於設定叢集訪問的檔案稱爲 **kubeconfig 檔案**。
這是引用到設定檔案的通用方法，並不意味着有一個名爲 `kubeconfig` 的檔案。
{{< /note >}}
...
-->
請務必僅使用來源可靠的 kubeconfig 檔案。使用特製的 kubeconfig 檔案可能會導致惡意代碼執行或檔案暴露。
如果必須使用不受信任的 kubeconfig 檔案，請首先像檢查 Shell 腳本一樣仔細檢查此檔案。
{{< /warning>}}
...
-->
預設情況下，`kubectl` 在 `$HOME/.kube` 目錄下查找名爲 `config` 的檔案。
你可以通過設置 `KUBECONFIG` 環境變量或者設置
[`--kubeconfig`](/docs/reference/generated/kubectl/kubectl/)參數來指定其他 kubeconfig 檔案。

<!--
For step-by-step instructions on creating and specifying kubeconfig files, see
[Configure Access to Multiple Clusters](/docs/tasks/access-application-cluster/configure-access-multiple-clusters).
-->
有關創建和指定 kubeconfig 檔案的分步說明，
...
-->
使用 kubeconfig 檔案，你可以組織叢集、使用者和命名空間。你還可以定義上下文，以便在叢集和命名空間之間快速輕鬆地切換。
...
-->
通過 kubeconfig 檔案中的 *context* 元素，使用簡便的名稱來對訪問參數進行分組。
每個 context 都有三個參數：cluster、namespace 和 user。
預設情況下，`kubectl` 命令列工具使用 **當前上下文** 中的參數與叢集進行通信。
...
-->
`KUBECONFIG` 環境變量包含一個 kubeconfig 檔案列表。
對於 Linux 和 Mac，此列表以英文冒號分隔。對於 Windows，此列表以英文分號分隔。
`KUBECONFIG` 環境變量不是必需的。
如果 `KUBECONFIG` 環境變量不存在，`kubectl` 將使用預設的 kubeconfig 檔案：`$HOME/.kube/config`。
...
-->
如果 `KUBECONFIG` 環境變量存在，`kubectl` 將使用 `KUBECONFIG` 環境變量中列舉的檔案合併後的有效設定。

<!--
## Merging kubeconfig files
-->
## 合併 kubeconfig 檔案   {#merge-kubeconfig-files}
...
-->
如前所述，輸出可能來自單個 kubeconfig 檔案，也可能是合併多個 kubeconfig 檔案的結果。
...
-->
1. 如果設置了 `--kubeconfig` 參數，則僅使用指定的檔案。不進行合併。此參數只能使用一次。

   否則，如果設置了 `KUBECONFIG` 環境變量，將它用作應合併的檔案列表。根據以下規則合併 `KUBECONFIG` 環境變量中列出的檔案：

   * 忽略空檔案名。
   * 對於內容無法反序列化的檔案，產生錯誤資訊。
   * 第一個設置特定值或者映射鍵的檔案將生效。
   * 永遠不會更改值或者映射鍵。示例：保留第一個檔案的上下文以設置 `current-context`。
     示例：如果兩個檔案都指定了 `red-user`，則僅使用第一個檔案的 `red-user` 中的值。
     即使第二個檔案在 `red-user` 下有非衝突條目，也要丟棄它們。
...
-->
4. 確定要使用的實際叢集資訊。此時，可能有也可能沒有叢集資訊。
   基於此鏈構建每個叢集資訊；第一個匹配項會被採用：

   1. 如果存在叢集資訊，則使用命令列參數：`--server`、`--certificate-authority` 和 `--insecure-skip-tls-verify`。
   2. 如果合併的 kubeconfig 檔案中存在叢集資訊屬性，則使用這些屬性。
...
-->
5. 確定要使用的實際使用者資訊。使用與叢集資訊相同的規則構建使用者資訊，但對於每個使用者只允許使用一種身份認證技術：

   1. 如果存在使用者資訊，則使用命令列參數：`--client-certificate`、`--client-key`、`--username`、`--password` 和 `--token`。
   2. 使用合併的 kubeconfig 檔案中的 `user` 字段。
   3. 如果存在兩種衝突技術，則設定無效。
...
-->
6. 對於仍然缺失的任何資訊，使用其對應的預設值，並可能提示輸入身份認證資訊。
...
在 `$HOME/.kube/config` 中，相對路徑按相對路徑儲存，而絕對路徑按絕對路徑儲存。
...
你可以在 `kubeconfig` 檔案中，爲每個叢集設定 `proxy-url` 來讓 `kubectl` 使用代理，例如：
