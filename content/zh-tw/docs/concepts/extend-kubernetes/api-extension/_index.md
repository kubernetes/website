...
Kubernetes 提供了兩種將自定義資源添加到叢集的方法：
...
- [聚合層（Aggregation Layer）](/zh-cn/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/)位於主
  API 伺服器後面，將 API 伺服器用作代理。
  這種安排稱爲 API 聚合（API Aggregation，AA），允許你通過編寫和部署自己的 API 伺服器來爲自定義資源提供專門的實現。
  主 API 伺服器將你指定的自定義 API 的請求委託給你的 API 伺服器，使其可供所有客戶端使用。
