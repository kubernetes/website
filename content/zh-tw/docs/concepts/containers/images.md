...
-->
容器鏡像（Image）所承載的是封裝了應用程序及其所有軟件依賴的二進制數據。
容器鏡像是可執行的軟件包，可以單獨運行；該軟件包對所處的運行時環境具有明確定義的運行時環境假定。

你通常會創建應用的容器鏡像並將其推送到某倉庫（Registry），然後在
...
的容器鏡像，請訪問[下載 Kubernetes](/zh-cn/releases/download/)。
{{< /note >}}
...
容器鏡像通常會被賦予 `pause`、`example/mycontainer` 或者 `kube-apiserver` 這類的名稱。
鏡像名稱也可以包含所在倉庫的主機名。例如：`fictional.registry.example/imagename`。
...
-->
在鏡像名稱之後，你可以添加一個**標籤（Tag）** 或 **摘要（digest）**
（與使用 `docker` 或 `podman` 等命令時的方式相同）。
使用標籤能讓你辨識同一鏡像序列中的不同版本。
摘要是特定版本鏡像的唯一標識符，是鏡像內容的哈希值，不可變。
...
-->
鏡像標籤可以包含小寫字母、大寫字母、數字、下劃線（`_`）、句點（`.`）和連字符（`-`）。
...
-->
鏡像摘要由哈希算法（例如 `sha256`）和哈希值組成，例如：
`sha256:1ff6c18fbef2045af6b9c16bf034cc421a29027b800e4f9b68ae9b1cb3e9ae07`。
你可以在 [OCI 鏡像規範](https://github.com/opencontainers/image-spec/blob/master/descriptor.md#digests)
中找到有關摘要格式的更多信息。

<!--
Some image name examples that Kubernetes can use are:
-->
Kubernetes 可以使用的一些鏡像名稱示例包括：
...
-->
- `busybox` - 僅包含鏡像名稱，沒有標籤或摘要，Kubernetes 將使用 Docker 公共鏡像倉庫和 `latest` 標籤。
  （例如 `docker.io/library/busybox:latest`）
- `busybox:1.32.0` - 帶標籤的鏡像名稱，Kubernetes 將使用 Docker 公共鏡像倉庫。
  （例如 `docker.io/library/busybox:1.32.0`）
- `registry.k8s.io/pause:latest` - 帶有自定義鏡像倉庫和 `latest` 標籤的鏡像名稱。
- `registry.k8s.io/pause:3.5` - 帶有自定義鏡像倉庫和非 `latest` 標籤的鏡像名稱。
- `registry.k8s.io/pause@sha256:1ff6c18fbef2045af6b9c16bf034cc421a29027b800e4f9b68ae9b1cb3e9ae07` - 帶摘要的鏡像名稱。
- `registry.k8s.io/pause:3.5@sha256:1ff6c18fbef2045af6b9c16bf034cc421a29027b800e4f9b68ae9b1cb3e9ae07` - 帶有標籤和摘要的鏡像名稱，鏡像拉取僅參考摘要。
...
Pod 中所有容器的默認鏡像拉取策略將被設置爲 `IfNotPresent`。這一策略會使得
{{< glossary_tooltip text="kubelet" term_id="kubelet" >}}
在鏡像已經存在的情況下直接略過拉取鏡像的操作。
...
these values have:
-->
### 鏡像拉取策略   {#image-pull-policy}

容器的 `imagePullPolicy` 和鏡像的標籤會影響
...
`Always`
: 每當 kubelet 啓動一個容器時，kubelet 會查詢容器的鏡像倉庫，
  將名稱解析爲一個鏡像[摘要](https://docs.docker.com/engine/reference/commandline/pull/#pull-an-image-by-digest-immutable-identifier)。
  如果 kubelet 有一個容器鏡像，並且對應的摘要已在本地緩存，kubelet 就會使用其緩存的鏡像；
  否則，kubelet 就會使用解析後的摘要拉取鏡像，並使用該鏡像來啓動容器。

`Never`
: kubelet 不會嘗試獲取鏡像。如果鏡像已經以某種方式存在本地，
  kubelet 會嘗試啓動容器；否則，會啓動失敗。
  更多細節見[提前拉取鏡像](#pre-pulled-images)。
...
-->
只要能夠可靠地訪問鏡像倉庫，底層鏡像提供者的緩存語義甚至可以使 `imagePullPolicy: Always` 高效。
你的容器運行時可以注意到節點上已經存在的鏡像層，這樣就不需要再次下載。
...
-->
爲了確保 Pod 總是使用相同版本的容器鏡像，你可以指定鏡像的摘要；
...
-->
當使用鏡像標籤時，如果鏡像倉庫修改了代碼所對應的鏡像標籤，可能會出現新舊代碼混雜在 Pod 中運行的情況。
鏡像摘要唯一標識了鏡像的特定版本，因此 Kubernetes 每次啓動具有指定鏡像名稱和摘要的容器時，都會運行相同的代碼。
通過摘要指定鏡像可固定你運行的代碼，這樣鏡像倉庫的變化就不會導致版本的混雜。

有一些第三方的[准入控制器](/zh-cn/docs/reference/access-authn-authz/admission-controllers/)
在創建 Pod（和 PodTemplate）時產生變更，這樣運行的工作負載就是根據鏡像摘要，而不是標籤來定義的。
無論鏡像倉庫上的標籤發生什麼變化，你都想確保你的整個工作負載都運行相同的代碼，那麼指定鏡像摘要會很有用。
...
-->
#### 默認鏡像拉取策略    {#imagepullpolicy-defaulting}

當你（或控制器）向 API 服務器提交一個新的 Pod 時，你的集羣會在滿足特定條件時設置 `imagePullPolicy` 字段：
...
- if you omit the `imagePullPolicy` field, and you specify a tag for the container
  ...
  `IfNotPresent`.
-->
- 如果你省略了 `imagePullPolicy` 字段，並且你爲容器鏡像指定了摘要，
  那麼 `imagePullPolicy` 會自動設置爲 `IfNotPresent`。
- 如果你省略了 `imagePullPolicy` 字段，並且容器鏡像的標籤是 `:latest`，
  `imagePullPolicy` 會自動設置爲 `Always`。
- 如果你省略了 `imagePullPolicy` 字段，並且沒有指定容器鏡像的標籤，
  `imagePullPolicy` 會自動設置爲 `Always`。
- 如果你省略了 `imagePullPolicy` 字段，並且爲容器鏡像指定了非 `:latest` 的標籤，
...
如果後來鏡像的標籤或摘要發生變化，則不會更新。

例如，如果你用一個**非** `:latest` 的鏡像標籤創建一個 Deployment，
並在隨後更新該 Deployment 的鏡像標籤爲 `:latest`，則 `imagePullPolicy` 字段**不會**變成 `Always`。
你必須手動更改已經創建的資源的拉取策略。
{{< /note >}}
...
因爲 Kubernetes 無法拉取容器鏡像（原因包括無效的鏡像名稱，或從私有倉庫拉取而沒有 `imagePullSecret`）。
`BackOff` 部分表示 Kubernetes 將繼續嘗試拉取鏡像，並增加回退延遲。
...
-->
### 基於運行時類的鏡像拉取  {#image-pull-per-runtime-class}
...
-->
Kubernetes 包含了根據 Pod 的 RuntimeClass 來執行鏡像拉取的 Alpha 支持。
...
kubelet 會通過一個由鏡像名稱和運行時處理程序構成的元組而不僅僅是鏡像名稱或鏡像摘要來引用容器鏡像。
你的{{< glossary_tooltip text="容器運行時" term_id="container-runtime" >}}可能會根據選定的運行時處理程序調整其行爲。
基於運行時類來拉取鏡像對於 Windows Hyper-V 容器這類基於 VM 的容器會有幫助。

<!--
## Serial and parallel image pulls
-->
## 串行和並行鏡像拉取  {#serial-and-parallel-image-pulls}
...
-->
默認情況下，kubelet 以串行方式拉取鏡像。
也就是說，kubelet 一次只向鏡像服務發送一個鏡像拉取請求。
其他鏡像拉取請求必須等待，直到正在處理的那個請求完成。
...
-->
節點獨立地做出鏡像拉取的決策。即使你使用串行的鏡像拉取，兩個不同的節點也可以並行拉取相同的鏡像。
...
-->
如果你想啓用並行鏡像拉取，可以在 [kubelet 配置](/zh-cn/docs/reference/config-api/kubelet-config.v1beta1/)
中將字段 `serializeImagePulls` 設置爲 false。
當 `serializeImagePulls` 設置爲 false 時，kubelet 會立即向鏡像服務發送鏡像拉取請求，多個鏡像將同時被拉動。
...
-->
啓用並行鏡像拉取時，確保你的容器運行時的鏡像服務可以處理並行鏡像拉取。
...
-->
kubelet 從不代表一個 Pod 並行地拉取多個鏡像。
例如，如果你有一個 Pod，它有一個初始容器和一個應用容器，那麼這兩個容器的鏡像拉取將不會並行。
但是，如果你有兩個使用不同鏡像的 Pod，且啓用並行鏡像拉取特性時，kubelet 會代表兩個不同的 Pod 並行拉取鏡像。

<!--
### Maximum parallel image pulls
-->
### 最大並行鏡像拉取數量  {#maximum-parallel-image-pulls}
...
-->
當 `serializeImagePulls` 被設置爲 false 時，kubelet 默認對同時拉取的最大鏡像數量沒有限制。
如果你想限制並行鏡像拉取的數量，可以在 kubelet 配置中設置字段 `maxParallelImagePulls`。
當 `maxParallelImagePulls` 設置爲 **n** 時，只能同時拉取 **n** 個鏡像，
超過 **n** 的任何鏡像都必須等到至少一個正在進行拉取的鏡像拉取完成後，才能拉取。
...
-->
當啓用並行鏡像拉取時，限制並行鏡像拉取的數量來防止鏡像拉取消耗過多的網絡帶寬或磁盤 I/O。
...
-->
## 帶鏡像索引的多架構鏡像  {#multi-architecture-images-with-image-indexes}

除了提供二進制的鏡像之外，
容器倉庫也可以提供[容器鏡像索引](https://github.com/opencontainers/image-spec/blob/master/image-index.md)。
鏡像索引可以指向鏡像的多個[鏡像清單](https://github.com/opencontainers/image-spec/blob/master/manifest.md)，
提供特定於體系結構版本的容器。
這背後的理念是讓你可以爲鏡像命名（例如：`pause`、`example/mycontainer`、`kube-apiserver`）
的同時，允許不同的系統基於它們所使用的機器體系結構獲取正確的二進制鏡像。
...
-->
Kubernetes 項目通常在命名容器鏡像時添加後綴 `-$(ARCH)`。
爲了向前兼容，在生成較老的鏡像時也提供後綴。
例如，名爲 `pause` 的鏡像是一個多架構鏡像，包含所有受支持架構的鏡像清單；
而 `pause-amd64` 是一個向後兼容的版本，用於舊的配置，
或用於 YAML 文件中硬編碼了帶後綴鏡像名稱的情況。
...
從私有倉庫讀取鏡像時可能需要發現和/或拉取鏡像的身份認證。憑據可以用以下方式提供:
...
- [配置節點向私有倉庫進行身份驗證](#configuring-nodes-to-authenticate-to-a-private-registry)
  - 所有 Pod 均可讀取任何已配置的私有倉庫。
  - 需要集羣管理員配置節點。
- 使用 **kubelet 憑據提供程序** [動態獲取私有倉庫的憑據](#kubelet-credential-provider)
  
  kubelet 可以被配置爲使用憑據提供程序 exec 插件來訪問對應的私有鏡像庫。
...
-->
- [預拉鏡像](#pre-pulled-images)
  - 所有 Pod 都可以使用節點上緩存的所有鏡像。
  - 需要所有節點的 root 訪問權限才能進行設置。
- 特定於廠商的擴展或者本地擴展
  
  如果你在使用定製的節點配置，你（或者雲平臺提供商）可以實現讓節點向容器倉庫認證的機制。
...
-->
Kubernetes 支持在 Pod 中設置容器鏡像倉庫的密鑰。所有 `imagePullSecrets`
...
-->
### 配置 Node 對私有倉庫認證  {#configuring-nodes-to-authenticate-to-a-private-registry}
...
-->
有關配置私有容器鏡像倉庫的示例，
請參閱任務[從私有鏡像庫中拉取鏡像](/zh-cn/docs/tasks/configure-pod-container/pull-image-private-registry)。
該示例使用 Docker Hub 中的私有鏡像倉庫。

<!--
### Kubelet credential provider for authenticated image pulls {#kubelet-credential-provider}
-->
### 用於認證鏡像拉取的 kubelet 憑據提供程序  {#kubelet-credential-provider}
...
-->
你可以配置 kubelet，以調用插件可執行文件的方式來動態獲取容器鏡像的倉庫憑據。
這是爲私有倉庫獲取憑據最穩健和最通用的方法，但也需要 kubelet 級別的配置才能啓用。

這種技術在運行依賴私有倉庫中容器鏡像的{{< glossary_tooltip term_id="static-pod" text="靜態 Pod" >}}
時尤其有用。在靜態 Pod 的規約中，不能使用 {{< glossary_tooltip term_id="service-account" >}}
或 {{< glossary_tooltip term_id="secret" >}} 來提供私有鏡像倉庫的憑據，因爲它**不能**在規約中引用其他 API 資源。

有關更多細節請參見[配置 kubelet 鏡像憑據提供程序](/zh-cn/docs/tasks/administer-cluster/kubelet-credential-provider/)。
...
successfully:
-->
鏡像拉取操作將每種有效模式的憑據都傳遞給 CRI 容器運行時。例如下面的容器鏡像名稱會匹配成功：
...
-->
kubelet 爲每個找到的憑據的鏡像按順序拉取。這意味着對於不同的路徑在 `config.json` 中也可能有多項：
...
-->
如果一個容器指定了要拉取的鏡像 `my-registry.io/images/subpath/my-image`，
並且其中一個失敗，kubelet 將嘗試同時使用兩個身份驗證源下載鏡像。

<!--
### Pre-pulled images
-->
### 提前拉取鏡像   {#pre-pulled-images}
...
-->
默認情況下，`kubelet` 會嘗試從指定的倉庫拉取每個鏡像。
但是，如果容器屬性 `imagePullPolicy` 設置爲 `IfNotPresent` 或者 `Never`，
則會優先使用（對應 `IfNotPresent`）或者一定使用（對應 `Never`）本地鏡像。
...
-->
如果你希望使用提前拉取鏡像的方法代替倉庫認證，就必須保證集羣中所有節點提前拉取的鏡像是相同的。

這一方案可以用來提前載入指定的鏡像以提高速度，或者作爲向私有倉庫執行身份認證的一種替代方案。
...
預拉取鏡像也適用於啓動依賴私有倉庫中鏡像的{{< glossary_tooltip text="靜態 Pod" term_id="static-pod" >}}。
...
-->
對預拉取鏡像的訪問可能需要根據[鏡像拉取憑據驗證](#ensureimagepullcredentialverification)進行授權。
{{< /note >}}

<!--
#### Ensure image pull credential verification {#ensureimagepullcredentialverification}
-->
#### 鏡像拉取憑據驗證   {#ensureimagepullcredentialverification}
...
-->
如果爲你的集羣啓用了 `KubeletEnsureSecretPulledImages` 特性門控，Kubernetes
將驗證每個需要憑據才能拉取的鏡像的憑據，即使該鏡像已經存在於節點上。
此驗證確保了在 Pod 請求中未成功使用提供的憑據拉取的鏡像必須從鏡像倉庫重新拉取。
此外，若之前使用相同的憑據已成功拉取過鏡像，
則再次使用這些憑據的鏡像拉取操作將不需要從鏡像倉庫重新拉取，
而是通過本地驗證（前提是鏡像在本地可用）而無需訪問鏡像倉庫。這由
...
-->
此配置控制在鏡像已經存在於節點上時，何時必須驗證鏡像拉取憑據：
...
* `NeverVerify`：模仿關閉此特性門控的行爲。
  如果鏡像本地存在，則不會驗證鏡像拉取憑據。

* `NeverVerifyPreloadedImages`：在 kubelet 外部拉取的鏡像不會被驗證，
  但所有其他鏡像都將驗證其憑據。這是默認行爲。

* `NeverVerifyAllowListedImages`：在 kubelet 外部拉取且列在
  kubelet 配置中的 `preloadedImagesVerificationAllowlist` 裏的鏡像不會被驗證。

* `AlwaysVerify`：所有鏡像在使用前都必須驗證其憑據。
...
-->
這種驗證適用於[預拉取鏡像](#pre-pulled-images)、
使用節點範圍的密鑰拉取的鏡像以及使用 Pod 級別密鑰拉取的鏡像。
...
-->
在憑據輪換的情況下，之前用於拉取鏡像的憑據將繼續驗證，
而無需訪問鏡像倉庫新的或已輪換的憑據將要求從鏡像倉庫重新拉取鏡像。
{{< /note >}}
...
你需要知道用於向倉庫進行身份驗證的用戶名、密碼和客戶端電子郵件地址，以及它的主機名。
運行以下命令，注意用合適的值替換佔位符：
...
-->
1. 集羣運行非專有鏡像（例如，開源鏡像）。鏡像不需要隱藏。
   - 使用來自公共倉庫的公共鏡像
     - 無需配置
     - 某些雲廠商會自動爲公開鏡像提供高速緩存，以便提升可用性並縮短拉取鏡像所需時間
...
-->
2. 集羣運行一些專有鏡像，這些鏡像需要對公司外部隱藏，對所有集羣用戶可見
   - 使用託管的私有倉庫
     - 在需要訪問私有倉庫的節點上可能需要手動配置
   - 或者，在防火牆內運行一個組織內部的私有倉庫，並開放讀取權限
     - 不需要配置 Kubernetes
   - 使用控制鏡像訪問的託管容器鏡像倉庫服務
     - 與手動配置節點相比，這種方案能更好地處理節點自動擴縮容
   - 或者，在不方便更改節點配置的集羣中，使用 `imagePullSecrets`
...
- 在每個節點上配置一個 kubelet 鏡像憑據提供程序；或
- 使用 `imagePullSecrets` 和至少一個 Secret 指定鏡像拉取憑據。
...
* 瞭解[容器鏡像垃圾收集](/zh-cn/docs/concepts/architecture/garbage-collection/#container-image-garbage-collection)。
* 瞭解[從私有倉庫拉取鏡像](/zh-cn/docs/tasks/configure-pod-container/pull-image-private-registry)。
