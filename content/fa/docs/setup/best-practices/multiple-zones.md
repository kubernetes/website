...
هنگامی که یک control plane کلاستر‌ی را مستقر می‌کنید، کپی‌هایی از اجزای control plane را در چندین منطقه خرابی قرار دهید. اگر در دسترس بودن
یک نگرانی مهم است، حداقل سه منطقه خرابی را انتخاب کنید و هر جزء control plane (API server, scheduler, etcd,
...
را روی گره‌های مختلف یک کلاستر توزیع می‌کند. این توزیع، اثر خرابی‌ها را کاهش می‌دهد.
...
اگر کلاستر شما چندین ناحیه یا منطقه را پوشش می‌دهد، می‌توانید برچسب‌های گره را
به‌همراه
...
چطور در کلاستر پخش شوند. این راهنماها به
...
تا به‌عنوان گره‌های worker کلاستر در چندین دامنه خطا اجرا شوند، و همچنین قوانینی
برای ترمیم خودکار کلاستر در صورت اختلال کامل یک ناحیه وضع کنید.
...
نواحی خطای مختلف—بسته به چگونگی پیکربندی کلاستر شما متفاوت است.

## بازیابی خطا

هنگامی که کلاستر خود را راه‌اندازی می‌کنید، ممکن است لازم باشد بررسی کنید
...
اطمینان حاصل کنید که هر کار تعمیر حیاتی کلاستر به وجود دست‌کم یک گره سالم
...
برای آشنایی با نحوه قرار دادن پادها در کلاستر توسط scheduler با رعایت محدودیت‌های پیکربندی‌شده،
به صفحه [زمانبندی(Scheduling) و تخلیه(Eviction)](/docs/concepts/scheduling-eviction/) مراجعه کنید.
