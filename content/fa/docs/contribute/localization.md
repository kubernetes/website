...
ابتدا، [شاخه‌ی خودتان را ایجاد کنید](https://kubernetes.io/docs/contribute/new-content/open-a-pr/#fork-the-repo) از مخزن [kubernetes/website](https://github.com/kubernetes/website).
...
برای پیشنهاد تغییرات در آن بومی‌سازی، از [پیشنهاد بهبود محتوا](https://kubernetes.io/docs/contribute/suggesting-improvements/) پیروی کنید. این فرآیند مشابه پیشنهاد تغییرات در محتوای بالادستی (انگلیسی) است.
...
در مرحله بعد، [یک درخواست ادغام را باز کنید](https://kubernetes.io/docs/contribute/new-content/open-a-pr/#open-a-pr) (درخواست ادغام) برای افزودن محلی سازی به مخزن «kubernetes/website». درخواست ادغام قبل از تأیید باید شامل همه [حداقل محتوای مورد نیاز](#حداقل-محتوای-مورد-نیاز) باشد.
...
- در دسترس بودن محلی‌سازی را از طریق کانال‌های [بنیاد رایانش ابری بومی](https://www.cncf.io/about/)(CNCF)، از جمله [وب نوشت کوبرنتیز](https://kubernetes.io/blog/) منتشر می‌کند.
...
-----|-----
خانه | [همه نشانی‌های اینترنتی (URL) عنوان و زیرعنوان](https://kubernetes.io/docs/home/)
نصب | [همه نشانی‌های اینترنتی (URL) عنوان و زیرعنوان](https://kubernetes.io/docs/setup/)
آموزش‌ها | [مبانی کوبرنتیز](https://kubernetes.io/docs/tutorials/kubernetes-basics/), [سلام Minikube](https://kubernetes.io/docs/tutorials/hello-minikube/)
رشته‌های سایت | [تمام رشته‌های تارنما](#رشته‌های-سایت-در-i18n) در یک پرونده TOML جدید و بومی‌سازی‌شده
انتشارات | [همه نشانی‌های اینترنتی (URL) عنوان و زیرعنوان](https://kubernetes.io/releases/)

اسناد ترجمه شده باید در زیرشاخه `content/**/` خود قرار گیرند، اما در غیر این صورت، همان مسیر URL منبع انگلیسی را دنبال کنند. به عنوان مثال، برای تهیه آموزش [Kubernetes Basics](https://kubernetes.io/docs/tutorials/kubernetes-basics/) برای ترجمه به آلمانی، یک زیرشاخه در زیر شاخه `content/de/` ایجاد کنید و منبع یا پوشه انگلیسی را رونوشت کنید:
...
1. **بررسی و آزمایش**: پس از انجام ترجمه‌های لازم و تبدیل متن به منحنی، تصویر SVG به‌روزرسانی‌شده را ذخیره و بررسی کنید تا از نمایش و ترازبندی صحیح متن اطمینان حاصل شود. [پیش‌نمایش تغییرات محلی](https://kubernetes.io/docs/contribute/new-content/open-a-pr/#preview-locally) را بررسی کنید.
...
نسخه قبلی | [`release-{{< skew prevMinorVersion >}}`](https://github.com/kubernetes/website/tree/release-1.32)
نسخه بعدی | [`dev-{{< skew nextMinorVersion >}}`](https://github.com/kubernetes/website/tree/dev-1.34)
...
برای مثال، به [راهنمای محلی‌سازی زبان کره‌ای](https://kubernetes.io/ko/docs/contribute/localization_ko/) مراجعه کنید که شامل مطالبی در مورد موضوعات زیر است:
