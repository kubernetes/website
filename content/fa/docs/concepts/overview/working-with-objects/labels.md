...
برچسب‌ها برای مشخص‌کردن ویژگی‌های شناسایی‌کننده اشیاء طراحی شده‌اند که برای کاربران معنادار و مرتبط‌اند، اما مستقیماً معنایی به سیستم هسته‌ای اضافه نمی‌کنند.  
...
برچسب‌ها به کاربران اجازه می‌دهند ساختارهای سازمانی خود را به‌صورت اتصال سست (loosely coupled) بر روی اشیاء سیستم نگاشت کنند، بدون آن‌که کلاینت‌ها نیاز به ذخیره این نگاشت‌ها داشته باشند.
...
_برچسب‌ها_ نگاشت‌های کلید/مقدار هستند. کلیدهای معتبر برچسب دو بخش دارند: یک پیشوند اختیاری و یک نام که با اسلش (`/`) از هم جدا می‌شوند. بخش نام الزامی است و باید حداکثر ۶۳ نویسه باشد؛ نام باید با یک نویسه الفانامریک (`[a-z0-9A-Z]`) شروع و پایان یابد و می‌تواند در میان شامل خط تیره (`-`)، زیرخط (`_`)، نقطه (`.`) و نویسه‌های الفانامریک باشد. پیشوند اختیاری است. اگر مشخص شود، باید یک زیر‌دامنه DNS باشد: مجموعه‌ای از برچسب‌های DNS که با نقطه (`.`) از هم جدا شده‌اند، مجموعاً بیشتر از ۲۵۳ نویسه نباشند و در پایان با اسلش (`/`) دنبال شوند.
...
* اگر خالی نباشد، باید با یک نویسه الفانامریک (`[a-z0-9A-Z]`) شروع و پایان یابد،  
...
در حال حاضر API دو نوع انتخابگر را پشتیبانی می‌کند: _مبتنی بر برابری_ و _مبتنی بر مجموعه_. یک انتخابگر برچسب می‌تواند از چند _الزام_ تشکیل شده باشد که با ویرگول از هم جدا شده‌اند. در صورت وجود چند الزام، همه آن‌ها باید برآورده شوند، بنابراین ویرگول نقش عملگر منطقی _AND_ (`&&`) را دارد.
...
یکی از سناریوهای کاربردی برای الزام مبتنی بر برابری، مشخص کردن معیارهای انتخاب نود برای پادها است. برای مثال، پاد نمونه زیر نودهایی را انتخاب می‌کند که برچسب `accelerator` در آن‌ها وجود دارد و مقدارش `nvidia-tesla-p100` است.
...
الزام‌های برچسب مبتنی بر _مجموعه_ امکان فیلتر کردن کلیدها بر اساس یک مجموعه از مقادیر را فراهم می‌کنند. سه نوع عملگر پشتیبانی می‌شوند: `in`، `notin` و `exists` (فقط شناسه کلید). برای مثال:
...
به‌نحوی مشابه، جداکننده ویرگول نقش عملگر _AND_ را دارد. بنابراین فیلتر کردن منابعی که کلید `partition` دارند (صرف‌نظر از مقدار) و مقادیر `environment` آن‌ها متفاوت از `qa` باشد، با عبارت زیر امکان‌پذیر است:  
...
برای عملیات **لیست** و **واچ**، می‌توانید انتخابگرهای برچسب را برای فیلتر کردن مجموعه اشیاء بازگردانده‌شده مشخص کنید؛ این فیلتر را با یک پارامتر پرس‌وجو تعیین می‌کنید.  
...
(در اینجا همان‌طور که در رشته پرس‌وجوی URL ظاهر می‌شوند نشان داده شده‌اند):
...
مجموعه پادهایی که یک `service` به آن‌ها هدف‌گذاری می‌کند با یک انتخابگر برچسب تعریف می‌شود.  
به‌طور مشابه، مجموعه پادهایی که یک `replicationcontroller` باید آن‌ها را مدیریت کند نیز با انتخابگر برچسب مشخص می‌شود.
...
یکی از کاربردهای انتخاب بر اساس برچسب، محدود کردن مجموعه نودهایی است  
...
اما یک برنامه چندلایه مانند [مثال guestbook](https://github.com/kubernetes/examples/tree/master/guestbook/)  
نیاز دارد که هر لایه را نیز از هم متمایز کند.  
لایه frontend می‌تواند برچسب‌های زیر را داشته باشد:
...
برای مثال، اگر بخواهید همه پادهای NGINX خود را به‌عنوان لایه frontend برچسب‌گذاری کنید، این دستور را اجرا کنید:
...
(که با گزینه `-L` یا `--label-columns` مشخص می‌شود).
...
- مقاله‌ای درباره [نوشتن کنترلر برای برچسب‌های پاد](/blog/2021/06/21/writing-a-controller-for-pod-labels/) بخوانید
