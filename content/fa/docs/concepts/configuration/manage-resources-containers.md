...
{{< note >}}
تغییر اندازه درجا در حال حاضر برای منابع سطح کانتینر اعمال می‌شود. برای تغییر اندازه منابع سطح پاد، به [تغییر اندازه‌ی منابع CPU و حافظه‌ی Pod](/docs/tasks/configure-pod-container/resize-pod-resources/). مراجعه کنید.
{{< /note >}}
...
برای جزئیات بیشتر درباره تغییر اندازه پاد، به [تغییر اندازه‌ی Podها](/docs/concepts/workloads/pods/pod-lifecycle/#pod-resize). مراجعه کنید.
...
همچنین می‌توانید از [مقیاس‌بند خودکار عمودی Pod](/docs/concepts/workloads/autoscaling/vertical-pod-autoscale/) برای مدیریت خودکار توصیه‌های منابع Pod استفاده کنید.
...
اگر [ابزارهای نظارت](/docs/tasks/debug/debug-cluster/resource-usage-monitoring/) در کلاستر شما موجود باشد، میزان استفاده از منابع Pod را می‌توان یا مستقیماً از [API معیارها](/docs/tasks/debug/debug-cluster/resource-metrics-pipeline/#metrics-api) یا از ابزارهای نظارتی خود بازیابی کرد.
...
* هدف از نوشتن فایل‌ها در یک volume، ذخیره داده‌ها یا انتقال آن بین برنامه‌ها است. نه کوبرنتیز و نه سیستم عامل ممکن است به طور خودکار فایل‌ها را از یک volume حذف کنند، بنابراین حافظه استفاده شده توسط آن فایل‌ها زمانی که سیستم یا pod تحت فشار حافظه هستند، قابل بازیابی نیست.
...
تخصیص منابع توسعه‌یافته توسط DRA به مدیران کلاستر اجازه می‌دهد تا یک `extendedResourceName` را در DeviceClass مشخص کنند، سپس دستگاه‌های منطبق با DeviceClass می‌توانند از درخواست‌های منابع توسعه‌یافته یک pod درخواست شوند. درباره [تخصیص منابع گسترش‌یافته (Extended Resource) توسط DRA](/docs/concepts/scheduling-eviction/dynamic-resource-allocation/#extended-resource).
 بیشتر بخوانید.
=
...
برای کسب اطلاعات بیشتر درباره منابع قابل تخصیص نود در کوبرنتیز، به بخش
...
* تجربه عملی [اختصاص منابع CPU به کانتینرها و Podها](/docs/tasks/configure-pod-container/assign-cpu-resource/).
...
*  تجربه عملی  [کلاس‌های کیفیت سرویس (Quality of Service) برای Podها](/docs/concepts/workloads/pods/pod-qos/)
