...
Pod Security یک کنترل‌کنندهٔ admission است که هنگام ایجاد پادهای جدید، بررسی‌هایی را بر اساس
[استانداردهای امنیت پاد](/docs/concepts/security/pod-security-standards/) کوبرنتیز انجام می‌دهد.
این قابلیت در نسخهٔ v1.25 به GA رسیده است.  
این راهنما به شما نشان می‌دهد چگونه استاندارد امنیت پاد `baseline` را در سطح کلاستر اجرا کنید
تا یک پیکربندی یکسان به همهٔ namespaceهای کلاستر اعمال شود.
...
این راهنما پیکربندی‌هایی را نشان می‌دهد که برای کلاستری از Kubernetes که کنترل کامل آن را در دست دارید
می‌توانید انجام دهید. اگر قصد دارید Pod Security Admission را برای کلاستر مدیریت‌شده‌ای پیکربندی کنید
...
1. یک کلاستر بدون اعمال هیچ استاندارد امنیت پاد ایجاد کنید:
...
در این بخش، استانداردهای امنیت پاد زیر را بر روی نسخهٔ `latest` اعمال می‌کنید:

* استاندارد `baseline` در حالت `enforce`
* استاندارد `restricted` در حالت‌های `warn` و `audit`

استاندارد امنیت پاد `baseline` یک میانهٔ مناسب فراهم می‌کند که
...
1. بر اساس وضعیت ریسک در یک کلاستر، ممکن است استاندارد سخت‌گیرانه‌ترِ
   `restricted` انتخاب بهتری باشد.
1. مستثناکردن namespace‌ `kube-system` اجازه می‌دهد پادها در این فضا با
   حالت `privileged` اجرا شوند. در کاربردهای واقعی، پروژهٔ Kubernetes
...
1. یک فایل پیکربندی ایجاد کنید که کنترلر Pod Security Admission بتواند از آن
   ...
   {{< note >}}
پیکربندی `pod-security.admission.config.k8s.io/v1` نیازمند نسخهٔ v1.25+ است.  
...
حالا با اجرای دستور زیر، کلاسترهایی که در بالا ایجاد کردید را حذف کنید:
...
- اجرای یک
  ...
  برای انجام همهٔ مراحل فوق به‌صورت یک‌جا:
  1. ایجاد یک پیکربندی سطح کلاستر بر پایهٔ استانداردهای امنیت پاد
  2. ایجاد فایلی تا سرور API بتواند این پیکربندی را مصرف کند
  3. ایجاد کلاستری که یک سرور API با این پیکربندی راه‌اندازی می‌کند
  4. تنظیم context ابزار `kubectl` روی این کلاستر جدید
  5. ایجاد یک فایل YAML حداقلی برای پاد
  6. اعمال این فایل برای ایجاد یک پاد در کلاستر جدید
