...
به پروژه‌ها، تیم‌ها یا مشتریان مختلف کمک کنید تا خوشه کوبرنتیز  را به اشتراک بگذارند.

این کار را با ارائه موارد زیر انجام می‌دهد:

1. یک محدوده برای [نام‌ها](/docs/concepts/overview/working-with-objects/names/).
2. سازوکاری برای ضمیمه کردن مجوز و سیاست به یک زیربخش از خوشه.

استفاده از چندین فضای نام اختیاری است.

این مثال نحوه استفاده از فضاهای نام کوبرنتیز برای تقسیم‌بندی خوشه شما را نشان می‌دهد.
...
به طور پیش‌فرض، یک خوشه کوبرنتیز هنگام آماده‌سازی خوشه، یک فضای نام پیش‌فرض را نمونه‌سازی می‌کند تا مجموعه پیش‌فرض پادها، سرویس‌ها و استقرارها مورد استفاده توسط خوشه را در خود جای دهد.

با فرض اینکه یک خوشه جدید دارید، می‌توانید با انجام موارد زیر، فضاهای نام موجود را بررسی کنید:
...
برای این تمرین، دو فضای نام کوبرنتیز اضافی برای نگهداری محتوای خود ایجاد خواهیم کرد.

بیایید سناریویی را تصور کنیم که در آن یک سازمان از یک خوشه مشترک کوبرنتیز برای موارد استفاده توسعه و تولید استفاده می‌کند.

تیم توسعه می‌خواهد فضایی را در خوشه حفظ کند که در آن بتوانند فهرست  پادها، سرویس‌ها و  استقرارهایی را که برای ساخت و اجرای برنامه خود استفاده می‌کنند، مشاهده کنند. در این فضا، منابع کوبرنتیز می‌آیند و می‌روند و محدودیت‌های مربوط به اینکه چه کسی می‌تواند یا نمی‌تواند منابع را تغییر دهد، کاهش می‌یابد تا توسعه چابک امکان‌پذیر شود.

تیم عملیات مایل است فضایی را در خوشه حفظ کند که در آن بتواند رویه‌های سختگیرانه‌ای را در مورد اینکه چه کسی می‌تواند یا نمی‌تواند مجموعه پادها، سرویس‌ها و استقرارهایی را که وبگاه عملیاتی را اداره می‌کنند، دستکاری کند، اعمال کند.

یکی از الگوهایی که این سازمان می‌تواند دنبال کند، تقسیم‌بندی خوشه کوبرنتیز به دو فضای نام است: «توسعه» و «تولید».
...
برای اطمینان از درست بودن همه چیز، بیایید تمام فضاهای نام موجود در خوشه خود را فهرست کنیم.
...
فضای نام  کوبرنتیز، محدوده‌ی  پادها، سرویس‌ها و  استقرارها را در خوشه فراهم می‌کند.
...
مرحله بعدی تعریف یک زمینه برای کلاینت kubectl است تا در هر فضای نام کار کند. مقدار فیلدهای "cluster" و "user" از زمینه فعلی کپی می‌شود.
...
به‌طور پیش‌فرض، دستورات بالا دو زمینه را اضافه می‌کنند که در پرونده «.kube/config» ذخیره می‌شوند. اکنون می‌توانید زمینه‌ها را مشاهده کنید و بسته به فضای نامی که می‌خواهید با آن کار کنید، متناوب با دو زمینه درخواستی جدید جایگزین کنید.

برای مشاهده‌ی زمینه‌های جدید:
...
شما می‌توانید با انجام موارد زیر، زمینه فعلی خود را تأیید کنید:
...
در این مرحله، تمام درخواست‌هایی که از خط فرمان به خوشه کوبرنتیز ارسال می‌کنیم، در فضای نام `development` قرار می‌گیرند.
...
```
ما یک استقرار با اندازه کپی ۲ عدد پاد ایجاد کرده‌ایم که پادی به نام «دانه برف» را با یک کانتینر پایه که نام میزبان را ارائه می‌دهد، اجرا می‌کند.
```shell
