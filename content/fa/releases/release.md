...
این سند بر توسعه‌دهندگان و مشارکت‌کنندگان کوبرنتیز متمرکز است که نیاز به ایجاد یک بهبود، مشکل یا درخواست pull دارند که یک نقطه عطف انتشار خاص را هدف قرار می‌دهد.
...
- [اضافه کردن یک مورد به مرحله‌ی مهم](#adding-an-item-to-the-milestone)
  ...
  - [افزودنی‌های Issue](#issue-additions)
  - [موارد افزوده‌شده در Pull Request](#pr-additions)
- [سایر برچسب های مورد نیاز](#other-required-labels)
  - [برچسب مالک SIG](#sig-owner-label)
  - [برچسب اولویت](#priority-label)
  - [برچسب نوع Issue/PR](#issuepr-kind-label)

فرآیند هدایت بهبودها، مشکلات و درخواست‌های pull به یک نسخه کوبرنتیز، ذینفعان متعددی را در بر می‌گیرد:

- افزایش تعداد issue , PR  های  مالکان  
- رهبران  SIG
...
مسئولیت اطمینان از برآورده شدن الزامات مربوط به نقاط عطف انتشار. در صورت نیاز به به‌روزرسانی، تیم اتوماسیون و انتشار با شما تماس خواهند گرفت., اما عدم اقدام می‌تواند منجر به حذف کار شما از مرحله‌ی مهم شود..
الزامات اضافی زمانی وجود دارند که نقطه عطف هدف، یک نسخه قبلی باشد.مشاهده کنید لینک پیوست شده را 
...
- *release-team*: هر نسخه کوبرنتیز دارای تیمی است که مدیریت پروژه را انجام می‌دهد.
...
- *افزایش*: ببینید "[آیا «My Thing» یک پیشرفت است؟](https://git.k8s.io/enhancements/README.md#is-my-thing-an-enhancement)"
...
توجه: نسخه‌های ۱.۱۹ و جدیدتر، ۱ سال پشتیبانی انتشار (patch)وصله دریافت می‌کنند و نسخه‌های ۱.۱۸ و قبل از آن، ۹ ماه پشتیبانی انتشار وصله دریافت می‌کنند.

## چرخه انتشار

![image یک چرخه انتشار کوبرنتیز](/images/releases/release-cycle.jpg)

انتشارهای کوبرنتیز در حال حاضر تقریباً سه بار در سال اتفاق می‌افتد.
...
پس از انجماد بهبودها (Enhancements Freeze)، پیگیری milestone‌ها milestones در Pull Requestها و Issueها اهمیت زیادی دارد. مواردی که درون یک milestone قرار دارند، به عنوان یک لیست انجام (punchdown list) برای تکمیل نسخه انتشار مورد استفاده قرار می‌گیرند.

در مورد Issueها، milestone‌ها باید به‌درستی اعمال شوند؛ این کار باید از طریق triage توسط گروه SIG انجام شود تا  تیم انتشار [تیم انتشارات][release-team] بتواند باگ‌ها و بهبودها را دنبال کند.
(هر Issue مرتبط با بهبود باید حتماً دارای یک milestone باشد.)
...
در زمان ایجاد، Pull Requestهایی که به شاخه `master` ارسال می‌شوند نیاز دارند که افراد به صورت دستی مشخص کنند که این PR باید به کدام milestone (milestone) اختصاص یابد.
پس از Merge شدن، این PRها به‌طور خودکار `master` دریافت می‌کنند و از آن به بعد نیازی به مدیریت دستی milestone توسط انسان نیست.

برای PRهایی که به شاخه‌های نسخه (release branches) ارسال می‌شوند، milestone‌ها به‌صورت خودکار در لحظه ایجاد PR اعمال می‌شوند؛ بنابراین هیچ‌گونه مدیریت دستی برای milestone در این موارد لازم نیست.


هر تلاش یا فعالیت دیگری که باید توسط تیم انتشار (Release Team) پیگیری شود اما تحت پوشش خودکارسازی قرار نمی‌گیرد، باید دارای یک milestone (milestone) مشخص باشد.
...
زمانی که code base به اندازه کافی پایدار شد، شاخه master برای توسعه عمومی باز می‌شود و کار روی milestone بعدی انتشار در آن آغاز می‌شود. هر تغییر باقی‌مانده برای انتشار فعلی از شاخه master به شاخه release به صورت cherry pick منتقل می‌شود. انتشار از شاخه release ساخته می‌شود.

هر release بخشی از چرخه‌ی عمر گسترده‌تر کوبرنتیز است:

![تصویر چرخه عمر انتشار کوبرنتیز که سه نسخه را پوشش می‌دهد](/images/releases/release-lifecycle.jpg)
...
- ثبت comment در GitHub با ذکر نام تیم SIG و اعضای SIG مربوطه بسته به نوع مسئله.
...
گره github این مسئولیت را بر عهده داردند که به‌روزرسانی ها و artifact های جدید را بروز رسانی کنند 
[نگهداری می شود](https://git.k8s.io/sig-release/release-team/README.md#milestone-maintainers) بررسی میشوند و اخرین به‌روزرسانی های leadership ها در این جا قرار می گیرند این گروه توسط 
...
رهبر تیم انتشار با عنوان [ارزیابی مشکلات توسط رهبر(leader)](https://git.k8s.io/sig-release/release-team/role-handbooks/bug-triage/README.md)و همچنین کل جامعه، مسائل ورودی را زیر نظر گرفته و آن‌ها را دسته‌بندی می‌کنند، همان‌طور که در بخش راهنمای مشارکت‌کنندگان درباره‌ی [یافتن مشکلات](https://k8s.dev/docs/guide/issue-triage/). توضیح داده شده است.
...
این برچسب‌ها با دستور /sig در Prow اضافه می‌شوند. به‌عنوان مثال، برای اضافه کردن برچسبی که نشان دهد SIG Storage مسئول است، کافی است commentی با متن /sig storage ارسال کنید.
...
- `kind/cleanup`: اضافه کردن تست‌ها، refactoring، رفع اشکالات قدیمی.
