...
نسخه‌های کوبرنتیز به صورت **x.y.z** نمایش داده می‌شوند، که در آن:

- **x** نسخه اصلی (major)
- **y** نسخه فرعی (minor)
- **z** نسخه پچ (patch)

این ساختار مطابق با استاندارد [Semantic Versioning](https://semver.org/) است.
...
{{< note >}}
اگر اختلاف نسخه (version skew) بین نمونه‌های `kube-apiserver` در یک کلاستر HAوجود داشته باشد، این موضوع دامنه نسخه‌های مجاز برای `kubelet` را محدود می‌کند.
{{</ note >}}
مثال:
...
* `kube-proxy` ممکن است تا سه نسخه minor قدیمی‌تر یا جدیدتر از `kubelet`نمونه ای  که در کنار آن اجرا می‌شود (برای مثال، `kube-proxy` نسخه‌های کمتر از 1.25 ممکن است حداکثر تا دو نسخه فرعی قدیمی‌تر یا جدیدتر از نمونه `kubelet` که کنار آن اجرا می‌شود باشد).
...
{{< note >}}
اگر اختلاف نسخه (version skew) بین نمونه‌های `kube-apiserver` در یک کلاستر HA وجود داشته باشد، این موضوع دامنه نسخه‌های مجاز برای `kube-proxy` را محدود می‌کند.
{{</ note >}}
...
{{< note >}}
اگر اختلاف نسخه (version skew) بین نمونه‌های `kube-apiserver` در یک کلاستر HA وجود داشته باشد، این موضوع نسخه‌های پشتیبانی شده kubectl را محدود می‌کند.
{{< /note >}}
...
## ترتیب ارتقاء اجزا پشتیبانی شده
...
در کلاستر های HA که این مولفه control plane می‌توانند با هر نمونه ای  از kube-apiserver در کلاستر ارتباط برقرار کنند،
...
می‌توانید این مولفه (component) را به هر ترتیبی که می‌خواهید یا حتی به‌صورت همزمان ارتقاء دهید.
...
{{< warning >}}
اجرای یک کلاستر با نمونه‌های `kube-proxy` که به‌طور مداوم سه نسخه جزئی از `kube-apiserver` عقب‌تر هستند، به این معنی است که آن‌ها باید قبل از ارتقاء control plane، به‌روزرسانی شوند.
{{</ warning >}}
