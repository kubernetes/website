...
Ви можете встановлювати та використовувати `kubeadm` на різних машинах: на вашому ноутбуці, хмарних серверах, Raspberry Pi та інших. Незалежно від того, чи ви розгортаєте у хмарі, чи на власних серверах, ви можете інтегрувати `kubeadm` у системи розгортання, такі як Ansible або Terraform.

## {{% heading "prerequisites" %}}

Для виконання цього керівництва вам потрібно мати:

- Одну чи кілька машин з операційною системою Linux, сумісною з deb/rpm, наприклад: Ubuntu чи CentOS.
...
IP-адреси, які ви призначаєте компонентам панелі управління, стають частиною полів альтернативного імені субʼєкта X.509 сертифіката. Зміна цих IP-адрес може вимагати підписання нових сертифікатів і перезапуску відповідних компонентів, щоб задіяти зміни у файлах сертифікатів. Дивіться [Ручне поновлення сертифікатів](/docs/tasks/administer-cluster/kubeadm/kubeadm-certs/#manual-certificate-renewal) для отримання докладнішої інформації з цього питання.
...
Kubeadm дозволяє вам використовувати власний репозиторій образів для необхідних образів. Дивіться [Використання власних образів](/docs/reference/setup-tools/kubeadm/kubeadm-init#custom-images) для отримання докладнішої інформації.
...
Хоча `--apiserver-advertise-address` може бути використано для встановлення оголошення адреси для API-сервера цього конкретного вузла панелі управління, `--control-plane-endpoint` може бути використано для встановлення загального endpoint для всіх вузлів управління.
...
Перетворення кластера з однією панеллю управління, створеного без `--control-plane-endpoint`, у високодоступний кластер не підтримується kubeadm.
...
Для налаштування компонентів панелі управління, включаючи необовʼязкове призначення IPv6 для перевірки справності для компонентів панелі управління та сервера etcd, надайте додаткові аргументи кожному компоненту, як описано на сторінці про [додаткові аргументи](/docs/setup/production-environment/tools/kubeadm/control-plane-flags/).
...
Щоб kubectl працював для вашого користувача без прав root, виконайте ці команди, які є також частиною виводу `kubeadm init`:
...
{{< caution >}}
Через контролер доступу `NodeRestriction`, ви **не можете** використовувати прапорець kubelet `--node-labels` для застосування обмежених міток (таких як `node-role.kubernetes.io/*`) під час ініціалізації.

Якщо ви спробуєте додати обмежені мітки за допомогою цього прапорця kubelet, вузол не зможе зареєструватися на сервері API.
{{< /caution >}}

Щоб застосувати ці мітки вручну, потрібно використовувати `kubectl label` після приєднання вузла до кластера. Переконайтеся, що ви використовуєте привілейований kubeconfig, такий як kubeadm-managed `/etc/kubernetes/admin.conf`.
...
Створений тут кластер має один вузол для панелі управління з однією базою даних etcd, яка працює на ньому. Це означає, що якщо вузол для панелі управління вийде з ладу, ваш кластер може втратити дані і може доведеться його перестворювати з нуля.
...
- [Інформація про SIG Cluster Lifecycle](https://github.com/kubernetes/community/tree/main/sig-cluster-lifecycle#readme)
