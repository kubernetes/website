...
{{< note >}}
Сертифікати `front-proxy` потрібні лише при використанні шару агрегації API-сервера для підтримки [розширеного API-сервера](/docs/tasks/extend-kubernetes/setup-extension-api-server/).
{{< /note >}}
...
На додачу до цих ЦС також необхідно отримати пару відкритого та закритого ключів для управління службовими обліковими записами, `sa.key` та `sa.pub`. Наведений нижче приклад ілюструє файли ключа та сертифіката ЦС, показані в попередній таблиці:
