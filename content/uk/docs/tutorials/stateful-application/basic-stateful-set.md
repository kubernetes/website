...
Цей підручник передбачає, що ваш кластер налаштований на динамічне забезпечення PersistentVolumes. Вам також потрібно мати [типовий StorageClass](/docs/concepts/storage/storage-classes/#default-storageclass). Якщо ваш кластер не налаштований для динамічного забезпечення сховища, вам доведеться вручну забезпечити два томи по 1 GiB перед початком цього уроку та налаштувати ваш кластер так, щоб ці PersistentVolumes відповідали шаблонам PersistentVolumeClaim, які визначає StatefulSet.
{{< /alert >}}

## {{% heading "objectives" %}}

StatefulSets призначені для використання з застосунками, які зберігають свій стан, та розподіленими системами. Однак, адміністрування стану застосунків та розподілених систем у Kubernetes є широкою та складною темою. Щоб продемонструвати базові можливості StatefulSet і не змішувати першу тему з другою, ви розгорнете простий вебзастосунок за допомогою StatefulSet.
...
Почніть зі створення StatefulSet (і Service, на який він спирається) за допомогою наведеного нижче прикладу. Він схожий на приклад, наведений у концепції [StatefulSets](/docs/concepts/workloads/controllers/statefulset/). Він створює [headless Service](/docs/concepts/services-networking/service/#headless-services), `nginx`, щоб опублікувати IP-адреси Podʼів у StatefulSet, `web`.
...
NGINX вебсервер типово обслуговує індексний файл із `/usr/share/nginx/html/index.html`. Поле `volumeMounts` в `spec` StatefulSet забезпечує, що тека `/usr/share/nginx/html` зберігається у PersistentVolume.
...
Навіть якщо `web-0` та `web-1` переплановані, вони продовжують обслуговувати свої імена хостів, оскільки PersistentVolumes, повʼязані з їхніми PersistentVolumeClaims, знову монтуються до їхніх `volumeMounts`. Незалежно від того, на якому вузлі заплановані `web-0` та `web-1`, їхні PersistentVolumes будуть підключені до відповідних точок монтування.
...
Ви обираєте цю стратегію оновлення для StatefulSet, встановивши `.spec.updateStrategy.type` на `OnDelete`.
...
Як ви бачили в розділі [Зменшення масштабу](#scaling-down), Podʼи видаляються по одному, з урахуванням зворотного порядку їх порядкових індексів. Перед видаленням Podʼа контролер StatefulSet чекає, поки Pod-наступник буде повністю видалено.
