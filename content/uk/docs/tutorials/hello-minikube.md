...
Цей посібник передбачає, що у вас вже є встановлений `minikube`. Дивіться __Крок 1__ в [minikube start](https://minikube.sigs.k8s.io/docs/start/) для інструкцій щодо встановлення.
...
{{< alert color="info" title="Примітка" >}}
Команда `minikube start` створює кластер з одним вузлом. Цей вузол виконує роль і панелі управління, і робочого вузла. Це відрізняється від багатьох виробничих кластерів Kubernetes, де вузли панелі управління зазвичай ізольовані від робочих вузлів за допомогою [taint](/docs/concepts/scheduling-eviction/taint-and-toleration/) або повністю невидимі для користувача.
{{< /alert >}}
...
Тепер можете використовувати цю URL-адресу та повернутися до термінала, де ви запустили `minikube start`.
...
Контейнер agnhost має точку доступу `/shell`, яка корисна для налагодження, але є небезпечною у випадку її експонування для публічного доступу з інтернету. Не запускайте цей контейнер в кластері, що має вихід до інтернету, або у виробничому кластері.
{{< /alert >}}
