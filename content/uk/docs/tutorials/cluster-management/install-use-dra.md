...
<!-- overview -->
Цей посібник показує, як встановити драйвери {{< glossary_tooltip term_id="dra" text="Динамічного виділення ресурсів (DRA)" >}} у вашому кластері та як використовувати їх разом з API DRA для виділення {{< glossary_tooltip text="пристроїв" term_id="device" >}} для Pod. Ця сторінка призначена для адміністраторів кластерів.
...
1.  Розгорніть фактичний драйвер DRA як DaemonSet, налаштований на запуск прикладу двійкового файлу драйвера з вищезазначеними дозволами.
    ...
    DaemonSet налаштовано з точками монтування томів, необхідними для взаємодії з підлягаючою текою Container Device Interface (CDI), і для відкриття його сокета для `kubelet` через теку `kubelet/plugins`.
...
Цей посібник демонструє лише один базовий приклад ResourceClaim DRA. Читайте [Динамічне виділення ресурсів](/docs/concepts/resource-management/dynamic-resource-allocation/), щоб дізнатися більше про ResourceClaims.
...
* [Дізнайтеся більше про DRA](/docs/concepts/resource-management/dynamic-resource-allocation/)
