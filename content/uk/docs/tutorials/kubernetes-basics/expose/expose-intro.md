...
* _ExternalName_ — звʼязує Service з вмістом поля `externalName` (наприклад, `foo.bar.example.com`), повертаючи запис `CNAME` із його значенням. Не встановлюється жодного проксі.
