...
* Оточення для запуску контейнерів (таке як containerd) забезпечує завантаження образу контейнера з реєстру, розпакування контейнера та запуск застосунку.
...
Памʼятаєте, що Podʼи працюють в ізольованій, приватній мережі — тому нам потрібно налаштувати проксі-доступ до них для налагодження та взаємодії з ними. Для цього ми використовуємо команду `kubectl proxy` для запуску проксі в **іншому терміналі**. Відкрийте нове вікно термінала і введіть у цьому новому терміналі:
...
Ви можете перевірити, чи працює застосунок, виконавши команду curl:
...
* Розділ [Використання Service для надання доступу до вашого застосунку](/docs/tutorials/kubernetes-basics/expose/expose-intro/).
