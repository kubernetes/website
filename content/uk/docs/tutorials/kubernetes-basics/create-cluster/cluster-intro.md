...
Цей посібник передбачає, що ви вже встановили `minikube`. Дивіться [minikube start](https://minikube.sigs.k8s.io/docs/start/) для інструкцій з встановлення.

{{< note >}}
Виконуйте лише інструкції з **кроку 1: Встановлення**. Решта покривається в цьому посібнику.
{{< /note >}}

Вам також потрібно встановити `kubectl`. Дивіться [Встановлення інструментів](/docs/tasks/tools/#kubectl) для інструкцій з встановлення.
...
## Створення кластера Minikube {#create-a-minikube-cluster}

Щоб запустити кластер Minikube:

```shell
minikube start
```

Щоб перевірити стан кластера:

```shell
minikube status
```

Для повного проходження, включаючи розгортання вашого першого застосунку та ознайомлення з Kubernetes Dashboard, дивіться підручник [Привіт Minikube](/docs/tutorials/hello-minikube/).

## {{% heading "whatsnext" %}}

* Розділ [Розгортання застосунку](/docs/tutorials/kubernetes-basics/deploy-app/deploy-intro/).
