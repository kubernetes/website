...
В редакторі, що зʼявиться, змініть значення ключа `sport` з `football` на `cricket`. Збережіть зміни. Інструмент kubectl відповідним чином оновлює ConfigMap (якщо виникне помилка, спробуйте ще раз).
