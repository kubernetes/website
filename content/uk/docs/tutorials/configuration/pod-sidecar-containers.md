...
Дивіться [відмінності від контейнерів ініціалізації](/docs/concepts/workloads/pods/sidecar-containers/#differences-from-init-containers) для додаткових відомостей.
...
Якщо інструменти передають невідомі поля як є, використовуючи різні стратегії виправлення для змінни обʼєкта Pod, це не буде проблемою. Однак є інструменти, які видалять невідомі поля; якщо у вас є такі, їх потрібно буде перекомпілювати з v1.28+ версією клієнтського коду Kubernetes API.
...
### Автоматичне впровадження sidecar-контейнерів {#automatic-injection-of-sidecars}

Якщо ви використовуєте програмне забезпечення, яке автоматично впроваджує sidecar-контейнери, існує кілька можливих стратегій, які ви можете застосувати, щоб забезпечити можливість використання нативних sidecar-контейнерів. Усі ці стратегії загалом є варіантами вибору того, чи буде Pod, до якого впроваджується sidecar, розміщений на вузлі, який підтримує цю функцію.
