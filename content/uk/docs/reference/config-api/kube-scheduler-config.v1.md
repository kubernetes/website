...
<td>
   <p>acceptContentTypes визначає заголовок Accept, який надсилається клієнтами при підключенні до сервера, перевизначаючи стандартне значення 'application/json'. Це поле буде контролювати всі зʼєднання з сервером, що використовуються певним клієнтом.</p>
</td>
</tr>
...
<td>
   ...
   <p>Стандартні значення та функціональні можливості:</p>
      <ul>
...
<td>
   <p>DelayCacheUntilActive визначає, коли починати кешування. Якщо це true і вибори лідера увімкнені, планувальник чекатиме, щоб заповнити кеші інформаторів, поки не стане лідером. Це призведе до повільнішого перемикання з резервного на основний вузол із перевагою нижчого використання памʼяті під час очікування стати лідером. Стандартно — false.</p>
</td>
</tr>
</tbody>
</table>
...
<a href="/docs/reference/generated/kubernetes-api/v1.37/#nodeaffinity-v1-core"><code>core/v1.NodeAffinity</code></a>
</td>
<td>
   <p>AddedAffinity застосовується до всіх Podʼів додатково до NodeAffinity, вказаного в PodSpec. Тобто вузли повинні відповідати AddedAffinity І .spec.NodeAffinity. AddedAffinity є стандартно порожнім (є збіг зі всіма вузлами). Коли використовується AddedAffinity, деякі Podʼи з вимогами щодо спорідненості, які збігаються з конкретним вузлом (наприклад, Podʼи Daemonset), можуть залишатися непридатними для планування.</p>
</td>
</tr>
</tbody>
</table>
...
<a href="/docs/reference/generated/kubernetes-api/v1.37/#topologyspreadconstraint-v1-core"><code>[]core/v1.TopologySpreadConstraint</code></a>
</td>
<td>
...
Plugin визначає імʼя втулка та його вагу, якщо це застосовується. Вага використовується лише для втулків типу Score та PlacementScore.</p>
...
<td>
   <p>Weight визначає вагу втулка, використовується лише для втулків типу Score та PlacementScore.</p>
</td>
</tr>
</tbody>
</table>
...
<tr><td><code>placementGenerate</code> <B>[Обовʼязкове]</B><br/>
<a href="#kubescheduler-config-k8s-io-v1-PluginSet"><code>PluginSet</code></a>
</td>
<td>
   <p>PlacementGenerate — це список втулків, які повинні бути викликані під час циклу планування групи Podʼів при визначенні розміщень для групи Podʼів.</p>
</td>
</tr>
<tr><td><code>placementScore</code> <B>[Обовʼязкове]</B><br/>
<a href="#kubescheduler-config-k8s-io-v1-PluginSet"><code>PluginSet</code></a>
</td>
<td>
   <p>PlacementScore — це список втулків, які повинні бути викликані під час циклу планування групи Podʼів при ранжуванні призначень для групи Podʼів.</p>
</td>
</tr>
<tr><td><code>podGroupPostFilter</code> <B>[Обовʼязкове]</B><br/>
<a href="#kubescheduler-config-k8s-io-v1-PluginSet"><code>PluginSet</code></a>
</td>
<td>
   <p>PodGroupPostFilter — це список втулків, які викликаються після фази планування робочого навантаження, але лише коли PodGroup неможливо запланувати (еквівалент PostFilter для окремих подів).</p>
</td>
</tr>
</tbody>
</table>
