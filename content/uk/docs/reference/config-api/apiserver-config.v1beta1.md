...
<table class="table">
    ...
    <tbody>
        ...
        <tr>
            <td><code>tlsServerName</code><br/>
                <code>string</code>
            </td>
            <td><p>tlsServerName використовується для перевірки сертифіката сервера. Якщо tlsServerName порожній, використовується імʼя хоста, яке використовується для звʼязку з сервером.</p></td>
        </tr>
    </tbody>
</table>
    ...
    <tbody>
        <tr>
           ...
           <tr>
            ...
            <td><p>UDS — це конфігурація UDS для звʼязку з konnectivity сервером через UDS. Вимагає налаштування хоча б одного з TCP або UDS</p></td>
        </tr>
    </tbody>
</table>
