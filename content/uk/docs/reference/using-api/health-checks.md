...
Ці точки доступу відповідають принципу роботи [HTTP-проб](/docs/concepts/workloads/pods/probes/#http-probes) Kubernetes:
...
* **readyz**: використовуйте це, щоб визначити, чи готовий API-сервер приймати трафік. Якщо `/readyz` повертає код статусу помилки, це означає, що сервер все ще ініціалізується або тимчасово не може обслуговувати запити (наприклад, чекає на доступність etcd або чекає, поки кожен зареєстрований кеш спостереження завершить початкову синхронізацію з etcd), і трафік слід перенаправити від нього.
