...
Kubernetes підтримує ефективні сповіщення про зміни ресурсів за допомогою *watches*. {{< glossary_definition prepend="В API Kubernetes, watch це" term_id="watch" length="short" >}} Kubernetes також забезпечує послідовні операції зі списками, щоб клієнти API могли ефективно кешувати, відстежувати та синхронізувати стан ресурсів.
...
Усі обʼєкти, які ви можете створити через API, мають унікальне {{< glossary_tooltip text="імʼя" term_id="name" >}}, що дозволяє ідемпотентне[^1] створення та отримання, за винятком віртуальних типів ресурсів, які можуть не мати унікальних імен, якщо вони не можуть бути отримані або не залежать від ідемпотентності.

У межах {{< glossary_tooltip text="простору імен" term_id="namespace" >}}, унікальна ідентичність обʼєкта визначається кортежем його **API Group**, **Resource**, **Namespace** та **Name**.
...
Стандартно, Kubernetes повертає [серіалізовані обʼєкти JSON](#json-encoding), використовуючи медіа-тип `application/json`. Хоча JSON є стандартним типом, клієнти можуть запитувати відповідь у форматі YAML, або більш ефективне двійкове подання у вигляді [Protobuf](#protobuf-encoding) для кращої продуктивності.

Kubernetes API реалізує стандартне узгодження типів вмісту HTTP: передача заголовка `Accept` у запиті `GET` вказує серверу спробувати повернути відповідь у бажаному медіа-типі. Якщо ви хочете надіслати обʼєкт у форматі Protobuf на сервер для запиту `PUT` або `POST`, вам необхідно відповідно встановити заголовок запиту `Content-Type`.
...
Kubernetes використовує обгортку-конверт для кодування відповідей у форматі [Protobuf](https://protobuf.dev/). Ця обгортка починається з 4 байтів магічного числа, щоб допомогти ідентифікувати вміст на диску або в etcd як Protobuf (на відміну від JSON). Дані з магічним числом (4 байти) слідують за повідомленням, закодованим у форматі Protobuf, яке описує кодування та тип основного обʼєкта. Усередині повідомлення Protobuf дані внутрішнього обʼєкта записуються за допомогою поля `raw` Unknown (дивіться [IDL](#protobuf-encoding-idl) для докладної інформації).
...
## Фрагментований list та watch {#sharded-list-and-watch}
...
Kubernetes {{< skew currentVersion >}} в альфа-версії представляє функцію, яка дозволяє клієнтам запитувати **фільтровані фрагменти** обʼєктів з API-сервера, використовуючи хешоване розподілення за метаданими. Щоб використовувати цю функцію, увімкніть функціональну можливість [`ShardedListAndWatch`](/docs/reference/command-line-tools-reference/feature-gates/#ShardedListAndWatch) на API-сервері. Коли функція увімкнена, API-сервер фільтрує як відповіді на list, так і потоки подій watch на стороні сервера, доставляючи лише ті обʼєкти та події, хешоване значення метаданих яких потрапляє в запитаний діапазон.

### Поле shardSelector {#the-shardselector-field}

Поле `ShardSelector` у [`ListOptions`](/docs/reference/generated/kubernetes-api/{{< param "version" >}}/#listoptions-v1-meta) приймає вираз на основі CEL з використанням функції `shardRange()`:
...
API-сервер обчислює детермінований 64-бітний [FNV-1a](https://en.wikipedia.org/wiki/Fowler%E2%80%93Noll%E2%80%93Vo_hash_function) хеш вказаного значення поля і повертає лише ті обʼєкти, хеш яких потрапляє в діапазон `[start, end)`. Функція хешування дає однаковий результат для одного й того ж вхідного значення на всіх екземплярах API-сервера, тому фрагментовані запити безпечні для використання з кількома репліками API-сервера.
...
### Використання фрагментованого list та watch у контролерах {#sharded-list-and-watch-controllers}

Контролери зазвичай використовують [інформери](/docs/reference/using-api/api-concepts/#efficient-detection-of-changes) для list та watch ресурсів. Щоб розподілити навантаження між репліками, кожна репліка вставляє поле `ShardSelector` у `ListOptions`, які використовуються її інформерами.
...
// shardSelector визначається ідентичністю репліки (наприклад, з порядкового номера StatefulSet або на основі оренди). Кожна репліка претендує на неперетинний діапазон простору хешу.
...
```go
// Репліка 0: нижня половина простору хешу
"shardRange(object.metadata.uid, '0x0000000000000000', '0x8000000000000000')"

// Репліка 1: верхня половина простору хешу
...
### Інформація про фрагменти у відповідях {#shard-information-in-responses}
...
### Виявлення підтримки сервером {#detecting-server-support}
...
Увімкнувши опцію видалення `ignoreStoreReadErrorWithClusterBreakingPotential`, користувач може виконати небезпечну операцію примусового **видалення** нерозшифрованого/пошкодженого ресурсу. Ця опція доступна, коли функціональну можливість [`AllowUnsafeMalformedObjectDeletion`](/docs/reference/command-line-tools-reference/feature-gates/#AllowUnsafeMalformedObjectDeletion) увімкнено, що є стандартним, починаючи з Kubernetes v1.37. Оператор кластера може вимкнути цю поведінку, встановивши параметр командного рядка `--feature-gates=AllowUnsafeMalformedObjectDeletion=false`.
...
Рядки версій ресурсів можна впорядкувати як монотонно зростаючі цілі числа в межах одного типу ресурсів для всіх типів, що обслуговуються kube-apiserver. Сюди входять вбудовані типи API та типи, що підтримуються визначеннями власних ресурсів. Обидві версії ресурсів повинні бути з обʼєктів однієї групи API та одного типу ресурсів. Наприклад, можна порівняти версії ресурсів двох розгортань з групи API застосунків, але не можна порівняти версії ресурсів Pod і Deployment. За умови, що два обʼєкти отримано з одного типу ресурсів API, їх можна порівняти, навіть якщо вони знаходяться в різних просторах імен.
...
### Ініціалізація кешу спостереження {#watch-cache-initialization}

Щоб запобігти перевантаженню панелі управління та голодуванню API Priority and Fairness (APF) під час ініціалізації або повторної ініціалізації кешу спостереження, сервер API Kubernetes застосовує два захисні механізми:

* **Захист готовності під час запуску**: Під час запуску сервер API затримує повідомлення про готовність (`/readyz`), доки кожен зареєстрований кеш спостереження не завершить свою початкову синхронізацію з etcd. Це запобігає передчасному спрямуванню клієнтського трафіку балансувальниками навантаження на сервер API, усуваючи стартові масові одночасні читання, які інакше оминали б неготовий кеш і потрапляли б безпосередньо в etcd.
* **Відхилення запитів під час ініціалізації кешу**: Якщо запити надходять, поки кеш спостереження для типу ресурсу не ініціалізовано, наприклад, під час завантаження сервера (до готовності) або після повторної синхронізації чи відключення кешу під час роботи, сервер API запобігає перевантаженню etcd дорогими запитами або їхньому нескінченному зависанню:
  * **Запити `WATCH`** та **непосторінкові або фільтровані запити `LIST`** (будь-який запит списку без ліміту посторінкової видачі `limit` або з селектором міток чи полів) негайно відхиляються з HTTP-кодом статусу `429 Too Many Requests` та динамічним заголовком `Retry-After`. Це запобігає зависанню goroutine, голодуванню місць APF та вичерпанню памʼяті etcd.
  * **Безпечні посторінкові читання**: Лише **запити `GET`** та **посторінкові, нефільтровані запити `LIST`** (`limit` > 0 без селектора міток чи полів) делегуються безпосередньо до etcd як безпечний запасний варіант.

Клієнти (включно з власними контролерами та операторами) мають бути розроблені так, щоб коректно обробляти відповіді HTTP `429 Too Many Requests`, поважаючи заголовки `Retry-After` та реалізуючи експоненційну затримку повторних спроб.
