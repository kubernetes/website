...
{{< note >}}
Поки kuberc знаходиться у бета-версії, `name` може використовуватися як псевдонім для `command` у записах allowlist. Починаючи з Kubernetes 1.36, `name` застаріває на користь `command`. Надання **обох** `name` та `command` в одному записі allowlist вважається помилкою, оскільки це налаштування є критично важливим для безпеки. Поле `name` буде повністю видалено, коли kuberc досягне GA.
{{< /note >}}
