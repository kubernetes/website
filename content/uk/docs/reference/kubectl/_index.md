...
Для огляду див. [Інструмент командного рядка kubectl](/docs/concepts/overview/kubectl/). Інструкції з встановлення знаходяться у статті [Встановлення kubectl](/docs/tasks/tools/#kubectl); короткий посібник є у [шпаргалці](/docs/reference/kubectl/quick-reference/). Якщо ви звикли користуватись інструментом командного рядка `docker`, [`kubectl` для користувачів Docker](/docs/reference/kubectl/docker-cli-to-kubectl/) пояснює деякі еквівалентні команди для Kubernetes.
...
`autoscale` | <code>kubectl autoscale (-f FILENAME &#124; TYPE NAME &#124; TYPE/NAME) [--min=MINPODS] --max=MAXPODS [--cpu=CPU] [flags]</code> | Автоматично масштабувати набір Podʼів, керованих контролером реплікації.
...
`-o kyaml`    | Вивести обʼєкт API у форматі [KYAML](/docs/reference/encodings/kyaml/).
...
`-o yaml`     | Вивести обʼєкт API у форматі YAML. KYAML є специфічним для Kubernetes діалектом YAML, і може читатись як YAML.
