...
## Автозавершення команд kubectl {#kubectl-autocomplete}
...
`-o=kyaml`    | Вивести API обʼєкт у форматі [KYAML](/docs/reference/encodings/kyaml). KYAML є специфічним для Kubernetes діалектом YAML, і може бути оброблений як YAML.
...
Деталізація виводу в kubectl контролюється за допомогою прапорця `-v` або `--v`, за яким слідує ціле число, що представляє рівень логування. Загальні конвенції щодо логування Kubernetes та повʼязані рівні описані [тут](https://github.com/kubernetes/community/blob/main/contributors/devel/sig-instrumentation/logging.md).
...
* Дивіться [огляд kubectl](/docs/concepts/overview/kubectl/) та його роль в екосистемі Kubernetes.

* Прочитайте [довідку kubectl](/docs/reference/kubectl/) та дізнайтеся про [JsonPath](/docs/reference/kubectl/jsonpath).
