...
<table style="width: 100%; table-layout: fixed;">
    ...
    <tbody>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>URL проксі для використання в запитах до сервера API</p></td>
        </tr>
        <tr>
...
* [kubectl annotate](../kubectl_annotate/) — Оновлення анотацій ресурсу
* [kubectl api-resources](../kubectl_api-resources/) — Вивід підтримуваних ресурсів API на сервері
* [kubectl api-versions](../kubectl_api-versions/) — Вивід підтримуваних версій API на сервері у форматі "група/версія"
* [kubectl apply](../kubectl_apply/) — Застосування конфігурації до ресурсу за іменем файлу або через stdin
* [kubectl attach](../kubectl_attach/) — Приєднання до запущеного контейнера
* [kubectl auth](../kubectl_auth/) — Інспектування авторизації
* [kubectl autoscale](../kubectl_autoscale/) — Автоматичне масштабування deployment, replica set, stateful set або контролера реплікацій
* [kubectl certificate](../kubectl_certificate/) — Зміна ресурсів сертифікатів
* [kubectl cluster-info](../kubectl_cluster-info/) — Вивід інформації про кластер
* [kubectl completion](../kubectl_completion/) — Вивід коду автозавершення оболонки для вказаної оболонки (bash, zsh, fish або powershell)
* [kubectl config](../kubectl_config/) — Зміна файлів kubeconfig
* [kubectl cordon](../kubectl_cordon/) — Позначення вузла як незапланованого
* [kubectl cp](../kubectl_cp/) — Копіювання файлів та тек до контейнерів і з них
* [kubectl create](../kubectl_create/) — Створення ресурсу з файлу або через stdin
* [kubectl debug](../kubectl_debug/) — Створення сеансів налагодження для усунення несправностей навантажень та вузлів
* [kubectl delete](../kubectl_delete/) — Видалення ресурсів за іменами файлів, через stdin, ресурсів та імен або за селектором ресурсів і міток
* [kubectl describe](../kubectl_describe/) — Показ деталей конкретного ресурсу або групи ресурсів
* [kubectl diff](../kubectl_diff/) — Показ різниці між поточною версією та потенційно застосованою версією
* [kubectl drain](../kubectl_drain/) — Очищення вузла перед технічним обслуговуванням
* [kubectl edit](../kubectl_edit/) — Редагування ресурсу на сервері
* [kubectl events](../kubectl_events/) — Вивід списку подій
* [kubectl exec](../kubectl_exec/) — Виконання команди в контейнері
* [kubectl explain](../kubectl_explain/) — Отримання документації для ресурсу
* [kubectl expose](../kubectl_expose/) — Використання контролера реплікації, service, deployment або pod та експонування його як нового сервіса Kubernetes
* [kubectl get](../kubectl_get/) — Показ одного чи кількох ресурсів
* [kubectl kuberc](../kubectl_kuberc/) — Керування файлами конфігурації kuberc
* [kubectl kustomize](../kubectl_kustomize/) — Побудова цільового kustomization з текуи або URL
* [kubectl label](../kubectl_label/) — Оновлення міток ресурсу
* [kubectl logs](../kubectl_logs/) — Вивід логів для контейнера в Pod
* [kubectl options](../kubectl_options/) — Вивід списку прапорців, успадкованих всіма командами
* [kubectl patch](../kubectl_patch/) — Оновлення полів ресурсу
* [kubectl plugin](../kubectl_plugin/) — Надає утиліти для взаємодії з втулками
* [kubectl port-forward](../kubectl_port-forward/) — Перенаправлення одного або декількох локальних портів до Podʼа
* [kubectl proxy](../kubectl_proxy/) — Запуск проксі для сервера API Kubernetes
* [kubectl replace](../kubectl_replace/) — Заміна ресурсу за іменем файлу або через stdin
* [kubectl rollout](../kubectl_rollout/) — Керування розгортанням ресурсу
* [kubectl run](../kubectl_run/) — Запуск певного образу в кластері
* [kubectl scale](../kubectl_scale/) — Встановлення нового розміру для deployment, replica set або контролера реплікацій
* [kubectl set](../kubectl_set/) — Встановлення конкретних функцій для обʼєктів
* [kubectl taint](../kubectl_taint/) — Оновлення taint на одному або кількох вузлах
* [kubectl top](../kubectl_top/) — Показ використання ресурсів (CPU/памʼять)
* [kubectl uncordon](../kubectl_uncordon/) — Позначення вузла як придатного для планування
* [kubectl version](../kubectl_version/) — Вивід інформації про версію клієнта та сервера
* [kubectl wait](../kubectl_wait/) — Експериментальна функція: очікування на визначену умову для одного або кількох ресурсів
