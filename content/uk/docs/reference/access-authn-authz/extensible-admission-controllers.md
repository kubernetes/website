...
### Відповідність запитів: objectSelector {#matching-requests-objectselector}
...
## Найкращі практики та попередження {#best-practices-and-warnings}
...
Рекомендується, щоб вебхуки допуску оцінювалися якомога швидше (зазвичай за мілісекунди), оскільки вони збільшують затримку запиту API. Рекомендується використовувати невеликий таймаут для вебхуків. Дивіться [Таймаути](#timeouts) для докладнішої інформації.
...
### Уникнення блокування вебхуків у самостійно розміщених вебхуках {#avoiding-deadlocks-in-self-hosted-webhooks}

Існує кілька способів, за допомогою яких вебхуки можуть викликати блокування, коли кластер не може досягти прогресу в у плануванні podʼів:

* Вебхук, запущений всередині кластера, може спричинити блокування власного розгортання, якщо його налаштовано на перехоплення ресурсів, необхідні для запуску його власних podʼів.

  Наприклад, модифікуючий вебхук допуску налаштований на допуск запитів на **створення** Podʼу лише в тому випадку, якщо у `pod` встановлено певну мітку (наприклад, `env: "prod"`). Однак, сервер вебхука працює як Deployment, що не встановлює мітку `env`. Коли вузол, на якому запущено Podʼи вебхука, стає непрацездатним, deployment вебхука спробує перепланувати Podʼи на інший вузол. Однак запити будуть відхилені існуючим сервером вебхука, оскільки мітка `env` не встановлена, і Podʼи заміни не можуть бути створені. Зрештою, весь Deployment для сервера вебхука може стати несправним.

  Якщо ви використовуєте вебхук допуску для перевірки Podʼів , зважте на виключення простору імен, де працює ваш вебхук, за допомогою [namespaceSelector](#matching-requests-namespaceselector).

* Якщо в кластері налаштовано кілька вебхуків (можливо, з незалежних застосунків, розгорнутих на кластері), вони можуть утворювати цикл.  Вебхук A повинен бути викликаний для обробки запуску блоків вебхука B, і навпаки. Якщо webhook A і webhook B стають недоступними одночасно (наприклад, через відключення кластера або збій у роботі вузла, де обидва podʼи працюють на одному вузлі), виникає глухий кут, оскільки жоден з podʼів вебхука не може бути відтворений без іншого, який вже запущено.

  Один із способів запобігти цьому — заборонити podʼу A впливати на вебхук B. Це дозволить запустити pod A, який, у свою чергу, дозволить запустити pod вебхука B. Якби у вас був третій вебхук, вебхук C, вам потрібно було б виключити з вебхука C як вебхук A, так і вебхук B. Це гарантує, що вебхук A може запускатися _завжди_, що дозволяє запускатися вебхуку B, який, у свою чергу, дозволяє запускатися вебхуку C.

  Якщо ви хочете забезпечити захист, який дозволяє уникнути цих ризиків, [ValidatingAdmissionPolicy](/docs/reference/access-authn-authz/validating-admission-policy/) може надати багато можливостей захисту без введення циклів залежностей.

* Вебхуки доступу можуть перехоплювати ресурси, що використовуються критично важливими надбудовами кластера, такими як CoreDNS, мережевими втулками або втулками для зберігання даних. Ці надбудови можуть знадобитися для планування або успішного запуску podʼів для певного вебхука в кластері. Це може призвести до глухого кута, якщо одночасно недоступні і вебхук, і критично важлива надбудова.

  Можливо, ви захочете виключити простори імен кластерної інфраструктури з вебхуків або переконатися, що вебхук не залежить від конкретної надбудови, з якою він взаємодіє.  Наприклад, запуск вебхука як мережевого podʼа хоста гарантує, що він не буде залежати від мережевого втулка.

  Якщо ви хочете забезпечити захист для основної надбудови та/або її простору імен, [ValidatingAdmissionPolicy](/docs/reference/access-authn-authz/validating-admission-policy/) може надати багато можливостей захисту без будь-якої залежності від робочих вузлів та Pods.
