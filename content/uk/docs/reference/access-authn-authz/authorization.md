...
* [Динамічне виділення ресурсів (DRA)](/docs/concepts/resource-management/dynamic-resource-allocation/)
...
* [Примусове видалення пошкоджених ресурсів](/docs/reference/using-api/api-concepts/#force-deletion)
  * Дієслово **unsafe-delete-ignore-read-errors** для ресурсу, яке перевіряється додатково до дієслова **delete**.
...
authorizers:
  - type: Webhook
    ...
    # Обовʼязково, немає типового значення
    name: webhook
    webhook:
      ...
      # Примітка: Встановлення authorizedTTL в `0` призводить до використання його типового значення.
      # Типове: true
      ...
      # Примітка: Встановлення unauthorizedTTL в `0` призводить до використання його типового значення.
      # Типове: true
      ...
      #
      # лише надсилати запити ресурсів до вебхука
