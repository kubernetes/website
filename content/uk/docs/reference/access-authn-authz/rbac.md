...
Щоб увімкнути RBAC, запустіть {{< glossary_tooltip text="API сервер" term_id="kube-apiserver" >}} з прапорцем `--authorization-config`, встановленим на файл, що включає `RBAC`; наприклад:
...
Опустіть поле `rules` з маніфестів для агрегованих ClusterRole. Його встановлення, навіть у вигляді порожнього списку, претендує на володіння полем, коли маніфест застосовується з [server-side apply](/docs/reference/using-api/server-side-apply/). Це володіння викликає конфлікти між застосовувачем та панеллю управління: наступні застосування або завершуються помилкою через конфлікт менеджера полів у `.rules`, або (якщо конфлікти примусово вирішуються, як це зазвичай роблять GitOps-контролери) неодноразово очищають агреговані правила, які панель управління потім заповнює знову.
{{< /caution >}}
...
# Панель управління автоматично заповнює правила
```
...
[Типові ролі для користувачів](#default-roles-and-role-bindings) використовують агрегацію ClusterRole. Це дозволяє вам, як адміністратору кластера, включати правила для спеціальних ресурсів, таких як ті, що надаються {{< glossary_tooltip term_id="CustomResourceDefinition" text="CustomResourceDefinitions" >}} або агрегованими API серверами, щоб розширити типові ролі.

Наприклад: наступні ClusterRole дозволяють типовим ролям "admin" і "edit" керувати спеціальним ресурсом з назвою CronTab, тоді як роль "view" може виконувати лише читання ресурсів CronTab. Ви можете припустити, що обʼєкти CronTab називаються `"crontabs"` в URL, як це бачить API сервер.
...
metadata:
  ...
  labels:
    # Додайте ці дозволи до типових ролей "admin" і "edit".
...
metadata:
  ...
  labels:
    # Додайте ці дозволи до типової ролі "view".
...
Для типового службового облікового запису у просторі імен "kube-system":
...
## Типові ролі та привʼязки ролей {#default-roles-and-role-bindings}

API-сервери створюють набір типових обʼєктів ClusterRole і ClusterRoleBinding. Багато з них мають префікс `system:`, що вказує на те, що ресурс безпосередньо керується панеллю управління кластера. Усі типові ролі та привʼязки ролей мають мітку `kubernetes.io/bootstrapping=rbac-defaults`.
...
При кожному запуску API-сервер оновлює типові ролі кластера, додаючи будь-які відсутні дозволи, та оновлює типові привʼязки ролей, додаючи будь-які відсутні субʼєкти. Це дозволяє кластеру виправляти випадкові зміни та допомагає підтримувати ролі та привʼязки ролей в актуальному стані, оскільки дозволи та субʼєкти змінюються у нових випусках Kubernetes.

Щоб відмовитися від цього узгодження, встановіть анотацію `rbac.authorization.kubernetes.io/autoupdate` на типовій кластерній ролі або типовій RoleBinding у значення `false`. Зверніть увагу, що відсутність типових дозволів та субʼєктів може призвести до несправних кластерів.

Автоматичне узгодження типово увімкнено, якщо авторизатор RBAC активний.

### Ролі виявлення API {#discovery-roles}

Типові кластерні привʼязки ролей дозволяють неавторизованим і авторизованим користувачам читати інформацію про API, яка вважається безпечною для публічного доступу (включаючи CustomResourceDefinitions). Щоб вимкнути анонімний неавторизований доступ, додайте прапорець `--anonymous-auth=false` до конфігурації API-сервера.
...
<thead>
<tr>
<th>Типова ClusterRole</th>
<th>Типова ClusterRoleBinding</th>
<th>Опис</th>
</tr>
</thead>
<tbody>
...
<td>Дозволяє користувачеві доступ лише для читання базової інформації про себе. До v1.14 ця роль також була типово повʼязана з <tt>system:unauthenticated</tt>.</td>
</tr>
<tr>
<td><b>system:discovery</b></td>
<td>Група <b>system:authenticated</b></td>
<td>Дозволяє доступ лише для читання точок доступу виявлення API, необхідних для виявлення та узгодження рівня API. До v1.14 ця роль також була типово повʼязана з <tt>system:unauthenticated</tt>.</td>
</tr>
<tr>
...
Деякі з типових ClusterRoles не мають префікса `system:`. Вони включають ролі суперкористувача (`cluster-admin`), ролі, призначені для надання у всьому кластері за допомогою ClusterRoleBindings, а також ролі, призначені для надання у певних просторах імен простору імен за допомогою RoleBindings (`admin`, `edit`, `view`).
...
Якщо використовується в <b>RoleBinding</b>, дозволяє доступ для читання/запису до більшості ресурсів у просторі імен, включаючи можливість створювати ролі та привʼязки ролей у просторі імен. Ця роль не дозволяє доступу для запису до квоти ресурсів або до самого простору імен. Ця роль також не дозволяє доступу для запису до EndpointSlices у кластерах, створених за допомогою Kubernetes v1.22+. Більше інформації доступно у розділі ["Права запису для EndpointSlices"](#write-access-for-endpoints).</td>
</tr>
<tr>
<td><b>edit</b></td>
<td>Немає</td>
...
Ця роль не дозволяє переглядати або змінювати ролі або привʼязки ролей. Однак ця роль дозволяє доступ до Secretʼів і запуску Podʼів як будь-який ServiceAccount у просторі імен, тому вона може бути використана для отримання рівня доступу API будь-якого ServiceAccount у просторі імен. Ця роль також не дозволяє доступу для запису до EndpointSlices у кластерах, створених за допомогою Kubernetes v1.22+. Більше інформації доступно у розділі ["Права запису для EndpointSlices"](#write-access-for-endpoints).</td>
</tr>
<tr>
<td><b>view</b></td>
<td>Немає</td>
...
<td>Роль для застарілого компонента kube-dns. (<a href="/uk/docs/concepts/services-networking/dns-pod-service/">CoreDNS</a> не використовує цю роль.)</td>
</tr>
<tr>
...
Наприклад, якщо `user-1` не має можливості отримання переліку Secrets на кластерному рівні, він не може створити ClusterRole, що містить цей дозвіл. Щоб дозволити користувачу створювати/оновлювати ролі:
...
1. Надати йому дозволи, необхідні для привʼязки певної ролі:
   * неявно, надаючи йому дозволи, що містяться в ролі.
...
* Використовуйте облікові дані з групою "system:masters", яка типово привʼязана до ролі суперкористувача "cluster-admin".
