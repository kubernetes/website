...
Усі значення є непрозорими для системи автентифікації та мають значення лише при інтерпретації [авторизатором](/uk/docs/reference/access-authn-authz/authorization/).
...
Ця функція є дуже корисною, коли використовується складний потік автентифікації в кластері Kubernetes, наприклад, якщо ви використовуєте [автентифікацію за допомогою токена через webhook](/uk/docs/reference/access-authn-authz/authentication/#webhook-token-authentication) або [автентифікацію через проксі](/uk/docs/reference/access-authn-authz/authentication/#authenticating-proxy).

{{< note >}}
API-сервер Kubernetes заповнює `userInfo` після застосування всіх механізмів автентифікації,
включаючи [імперсонізаацію](/uk/docs/reference/access-authn-authz/authentication/#user-impersonation). Якщо ви, або автентифікаційний проксі, робите SelfSubjectReview за допомогою імперсонізації, ви побачите деталі та властивості користувача, який був імперсонований.
{{< /note >}}
...
- включено [функціонал](/uk/docs/reference/command-line-tools-reference/feature-gates/) `APISelfSubjectReview` для вашого кластера (не потрібно для Kubernetes {{< skew currentVersion >}}, але більш старі версії Kubernetes можуть не надавати цю функцію або за стандартно вимикати її)
...
- Прочитайте [довідник з автентифікації клієнта (v1beta1)](/uk/docs/reference/config-api/client-authentication.v1beta1/)
- Прочитайте [довідник з автентифікації клієнта (v1)](/uk/docs/reference/config-api/client-authentication.v1/)
