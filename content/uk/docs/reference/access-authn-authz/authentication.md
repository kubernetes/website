...
API запити привʼязані або до звичайного користувача, або до службового облікового запису, або обробляються як [анонімні запити](#anonymous-requests). Це означає, що кожен процес всередині або поза кластером, від користувача, який вводить `kubectl` на робочій станції, до `kubelets` на вузлах, до членів панелі управління, повинен автентифікуватися при виконанні запитів до API сервера, або бути обробленим як анонімний користувач.
...
- [токени службових облікових записів](#service-account-tokens) для ServiceAccounts
...
- [Зовнішні інтеграції](#external-integrations)
  ...
  - [Вебхук автентифікації токенів](#webhook-token-authentication)
  - [Автентифікація через зворотний проксі](#authenticating-proxy)
...
Щоб скористатися цією функцією, сертифікат повинен містити атрибут `1.3.6.1.4.1.57683.2`, а також має бути увімкнено [можливість](/docs/reference/command-line-tools-reference/feature-gates/) `AllowParsingUserUIDFromCertAuth` (типово увімкнено).
...
### Автентифікація з JSON Web Token  {#json-web-token-authentication}

Ви можете налаштувати Kubernetes для автентифікації користувачів за допомогою токенів, сумісних із [JSON Web Token](https://www.rfc-editor.org/info/rfc7519) (JWT). Механізм автентифікації JWT використовується для токенів ServiceAccount, які видає сам Kubernetes, а також його можна використовувати для інтеграції з іншими джерелами ідентифікації.
...
- issuer:
    ...
    # - controlplane: для трафіку, призначеного для панелі управління.
  ...
  claimMappings:
    ...
    - expression: "!user.username.startsWith('system:')"
      ...
      message: 'неможна використовувати це імʼя користувача, зарезервовано префіксом system:'
    - expression: "user.groups.all(group, !group.startsWith('system:'))"
      message: 'неможна використовувати цю назву групи, зарезервовано префіксом system:'
{{< /highlight >}}
...
- Вираз зіставлення твердження
  ...
  - issuer:
    ...
    claimMappings:
      ...
      - key: 'example.com/tenant'
        valueExpression: 'claims.tenant'
    userValidationRules:
    - expression: "!user.username.startsWith('system:')" # вираз буде оцінений як true, тож валідація пройде успішно.
  ...
  Токен із зазначеною вище `AuthenticationConfiguration` створить наступний обʼєкт `UserInfo` і успішно автентифікує користувача.
  ...
  який не пройде перевірку користувача, оскільки імʼя користувача починається з `system:`. Сервер API поверне помилку `401 Unauthorized`.
  {{% /tab %}}
  {{< /tabs >}}
...
_Вебхук автентифікації_ Kubernetes — є механізмом який здійснює виклик HTTP для перевірки токенів на предʼявника.
...
clusters:
  - name: name-of-remote-authn-service
    cluster:
      ...
      server: https://authn.example.com/authenticate # URL віддаленого сервісу для запиту. 'https' рекомендовано для операційного використання.
...
```yaml
{
  ...
  "kind": "TokenReview",
  "spec": {
    # Непрозорий токен на предʼявника носія, надісланий на API-сервер
    "token": "014fbff9a07c...",
...
### Автентифікація через зворотний проксі-сервер {#authenticating-proxy}
...
: _Обовʼязково; нечутливий до регістру._ Імена заголовків, які слід перевірити, в зазначеному порядку, для ідентифікації користувача. Перший заголовок, що містить значення, використовується як імʼя користувача.

`--requestheader-group-headers`
: _Необовʼязково; нечутливий до регістру._ Імена заголовків, які слід перевірити, в зазначеному порядку, для груп користувача. Рекомендується використовувати `X-Remote-Group`. Усі значення у всіх зазначених заголовках використовуються як імена груп.
...
- name: my-user
  user:
    exec:
      ...
      # встановіть змінну середовища, передайте аргумент інструменту, що вказує, яку версію очікує втулок виконання,
      # або прочитайте версію з обʼєкта ExecCredential у змінній середовища KUBERNETES_EXEC_INFO.
...
Альтернативно, можна повернути PEM-кодований сертифікат клієнта та ключ для використання TLS-автентифікації клієнта. Якщо втулок повертає різний сертифікат та ключ при наступному виклику, `k8s.io/client-go` закриє існуючі зʼєднання з сервером, щоб змусити новий TLS-обмін.
