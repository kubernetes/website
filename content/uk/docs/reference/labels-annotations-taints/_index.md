...
Частина специфікації, яка використовується для реалізації [обрізки на основі ApplySet в kubectl](/docs/tasks/manage-kubernetes-objects/declarative-config/#alternative-kubectl-apply-f-directory-prune). Ця анотація застосовується до батьківського обʼєкта, який використовується для відстеження ApplySet для оптимізації списку обʼєктів-членів ApplySet. Не є обовʼязковою у специфікації ApplySet, оскільки інструменти можуть виконувати виявлення або використовувати іншу оптимізацію. Однак, починаючи з версії Kubernetes {{< skew currentVersion >}}, kubectl вимагає її наявності. Якщо присутнє, значення цієї анотації має бути розділеним комами списком типів груп у форматі повної назви, тобто. `<resource>.<group>`.
...
Частина специфікації, яка використовується для реалізації [обрізки на основі ApplySet в kubectl](/docs/tasks/manage-kubernetes-objects/declarative-config/#alternative-kubectl-apply-f-directory-prune). Ця мітка робить обʼєкт батьківським обʼєктом ApplySet. Його значенням є унікальний ідентифікатор ApplySet, який походить від ідентифікатора самого батьківського обʼєкта. Цей ідентифікатор **повинен** бути кодуванням base64 (з використанням безпечного для URL кодування RFC4648) хешу group-kind-name-namespace обʼєкта, на якому він знаходиться, у вигляді: `<base64(sha256(<name>.<namespace>.<kind>.<group>))>`. Між значенням цієї мітки та UID обʼєкта немає звʼязку.
...
Ця анотація додається до створених вимог на ресурси. Її значення відповідає назві вимоги ресурсу у файлі `.spec` будь-якого Podʼа(ів), для якого було створено ResourceClaim. У рамках [динамічного розподілу ресурсів](/docs/concepts/resource-management/dynamic-resource-allocation/) функція метаданих виявлених пристроїв використовує цю анотацію для привʼязки згенерованого ResourceClaim до імені заявки Podʼа (`pod.spec.resourceClaims[].name`) для заявок на основі шаблонів. Цю анотацію управляє Kubernetes, тому її не слід змінювати.
...
Приклад: `csi.alpha.kubernetes.io/node-id: "node-12345"`

Використовується для: VolumeAttachments

Ця анотація записує ідентифікатор вузла, який використовує драйвер CSI, як резервний варіант, коли обʼєкт [CSINode](/docs/reference/kubernetes-api/storage/csi-node-v1/) недоступний. Sidecar контейнер external-attacher CSI заповнює її перед тим, як том буде приєднано.

Вона забезпечує резервний механізм для відʼєднання томів, коли відповідний CSINode відсутній.

Оскільки ця анотація застаріла, проєкт Kubernetes рекомендує **не** встановлювати її на VolumeAttachment або на будь-який інший обʼєкт.

### csi.volume.kubernetes.io/nodeid (deprecated) {#csi-volume-kubernetes-io-nodeid}

Тип: Annotation

Приклад: `csi.volume.kubernetes.io/nodeid: "node-12345"`

Використовується для: Nodes

Ця анотація визначає ідентифікатор вузла, як його розуміє драйвер Container Storage Interface (CSI). `kubelet` заповнює цю анотацію, викликаючи метод gRPC `NodeGetInfo` драйвера CSI, щоб отримати ідентифікатор вузла під час реєстрації драйвера. Sidecar контейнер external-attacher зчитує цю анотацію, щоб отримати ідентифікатор вузла під час приєднання або відʼєднання томів.

Ця анотація застаріла на користь обʼєкта CSINode, який надає ту саму функціональність через поле [`spec.drivers[].nodeID`](/docs/reference/kubernetes-api/storage/csi-node-v1/#CSINodeDriver).
...
Ця анотація зберігає правила спорідненості вузла для PersistentVolume у вигляді серіалізованого в JSON обʼєкта `NodeAffinity`. Планувальник використовував ці правила для обмеження вузлів, які можуть отримати доступ до тому.

Ця анотація застаріла починаючи з Kubernetes v1.10. Замість неї використовуйте [поле `nodeAffinity`](/docs/concepts/storage/persistent-volumes/#node-affinity) у специфікації PersistentVolume.
...
Ця анотація додається інструментами, які автоматично створюють ресурси [Gateway API](/docs/concepts/services-networking/gateway/). Значення визначає інструмент, який створив ресурс (наприклад, `ingress2gateway`). Анотація є лише інформаційною і не впливає на поведінку жодної реалізації Gateway API.
...
Позначення буде додано до вузла під час його ініціалізації, щоб уникнути гонитви.
...
Докладнішу інформацію див. у статті [Доступ адміністратора до динамічного розподілу ресурсів](/docs/concepts/resource-management/dynamic-resource-allocation/dra-api/#admin-access).
