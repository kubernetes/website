...
Ця анотація додається до створених вимог на ресурси. Її значення відповідає назві вимоги ресурсу у файлі `.spec` будь-якого Podʼа(ів), для якого було створено ResourceClaim. У рамках [динамічного розподілу ресурсів](/docs/concepts/scheduling-eviction/dynamic-resource-allocation/) функція метаданих виявлених пристроїв використовує цю анотацію для привʼязки згенерованого ResourceClaim до імені заявки Podʼа (`pod.spec.resourceClaims[].name`) для заявок на основі шаблонів. Цю анотацію управляє Kubernetes, тому її не слід змінювати.
