...
Файл: Шлях, переданий як прапорець у командному рядку. Файли за цим шляхом будуть періодично моніторитися на наявність оновлень. Період моніторингу типовно становить 20 секунд і налаштовується за допомогою прапорця.
...
<table style="width: 100%; table-layout: fixed;">
    ...
    <tbody>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Необовʼязковий кореневий cgroup для використання з Podʼами. Обробляється контейнерним середовищем на основі принципу найкращих зусиль. Типово: '', що означає використання типовного значення контейнерного середовища. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Kubelet завантажить свою початкову конфігурацію з цього файлу. Шлях може бути абсолютним або відносним; відносні шляхи починаються з поточної робочої теки kubelet. Пропустіть цей прапорець, щоб використовувати вбудовані типовні значення конфігурації. Прапорці командного рядка переважають над конфігурацією з цього файлу.</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Шлях до теки, що містить додаткові конфігураційні файли, які перевизначають типовні налаштування та налаштування з файлу --config. Шлях може бути абсолютним або відносним; відносні шляхи починаються з поточної робочої теки kubelet. Додаткові файли конфігурації повинні мати суфікс '.conf' (наприклад, '99-kubelet-address.conf') і обробляються в лексичному порядку. Завантажуються лише файли '.conf'; всі інші файли ігноруються. Всі файли .conf у теці та її підтеках обробляються в лексичному порядку. [default='']</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Встановлює значення періоду квоти CPU CFS, <code>cpu.cfs_period_us</code>, зазвичай використовується типовне значення ядра Linux. Потрібно увімкнути функціональну можливість CustomCPUCFSQuotaPeriod, щоб встановити нетиповні значення. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Набір параметрів key=value CPU Manager, які можна використовувати для точного налаштування їхньої поведінки. Якщо не надано, залишити типовну поведінку. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>QPS для обмеження створення подій. Число має бути &gt;= 0. Якщо 0, обмеження не застосовується. Примітка: якщо не встановлено, застосовується типова конфігурація kubelet, наразі 50. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Максимальний дозволений період відстрочки (у секундах) для використання при припиненні роботи подів у відповідь на досягнення порогу мʼякого виселення.  Якщо значення відʼємне, відкладіть до значення, вказаного для підсистеми. (ВИКОРИСТАННЯ ПАРАМЕТРУ ЗАСТАРІЛЕ: Цей параметр слід встановлювати через файл конфігурації, вказаний прапорцем --config Kubelet. Докладнішу інформацію див. на сторінці <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/.)</a></p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>[Експериментально] Шлях до виконуваного файлу монтувальника. Залиште порожнім, щоб використовувати типовний <code>mount</code>. (ЗАСТАРІЛО: буде видалено в версії 1.24 або пізніше на користь використання CSI.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Точка доступу до віддаленого сервісу образів. Якщо не вказано, типовно буде така ж, як і у --container-runtime-endpoint. Unix Domain Socket підтримуються в Linux, тоді як точки доступу 'npipe' і 'tcp' підтримуються у Windows. Приклади: 'unix:///path/to/runtime.sock', 'npipe:////./pipe/runtime'. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Якщо увімкнено, kubelet буде інтегруватися з повідомленням memcg ядра для визначення, чи перевищено порогові значення памʼяті для виселення, замість періодичного опитування. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>IP-адреса (або список IP-адрес для двох стеків, розділених комами) вузла. Якщо не встановлено, kubelet використовуватиме типовну IPv4-адресу вузла, якщо така є, або його типовну IPv6-адресу, якщо IPv4-адрес немає. Ви можете передати '::', щоб віддати перевагу типовній IPv6-адресі замість типовної IPv4-адреси. Якщо хмарний провайдер налаштований як зовнішній, цей прапорець допоможе запустити вузол з відповідною IP-адресою.</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Встановлює максимальну кількість процесів на Pod. Якщо <code>-1</code>, kubelet типовно використовує доступну на вузлі ємність PID. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Стандартна поведінка kubelet для налаштування ядра. Якщо встановлено, kubelet видасть помилку, якщо будь-яке з налаштувань ядра відрізняється від типовних значень kubelet. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Вмикає використання RuntimeDefault як типовного профілю seccomp для всіх навантажень.</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Витягує образи по одному. Рекомендується <em>не</em> змінювати типовні значення на вузлах, які використовують демон Docker версії &lt; 1.9 або сховище Aufs. Деталі дивіться в тікеті #10959. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;">Список наборів шифрів для сервера, розділений комами. Якщо не вказано, будуть використані типовні набори шифрів Go.<br/>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Набір параметрів політики Topology Manager у форматі key=value, які можна використовувати для тонкого налаштування їх поведінки. Якщо не вказані, зберігається типовна поведінка. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
        <tr>
        ...
        <tr>
            <td></td>
            <td style="line-height: 130%; word-wrap: break-word;"><p>Вказує інтервал, через який kubelet обчислює та кешує використання диска томів для всіх Podʼів і томів. Щоб вимкнути обчислення томів, встановіть відʼємне число. (ЗАСТАРІЛО: Цей параметр слід налаштовувати через файл конфігурації, вказаний прапорцем kubelet <code>--config</code>. Дивіться <a href="/uk/docs/tasks/administer-cluster/kubelet-config-file/">https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/</a> для отримання додаткової інформації.)</p></td>
        </tr>
    </tbody>
</table>
