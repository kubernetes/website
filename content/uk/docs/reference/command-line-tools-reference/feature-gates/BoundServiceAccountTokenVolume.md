...
removed: true
---
Переносить томи ServiceAccount для використання проєкційного тому, що складається з ServiceAccountTokenVolumeProjection. Адміністратори кластера можуть використовувати метрику `serviceaccount_stale_tokens_total` для моніторингу робочих навантажень, які залежать від розширених токенів. Якщо таких навантажень немає, вимкніть розширені токени, запустивши `kube-apiserver` з прапорцем `--service-account-extend-token-expiration=false`.
