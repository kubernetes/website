...
Функціональна можливість застосовується до пристроїв, які керуються як [втулками пристроїв](/docs/concepts/extend-kubernetes/compute-storage-net/device-plugins/#device-plugin-and-unhealthy-devices), так і [динамічним розподілом ресурсів](/docs/concepts/resource-management/dynamic-resource-allocation/dra-observability/#device-health-monitoring). Див. [Device plugin and unhealthy devices](/docs/concepts/extend-kubernetes/compute-storage-net/device-plugins/#device-plugin-and-unhealthy-devices) для отримання додаткової інформації.
