...
{{< note >}}
`/flagz` повідомляє прапорці командного рядка та стандартні значення. Для компонентів, які також завантажують файли конфігурації, такі як kubelet та kube-proxy, фактична робоча конфігурація може відрізнятися від `/flagz`; використовуйте `/configz`, де це доступно, щоб перевірити обʼєднану конфігурацію компонента.
{{< /note >}}
