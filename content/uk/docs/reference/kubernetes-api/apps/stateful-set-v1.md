...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      Можливі значення enum:
      <ul>
        <li><code>"OnDelete"</code> вимикає впорядковані поступові перезапуски. Відстеження версій виконується на основі «best-effort» — контролер намагатиметься врешті-решт звести currentRevision StatefulSet до updateRevision. Поди створюються заново з StatefulSetSpec, коли вони видаляються вручну. Коли виконується операція масштабування з цією стратегією, нові поди будуть створені з версії специфікації, вказаної в updateRevision StatefulSet.</li>
        <li><code>"Recreate"</code> вказує, що всі наявні поди будуть видалені та повністю завершені перед створенням будь-яких подів нової ревізії. Це гарантує, що поди старої та нової ревізії ніколи не запускаються одночасно. Це тип альфа і вимагає увімкнення функціональної можливості StatefulSetRecreateStrategy. Перехід на стратегію Recreate дозволений, коли функціональну можливість увімкнено, а перехід від неї до іншої стратегії оновлення також дозволений. +featureGate=StatefulSetRecreateStrategy</li>
