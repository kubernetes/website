...
- **type** (string)
  ...
  - `"LoadBalancer"` означає, що сервіс буде відкритий через зовнішній балансувальник навантаження (якщо хмарний провайдер підтримує це), на додачу до типу 'NodePort'.
  - `"NodePort"` означає, що сервіс буде відкритий на одному порту кожного вузла, на додачу до типу 'ClusterIP'.
...
- **ipFamilyPolicy** (string)
  ...
  Можливі значення enum:
  - `"PreferDualStack"` означає, що цей сервіс віддає перевагу подвійному стеку, коли кластер налаштований для подвійного стеку. Якщо кластер не налаштований для подвійного стеку, сервіс буде призначено одне IP-сімейство. Якщо IP-сімейство не встановлено в service.spec.ipFamilies, сервісу буде призначено стандартне IP-сімейство, налаштоване в кластері.
  - `"RequireDualStack"` означає, що цей сервіс вимагає подвійного стеку. Використання IPFamilyPolicyRequireDualStack в кластері з одним стеком призведе до помилок перевірки. IP-сімейства (і їх порядок), призначені цьому сервісу, базуються на service.spec.ipFamilies. Якщо service.spec.ipFamilies не було надано, вони будуть призначені відповідно до налаштувань кластера. Якщо service.spec.ipFamilies має лише один запис, альтернативне IP-сімейство буде додано apiserver.
  - `"SingleStack"` означає, що цей сервіс повинен мати одне IP-сімейство. Призначене IP-сімейство базується на стандартному IP-сімействі, використовуваному кластером, або як визначено в полі service.spec.ipFamilies.
...
- **externalIPs** ([]string)
  ...
  *Atomic: буде замінено під час злиття*
  ...
  Можливі значення enum:
  - `"ClientIP"` є заснованим на IP-адресі клієнта.
...
- **externalTrafficPolicy** (string)
  ...
  - `"Local"` зберігає IP-адресу джерела трафіку, маршрутизуючи лише до точок доступу на тому ж вузлі, на якому був отриманий трафік (трафік буде відхилено, якщо немає локальних точок доступу).
...
- **internalTrafficPolicy** (string)
  ...
  Можливі значення enum:
  - `"Cluster"` маршрутизує трафік до всіх точок доступу.
  - `"Local"` маршрутизує трафік лише до точок доступу на тому ж вузлі, що й Pod (трафік буде відхилено, якщо немає локальних точок доступу).
