...
- **completionMode** (string)
  ...
  - `"Indexed"` є режимом завершення Завдання. У цьому режимі Podʼи, якими керує Job, отримують асоційований індекс завершення від 0 до (.spec.completions - 1). Завдання вважається завершеним, коли для кожного індексу є один успішно завершений Pod.
  - `"NonIndexed"` є режимом завершення Завдання. У цьому режимі завдання вважається завершеним, коли є .spec.completions успішно завершених Podʼів. Завершення Podʼів є аналогічним одне одному.
...
- **podReplacementPolicy** (string)
  ...
  При використанні podFailurePolicy, Failed є єдиним дозволеним значенням. TerminatingOrFailed і Failed дозволені значення, коли podFailurePolicy не використовується.

  Можливі значення enum:
  - `"Failed"` означає чекати, поки раніше створений Pod повністю не завершиться (буде мати фазу Failed або Succeeded), перш ніж створювати замінний Pod.
  - `"TerminatingOrFailed"` означає, що ми відтворюємо Podʼи, коли вони завершуються (мають metadata.deletionTimestamp) або зазнали невдачі.
  ...
  - **podFailurePolicy.rules** ([]PodFailurePolicyRule), обовʼязково
    ...
    *PodFailurePolicyRule описує, як обробляється невдача Podʼа, коли виконуються вимоги. Одне з onExitCodes і onPodConditions, але не обидва, можуть бути використані в кожному правилі.*
    ...
    - **podFailurePolicy.rules.action** (string), обовʼязково
      ...
      Можливі значення enum:
      - `"Count"` є дією, яка може бути виконана при невдачі Podʼа — невдача Podʼа обробляється стандартним способом — збільшення лічильника по відношенню до .backoffLimit, представлений полем .status.failed завдання, збільшується.
      - `"FailIndex"` є дією, яка може бути виконана при невдачі Podʼа — позначає індекс завдання як Failed, щоб уникнути перезапусків у цьому індексі. Цю дію можна використовувати лише тоді, коли встановлено backoffLimitPerIndex.
      - `"FailJob"` є дією, яка може бути виконана при невдачі Podʼа — позначає завдання Podʼа як Failed і завершує всі запущені Podʼи.
      - `"Ignore"` є дією, яка може бути виконана при невдачі Podʼа — лічильник по відношенню до .backoffLimit, представлений полем .status.failed завдання, не збільшується і створюється заміна Pod.
