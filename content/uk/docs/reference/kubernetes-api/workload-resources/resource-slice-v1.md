  ...
  *Device представляє один індивідуальний апаратний екземпляр, який може бути вибраний на основі його атрибутів. Окрім імені, має бути встановлено точно одне поле.*
  ...
  - **devices.bindingConditions** ([]string)
    ...
    *Atomic: буде замінено під час злиття*
    ...
    Це бета-поле, яке вимагає увімкнення функціональних можливостей DRADeviceBindingConditions та DRAResourceClaimDeviceStatus.
  ...
  - **devices.nodeAllocatableResourceMappings** (map[string]NodeAllocatableResourceMapping)
    ...
    NodeAllocatableResourceMappings визначає зіставлення ресурсів вузла, які керуються драйвером DRA, що надає цей пристрій. Це включає ресурси, які наразі повідомляються в v1.Node `status.allocatable` і не є розширеними ресурсами (див. [https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#extended-resources](/docs/concepts/configuration/manage-resources-containers/#extended-resources)). Приклади включають "cpu", "memory", "ephemeral-storage" та hugepages. Крім стандартних запитів, зроблених через Pod `spec`, ці ресурси також можуть бути запитані через вимоги та виділені драйвером DRA. Наприклад, драйвер DRA для CPU може виділяти ексклюзивні CPU або допоміжні залежності пам'яті вузла для пристрою-акселератора. Ключі цієї карти є назвами ресурсів, доступних на вузлі (наприклад, "cpu", "memory"). Розширені назви ресурсів не дозволяються як ключі.
    ...
    *NodeAllocatableResourceMapping визначає перетворення між одиницями пристрою/ємності DRA, запитаними, та відповідною кількістю ресурсу вузла, доступного для виділення.*
    ...
    - **devices.nodeAllocatableResourceMappings.allocationMultiplier** (<a href="{{< ref "../common-definitions/quantity#Quantity" >}}">Quantity</a>)
      ...
      AllocationMultiplier використовується як множник для виділеної кількості пристроїв або виділеної ємності в запиті. Зазвичай дорівнює 1, якщо не вказано. Використання цього поля також залежить від того, чи встановлено `capacityKey`.

      1. Якщо `capacityKey` НЕ встановлено: `allocationMultiplier` множить кількість пристроїв, виділених для запиту.

         - a. Драйвер DRA, який представляє кожне ядро CPU як пристрій, матиме {ResourceName: "cpu", allocationMultiplier: "2"} у своїх `nodeAllocatableResourceMappings`. Якщо 4 пристрої виділено для запиту, 4 \* 2 CPU будуть вважатися виділеними та відняті з ємності вузла.
         - b. GPU-пристрій, який потребує додаткової памʼяті вузла на кожне виділення GPU, матиме {ResourceName: "memory", allocationMultiplier: "2Gi"}. Кожен виділений екземпляр GPU цього типу враховуватиме 2Gi памʼяті.

      2. Якщо `capacityKey` встановлено: `allocationMultiplier` множиться на кількість спожитої ємності. Остаточна кількість ресурсу вузла, доступного для виділення, дорівнює `consumedCapacity[capacityKey]` \* `allocationMultiplier`. Наприклад, якщо споживається ємність пристрою "dra.example.com/cores", і кожне "ядро" забезпечує 2 "cpu", зіставлення буде: {ResourceName: "cpu", capacityKey: "dra.example.com/cores", allocationMultiplier: "2"}. Якщо запит споживає 8 "dra.example.com/cores", відбиток CPU становитиме 8 \* 2 = 16.

    - **devices.nodeAllocatableResourceMappings.capacityKey** (string)

      CapacityKey посилається на назву ємності, визначену як ключ у мапі `spec.devices[*].capacity`. Коли це поле встановлено, значення, повʼязане з цим ключем у мапі `status.allocation.devices.results[*].consumedCapacity` (для конкретного виділення запиту), визначає базову кількість для ресурсу вузла, доступного для виділення. Якщо також встановлено `allocationMultiplier`, він множиться на базову кількість. Наприклад, якщо `spec.devices[*].capacity` має запис "dra.example.com/memory": "128Gi", і це поле встановлено на "dra.example.com/memory", тоді для виділення запиту, яке споживає { "dra.example.com/memory": "4Gi" }, базова кількість для ресурсу вузла буде "4Gi", а `allocationMultiplier` слід опустити або встановити на "1".
  ...
  - **devices.taints** ([]DeviceTaint)
    ...
    *Пристрій, до якого прикріплено цей taint, має «вплив» на будь-яку заявку, яка не толерує taint, і, через заявку, на podʼи, що використовують цю заявку.*
    ...
    - **devices.taints.effect** (string), обовʼязково
      ...
      - `"NoExecute"` виселяє будь-які вже запущені pod, які не толерують taint пристрою.
      - `"NoSchedule"` не дозволяє новим pod плануватися з використанням tainted пристрою, якщо вони не толерують taint, але дозволяє всім pod, поданим до Kubelet без проходження через планувальник, запускатися, і дозволяє всім вже запущеним pod продовжувати працювати.
    ...
    - **devices.taints.timeAdded** (Time)
      ...
      TimeAdded представляє час, коли taint було додано або (тільки в DeviceTaintRule) ефект було змінено. Додається автоматично під час створення або оновлення, якщо не встановлено.
