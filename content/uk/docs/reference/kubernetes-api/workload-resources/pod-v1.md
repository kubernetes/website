...
- **tolerations** ([]Toleration)
  ...
  *Pod, до якого прикріплено цю толерантність, толерує будь-який taint, який відповідає трійці <key,value,effect> за допомогою оператора зіставлення <operator>.*
  ...
  - **tolerations.effect** (string)
    ...
    Можливі значення enum:
    - `"NoExecute"` Виселити будь-які вже запущені podʼи, які не толерують taint. Наразі застосовується NodeController.
    - `"NoSchedule"` Не дозволяти новим podʼам плануватися на вузол, якщо вони не толерують taint, але дозволяти всім podʼам, поданим до Kubelet без проходження через планувальник, запускатися, і дозволяти всім вже запущеним podʼам продовжувати працювати. Застосовується планувальником.
    - `"PreferNoSchedule"` Як TaintEffectNoSchedule, але планувальник намагається не планувати нові podʼи на вузол, а не забороняє новим podʼам плануватися на вузол повністю. Застосовується планувальником.
...
- **preemptionPolicy** (string)
  ...
  Можливі значення enum:
  - `"Never"` означає, що pod ніколи не випереджає інші podʼи з нижчим пріоритетом.
  ...
  *TopologySpreadConstraint визначає, як розподіляти відповідні Podʼи серед заданої топології.*
  ...
  - **topologySpreadConstraints.whenUnsatisfiable** (string), обовʼязково
    ...
    - ScheduleAnyway наказує планувальнику розмістити Pod у будь-якому місці, але з наданням більшої переваги топологіям, які допоможуть зменшити нерівномірність розподілу. Умова вважається "незадовільною" для вхідного Podʼа, якщо і тільки якщо кожне можливе призначення вузла для цього Podʼа порушуватиме "MaxSkew" у деякій топології. Наприклад, у кластері з 3 зонами MaxSkew встановлено на 1, і Podʼи з однаковим labelSelector розподіляються як 3/1/1:
      ...
      Можливі значення enum:
    - `"DoNotSchedule"` наказує планувальнику не розміщувати Pod, якщо обмеження не задовольняються.
    - `"ScheduleAnyway"` наказує планувальнику розмістити Pod, навіть якщо обмеження не задовольняються.
  ...
  - **topologySpreadConstraints.nodeAffinityPolicy** (string)
    ...
    Можливі значення enum:
    - `"Honor"` означає використовувати цю директиву планування при розрахунку перекосу розподілу топології Pod.
    - `"Ignore"` означає ігнорувати цю директиву планування при розрахунку перекосу розподілу топології Pod.

  - **topologySpreadConstraints.nodeTaintsPolicy** (string)

    NodeTaintsPolicy вказує, як ми будемо враховувати taints вузлів при розрахунку перекосу розподілу топології Pod. Варіанти:
    - Honor: вузли без taints, а також вузли з taints, для яких вхідний Pod має toleration, включаються до розрахунків.
    - Ignore: taints вузлів ігноруються. Всі вузли включаються до розрахунків.

    Якщо це значення дорівнює null, поведінка еквівалентна політиці Ignore.
...
- **schedulingGroup** (PodSchedulingGroup)
  ...
  SchedulingGroup надає посилання на об’єкт групування середовища виконання безпосереднього планування, до якого належить цей Pod. Це поле використовується планувальником для ідентифікації групи та застосування відповідних політик групового планування. Приналежність до групи також впливає на інші аспекти життєвого циклу Podʼа, що мають значення в ширшому контексті планування, такі як випередження, прив’язка ресурсів тощо. Якщо це поле не вказано, Pod розглядається як окрема одиниця в усіх цих аспектах. Обʼєкт групи, на який посилається це поле, може не існувати на момент створення Podʼа. Це поле є незмінним, але обʼєкт групи з тим самим іменем може бути створений заново з іншими політиками. Виконання цієї дії під час планування Podʼа може призвести до розміщення, що не відповідає очікуваним політикам.

  <a name="PodSchedulingGroup"></a>
  *PodSchedulingGroup визначає екземпляр групи планування середовища виконання, до якого належить Pod. Планувальник використовує цю інформацію для застосування семантики планування з урахуванням навантаження. Повинно бути вказано точно одне поле.*

  - **schedulingGroup.podGroupName** (string)

    PodGroupName визначає імʼя окремого обʼєкта PodGroup, який представляє екземпляр середовища виконання цієї групи. Має бути піддоменом DNS.
...
- **dnsPolicy** (string)
  ...
  Можливі значення enum:
  - `"ClusterFirst"` вказує, що pod повинен використовувати DNS кластера спочатку, якщо hostNetwork не встановлено в true, якщо він доступний, тоді використовувати стандартні налаштування DNS (визначені kubelet).
  - `"ClusterFirstWithHostNet"` вказує, що pod повинен використовувати DNS кластера спочатку, якщо він доступний, тоді використовувати стандартні налаштування DNS (визначені kubelet).
  - `"Default"` вказує, що pod повинен використовувати стандартні налаштування DNS (визначені kubelet).
  - `"None"` вказує, що pod повинен використовувати порожні налаштування DNS. Параметри DNS, такі як nameservers та search paths, повинні бути визначені через DNSConfig.
...
- **securityContext** (PodSecurityContext)
  ...
  *PodSecurityContext містить атрибути безпеки на рівні Podʼа та загальні налаштування контейнера. Деякі поля також присутні у container.securityContext. Значення полів container.securityContext мають пріоритет над значеннями полів PodSecurityContext.*
  ...
  - **securityContext.appArmorProfile** (AppArmorProfile)
    ...
    *AppArmorProfile визначає налаштування AppArmor для контейнера або podʼа.*
    ...
    - **securityContext.appArmorProfile.type** (string), обовʼязково
      ...
      - `"Unconfined"` вказує, що жоден профіль AppArmor не буде застосовано.
  ...
  - **securityContext.fsGroupChangePolicy** (string)
    ...
    Можливі значення enum:
    - `"Always"` вказує, що права власності та дозволи тому завжди повинні змінюватися щоразу, коли том монтується всередині Podʼа. Це типова поведінка.
    - `"OnRootMismatch"` вказує, що права власності та дозволи тому будуть змінюватися лише тоді, коли права та власність кореневої теки не відповідають очікуваним правам на томі. Це може допомогти скоротити час, необхідний для зміни прав власності та дозволів тому.
  ...
  - **securityContext.seccompProfile** (SeccompProfile)
    ...
    *SeccompProfile визначає налаштування профілю seccomp для Podʼа/контейнера. Може бути встановлено лише одне джерело профілю.*
    ...
    - **securityContext.seccompProfile.type** (string), обовʼязкове
      ...
      Можливі значення enum:
      - `"Localhost"` вказує, що має бути використаний профіль, визначений у файлі на вузлі. Розташування файлу відносно \<kubelet-root-dir>/seccomp.
      - `"RuntimeDefault"` представляє стандартний профіль seccomp середовища виконання контейнерів.
  ...
  - **securityContext.supplementalGroupsPolicy** (string)
    ...
    Можливі значення enum:
    - `"Merge"` означає, що надані контейнером SupplementalGroups та FsGroup (вказані в SecurityContext) будуть обʼєднані з групами основного користувача, визначеними в образі контейнера (у /etc/group).
    - `"Strict"` означає, що надані контейнером SupplementalGroups та FsGroup (вказані в SecurityContext) будуть використані замість будь-яких груп, визначених в образі контейнера.
...
- **resourceClaims** ([]PodResourceClaim)
  ...
  *Коли функціональна можливість DRAWorkloadResourceClaims увімкнена і цей Pod належить до PodGroup, PodResourceClaim співставляється з PodGroupResourceClaim, якщо всі їхні поля збігаються (Name, ResourceClaimName та ResourceClaimTemplateName). Співставлений запит посилається на один ResourceClaim, спільний для всіх Pod у PodGroup, зарезервований для PodGroup у ResourceClaimStatus.ReservedFor, а не для окремих Pod.*
...
- **imagePullPolicy** (string)
  ...
  Можливі значення:
  - `"Always"` означає, що kubelet завжди намагається отримати останній образ. Контейнер не запуститься, якщо отримання не вдасться.
  - `"IfNotPresent"` означає, що kubelet отримує образ, якщо його немає на диску. Контейнер не запуститься, якщо образу немає і отримання не вдасться.
  - `"Never"` означає, що kubelet ніколи не отримує образ, а використовує лише локальний образ. Контейнер не запуститься, якщо образу немає.
...
- **ports** ([]ContainerPort)
  ...
  *ContainerPort представляє мережевий порт в одному контейнері.*
  ...
  - **ports.protocol** (string)
    ...
    Можливі значення перерахування:
    - `"SCTP"` — протокол SCTP.
    - `"TCP"` — протокол TCP.
    - `"UDP"` — протокол UDP.
...
- **volumeMounts** ([]VolumeMount)
  ...
  *VolumeMount описує монтування Тому всередині контейнера.*
  ...
  - **volumeMounts.mountPropagation** (string)
    ...
    - `"HostToContainer"` означає, що том у контейнері буде отримувати нові монтування від хоста або інших контейнерів, але файлові системи, змонтовані всередині контейнера, не будуть поширюватися на хост або інші контейнери. Цей режим рекурсивно застосовується до всіх монтувань у томі ("rslave" у термінології Linux).
    - `"None"` означає, що том у контейнері не буде отримувати нові монтування від хоста або інших контейнерів, а файлові системи, змонтовані всередині контейнера, не будуть поширюватися на хост або інші контейнери. Цей режим відповідає "private" у термінології Linux.
...
- **terminationMessagePolicy** (string)
  ...
  Можливі значення enum:
  - `"FallbackToLogsOnError"` буде читати останній вміст журналів контейнера для повідомлення про стан контейнера, коли контейнер виходить з помилкою і terminationMessagePath не має вмісту.
  - `"File"` є стандартною поведінкою і встановлює повідомлення про стан контейнера на вміст terminationMessagePath контейнера при виході.
...
- **securityContext** (SecurityContext)
  ...
  *SecurityContext містить конфігурацію безпеки, яка буде застосована до контейнера. Деякі поля присутні як у SecurityContext, так і в PodSecurityContext. Якщо обидва встановлені, значення в SecurityContext мають пріоритет.*
  ...
  - **securityContext.procMount** (string)
    ...
    procMount вказує тип монтування файлової системи `/proc`, який слід використовувати для контейнерів. Стандартне значення — `Default`, що використовує стандартні налаштування середовища виконання контейнерів для шляхів тільки для читання та замаскованих шляхів. Зверніть увагу, що це поле не може бути встановлено, коли spec.os.name є windows.

    Можливі значення enum:
    - `"Default"` використовує стандартні налаштування середовища виконання контейнерів для шляхів тільки для читання та замаскованих шляхів у /proc. Більшість середовищ виконання контейнерів маскують певні шляхи в /proc, щоб уникнути випадкового розкриття спеціальних пристроїв або інформації.
    - `"Unmasked"` обходить стандартну поведінку маскування середовища виконання контейнерів і забезпечує, що новостворений /proc контейнера залишиться без змін.
  ...
  - **securityContext.seccompProfile** (SeccompProfile)
    ...
    *SeccompProfile визначає налаштування профілю seccomp для Podʼа/контейнера. Може бути встановлено лише одне джерело профілю.*
    ...
    - **securityContext.seccompProfile.type** (string), обовʼязкове
      ...
      - `"Localhost"` вказує, що має бути використаний профіль, визначений у файлі на вузлі. Розташування файлу відносно \<kubelet-root-dir>/seccomp.
      - `"RuntimeDefault"` представляє стандартний профіль seccomp середовища виконання контейнерів.
      - `"Unconfined"` вказує, що жоден профіль seccomp не застосовується (так званий unconfined).
...
- **imagePullPolicy** (string)
  ...
  Можливі значення enum:
  - `"Always"` означає, що kubelet завжди намагається завантажити останній образ. Контейнер завершиться з помилкою, якщо завантаження не вдасться.
  - `"IfNotPresent"` означає, що kubelet завантажує образ, якщо його немає на диску. Контейнер завершиться з помилкою, якщо образу немає і завантаження не вдасться.
  - `"Never"` означає, що kubelet ніколи не завантажує образ, а використовує лише локальний образ. Контейнер завершиться з помилкою, якщо образу немає.
...
- **volumeMounts** ([]VolumeMount)
  ...
  *VolumeMount описує монтування Тому всередині контейнера.*
  ...
  - **volumeMounts.mountPropagation** (string)
    ...
    - `"None"` означає, що том у контейнері не буде отримувати нові монтування від хоста або інших контейнерів, а файлові системи, змонтовані всередині контейнера, не будуть поширюватися на хост або інші контейнери. Зверніть увагу, що цей режим відповідає "private" у термінології Linux.
...
- **terminationMessagePolicy** (string)
  ...
  Можливі значення enum:
  - `"FallbackToLogsOnError"` буде читати останній вміст журналів контейнера для повідомлення про стан контейнера, коли контейнер виходить з помилкою, а terminationMessagePath не має вмісту.
  - `"File"` є стандартною поведінкою і встановлює повідомлення про стан контейнера відповідно до вмісту terminationMessagePath контейнера при виході контейнера.
...
- **securityContext** (SecurityContext)
  ...
  *SecurityContext містить конфігурацію безпеки, яка буде застосована до контейнера. Деякі поля присутні як у SecurityContext, так і в PodSecurityContext. Якщо обидва встановлені, значення в SecurityContext мають пріоритет.*
  ...
  - **securityContext.procMount** (string)
    ...
    procMount позначає тип монтування proc, який слід використовувати для контейнерів. Стандартне значення — Default, яке використовує стандартні налаштування середовища виконання контейнера для шляхів тільки для читання та змаскованих шляхів. Зверніть увагу, що це поле не може бути встановлено, коли spec.os.name є windows.

    Можливі значення enum:
    - `"Default"` використовує стандартні налаштування середовища виконання контейнера для шляхів тільки для читання та змаскованих шляхів для /proc. Більшість середовищ виконання контейнера маскують певні шляхи в /proc, щоб уникнути випадкового розкриття спеціальних пристроїв або інформації.
...
- **phase** (string)
  ...
  - `"Pending"` означає, що Pod прийнято системою, але один або більше контейнерів ще не запущено. Це включає час до привʼязки до вузла, а також час, витрачений на завантаження образів на хост.
  - `"Running"` означає, що Pod привʼязано до вузла і всі контейнери запущено. Принаймні один контейнер все ще працює або знаходиться в процесі перезапуску.
...
- **conditions** ([]PodCondition)
  ...
  *PodCondition містить деталі поточного стану цього Podʼа.*
  ...
  - **conditions.observedGeneration** (int64)
    ...
    Якщо встановлено, це означає .metadata.generation, на основі якого було встановлено стан podʼа.
  ...
  *ContainerStatus містить деталі поточного стану цього контейнера.*
    ...
    - **initContainerStatuses.allocatedResourcesStatus.resources** ([]ResourceHealth)
      ...
      *ResourceHealth представляє стан справності ресурсу. Він містить останню інформацію про стан пристрою. Це частина KEP https://kep.k8s.io/4680.*
      ...
      - **initContainerStatuses.allocatedResourcesStatus.resources.message** (string)
        ...
        Message надає зрозумілий для людини контекст для Health (наприклад, "ECC error count exceeded threshold"). Це поле заповнюється kubelet, коли ResourceHealthStatusMessage увімкнено, якщо втулок DRA повертає повідомлення, і є null в іншому випадку.
  ...
  - **initContainerStatuses.volumeMounts** ([]VolumeMountStatus)
    ...
    *VolumeMountStatus показує стан монтування томів.*
    ...
    - **initContainerStatuses.volumeMounts.volumeStatus** (VolumeStatus)

      volumeStatus представляє статус, специфічний для типу тома, щодо змонтованого тому.

      <a name="VolumeStatus"></a>
      *VolumeStatus представляє статус змонтованого тому. Не більше одного з його членів має бути вказано.*

      - **initContainerStatuses.volumeMounts.volumeStatus.image** (ImageVolumeStatus)

        image представляє OCI-об'єкт (контейнерний образ або артефакт), витягнутий і змонтований на хості kubelet.

        <a name="ImageVolumeStatus"></a>
        *ImageVolumeStatus представляє статус тома на основі образу.*

        - **initContainerStatuses.volumeMounts.volumeStatus.image.imageRef** (string), обов'язково

          ImageRef є дайджестом образу, використаного для цього тому. Він повинен мати значення, подібне до status пода.containerStatuses[i].imageID. Довжина ImageRef не повинна перевищувати 256 символів.
  ...
  *ContainerStatus містить деталі поточного стану цього контейнера.*
    ...
    - **containerStatuses.allocatedResourcesStatus.resources** ([]ResourceHealth)
      ...
      *ResourceHealth представляє стан справності ресурсу. Він містить останню інформацію про стан пристрою. Це частина KEP https://kep.k8s.io/4680.*
      ...
      - **containerStatuses.allocatedResourcesStatus.resources.message** (string)
        ...
        Message надає контекст, зрозумілий для людини, для Health (наприклад, "ECC error count exceeded threshold"). Це поле заповнюється kubelet, коли ResourceHealthStatusMessage увімкнено, якщо плагін DRA повертає повідомлення, і є null в іншому випадку.
  ...
  - **containerStatuses.volumeMounts** ([]VolumeMountStatus)
    ...
    *VolumeMountStatus показує стан монтування томів.*
    ...
    - **containerStatuses.volumeMounts.volumeStatus** (VolumeStatus)
      ...
      *VolumeStatus представляє статус змонтованого тому. Не більше одного з його членів має бути вказано.*
      ...
      - **containerStatuses.volumeMounts.volumeStatus.image** (ImageVolumeStatus)

        image представляє обʼєкт OCI (контейнерний образ або артефакт), який був витягнутий і змонтований на хості kubelet.

        <a name="ImageVolumeStatus"></a>
        *ImageVolumeStatus представляє статус тома на основі образу.*

        - **containerStatuses.volumeMounts.volumeStatus.image.imageRef** (string), required

          ImageRef є дайджестом образу, використаного для цього тому. Він повинен мати значення, подібне до status.containerStatuses[i].imageID podʼа. Довжина ImageRef не повинна перевищувати 256 символів.
  ...
  *ContainerStatus містить деталі поточного стану цього контейнера.*
  ...
  - **ephemeralContainerStatuses.volumeMounts** ([]VolumeMountStatus)
    ...
    *VolumeMountStatus показує стан монтування томів.*
    ...
    - **ephemeralContainerStatuses.volumeMounts.volumeStatus** (VolumeStatus)

      volumeStatus представляє стан, специфічний для типу тома, щодо змонтованого тому.

      <a name="VolumeStatus"></a>
      *VolumeStatus представляє стан змонтованого тому. Необхідно вказати не більше одного з його членів.*

      - **ephemeralContainerStatuses.volumeMounts.volumeStatus.image** (ImageVolumeStatus)

        image представляє OCI обʼєкт (контейнерний образ або артефакт), витягнутий і змонтований на хості kubelet.

        <a name="ImageVolumeStatus"></a>
        *ImageVolumeStatus представляє стан тома на основі образу.*

        - **ephemeralContainerStatuses.volumeMounts.volumeStatus.image.imageRef** (string), обовʼязково
...
- **resourceClaimStatuses** ([]PodResourceClaimStatus)
  ...
  *PodResourceClaimStatus зберігається у PodStatus для кожної PodResourceClaim, яка посилається на ResourceClaimTemplate. Він зберігає згенеровану назву для відповідної ResourceClaim.*
  ...
  - **resourceClaimStatuses.resourceClaimName** (string)
    ...
    Коли функція DRAWorkloadResourceClaims увімкнена і відповідна PodResourceClaim збігається з PodGroupResourceClaim, створеною PodGroup цього Podʼа, тоді це імʼя ResourceClaim, згенерованого та зарезервованого для PodGroup.

    Якщо це поле не встановлено, то створення ResourceClaim не було необхідним. Запис pod.spec.resourceClaims можна ігнорувати в цьому випадку.

- **nodeAllocatableResourceClaimStatuses** ([]NodeAllocatableResourceClaimStatus)

  *Atomic: буде замінено під час злиття*

  NodeAllocatableResourceClaimStatuses містить статус ресурсів, доступних на вузлі, які були виділені для цього пода через DRA-заявки. Це включає ресурси, які наразі повідомляються в v1.Node `status.allocatable` і не є розширеними ресурсами (див. [https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#extended-resources](/docs/concepts/configuration/manage-resources-containers/#extended-resources)). Прикладами є "cpu", "memory", "ephemeral-storage" та hugepages.
  ...
  *NodeAllocatableResourceClaimStatus описує стан ресурсів, доступних на вузлі, які були виділені через DRA.*
  ...
  - **nodeAllocatableResourceClaimStatuses.resourceClaimName** (string), обовʼязково

    ResourceClaimName є назвою ресурсної заявки, на яку посилається под, що призвела до цього виділення ресурсів на вузлі.

  - **nodeAllocatableResourceClaimStatuses.resources** (map[string]<a href="{{< ref "../common-definitions/quantity#Quantity" >}}">Quantity</a>), обовʼязково

    Resources є мапою, де ключем є назва ресурсу, доступного на вузлі, а значенням — загальна кількість, виділена для цієї заявки.

  - **nodeAllocatableResourceClaimStatuses.containers** ([]string)

    *Set: unique values will be kept during a merge*

    Containers перераховує імена всіх контейнерів у цьому поді, які посилаються на цю заявку.
