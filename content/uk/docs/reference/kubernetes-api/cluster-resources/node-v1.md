...
- **taints** ([]Taint)
  ...
  *Вузол, до якого прив’язано цей Taint, має "ефект" на будь-який Pod, який не толерує Taint.*
  ...
  - **taints.effect** (string), обовʼязково
    ...
    - `"NoSchedule"` Не дозволяти новим Podʼам плануватися на вузлі, якщо вони не толерують taint, але дозволяти всім Podʼам, поданим до Kubelet без проходження через планувальник, запускатися, і дозволяти всім вже запущеним Podʼам продовжувати працювати. Застосовується планувальником.
    - `"PreferNoSchedule"` Як TaintEffectNoSchedule, але планувальник намагається не планувати нові Podʼи на вузлі, а не забороняє нові Podʼи плануватися на вузлі повністю. Застосовується планувальником.
...
- **phase** (string)
  ...
  - `"Running"` означає, що вузол налаштований і має працюючі компоненти Kubernetes.
