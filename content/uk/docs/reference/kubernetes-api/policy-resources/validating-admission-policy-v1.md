...
- **spec** (ValidatingAdmissionPolicySpec)
  ...
  spec — специфікація бажаної поведінки ValidatingAdmissionPolicy.
  ...
  *ValidatingAdmissionPolicySpec — це специфікація бажаної поведінки AdmissionPolicy.*
  ...
  - **spec.failurePolicy** (string)
    ...
    - `"Fail"` означає, що помилка виклику webhook призводить до відхилення запиту.
    - `"Ignore"` означає, що помилка виклику webhook ігнорується.
  ...
  - **spec.matchConstraints** (MatchResources)
    ...
    *MatchResources вирішує, чи запускати політику контролю доступу до обʼєкта на основі того, чи відповідає він критеріям відповідності. Правила виключення мають пріоритет над правилами включення (якщо ресурс відповідає обом, він виключається)*
    ...
    - **spec.matchConstraints.excludeResourceRules** ([]NamedRuleWithOperations)
      ...
      *NamedRuleWithOperations є кортежем Operations та Resources з ResourceNames.*
      ...
      - **spec.matchConstraints.excludeResourceRules.scope** (string)
        ...
        - `"Cluster"` означає, що область обмежена обʼєктами з областю дії кластера. Обʼєкти простору імен є кластерними.
        - `"Namespaced"` означає, що область обмежена обʼєктами з простору імен.
    ...
    - **spec.matchConstraints.matchPolicy** (string)
      ...
      Можливі значення enum:
      - `"Equivalent"` означає, що запити повинні бути надіслані до вебхука, якщо вони змінюють ресурс, зазначений у правилах, через іншу групу або версію API.
      - `"Exact"` означає, що запити повинні бути надіслані до вебхука лише в разі точного збігу з певним правилом.
...
- **status** (ValidatingAdmissionPolicyStatus)
  ...
  status — представляє статус ValidatingAdmissionPolicy, включаючи попередження, які корисні для визначення, чи працює політика відповідно до очікувань. Заповнюється системою. Тільки для читання.
  ...
  *ValidatingAdmissionPolicyStatus представляє статус політики перевірки допуску.*
  ...
  - **status.conditions** ([]Condition)
    ...
    *Map: унікальні значення по ключу type будуть зберігатися під час злиття*
    ...
    conditions — представляють останні доступні спостереження поточного стану політики.
  ...
  - **status.typeChecking** (TypeChecking)
    ...
    typeChecking — результати перевірки типу для кожного виразу. Наявність цього поля вказує на завершення перевірки типу.
    ...
    *TypeChecking містить результати перевірки типу виразів у ValidatingAdmissionPolicy*
    ...
    - **status.typeChecking.expressionWarnings** ([]ExpressionWarning)
      ...
      *Atomic: буде замінено під час злиття*
      ...
      expressionWarnings — містить попередження перевірки типу для кожного виразу.
      ...
      *ExpressionWarning — це інформація про попередження, що стосується конкретного виразу.*
      ...
      - **status.typeChecking.expressionWarnings.fieldRef** (string), обовʼязково
        ...
        fieldRef — шлях до поля, що посилається на вираз. Наприклад, посилання на вираз першого елемента перевірок є "spec.validations[0].expression".

      - **status.typeChecking.expressionWarnings.warning** (string), обовʼязково

        warning — вміст інформації про попередження перевірки типу у формі, зручною для читання людиною. Кожен рядок попередження містить тип, за яким перевірено вираз, а потім помилку перевірки типу від компілятора.
...
- **metadata** (<a href="{{< ref "../common-definitions/list-meta#ListMeta" >}}">ListMeta</a>)
  ...
  metadata — стандартні метадані списку. Докладніше: <https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds>.
...
- **spec** (ValidatingAdmissionPolicyBindingSpec), обовʼязково

  spec визначає бажану поведінку ValidatingAdmissionPolicyBinding.
  ...
  *ValidatingAdmissionPolicyBindingSpec — це специфікація ValidatingAdmissionPolicyBinding.*
  ...
  - **spec.policyName** (string), обовʼязково

    policyName посилається на імʼя ValidatingAdmissionPolicy, до якого привʼязується ValidatingAdmissionPolicyBinding. Якщо посиланий ресурс не існує, це привʼязування вважається недійсним і буде ігноруватися. Обовʼязково.
  - **spec.validationActions** ([]string), обовʼязково

    *Set: унікальні значення зберігатимуться під час обʼєднання*

    validationActions визначає, як застосовуються перевірки, визначені у вказаній політиці ValidatingAdmissionPolicy. Якщо результат перевірки дорівнює false, ці дії завжди застосовуються відповідно до зазначених дій.

    Помилки, визначені у FailurePolicy ValidatingAdmissionPolicy, застосовуються відповідно до цих дій лише якщо FailurePolicy встановлено на Fail, інакше помилки ігноруються. Це включає помилки компіляції, помилки виконання та неправильні конфігурації політики.

    validationActions оголошується як набір значень дій. Порядок не має значення. validationActions не може містити дублікатів однієї й тієї ж дії.

    Підтримувані значення дій:

    "Deny" вказує, що помилка перевірки призводить до відхилення запиту.

    "Warn" вказує, що помилка перевірки повідомляється клієнту запиту в заголовках HTTP Warning з кодом попередження 299. Попередження можуть надсилатися як для дозволених, так і для відхилених відповідей на допуск.

    "Audit" вказує, що помилка перевірки включається до опублікованого аудиту події для запиту. Подія аудиту міститиме анотацію аудиту `validation.policy.admission.k8s.io/validation_failure` зі значенням, що містить деталі помилок перевірки, відформатовані як список обʼєктів JSON, кожен з яких має наступні поля:

    - message: рядок повідомлення про помилку перевірки
    ...
    - validationActions: дії, застосовані для помилки перевірки
    ...
    "Deny" та "Warn" не можуть використовуватися разом, оскільки ця комбінація безпідставно дублює помилку перевірки як у тілі відповіді API, так і в заголовках HTTP Warning.
  ...
  - **spec.matchResources** (MatchResources)
    ...
    *MatchResources вирішує, чи запускати політику контролю допуску на обʼєкті на основі відповідності критеріям відповідності. Правила виключення мають пріоритет над правилами включення (якщо ресурс відповідає обом, він виключається).*
    ...
    - **spec.matchResources.matchPolicy** (string)
      ...
      Можливі значення enum:
      - `"Equivalent"` означає, що запити повинні надсилатися до вебхука, якщо вони змінюють ресурс, зазначений у правилах, через іншу групу або версію API.
      - `"Exact"` означає, що запити повинні надсилатися до вебхука лише у випадку точного збігу з зазначеним правилом.
    ...
    - **spec.matchResources.resourceRules** ([]NamedRuleWithOperations)
      ...
      *NamedRuleWithOperations є кортежем Operations та Resources з ResourceNames.*
      ...
      - **spec.matchResources.resourceRules.scope** (string)
        ...
        Можливі значення enum:
        - `"*"` означає, що всі області включені.
        - `"Cluster"` означає, що область обмежена обʼєктами на рівні кластера. Обʼєкти простору імен є обʼєктами на рівні кластера.
        - `"Namespaced"` означає, що область обмежена обʼєктами простору імен.
...
- **kind** (string)
  ...
  kind визначає тип REST-ресурсу, який представляє цей обʼєкт. Сервери можуть визначати це з кінцевої точки, до якої клієнт надсилає запити. Не можна оновлювати. У CamelCase. Більше інформації: <https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds>

- **metadata** (<a href="{{< ref "../common-definitions/object-meta#ObjectMeta" >}}">ObjectMeta</a>)

  metadata є стандартними метаданими обʼєкта; Більше інформації: <https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata>.
