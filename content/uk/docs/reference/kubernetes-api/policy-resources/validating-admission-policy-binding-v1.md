...
- **metadata** (<a href="{{< ref "../common-definitions/object-meta#ObjectMeta" >}}">ObjectMeta</a>)
  ...
  metadata — стандартний обʼєкт метаданих; Докладніше: <https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata>.

- **spec** (ValidatingAdmissionPolicyBindingSpec)

  spec — визначення бажаної поведінки ValidatingAdmissionPolicyBinding.
  ...
  *ValidatingAdmissionPolicyBindingSpec є специфікацією ValidatingAdmissionPolicyBinding.*
  ...
  - **spec.policyName** (string), обовʼязково
    ...
    policyName посилається на імʼя ValidatingAdmissionPolicy, до якого прив'язується ValidatingAdmissionPolicyBinding. Якщо посиланий ресурс не існує, це зв'язування вважається недійсним і буде ігноруватися. Обов'язково.
  ...
  - **spec.validationActions** ([]string), обовʼязково
    ...
    *Set: унікальні значення зберігатимуться під час обʼєднання*
    ...
    Помилки, визначені політикою FailurePolicy ValidatingAdmissionPolicy, застосовуються відповідно до цих дій лише у випадку, якщо FailurePolicy встановлено на Fail, інакше помилки ігноруються. Це включає помилки компіляції, помилки виконання та неправильні конфігурації політики.
    ...
    Допустимі значення дій:

    "Deny" визначає, що невдача перевірки призводить до відхилення запиту.

    "Warn" визначає, що невдача перевірки повідомляється клієнту запиту в заголовках HTTP Warning з кодом попередження 299. Попередження можуть надсилатися як для дозволених, так і для відхилених відповідей на запит.

    "Audit" визначає, що невдача перевірки включається в опубліковану подію аудиту для запиту. Подія аудиту міститиме анотацію аудиту `validation.policy.admission.k8s.io/validation_failure` зі значенням, що містить деталі невдач перевірки, відформатовані як список обʼєктів у форматі JSON, кожен з яких має наступні поля:
    ...
    - validationActions: дії, застосовані для невдачі перевірки
  ...
  - **spec.matchResources** (MatchResources)
    ...
    *MatchResources визначає, чи слід застосовувати політику контролю доступу до обʼєкта на основі того, чи відповідає він критеріям відповідності. Правила виключення мають перевагу над правилами включення (якщо ресурс відповідає обом, він буде виключений).*
    ...
    - **spec.matchResources.excludeResourceRules** ([]NamedRuleWithOperations)
      ...
      *NamedRuleWithOperations є кортежем Operations та Resources з ResourceNames.*
      ...
      - **spec.matchResources.excludeResourceRules.scope** (string)
        ...
        - `"Cluster"` означає, що область обмежена обʼєктами з областю кластера. Обʼєкти простору імен є кластерними.
    ...
    - **spec.matchResources.matchPolicy** (string)
      ...
      - `"Exact"` означає, що запити повинні надсилатися до вебхука лише тоді, коли вони точно відповідають зазначеному правилу.
...
- **spec** (ValidatingAdmissionPolicySpec)
  ...
  spec — визначення бажаної поведінки ValidatingAdmissionPolicy.
  ...
  *ValidatingAdmissionPolicySpec — це специфікація бажаної поведінки AdmissionPolicy.*
  ...
  - **spec.failurePolicy** (string)
    ...
    Можливі значення enum:
    - `"Fail"` означає, що помилка виклику вебхука призводить до відхилення запиту.
...
- **status** (ValidatingAdmissionPolicyStatus)
  ...
  status представляє поточний стан ValidatingAdmissionPolicy, включаючи попередження, які корисні для визначення, чи працює політика відповідно до очікувань. Заповнюється системою. Тільки для читання.
  ...
  *ValidatingAdmissionPolicyStatus представляє статус політики перевірки допуску.*
  ...
  - **status.observedGeneration** (int64)
    ...
    observedGeneration — покоління, яке спостерігалося контролером.

  - **status.typeChecking** (TypeChecking)

    typeChecking містить результати перевірки типу для кожного виразу. Наявність цього поля вказує на завершення перевірки типу.
    ...
    - **status.typeChecking.expressionWarnings** ([]ExpressionWarning)
      ...
      *ExpressionWarning — це інформація про попередження, що стосується конкретного виразу.*
      ...
      - **status.typeChecking.expressionWarnings.fieldRef** (string), обовʼязково
        ...
        fieldRef містить шлях до поля, що посилається на вираз. Наприклад, посилання на вираз першого елемента перевірок є "spec.validations[0].expression".

      - **status.typeChecking.expressionWarnings.warning** (string), обовʼязково

        warning містить вміст інформації про попередження перевірки типу у формі, зручною для читання людиною. Кожен рядок попередження містить тип, за яким перевірено вираз, а потім помилку перевірки типу від компілятора.
