...
title: "MutatingAdmissionPolicyBinding v1"
weight: 10
auto_generated: true
---
...
- **spec** (MutatingAdmissionPolicyBindingSpec)
  ...
  spec — визначення бажаної поведінки привʼязки MutatingAdmissionPolicyBinding.
  ...
  - **spec.matchResources** (MatchResources)
    ...
    *MatchResources вирішує, чи застосовувати політику контролю доступу до обʼєкта на основі того, чи відповідає він критеріям відповідності. Правила виключення мають пріоритет над правилами включення (якщо ресурс відповідає обом критеріям, він виключається)*
      ...
      *NamedRuleWithOperations — це кортеж операцій та ресурсів з іменами ресурсів.*
      ...
      - **spec.matchResources.excludeResourceRules.operations** ([]string)
        ...
        *Atomic: буде замінено під час злиття*
        ...
        operations - це операції, які цікавлять хук допуску — CREATE, UPDATE, DELETE, CONNECT або \* для всіх цих операцій і будь-яких майбутніх операцій допуску, які будуть додані. Якщо '\*' присутній, довжина фрагмента повинна бути одиницею. Обовʼязковий параметр.
    ...
    - **spec.matchResources.matchPolicy** (string)
      ...
      - Equivalent: збіг запиту, якщо він змінює ресурс, перелічений у правилах, навіть через іншу групу або версію API. Наприклад, якщо розгортання можуть бути змінені через apps/v1, apps/v1beta1 і extensions/v1beta1, а "rules" включають тільки `apiGroups:["apps"], apiVersions:["v1"], resources: ["deployments"]`, запит до apps/v1beta1 або extensions/v1beta1 буде конвертовано в apps/v1 і надіслано до ValidatingAdmissionPolicy.

      Стандартне значення "Equivalent"

      Можливі значення enum:
      - `"Equivalent"` означає, що запити повинні надсилатися до вебхука, якщо вони змінюють ресурс, перелічений у правилах, через іншу групу або версію API.
      - `"Exact"` означає, що запити повинні надсилатися до вебхука лише у тому випадку, якщо вони точно відповідають вказаному правилу.
    ...
    - **spec.matchResources.objectSelector** (<a href="{{< ref "../common-definitions/label-selector#LabelSelector" >}}">LabelSelector</a>)
      ...
      objectSelector вирішує, чи запускати перевірку на основі того, чи мають обʼєкти відповідні мітки. objectSelector порівнюється з oldObject і newObject, які будуть надіслані до cel перевірки, і вважається, що вони збігаються, якщо будь-який з обʼєктів збігається з селектором. Нульовий обʼєкт (oldObject у випадку створення або newObject у випадку видалення) або обʼєкт, який не може мати міток (наприклад, обʼєкт DeploymentRollback або обʼєкт PodProxyOptions), не вважається таким, що збігається з селектором. Використовуйте селектор обʼєктів, тільки якщо вебхук є активним, оскільки кінцеві користувачі можуть пропустити вебхук допуску, встановивши мітки. Стандартно використовується порожній LabelSelector, який відповідає всьому.
  ...
  - **spec.paramRef** (ParamRef)
    ...
    *ParamRef описує, як знайти параметри, які будуть використані як вхідні дані для виразів правил, що застосовуються привʼязкою політики.*
    ...
    - **spec.paramRef.name** (string)
      ...
      Одне з `name` або `selector` має бути встановлено, але `name` і `selector` є взаємовиключними властивостями. Якщо одне з них встановлено, інше має бути пустим.

      Один параметр, який використовується для всіх запитів на допуск, можна налаштувати, встановивши поле `name`, залишивши `selector` порожнім і встановивши простір імен, якщо `paramKind` обмежений простором імен.
    ...
    - **spec.paramRef.parameterNotFoundAction** (string)
      ...
      Допустимими значеннями є `Allow` або `Deny`.
...
- **spec** (MutatingAdmissionPolicySpec)
  ...
  *MutatingAdmissionPolicySpec — це специфікація бажаної поведінки політики допуску.*
  ...
  - **spec.matchConditions** ([]MatchCondition)
    ...
    *MatchCondition представляє умову, яка повинна бути виконана, щоб запит був надісланий до вебхуку.*
  ...
  - **spec.matchConstraints** (MatchResources)
    ...
    *MatchResources вирішує, чи застосовувати політику контролю допуску до обʼєкта на основі того, чи відповідає він критеріям відповідності. Правила виключення мають пріоритет над правилами включення (якщо ресурс відповідає обом критеріям, він виключається)*
    ...
    - **spec.matchConstraints.matchPolicy** (string)
      ...
      - Exact: збіг запиту лише у тому випадку, якщо він точно відповідає вказаному правилу. Наприклад, якщо розгортання можуть бути змінені через apps/v1, apps/v1beta1 і extensions/v1beta1, але "rules" включають лише `apiGroups:["apps"], apiVersions:["v1"], resources: ["deployments"]`, запити до apps/v1beta1 або extensions/v1beta1 не будуть надсилатися до ValidatingAdmissionPolicy.

      - Equivalent: збіг запиту, якщо він змінює ресурс, перелічений у правилах, навіть через іншу групу або версію API. Наприклад, якщо розгортання можуть бути змінені через apps/v1, apps/v1beta1 і extensions/v1beta1, а "rules" включають тільки `apiGroups:["apps"], apiVersions:["v1"], resources: ["deployments"]`, запити до apps/v1beta1 або extensions/v1beta1 будуть конвертовані в apps/v1 і надіслані до ValidatingAdmissionPolicy.
  ...
  - **spec.mutations** ([]Mutation)
    ...
    *Mutation задає CEL-вираз, який використовується для застосування зміни.*
    ...
    - **spec.mutations.patchType** (string), обовʼязково
      ...
      Можливі значення enum:
      - `"ApplyConfiguration"` означає, що мутація використовує конфігурацію застосування для зміни обʼєкта.
      - `"JSONPatch"` означає, що обʼєкт змінюється за допомогою JSON Patch.
  ...
  - **spec.reinvocationPolicy** (string)
    ...
    Можливі значення enum:
    - `"IfNeeded"` означає, що мутація може бути викликана принаймні ще один раз у рамках оцінки допуску, якщо обʼєкт, що допускається, змінюється іншими втулками допуску після початкового виклику мутації.
    - `"Never"` означає, що мутація не повинна викликатися більше одного разу в одній оцінці допуску.
...
- **items** ([]<a href="{{< ref "../policy-resources/mutating-admission-policy-binding-v1#MutatingAdmissionPolicyBinding" >}}">MutatingAdmissionPolicyBinding</a>), обовʼязково
...
- **body**: <a href="{{< ref "../policy-resources/mutating-admission-policy-binding-v1#MutatingAdmissionPolicyBinding" >}}">MutatingAdmissionPolicyBinding</a>, обовʼязково
