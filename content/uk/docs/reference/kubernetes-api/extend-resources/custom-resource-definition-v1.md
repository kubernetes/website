...
- **x-kubernetes-validations.messageExpression** (string)
  ...
  - **x-kubernetes-validations.reason** (string)
    ...
    - `"FieldValueDuplicate"` використовується для повідомлення про конфлікти значень, які повинні бути унікальними (наприклад, унікальні ідентифікатори).
    - `"FieldValueForbidden"` використовується для повідомлення про дійсні (згідно з правилами форматування) значення, які були б прийняті за певних умов, але не дозволені за поточних умов (наприклад, політика безпеки).
    - `"FieldValueInvalid"` використовується для повідомлення про неправильні значення (наприклад, невдале співпадіння з регулярним виразом, занадто довге, поза межами).
    - `"FieldValueRequired"` використовується для повідомлення про обовʼязкові значення, які не надані (наприклад, порожні рядки, null значення або порожні масиви).
