  ...
  - **selectors.cel** (CELDeviceSelector)
    ...
    *CELDeviceSelector містить вираз CEL для вибору пристрою.*
    ...
    - **selectors.cel.expression** (string), обовʼязково
      ...
      Коли функціональна можливість DRAListTypeAttributes увімкнена, доступна допоміжна функція includes(), яка може працювати як для скалярних, так і для спискових атрибутів. Вона була введена для підтримки плавної міграції від скалярних атрибутів до спискових, зберігаючи простоту виразів CEL. Наприклад:
