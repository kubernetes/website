...
shardSelector обмежує список обʼєктів, що повертаються, за допомогою виразу селектора шардінгу на основі CEL. Формат використовує функцію shardRange() у поєднанні з || (логічне АБО), щоб вказати один або кілька діапазонів хешів:
...
Шляхи полів використовують синтаксис CEL, що починається з обʼєкта (наприклад, "object.metadata.uid"), а не формат fieldSelector ("metadata.uid"). Підтримувані шляхи:

- object.metadata.uid
- object.metadata.namespace

hexStart та hexEnd є рядковими літералами CEL у одинарних лапках з префіксом '0x', що визначають включну нижню та виключну верхню межі в 64-бітному просторі хешів FNV-1a. Повний діапазон становить [0x0, 0x10000000000000000), де виключна верхня межа дорівнює 2^64.
...
Це поле є альфа-рівнем і вимагає увімкнення функціональної можливості ShardedListAndWatch.
