...
- **shardInfo** (ShardInfo)
  ...
  Параметр `shardInfo` встановлюється, коли список є відфільтрованою підмножиною повної колекції, відібраною за допомогою селектора шардів у запиті. Він повертає значення селектора, щоб клієнти могли перевірити, який шард вони отримали, та об’єднати відповіді, розділені за шардами. Клієнтам не слід кешувати відповіді зі списками, розділеними за шардами, як повне відображення колекції.

  Це альфа-поле і вимагає увімкнення функціонального прапора ShardedListAndWatch.

  <a name="ShardInfo"></a>
  *ShardInfo описує селектор шарда, який був застосований для формування відповіді списку. Його наявність у відповіді списку вказує на те, що список є відфільтрованою підмножиною.*

  - **shardInfo.selector** (string), обов'язково

    selector — це рядок селектора шарда з запиту, який повертається назад, щоб клієнти могли перевірити, який шард вони отримали, та об’єднати відповіді з кількох шардів.
