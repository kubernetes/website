...
- **nodeAllocatableUpdatePeriodSeconds** (int64)
  ...
  Ця функція та вимагає увімкнення функціональної можливості MutableCSINodeAllocatableCount.
...
- **preventPodSchedulingIfMissing** (boolean)
  ...
  preventPodSchedulingIfMissing вказує, що драйвер CSI хоче запобігти плануванню Podʼа, якщо драйвер CSI на вузлі відсутній.

  Увімкнення цієї опції запобігатиме плануванню Podʼів на вузли, де драйвер CSI не встановлений.

  Для компонентів (таких як cluster-autoscaler), які вбудовують планувальник і виконують симуляції розміщення Podʼів за допомогою плагінів планувальника, вони МАЮТЬ бути обізнані про інформацію про реєстрацію драйвера CSI через об'єкт CSINode. Вони повинні створювати симульовані об'єкти CSINode на додаток до об'єктів Node під час симуляції планування, інакше, якщо PreventPodSchedulingIfMissing увімкнено глобально для об'єкта CSIDriver, будь-який новостворений вузол може бути відхилений планувальником через відсутність інформації про драйвер CSI на вузлі.

  Це альфа-функція і вимагає увімкнення функціональної можливості VolumeLimitScaling. Стандартне значення — "false".
