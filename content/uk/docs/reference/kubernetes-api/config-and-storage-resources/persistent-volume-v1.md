...
- **persistentVolumeReclaimPolicy** (string)
  ...
  - `"Delete"` означає, що том буде видалено з Kubernetes після звільнення від його заявки. Втулок тому повинен підтримувати видалення.
  - `"Recycle"` означає, що том буде перероблено назад у пул невикористаних постійних томів після звільнення від його заявки. Втулок тому повинен підтримувати Recycling.
  - `"Retain"` означає, що том залишиться у своєму поточному стані (Released) для ручного відновлення адміністратором. Політика за замовчуванням - Retain.
...
- **volumeMode** (string)
  ...
  Можливі значення enum:
  - `"Block"` означає, що том не буде форматовано файловою системою і залишиться у вигляді необробленого блочного пристрою.
  - `"Filesystem"` означає, що том буде або вже форматовано файловою системою.
...
- **hostPath** (HostPathVolumeSource)
  ...
  *Представляє шлях на хості, зіставлений зі шляхом у Podʼі. Шляхи томів хосту не підтримують управління власністю або перепризначення міток SELinux.*
  ...
  - **hostPath.type** (string)
    ...
    - `"BlockDevice"` Повинен існувати блочний пристрій за вказаним шляхом
    - `"CharDevice"` Повинен існувати символьний пристрій за вказаним шляхом
    - `"Directory"` Повинна існувати тека за вказаним шляхом
    - `"DirectoryOrCreate"` Якщо нічого не існує за вказаним шляхом, буде створено порожню теку за потреби з доступом 0755, з тією ж групою та власником, що й у Kubelet.
    - `"File"` Повинен існувати файл за вказаним шляхом
    - `"FileOrCreate"` Якщо нічого не існує за вказаним шляхом, буде створено порожній файл за потреби з доступом 0644, з тією ж групою та власником, що й у Kubelet.
    - `"Socket"` Повинен існувати UNIX-сокет за вказаним шляхом
...
- **portworxVolume** (PortworxVolumeSource)
  ...
  portworxVolume представляє ресурс тома Portworx, приєднаний і змонтований на хост-машині kubelet. Застаріло: PortworxVolume застарів. Всі операції для внутрішнього типу portworxVolume перенаправляються на CSI-драйвер pxd.portworx.com.
