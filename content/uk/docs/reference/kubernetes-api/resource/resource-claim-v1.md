...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>spec</code><br/><em><a href="{{< ref "#ResourceClaimSpec" >}}">ResourceClaimSpec</a></em></td>
...
## DeviceDerivedAttribute {#DeviceDerivedAttribute}

DeviceDerivedAttribute визначає похідний атрибут, обчислюваний за допомогою CEL.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Expression є виразом CEL, який обчислюється для кожного пристрою-кандидата. Вираз повинен обчислюватися до примітивного скалярного значення (string, integer, boolean або semver) або списку таких скалярних значень ([]string, []int64, []bool, []semver), щоб діяти як віртуальний груповий ключ. Будь-який інший тип результату є помилкою і призводить до збою обчислення CEL для пристрою.</td>
    </tr>
    <tr>
      <td><code>name</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Name є ідентифікатором цього похідного атрибута, який використовується в обмеженнях. Він повинен бути піддоменом DNS, за яким слідує скісна риска ("/"), а потім — C-ідентифікатор (наприклад, "example.com/numaNode" або "derived/numaNode"). Якщо обрана назва збігається з наявним фізичним атрибутом від драйвера, вираз похідного атрибута затінить фізичний атрибут, і його обчислене значення буде використовуватися в обмеженнях замість нього. Якщо метою є визначення похідного атрибута, який використовується лише в межах ResourceClaim і не призначений для затінення наявного атрибута, використовуйте доменний префікс, який не повинен використовувати жоден драйвер DRA (наприклад, "derived/myAttribute"). Неприпустимо визначати похідний атрибут, який не використовується принаймні в одному обмеженні.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>device</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Device посилається на один екземпляр пристрою за його назвою в пулі ресурсів драйвера. Має бути міткою DNS.</td>
    </tr>
    <tr>
      <td><code>driver</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
    ...
    <tr>
      ...
      <td>SkipNodeOperations перелічує локальні для вузла операції з ресурсами (gRPC-виклики), які будуть пропущені для цього виділеного пристрою під час визначення того, чи потрібні операції на вузлі. Якщо всі виділені пристрої для драйвера в заявці пропускають операцію, цей gRPC-виклик буде пропущений. Це копія значення ResourceSlice.spec.skipNodeOperations на момент виділення пристрою.</td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>derivedAttributes</code><br/><em><a href="{{< ref "#DeviceDerivedAttribute" >}}">DeviceDerivedAttribute array</a></em></td>
      <td><p>DerivedAttributes визначає набір віртуальних атрибутів, обчислюваних за допомогою виразів CEL для кожного пристрою-кандидата. На ці віртуальні атрибути можна посилатися в <code>.devices.constraints</code>, щоб вирівнювати та зіставляти різні пристрої (наприклад, спільно виділяти GPU та NIC на тому самому NUMA-вузлі), навіть якщо їхні драйвери публікують різні атрибути. Похідні атрибути недоступні через <code>device.attributes</code> у середовищі CEL під час обчислення виразів селекторів.</p>
      <p>Похідні атрибути дозволяють витягувати, перетворювати або нормалізувати інформацію про топологію (наприклад, витягувати індекс NUMA зі складного топологічного рядка або перейменовувати атрибут, специфічний для постачальника) у загальне віртуальне імʼя атрибута під час планування. Планувальник потім обчислює ці віртуальні атрибути точно так само, як статичні атрибути, під час зіставлення обмежень.</p>
      <p>Кожен похідний атрибут, визначений у цьому списку, повинен бути згаданий принаймні одним обмеженням MatchAttribute або DistinctAttribute у списку <code>.devices.constraints</code>.</p>
      <p>Максимальна кількість похідних атрибутів — 32.</p>
      <p>Це альфа-поле і вимагає увімкнення функціональної можливості DRADerivedAttributes.</p></td>
    </tr>
    <tr>
