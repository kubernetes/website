...
description: "ResourcePoolStatusRequest запускає одноразовий розрахунок стану пулу ресурсів на основі наданих фільтрів. Після встановлення стану запит вважається завершеним і не буде повторно оброблятися. Користувачі повинні видаляти та створювати запити заново, щоб отримати оновлену інформацію."
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Spec визначає фільтри для включення пулів у статус. Spec є незмінним після створення.</td>
    </tr>
    <tr>
      <td><code>status</code><br/><em><a href="{{< ref "#ResourcePoolStatusRequestStatus" >}}">ResourcePoolStatusRequestStatus</a></em></td>
      <td>Status заповнюється контролером з обчисленим станом пулу. Коли status не є nil, запит вважається завершеним, і весь обʼєкт стає незмінним.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>DefaultPartitionTypeAttribute за бажанням називає атрибут пристрою (за його повним імʼям, напр., "gpu.example.com/profile") як стандартний атрибут групування для розділюваних пристроїв, чий зріз не оголосив власний.  Власний PartitionTypeAttribute зрізу завжди має перевагу. Цей стандарт застосовується лише до пристроїв, чий зріз не оголошує його, тому запит все ще може отримати точний partitionSummary від драйвера, який не було оновлено для його оголошення. Коли ні зріз, ні цей стандарт не називають атрибут, розділюваний пул не повідомляє partitionSummary.  Має містити кваліфікатор домену.</td>
    </tr>
    <tr>
      <td><code>driver</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Driver визначає назву драйвера DRA для фільтрації пулів. Включаються лише пули з ResourceSlices з цим драйвером. Має бути піддоменом DNS (наприклад, "gpu.example.com").</td>
    </tr>
    <tr>
      <td><code>limit</code><br/><em>integer</em></td>
      <td><p>Limit необовʼязково визначає максимальну кількість пулів для повернення у статусі. Якщо більше пулів відповідають критеріям фільтрації, відповідь буде скорочена (тобто len(status.pools) &lt; status.poolCount).</p>
      <p>Стандартно: 100 Мінімум: 1 Максимум: 1000</p></td>
    </tr>
    <tr>
      <td><code>poolName</code><br/><em>string</em></td>
      <td>PoolName необовʼязково фільтрує за конкретною назвою пулу. Якщо не вказано, включаються всі пули з зазначеного драйвера. Коли вказано, повинно бути непустим дійсним імʼям пулу ресурсів (піддомени DNS, розділені "/").</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><p>Conditions надають інформацію про стан запиту. Стан з type=Complete або type=Failed завжди буде встановлено, коли статус заповнено.</p>
      <p>Відомі типи умов:
      <ul>
        <li>"Complete": True, коли запит було успішно оброблено</li>
        <li>"Failed": True, коли запит не вдалося обробити</li>
      </ul></p></td>
    </tr>
    <tr>
      <td><code>poolCount</code>&nbsp;<strong>*</strong><br/><em>integer</em></td>
      <td>PoolCount є загальною кількістю пулів, які відповідають критеріям фільтрації, незалежно від усічення. Це допомагає користувачам зрозуміти, скільки пулів існує, навіть коли відповідь усічена. Значення 0 означає, що жоден пул не відповідав критеріям фільтрації.</td>
    </tr>
    <tr>
      <td><code>pools</code><br/><em><a href="{{< ref "#PoolStatus" >}}">PoolStatus array</a></em></td>
      <td>Pools містить перші <code>spec.limit</code> відповідних пулів, відсортованих за драйвером, а потім за назвою пулу. Якщо <code>len(pools) &lt; poolCount</code>, список був усічений. Коли пропущено, жоден пул не відповідав фільтрам запиту.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Items є списком ResourcePoolStatusRequests.</td>
    </tr>
    <tr>
      <td><code>kind</code><br/><em>string</em></td>
      <td>Kind визначає тип REST-ресурсу, який представляє цей обʼєкт. Сервери можуть визначати це з точки доступу, до якої клієнт надсилає запити. Не може бути оновлено. У CamelCase. Детальніше: <a href="https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds">https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds</a></td>
...
PartitionTypeStatus повідомляє можливість виділення для одного типу розділу, визначеного значенням атрибута групування.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Allocatable — це кількість додаткових пристроїв цього типу розділу, які ще можна виділити з урахуванням поточного споживання спільних лічильників.</td>
    </tr>
    <tr>
      <td><code>attribute</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Attribute — це повне імʼя атрибута пристрою, значення якого групує цей запис. Це PartitionTypeAttribute, оголошений власним зрізом пристроїв, або стандарт, вказаний у запиті, коли їхній зріз не оголошує жодного.</td>
    </tr>
    <tr>
      <td><code>total</code>&nbsp;<strong>*</strong><br/><em>integer</em></td>
      <td>Total — це кількість пристроїв цього типу розділу в пулі.</td>
    </tr>
    <tr>
      <td><code>type</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Type — це значення типу розділу (напр., "Full" або "Half").</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>AllocatedDevices — це кількість пристроїв, які наразі виділені для запитів. Значення 0 означає, що жоден пристрій не виділено. Може бути не встановлено, коли встановлено validationError.</td>
    </tr>
    <tr>
      <td><code>availableDevices</code><br/><em>integer</em></td>
      <td>AvailableDevices — це кількість пристроїв, доступних для виділення. Це дорівнює TotalDevices - AllocatedDevices - UnavailableDevices. Значення 0 означає, що наразі жоден пристрій не доступний. Може бути не встановлено, коли встановлено validationError.</td>
    </tr>
    <tr>
      <td><code>driver</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Driver є імʼям драйвера DRA для цього пулу. Має бути піддоменом DNS (наприклад, "gpu.example.com").</td>
    </tr>
    <tr>
      <td><code>generation</code>&nbsp;<strong>*</strong><br/><em>integer</em></td>
      <td>Generation є поколінням пулу, спостережуваним у всіх ResourceSlices цього пулу. Звіт продається лише для останнього покоління. Під час розгортання покоління, якщо не всі зрізи останнього покоління були опубліковані, пул включається з validationError, а поля кількості пристроїв не встановлюються.</td>
    </tr>
    <tr>
      <td><code>nodeName</code><br/><em>string</em></td>
      <td>NodeName є вузлом, з яким повʼязаний цей пул. Якщо не вказано, пул не повʼязаний з конкретним вузлом. Має бути дійсним піддоменом DNS (RFC1123).</td>
    </tr>
    <tr>
      <td><code>partitionSummary</code><br/><em><a href="{{< ref "#PartitionTypeStatus" >}}">PartitionTypeStatus array</a></em></td>
      <td>PartitionSummary повідомляє можливість виділення на (атрибут, тип розділу) для розділюваного пулу, який публікує SharedCounters. Кожен запис називає атрибут групування, з якого його було отримано: PartitionTypeAttribute, оголошений власним зрізом пристрою, або для пристроїв, чий зріз не оголошує його, стандарт, вказаний у запиті. Пул, який змішує розділи, оголошені під різними атрибутами, повідомляє кожен незалежно. Коли жоден зріз не оголошує атрибут і запит не називає стандарт, пул не повідомляє зведення розділів.</td>
    </tr>
    <tr>
      <td><code>poolName</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>PoolName є імʼям пулу. Має бути дійсним імʼям пулу ресурсів (піддомени DNS, розділені "/").</td>
    </tr>
    <tr>
      <td><code>resourceSliceCount</code><br/><em>integer</em></td>
      <td>ResourceSliceCount є кількістю ResourceSlices, що складають цей пул. Може бути не встановлено, коли встановлено validationError.</td>
    </tr>
    <tr>
      <td><code>shareableSummary</code><br/><em><a href="{{< ref "#ShareableSummaryStatus" >}}">ShareableSummaryStatus</a></em></td>
      <td>ShareableSummary повідомляє сукупну ємність для пулу, який містить пристрої з AllowMultipleAllocations. Заповнюється лише тоді, коли принаймні один пристрій у пулі є спільним.</td>
    </tr>
    <tr>
      <td><code>totalDevices</code><br/><em>integer</em></td>
      <td>TotalDevices є загальною кількістю пристроїв у пулі по всіх зрізах. Значення 0 означає, що пул не має пристроїв. Може бути не встановлено, коли встановлено validationError.</td>
    </tr>
    <tr>
      <td><code>unavailableDevices</code><br/><em>integer</em></td>
      <td>UnavailableDevices є кількістю пристроїв, які недоступні через taints або інші умови, але не виділені. Значення 0 означає, що всі невиділені пристрої доступні. Може бути не встановлено, коли встановлено validationError.</td>
    </tr>
    <tr>
      <td><code>validationError</code><br/><em>string</em></td>
      <td>ValidationError встановлюється, коли дані пулу не можуть бути повністю перевірені (наприклад, неповна публікація зрізів). Коли встановлено, поля кількості пристроїв і ResourceSliceCount можуть бути не встановлені.</td>
    </tr>
  </tbody>
</table>
...
ShareableCapacityStatus повідомляє сукупні значення для одного ключа спільної ємності.
...
ShareableSummaryStatus повідомляє сукупну ємність для пулу, який містить пристрої з AllowMultipleAllocations.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Capacity повідомляє сукупні total, consumed та available значення для кожного ключа спільної ємності по всьому пулу.</td>
    </tr>
    <tr>
      <td><code>fullyAvailableDevices</code>&nbsp;<strong>*</strong><br/><em>integer</em></td>
      <td>FullyAvailableDevices — це кількість спільних пристроїв без жодного спожитого обсягу ємності.</td>
    </tr>
    <tr>
      <td><code>partiallyAvailableDevices</code>&nbsp;<strong>*</strong><br/><em>integer</em></td>
      <td>PartiallyAvailableDevices — це кількість спільних пристроїв із частково, але не повністю, спожитою ємністю.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><em>string</em></td>
      <td>Назва ResourcePoolStatusRequest</td>
    </tr>
  </tbody>
</table>
