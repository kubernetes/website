...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>PartitionTypeAttribute називає атрибут пристрою строкового типу (за повним імʼям, наприклад "gpu.example.com/profile"), значення якого позначає кожен пристрій його типом розділу, наприклад "Full" або "Half" для GPU у стилі MIG. Коли встановлено, кожен пристрій, який можна розділити у цьому slice, повинен нести цей атрибут, а пристрої, що поділяють однакове значення, повинні поділяти однакову вартість ConsumesCounters.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>SkipNodeOperations перелічує локальні для вузла операції над ресурсами (виклики gRPC), які будуть пропущені для пристроїв у цьому slice під час визначення, чи потрібні операції на вузлі. Якщо всі виділені пристрої для драйвера у заявці пропускають операцію, цей виклик gRPC буде пропущено. Дійсні значення:
      <ul>
        <li>"NodePrepareResources": виклики gRPC NodePrepareResources пропускаються. Це значення не можна вказати, якщо "NodeUnprepareResources" також не вказано (або не вказано "*").</li>
        <li>"NodeUnprepareResources": виклики gRPC NodeUnprepareResources пропускаються.</li>
        <li>"*": всі локальні для вузла операції над ресурсами пропускаються.</li>
      </ul>
      Інші значення можуть бути додані в майбутньому. Kubelet повинен ігнорувати невідомі значення.</td>
    </tr>
  </tbody>
</table>
...
ResourceSliceList — це колекція обʼєктів ResourceSlice.
...
Якщо функційну можливість DRAFractionalCapacityRange увімкнено і принаймні одне з Min, Max або Step є дробовою величиною (тобто її значення не є цілим числом), замість цього використовується арифметика з мілі-одиницями, що підтримує значення з точністю до 3 знаків після коми (наприклад, 100m = 0.1). Найбільше підтримуване значення тоді в 1000 разів менше порівняно з використанням 64-бітових цілих чисел. В іншому випадку всі порівняння використовують 64-бітову цілочисельну арифметику через resource.Quantity.Value().
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>CompatibilityGroups — це список непрозорих назв груп для цього споживання набору лічильників. Пристрої, які споживають лічильники з одного набору лічильників, можуть бути виділені одночасно («спільно виділені»), лише якщо всі вони поділяють принаймні одну спільну групу: перетин CompatibilityGroups усіх спільно виділених пристроїв у цьому наборі лічильників повинен бути непорожнім. Пристрої, які споживають із різних наборів лічильників, ніколи не порівнюються через це поле. Невстановлене поле, явний nil та порожній список еквівалентні і означають «без груп»: такий пристрій може бути спільно виділений лише з сусідніми пристроями в тому самому наборі лічильників, які також не мають груп, і ніколи не може бути спільно виділений з пристроєм, який оголошує одну або більше груп. Назви груп є непрозорими та значущими лише в межах пулу драйвера, що публікує. Максимальна кількість груп становить 2, а назви мають бути унікальними.</td>
    </tr>
    <tr>
...
NodeAllocatableMapping визначає, як розподіл DRA безпосередньо перетворюється на кількість доступного для виділення ресурсу вузла. Зіставлення може бути отримано або з кількості виділених пристроїв (через deviceMultiplier), або з конкретно спожитої ємності (через capacityKey та capacityMultiplier). Ці варіанти є взаємовиключними. Kubelet додає цю зіставлену кількість ресурсу із заявки і до запитів, і до лімітів у cgroup рівня пода, а до лімітів — у cgroup рівня контейнера для кожного контейнера, що посилається на заявку.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>CapacityKey посилається на назву ємності, визначену як ключ у мапі <code>spec.devices[*].capacity</code>. Коли це поле встановлено, значення, повʼязане з цим ключем у мапі <code>status.allocation.devices.results[*].consumedCapacity</code> (для конкретного розподілу заявки), визначає базову кількість для доступного для виділення ресурсу вузла. <code>capacityMultiplier</code> також має бути встановлений і множиться на базову кількість. Наприклад, якщо <code>spec.devices[*].capacity</code> має запис "dra.example.com/memory": "128Gi", і це поле встановлено на "dra.example.com/memory", тоді для розподілу заявки, яке споживає { "dra.example.com/memory": "4Gi" }, базова кількість для зіставлення доступного для виділення ресурсу вузла буде "4Gi". Остаточна кількість доступного для виділення ресурсу вузла становить <code>consumedCapacity[capacityKey]</code> * <code>capacityMultiplier</code>.</td>
    </tr>
    <tr>
      <td><code>capacityMultiplier</code><br/><em><a href="{{< ref "../definitions/quantity-resource#Quantity" >}}">Quantity</a></em></td>
      <td>CapacityMultiplier використовується як множник для спожитої виділеної ємності. Він дійсний лише якщо встановлено <code>capacityKey</code>. Остаточна кількість доступного для виділення ресурсу вузла становить <code>consumedCapacity[capacityKey]</code> * <code>capacityMultiplier</code>. Наприклад, якщо споживається ємність пристрою "dra.example.com/cores", і кожне "ядро" забезпечує 2 "cpu", зіставлення буде: {ResourceName: "cpu", capacityKey: "dra.example.com/cores", capacityMultiplier: "2"}. Якщо заявка споживає 8 "dra.example.com/cores", відбиток CPU становить 8 * 2 = 16.</td>
    </tr>
    <tr>
      <td><code>deviceMultiplier</code><br/><em><a href="{{< ref "../definitions/quantity-resource#Quantity" >}}">Quantity</a></em></td>
      <td>DeviceMultiplier використовується як множник для кількості виділених пристроїв у заявці. Остаточна кількість доступного для виділення ресурсу вузла становить <code>deviceCount</code> * <code>deviceMultiplier</code>. Наприклад, DRA-драйвер, який представляє кожен кеш-комплекс (CCX) як пристрій, матиме {ResourceName: "cpu", deviceMultiplier: "8"} у своєму <code>nodeAllocatableResources</code>. Якщо 2 пристрої (CCX) виділено для заявки, 2 * 8 = 16 CPU вважатимуться виділеними. Він дійсний лише тоді, коли <code>capacityKey</code> та <code>capacityMultiplier</code> не встановлені.</td>
    </tr>
  </tbody>
</table>
...
NodeAllocatableOverhead визначає додаткові витрати на ресурси, що виникають під час виділення пристрою. Ці витрати можна вказати як фіксовану вартість на кожен pod, що посилається на заявку, як змінну вартість на кожне посилання на контейнер або як обидва варіанти. Kubelet враховує ці витрати, додаючи їх до cgroups як на рівні pod, так і на рівні контейнера для контейнерів, що посилаються на пристрій.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>PerContainer застосовується на кожне посилання контейнера на заявку. Це моделює накладні витрати, що масштабуються лінійно з кількістю контейнерів, які активно використовують пристрій. Коли вказані і PerPod, і PerContainer, загальні накладні витрати, виділені для кожного пода, що посилається на заявку, обчислюються як: Quantity = PerPod + (PerContainer * NumReferences) Kubelet враховує ці накладні витрати в cgroup:
      <ul>
        <li>cgroup рівня пода (запити та ліміти): Kubelet додає PerPod + (PerContainer * NumReferences).</li>
        <li>cgroup рівня контейнера (лише ліміти): Kubelet додає PerPod + PerContainer для кожного контейнера, що посилається на заявку. Це дозволяє будь-якому окремому контейнеру отримати доступ до накладних витрат рівня пода, тоді як батьківська cgroup обмежує загальне використання, щоб врахувати PerPod рівно один раз.</li>
      </ul></td>
    </tr>
    <tr>
      <td><code>perPod</code><br/><em><a href="{{< ref "../definitions/quantity-resource#Quantity" >}}">Quantity</a></em></td>
      <td>PerPod — це накладні витрати, що застосовуються один раз на под, який посилається на заявку на цьому вузлі. Це фіксовані накладні витрати, що виникають для кожного пода, який посилається на заявку.</td>
    </tr>
  </tbody>
</table>
...
NodeAllocatableResource визначає перетворення між запитаними одиницями пристрою/ємності DRA та відповідною кількістю доступного для виділення ресурсу вузла. Має бути вказано принаймні щось одне або Mapping або Overhead. Якщо не вказано жодного, це є недійсною конфігурацією.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Mapping використовується, коли пристрій безпосередньо моделює ресурс, що може бути виділений на вузлі, наприклад стандартний CPU або пам’ять (наприклад, за допомогою драйвера CPU DRA). Розрахована величина враховується саме один раз для кожного екземпляра запиту на вузлі. Щоб уникнути конфліктів ізоляції cgroup на вузлі, планувальник явно блокує спільне використання відображених запитів пристроїв між кількома подами.</td>
    </tr>
    <tr>
      <td><code>overhead</code><br/><em><a href="{{< ref "#NodeAllocatableOverhead" >}}">NodeAllocatableOverhead</a></em></td>
      <td>Overhead містить поля для моделювання допоміжних накладних витрат на доступні для виділення ресурси вузла, які виникають при виділенні пристроїв, що самі не моделюють доступний для виділення ресурс вузла (наприклад, накладні витрати памʼяті хоста для GPU). Спільне використання заявок із накладними витратами, позначеними через mapping, між кількома подами дозволене. Накладні витрати на доступний для виділення ресурс вузла враховуються окремо для кожного пода, що посилається на заявку. Накладні витрати завжди віднімаються від доступної для виділення ємності вузла для цього ресурсу, навіть коли mapping вказано для того самого ресурсу. Наприклад: якщо пристрій моделює ємність памʼяті на сокет як пул споживаної ємності через Mapping (з CapacityKey), будь-які накладні витрати, вказані для того самого ресурсу, будуть відняті від загальної доступної для виділення ємності вузла, а не від пулу ємності на сокет у Mapping.</td>
    </tr>
  </tbody>
</table>
