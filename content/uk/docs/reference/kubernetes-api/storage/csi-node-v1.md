...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><p>metadata — стандартні метадані обʼєкта. metadata.name повинен відповідати імені вузла Kubernetes.</p></td>
    </tr>
    <tr>
      <td><code>spec</code>&nbsp;<strong>*</strong><br/><em><a href="{{< ref "#CSINodeSpec" >}}">CSINodeSpec</a></em></td>
      <td>spec є специфікацією CSINode</td>
    </tr>
    <tr>
      <td><code>status</code><br/><em><a href="{{< ref "#CSINodeStatus" >}}">CSINodeStatus</a></em></td>
      <td>status містить інформацію про стан і роботу системи зберігання вузла.</td>
    </tr>
  </tbody>
</table>
...
CSINodeSpec містить інформацію про специфікацію всіх CSI драйверів, встановлених на вузлі.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>drivers є списком інформації про всі CSI драйвери, що існують на вузлі. Якщо всі драйвери в списку видалені, цей список може стати порожнім.</td>
    </tr>
  </tbody>
</table>
...
CSINodeStatus містить інформацію про стан і роботу системи зберігання на вузлі.
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>storageHealth</code><br/><em><a href="{{< ref "#StorageHealth" >}}">StorageHealth array</a></em><br/><em>patch strategy: злиття за ключем <code>name</code></em></td>
      <td>storageHealth містить звіти про стан роботи бекенду для драйверів CSI, зареєстрованих на вузлі.</td>
    </tr>
  </tbody>
</table>
...
CSINodeList є колекцією обʼєктів CSINode.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>items є списком обʼєктів CSINode</td>
    </tr>
    <tr>
      <td><code>kind</code><br/><em>string</em></td>
      <td>Kind визначає тип REST-ресурсу, який представляє цей обʼєкт. Сервери можуть визначати це з точки доступу, до якої клієнт надсилає запити. Не може бути оновлено. У CamelCase. Детальніше: <a href="https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds">https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds</a></td>
...
CSINodeDriver містить інформацію про специфікацію одного CSI драйвера, встановленого на вузлі.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>allocatable представляє ресурси томів вузла, доступні для планування. Це поле є бета-версією.</td>
    </tr>
    <tr>
      <td><code>name</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>name представляє імʼя CSI драйвера, до якого відноситься цей обʼєкт. Воно ПОВИННО збігатися з імʼям, повернутим викликом CSI GetPluginName() для цього драйвера.</td>
    </tr>
    <tr>
      <td><code>nodeID</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>nodeID вузла з точки зору драйвера. Це поле дозволяє Kubernetes взаємодіяти з системами зберігання, які не використовують однакову номенклатуру для вузлів. Наприклад, Kubernetes може посилатися на певний вузол як "node1", але система зберігання може посилатися на той самий вузол як "nodeA". Коли Kubernetes видає команду системі зберігання для приєднання тому до конкретного вузла, він може використовувати це поле для посилання на імʼя вузла, використовуючи ID, який система зберігання зрозуміє, наприклад "nodeA" замість "node1". Це поле є обовʼязковим.</td>
    </tr>
    <tr>
      <td><code>topologyKeys</code><br/><em>string array</em></td>
      <td>topologyKeys є списком ключів, підтримуваних драйвером. Коли драйвер ініціалізується в кластері, він надає набір ключів топології, які він розуміє (наприклад, "company.com/zone", "company.com/region"). Коли драйвер ініціалізується на вузлі, він надає ті ж ключі топології разом зі значеннями. Kubelet буде відображати ці ключі топології як мітки на своєму власному обʼєкті вузла. Коли Kubernetes виконує розподіл з урахуванням топології, він може використовувати цей список для визначення, які мітки слід отримати з обʼєкта вузла і передати назад драйверу. Можливо, що різні вузли використовують різні ключі топології. Це поле може бути порожнім, якщо драйвер не підтримує топологію.</td>
    </tr>
  </tbody>
</table>
...
StorageHealth містить звіт про стан роботи бекенду системи зберігання, наданий драйвером CSI на вузлі.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>healthConditions — це несприятливі стани бекенду системи зберігання, про які повідомив драйвер CSI. Може бути повідомлено щонайбільше про 16 станів.</td>
    </tr>
    <tr>
      <td><code>name</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>name — це назва драйвера CSI, яка відповідає CSINodeDriver.name.</td>
    </tr>
  </tbody>
</table>
...
StorageHealthCondition представляє несприятливий стан роботи, про який драйвер CSI повідомив для свого бекенду системи зберігання на вузлі.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>accessMode — це режим доступу, на який впливає стан. Nil означає, що впливає на всі режими доступу.<br/><br/>
      Можливі значення enum:
      <ul>
        <li><code>"ReadOnlyMany"</code> можна змонтувати в режимі лише для читання на багатьох хостах</li>
        <li><code>"ReadWriteMany"</code> можна змонтувати в режимі читання/запису на багатьох хостах</li>
        <li><code>"ReadWriteOnce"</code> можна змонтувати в режимі читання/запису рівно на 1 хост</li>
        <li><code>"ReadWriteOncePod"</code> можна змонтувати в режимі читання/запису рівно на 1 под, не можна використовувати в комбінації з іншими режимами доступу</li>
      </ul></td>
    </tr>
    <tr>
      <td><code>lastTransitionTime</code><br/><em><a href="{{< ref "../definitions/time-v1-meta#Time" >}}">Time</a></em></td>
      <td>lastTransitionTime — це час, коли цей стан вперше зʼявився у своєму поточному стані.</td>
    </tr>
    <tr>
      <td><code>message</code><br/><em>string</em></td>
      <td>message — це зрозумілий людині опис. Максимально допустима довжина повідомлення — 1024 символи.</td>
    </tr>
    <tr>
      <td><code>reason</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>reason — це коротка причина в стилі CamelCase, яку можна обробити машиною. Максимально допустима довжина причини — 256 символів.</td>
    </tr>
    <tr>
      <td><code>status</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>status — це категорія стану роботи. Одне з "StorageUnreachable", "StorageDegraded".<br/><br/>
      Можливі значення enum:
      <ul>
        <li><code>"StorageDegraded"</code> вказує, що бекенд системи зберігання працює зі зниженою функціональністю.</li>
        <li><code>"StorageUnreachable"</code> вказує, що бекенд системи зберігання недосяжний.</li>
      </ul></td>
    </tr>
    <tr>
      <td><code>volumeMode</code><br/><em>string</em></td>
      <td>volumeMode — це режим тому, на який впливає стан. Nil означає, що впливає на обидва режими.<br/><br/>
      Можливі значення enum:
      <ul>
        <li><code>"Block"</code> означає, що том не буде відформатовано у файлову систему і залишиться як необроблений блочний пристрій.</li>
        <li><code>"Filesystem"</code> означає, що том буде або вже відформатовано у файлову систему.</li>
      </ul></td>
    </tr>
  </tbody>
</table>
