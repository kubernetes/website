...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>evictionResponders</code><br/><em><a href="{{< ref "#EvictionResponder" >}}">EvictionResponder array</a></em><br/><em>patch strategy: злиття за ключем <code>name</code></em></td>
      <td><p>evictionResponders посилаються на відповідачів, які реагують на виселення на основі EvictionRequests. Відповідачі повинні відстежувати стан та обмінюватися даними через API ресурсу Eviction, щоб сприяти коректному завершенню роботи пода. Відповідачі обираються послідовно відповідно до вказаного пріоритету.</p>
      <p>Відповідачі повинні періодично звітувати про хід виселення, оновлюючи поле .status.responders[].heartbeatTime об’єкта Eviction. Якщо це поле не оновлюється протягом терміну, визначеного API Eviction (наразі 20 хвилин), виселення передається наступному відповідачу з нижчим пріоритетом. Якщо інших відповідачів немає, останній відповідач за визначенням imperative-eviction.k8s.io/evictor із пріоритетом 100 витіснить под за допомогою імперативного API Eviction (підресурс pods/&lt;name&gt;/eviction).</p>
      <p>Максимальна довжина списку респондерів становить 10. Респондери не підтримуються, якщо под є частиною PodGroup (встановлено .spec.schedulingGroup). Це поле можна встановити лише під час створення, після чого воно стає незмінним.</p></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <ul>
        ...
        <li><code>hostNetwork</code> має бути встановлено на false. Це поле має бути дійсним субдоменом DNS, як визначено в RFC 1123, і містити не більше 64 символів.</li>
      </ul></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>PreemptionPolicy є Політикою витіснення подів з нижчим пріоритетом. Одне з: Never, PreemptLowerPriority. Коли Priority Admission Controller увімкнено, він не дозволяє користувачам встановлювати це поле. Контролер допуску заповнює це поле з PriorityClassName. Стандартно встановлено PreemptLowerPriority, якщо не вказано.
      <br/><br/>
      Можливі значення enum:
      <ul>
        <li><code>"Never"</code> означає, що под ніколи не витісняє інші поди з нижчим пріоритетом.</li>
        <li><code>"PreemptLowerPriority"</code> означає, що под може витісняти інші поди з нижчим пріоритетом.</li>
      </ul>
    </td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>nodeAllocatableResourceClaimStatuses</code><br/><em><a href="{{< ref "#NodeAllocatableResourceClaimStatus" >}}">NodeAllocatableResourceClaimStatus array</a></em><br/><em>patch strategy: злиття за ключем <code>resourceClaimName</code></em></td>
    ...
    <tr>
      ...
      <td>nominatedNodeName встановлюється лише тоді, коли цей под передує іншим подам на вузлі, але його не можна запланувати негайно, оскільки жертви витіснення отримують свої періоди належного завершення. Це поле не гарантує, що под буде заплановано на цьому вузлі. Планувальник може вирішити розмістити под в іншому місці, якщо інші вузли стануть доступними раніше. Планувальник також може вирішити надати ресурси на цьому вузлі поду з вищим пріоритетом, який створено після витіснення. В результаті це поле може відрізнятися від PodSpec.nodeName, коли под заплановано.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>volumeHealth містить інформацію про стан кожного тому, який використовує под, надану вузлом. Заповнюється kubelet на вузлі поду.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>user є опціональним: UID власника створеного файлу. Якщо вказано, поле user на рівні елемента має пріоритет над defaultUser. (Alpha) Це поле вимагає увімкнення функціональної можливості AtomicWriteVolumeUserFields.</td>
    </tr>
  </tbody>
</table>
...
Вміст поля Data цільової ConfigMap буде представляти пари ключ-значення як змінні середовища. Ключі з поля BinaryData наразі не передаються у змінні середовища контейнера.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Ключ для вибору з поля Data ConfigMap. Ключі з поля BinaryData наразі не передаються у змінні середовища контейнера.</td>
    </tr>
    <tr>
      <td><code>name</code><br/><em>string</em></td>
      <td>Імʼя референта. Це поле фактично обовʼязкове, але через зворотну сумісність дозволяється залишати його порожнім. Екземпляри цього типу з порожнім значенням тут майже напевно неправильні. Більше інформації: <a href="/uk/docs/concepts/overview/working-with-objects/names/#names">https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names</a></td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>defaultUser є опціональним: як правило UID власника створених файлів. Поле defaultUser використовується лише як резервне, коли поле user на рівні елемента не встановлено. (Alpha) Це поле вимагає увімкнення функціональної можливості AtomicWriteVolumeUserFields.</td>
    </tr>
    <tr>
      <td><code>items</code><br/><em><a href="{{< ref "#KeyToPath" >}}">KeyToPath array</a></em></td>
      <td>Якщо не вказано, кожну пару ключ-значення в полі Data вказаного ConfigMap буде спроєцьовано в том як файл, імʼя якого є ключем, а вміст — значенням. Якщо вказано, перелічені ключі будуть спроєцьовані в зазначені шляхи, а невказані ключі не будуть присутні. Якщо вказано ключ, якого немає в ConfigMap, налаштування тому завершиться помилкою, якщо він не позначений як необовʼязковий. Шляхи повинні бути відносними і не можуть містити '..' або починатися з '..'.</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>mode</code><br/><em>integer</em></td>
      <td>mode визначає біти дозволів для теки emptyDir у числовій нотації (наприклад, 0755, 01777). Має бути значенням між 0000 та 01777. Якщо не вказано, стандартно — 0777. Це може конфліктувати з іншими опціями, що впливають на режим файлу, наприклад fsGroup. Якщо вказано fsGroup, дозволи fsGroup перевизначать тут вказаний режим. Це поле не має ефекту у Windows. Це поле є альфа і вимагає увімкнення функціональної можливості EmptyDirVolumeMode.</td>
    </tr>
    <tr>
...
EvictionResponder дозволяє вказати відповідача, який реагує на виселення. Відповідачі повинні спостерігати та спілкуватися через Eviction Resource API, щоб сприяти коректному виселенню цілі (наприклад, завершенню роботи пода).
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>name</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>name дозволяє ідентифікувати відповідача, який реагує на виселення. Він повинен бути дійсним ключем з префіксом домену (наприклад, "acme.io/foo"). Імена доменів *.k8s.io та *.kubernetes.io зарезервовані. Це поле має бути унікальним для кожного відповідача. Це поле обов'язкове.</td>
    </tr>
    <tr>
      <td><code>priority</code>&nbsp;<strong>*</strong><br/><em>integer</em></td>
      <td><p>priority для цього відповідача. Вищі пріоритети обираються першими контролером evictionrequest. Якщо є відповідачі з однаковим пріоритетом, буде обрано відповідача, чиє доменне імʼя йде першим у алфавітному порядку. Це означає, що верхні мітки домену порівнюються спочатку за алфавітом, потім нижні мітки домену. Ключ порівнюється останнім.</p>
      <p>Відповідач, який є керуючим контролером пода, повинен встановити значення цього поля на 10000, щоб дозволити як пріоритетне, так і резервне реєстрування іншими відповідачами. Мінімальне значення — 0, максимальне — 100000. Інтервал 0-999 зарезервований для відповідачів з суфіксом *.k8s.io. Це поле обов'язкове.</p></td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>mode вказує режим підключення для перевірки стану gRPC. Встановіть "TLS", щоб використовувати TLS без перевірки сертифіката. Встановіть "Plaintext", щоб явно використовувати незашифроване (ненадійне) підключення. Якщо не вказано, перевірка використовує незашифроване (ненадійне) підключення.<br/><br/>
      Можливі значення перерахування:
      <ul>
        <li><code>"Plaintext"</code> вказує, що перевірка повинна використовувати незашифроване (ненадійне) gRPC-підключення.</li>
        <li><code>"TLS"</code> вказує, що перевірка повинна підключатися з використанням TLS без перевірки сертифіката.</li>
      </ul></td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>protocol</code><br/><em>string</em></td>
      <td>Protocol обирає протокол для підключення перевірки. Nil стандартно використовує HTTP/1.1.<br/><br/>
      Можливі значення enum:
      <ul>
        <li><code>"HTTP1"</code> використовує HTTP/1.1 (наявний типовий стандарт).</li>
        <li><code>"HTTP2"</code> використовує HTTP/2. Наразі підтримується лише незашифрований протокол з попереднім знанням (h2c) і повинен використовуватися зі схемою HTTP.</li>
      </ul></td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>user</code><br/><em>integer</em></td>
      <td>user є опціональним: UID власника створеного файлу. Якщо вказано, поле user на рівні елемента має пріоритет над defaultUser. (Alpha) Це поле вимагає, щоб функціональний прапорець AtomicWriteVolumeUserFields був увімкнений.</td>
    </tr>
  </tbody>
</table>
...
NodeAllocatableMappedResources описує зіставлені виділення ресурсів, доступних на вузлі.
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>name</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Назва ресурсу (наприклад, cpu, memory).</td>
    </tr>
    <tr>
      <td><code>quantity</code>&nbsp;<strong>*</strong><br/><em><a href="{{< ref "../definitions/quantity-resource#Quantity" >}}">Quantity</a></em></td>
      <td>Quantity — це загальна ємність ресурсу, доступного на вузлі, виділена для цього запиту. Виділені пристрої цього запиту спільно використовуються всіма контейнерами, які посилаються на цей запит. Kubelet додає це значення як до запитів, так і до обмежень на рівні cgroup поду, а також до обмежень на рівні cgroup контейнера для кожного контейнера, який посилається на цей запит.</td>
    </tr>
  </tbody>
</table>
...
NodeAllocatableOverheadResources описує допоміжні виділення ресурсів на накладні витрати.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>PerContainer — це змінна величина накладних витрат, що застосовується для кожного контейнера, який посилається на цей запит. Посилання на контейнери записуються в <code>nodeAllocatableResourceClaimStatuses.containers</code>. Загальна величина накладних витрат, виділена для цього запиту, обчислюється так: Quantity = PerPod + (PerContainer * NumReferences) Kubelet враховує ці накладні витрати в cgroup:
      <ul>
        <li>cgroup на рівні поду (запити та обмеження): Kubelet додає PerPod + (PerContainer * NumReferences).</li>
        <li>cgroup на рівні контейнера (тільки обмеження): Kubelet додає PerPod + PerContainer для кожного контейнера, що посилається на запит. Це дозволяє будь-якому окремому контейнеру отримати доступ до накладних витрат на рівні поду, тоді як батьківська cgroup обмежує загальне використання, щоб врахувати PerPod лише один раз.</li>
        <li>Принаймні одне з полів PerPod або PerContainer має бути вказане. Вказівка жодного з них є некоректною конфігурацією.</li>
      </ul>
      </td>
    </tr>
    <tr>
      <td><code>perPod</code><br/><em><a href="{{< ref "../definitions/quantity-resource#Quantity" >}}">Quantity</a></em></td>
      <td>PerPod — це фіксована величина накладних витрат, виділена на кожен под. Додавання до обмеження кожного контейнера дозволяє окремим контейнерам використовувати накладні витрати, тоді як обмеження cgroup на рівні поду обмежує загальне використання на межі поду, де накладні витрати враховуються лише один раз. Принаймні одне з полів PerPod або PerContainer має бути вказане. Вказівка жодного з них є некоректною конфігурацією.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>mapping</code><br/><em><a href="{{< ref "#NodeAllocatableMappedResources" >}}">NodeAllocatableMappedResources array</a></em><br/><em>patch strategy: злиття за ключем <code>name</code></em></td>
      <td>Mapping містить виділення через пристрої, зіставлені в полі <code>nodeAllocatableResources[...].mapping</code> специфікації пристрою. Це використовується kubelet для забезпечення cgroup на рівні поду та контейнера.</td>
    </tr>
    <tr>
      <td><code>overhead</code><br/><em><a href="{{< ref "#NodeAllocatableOverheadResources" >}}">NodeAllocatableOverheadResources array</a></em><br/><em>patch strategy: злиття за ключем <code>name</code></em></td>
      <td>Overhead містить виділення через пристрої, зіставлені в полі <code>nodeAllocatableResources[...].overhead</code> специфікації пристрою. Це використовується kubelet для забезпечення cgroup на рівні поду та контейнера.</td>
    </tr>
    <tr>
      <td><code>resourceClaimName</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>ResourceClaimName — це запит ресурсів, на який посилається под, що призвів до цього виділення ресурсів на вузлі.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>user</code><br/><em>integer</em></td>
      <td>user є опціональним: власник UID створеного файлу. Якщо вказано, поле user на рівні елемента має пріоритет над defaultUser. (Alpha) Це поле вимагає, щоб функціональний прапорець AtomicWriteVolumeUserFields був увімкнений.</td>
    </tr>
    <tr>
...
PodVolumeHealth містить інформацію про стан справності тому, що використовується подом, і повідомляється втулком CSI node через kubelet.
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>healthConditions</code><br/><em><a href="{{< ref "../definitions/volume-health-condition-v1#VolumeHealthCondition" >}}">VolumeHealthCondition array</a></em><br/><em>patch strategy: злиття за ключем <code>status</code></em></td>
      <td>conditions — це набір несприятливих умов, про які повідомляє втулок CSI node для цього тому на цьому вузлі. Може бути повідомлено не більше 16 умов.</td>
    </tr>
    <tr>
      <td><code>lastTransitionTime</code><br/><em><a href="{{< ref "../definitions/time-v1-meta#Time" >}}">Time</a></em></td>
      <td>lastTransitionTime — це час, коли поточний набір умов вперше зʼявився.</td>
    </tr>
    <tr>
      <td><code>name</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>name відповідає запису в pod.spec.volumes.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>defaultUser</code><br/><em>integer</em></td>
      <td>defaultUser є опціональним: як правило UID власника створених файлів. Поле defaultUser використовується лише як резервне, коли поле користувача на рівні елемента не встановлено. (Alpha) Це поле вимагає увімкнення функціонального прапора AtomicWriteVolumeUserFields.</td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>name</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Назва ресурсу. Має бути унікальною в межах Podʼа і, у випадку ресурсу без DRA, відповідати одному з ресурсів з специфікації Podʼа. Для ресурсів DRA значення має бути "claim:&lt;claim_name&gt;/&lt;request&gt;" коли встановлено <code>container.resources.claims[*].request</code> або "claim:&lt;claim_name&gt;" коли <code>container.resources.claims[*].request</code> порожнє. Для розширених ресурсів, що підтримуються DRA, "claim:&lt;claim_name&gt;/&lt;request&gt;" використовується, коли імʼя заявки та імʼя запиту зафіксовані в <code>pod.status.extendedResourceClaimStatus</code>. Коли цей статус повідомляється про контейнер, "claim_name" і "request" мають відповідати одній з заявок цього контейнера.</td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>user</code><br/><em>integer</em></td>
      <td>user є опціональним: як правило UID власника створеного файлу. Якщо вказано, поле user на рівні елемента має пріоритет над defaultUser. (Alpha) Це поле вимагає увімкнення функціональної можливості AtomicWriteVolumeUserFields.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>bindMountOptions є списком додаткових параметрів bind mount, які застосовуються під час монтування цього тому всередині контейнера. Дозволені значення: noexec, nodev і nosuid. Це параметри монтування Linux і не впливають на вузли Windows. Це поле не підтримується для image volumes. Це альфа-поле і вимагає увімкнення функціональної можливості VolumeBindMountOptions.</td>
    </tr>
    <tr>
      <td><code>mountPath</code>&nbsp;<strong>*</strong><br/><em>string</em></td>
      <td>Шлях всередині контейнера, до якого буде змонтовано том.</td>
    </tr>
    <tr>
