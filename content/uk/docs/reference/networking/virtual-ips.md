...
Якщо ви не обираєте режим явно (через параметр командного рядка `--proxy-mode` або поле `mode` у файлі конфігурації) під час запуску kube-proxy, тоді він використовуватиме рекомендовану стандартну версію. У Kubernetes {{< skew currentVersion >}} це `iptables`, але майбутня версія Kubernetes змінить стандартний режим на `nftables`. Щоб уникнути несподіваної зміни бекенду проксі у кластері під час оновлення, ви повинні переконатися, що всі кластери мають конфігурацію kube-proxy, яка явно вказує, який режим використовувати.
...
**Режим проксі `ipvs` є застарілим**. Підтримку режиму `ipvs` буде вимкнено стандартно, починаючи з Kubernetes v1.40 (ви можете знову увімкнути її за допомогою [функціональної можливості](/docs/reference/command-line-tools-reference/feature-gates/) `KubeProxyIPVS`); режим `ipvs` буде повністю видалено в Kubernetes v1.43.
...
- **Сервіси `type: NodePort` на `127.0.0.1`**: У режимі `iptables`, якщо діапазон `--nodeport-addresses` включає `127.0.0.1` (і не передано опцію `--iptables-localhost-nodeports false`), тоді Сервіси `type: NodePort` будуть доступні навіть на "localhost" (`127.0.0.1`). Спочатку в режимі `nftables` це не працювало. Однак у Kubernetes {{< skew currentVersion >}} ви можете увімкнути localhost NodePorts у режимі `nftables`, увімкнувши функціональну можливість `KubeProxyNFTablesLocalhostNodePorts` і встановивши `--nodeport-addresses` у значення `primary,localhost` замість стандартного значення `primary`.

  Якщо ви не впевнені, чи залежите від цієї функціональності, ви можете перевірити метрику kube-proxy `iptables_localhost_nodeports_accepted_packets_total`; якщо вона не дорівнює 0, це означає, що якийсь клієнт підключився до Сервісу `type: NodePort` через localhost.
...
Для використання цієї можливості вам необхідно запускати kube-proxy з аргументом командного рядка `--enable-dsr` **і** включити функціональну можливість [`WinDSR`](/docs/reference/command-line-tools-reference/feature-gates/#WinDSR).
...
### Політика внутрішнього трафіку {#internal-traffic-policy}
...
### Трафік до термінальних точок доступу {#traffic-to-terminating-endpoints}

{{< feature-state for_k8s_version="v1.28" state="stable" >}}

Якщо в kube-proxy включена функціональна можливість [`ProxyTerminatingEndpoints`](/docs/reference/command-line-tools-reference/feature-gates/#ProxyTerminatingEndpoints) і політика трафіку встановлена на `Local`, `kube-proxy` на даному вузлі використовує більш складний алгоритм для вибору точок доступу Service. З цим включеним функціоналом, `kube-proxy` перевіряє наявність локальних точок доступу і те, чи всі локальні кінцеві точки позначені як термінальні. Якщо всі локальні кінцеві точки позначені як термінальні, тоді `kube-proxy` направлятиме трафік на ці термінальні кінцеві точки. В іншому випадку `kube-proxy` завжди віддає перевагу направленню трафіку на кінцеві точки, які не є термінальними.

Це поведінка направлення для термінальних точок доступу існує для того, щоб `NodePort` і `LoadBalancer` сервіси могли відповідно завершувати зʼєднання при використанні `externalTrafficPolicy: Local`.
