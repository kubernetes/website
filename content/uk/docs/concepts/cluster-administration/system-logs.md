...
Якщо розробники використовують додаткові функції, такі як `WithValues` або `WithName`, у своїх компонентах, то записи в журнал міститимуть додаткову інформацію, яка передається у функції викликачем.
...
Коли контекстне ведення логів вимкнене, функції `WithValues` та `WithName` нічого не роблять, а виклики ведення логів проходять через глобальний реєстратор klog. Отже, ця додаткова інформація більше не виводиться в журнал:
...
Функція Log Query може допомогти у пошуку проблем як на вузлах Linux, так і на вузлах Windows. Введена в Kubernetes v1.27, ця функція дозволяє переглядати логи сервісів, що працюють на вузлі. Щоб використовувати цю функцію, переконайтеся, що параметри конфігурації kubelet `enableSystemLogHandler` та `enableSystemLogQuery` обидва встановлені в *true* для цільового вузла.

В Kubernetes v1.36 ця функція стала стабільною, і функціональна можливість [`NodeLogQuery`](/docs/reference/command-line-tools-reference/feature-gates/#NodeLogQuery) тепер зафіксовано на *true*, отже, її стандартно увімкнено, залишаючи `enableSystemLogHandler` єдиним параметром, необхідним для увімкнення або вимкнення функції Log Query.

`enableSystemLogHandler` зазвичай встановлено в *false* і рекомендується залишати вимкненою, якщо не ведеться активне відлагодження.

{{< warning >}}
Надання дозволів на `nodes/proxy` (навіть лише **get** дозвіл) також надає доступ до потужних API kubeletʼа, які можна використовувати для виконання команд у будь-якому контейнері, що працює на вузлі, тому будьте обережні щодо того, як ви ними керуєте. Див. [Автентифікація/авторизація kubelet](/docs/reference/access-authn-authz/kubelet-authn-authz/#get-nodes-proxy-warning) для отримання додаткової інформації.
{{< /warning >}}

На Linux передбачається, що логи сервісів доступні через *journald*. На Windows передбачається, що логи сервісів доступні через провайдера журналу застосунків. На обох операційних системах логи також доступні шляхом читання файлів у теці `/var/log/`.
...
- Дізнайтеся про [Умови ведення логу за важливістю](https://github.com/kubernetes/community/blob/main/contributors/devel/sig-instrumentation/logging.md)
