...
{{< caution >}}
Пакунок `unattended-upgrades` з Debian конфліктує з належним вимкненням вузла у своїй стандартній конфігурації. Якщо ви використовуєте стандартну конфігурацію `unattended-upgrades`, яка налаштовує період очікування завершення роботи сервера, kubelet не зможе отримати необхідний механізм блокування для правильної обробки подій вимкнення.

Це відбувається, якщо значення `shutdownGracePeriod` перевищує 30 секунд. Щоб уникнути цього, ви можете подавити частину конфігурації `unattended-upgrades`, зробивши `/etc/systemd/logind.conf.d/unattended-upgrades-logind-maxdelay.conf` символічним посиланням на `/dev/null`.

Для отримання додаткової інформації дивіться документацію [`logind.conf`](https://www.freedesktop.org/software/systemd/man/latest/logind.conf.html).
{{< /caution >}}
...
- `shutdownGracePeriod`:
  - Визначає загальний час, протягом якого вузол повинен затримати вимикання. Це загальний період, протягом якого завершуються як звичайні, так і [критичні](/docs/tasks/administer-cluster/guaranteed-scheduling-critical-addon-pods/#marking-pod-as-critical) Podʼи.
...
Функція [належного вимикання вузла](#graceful-node-shutdown), яка описана вище, вимикає Podʼи у дві фази: звичайні Podʼи, а потім критичні Podʼи. Якщо потрібна додаткова гнучкість для явного визначення порядку Podʼів під час вимикання в більш деталізований спосіб, можна використовувати належне (graceful) вимикання вузла на основі пріоритету Podʼа.
