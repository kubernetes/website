...
Компоненти панелі управління Kubernetes, так само як і багато надбудов, генерують та надсилають ці сигнали. Обʼєднуючи та корелюючи їх, ви можете отримати єдину картину панелі управління, надбудов і застосунків у вашому кластері.
...
Типовий потік метрик Kubernetes періодично зчитує ці точки доступу та зберігає відібрані зразки в базі даних часових рядів (приклад з Prometheus).
...
### API метрик {#metrics-api}

{{< feature-state for_k8s_version="v1.37" state="stable" >}}

API метрик Kubernetes надає дані про використання CPU та памʼяті вузлами та Podʼами. Команда `kubectl top` та такі компоненти, як [HorizontalPodAutoscaler](/docs/concepts/workloads/autoscaling/horizontal-pod-autoscale/) і [VerticalPodAutoscaler](/docs/concepts/workloads/autoscaling/vertical-pod-autoscale/), використовують цей API.

`kubectl top` підтримує як `metrics.k8s.io/v1`, так і `metrics.k8s.io/v1beta1`. Він надає перевагу `v1`, коли ця версія доступна, і повертається до `v1beta1`. У Kubernetes v1.37 контролер HorizontalPodAutoscaler підтримує лише `metrics.k8s.io/v1beta1`; підтримка `metrics.k8s.io/v1` запланована, але ще недоступна.

На відміну від точок доступу метрик компонентів, описаних раніше, API метрик обслуговується через [шар агрегації API](/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/) Kubernetes. Ваш кластер повинен запускати [Metrics Server](https://github.com/kubernetes-sigs/metrics-server) або іншу реалізацію, яка надає цей API. API метрик навмисно надає лише метрики ресурсів, необхідні для автомасштабування та базового огляду; він не замінює повноцінний конвеєр моніторингу.

Щоб дізнатися про API, його реалізації та потік даних від kubelet-ів до його клієнтів, дивіться [конвеєр метрик ресурсів](/docs/tasks/debug/debug-cluster/resource-metrics-pipeline/).
...
Логи дозволяють отримати хронологічні записи подій всередині застосунків, компонентів системи Kubernetes та діяльності, повʼязаної з безпекою, такої як журнали аудиту.
...
flowchart LR
    subgraph Джерела
        A[Вивід застосунку<br/>в stdout / stderr]
...
**Note:** Цей розділ містить посилання на сторонні проєкти, які надають можливості спостережуваності, необхідні для Kubernetes. Автори проєкту Kubernetes не несуть відповідальності за ці проєкти, які перераховані в алфавітному порядку. Щоб додати проєкт до цього списку, прочитайте [посібник зі вмісту](/docs/contribute/style/content-guide/) перед внесенням змін.
...
- [Thanos](https://thanos.io/) розширює Prometheus механізмом глобальних запитів, зниженням частоти дискретизації (downsampling) та додаючи підтримку обʼєктного зберігання.
...
- Ознайомтесь з [посібником із системних метрик](/docs/concepts/cluster-administration/system-metrics/) для отримання відомостей про точки доступу компонентів та їх стабільність
