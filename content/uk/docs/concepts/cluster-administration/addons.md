...
* [Gateway API](/docs/concepts/services-networking/gateway/): відкритий проєкт, керований [SIG Network](https://github.com/kubernetes/community/tree/main/sig-network), який надає виразний, розширюваний та рольовий API для моделювання мережевих сервісів.
* [Knitter](https://github.com/ZTE/Knitter/) є втулком для підтримки кількох мережевих інтерфейсів в Podʼі Kubernetes.
* [kube-router](https://github.com/cloudnativelabs/kube-router) є відкритим рішенням для мережі Kubernetes з метою забезпечення простоти експлуатації та високої продуктивності. Він використовує Kubernetes API, BGP та Golang для контрольного шляху та примітиви мережі Linux (IPVS, nftables тощо) для шляху даних. Це забезпечує низьке навантаження та використовується як у k0s, так і в k3s.
...
* [Nodus](https://github.com/akraino-edge-stack/icn-nodus): втулок контролера CNI на основі OVN для надання ланцюжка сервісних функцій (SFC).
...
* [Spiderpool](https://github.com/spidernet-io/spiderpool): мережеве рішення underlay та RDMA для Kubernetes. Spiderpool підтримується на фізичному обладнанні, віртуальних машинах та публічних хмарних середовищах.
...
* [Weave Net](https://github.com/rajch/weave#using-weave-on-kubernetes): надає мережеве рішення та мережеві політики, буде продовжувати працювати з обох боків розділення мережі та не потребує зовнішньої бази даних.
...
* [KubeVirt](https://kubevirt.io/user-guide/#/installation/installation) — це надбудова для запуску віртуальних машин в Kubernetes. Зазвичай використовується в кластерах, розгорнутих на фізичному обладнанні.
...
Існують ще кілька інших надбудов, які визнані застарілими та знаходяться у теці [cluster/addons](https://git.k8s.io/kubernetes/cluster/addons).
