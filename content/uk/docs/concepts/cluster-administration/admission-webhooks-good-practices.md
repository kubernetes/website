...
* [Встановлення стандартних значень](/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#defaulting)
...
Під час проєктування ваших модифікуючих вебхуків враховуйте ваші вимоги до затримки та толерантність. Чим більше модифікуючих вебхуків у вашому кластері, тим більша ймовірність збільшення затримки.
...
* Збіг має бути зі всіма версіями обʼєкта.
...
* Не зіставляйте TokenReview, SubjectAccessReview або інші [віртуальні ресурси автентифікації та авторизації](/docs/reference/access-authn-authz/extensible-admission-controllers/#excluded-virtual-resources). Це завжди запити тільки для читання, і їх перехоплення може порушити роботу вашого кластера. Починаючи з Kubernetes v1.37, вебхуки допуску зазвичай не викликаються для цих ресурсів.
...
* Уникайте самозмін.
* Не приховуйте збої (fail open) та перевіряйте кінцевий стан.
...
* Запобігайте перевірці або зміні інших вебхуків. Розгляньте можливість [виключення конкретних просторів імен](/docs/reference/access-authn-authz/extensible-admission-controllers/#matching-requests-namespaceselector) з виклику вашого вебхука.
...
Дозвольте вашим модифікуючим вебхукам "не приховувати збої (fail open)", встановивши поле `failurePolicy` в Ignore. Використовуйте контролер перевірки для перевірки стану запитів, щоб забезпечити їх відповідність вашим політикам.
...
Щоб уникнути цього, розгляньте можливість встановлення унікальної мітки у будь-яких ресурсах, які створює ваш вебхук. Перевіряйте наявність цієї мітки в умовах збігу вашого вебхуку.
...
### Переконайтеся, що модифікуючі вебхуки у вашому кластері є ідемпотентними {#ensure-mutating-webhook-idempotent}
...
3. Для запиту **create** Pod додайте контейнер sidecar з імʼям `foo-sidecar` без перевірки, чи існує контейнер `foo-sidecar`.
   ...
   Повторний виклик вебхука призведе до дублювання контейнерів у Pod, що робить запит недійсним і відхиленим API-сервером.
