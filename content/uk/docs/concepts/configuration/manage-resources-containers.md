...
Якщо вузол, на якому запущений Pod, має достатньо вільного ресурсу, то контейнер може (і має право) використовувати більше ресурсу, ніж зазначено в його `запиті`.
...
Обмеження — то вже інша справа. Обидва обмеження `cpu` та `memory` застосовуються kubeletʼом (та {{< glossary_tooltip text="середовищем виконання контейнерів" term_id="container-runtime" >}}), і, зрештою, забезпечуються ядром. На вузлах Linux, ядро накладає обмеження за допомогою {{< glossary_tooltip text="cgroups" term_id="cgroup" >}}. Поведінка застосування обмежень `cpu` та `memory` відрізняється.

Застосування обмеження `cpu` виконується через зниження частоти доступу до процесора. Коли контейнер наближається до свого обмеження, ядро обмежує доступ до процесора, відповідно до обмежень контейнера. Тож, обмеження `cpu` є жорстким обмеженням, що застосовує ядро. Контейнери не можуть використовувати більше CPU, ніж зазначено в їх обмеженні `cpu`.

Застосування обмеження `memory` виконується ядром за допомогою припинення процесів через механізм браку памʼяті (OOM — Out Of Memory kills). Якщо контейнер намагається використати більше памʼяті, ніж його обмеження, ядро може припинити його роботу. Однак, припинення роботи контейнера відбувається тільки тоді, коли ядро виявляє тиск на памʼять. Це означає, що обмеження `memory` застосовується реактивно. Контейнер може використовувати більше памʼяті, ніж його обмеження, однак, якщо він перевищує ліміт, то його робота може бути припинена.

{{< note >}}
Існує функція `MemoryQoS`, яка додає обмеження памʼяті та необовʼязкове багаторівневе резервування памʼяті на вузлах Linux за допомогою cgroup v2. Для деталей дивіться [Memory QoS з cgroup v2](/docs/concepts/workloads/pods/pod-qos/#memory-qos-with-cgroup-v2).
{{< /note >}}

{{< note >}}
Якщо ви вказали обмеження для ресурсу, але не вказали жодного запиту, і жодний механізм допуску не застосував стандартного значення запиту для цього ресурсу, то Kubernetes скопіює обмеження, яке ви вказали, та використовує його як запитане значення для ресурсу.
{{< /note >}}

## Типи ресурсів {#resource-types}

_Тип ресурсу_ має базові одиниці вимірювання і може бути запитаним, обмеженим або обома. Kubernetes має наступні вбудовані типи ресурсів:

| Тип ресурсу | Опис | Базова одиниця |
|---|---|---|
| `cpu` | Обчислювальні ресурси | cpu (ядро) |
| `memory` | Оперативна памʼять | Байти |
| `ephemeral-storage` | [Локальне тимчасове сховище](/docs/concepts/storage/ephemeral-storage/) | Байти |
| `hugepages-<size>` | [Великі сторінки](#huge-pages) (тільки Linux) | Байти |

Кластери також можуть надавати [розширені ресурси](/docs/concepts/configuration/manage-resources-containers/#extended-resources) (ресурси з власним імʼям, зазвичай надаються втулками пристроїв).

### Великі сторінки {#huge-pages}

Для робочих завдань на Linux ви можете вказати _huge page_ ресурсів. Huge page є функцією, специфічною для Linux, де ядро вузла виділяє блоки памʼяті, що значно більше, ніж розмір стандартної сторінки.
...
Хоча ви можете вказувати запити та обмеження тільки для окремих контейнерів, також корисно думати про загальні запити та обмеження ресурсів для Podʼа. Для певного ресурсу _запит/обмеження ресурсів Podʼа_ — це сума запитів/обмежень цього типу для кожного контейнера в Podʼі.

## Специфікація ресурсів на рівні Podʼів {#pod-level-resource-specification}

{{< feature-state feature_gate_name="PodLevelResources" >}}

За умови, що у вашому кластері увімкнено функціональну можливість  [`PodLevelResources`](/docs/reference/command-line-tools-reference/feature-gates/#PodLevelResources), ви можете вказувати запити на ресурси та ліміти на рівні Podʼа. На рівні Podʼа, Kubernetes {{< skew currentVersion >}} підтримує запити на ресурси або ліміти лише для певних типів ресурсів: `cpu` та/або `memory`, та/або `hugepages`. Завдяки цій функції Kubernetes дозволяє вам декларувати загальний бюджет ресурсів для Podʼів, що є особливо корисним при роботі з великою кількістю контейнерів, де може бути важко точно визначити індивідуальні потреби у ресурсах. Крім того, це дає змогу контейнерам в рамках Podʼа ділитися один з одним ресурсами, що простоюють, покращуючи використання ресурсів.
...
Обмеження та запити для `memory` вимірюються в байтах. Ви можете вказувати памʼять як звичайне ціле число або як число з плаваючою комою, використовуючи один з наступних суфіксів [кількості](/docs/reference/kubernetes-api/common-definitions/quantity/): E, P, T, G, M, k. Ви також можете використовувати еквіваленти степенів двійки: Ei, Pi, Ti, Gi, Mi, Ki. API Kubernetes також дозволяє використовувати суфікс m (для мілібайтів: 1/1000 байта), але це не корисно для вказівки: завжди потрібно призначати цілі числа байтів або іноді більші блоки, такі як кратні 1 гібібайту.

Ось кілька прикладів визначення обсягу памʼяті, які приблизно представляють те ж саме значення:

```shell
128974848, 129e6, 129M,  128974848000m, 123Mi
```

Звертайте увагу на регістр суфіксів.  "M" означає мегабайти, тоді як "m" означає мілібайти. Якщо ви запитуєте `400m` памʼяті, це запит на 0.4 байта. Той, хто вводить це, ймовірно, мав на увазі запросити 400 мебібайтів (`400Mi`)
...
Цю можливість можна увімкнути встановленням функціональної можливості [`PodLevelResources`](/docs/reference/command-line-tools-reference/feature-gates/#PodLevelResources). Наступний Pod має явний запит на 1 CPU та 100 MiB памʼяті, а також явний ліміт на 1 CPU та 200 MiB памʼяті.  Контейнер `pod-resources-demo-ctr-1` має явні запити та обмеження. Однак, контейнер `pod-resources-demo-ctr-2` буде просто ділити ресурси, доступні в межах ресурсів Podʼа, оскільки він не має явних запитів і обмежень.
...
Коли увімкнено функціональну можливість `InPlacePodVerticalScalingMemoryBackedVolumes`, ви можете динамічно змінювати `sizeLimit` тому `emptyDir`, що зберігається в памʼяті (`medium: Memory`), на працюючому Podʼі без повторного створення Podʼа або перезапуску контейнерів. Покрокові інструкції див. у розділі [Зміна розміру ресурсів CPU та памʼяті, призначених контейнерам](/docs/tasks/configure-pod-container/resize-container-resources/#resizing-memory-backed-emptydir-volumes).
{{< /note >}}
...
Як альтернатива, адміністратор кластера може забезпечити дотримання обмежень на розмір томів `emptyDir` в нових Podʼах, використовуючи механізм політики, такий як [ValidatingAdmissionPolicy](/docs/reference/access-authn-authz/validating-admission-policy).

## Локальне тимчасове сховище {#local-ephemeral-storage}

Щодо загальних концепцій про локальне тимчасове сховище та порад щодо налаштування запитів і/або обмежень локального тимчасового сховища для контейнера, дивіться сторінку [локальне тимчасове сховище](/docs/concepts/storage/ephemeral-storage/).
...
{{< note >}}
У запиті `~1` — це кодування символу `/` в шляху патча. Значення операційного шляху у JSON-Patch інтерпретується як JSON-вказівник. Для отримання докладнішої інформації дивіться [RFC 6901, розділ 3](https://datatracker.ietf.org/doc/html/rfc6901#section-3).
{{< /note >}}
...
Розширене розподілення ресурсів DRA дозволяє адміністраторам кластерів вказувати `extendedResourceName` у DeviceClass, після чого пристрої, що відповідають DeviceClass, можуть бути запитані з розширених запитів ресурсів Podʼа. Читайте більше про [Розширене розподілення ресурсів DRA](/docs/concepts/resource-management/dynamic-resource-allocation/dra-features/#extended-resource).
...
Сервер API обмежує кількість розширених ресурсів цілими числами. Приклади _дійсних_ значень: `3`, `3000m` та `3Ki`. Приклади _недійсних_ значень: `0.5` та `1500m` (тому що `1500m` буде давати в результаті `1.5`).
...
- Перевірте наявність "taint" вузла. Якщо більшість ваших вузлів мають "taint", і новий Pod не толерує цей "taint", планувальник розглядає розміщення лише на решті вузлів, які не мають цього "taint".
...
### Робота мого контейнера завершується примусово {#my-container-is-terminated}
...
Наступним кроком може бути перевірка коду програми на витік памʼяті. Якщо ви виявите, що програма працює так, як очікувалося, розгляньте встановлення вищого ліміту памʼяті (і, можливо, запиту) для цього контейнера.
...
- Дізнайтеся більше про [Розширене розподілення ресурсів через DRA](/docs/concepts/resource-management/dynamic-resource-allocation/dra-features/#extended-resource)
