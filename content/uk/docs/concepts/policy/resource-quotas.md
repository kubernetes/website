...
{{< caution >}}
Ані конкуренція, ані зміни до квоти не вплинуть на вже створені ресурси.
{{< /caution >}}
...
У випадку, коли загальна місткість кластера менше суми квот просторів імен, може виникнути конкуренція за ресурси. Це обробляється за принципом "хто перший прийшов, той і молотить" (FIFO).
...
Коли увімкнено [Розширений розподіл ресурсів за допомогою DRA](/docs/concepts/resource-management/dynamic-resource-allocation/dra-features/#extended-resource), той самий клас пристроїв із назвою `examplegpu` можна запитувати через розширений ресурс або явно, коли вказано поле ExtendedResourceName класу пристроїв, наприклад, `example.com/gpu`, тоді ви можете визначити квоту таким чином:
...
| `pods` | Загальна кількість Podʼів у просторі імен, що не перебувають в стані завершення роботи. Pod вважається завершеним, якщо `.status.phase in (Failed, Succeeded)` є true. |
...
Наприклад, квота `pods` рахує та обмежує максимальну кількість `Pod`ʼів, створених у одному просторі імен, що не перебувають в стані завершення роботи. Ви можете встановити квоту `pods` у просторі імен, щоб уникнути випадку, коли користувач створює багато невеликих Podʼів і вичерпує запаси IP-адрес Podʼів у кластері.
...
Podʼи можуть бути створені з певним [пріоритетом](/docs/concepts/scheduling-eviction/pod-priority-preemption/#pod-priority). Ви можете контролювати споживання системних ресурсів подом на основі його пріоритету, використовуючи поле `scopeSelector` у специфікації квоти.
...
## Типове обмеження споживання PriorityClass {#limit-priority-class-consumption-by-default}
...
Ця область відстежує лише квоту, спожиту PersistentVolumeClaims.
