...
[kube-proxy](/docs/concepts/architecture/#kube-proxy) (опціонально)
...
Ваш кластер може потребувати додаткового програмного забезпечення на кожному вузлі; наприклад, ви можете також запускати [systemd](https://systemd.io/) на вузлі Linux для контролю локальних компонентів.
...
: Збирає та зберігає метрики контейнерів.

[Cluster-level Logging](/docs/concepts/architecture/#cluster-level-logging)
: Для збереження логів контейнерів у централізованому сховищі логів.
...
Для отримання більш детальної інформації про кожен компонент та різні способи налаштування кластерної архітектури, дивіться сторінку [Архітектура кластера](/docs/concepts/architecture/).
