...
- Розширене управління розподілом ресурсів та ізоляція між різними ресурсами
  ...
  - Облік відкладених змін ресурсів, таких як запис кешу сторінок
...
При переході на cgroup v2 не повинно бути помітної різниці в використанні, якщо користувачі не звертаються безпосередньо до файлової системи cgroup чи на вузлі, чи зсередини контейнерів.
...
- Якщо ви розгортаєте застосунки на [Node.js](https://nodejs.org/), віддавайте перевагу версіям, які виявляють обмеження памʼяті cgroup v2. Node.js читає обмеження памʼяті cgroup v2 (через [libuv](https://libuv.org/)) починаючи з Node.js v20.3.0. Лінія випуску v18 не надійно виявляє обмеження памʼяті cgroup v2. Версії без цієї підтримки можуть читати загальну памʼять хоста замість обмеження, застосованого до поду, що може призвести до неправильно розміру heap і завершення роботи через нестачу памʼяті (OOM). У версіях, що мають таку вразливість, встановіть розмір heap явно, наприклад за допомогою прапораця `--max-old-space-size`.
...
Kubelet більше стандартно не буде запускатися на вузлі cgroup v1. Щоб вимкнути це налаштування, адміністратор кластера повинен встановити `failCgroupV1` на false у [файлі конфігурації kubelet](/docs/tasks/administer-cluster/kubelet-config-file/).
