...
### Звільнення блокування вибору лідера kube controller manager при виході {#kube-controller-manager-lock-release-on-exit}

{{< feature-state feature_gate_name="ControllerManagerReleaseLeaderElectionLockOnExit" >}}

Коли функціональна можливість `ControllerManagerReleaseLeaderElectionLockOnExit` увімкнена, `kube-controller-manager` активно звільняє блокування вибору лідера під час переходу лідерства, замість того, щоб чекати закінчення терміну дії блокування. Це дозволяє швидше обрати нового лідера, зменшуючи затримку переходу лідерства.
...
Хеш SHA256, який використовується в назвах лізингу, базується на імені хосту операційної системи, яку бачить цей API сервер. Кожен kube-apiserver повинен бути налаштований на використання унікального імені в межах кластера. Нові екземпляри kube-apiserver, які використовують те саме імʼя хосту, перехоплюють наявні лізинги за допомогою нового ідентифікатора власника, замість того щоб створювати нові обʼєкти лізингу. Ви можете перевірити імʼя хосту, яке використовує kube-apiserver, перевіривши значення мітки `kubernetes.io/hostname`:
...
Ваші власні робочі навантаження можуть визначити своє власне використання лізингів. Наприклад, ви можете запускати власний {{< glossary_tooltip term_id="controller" text="контролер" >}}, де основний член чи лідер виконує операції, які його партнери не виконують. Ви визначаєте лізинг так, щоб репліки контролера могли вибирати чи обирати лідера, використовуючи API Kubernetes для координації. Якщо ви використовуєте лізинг, це гарна практика визначати імʼя лізингу, яке очевидно повʼязане з продуктом чи компонентом. Наприклад, якщо у вас є компонент з іменем Example Foo, використовуйте лізинг з іменем `example-foo`.

Якщо оператор кластера чи інший кінцевий користувач може розгортати кілька екземплярів компонента, виберіть префікс імені і виберіть механізм (такий як хеш від назви Deployment), щоб уникнути конфліктів імен для лізингів.

Ви можете використовувати інший підхід, якщо він досягає того самого результату: відсутність конфліктів між різними програмними продуктами.
