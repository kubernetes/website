...
*Власні ресурси* є екземплярами типів ресурсів, доданими до Kubernetes через розширення API. Ця сторінка обговорює, коли додавати власний ресурс до вашого кластера Kubernetes та коли використовувати як самостійний сервіс. Вона описує два методи додавання власних ресурсів і як вибрати між ними.
...
З архітектурного погляду [хмарна](https://www.cncf.io/about/faq/#what-is-cloud-native) архітектура застосунків надає перевагу вільному звʼязуванню між компонентами. Якщо частина вашого завдання вимагає сервісу підтримки для його рутинної роботи, запускайте цей сервіс як компонент або використовуйте його як зовнішній сервіс. Таким чином, ваше робоче навантаження не залежить від API Kubernetes для його нормальної роботи.
{{< /note >}}
...
| ------- | ---- | --- | -------------- |
| Валідація | Допомагає користувачам уникати помилок та дозволяє вам самостійно розвивати ваше API незалежно від ваших клієнтів. Ці функції найбільш корисні, коли є багато клієнтів, які не можуть всі одночасно оновлюватися. | Так. Більшість валідацій можуть бути вказані у CRD за допомогою [валідації OpenAPI v3.0](/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#validation). Функціональна можливість [CRDValidationRatcheting](/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#validation-ratcheting) дозволяє ігнорувати валідації, вказані за допомогою OpenAPI, якщо збійна частина ресурсу не була змінена. Будь-які інші валідації підтримуються за допомогою [Вебхука валідації](/docs/reference/access-authn-authz/admission-controllers/#validatingadmissionwebhook-alpha-in-1-8-beta-in-1-9). | Так, довільні перевірки валідації |
...
Власні ресурси розміщуються у зберіганні на основі поточної версії зберігання ресурсу, визначеної у специфікації CRD. При будь-якому оновленні власного ресурсу для його зберігання використовується поточна визначена версія зберігання. Усі інші версії повинні або містити всі поля цієї версії, або визначати перетворення, щоб працювати належним чином.
...
Якщо ви використовуєте RBAC для авторизації, більшість ролей RBAC не надають доступ до нових ресурсів (окрім ролі cluster-admin або будь-якої ролі, створеної з wildcard-правилами). Вам потрібно явно надати доступ до нових ресурсів. CRD та агреговані сервери API часто постачаються з новими визначеннями ролей для типів, які вони додають.
...
Поле `spec.versions[*].selectableFields` в {{< glossary_tooltip term_id="CustomResourceDefinition" text="CustomResourceDefinition" >}} може бути використане для вказівки, які інші поля в власному ресурсі можуть бути використані у селекторах полів.
