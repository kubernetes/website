...
Простори користувачів — це функція Linux, яка дозволяє зіставляти користувачів у контейнері з користувачами на хості. Крім того, можливості, надані Pod в просторі користувача, є дійсними лише в просторі та не виходять за його межі.
...
Наприклад, ви можете визначити `/etc/subuid` та `/etc/subgid`, щоб обидва мали такі записи для користувача `kubelet`:

```none
# Формат такий
#   імʼя:першийID:кількість ідентифікаторів
# де
# - першийID - 65536 (мінімально можливе значення)
# - кількість ідентифікаторів - 110 (стандартний ліміт кількості) * 65536
kubelet:65536:7208960
```

### Примітка, якщо ви переконфігуруєте вузол

Якщо у вас є вузол, що запускає pod з user-namespaces, і ви хочете внести вищезазначені конфігурації, ось кілька важливих приміток.

Конфігурацію слід змінювати, коли на вузлі не запущено жодного пода, що використовує user-namespaces. При зміні на вузлі, що запускає будь-які Pod з просторами імен користувачів, вам потрібно спочатку {{< glossary_tooltip text="очистити" term_id="drain" >}} вузол перед застосуванням конфігурації та перезапуском kubelet. При очищенні вузла памʼятайте, що DaemonSet Pod, або інші Podʼи, що толерують unschedulable taint, **не** будуть виселені.

Причина, чому не може бути запущено жодного пода, що використовують user-namespaces, полягає в тому, що вони можуть використовувати будь-який діапазон, потенційно поза новим налаштованим діапазоном. Kubelet не зможе запуститися, якщо він не зможе застосувати нову конфігурацію для наявних подів на вузлі.
...
### Підтримка файлової системи {#filesystem-support}

Podʼи, які використовують простір імен користувачів, потребують, щоб файлова система підтримувала монтування idmap. Деякі файлові системи не підтримують монтування idmap, і тому не можуть використовуватися з просторами користувачів. У таких випадках будуть згенеровані наступні події. Зверніть увагу, що деталі попередження залежать від середовища виконання контейнерів, яке ви використовуєте.
...
NFS томи не можуть бути змонтовані в Pod з простором імен користувачів, оскільки клієнт Linux NFS ще не підтримує монтування idmap. Для поточного списку підтримуваних файлових систем дивіться сторінку керівництва Linux [`mount_setattr(2)`](https://man7.org/linux/man-pages/man2/mount_setattr.2.html).
