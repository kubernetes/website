...
Коли створюється Podʼа, імʼя його хосту (яке спостерігається зсередини Podʼа) походить від значення metadata.name Podʼа. І імʼя хосту, і відповідне повністю кваліфіковане доменне імʼя (FQDN) встановлюються зі значення metadata.name (з точки зору Podʼа)
...
Специфікація Podʼа включає необовʼязкове поле `hostname`. Коли воно встановлене, це значення має пріоритет над `metadata.name` Podʼа як імʼя хосту (яке спостерігається зсередини Podʼа). Наприклад, Pod зі spec.hostname встановленим як `my-host` матиме імʼя хосту встановлене як `my-host`.
