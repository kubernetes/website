...
* `PodReadyToStartContainers`: Pod sandbox був успішно створений, мережа налаштована, томи зберігання змонтовані, а будь-які динамічні ресурси (якщо запитано) виділені.
...
Після того, як Pod отримує призначення на вузол, йому потрібно бути допущеним до kubelet та мати встановлені всі необхідні томи зберігання. Як тільки ці фази завершаться, kubelet співпрацює з середовищем виконання контейнерів (використовуючи {{< glossary_tooltip term_id="cri" >}}), щоб налаштувати ізольоване середовище виконання та налаштувати мережу для Podʼа. Якщо Pod використовує [Динамічне виділення ресурсів](/docs/concepts/scheduling-eviction/dynamic-resource-allocation/), ці ресурси також виділяються під час цієї фази. Стан `PodReadyToStartContainers` додається до поля `status.conditions` Podʼа.

Стан встановлюється в `False` kubeletʼом, коли він виявляє, що у Podʼа немає ізольованого середовища виконання із налаштованою мережею. Це трапляється в
наступних випадках:
