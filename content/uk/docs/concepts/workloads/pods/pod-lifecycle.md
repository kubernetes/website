...
Поки Pod працює, kubelet керує контейнерами та передає специфікацію Podʼів до середовища виконання контейнерів. Kubelet також керує виконанням [проб](#container-probes), які відстежують стан вашого застосунку.
...
Поки Pod працює, kubelet може перезапускати контейнери, щоб вирішити деякі види несправностей. У межах Podʼа Kubernetes відстежує різні [стани контейнера](#container-states) і визначає дії, які слід вжити, щоб знову забезпечити справність Podʼів. Це робиться в [циклі опитування](/docs/reference/node/kubelet-sync-loop/), який періодично узгоджує бажаний стан (специфікацію Pod) з фактичним станом запущених контейнерів.
...
Фази Podʼа — це простий, високорівневий підсумок того, на якому етапі свого життєвого циклу знаходиться Pod. Фаза не призначена бути всеосяжним підсумком спостережень за станом контейнера чи Podʼа, і бути процесом спостереження стану.
...
Переконайтеся, що ви не плутаєте _Status_, поле відображення kubectl, призначене для інтуїтивного сприйняття користувача, з `phase`. Фаза є явною частиною моделі даних Kubernetes і документації [Pod API](/docs/reference/kubernetes-api/workload-resources/pod-v1/).
...
Починаючи з Kubernetes 1.27, kubelet переводить видалені Podʼи в термінальну фазу (`Failed` або `Succeeded` залежно від статусів завершення роботи контейнерів Podʼа) перед їх видаленням з сервера API, за винятком двох випадків:

* [статичних Podʼів](/docs/tasks/configure-pod-container/static-pod/) (які керуються безпосередньо kubelet і представлені {{< glossary_tooltip text="дзеркальними Podʼами" term_id="mirror-pod" >}})
* [примусово видалених Podʼів](/docs/concepts/workloads/pods/pod-lifecycle/#pod-termination-forced) без завершувача
...
##### Порівняння поведінки перезапуску {#restart-behavior-comparison}

Наступна таблиця показує, як контейнери поводяться при різних політиках перезапуску та кодах виходу:

| Код виходу | `restartPolicy: Always` | `restartPolicy: OnFailure` | `restartPolicy: Never` | Контейнери Sidecar |
|------------|-------------------------|----------------------------|------------------------|--------------------|
| 0 (Успіх)  | Перезапускається | Не перезапускається | Не перезапускається | Завжди перезапускається |
| Ненульовий (Помилка) | Перезапускається | Перезапускається | Не перезапускається | Завжди перезапускається |

{{< note >}}
Поведінка перезапуску особливо важлива при виборі між Deployment та Job:

* **Deployments** зазвичай використовують `restartPolicy: Always` (єдине дозволене значення) для безперервного запуску застосунків.
* **Jobs** зазвичай використовують `restartPolicy: OnFailure` або `restartPolicy: Never` для належного оброблення завдань пакетної обробки.
* **Контейнери Sidecar** є контейнерами ініціалізації, які завжди перезапускаються незалежно від `restartPolicy` Podʼа, оскільки вони мають власну політику перезапуску на рівні контейнера `restartPolicy: Always`.
{{< /note >}}

##### Приклади сценаріїв {#example-scenarios}

Ось конкретні приклади, що демонструють різну поведінку перезапуску:

**Приклад 1: Вебсервер з `restartPolicy: Always` (типово для Deployments)**
...
metadata:
  name: web-server
spec:
  restartPolicy: Always  # Контейнер перезапускається незалежно від коду виходу
  containers:
  - name: nginx
    image: nginx:1.14.2
    # Якщо цей контейнер аварійно завершить роботу або вийде з будь-якої причини, він буде перезапущений
```

**Приклад 2: Завдання пакетної обробки з `restartPolicy: OnFailure`**
...
spec:
  template:
    spec:
      restartPolicy: OnFailure  # Перезапускається тільки при ненульовому коді виходу
      containers:
      - name: processor
        image: busybox:1.28
        command: ['sh', '-c', 'echo "Processing data..."; exit 0']
        # Код виходу 0: Завдання успішно завершено, перезапуск не відбувається
        # Код виходу 1+: Контейнер перезапускається для повторної спроби завдання
```

**Приклад 3: Одноразове завдання з `restartPolicy: Never`**
...
metadata:
  name: migration-task
spec:
  restartPolicy: Never  # Ніколи не перезапускається, незалежно від коду виходу
  containers:
  - name: migrate
    image: busybox:1.28
    command: ['sh', '-c', 'echo "Running migration..."; exit 1']
    # Навіть з кодом виходу 1 (помилка), контейнер не буде перезапущений
    # Pod залишиться в стані Failed
```

##### Контейнери Sidecar та політики перезапуску {#sidecar-containers-and-restart-policies}

[Контейнери Sidecar](/docs/concepts/workloads/pods/sidecar-containers/) мають особливу поведінку перезапуску, яка відрізняється від звичайних контейнерів застосунків:

* **Контейнери Sidecar ігнорують політику `restartPolicy` на рівні Pod**: Вони використовують власне поле `restartPolicy` на рівні контейнера, яке завжди встановлено в `Always`
* **Незалежний життєвий цикл**: Контейнери Sidecar можуть перезапускатися незалежно від основного контейнера застосунку
* **Постійна робота**: Контейнери Sidecar залишаються працювати протягом усього часу життя Podʼа, надаючи допоміжні сервіси

**Приклад: Pod з контейнером Sidecar**
...
spec:
  restartPolicy: OnFailure  # Застосовується лише до основного контейнера
  initContainers:
  - name: logging-sidecar    # Це контейнер Sidecar
    image: fluent/fluent-bit:1.8
    restartPolicy: Always    # Sidecar завжди перезапускається незалежно від коду виходу
    # Надає послуги логування протягом усього часу життя Pod
  containers:
  - name: main-app          # Це слідує політиці перезапуску на рівні Pod
    image: nginx:1.14.2
    # Перезапускається лише у разі помилки (ненульовий код виходу) через політику OnFailure на рівні Pod
```

{{< note >}}
Хоча основний контейнер застосунку слідує політиці `restartPolicy: OnFailure` на рівні Podʼа, контейнер Sidecar буде перезапущений незалежно від його коду виходу, оскільки контейнери Sidecar завжди мають `restartPolicy: Always` на рівні контейнера.
{{< /note >}}
...
* `PodReadyToStartContainers`: (бета функція; типово [увімкнена](#pod-ready-to-start-containers)) Pod sandbox був успішно створений, мережа налаштована, томи зберігання змонтовані, а будь-які динамічні ресурси (якщо запитано) виділені.
* `ContainersReady`: всі контейнери в Podʼі готові.
...
### Готовність Podʼа до запуску контейнерів {#pod-ready-to-start-containers}
...
Після того, як Pod отримує призначення на вузол, йому потрібно бути допущеним до kubelet та мати встановлені всі необхідні томи зберігання. Як тільки ці фази завершаться, kubelet співпрацює з середовищем виконання контейнерів (використовуючи {{< glossary_tooltip term_id="cri" >}}), щоб налаштувати ізольоване середовище виконання та налаштувати мережу для Podʼа. Якщо Pod використовує [Динамічне виділення ресурсів](/docs/concepts/scheduling-eviction/dynamic-resource-allocation/), ці ресурси також виділяються під час цієї фази. Якщо функціональну можливість [`PodReadyToStartContainersCondition`](/docs/reference/command-line-tools-reference/feature-gates/#PodReadyToStartContainersCondition) увімкнено (є типово увімкненою для Kubernetes {{< skew currentVersion >}}), стан `PodReadyToStartContainers` буде додано до поля `status.conditions` Podʼа.
...
Після створення sandbox, налаштування мережі, монтування томів та (якщо запитано) динамічного виділення ресурсів kubelet встановлює стан `PodReadyToStartContainers` в `True`. Витягування образу та створення контейнера відбуваються після цього.
...
Якщо ви вказали ресурси на рівні Podʼа, ви також можете змінювати їх на місці. Для отримання додаткової інформації див. [Зміна розміру ресурсів процесора та памʼяті, призначених Podʼам](/docs/tasks/configure-pod-container/resize-pod-resources/).
...
* На місці можна змінювати розмір лише ресурсів процесора та памʼяті.
...
[Зміна розміру ресурсів процесора та памʼяті, призначених контейнерам](/docs/tasks/configure-pod-container/resize-container-resources/).
...
* Призводить до заміни Podʼа, тому вам слід спроєктувати робоче навантаження таким чином, щоб воно могло обробляти [заплановані порушення](/docs/concepts/workloads/pods/disruptions/). Розгляньте можливість використання [PodDisruptionBudget](/docs/tasks/run-application/configure-pdb/) для контролю доступності.
...
Kubernetes дозволяє визначати _проби_ для безперервного моніторингу стану контейнерів у Podʼі. Проба — це діагностика, яку періодично виконує {{< glossary_tooltip text="kubelet" term_id="kubelet" >}} для контейнера. Для виконання діагностики kubelet або виконує код всередині контейнера, або виконує мережевий запит.

На основі результатів проб Kubernetes може перезапускати непрацюючі контейнери або припиняти надсилання трафіку до контейнерів, які не готові.

Kubelet може опціонально виконувати та реагувати на три типи проб для робочих контейнерів, кожна з яких виконує різну функцію. Для механізмів проб (`exec`, `grpc`, `httpGet`, `tcpSocket`), полів конфігурації та детальних рекомендацій щодо використання дивіться [Проби життєздатності, готовності та запуску](/docs/concepts/workloads/pods/probes/).

### Проби запуску (Startup probe) {#startup-probe}

Проби запуску перевіряють, чи запущено застосунок всередині контейнера. Якщо проба запуску налаштована, Kubernetes не виконує проби життєздатності або готовності, поки проба запуску не буде успішною, що дозволяє застосунку завершити ініціалізацію.

Цей тип проби виконується лише під час запуску, на відміну від проб життєздатності та готовності, які виконуються періодично.

Якщо проба запуску не вдається, kubelet зупиняє контейнер, і контейнер підлягає застосуванню своєї [політиці перезапуску](/docs/concepts/workloads/pods/pod-lifecycle/#restart-policy).

### Проби життєздатності (Liveness probe) {#liveness-probe}

Проби життєздатності визначають, коли потрібно перезапустити контейнер. Наприклад, проби життєздатності можуть виявити стан "зависання", коли застосунок працює, але не може просуватися далі. Перезапуск контейнера в такому стані може допомогти зробити застосунок більш доступним, незважаючи на помилки.

Якщо контейнер не проходить пробу життєздатності кількість разів більше, ніж дозволено налаштуванням, kubelet перезапускає цей контейнер. Проби життєздатності не чекають успіху проб готовності. Якщо ви хочете почекати перед виконанням проби життєздатності, ви можете або визначити `initialDelaySeconds`, або використовувати пробу запуску.

### Проби готовності (Readiness probe) {#readiness-probe}

Проби готовності визначають, коли контейнер готовий приймати трафік. Це корисно, коли потрібно дочекатися виконання застосунком тривалих початкових завдань, таких як встановлення мережевих зʼєднань, завантаження файлів та прогрів кешів. Проби готовності також можуть бути корисними пізніше в життєвому циклі контейнера, наприклад, при відновленні після тимчасових збоїв або перевантажень.

Якщо проба готовності повертає стан "неуспіх", контролер {{< glossary_tooltip text="EndpointSlice" term_id="endpoint-slice" >}} видаляє IP-адресу Podʼа з EndpointSlices всіх Сервісів, які відповідають Podʼу.

Проби готовності виконуються в контейнері протягом усього його життєвого циклу.
