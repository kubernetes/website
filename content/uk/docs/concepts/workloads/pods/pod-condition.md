...
У Kubernetes багато обʼєктів мають _стани_. Стан — це показник певного аспекту фактичного стану обʼєкта, який представляє цей обʼєкт. Podʼи мають стани, і стани Podʼів у Kubernetes є важливим аспектом, що дозволяє контролерам (а також тим, хто займається усуненням несправностей) оцінювати справність Podʼів.
...
Після того, як Pod заплановано на вузол, його потрібно допустити за допомогою kubelet і змонтувати всі необхідні томи зберігання. Після завершення цих фаз kubelet працює з  середовищем виконання контейнерів (використовуючи {{< glossary_tooltip text="Container Runtime Interface, CRI" term_id="cri" >}}), щоб налаштувати середовище виконання та налаштувати мережу для Podʼа. Стан `PodReadyToStartContainers` додається до поля `status.conditions` Podʼа.

Стан встановлюється в `False` kubeletʼом, коли він виявляє, що Pod не має середовища виконання з налаштованою мережею. Це відбувається в наступних сценаріях:
...
Спеціальний стан Pod `DisruptionTarget` додано для того, щоб вказати, що Pod буде видалено через {{<glossary_tooltip term_id="disruption" text="розлади">}}. Поле `reason` цього стану додатково вказує одну з таких причин припинення роботи Podʼа:

`PreemptionByScheduler`
: Pod підлягає {{<glossary_tooltip term_id="preemption" text="витісненню">}} планувальником, щоб звільнити місце для нового Podʼа з вищим пріоритетом. Для отримання додаткової інформації див. [Пріоритет та витіснення Podʼів](/docs/concepts/scheduling-eviction/pod-priority-preemption/).
...
: Pod підлягає виселенню за допомогою {{<glossary_tooltip term_id="api-eviction" text="API Kubernetes">}}.
...
: Pod був завершений kubelet, {{<glossary_tooltip term_id="node-pressure-eviction" text="через тиск на вузол">}}, [належне вимикання вузла](/docs/concepts/cluster-administration/node-shutdown/#graceful-node-shutdown), або витісненням [системно критичними Podʼами](/docs/tasks/administer-cluster/guaranteed-scheduling-critical-addon-pods/).
