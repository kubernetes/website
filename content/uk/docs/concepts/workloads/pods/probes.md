...
Якщо процес у вашому контейнері може самостійно завершуватися при виникненні проблем або погіршенні стану, вам не обовʼязково потрібна проба життєздатності; kubelet автоматично виконає правильну дію відповідно до `restartPolicy` Podʼа.
...
`Failure`
: Контейнер не пройшов діагностику. Для проб життєздатності та запуску kubelet припиняє роботу контейнера, і до нього застосовується відповідна [політика перезапуску](/docs/concepts/workloads/pods/pod-lifecycle/#restart-policy). Для проб готовності kubelet позначає контейнер як неготовий, і Pod перестає отримувати трафік від відповідних сервісів.
...
- `protocol`: Протокол для використання в запиті проби. Стандартно `HTTP1`. Встановіть `HTTP2`, щоб виконувати пробу через HTTP/2 без шифрування (h2c). Вимагає увімкнення функціональної можливості [`H2CContainerProbe`](/docs/reference/command-line-tools-reference/feature-gates/#H2CContainerProbe).
...
Ви можете налаштувати HTTP-пробу для використання HTTP/2 без шифрування (h2c), встановивши поле `protocol` на `HTTP2`. Коли `protocol` встановлено на `HTTP2`, kubelet використовує h2c (HTTP/2 через звичайний TCP) замість HTTP/1.1. Типове значення `protocol` — `HTTP1`. Встановлення `protocol: HTTP2` вимагає `scheme: HTTP` (типове значення); сервер API відхиляє комбінацію `protocol: HTTP2` з `scheme: HTTPS`. Для отримання додаткових відомостей див. [Використання HTTP/2 без шифрування (h2c) з HTTP-пробами](/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#use-h2c-with-http-probes).
...
Ви можете налаштувати gRPC-пробу для підключення через TLS, встановивши поле `mode` на `TLS`. Коли `mode` встановлено на `TLS`, kubelet використовує TLS з `InsecureSkipVerify` (він не перевіряє сертифікат сервера). Типове значення `mode` — `Plaintext`. Для отримання додаткових відомостей див. [Використання TLS з gRPC-пробами](/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#grpc-probe-tls).
