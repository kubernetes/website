...
Необовʼязкове поле `spec.podGroupTemplateRef` повʼязує PodGroup з PodGroupTemplate у робочому навантаженні, з якого вона була створена. Це корисно для спостереження та інструментів.
...
### Пріоритет і режим розладу {#priority-and-disruption-mode}

Кожна PodGroup також може визначати власні `spec.priority`, `spec.preemptionPolicy` та `spec.disruptionMode`. Пріоритет і політика витіснення встановлюються за допомогою поля `spec.priorityClassName`, яке вказує на ресурс `PriorityClass`. Див. [Розлад та пріоритет Pod Group](/docs/concepts/workloads/workload-api/disruption-and-priority/) для детального опису цих полів.

Коли контролер робочого навантаження створює PodGroup, ці поля копіюються з PodGroupTemplate робочого навантаження під час створення. Для автономних PodGroup ви встановлюєте поля безпосередньо.
...
{{< glossary_tooltip text="ResourceClaims" term_id="resourceclaim" >}} повʼязані з PodGroups можуть бути спільно використані всіма Podʼами, що належать до групи. Замість того, щоб мати посилання на кожен окремий Pod у `status.reservedFor` ResourceClaim, достатньо посилання на PodGroup, і будь-яка кількість Podʼів у тій самій PodGroup може спільно використовувати ResourceClaim. ResourceClaims також можуть бути створені з {{< glossary_tooltip text="ResourceClaimTemplates" term_id="resourceclaimtemplate" >}} для кожної PodGroup, що дозволяє пристроям, виділеним для кожного створеного ResourceClaim, бути спільно використаними Podʼами в кожній PodGroup.

За докладною інформацією та більш повним прикладом звертайтесь до [документації DRA](/docs/concepts/resource-management/dynamic-resource-allocation/dra-api/#workload-resource-claims).

### Статус {#status}

Планувальник оновлює `status.conditions`, щоб повідомити, чи група була успішно запланована. Основною умовою є `PodGroupInitiallyScheduled`, яка є `True`, коли всі необхідні Podʼи були розміщені, і `False`, коли планування не вдається.

{{< note >}}
Стан `PodGroupInitiallyScheduled` відображає лише початкове рішення планувальника. Планувальник не оновлює його, якщо Podʼи пізніше зазнають невдачі або будуть виселені. Детальніше див. у розділі [Обмеження](/docs/concepts/workloads/podgroup-api/lifecycle/#limitations).
{{< /note >}}
...
## Батьківська група {#parent-group}

{{< feature-state feature_gate_name="CompositePodGroup" >}}

Коли функціональну можливість [`CompositePodGroup`](/docs/reference/command-line-tools-reference/feature-gates/#CompositePodGroup) увімкнено, `PodGroup` може діяти як кінцевий вузол у багаторівневій ієрархії груп, що складається з `PodGroups` та `CompositePodGroups`. `PodGroup` може вказати свою батьківську групу за допомогою поля `spec.parentCompositePodGroupName`. Організація робочого навантаження в ієрархію груп може використовуватися для вираження багаторівневих вимог групового планування, багаторівневих обмежень топології та спільного розподілу розладів між частинами робочого навантаження.

Для отримання додаткових відомостей про ієрархічні структури груп та багаторівневе групове планування див. огляд [CompositePodGroup API](/docs/concepts/workloads/compositepodgroup-api/).

{{< note >}}
`PodGroup`, який вказує `spec.parentCompositePodGroupName`, також повинен вказувати `spec.workloadRef`, повʼязуючи `PodGroup` з його шаблоном у `Workload`. Автономним обʼєктам `PodGroup` (створеним без `spec.workloadRef`) не дозволяється вказувати батьківський `CompositePodGroup`.
{{< /note >}}
