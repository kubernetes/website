...
Цей режим доступний як альфа-функція у VPA 1.7.0 і вимагає Kubernetes 1.33 або новішої версії з увімкненою функціональною можливістю кластера `InPlacePodVerticalScaling`, а також увімкненою функціональною можливістю `InPlace` для оновлювача VPA та контролера допуску. Він використовує функцію [зміни розміру Podʼа на місці](/docs/concepts/workloads/pods/pod-lifecycle/#pod-resize), щоб застосовувати оновлення без порушення роботи Pod.

У режимі `InPlace` VPA намагається оновити запити та обмеження ресурсів Podʼа без перезапуску або виселення Podʼа. На відміну від `InPlaceOrRecreate`, цей режим **ніколи не повертається до виселення**. Якщо оновлення на місці не може бути застосоване (наприклад, через недостатню ємність вузла), VPA відкладає оновлення та повторює спробу в наступному циклі узгодження.

В режимі `InPlace` VPA намагається оновити запити та обмеження ресурсів Podʼа без перезапуску або виселення Podʼа. На відміну від `InPlaceOrRecreate`, цей режим **ніколи не повертається до виселення**. Якщо оновлення на місці не може бути застосоване (наприклад, через недостатню ємність вузла), VPA відкладає оновлення та повторює спробу в наступному циклі узгодження.

Щоб використовувати режим `InPlace`, увімкніть функціональну можливість `InPlace` як для оновлювача VPA, так і для контролера допуску:

```shell
--feature-gates=InPlace=true
```

Потім встановіть `updateMode` на `"InPlace"` у вашій специфікації VPA:
...
**Ключова відмінність від `InPlaceOrRecreate`:** Коли зміна розміру відкладається, виконується або є неможливою, режим `InPlace` завжди чекає та повторює спробу — він ніколи не виселяє Pod, незалежно від того, як довго оновлення очікує.
