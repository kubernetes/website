...
Перед перевіркою допустимості та визначенням кінцевих значень панель управління також розглядає, чи є відсутні будь-які метрики, і скільки Podʼів є [`Ready`](/docs/concepts/workloads/pods/pod-lifecycle/#pod-conditions). Щодо метрик ресурсів на рівні подів, ігноруються всі Podʼи із заданим часом видалення (обʼєкти з часом видалення перебувають у процесі завершення роботи/видалення), а всі Podʼи, що зазнали збою, вилучаються. Щодо зовнішніх метрик та метрик обʼєктів, кількість реплік розраховується на основі кількості Podʼів у станах Running та Ready; Podʼи, що завершують роботу, але все ще перебувають у стані Ready, продовжують враховуватися в цій загальній кількості.

Якщо конкретний Pod не має метрики, його відкладено на потім; Podʼи з відсутніми метриками будуть використані для коригування кінцевої кількості масштабування.
...
## Готовність Podʼа та метрики автомасштабування {#pod-readiness-and-autoscaling-metrics}
...
1. `--horizontal-pod-autoscaler-initial-readiness-delay` (стандартно: 30 секунд)
...
- Відповідні API мають бути зареєстровані:
  ...
  - Для метрик ресурсів це `metrics.k8s.io` [API](/docs/reference/external-api/metrics.v1beta1/), яке зазвичай надає [metrics-server](https://github.com/kubernetes-sigs/metrics-server). Це може бути запущено як надбудова кластера.

    {{< note >}}
    HorizontalPodAutoscaler наразі підтримує API `metrics.k8s.io/v1beta1` для метрик ресурсів. Він ще не підтримує `metrics.k8s.io/v1`.
    {{< /note >}}
...
Політики масштабування також дозволяють контролювати швидкість зміни реплік під час масштабування. Також можна використати два параметри для запобігання [коливанню](#flapping): ви можете вказати _вікно стабілізації_ для чіткішого підрахунку кількості реплік та допуск для ігнорування незначних коливань метрик нижче вказаного порогового значення.
...
Поле `tolerance` налаштовує порогове значення для мінливості метрики, запобігаючи масштабуванню нижче вказаного значення.
...
behavior:
  scaleUp:
    tolerance: 0.05 # 5% допуск для масштабування
```
...
### Приклад: обмеження коефіцієнту зменшення масштабу {#example-limiting-scale-down-rate}

Щоб обмежити коефіцієнт, з яким Podʼи видаляються HPA, до 10% за хвилину, до HPA додається наступна поведінка:
...
Крім того, є спеціальна команда `kubectl autoscale` для створення обʼєкта HorizontalPodAutoscaler. Наприклад, виконання `kubectl autoscale rs foo --min=2 --max=5 --cpu=80%` створить автомасштабувальник для ReplicaSet _foo_, з цільовим використанням процесора, встановленим на `80%` і кількістю реплік від 2 до 5.

## Масштабування до нуля та з нуля {#scaling-to-and-from-zero}

{{< feature-state feature_gate_name="HPAScaleToZero" >}}

Для HorizontalPodAutoscaler, які масштабуються на основі [власних](#scaling-on-custom-metrics) (обʼєктних) або зовнішніх метрик, ви можете встановити `spec.minReplicas` у `0`. Тоді робоче навантаження масштабується повністю до нуля реплік, коли немає навантаження, і масштабується назад, коли метрика вказує, що потрібна принаймні одна репліка. Це корисно для робочих навантажень, які простоюють протягом тривалого часу і дорогі в утриманні, наприклад, споживачі черги, якою користуються час від часу, або завдання, що потребують спеціального обладнання, як-от GPU.

Масштабування до нуля підтримується лише для обʼєктних та зовнішніх метрик. Воно недоступне для метрик ресурсів (таких як використання CPU або памʼяті), оскільки їх можна виміряти лише на працюючих Podʼах. Встановлення `minReplicas: 0` вимагає наявності принаймні однієї обʼєктної або зовнішньої метрики; інакше сервер API відхиляє HorizontalPodAutoscaler.

Ця поведінка контролюється функціональною можливістю [`HPAScaleToZero`](/docs/reference/command-line-tools-reference/feature-gates/#HPAScaleToZero), яка стандартно увімкнена. Функціональна можливість має бути увімкнена як на {{< glossary_tooltip text="kube-apiserver" term_id="kube-apiserver" >}} (який дозволяє `minReplicas: 0`), так і на {{< glossary_tooltip text="kube-controller-manager" term_id="kube-controller-manager" >}} (який виконує масштабування).

Поки HPA утримує робоче навантаження на нулі реплік, він записує стан `ScaledToZero` зі значенням `True` у статус HorizontalPodAutoscaler. HPA використовує цей стан, щоб відрізнити робоче навантаження, яке він масштабував до нуля (і масштабує назад, коли метрика повертається), від того, яке було [неявно деактивовано](#implicit-maintenance-mode-deactivation) шляхом встановлення кількості його реплік у `0`. Після того, як робоче навантаження масштабовано назад, стан встановлюється у `False` з причиною `NotScaledToZero`.
