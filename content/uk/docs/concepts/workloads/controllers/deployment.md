...
feature:
  ...
  description: >
    Kubernetes поступово впроваджує зміни до вашого застосунку або його конфігурації, одночасно відстежуючи його стан, щоб переконатися, що він не припиняє роботу всіх робочих екземплярів застосунку одночасно. Якщо щось піде не так, Kubernetes відкотить зміни за вас. Скористайтесь перевагами зростаючої екосистеми рішень для розгортання.
description: >-
...
2. Виконайте `kubectl get deployments` для перевірки створення Deployment.
   ...
   * `AGE` — показує час впродовж якого застосунок працює.
   ...
   Зверніть увагу, що бажана кількість реплік дорівнює 3 відповідно до поля `.spec.replicas`.
...
Зазвичай не рекомендується вносити оновлення до селектора міток, і рекомендується планувати ваші селектори наперед.

Селектор міток Deployment є **незмінним** після створення; його не можна оновити за допомогою `kubectl patch`, `kubectl edit`, `kubectl apply` або інструментів, таких як `helm upgrade`.

Якщо вам потрібно змінити селектор, вам доведеться видалити Deployment і створити його заново. За звичай видалення Deployment також видаляє його запущені Podʼи, що призводить до простою; використовуйте `--cascade=orphan`, якщо вам потрібно, щоб ці Podʼи продовжували працювати під час повторного створення Deployment (див. наслідки нижче). Будьте дуже обережні та переконайтеся, що ви розумієте наступні наслідки:

* **Додавання:** Коли ви створюєте новий Deployment з більш вузьким селектором, новий Deployment **повинен** також мати відповідний шаблон Podʼа. Якщо у вас є наявний маніфест і ви редагуєте його, щоб звузити селектор, вам потрібно відредагувати метадані шаблону Podʼа всередині цього Deployment, додавши нові мітки для відповідності, інакше сервер API поверне помилку валідації. Це є _неперекриваючою_ зміною: новий Deployment не "побачить" старі Podʼи (які не мають нової мітки), що призведе до того, що старий **ReplicaSet** стане сирітським і буде створено абсолютно новий **ReplicaSet**.
* **Оновлення значень:** Зміна поточного значення в ключі селектора (наприклад, з `v1` на `v2`) призводить до тієї ж поведінки, що й додавання (сирітство та повторне створення).
* **Видалення:** Видалення поточного ключа з селектора Deployment не вимагає жодних змін у мітках шаблону Podʼа. Це є _перекриваючою_ зміною: новий, ширший селектор відповідатиме старим Podʼам. Наявні ReplicaSet не стають сирітськими, і новий ReplicaSet не створюється, але зверніть увагу, що видалена мітка все ще існує в будь-яких наявних Podʼах та ReplicaSet. Ви можете очистити це, запустивши розгортання для Deployment.
...
* Отримання опису розгортання:
  ...
  Для виправлення цього вам потрібно відкотитися до попередньої ревізії Deployment, яка є стабільною.
...
Очищення починається лише **після** того, як Deployment досягне [завершеного стану](/docs/concepts/workloads/controllers/deployment/#complete-deployment). Якщо ви встановите `.spec.revisionHistoryLimit` у 0, будь-яке розгортання все одно спричинить створення нового ReplicaSet до того, як Kubernetes видалить старий.

Навіть з ненульовим лімітом історії ревізій ви можете мати більше ReplicaSets, ніж ви налаштували. Наприклад, якщо у podʼах відбувається аварійне завершення, і з часом запускається кілька подій оновлення, ви можете мати більше ReplicaSets, ніж встановлено `.spec.revisionHistoryLimit`, оскільки Deployment ніколи не досягає завершеного стану.

## Canary-розгортання {#canary-deployments}

Якщо ви хочете впроваджувати релізи для підмножини користувачів чи серверів, використовуючи Deployment, ви можете створити кілька Deployment, один для кожного релізу, дотримуючись шаблону Canary, описаному в [управлінні ресурсами](/docs/concepts/workloads/management/#canary-deployments).

Щоб дізнатися, що це передбачає, пройдіть навчальний посібник [Розгортання релізу за допомогою canary-розгортання](/docs/tutorials/stateless-application/canary-deployment/).
...
* Прочитайте {{< api-reference page="apps/deployment-v1" >}}, щоб зрозуміти API Deployment.
