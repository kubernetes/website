...
{{< note >}}
Ви можете налаштувати ліміт затримки для кожного індексу для [Індексованих](#completion-mode) Завдань, якщо у вас увімкнено [функціональну можливість](/docs/reference/command-line-tools-reference/feature-gates/) `JobBackoffLimitPerIndex` в вашому кластері.
{{< /note >}}
...
Один приклад такого підходу — це Завдання, яке створює Podʼи, які виконують сценарій, який з свого боку запускає контролер Spark master (див. [приклад Spark](https://github.com/kubernetes/examples/tree/master/staging/spark/README.md)), запускає драйвер Spark, а потім робить очищення.
