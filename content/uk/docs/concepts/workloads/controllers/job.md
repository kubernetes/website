...
Коли функціональну можливість [`WorkloadWithJob`](/docs/reference/command-line-tools-reference/feature-gates/#WorkloadWithJob) увімкнено, контролер Job автоматично створює обʼєкти [Workload](/docs/concepts/workloads/workload-api/) та [PodGroup](/docs/reference/kubernetes-api/workload-resources/workload-v1alpha1/) для [кваліфікованих паралельних Job](#qualifying-criteria) перед створенням будь-яких Podʼів. Це дозволяє нативне [групове планування](/docs/concepts/scheduling-eviction/gang-scheduling/), де всі Podʼи у Job плануються разом або жоден не планується.

### Критерії кваліфікації {#qualifying-criteria}

Контролер Job створює Workload з [політикою групового планування](/docs/concepts/workloads/workload-api/policies/#gang-policy), коли Job відповідає всім наступним умовам:

- `.spec.parallelism` більше 1
- `.spec.completionMode` дорівнює `Indexed`
- `.spec.parallelism` дорівнює `.spec.completions`
- `.spec.template.spec.schedulingGroup` не встановлено

Jobs, які не відповідають цим критеріям, продовжують планувати Podʼи незалежно, без створення `Workload` або `PodGroup`.

Наприклад, наступний Job запускає 8 паралельних індексованих виконавців. Коли функціональна можливість увімкнена, контролер Job створює `Workload` та `PodGroup` з `minCount: 8` перед створенням будь-яких Podʼів, забезпечуючи, що всі 8 виконавців плануються разом:
...
Коли контролер Job обробляє цей Job, він автоматично:

1. Створює обʼєкт [Workload](/docs/concepts/workloads/workload-api/) у тому ж самому просторі імен. Workload містить `podGroupTemplate` з [політикою групового планування](/docs/concepts/workloads/workload-api/policies/#gang-policy), де `minCount` дорівнює паралелізму Job.
1. Створює обʼєкт [PodGroup](/docs/reference/kubernetes-api/workload-resources/workload-v1alpha1/) на основі цього шаблону. PodGroup є автономною одиницею планування під час виконання, яка містить вбудовану копію політики групового планування.
...
Виявлення цих обʼєктів базується на посиланнях у специфікації (`controllerRef` та `podGroupTemplateRef`).

Workload та PodGroup належать Job (через `ownerReferences`) і автоматично видаляються при видаленні Job.

### Відмова від використання для контролерів вищого рівня {#opt-out-for-higher-level-controllers}

Якщо у шаблоні Pod завдання (Job) вже задано `spec.schedulingGroup`, контролер завдання (Job) не створює обʼєкти `Workload` або `PodGroup`. Це дозволяє контролерам вищого рівня, таким як `JobSet`, самостійно керувати життєвим циклом обʼєктів `Workload` та `PodGroup`.

### Поведінка CronJob {#cronjob-behavior}

Jobs, створені за допомогою `CronJob`, не мають `schedulingGroup`, встановленого в `PodTemplate`. Якщо Job, створений CronJob, відповідає критеріям групового планування, контролер Job створює окремі обʼєкти `Workload` та `PodGroup` для кожного екземпляра Job.

### Обмеження для альфа-релізу {#workload-integration-limitations}

- Кожен Job відповідає точно одному обʼєкту `PodGroup`. Всі Podʼи в Job належать до однієї групи планування.
- Значення `minCount` у політиці групового планування є незмінним. Оновлення `.spec.parallelism` відхиляються для Job, які використовують групове планування. Детальніше див. [Еластичні Індексовані Завдання](#elastic-indexed-jobs).
...
{{< note >}}
Коли функціональна можливість [`WorkloadWithJob`](/docs/reference/command-line-tools-reference/feature-gates/#WorkloadWithJob) увімкнена і Завдання відповідає критеріям [групового планування](#integrate-with-workload-apis), оновлення `.spec.parallelism` відхиляються, оскільки поле `minCount` у `Workload` є незмінним. Щоб масштабувати завдання з груповим плануванням, видаліть його та створіть заново з новим значенням паралелізму.
{{< /note >}}
