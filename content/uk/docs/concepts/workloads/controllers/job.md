...
Якщо у шаблоні Pod завдання (Job) вже задано `spec.schedulingGroup`, контролер завдання (Job) не створює об’єкти `Workload` або `PodGroup`. Це дозволяє контролерам вищого рівня, таким як `JobSet`, самостійно керувати життєвим циклом об’єктів `Workload` та `PodGroup`.

### Поведінка CronJob {#cronjob-behavior}

Jobs, створені за допомогою `CronJob`, не мають `schedulingGroup`, встановленого в `PodTemplate`. Якщо Job, створений CronJob, відповідає критеріям групового планування, контролер Job створює окремі об’єкти `Workload` та `PodGroup` для кожного екземпляра Job.
