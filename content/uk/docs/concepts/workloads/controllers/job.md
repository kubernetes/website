...
Якщо ви хочете запустити завдання (або одне завдання, або кілька паралельно) за розкладом, див. [CronJob](/docs/concepts/workloads/controllers/cron-jobs/).
...
1. Паралельні Jobs із *фіксованою кількістю завершень*
   ...
   - Job являє собою загальне завдання і є завершеним, коли кількість успішних Podʼів відповідає критерію `.spec.completions`.
...
- `NonIndexed` (стандартно): Завдання вважається завершеним, коли є кількість успішно завершених Podʼів вказана в `.spec.completions`. Іншими словами, завершення кожного Podʼа гомологічне одне одному. Зверніть увагу, що Завдання, у яких `.spec.completions` дорівнює null, неявно є `NonIndexed`.
...
## Інтеграція з Workload API {#integrate-with-workload-apis}

{{< feature-state feature_gate_name="WorkloadWithJob" >}}

Коли функціональну можливість [`WorkloadWithJob`](/docs/reference/command-line-tools-reference/feature-gates/#WorkloadWithJob) увімкнено, контролер Job компілює конфігурацію `.spec.scheduling` Job в обʼєкти [Workload](/docs/concepts/workloads/workload-api/) та [PodGroup](/docs/concepts/workloads/podgroup-api/) (`scheduling.k8s.io/v1beta1`) перед створенням будь-яких Podʼів. Це дозволяє вам виражати явний намір планування для Job, такий як [групове планування](/docs/concepts/scheduling-eviction/gang-scheduling/) (всі Podʼи плануються разом або жодного), спільне розташування за топологією та поведінку при розладах.

Коли ви опускаєте `.spec.scheduling`, Job зазвичай використовує політику планування `Basic`, яка зберігає стандартний результат планування Pod за Podʼом для звичайного Job. Контролер все одно створює `Workload` та `PodGroup` для кожного відповідного Job (включно з `Basic`), тому спостережувані обʼєкти є узгодженими незалежно від політики.

### Конфігурація планування {#scheduling-configuration}

Поле `.spec.scheduling` приймає наступні поля:

- `schedulingPolicy`: потрібно вказати рівно одне з `basic` або `gang`. З `gang` всі Podʼи мають бути запланованими разом, перш ніж будь-який з них буде привʼязаний. Опущене `gang.minCount` зазвичай дорівнює `.spec.parallelism` Job. Див. [Політики планування PodGroup](/docs/concepts/workloads/workload-api/policies/).
- `schedulingConstraints`: обмеження [спільного розташування за топологією](/docs/concepts/workloads/workload-api/topology-aware-scheduling/) для Podʼів Job.
- `disruptionMode`: чи Podʼи отримують розлади індивідуально (`single`) чи як група (`all`). Див. [розлади та пріоритет](/docs/concepts/workloads/workload-api/disruption-and-priority/).
- `resourceClaims`: динамічні запити на ресурси, спільні для Podʼів Job.

Всі поля `.spec.scheduling` є незмінними після створення Job, окрім `schedulingPolicy.gang.minCount`, яке ви можете змінювати, щоб [еластично масштабувати групу](#elastic-indexed-jobs).

### Групове планування {#gang-scheduling}

Наступний Job запитує групове планування для своїх 8 Podʼів, розташованих в одній зоні та розлаштованих разом:
...
spec:
  ...
  scheduling:
    schedulingPolicy:
      gang: {}                # minCount пропущено -> стандартно дорівнює parallelism (8)
    schedulingConstraints:
      key:
      - level: topology.kubernetes.io/zone
    disruptionMode:
      all: {}                 # вся група отримує розлади разом
  template:
    spec:
      restartPolicy: Never
      containers:
...
Коли контролер Job обробляє цей Job, він:

1. Створює обʼєкт [Workload](/docs/concepts/workloads/workload-api/) у тому ж самому просторі імен, що містить `podGroupTemplate`, скомпільований з `.spec.scheduling` (тут — [політика групового планування](/docs/concepts/workloads/workload-api/policies/) з `minCount: 8`).
1. Створює обʼєкт PodGroup на основі цього шаблону. PodGroup є одиницею планування під час виконання і містить вбудовану копію політики.
1. Створює Podʼи з `.spec.schedulingGroup.podGroupName`, встановленим на імʼя PodGroup, звʼязуючи кожен Pod зі своєю групою планування.

Контролер виявляє ці обʼєкти через посилання у специфікації (`Workload.spec.controllerRef` та `PodGroup.spec.workloadRef`), а не за іменем. Обʼєкти, які створює контролер, містять запис `ownerReferences`, що вказує на Job, тому вони видаляються збирачем сміття, коли Job видаляється.

### Типове (Basic) планування {#default-basic-scheduling}

Job, який опускає `.spec.scheduling`, типово використовує `Basic`, що діє як неявна відмова від групового планування. Його Podʼи плануються так само, як у звичайного Job, при цьому все одно створюються `Workload` та `PodGroup` типу `Basic`:
...
spec:
  parallelism: 10
  completions: 10
  # .spec.scheduling пропущено -> типово використовується Basic scheduling.
  template:
    spec:
      restartPolicy: Never
      containers:
...
### Контролери вищого рівня {#higher-level-controllers}

Для автономного Job контролер Job володіє як `Workload`, так і `PodGroup`. Коли Job натомість містить `ownerReference` на батьківський контролер, який компілює `Workload` (наприклад, `JobSet`), контролер Job делегує володіння `Workload` цьому батьківському контролеру. Чи створює контролер Job все ще `PodGroup` під час виконання, залежить від того, що делегує батьківський контролер:

- Якщо батьківський контролер встановлює анотацію `scheduling.k8s.io/group-template-name` на Job, контролер Job створює та володіє `PodGroup`, зіставленим з іменованим `PodGroupTemplate` батьківського контролера.
- В іншому випадку батьківський контролер володіє обома обʼєктами, і контролер Job не створює жодного; він виявляє наявні обʼєкти та використовує їх при створенні Podʼів.

Якщо у шаблоні Pod завдання (Job) вже задано `.spec.template.spec.schedulingGroup`, контролер Job не створює жодного обʼєкта, дозволяючи вам (або контролеру вищого рівня) самостійно керувати життєвим циклом `Workload`/`PodGroup`.

### Поведінка CronJob {#cronjob-behavior}

Jobs, створені за допомогою `CronJob`, є автономними; `CronJob` не створює та не керує обʼєктами `Workload`. Якщо `jobTemplate` CronJob встановлює `.spec.scheduling`, контролер Job створює окремі обʼєкти `Workload` та `PodGroup` для кожного екземпляра Job, скомпільовані з `.spec.scheduling` цього Job (типово `Basic`, якщо опущено). Ці обʼєкти видаляються збирачем сміття, коли кожен Job завершується або видаляється.

### Обмеження для альфа-релізу {#workload-integration-limitations}

- Кожен Job відповідає точно одному обʼєкту `PodGroup`; всі Podʼи в Job поділяють єдину політику планування.
- Лише `schedulingPolicy.gang.minCount` є змінним; всі інші поля `.spec.scheduling` є незмінними після створення.
- Призупинені Job зберігають свої обʼєкти `Workload` та `PodGroup`; вони не видаляються при призупиненні і не створюються заново при відновленні.
...
Додатково ви можете використовувати ліміт затримки для кожного індексу разом з [політикою збоїв Podʼа](#pod-failure-policy). Коли використовується ліміт затримки для кожного індексу, доступна нова дія `FailIndex`, яка дозволяє вам уникати непотрібних повторів всередині індексу.
...
{{< note >}}
Якщо Завдання виявилося невдалим, будь-то через політику збою Podʼа, чи через політику затримки збою Podʼа, і завдання виконується кількома Podʼами, Kubernetes завершує всі Podʼи в цьому Завданні, які все ще перебувають у статусах Pending або Running.
{{< /note >}}
...
- ви можете вказати дію, яка виконується, коли політика відмов Podʼа отримує збіг, у `spec.podFailurePolicy.rules[*].action`. Можливі значення:
  ...
  - `Ignore`: використовуйте це, щоб вказати, що значення лічильника, що рахує `.spec.backoffLimit`, не повинне збільшуватися, і має бути створений Pod-заміна.
...
{{< note >}}
Коли використовується `podFailurePolicy`, контролер завдань враховує лише Podʼи у фазі `Failed`. Podʼи з відміткою про видалення, які не знаходяться в термінальній фазі (`Failed` чи `Succeeded`), вважаються такими, що примусово припиняють свою роботу. Це означає, що такі Podʼи зберігають [завершувачі відстеження](#job-tracking-with-finalizers) доки не досягнуть термінальної фази. З Kubernetes 1.27 Kubelet переводить видалені Podʼи в термінальну фазу (див.: [Фаза Podʼа](/docs/concepts/workloads/pods/pod-lifecycle/#pod-phase)). Це забезпечує видалення завершувачів контролером Job.
{{< /note >}}
...
Наприклад, ви можете захотіти вирішити, коли створити замінний Job, що замінює невдалий. Якщо ви заміните невдале Завдання, коли зʼявляється умова `FailureTarget`, ваше замінне Завдання запуститься раніше, але це може призвести до того, що Podʼи з невдалого та замінного Завдання будуть працювати одночасно, використовуючи додаткові обчислювальні ресурси.

Альтернативно, якщо ваш кластер має обмежену ресурсну ємність, ви можете вибрати чекати, поки зʼявиться умова `Failed` на завданні, що затримає ваше замінне Завдання, але забезпечить збереження ресурсів, чекаючи, поки всі невдалі Podʼи не будуть видалені.

## Автоматичне очищення завершених завдань {#clean-up-finished-jobs-automatically}

Зазвичай завершені Завдання вже не потрібні в системі. Зберігання їх в системі може створювати тиск на сервер API. Якщо Завдання керується безпосередньо контролером вищого рівня, таким як [CronJobs](/docs/concepts/workloads/controllers/cron-jobs/), Завдання можна очищати за допомогою CronJobs на основі визначеної політики очищення з урахуванням місткості.
...
- Кілька підходів використовують чергу робіт. Це вимагає запуску служби черги та модифікацій існуючої програми чи контейнера, щоб зробити його сумісним із чергою робіт. Інші підходи легше адаптувати до наявного контейнеризованого застосунку.
...
Коли створюється Job, контролер Job негайно починає створювати Podʼи, щоб відповідати вимогам Job і продовжує це робити, доки Job не завершиться. Однак іноді ви можете хотіти тимчасово призупинити виконання Job та відновити його пізніше, або створити Завдання у призупиненому стані та мати власний контролер, який вирішить, коли їх запустити.
...
У більшості випадків, паралельні завдання вимагатимуть, щоб їхні Podʼи запускались з певними обмеженнями, типу "всі в одній зоні" або "всі на GPU моделі x або y", але не комбінація обох.

[Поле призупинення](#suspending-a-job) є першим кроком для досягнення цих семантик. Призупинення дозволяє власному контролеру черги вирішити, коли повинне початися завдання; Однак після того, як завдання перестає бути призупиненим, власний контролер черги не впливає на те, де насправді буде розташований Pod завдання.
...
#### Змінні директиви планування для призупинених Завдань {#mutable-scheduling-directives-for-suspended-jobs}

{{< feature-state feature_gate_name="MutableSchedulingDirectivesForSuspendedJobs" >}}

У Kubernetes 1.34 та раніших версіях змінювати директиви планування Podʼа дозволяється лише для призупинених Завдань, які ніколи не відновлювалися раніше. Починаючи з Kubernetes 1.35, це дозволяється для будь-яких призупинених Завдань, коли увімкнено функціональну можливість `MutableSchedulingDirectivesForSuspendedJobs`.

Крім того, ця функціональна можливість дозволяє очищати поле `.status.startTime` при [призупиненні Завдання](#suspending-a-job).
...
Саме нове Завдання матиме інший uid від `a8f3d00d-c6d2-11e5-9f87-42010af00002`. Встановлення `manualSelector: true` говорить системі, що ви розумієте, що робите, і дозволяє цю невідповідність.
...
{{< note >}}
Коли функціональна можливість [`WorkloadWithJob`](/docs/reference/command-line-tools-reference/feature-gates/#WorkloadWithJob) увімкнена і Job використовує [групове планування](#integrate-with-workload-apis), розмір групи слідує за змінним `schedulingPolicy.gang.minCount` (або `.spec.parallelism`, коли `minCount` не встановлено). Ви можете масштабувати групу одним із двох способів:

- встановити `.spec.scheduling.schedulingPolicy.gang.minCount` безпосередньо, або
- змінити `.spec.parallelism`, коли `minCount` не встановлено.

При будь-якій зміні контролер перекомпілює `Workload` та повторно синхронізує розмір `PodGroup`, масштабуючи групу на місці без перестворення Job. Оновлення застосовуються лише до Podʼів, які оцінюються в майбутніх циклах планування, і не впливають на вже заплановані Podʼи. `gang.minCount`, більше за `.spec.parallelism`, відхиляється, оскільки така група ніколи не може бути задоволена.
{{< /note >}}
...
При увімкненій функціональній можливості у вашому кластері ви можете перевірити поле `.status.terminating` Завдання. Значення цього поля — це кількість Podʼів, якими володіє Завдання, які зараз перебувають у стані завершення.
...
- `Job` є частиною REST API Kubernetes. Ознайомтесь з визначенням обʼєкта {{< api-reference page="batch/job-v1" >}}, щоб зрозуміти API для завдань.
...
- Попрактикуйтесь в налаштуванні обробки відновлюваних і невідновлюваних збоїв Podʼів за допомогою `podFailurePolicy` на основі покрокових [прикладів](/docs/tasks/job/pod-failure-policy/).
- Дізнайтеся про [групове планування](/docs/concepts/scheduling-eviction/gang-scheduling/)
  для планування паралельних Завдань за принципом "все або нічого".
