...
* `Allow` (стандартно): CronJob дозволяє паралельні запуски завдань
...
CronJob відповідає лише за створення Job, які відповідають його розкладу, а Job, зі свого боку, відповідає за управління Podʼами, які він представляє.
...
* `CronJob` є частиною Kubernetes REST API. Дивіться довідку {{< api-reference page="batch/cron-job-v1" >}} API для отримання докладних деталей.
