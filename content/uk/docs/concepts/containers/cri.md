...
Стандартні RPC-запити списку CRI (`ListContainers`, `ListPodSandbox`, `ListImages`) повертають усі результати в єдиній унітарній відповіді. На вузлах із великою кількістю контейнерів (наприклад, понад приблизно 10 000, включаючи як ті, що працюють, так і зупинені), ці відповіді можуть перевищувати стандартне обмеження розміру повідомлення gRPC у 16 Мбайт, що призводить до збою kubelet під час узгодження стану з середовищем виконання контейнера.

З увімкненою функціональною можливістю `CRIListStreaming` kubelet використовує серверні потокові RPC (такі як `StreamContainers`, `StreamPodSandboxes`, `StreamImages`), які дозволяють середовищу виконання контейнера розподіляти результати по кількох повідомленнях відповіді, обходячи обмеження розміру повідомлення. Це особливо корисно для:

- Середовищ із високим обігом контейнерів (системи CI/CD)
- Великомасштабних пакетних обчислювальних навантажень

Якщо середовище виконання контейнера не підтримує потокові RPC, kubelet автоматично повертається до стандартних унітарних RPC для забезпечення зворотної сумісності.
