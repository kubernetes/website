...
Контейнерам доступні два хуки:

`PostStart`

Цей хук виконується відразу після створення контейнера. Він працює **одночасно** з `ENTRYPOINT` контейнера (головним процесом), що означає, що хук може виконуватися до, під час або після запуску головного процесу.

До обробника не передаються жодні параметри.

{{< note >}}
Хоча хук працює одночасно з процесом контейнера,
він може затримувати оновлення статусу контейнера; контейнер може не переходити в стан `Running` до завершення роботи хука.
{{< /note >}}
