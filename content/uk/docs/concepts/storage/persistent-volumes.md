...
Дивіться [докладний огляд із робочими прикладами](/docs/tutorials/configuration/configure-persistent-volume-storage).
...
Мета функції захисту обʼєктів зберігання — забезпечення того, що PersistentVolumeClaims (PVC), які активно використовуються Podʼом, та PersistentVolume (PV), які привʼязані до PVC, не видаляються з системи, оскільки це може призвести до втрати даних.
...
Для втулків томів, які підтримують політику повторного використання `Delete`, ця політика видаляє як обʼєкт PersistentVolume з Kubernetes, так і повʼязаний ресурс зовнішньої інфраструктури. Томи, які були динамічно виділені, успадковують [політику повторного використання з їх StorageClass](#reclaim-policy), яка стандартно встановлена в `Delete`. Адміністратор повинен налаштувати StorageClass відповідно до очікувань користувачів; в іншому випадку PV повинен бути відредагований або виправлений (patch) після створення. Див. [Змінення політики повторного використання PersistentVolume](/docs/tasks/administer-cluster/change-pv-reclaim-policy/).
...
Якщо розширення базового сховища не вдається, адміністратор кластера може вручну відновити стан Persistent Volume Claim (PVC) та скасувати запити на зміну розміру. Інакше запити на зміну розміру будуть продовжені автоматично контролером без втручання адміністратора.
...
5. Не забудьте відновити політику повторного використання PV.
...
Кожен PersistentVolume (PV) містить специфікацію та статус, які є характеристиками та стан тому. Імʼя обʼєкта PersistentVolume повинно бути дійсним [DNS імʼям субдомену](/docs/concepts/overview/working-with-objects/names#dns-subdomain-names).
...
: {{< feature-state for_k8s_version="v1.29" state="stable" >}}
  том може бути підключений для читання-запису одним Podʼом. Використовуйте режим доступу ReadWriteOncePod, якщо ви хочете забезпечити, що тільки один Pod на весь кластер може читати цей PVC або писати в нього.
...
PVC не обовʼязково повинен вимагати клас. PVC зі своїм `storageClassName`, встановленим рівним `""`, завжди інтерпретується як PVC, який вимагає PV без класу, тобто він може бути привʼязаний лише до PV без класу (без анотації або з анотацією, встановленою рівною `""`). PVC без `storageClassName` не зовсім те ж саме і відзначається по-іншому кластером, залежно від того, чи включений [втулок допуску `DefaultStorageClass`](/docs/reference/access-authn-authz/admission-controllers/#defaultstorageclass).

* Якщо втулок допуску увімкнено, адміністратор може вказати типовий StorageClass. Усі PVC, у яких немає `storageClassName`, можуть бути привʼязані лише до PV з цим типовим StorageClass. Вказання типового StorageClass виконується, встановленням анотації `storageclass.kubernetes.io/is-default-class` рівною `true` в обʼєкті StorageClass. Якщо адміністратор не вказав типовий StorageClass, кластер відповідає на створення PVC так, ніби втулок допуску був вимкнений. Якщо вказано більше одного типового StorageClass, для привʼязки PVC використовується остання версія типового StorageClass, коли PVC динамічно виділяється.
* Якщо втулок допуску вимкнено, немає поняття типового StorageClass. Усі PVC, у яких `storageClassName` встановлено рівно `""`, можуть бути привʼязані лише до PV, у яких `storageClassName` також встановлено рівно `""`. Однак PVC з відсутнім `storageClassName` можна буде оновити пізніше, коли стане доступним типовий StorageClass. Якщо PVC оновлюється, він більше не буде привʼязуватися до PV з `storageClassName`, також встановленим рівно `""`.
...
### Відстеження невикористаних PVC {#unused-pvc-tracking}

{{< feature-state feature_gate_name="PersistentVolumeClaimUnusedSinceTime" >}}

Коли увімкнено, контролер захисту PVC додає [стан](/docs/concepts/workloads/pods/pod-lifecycle/#pod-conditions) `Unused` до кожного PersistentVolumeClaim, щоб вказати, чи він наразі використовується будь-яким Podʼом, ще не перебуває в стані завершення роботи.

Стан має два значення:

`Unused` зі статусом `"True"` (причина `NoPodsUsingPVC`)
: Жоден не завершений Pod не посилається на цей PVC. `lastTransitionTime` фіксує час, коли PVC став невикористовуваним.

`Unused` зі статусом `"False"` (причина `PodUsingPVC`)
: Принаймні один не завершений Pod наразі посилається на цей PVC. `lastTransitionTime` фіксує час, коли PVC почав використовуватися.

Pod вважається не завершеним, якщо його фаза не є `Succeeded` або `Failed`. Це означає, що Pending Pod (навіть той, який ще не був запланований) враховується як такий, що використовує PVC.

`lastTransitionTime` стану `Unused` може використовуватися адміністраторами кластера, інструментами моніторингу та зовнішніми контролерами для визначення PVC, які не використовувалися протягом тривалого часу. Наприклад, щоб знайти всі PVC, які не використовувалися більше 30 днів, можна виконати запит для PVC, де стан `Unused` має `status: "True"` і `lastTransitionTime` старший за 30 днів.

{{< note >}}
Тривалість невикористання, вказана цим станом, може бути коротшою за фактичний час невикористання через затримки обробки в контролері або через те, що функція була увімкнена після того, як PVC вже не використовувався. Стан не оновлюється, коли у PVC встановлено `deletionTimestamp` (тобто PVC, які видаляються).
{{< /note >}}
...
PersistentVolume типу `hostPath` використовує файл або теку на вузлі для емуляції мережевого сховища. Див. [приклад тому типу `hostPath`](/docs/tutorials/configuration/configure-persistent-volume-storage/#create-a-persistentvolume).
...
втулки томів можна прочитати в [ЧаПи втулків томів](https://github.com/kubernetes/community/blob/main/sig-storage/volume-plugin-faq.md).
...
[Клонування томів](#volume-cloning) та [відновлення зі знімків](#volume-snapshot-and-restore-volume-from-snapshot-support) попередньо наповнюють новий том з вбудованого _джерела даних_. _Наповнювачі томів_ розширюють цей механізм так, що PersistentVolumeClaim може бути попередньо наповнений з інших видів джерел (користувацький ресурс), на які посилаються через поле `dataSourceRef`:
...
Докладніше, включаючи джерела даних зі змішаними просторами імен, див. [Наповнювачі томів та джерела даних](/docs/concepts/storage/volume-populators-and-data-sources/).

## Створення переносимої конфігурації {#writing-portable-configuration}

Якщо ви створюєте шаблони конфігурації або приклади, які повинні працювати на широкому спектрі кластерів і вам потрібне постійне сховище, рекомендується використовувати такий підхід:

* Включайте обʼєкти PersistentVolumeClaim у ваш набір конфігурації (нарізі з Deployment, ConfigMap і т. д.).
* Не включайте обʼєкти PersistentVolume в конфігурацію, оскільки користувач, який створює конфігурацію, може не мати прав на створення PersistentVolumes.
...
* Дізнайтеся більше про [створення PersistentVolume](/docs/tutorials/configuration/configure-persistent-volume-storage/#create-a-persistentvolume).
* Дізнайтеся більше про [створення PersistentVolumeClaim](/docs/tutorials/configuration/configure-persistent-volume-storage/#create-a-persistentvolumeclaim).
