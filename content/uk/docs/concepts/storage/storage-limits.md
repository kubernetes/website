...
[`VolumeLimitScaling`](/docs/reference/command-line-tools-reference/feature-gates#VolumeLimitScaling) увімкнена стандартно в Kubernetes v1.37.

Однак запобігання розміщенню подів на вузлах без драйвера CSI вимагає явного погодження через поле `spec.preventPodSchedulingIfMissing` обʼєкта `CSIDriver`.

Поле `preventPodSchedulingIfMissing` стандартно має значення `false`, і його потрібно встановити в `true`, якщо ви не хочете, щоб поди планувалися на вузлах без драйвера CSI. Рішення встановити стандартне значення `false` було прийнято з міркувань зворотної сумісності та сумісності з [Cluster AutoScaler](https://github.com/kubernetes/autoscaler/tree/master/cluster-autoscaler), який може не знати про ліміти томів CSI під час фази автоматичного масштабування
(див. розділ нижче).
...
### Ліміти приєднання томів CSI та автомасштабування кластера {#csi-volume-attach-limits-and-cluster-autoscaler}

[Cluster autoscaler](https://github.com/kubernetes/autoscaler/tree/master/cluster-autoscaler) може враховувати ліміти томів CSI, коли `--enable-csi-node-aware-scheduling=true`. Ця опція не залежить від функціональної можливості `VolumeLimitScaling`.

Якщо ви використовуєте cluster autoscaler, встановлюйте `spec.preventPodSchedulingIfMissing` у `true` лише тоді, коли cluster autoscaler налаштовано з `--enable-csi-node-aware-scheduling=true`. Інакше його симуляції планування не включають необхідну інформацію `CSINode` для нових вузлів, і cluster autoscaler може не збільшити масштаб для подів, що очікують та використовують томи CSI.
