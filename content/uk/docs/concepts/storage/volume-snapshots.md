...
У кластерах, де знімок доступний для використання лише з частини кластера (наприклад, знімок тому в одній зоні багатозонового кластера без спільного сховища), топологія знімка може бути записана та врахована при відновленні з нього. За подробицями звертайтеся до додаткової [документації CSI](https://kubernetes-csi.github.io/docs/introduction.html).
...
### Виділення знімків томів {#provisioning-volume-snapshot}

Існує два способи виділення знімків: статично попередньо виділені або динамічно виділені.
...
Контролер знімків відповідає за звʼязування обʼєкта `VolumeSnapshot` з відповідним обʼєктом `VolumeSnapshotContent`, як у випадку попереднього виділення, так і у випадку динамічного виділення. Звʼязування є зіставлення один до одного.

У випадку попереднього виділення, обʼєкт `VolumeSnapshot` залишається незвʼязаним до тих пір, доки не буде створено запитаний обʼєкт `VolumeSnapshotContent`.
...
Видалення спровоковане видаленням обʼєкта `VolumeSnapshot`, і буде дотримано `DeletionPolicy`. Якщо `DeletionPolicy` — це `Delete`, тоді підлеглий знімок сховища буде видалений разом з обʼєктом `VolumeSnapshotContent`. Якщо `DeletionPolicy` — це `Retain`, то як сховища, так і `VolumeSnapshotContent` залишаться.
...
## Топологія знімків томів {#volume-snapshot-topology}

У деяких кластерах знімок доступний для використання лише з частини кластера. Наприклад, у багатозоновому кластері, де сховище не є спільним між зонами, знімок тому в одній зоні доступний для використання лише з цієї зони: томи, відновлені зі знімка, можуть створюватися лише там, де знаходяться дані знімка.

Коли функцію `VolumeSnapshotTopology` увімкнено, ця топологія записується та дотримується:

- Адміністратор кластера може встановити `allowedTopologies` у [VolumeSnapshotClass](/docs/concepts/storage/volume-snapshot-classes/), щоб вказати, де мають бути доступні для використання динамічно створені знімки.
- Драйвер CSI повідомляє, звідки доступний для використання знімок, а сайдкар csi-snapshotter записує це у `VolumeSnapshotContent.spec.nodeAffinity`.
- Коли ви виділяєте том зі знімка, ця топологія використовується для розміщення відновленого тому там, де доступні дані знімка.
