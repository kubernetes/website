...
Kubernetes підтримує багато типів томів. {{<glossary_tooltip term_id="pod" text="Pod">}} може використовувати одночасно будь-яку кількість типів томів. Тип [ефемерний том](/docs/concepts/storage/ephemeral-volumes/) існує протягом життєвого циклу повʼязаного з ним Podʼа, але [постійні томи](/docs/concepts/storage/persistent-volumes/) існують поза життєвим циклом будь-якого окремого Podʼа. Коли Pod припиняє існування, Kubernetes знищує ефемерні томи; однак Kubernetes не знищує постійні томи. Для будь-якого виду тому в певному Podʼі дані зберігаються при перезапуску контейнера.
...
Поле `emptyDir.mode` дозволяє вам встановити біти дозволів Unix на теці emptyDir, вказавши значення між `0000` та `01777` (вісімкове). Це відповідає тому самому шаблону, що й поле `defaultMode` у томах Secret та ConfigMap. Якщо `mode` не вказано, тека створюється з типовими дозволами `0777`.

Встановлення власного режиму корисне, коли ви хочете обмежити доступ лише власником та групою (наприклад, `0750`), або встановити sticky bit на спільній теці, щоб лише власники файлів могли видаляти власні файли (наприклад, `01777`).

{{< note >}}
Якщо `fsGroup` встановлено в контексті безпеки Podʼа, групові дозволи, застосовані `fsGroup`, перевизначають `mode`, вказаний тут. Поле `mode` не впливає на Windows.
{{< /note >}}
...
Коли функціональну можливість `InPlacePodVerticalScalingMemoryBackedVolumes` увімкнено, ви можете динамічно регулювати `sizeLimit` томів `emptyDir`, що зберігаються в памʼяті (`medium: Memory`), без перезапуску Podʼа. Докладніше див. [Зміна розміру ресурсів CPU та памʼяті, призначених контейнерам](/docs/tasks/configure-pod-container/resize-container-resources/#resizing-memory-backed-emptydir-volumes).
{{< /note >}}

#### Приклад конфігурації дозволів emptyDir {#emptydir-permissions-configuration-example}
...
### gitRepo (вимкнено) {#gitrepo}

{{< warning >}}
Kubernetes {{< skew latestVersion >}} _не містить_ драйвер томів `gitRepo`. Остання версія, яка надавала можливість використовувати цей драйвер, була Kubernetes v1.35, і він був застарілим з моменту випуску [v1.11](/releases/1.11).

Щоб створити Pod, що має змонтований репозиторій git, ви можете змонтувати том [EmptyDir](#emptydir), який монтується в [init container](/docs/concepts/workloads/pods/init-containers/), що клонує репозиторій за допомогою git, а потім монтує [EmptyDir](#emptydir) в контейнер Podʼа.
...
Том визначається при запуску Podʼа залежно від значення `pullPolicy`:
...
- [`subPath`](/docs/concepts/storage/volumes/#using-subpath) або [`subPathExpr`](/docs/concepts/storage/volumes/#using-subpath-expanded-environment) монтуються для контейнерів (`spec.containers[*].volumeMounts[*].subPath`, `spec.containers[*].volumeMounts[*].subPathExpr`) підтримуються тільки у Kubernetes v1.33.
- Поле `spec.securityContext.fsGroupChangePolicy` не має жодного ефекту для цього типу тому.
- [Контролер допуску `AlwaysPullImages`](/docs/reference/access-authn-authz/admission-controllers/#alwayspullimages) також працює для цього джерела тому, як і для образів контейнерів.
...
#### Статус Podʼа та томи `image` {#image-volume-pod-status}

{{< feature-state feature_gate_name="ImageVolumeWithDigest" >}}

Якщо функціональну можливість [`ImageVolumeWithDigest`](/docs/reference/command-line-tools-reference/feature-gates/#ImageVolumeWithDigest) увімкнено у вашому кластері, то щоразу, коли ви вказуєте том `image` для Podʼа, kubelet оновлює статус Podʼа, щоб записати _digest_ образу контейнера, який використовується як джерело тому.

Ось спрощений приклад запущеного Podʼа, представлений у YAML, включаючи оновлення статусу. Зверніть увагу на нове поле `ImageRef` в `volumeMounts` у статусі контейнера.
...
Порівняно з томами `hostPath`, томи `local` використовуються довговічніше і мобільніше, без ручного планування Podʼів на вузли. Система обізнана з обмеженнями вузла тому, переглядаючи приналежність вузла до PersistentVolume.
...
PersistentVolume `volumeMode` може бути встановлено на "Block" (замість стандартного значення "Filesystem") для експонування локального тому як чистого (raw) блокового пристрою.
...
{{< note >}}
Локальний PersistentVolume вимагає ручного очищення та видалення користувачем, якщо зовнішній статичний постачальник  не використовується для управління циклом життя тому.
{{< /note >}}
...
Том проєкційного типу підʼєднує кілька наявних джерел томів в одну теку. Докладніше дивіться [проєкційні томи](/docs/concepts/storage/projected-volumes/).
...
Носій інформації (наприклад, Disk чи SSD) тома `emptyDir` визначається середовищем файлової системи, яке утримує коренева тека kubelet (зазвичай `/var/lib/kubelet`). Немає обмежень щодо того, скільки місця може використовувати том `emptyDir` чи `hostPath`, і відсутня ізоляція між контейнерами чи між Podʼами.
...
До сторонніх втулків томів належать {{< glossary_tooltip text="Container Storage Interface" term_id="csi" >}} (CSI), а також FlexVolume (який є застарілим). Ці втулки дозволяють виробникам засобів для зберігання створювати власні зовнішні втулки томів без додавання їх вихідного коду до репозиторію Kubernetes.
...
Для виробників засобів для зберігання, які прагнуть створити зовнішній втулок тома, будь ласка, звертайтеся до [Поширених питань щодо втулків томів](https://github.com/kubernetes/community/blob/main/sig-storage/volume-plugin-faq.md).
...
- `readOnly`: Необовʼязкове булеве значення, яке вказує, чи повинен бути том "ControllerPublished" (приєднаний) як тільки для читання. Стандартно — false. Це значення передається драйверу CSI через поле `readonly` у `ControllerPublishVolumeRequest`.
...
Драйвери вузла CSI повинні виконувати різні привілейовані операції, такі як сканування пристроїв диска та монтування файлових систем. Ці операції відрізняються для кожної операційної системи хоста. Для робочих вузлів під управлінням Linux контейнеризовані втулки вузлів CSI зазвичай розгортаються як контейнери з підвищеними правами. Для робочих вузлів під управлінням Windows підтримуються привілейовані операції для контейнеризованих втулків вузлів CSI за допомогою [csi-proxy](https://github.com/kubernetes-csi/csi-proxy) — автономного бінарного файлу, що підтримується спільнотою і який необхідно попередньо встановити на кожному вузлі Windows.
...
Монтування може бути зроблено тільки для читання, встановленням у поле `.spec.containers[*].volumeMounts[*].readOnly` значення `true`. Це не робить том сам собою тільки для читання, але конкретний контейнер не зможе записувати в нього. Інші контейнери в Podʼі можуть монтувати той самий том з правами читання-запису.
...
## Власник файлу {#file-owner}

### Групове володіння (GID) {#group-ownership-gid}

Групове володіння (GID) файлів тому контролюється полем `spec.securityContext.fsGroup` Podʼа.

Докладні кроки налаштування див. у [Налаштування контексту безпеки для Podʼа або контейнера](/docs/tasks/configure-pod-container/security-context/).

### Володіння користувачем (UID) {#user-ownership-uid}

{{< feature-state feature_gate_name="AtomicWriteVolumeUserFields" >}}

Встановлення функціональної можливості [`AtomicWriteVolumeUserFields`](/docs/reference/command-line-tools-reference/feature-gates/#AtomicWriteVolumeUserFields) увімкнює поля володіння файлами користувачем (UID) для томів `configMap`, `secret`, `downwardAPI` та `projected`.

Коли `defaultUser` вказано на рівні тому, він встановлює UID власника для всіх його файлів даних під час створення. На рівні елемента поле `user` контролює UID власника окремого файлу та має перевагу над `defaultUser`.
...
## Параметри bind mount {#bind-mount-options}

{{< feature-state feature_gate_name="VolumeBindMountOptions" >}}

Поле `.spec.containers[*].volumeMounts[*].bindMountOptions` дозволяє вам застосовувати повʼязані з безпекою прапорці bind mount Linux до будь-якого монтування тому. Допустимі значення:

- `noexec` — запобігає виконанню бінарних файлів на змонтованому томі
- `nodev` — ігнорує спеціальні файли пристроїв на змонтованому томі
- `nosuid` — ігнорує біти set-user-identifier або set-group-identifier на змонтованому томі

Ці параметри застосовуються для кожного контейнера, тому різні контейнери в одному Podʼі можуть монтувати той самий том із різними параметрами bind mount. Поле не підтримується з [томами образів](#image).

{{< note >}}
Середовище виконання контейнерів (таке як containerd або CRI-O) повинно підтримувати поле `mount_options` у повідомленні CRI `Mount`. Якщо середовище виконання не повідомляє про підтримку, kubelet відхиляє Podʼи, які використовують `bindMountOptions`. Це поле не впливає на вузли Windows.
{{< /note >}}
