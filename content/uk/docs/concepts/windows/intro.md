...
Організації, що інвестували у застосунки на базі Windows та Linux, не мають шукати окремі оркестратори для управління своїми навантаженнями, що призводить до збільшення оперативної ефективності у їх розгортаннях, незалежно від операційної системи.
...
Для запуску контейнерів Windows ваш Kubernetes кластер має включати декілька операційних систем. Хоча {{< glossary_tooltip text="панель управління" term_id="control-plane" >}} можна запускати лише на Linux, ви можете розгортати робочі вузли, що працюють як на Windows, так і на Linux.
...
Цей документ використовує термін *контейнери Windows* для позначення контейнерів Windows з ізоляцією на рівні процесу. Kubernetes не підтримує запуск контейнерів Windows з [ізоляцією Hyper-V](https://docs.microsoft.com/en-us/virtualization/windowscontainers/manage-containers/hyperv-container).
...
* [Podʼи](/docs/concepts/workloads/pods/)
  ...
  Pod є базовим будівельним блоком Kubernetes — найменшою та найпростішою одиницею в моделі обʼєктів Kubernetes, яку ви створюєте або розгортаєте. Ви не можете розгортати Windows та
...
* `requests.cpu` та `requests.memory` — запити віднімаються від доступних ресурсів вузла, тому вони можуть використовуватися для уникнення надлишкового резервування вузла. Проте вони не можуть гарантувати ресурси в надлишково зарезервованому вузлі. Їх слід застосовувати до всіх контейнерів як найкращу практику, якщо оператор хоче уникнути надлишкового резервування повністю.
* `securityContext.allowPrivilegeEscalation` — неможливо на Windows; жодне з capabilities не підключено
...
## Pause контейнер {#pause-container}

В Podʼі Kubernetes першим створюється інфраструктурний або «pause» контейнер, щоб Pod мав контейнер. У Linux, групи керування та простори імен, що утворюють Pod, потребують процесу для підтримки їхнього подальшого існування; процес pause забезпечує це. Контейнери, які належать до одного Podʼа, включаючи інфраструктурні та робочі контейнери, мають спільну мережеву точку доступу (з тою ж самою IPv4 та / або IPv6 адресою, тими самими просторами портів мережі). Kubernetes використовує pause контейнери для того, щоб дозволити робочим контейнерам виходити з ладу або перезапускатися без втрати будь-якої конфігурації мережі.

Kubernetes підтримує багатоархітектурний образ, який включає підтримку для Windows. Для Kubernetes v{{< skew currentPatchVersion >}} рекомендований pause образ — `registry.k8s.io/pause:3.6`. [Вихідний код](https://github.com/kubernetes/kubernetes/tree/master/build/pause) доступний на GitHub.
...
Наступні середовища виконання контейнерів працюють у Windows:
...
На вузлах Windows діють строгі правила сумісності, де версія операційної системи хосту повинна відповідати версії операційної системи базового образу контейнера.
