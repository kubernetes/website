...
Canary-розгортання є поширеним випадком використання Service. Щоб дізнатися, що це передбачає, ознайомтесь з наступним навчальним посібником [Розгортання релізу за допомогою canary-розгортання](/docs/tutorials/stateless-application/canary-deployment/).
...
#### Налаштування IP-адреси для Сервісів `type: NodePort` {#service-nodeport-custom-listen-address}

При використанні kube-proxy у режимі [`iptables`](/docs/reference/networking/virtual-ips/#proxy-mode-iptables) Сервіси NodePort зазвичай доступні на всіх IP-адресах вузлів. При використанні режиму [`nftables`](/docs/reference/networking/virtual-ips/#proxy-mode-nftables) вони типово доступні лише на основній IP-адресі вузла (або основних IP-адресах у разі dual-stack).

Ви можете змінити набір IP-адрес вузлів, на яких доступні Сервіси NodePort, за допомогою прапорця `--nodeport-addresses` для kube-proxy або еквівалентного поля `nodePortAddresses` у [файлі конфігурації kube-proxy](/docs/reference/config-api/kube-proxy-config.v1alpha1/). Він приймає список IP-блоків через кому (наприклад, `10.0.0.0/8`, `192.0.2.0/25`) або одне чи кілька з наступних ключових слів:

* `primary` — основна IPv4 та/або IPv6 адреса вузла, згідно з обʼєктом Node. (Це типове значення для режиму `nftables`.)
* `localhost` — адреси loopback вузла (`127.0.0.0/8`, `::1/128`).
* `all` — усі адреси. (Це типове значення для режимів `iptables` та `ipvs`.)

Наприклад, якщо ви запускаєте kube-proxy з прапорцем `--nodeport-addresses=192.168.0.0/24`, то kube-proxy спробує знайти локальну IP-адресу в цій підмережі на кожному вузлі та обслуговуватиме Сервіси NodePort лише через цю IP-адресу.

#### Сервіси `type: NodePort` через localhost {#localhost-nodeports}

Механізми, які використовують проксі Сервісів для реалізації Сервісів NodePort, не завжди підтримують надання Сервісів NodePort на localhost. Для kube-proxy:

* При використанні режиму `iptables` зі значенням `--nodeport-addresses`, яке включає `127.0.0.1`, Сервіси NodePort будуть доступні на `127.0.0.1`. Однак це вимагає увімкнення системного sysctl (`route_localnet`), який може мати небезпечні побічні ефекти в деяких кластерах. NodePort на localhost для IPTables можна вимкнути, передавши `--iptables-localhost-nodeports false` до kube-proxy або встановивши `--nodeport-addresses` на діапазон, який не включає `127.0.0.1`.

* При використанні режиму `ipvs` або режиму `iptables` у кластерах з одним стеком IPv6, Сервіси NodePort недоступні на localhost.

{{< feature-state feature_gate_name="KubeProxyNFTablesLocalhostNodePorts" >}}

* При використанні режиму `nftables` Сервіси NodePort будуть доступні на localhost, коли увімкнено функціональну можливість `KubeProxyNFTablesLocalhostNodePorts` і `--nodeport-addresses` встановлено на значення, яке явно включає `localhost`. (Встановлення лише `all` _не_ увімкне Сервіси NodePort на localhost.) Це реалізовано шляхом перенаправлення зʼєднань NodePort на localhost через проксі в просторі користувача, тому це не так ефективно, як звичайне проксіювання Сервісів.

Сторонні мережеві втулки, які мають власні реалізації проксі Сервісів, можуть підтримувати або не підтримувати NodePort на localhost; зверніться до документації цих плагінів.
...
Для Service з типом `type: LoadBalancer`, контролер може встановити `.status.loadBalancer.ingress.ipMode`. `.status.loadBalancer.ingress.ipMode` вказує, як веде себе IP-адреса балансувальника. Режим може бути вказаний лише тоді, коли вказано поле `.status.loadBalancer.ingress.ip`.

Є два можливі значення для `.status.loadBalancer.ingress.ipMode`: "VIP" та "Proxy". Типове значення "VIP" означає, що трафік подається на вузол з призначенням, встановленим на IP та порт балансувальника. Є два випадки, коли встановлено значення "Proxy", залежно від того, як постачальник хмари балансує трафік:
