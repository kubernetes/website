...
Забезпечте доступ до мережевих служб за допомогою розширюваного, орієнтованого на ролі та протокол-орієнтованого механізму конфігурації. [Gateway API](https://gateway-api.sigs.k8s.io/) є {{<glossary_tooltip text="надбудовою" term_id="addons">}}, що містить [види API](https://gateway-api.sigs.k8s.io/references/spec/), які надають динамічну інфраструктуру надання та розширену маршрутизацію трафіку.
...
- __Експресивний:__ Види Gateway API підтримують функціональність для загальних випадків маршрутизації трафіку, таких як відповідність на основі заголовків, визначення пріоритету трафіку та інших, які були можливі тільки в [Ingress](/docs/concepts/services-networking/ingress/) за допомогою власних анотацій.
...
Дивіться [специфікацію Gateway](https://gateway-api.sigs.k8s.io/references/spec/#gateway.networking.k8s.io/v1.Gateway) для повного визначення цього виду API. Для інструкцій з налаштування прослуховувачів HTTPS/TLS дивіться [Посібник TLS Gateway API](https://gateway-api.sigs.k8s.io/guides/tls/).
...
Повне визначення цього типу API див. у довідці [GRPCRoute](https://gateway-api.sigs.k8s.io/references/spec/#grpcroute).
...
Дивіться [документацію щодо відповідності](https://gateway-api.sigs.k8s.io/docs/concepts/conformance/) для розуміння деталей, таких як канали випусків, рівні підтримки та виконання тестів відповідності.
...
Дивіться [специфікацію API](https://gateway-api.sigs.k8s.io/reference/api-spec/main/spec/) для отримання додаткових деталей про всі види API Gateway API.
