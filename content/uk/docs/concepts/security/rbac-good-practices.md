...
В ідеалі, мінімальні права RBAC повинні бути надані користувачам та службовим обліковим записам. Використовуйте лише дозволи, які явно потрібні для їхньої роботи. Хоча кожен кластер буде відрізнятися, деякі загальні правила, які можна застосувати, включають наступне:
...
### Доступ до субресурсу `proxy` обʼєктів Node {#access-to-proxy-subresource-of-nodes}

Користувачі, які мають доступ до субресурсу `nodes/proxy`, мають права на використання API Kubelet, що дозволяє виконувати команди на кожному Podʼі на вузлі (вузлах), до якого вони мають права доступу. Цей доступ обходить логування подій та контроль допуску, тому слід бути обережним перед наданням будь-яких прав на цей ресурс. Ці API можна використовувати за допомогою HTTP-запитів `GET` через веб-сокет, для чого потрібна лише авторизація дієслова **get**. Це означає, що дозвіл **get** на `nodes/proxy` не є дозволом тільки для читання. Наприклад, дозвіл **get** на `nodes/proxy` надає доступ до привілейованих API Kubelet, які можуть отримувати журнали контейнерів або виконувати та підключатися до процесів Pod, навіть якщо виконавець не має еквівалентних дозволів через API Kubernetes.

Див. [Автентифікація/авторизація Kubelet](/docs/reference/access-authn-authz/kubelet-authn-authz/#get-nodes-proxy-warning)
