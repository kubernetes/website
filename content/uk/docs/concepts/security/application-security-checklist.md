...
- Цей список припускає, що `розробник` є користувачем кластера Kubernetes, який взаємодіє з обʼєктами в межах простору імен.

{{< caution >}}
Списки **не є** достатніми для досягнення хорошого рівня безпеки самі по собі. Хороша безпека вимагає постійної уваги та вдосконалення, але список може бути першим кроком на нескінченному шляху до готовності до забезпечення безпеки. Деякі з рекомендацій у цьому списку можуть бути занадто обмежувальними або надто мʼякими для ваших конкретних потреб у безпеці. Оскільки безпека в Kubernetes не є універсальною, кожну категорію елементів списку слід оцінювати за її власною суттю.
{{< /caution >}}
...
- [ ] Встановлено обмеження памʼяті для робочих навантажень з обмеженням, яке дорівнює або більше за запит.
...
- [ ] Вимкніть усі можливості (capabilities) у контейнерах і додайте лише конкретні, необхідні для роботи контейнера.
...
Дії **create**, **update** і **delete** слід дозволяти обачно. Дія **patch**, якщо дозволена для простору імен, може
[дозволити користувачам оновлювати мітки у просторі імен або розгортанні](/docs/concepts/security/rbac-good-practices/#namespace-modification), що може збільшити поверхню атаки.
...
- [ ] Використовуйте інструмент для сканування образів перед розгортанням контейнерів у кластері Kubernetes.
...
Деяким контейнерам може знадобитися інший рівень ізоляції, ніж той, що надається типовим середовищем виконання кластера. `runtimeClassName` можна використовувати у podspec для визначення іншого класу виконання.
...
У середовищах високої довіри розгляньте використання [конфіденційних віртуальних машин](/blog/2023/07/06/confidential-kubernetes/) для подальшого підвищення безпеки чутливих робочих навантажень.
