...
* `Gt` зіставляється, коли значення толерантності більше за значення taint.
* `Lt` відповідає, коли значення толерантності менше за значення taint.
...
* Для `PreferNoSchedule` з числовими операторами: якщо толерантність пода не задовольняє числовому порівнянню (наприклад, значення толерантності < значення taint при використанні `Gt`), планувальник надає вузлу нижчий пріоритет, але все одно може планувати там, якщо немає кращих варіантів.
{{< /note >}}
