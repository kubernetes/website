...
Ви можете вказати `tolerationSeconds` для Podʼа, щоб визначити, як довго цей Pod залишається привʼязаним до несправного вузла або вузла, який не відповідає.
...
{{< note >}}
Kubernetes автоматично додає толерантність для `node.kubernetes.io/not-ready` та `node.kubernetes.io/unreachable` з `tolerationSeconds=300`, якщо не ви або контролер встановили ці толерантності явно.
...
Замість того, щоб позначати цілі вузли, адміністратори можуть також [позначати окремі пристрої](/docs/concepts/resource-management/dynamic-resource-allocation/device-taints/#device-taints-and-tolerations) коли кластер використовує [динамічний розподіл ресурсів](/docs/concepts/resource-management/dynamic-resource-allocation/) для керування спеціальним обладнанням. Перевага полягає у тому, що позначення може бути спрямоване саме на те обладнання, яке є несправним або потребує обслуговування. Толерування також підтримуються і можуть бути вказані при запиті пристроїв. Як і позначки taint, вони застосовуються до всіх Podʼів, які мають один і той самий виділений пристрій.
...
* Прочитайте про [позначки taints та толерантність пристроїв](/docs/concepts/resource-management/dynamic-resource-allocation/device-taints/#device-taints-and-tolerations)
