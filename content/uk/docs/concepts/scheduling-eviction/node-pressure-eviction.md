...
Kubelet використовує такі сигнали виселення:
...
На вузлах із налаштованими [hugepages](/docs/tasks/manage-hugepages/scheduling-hugepages/) kubelet віднімає загальну потужність hugepages вузла від `memory.available`, яку використовує менеджер виселення. Без цього коригування зарезервована під hugepages оперативна памʼять завищує `AvailableBytes`, оскільки контролер cgroup памʼяті не відстежує виділення hugetlb у своєму робочому наборі. Це може затримати виселення та призвести до OOM-завершень. Щоб відновити попередню поведінку, вимкніть [функціональну можливість](/docs/reference/command-line-tools-reference/feature-gates/) `HugepageAwareEviction`.
{{</note>}}
...
1. `imagefs`: Опціональна файлова система, яку середовища виконання контейнерів можуть використовувати для зберігання образів контейнерів (які є шарами тільки для читання). Якщо окремої файлової системи `containerfs` немає, файлова система образів також зберігає шари контейнера, доступні для запису.

1. `containerfs`: Опціональна файлова система, яку середовища виконання контейнерів можуть використовувати для зберігання записуваних шарів контейнерів. Коли використовується `containerfs`, файлова система `imagefs` може бути розділена так, щоб зберігати лише образи (шари тільки для читання) і більше нічого.

Ці ідентифікатори описують файлові системи так, як їх бачить kubelet. Вони не завжди означають три різні точки монтування: у звичайних конфігураціях два або всі три ідентифікатори можуть посилатися на одну й ту ж базову файлову систему.
...
Наразі налаштування власних порогових значень, повʼязаних із `containerfs`, не підтримується, і буде видано попередження, якщо спробувати це зробити; будь-які надані власні значення будуть ігноровані.
...
- Якщо файлова система `containerfs` досягає порогових значень для виселення, kubelet виконує збір сміття мертвих Podʼів і контейнерів.
...
Якщо ви використовуєте [статичний Pod](/docs/concepts/workloads/pods/#static-pods) і хочете уникнути його виселення під час нестачі ресурсів, встановіть поле `priority` для цього Podʼа безпосередньо. Статичні Podʼи не підтримують поле `priorityClassName`.
...
Ви можете обійти цю проблему, встановивши для контейнерів, які ймовірно виконують інтенсивну діяльність з операцій введення/виведення, однакове обмеження памʼяті та запит памʼяті. Вам потрібно оцінити або виміряти оптимальне значення обмеження памʼяті для цього контейнера.
