...
3. **Фільтрація ResourceSlice**: для кожного Podʼа Kubernetes перевіряє ResourceSlices у кластері, щоб знайти пристрій, який задовольняє всі наступні критерії:
   ...
   * ResourceSlice має нерозподілені ресурси, які відповідають вимогам ResourceClaim Pod.
...
Детальніше про поле `status.devices` дивіться {{< api-reference page="resource/resource-claim-v1" anchor="ResourceClaimStatus" text="ResourceClaim" >}} в довіднику API.
...
Краще уникати цього оминаючи планувальник, оскільки Pod, який призначений для вузла, блокує нормальні ресурси (ОЗП, ЦП), які потім не можуть бути використані для інших Podʼів, поки Pod застряг. Щоб запустити Pod на певному вузлі, при цьому проходячи через звичайний потік планування, створіть Pod із селектором вузла, який точно відповідає бажаному вузлу:
...
У наступних розділах описано функції DRA, що підтримують розширені сценарії використання. Їх використання є необовʼязковим і може бути актуальним лише для драйверів DRA, які їх підтримують.
...
spec:
  ...
  # Виселити всі Podʼи і не планувати нові.
  deviceSelector:
    driver: dra.example.com
  taint:
...
Podʼи виселяються шляхом їх видалення. Зазвичай це відбувається дуже швидко, за винятком випадків, коли толерантність до taint затримує цей процес на певний період або коли потрібно виселити дуже багато Podʼів. Якщо це займає більше часу, повідомлення надає інформацію про поточний стан:
...
Слід бути обережним щодо потенційного стану перегонів між планувальником і контролером, які спостерігають за новим taint у різний час, що може призвести до того, що Podʼи все ще будуть заплановані в той час, коли контролер вважає, що немає жодного, який потрібно виселити, і тому встановлює цей стан на `False`. На практиці такий стан перегонів є дуже малоймовірним, оскільки оновлення стану відбувається лише після навмисної затримки в кілька секунд.

Для `effect: None` повідомлення надає інформацію про кількість уражених пристроїв, скільки з них виділено і скільки Podʼів буде виселено, якщо ефект буде `NoExecute`. Це можна використовувати для пробного запуску перед фактичним запуском виселення:
...
* Перегляньте повідомлення:
  ...
  Опубліковані пристрої — це пристрої, перелічені в ResourceSlices. Позначення їх taint запобігає виділенню нових Podʼів. Тільки виділені пристрої спричиняють виселення Podʼів, які їх використовують.
...
Коли метадані пристроїв увімкнено, драйвер генерує файли метаданих і специфікації CDI bind-mount під час підготовки виділених пристроїв для Podʼа, перед запуском контейнерів, що їх споживають. Метадані зʼявляються всередині контейнерів за відомими шляхами, як [визначено вище](#device-metadata-protocol).
