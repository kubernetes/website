...
Планувальник чекає до **600 секунд**, щоб усі `bindingConditions` стали `True`. Якщо тайм-аут досягається або будь-які `bindingFailureConditions` є `True`, планувальник очищає виділення та перенаправляє Pod.
