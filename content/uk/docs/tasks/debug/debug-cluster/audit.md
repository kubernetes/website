...
- `--audit-webhook-batch-throttle-burst` визначає максимальну кількість пакетів, які генеруються в той же момент, якщо дозволений QPS раніше не використовувався повністю. Стандартне значення — 15.
{{% /tab %}}
{{% tab name="log" %}}
