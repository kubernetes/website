...
## Авторизація та міркування безпеки {#authorization-and-security-considerations}

Доступ до `kubectl port-forward` контролюється механізмами авторизації Kubernetes, такими як {{< glossary_tooltip text="Role-Based Access Control (RBAC)" term_id="rbac" >}}. Авторизація забезпечується сервером API Kubernetes, а не клієнтом `kubectl`.

Щоб використовувати `kubectl port-forward`, користувач повинен мати дозвіл на доступ до цільового ресурсу (наприклад, Pod або Service) та субресурсу `portforward`. Типові необхідні дозволи включають `get` на `pods` та `create` на `pods/portforward`.

Адміністратори кластерів повинні ретельно обмежувати ці дозволи, оскільки порт-форвардинг може надавати прямий мережевий доступ до робочих навантажень і обходити засоби контролю на рівні мережі.
