...
Завантажений вами файл ще не є валідним {{< glossary_tooltip text="маніфестом" term_id="manifest" >}} Kubernetes. Замість цього цей шаблон є YAML-представленням обʼєкта Job з деякими заповнювачами, які потрібно замінити перед використанням. Синтаксис `$ITEM` не має значення для Kubernetes.
...
```shell
# Розмноження шаблону на кілька файлів, по одному для кожного процесу
mkdir -p ./jobs
for item in apple banana cherry
do
  cat job-tmpl.yaml | sed "s/\$ITEM/$item/g" > ./jobs/job-$item.yaml
done
```
...
# Ваш кластер автоматично видаляє повʼязані з Job Pod
kubectl delete jobs -l jobgroup=jobexample
```

## Використання розширених параметрів шаблонів {#use-advanced-template-parameters}

В [першому прикладі](#create-jobs-based-on-a-template), кожен екземпляр шаблону мав один параметр, цей параметр також використовувався в назві Job. Однак, [назви](/docs/concepts/overview/working-with-objects/names/#names) обмежені набором символів, які можна використовувати.
