---
title: Надання ресурсів CPU та памʼяті рівня Pod
content_type: task
weight: 30
...
Ця сторінка показує, як вказати ресурси CPU та памʼяті для Pod рівня Pod, додатково до специфікацій ресурсів на рівні контейнера. Вузол Kubernetes виділяє ресурси для podʼів на основі запитів ресурсів Pod. Ці запити можуть бути визначені на рівні Pod або індивідуально для контейнерів у Pod. Коли присутні обидва, запити на рівні Pod мають пріоритет.

Аналогічно, використання ресурсів Pod обмежується лімітами, які також можуть бути встановлені рівня Pod або індивідуально для контейнерів у Pod. Знову ж таки, ліміти рівня Pod мають пріоритет, коли присутні обидва. Це дозволяє гнучко керувати ресурсами, дозволяючи контролювати розподіл ресурсів як на рівні Pod, так і на рівні контейнера.
...
* Пріоритет: Коли вказані як ресурси на рівні Pod, так і на рівні контейнера, ресурси на рівні Pod мають пріоритет.
* QoS: Ресурси на рівні Pod мають пріоритет у впливі на клас QoS Pod.
* OOM Score: Розрахунок коригування OOM score враховує як ресурси на рівні Pod, так і на рівні контейнера.
...
Для Kubernetes {{< skew currentVersion >}}, зміна розміру ресурсів на рівні Pod має такі обмеження:

* **Типи ресурсів:** Тільки ресурси CPU, памʼяті та hugepages можуть бути вказані на рівні Pod.
* **Операційна система:** Ресурси на рівні Pod не підтримуються для Windows Podʼів.
* **Менеджери ресурсів:** Менеджери ресурсів Topology Manager, Memory Manager та CPU Manager підтримують ресурси на рівні Pod, коли увімкнено функціональну можливість [`PodLevelResourceManagers`](/docs/reference/command-line-tools-reference/feature-gates/#PodLevelResourceManagers). Див. [Менеджери ресурсів на рівні Pod](/docs/concepts/resource-management/resource-managers/#pod-level-resource-managers) для отримання додаткової інформації. Без увімкнення цього feature gate вони не вирівнюють Podʼи та контейнери на основі ресурсів на рівні Pod.
* **In-Place Resize:** [Зміна розміру ресурсів на місці](https://kubernetes.io/docs/tasks/configure-pod-container/resize-container-resources/) на рівні Pod вимагає функціональної можливості `InPlacePodLevelResourcesVerticalScaling`, яка перебуває в стані альфа у Kubernetes {{< skew currentVersion >}}. Для отримання додаткової інформації див. [Зміна розміру ресурсів CPU та памʼяті Podʼів](/docs/tasks/configure-pod-container/resize-pod-resources/).
...
* [Менеджери ресурсів на рівні Podʼа](/docs/concepts/resource-management/resource-managers/#pod-level-resource-managers)
