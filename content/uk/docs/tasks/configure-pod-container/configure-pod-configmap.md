...
{{< caution >}}
Як і раніше, усі попередні файли у теці `/etc/config/` буде недоступним.
{{< /caution >}}
...
* ConfigMaps знаходяться в конкретному {{< glossary_tooltip term_id="namespace" >}}. Podʼи можуть посилатися лише на ConfigMaps, які знаходяться в тому ж просторі імен, що і сам Pod.
...
Якщо ви створили теку `configure-pod-container` і вже не потребуєте її, вам слід також її видалити або перемістити в кошик або місце для видалених файлів.
