...
* `allowPrivilegeEscalation`: Контролює, чи може процес отримувати більше привілеїв, ніж його батьківський процес, наприклад, шляхом виконання [setuid-бінарного файлу](https://en.wikipedia.org/wiki/Setuid). Ця логічна величина безпосередньо контролює, чи встановлюється прапорець [`no_new_privs`](https://www.kernel.org/doc/Documentation/prctl/no_new_privs.txt) для процесу контейнера, зауважте, що значення інвертоване: якщо `allowPrivilegeEscalation` дорівнює true, то `no_new_privs` буде встановлено на false.

  Якщо `allowPrivilegeEscalation` не визначено в полі `securityContext` специфікації контейнера, то зазвичай воно буде true.

  `allowPrivilegeEscalation: false` є несумісним, і тому не може бути встановлено в поєднанні з контейнером, який:

  * запущений з привілеями, або
  * має можливість `CAP_SYS_ADMIN`
...
Зазвичай Kubernetes застосовує мітку SELinux до відповідних томів миттєво, використовуючи параметр монтування `-o context=<мітка>`.

Щоб скористатися цим прискоренням, мають бути виконані всі ці умови:

* Pod повинен використовувати PersistentVolumeClaim.
* Pod має `spec.securityContext.seLinuxChangePolicy` або nil (стандартно), або `MountOption`.
...
2. Підніміть метрику `selinux_warning_controller_selinux_volume_conflict`. Метрика містить назви podʼів + простори імен у якості міток для легкої ідентифікації постраждалих томів.

Адміністратор кластера може використовувати цю інформацію, щоб визначити, на які саме Podʼи впливає ця зміна, і проактивно виключити їх з оптимізації (наприклад, встановити `spec.securityContext.seLinuxChangePolicy: Recursive`).

{{< warning >}}
Ми наполегливо рекомендуємо кластерам, що використовують SELinux, увімкнути цей контролер і переконатися, що метрика `selinux_warning_controller_selinux_volume_conflict` не повідомляє про жодні конфлікти, перш ніж оновити Kubernetes до v1.37 або новішої версії, де `SELinuxMount` є GA та стандартно увімкненим.
{{< /warning >}}
...
* `SELinuxMountReadWriteOncePod`: вмикає оптимізацію для томів з `accessModes: ["ReadWriteOncePod"]`. Це дуже безпечна можливість, оскільки не може статися так, що два пристрої можуть використовувати один том з таким режимом доступу. GA з v1.36.
* `SELinuxChangePolicy`: вмикає поле `spec.securityContext.seLinuxChangePolicy` у Pod і повʼязаний з ним SELinuxWarningController у kube-controller-manager. Це можна використовувати, щоб виключити Podʼи з оптимізації монтування, коли декілька Podʼів з різними мітками SELinux спільно використовують том. GA з v1.36.
* `SELinuxMount`: вмикає оптимізацію для всіх допустимих томів. GA з v1.37. Під час оновлення зі старіших версій Kubernetes, де `SELinuxMount` було стандартно вимкнено, ми рекомендуємо спочатку увімкнути функціональну можливість `SELinuxChangePolicy` + SELinuxWarningController, щоб перевірити вплив змін.
