...
Всі змінні оточення також передаються втулку як вони є:
...
Крім того, перший аргумент, який передається втулку, завжди буде повним шляхом до місця, де його було викликано (`$0` буде дорівнювати `/usr/local/bin/kubectl-foo` в прикладі вище).
...
# встановимо ваш втулок, перемістивши його у теку з $PATH
...
# до виконуваного файлу вашого втулка.
...
```bash
# Ви також можете викликати свою власну команду з підкресленнями
kubectl foo_bar
```
...
* У разі будь-яких питань, не соромтеся звертатися до [команди SIG CLI](https://github.com/kubernetes/community/tree/main/sig-cli).
