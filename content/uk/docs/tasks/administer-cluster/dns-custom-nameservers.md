...
На цій сторінці пояснюється, як налаштувати ваші DNS {{< glossary_tooltip text="Pod" term_id="pod" >}} та налаштувати процес розвʼязання імен DNS у вашому кластері.
...
Якщо ви працюєте з CoreDNS як з Deployment, його зазвичай експонують як Service Kubernetes зі статичною IP-адресою. Kubelet передає інформацію про резолвер DNS кожному контейнеру з прапорцем `--cluster-dns=<ір-адреса-dns-служби>`.
...
DNS-сервер підтримує прямий пошук (записи A та AAAA), пошук портів (записи SRV), зворотний пошук IP-адрес (записи PTR) та інші. Для отримання додаткової інформації дивіться [DNS для Service та Pod](/docs/concepts/services-networking/dns-pod-service/).
...
* [loop](https://coredns.io/plugins/loop/): Виявлення простих переспрямувань та припинення процесу CoreDNS у випадку виявлення циклу.
