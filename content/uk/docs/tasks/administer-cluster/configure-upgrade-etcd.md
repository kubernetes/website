...
## Потокове читання з etcd {#streaming-reads-from-etcd}

{{< feature-state feature_gate_name="EtcdRangeStream" >}}

Якщо увімкнено функціональну можливість `EtcdRangeStream` та використовується etcd v3.7 або новіша версія, сервер API читає великі колекції з etcd як потік замість сторінок, що знижує пікове використання памʼяті з обох сторін. Якщо бекенд не реалізує RPC `RangeStream`, сервер API виявляє відповідь gRPC `Unimplemented` і повертається до посторінкового читання. Якщо ваш сумісний з etcd бекенд або проксі не виконує коректний відкат, вимкніть цю функціональну можливість за допомогою `--feature-gates=EtcdRangeStream=false`.

{{< note >}}
Потокові читання записуються як `operation="listStream"` у метриці `etcd_request_duration_seconds`. Оновіть будь-яку панель або сповіщення, яке відповідає `operation="list"`.
{{< /note >}}
