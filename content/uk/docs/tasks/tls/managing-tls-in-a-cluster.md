...
Kubernetes надає API `certificates.k8s.io`, який дозволяє вам отримувати TLS сертифікати, підписані Центрами Сертифікації (CA), які ви контролюєте. Ці CA та сертифікати можуть використовуватися вашими робочими навантаженнями для встановлення довіри.
...
{{< note >}}
Цей підручник використовує CFSSL: інструментарій PKI та TLS від Cloudflare. Перегляньте статтю в блозі Cloudflare [Introducing CFSSL — CloudFlare's PKI toolkit](https://blog.cloudflare.com/introducing-cfssl/), щоб дізнатися більше.
{{< /note >}}
...
CSR тепер має бути видимий в API у стані Pending. Ви можете побачити це, виконавши:
...
Нарешті, ви можете зберігати `ca.pem` у {{< glossary_tooltip text="ConfigMap" term_id="configmap" >}} і використовувати його як кореневий довірчий сертифікат для перевірки серверного сертифіката:
...
Чи є затверджувач машиною або людиною, яка використовує kubectl, роль _схвалювача_ полягає у перевірці, що CSR відповідає двом вимогам:
