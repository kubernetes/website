...
Наприклад, якщо поточне значення метрики — `200м`, а бажане значення — `100м`, кількість реплік буде подвоєна, оскільки \\( { 200.0 \div 100.0 } = 2.0 \\). Якщо поточне значення замість цього — `50м`, кількість реплік буде зменшена вдвічі, оскільки \\( { 50.0 \div 100.0 } = 0.5 \\). Панель управління пропускає будь-яку дію масштабування, якщо співвідношення достатньо близьке до 1,0 (в межах глобально налаштованої допустимості, типово 0.1).
