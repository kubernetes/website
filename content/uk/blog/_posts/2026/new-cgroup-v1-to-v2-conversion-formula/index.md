...
Kubernetes спочатку був розроблений з урахуванням cgroup v1, де CPU shares визначалися просто шляхом присвоєння контейнеру CPU-запитів у вигляді millicpu. Формула виглядає так:

```math
cpu.shares = milliCPU \times \frac{1024}{1000}
```

Зауважте, що значення 1024 у цій формулі є стандартним значенням `cpu.shares` у cgroup v1 і не повʼязане з millicores. Наприклад, контейнер, який запитує 1 CPU (1000m), отримає \(cpu.shares = 1000 \times 1024 / 1000 = 1024\), а контейнер, який запитує 100m, отримає \(cpu.shares = 100 \times 1024 / 1000 = 102\).
...
1. **Кращя узгодженість пріоритетів:**
   - Контейнер, що запитує 1 CPU (1000m) тепер отримає `cpu.weight = 102`. Це значення близьке до стандартного значення 100 в cgroup v2. Це відновлює передбачуване співвідношення пріоритетів між робочими навантаженнями Kubernetes і системними процесами.
...
Також зверніть увагу, що зворотне перетворення з `cpu.weight` назад у milliCPU не завжди дає точне початкове значення. Існують два джерела втрати інформації: перетворення milliCPU у `cpu.shares` включає усічення до цілого числа (наприклад, 100m стає 102 shares, а не 102.4), і, що більш важливо, відображення shares у weight є багато-до-одного (наприклад, значення milliCPU від 90 до 109 всі відображаються на `cpu.weight = 17`). Інструменти, яким потрібні точні значення запитів CPU, повинні читати їх безпосередньо з pod spec, а не виводити з параметрів cgroup.
