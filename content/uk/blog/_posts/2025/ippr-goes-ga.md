...
* **Більш безпечне зменшення обмеження памʼяті** Перевірка Kubelet на запобігання OOM-kill може бути зроблена ще безпечнішою шляхом переміщення перевірки використання памʼяті в сам контейнерний runtime. Дивіться [тікет](https://github.com/kubernetes/kubernetes/issues/135670) для більш детальної інформації.
