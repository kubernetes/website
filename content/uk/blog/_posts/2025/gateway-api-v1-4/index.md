...
- **Політики:** Дозволяє політикам ([GEP-713](https://gateway-api.sigs.k8s.io/geps/gep-713)) орієнтуватися на конкретні правила маршрутизації через поле `sectionName` у їх `targetRef[s]`.
...
Учасники проєкту хотіли б подякувати _усім_, хто долучився до Gateway API, чи то через коміти в репозиторії, обговорення, ідеї чи загальну підтримку. Ми ніколи не змогли б досягти такого прогресу без підтримки цієї відданої та активної спільноти.
