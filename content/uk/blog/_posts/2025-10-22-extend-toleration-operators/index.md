...
## Обмеження поточних толерантностей {#limitations-of-current-tolerations}

На сьогодні Kubernetes підтримує два оператори толерантності:
...
Ці оператори добре працюють у ситуаціях, що мають чітку категоризацію, наприклад, толерантність до вузлів, призначених для конкретних команд, типів обладнання або періодів технічного обслуговування. Однак вони не підходять, коли потрібно приймати рішення на основі числових порівнянь.
...
Функція розширених операторів толерантності представляє два нових оператори для толерантності:

- **`Gt` (більше ніж)**: толерантність узгоджується, якщо числове значення taint менше за значення толерантності
- **`Lt` (менше ніж)**: толерантність відповідає, якщо числове значення taint більше за значення толерантності
