...
Англійська версія цього контенту генерується автоматично, і посилання можуть не працювати. Джерело документа знаходиться [тут](https://github.com/kubernetes/community/blob/main/contributors/devel/sig-release/release.md).
{{% /pageinfo %}}
...
- *release milestone* (*віха випуску*): семантичний рядок версії або
  ...
  Дивіться також [версіювання випусків](https://git.k8s.io/sig-release/release-engineering/reference/versioning.md#kubernetes-release-versioning).

- *гілка випуску* (*release branch*): Гілка Git `release-X.Y`, створена для віхи `vX.Y`.

  Створюється на момент випуску `vX.Y-rc.0` і підтримується після випуску приблизно протягом 12 місяців з випусками виправлення `vX.Y.Z`.

  Примітка: випуски 1.19 і новіші отримують 1 рік підтримки виправлень, а випуски 1.18 і раніші отримували 9 місяців підтримки виправлень.
...
Відкритий тікет більше не є обовʼязковим для PR, але відкриті тікети та повʼязані PR повинні мати синхронізовані мітки. Наприклад, PR з високим пріоритетом проблеми може не бути злитим, якщо він позначений як нижчий пріоритет.
