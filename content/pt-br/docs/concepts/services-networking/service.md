...
A API Service, parte do Kubernetes, é uma abstração para ajudá-lo a expor grupos de Pods em uma rede. 
Cada objeto do tipo Service define um conjunto lógico de endpoints (geralmente estes endpoints são Pods) 
juntamente com uma policy sobre como tornar esses pods acessíveis.

Por exemplo, considere um backend de processamento de imagem stateless (sem estado) sendo executado com 
3 réplicas.  essas réplicas são fungíveis&mdash;frontends não se importam com qual backend
eles usam.  Embora os Pods que compõe o pool de backend possam mudar, os clientes
frontend não deveriam precisar estar cientes disso, nem deveriam precisar manter 
um rastreio do conjunto de seus próprios backends.

A abstração do Service permite essa desacoplamento.

O conjunto de Pods marcados por um Service geralmente é determinado 
por um {{< glossary_tooltip text="selector" term_id="selector" >}} que você define.
Para aprender mais a respeito de outras maneiras para definir Service endpoints,
veja [Services _without_ selectors](#services-without-selectors).

Se seu workload fala por HTTP, você pode optar por usar um
[Ingress](/docs/concepts/services-networking/ingress/) para controlar como o tráfego web 
alcança o seu workload.
Ingress não é um tipo Service, mas atua como uma ponto de entrada para o seu 
cluster. Um Ingress permite à você combinar suas regras de roteamento em um único recurso, 
desta maneira, você pode expor múltiplos componentes do seu workload que são executados 
separadamente em seu cluster, atrás de um único listener.

A API [Gateway](https://gateway-api.sigs.k8s.io/#what-is-the-gateway-api) para Kubernetes
fornece recursos extras além do Ingress e do Service. Você pode adicionar Gateway para o seu cluster -
isto é uma família de extensão de APIs implementadas usando
{{< glossary_tooltip term_id="CustomResourceDefinition" text="CustomResourceDefinitions" >}} -
em em seguida, use-as para configurar o acesso aos serviços de rede que estão em execução em seu cluster.
