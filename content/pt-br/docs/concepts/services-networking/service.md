...
feature:
  ...
  description: >
    Não há necessidade de modificar sua aplicação para usar um mecanismo de descoberta de serviço desconhecido. O Kubernetes fornece aos Pods seus próprios endereços IP e um único nome DNS para um conjunto de Pods, e pode equilibrar a carga entre eles.  
description: >-
  Expõe uma aplicação em execução no seu cluster atrás de um endpoint voltado
  para fora, mesmo quando a carga de trabalho estiver dividida em vários backends.
...
{{< glossary_definition term_id="service" length="short" prepend="No Kubernetes, um Serviço é" >}}

Um dos principais objetivos dos serviços no Kubernetes é que você não precisa modificar sua aplicação existente para usar o mecanismo de descoberta de serviço desconhecido. 
Você pode executar o código em Pods, seja um código projetado para um mundo nativo da nuvem ou uma aplicação mais antiga que você contêinerizou. 
Você usa um serviço para disponibilizar esse conjunto de Pods na rede para que os clientes possam interagir com ele.

Se você usar um {{< glossary_tooltip term_id="deployment" >}} para executar sua aplicação, esse deployment poderá criar e destruir Pods dinamicamente. 
De um momento para o outro, você não sabe quantos desses Pods estão funcionando e quantos estão saudáveis; você pode nem saber como esses Pods saudáveis são nomeados. 
Os {{< glossary_tooltip term_id="pod" text="Pods" >}} Kubernetes são criados e destruídos para corresponder ao estado desejado do seu cluster. 
Pods são recursos efêmeros (você não deve esperar que um Pod individual seja confiável e durável).

Cada Pod recebe seu próprio endereço IP (O Kubernetes espera que os plugins de rede garantam isso). 
Para um determinado deployment em seu cluster, o conjunto de Pods em execução em um determinado momento pode ser diferente do conjunto de Pods em execução dessa aplicação um momento depois.

Isso leva a um problema: se algum conjunto de Pods (chame-os de "backends") fornece funcionalidade a outros Pods (chame-os de "frontends") dentro do seu cluster, como os frontends descobrem e acompanham a qual endereço IP se conectar, para que o frontend possa usar a parte da carga de trabalho do backend?

Aqui entra os _Serviços_.

<!-- body -->

## Serviços no Kubernetes

A API de Serviço, parte do Kubernetes, é uma abstração para ajudá-lo a expor grupos de Pods em uma rede. 
Cada objeto de Serviço define um conjunto lógico de endpoints (geralmente esses endpoints são Pods), 
juntamente com uma política sobre como tornar esses Pods acessíveis.

Por exemplo, considere um backend stateless de processamento de imagem que esteja sendo executado com três réplicas. 
Essas réplicas são fungíveis &mdash; os frontends não se importam com qual backend eles usam. 
Embora os Pods atuais que compõem o conjunto de backend possam mudar, 
os clientes de frontend não devem precisar estar cientes disso, 
nem devem precisar acompanhar o conjunto de backends.

A abstração do Serviço permite esse desacoplamento.

O conjunto de Pods marcados por um Serviço geralmente é determinado por um {{< glossary_tooltip text="seletor" term_id="selector" >}} que você define. 
Para saber mais sobre outras maneiras de definir os endpoints do Serviço, consulte [Serviços _sem_ seletores](#services-without-selectors).

Se a sua carga de trabalho usa HTTP, você pode optar por usar um 
[Ingress](/pt-br/docs/concepts/services-networking/ingress/) 
para controlar como o tráfego da web atinge essa carga de trabalho. 
O Ingress não é um tipo de Serviço, mas atua como o ponto de entrada para o seu cluster. 
Um Ingress permite que você consolide suas regras de roteamento em um único recurso, 
para que você possa expor vários componentes de sua carga de trabalho, 
em execução separadamente em seu cluster, por trás de um único _listener_.

O API [Gateway](https://gateway-api.sigs.k8s.io/#what-is-the-gateway-api) 
para o Kubernetes fornece recursos extras além do Ingress e do Serviço. 
Você pode adicionar o Gateway ao seu cluster - é uma família de APIs de extensão, 
implementadas usando {{< glossary_tooltip term_id="CustomResourceDefinition" text="CustomResourceDefinitions" >}} 
- e depois usá-las para configurar o acesso aos serviços de rede que estão em execução no seu cluster.


### Descoberta de serviços nativos da nuvem

Se você usar as APIs do Kubernetes para descoberta de Serviços de sua aplicação, 
poderá consultar o {{< glossary_tooltip text="servidor de API" term_id="kube-apiserver" >}} para combinar EndpointSlices. 
O Kubernetes atualiza os EndpointSlices para um Serviço sempre que o conjunto de Pods em um Serviço alterar.

Para aplicações não nativas, o Kubernetes oferece maneiras de colocar uma porta de rede ou balanceador de carga entre a sua aplicação e os Pods de backend.

De qualquer forma, sua carga de trabalho pode usar esses mecanismos de 
[descoberta de serviços](#discovering-services) para encontrar o alvo ao qual deseja se conectar.

## Definindo um Serviço

Um Serviço é um {{< glossary_tooltip text="objeto" term_id="object" >}} 
(da mesma forma que um Pod ou um ConfigMap é um objeto). 
Você pode criar, visualizar ou modificar as definições de Serviço usando a API Kubernetes. 
Normalmente, você usa uma ferramenta como o `kubectl` para fazer essas chamadas de API para você.

Por exemplo, suponha que você tenha um conjunto de Pods, que cada um escute na porta TCP 9376, e seja rotulado como `app.kubernetes.io/name=MyApp`. 
Você pode definir um Serviço para publicar esse TCP listener:
...
A aplicação deste manifesto cria um novo Serviço chamado "my-service", 
que tem como alvo a porta 9376/TCP em qualquer Pod com a label `app.kubernetes.io/name: MyApp`.

O Kubernetes atribui a este Serviço um endereço IP (o IP do cluster), 
que é usado pelo mecanismo de endereço IP virtual. 
Para mais detalhes sobre esse mecanismo, leia 
[IPs Virtuais e Proxies de Serviço](/docs/reference/networking/virtual-ips/).

O controlador para esse Serviço verifica continuamente os Pods que correspondem ao seu seletor, 
para em seguida fazer as atualizações necessárias no conjunto de EndpointSlices no Serviço.

O nome de um objeto de Serviço válido deve ser um nome previsto na 
[RFC 1035](/pt-br/docs/concepts/overview/working-with-objects/names/#nomes-de-rótulo-da-rfc-1035).

{{< note >}}
Um Serviço pode mapear _qualquer_ porta de entrada para uma porta de destino (targetPort). 
Por padrão e por conveniência, a `targetPort` é definida para o mesmo valor que o campo `port`.
{{< /note >}}
...
### Serviços sem seletores {#services-without-selectors}
...
#### Acessando um serviço sem um seletor {#service-no-selector-access}
...
#### Excesso de capacidade nos endpoints
...
## Tipos de serviço  {#publishing-services-service-types}
...
#### Personalizando a porta {#nodeport-custom-port}
...
#### Reserve intervalos de Nodeport para evitar colisões ao atribuir portas
...
#### Configuração de endereço IP personalizado para serviço `type: NodePort` {#service-nodeport-custom-listen-address}
...
#### Balanceadores de carga com tipos de protocolo misto
...
#### Desabilitando a alocação NodePort do balanceador de carga {#load-balancer-nodeport-allocation}
...
#### Especificando a classe do balanceador de carga implementado {#load-balancer-class}
...
#### Suporte TLS na AWS {#ssl-support-on-aws}
...
#### Suporte ao protocolo PROXY na AWS
...
#### Registros de Acesso ELB na AWS
...
#### Suporte ao balanceador de carga de rede na AWS {#aws-nlb-support}
...
## Descobrindo serviços {#discovering-services}
...
## Mecanismo de endereçamento IP virtual
