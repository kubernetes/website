---
layout: blog
title: 'Kubernetes v1.31：通过缓存的一致性读加速集群性能'
date: 2024-08-15
...
Etcd 数据存储进行强一致性读取，这通常需要资源密集型仲裁读取操作。
...
-->
今天，Kubernetes 社区很高兴地宣布一个重大改进：**缓存的一致性读**，
已在 Kubernetes v1.31 中晋升至 Beta 阶段。

### 为什么一致性读取如此重要   {#why-consistent-reads-matter}
...
-->
一致性读取是确保 Kubernetes 组件准确了解最新集群状态的关键。
保证一致性读取对于保持 Kubernetes 操作准确性和可靠性至关重要，
使组件能够根据最新信息做出明智决策。
...
然而，直到现在,它无法直接支持一致性读取，因为没有机制保证缓存是最新的。
...
-->
**缓存的一致性读** 特性通过使用 Etcd 的
[进度通知](https://etcd.io/docs/v3.5/dev-guide/interacting_v3/#watch-progress)
机制来解决这一问题。这些通知会向监视缓存说明其数据与 Etcd 相比有多新。
当发出一致性读取请求时，系统会首先检查监视缓存是否为最新状态。
...
-->
* **降低 Etcd 负载：** Kubernetes v1.31 可以将部分工作从 Etcd 转移，
  为其他关键操作释放资源。
...
-->
**5千节点扩展性测试结果：** 在最近针对 5,000 节点集群的扩展性测试中，
启用缓存的一致性读带来了显著提升：
...
随着晋升至 Beta 版，缓存的一致性读特性已默认启用，为所有使用受支持 Etcd
...
是体验缓存的一致性读优势的最简单方法。如果有任何问题或反馈，不要犹豫，
随时联系 Kubernetes 社区。
...
-->
**告诉我们** **缓存的一致性读** **如何改善了你的 Kubernetes 体验！**  
