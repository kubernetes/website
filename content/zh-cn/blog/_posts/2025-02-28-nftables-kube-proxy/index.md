...
-->
在 Kubernetes 1.29 中引入了一种新的Alpha 特性：kube-proxy 的 nftables 模式。
...
table ip kube-proxy {
	...
	map service-ips {
        ...
        # （此规则表示，"从目标 IP 地址、第 4 层协议和目标端口构建一个元组；
        # 在 'service-ips' 中查找该元组；如果找到匹配项，则执行关联的判决。"）
	chain services {
		ip daddr . meta l4proto . th dport vmap @service-ips
	}
...
-->
但请注意 iptables 和 nftables 图表之间垂直轴的巨大差异！
...
这往往使得用户无法将集群扩展到如此规模：当 Service 及其端点发生变化时，kube-proxy
更新 iptables 规则所需的时间。
...
更新的规模仅与自上次同步以来发生变化的 Service 和端点数量成 **O(n)** 关系，而与总的 Service 和端点数量无关。
...
-->
## 为什么暂时可能不选择 nftables？  {#why-not-nftables}

尽管如此，仍有几个原因可能导致你目前不希望立即使用 nftables 后端。
...
此外，由于早期版本的 `nft` 命令行工具存在 BUG，不应在主机文件系统中运行旧版本（早于 1.0.0）的
`nft` 节点上以 nftables 模式运行 kube-proxy（否则 kube-proxy 对 nftables
的使用可能会影响系统上其他对 nftables 的使用）。
...
此外，尚未更新的可观测性和监控工具在 nftables 模式下可能会为 kube-proxy
提供较少的数据，相较于 iptables 模式下的情况。
...
[kube-proxy 文档]: https://kubernetes.io/docs/reference/networking/virtual-ips/#migrating-from-iptables-mode-to-nftables
...
如果你使用 kubeadm 设置集群，kubeadm 文档解释了[如何向 `kubeadm init` 传递 `KubeProxyConfiguration`]。
...
在以 nftables 模式重新启动时，kube-proxy 会删除任何现有的 iptables 或 ipvs 规则；
同样，如果你之后切换回 iptables 或 ipvs 模式，它将删除任何现有的 nftables 规则。）

[如何向 `kubeadm init` 传递 `KubeProxyConfiguration`]: /docs/setup/production-environment/tools/kubeadm/control-plane-flags/#customizing-kube-proxy  
[通过 `kind` 部署基于 nftables 的集群]: https://kind.sigs.k8s.io/docs/user/configuration/#kube-proxy-mode
...
如上所述，虽然 nftables 现在是 kube-proxy 最佳的模式，但它还不是默认模式，
...
（理论上，IPVS 模式还具有使用其他 IPVS 功能的优势，例如替代的"调度器"来平衡端点。
但实际上，这并不太有用，因为 kube-proxy 在每个节点上独立运行，每个节点上的 IPVS
调度器无法与其他节点上的代理共享状态，从而阻碍了更智能地平衡流量的努力。）
...
- "[KEP-3866：添加了一个基于 nftables 的 kube-proxy 后端]"记录了此新特性的历史。

- "[表的转变：Kubernetes 告别 IPTables]"，来自 2024 年
  KubeCon/CloudNativeCon 北美大会，讨论了将 kube-proxy 和 Calico 从 iptables 迁移到 nftables 的过程。

- "[从可观测性到性能优化]"，同样来自 2024 年 KubeCon/CloudNativeCon 北美大会。
 （kube-proxy 延迟数据来源于此；[图表的原始数据]也可用。）
