---
layout: blog
title: "Gateway API v1.3.0：流量复制、CORS、网关合并和重试预算的改进"
...
budgets.
-->
Gateway API v1.3.0 为 **Standard** 通道（Gateway API 的正式发布通道）带来了一个新功能：**基于百分比的流量复制**，
并引入了三个新的实验性功能：
- 跨源资源共享（CORS）过滤器
...
-->
## 升级至 Standard 通道 {#graduation-to-standard-channel}
...
-->
对于 Gateway API 的功能来说，升级到 Standard 通道是一个重要的里程碑。
被纳入 Standard 发布通道表明我们对该 API 接口的稳定性具有高度信心，并且承诺向后兼容。
当然，与任何其他 Kubernetes API 一样， Standard 通道中的功能仍可通过向后兼容的方式不断演进。
...
它可用于评估请求扩展对应用程序性能的影响，而不会影响对客户端的响应。
...
-->
以下是一个示例，将 42% 发送到 "foo-v1" 的流量复制到 "foo-v2"：
...
-->
你也可以通过调整分数来实现部分流量复制。
以下是一个示例，将每 1000 个发送到 "foo-v1" 的请求中的 5 个复制到 "foo-v2"。
...
-->
## 对 Experimental 通道的补充 {#additions-to-Experimental-channel}
...
-->
实验通道是 Gateway API 用于试验新功能的渠道，以便在功能成熟之前积累足够信心，
再将其升级为标准通道功能。
请注意：实验通道可能包含后续会被更改或移除的功能。
...
-->
从 v1.3.0 版本开始，为了区分实验通道资源和 Standard 通道资源，
所有新的实验性 API 类型都带有 "**X**" 前缀。
出于同样的原因，实验性资源现在被添加到 API 组 `gateway.networking.x-k8s.io`，
而不是 `gateway.networking.k8s.io`。
请注意，使用新的实验通道资源意味着它们可以与 Standard 通道资源共存，
若要将这些资源迁移到 Standard 通道，则需要使用 Standard 通道的名称和 API 组
...
要使用实验性 API 类型，你需要从下面列出的位置安装实验通道 Gateway API YAML 文件。
...
允许网页从与提供网页的域不同的源（域、方案或端口）访问受限资源。
...
-->
要使用实验性 CORS 过滤，你需要安装[实验通道 Gateway API HTTPRoute yaml](https://github.com/kubernetes-sigs/gateway-api/blob/main/config/crd/experimental/gateway.networking.k8s.io_httproutes.yaml)。
...
-->
在这种情况下，网关返回一个 **origin header** 为 "*"，这意味着请求的资源可以从任何源引用；
...
* `allowMethods`（允许的方法）
* `allowHeaders`（允许的头信息）
* `allowCredentials`（允许的凭证）
* `exposeHeaders`（暴露的头信息）
* `maxAge`（最大年龄）
...
-->
### XListenerSets（监听器和网关合并的标准化机制）{#XListenerSet}
...
-->
GEP-1713：[ListenerSets - 合并多个网关的标准机制](https://github.com/kubernetes-sigs/gateway-api/pull/3213)
...
-->
此版本添加了一个新的实验性 API 类型 XListenerSet，它允许将 **listeners** 的共享列表附加到一个或多个父网关。
此外，它还扩展了现有的建议，即 Gateway API 实现可以合并来自多个网关对象的配置。它还包括：
...
-->
- 向网关的 `.spec` 添加了一个新字段 `allowedListeners`。
  `allowedListeners` 字段定义了从哪些命名空间选择允许附加到该网关的 XListenerSets：
  相同、全部、无或基于选择器。
...
-->
要使用实验性 XListenerSet，你需要安装[实验通道 Gateway API XListenerSet yaml](https://github.com/kubernetes-sigs/gateway-api/blob/main/config/crd/experimental/gateway.networking.x-k8s.io_xlistenersets.yaml)。
...
这用于在达到配置的阈值后限制额外的客户端重试。
配置预算时，可以指定可能由重试组成的活动请求的最大百分比，
以及在计算重试阈值时考虑请求的时间间隔。
...
-->
要使用实验性重试预算（Retry budgets），你需要安装[实验通道 Gateway API XBackendTrafficPolicy yaml](https://github.com/kubernetes-sigs/gateway-api/blob/main/config/crd/experimental/gateway.networking.x-k8s.io_xbackendtrafficpolicies.yaml)。
...
截至本文撰写时，已有四个实现符合 Gateway API v1.3 实验通道功能。按字母顺序排列：
...
and service mesh.
-->
想知道何时会添加功能？有很多机会参与并帮助定义 Kubernetes API 路由的未来，包括入口和服务网格。
...
-->
* 试用[现有的网关控制器](https://gateway-api.sigs.k8s.io/implementations/)之一。
...
-->
维护者要感谢所有为 Gateway API 做出贡献的人，无论是通过提交代码、讨论、想法还是一般支持。
没有这个专注和活跃的社区的支持，我们永远无法取得这样的进展。
