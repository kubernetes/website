...
我们建议迁移到其中一个替代方案。考虑[迁移到 Gateway API](https://gateway-api.sigs.k8s.io/guides/)，
...
Ingress NGINX 是一个广泛使用的 Kubernetes Ingress 控制器，它通过
NGINX 提供了反向代理和负载均衡功能。它使得外部访问内部服务成为可能，
并支持多种特性如 TLS 终止、名称基于的虚拟主机以及对 HTTP/2 的支持。

随着 Kubernetes 生态系统的演进，SIG Network 和社区决定推动 Gateway API
作为 Ingress 的继任者。Gateway API 提供了更强大和灵活的功能集，
旨在解决 Ingress 在复杂场景下的限制。
...
尽管该项目在用户中非常受欢迎，但 Ingress NGINX 在维护者数量不足或勉强足够方面一直存在问题。
