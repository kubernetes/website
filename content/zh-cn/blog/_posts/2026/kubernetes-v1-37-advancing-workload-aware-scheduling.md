...
-->
除新增这些 API 外，v1.37 还引入了一组新的**控制器集成 API** 和 `workloadbuilder` Go 库，
...
[PodGroup](/zh-cn/docs/concepts/workloads/podgroup-api/) API 和**编组调度**正式进阶到 Beta。
...
-->
在 Kubernetes v1.37 中，用于**工作负载感知抢占**的独立
...
* **递归评估**：调度器从根 CompositePodGroup 开始向下遍历层次结构，
  直至叶级 PodGroup 对象。
...
* **全有或全无调度**：一旦找到满足根 CompositePodGroup 要求的有效子组组合，
...
* **`single`**：允许 CompositePodGroup 中的各个子组被独立抢占和干扰。
  未设置 `disruptionMode` 时采用此行为。
* **`all`**：在整个 CompositePodGroup 层次结构中强制执行“全有或全无”的干扰语义。
...
-->
在分层调度期间，`kube-scheduler` 以**自顶向下**的方式解析多级拓扑约束。
具体而言，调度子组时所考虑的拓扑域，
...
-->
**`workloadbuilder` 库**会将这些意图转换为调度对象。
...
-->
基于新的**控制器集成** API，Job API 现在提供显式的 `.spec.scheduling` 字段，
因此你可以声明 Job 应如何调度，
...
同时 `DRAWorkloadResourceClaims` 特性门控处于**禁用**状态时，
...
-->
* **Workload 和 PodGroup API 进阶到 GA：**
  将工作负载感知调度的核心基础巩固为稳定的 Kubernetes API。
* **拓扑感知调度（TAS）和 CompositePodGroup（CPG）进阶到 Beta：**
