...
此次迁移非常复杂，部分逻辑必须从核心组件中提取，并构建了四个新的子系统。
...
以便云控制器管理器可以优先处理该节点。初始的 Node 缺少与云提供商相关的信息，
...
由于逻辑已移至云控制管理器，这可能会导致那些不将控制器管理器作为控制平面其他组件
（通常以静态 Pod、独立二进制文件或具有容忍污点功能的 DaemonSet/Deployment，并使用 `hostNetwork`）
部署的 Kubernetes 架构在集群引导过程中出现[鸡和蛋问题][chicken-and-egg]（更多内容见下文）。
...
如上所述，在引导过程中，云控制管理器可能无法被调度，
因此集群将无法正确初始化。以下是此问题可能表现的几个具体示例及其根本原因。

这些示例假设你使用 Kubernetes 资源（例如 Deployment、DaemonSet 或类似资源）来控制
云控制管理器的生命周期。由于这些方法依赖于 Kubernetes 来调度云控制管理器，
因此必须确保其能够正确调度。
...
的不可调度污点。由于云控制器管理器负责移除该不可调度污点，这可能会导致由 Kubernetes
...
-->
当容器网络尚未在该节点上初始化时，可能导致 Node 资源具有此污点。由于云控制器管理器负责为
Node 资源添加 IP 地址，而容器网络控制器需要这些 IP 地址来正确配置容器网络，因此在某些情况下，
节点可能会永久处于未准备好且未初始化的状态。

这种情况的发生原因与第一个示例类似，但在此情况下，`node.kubernetes.io/not-ready`
污点使用了 no execute 效果，从而导致云控制器管理器无法在带有该污点的节点上运行。
...
对于在同一集群中运行的云控制器管理器，它们所管理的集群需要特别注意。
...
1. 使用主机网络模式，而不是 Pod 网络：在大多数情况下，云控制器管理器需要与基础设施相关的 API 服务端点进行通信。
   ...
   这还将消除对网络插件的依赖。这可以确保云控制器能够访问基础设施端点（始终根据你的基础设施提供商的说明检查网络配置）。
2. 使用可扩展的资源类型。`Deployment` 和 `DaemonSet` 对于控制云控制器的生命周期非常有用。
   它们允许轻松运行多个副本以实现冗余，并利用 Kubernetes 调度来确保在集群中的正确放置。
...
3. 将控制器管理器容器定位到控制平面。可能存在一些需要在控制平面之外运行的其他控制器
  （例如，Azure 的节点管理器控制器），但控制器管理器本身应部署到控制平面。
   使用节点选择器或亲和性配置将云控制器的调度定向到控制平面，以确保它们运行在受保护的空间中。
   云控制器在集群中添加和移除节点时至关重要，因为它们构成了 Kubernetes 与物理基础设施之间的桥梁。
   将它们运行在控制平面上有助于确保它们以与其他核心集群控制器相似的优先级运行，并且与非特权用户工作负载有所隔离。
   1. 值得注意的是，使用反亲和性配置以防止云控制器运行在同一主机上也非常有用，这可以确保单个节点故障不会影响云控制器的性能。
<!--
...
for these scenarios.
-->
4. 确保容忍规则允许操作。在云控制器容器的清单中使用容忍规则，以确保其能够调度到正确的节点，
...
对于不在其所管理的集群上运行的云控制器管理器（例如，在单独集群上的托管控制平面上），
...
自动扩展。运行多个云控制器管理器副本是确保高可用性和冗余的良好实践，但这并不会提高性能。
