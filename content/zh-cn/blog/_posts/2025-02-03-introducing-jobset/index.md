...
而这片碎片化的领域中每个现有的解决方案都存在某些限制，
使得它们在分布式机器学习训练方面并非最优选择。

例如，KubeFlow 训练操作符为不同的框架定义了自定义 API（例如 PyTorchJob、TFJob、MPIJob 等）。
...
-->
另一方面，Job API 弥补了运行批处理工作负载的许多空白，包括索引完成模式（Indexed completion mode）、
...
-->
- 启动顺序：某些任务需要特定的 Pod 启动顺序；有时需要驱动程序（driver）首先启动（例如 Ray 或 Spark），
  而在其他情况下，则期望工作节点（workers）在驱动程序启动之前就绪（例如 MPI）。
...
它通过 ReplicatedJob 的抽象来管理子 Job，其中 ReplicatedJob 本质上是一个带有指定副本数量的
...
解决上述问题的其他一些关键 JobSet 功能包括：

- **复制任务（Replicated Jobs）**：在现代数据中心中，硬件加速器（如 GPU 和 TPU）通常以同质加速器岛的形式分配，
...
-->
- **自动无头服务的创建、配置和生命周期管理**：默认情况下，通过 Pod 主机名启用
  Pod 到 Pod 的通信，并通过无头服务的自动配置和生命周期管理来支持这一功能。

- **可配置的成功策略**：JobSet 提供了可配置的成功策略，这些策略针对特定的 ReplicatedJobs，
  并可通过操作符指定"任一"或"全部"子任务。例如，你可以将 JobSet 配置为仅在属于 "worker"
