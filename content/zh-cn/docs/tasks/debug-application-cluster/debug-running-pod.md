...
* 你的 {{< glossary_tooltip text="Pod" term_id="pod" >}} 应该已经被调度并正在运行中，
  如果你的 Pod 还没有运行，请参阅
  [应用问题排查](/zh-cn/docs/tasks/debug-application-cluster/debug-application/)。
...
若要了解更多内容，可查看[获取正在运行容器的 Shell](/zh-cn/docs/tasks/debug-application-cluster/get-shell-running-container/)。
