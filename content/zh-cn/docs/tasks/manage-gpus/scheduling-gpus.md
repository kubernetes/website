...
-->
Kubernetes 支持对节点上的 GPU（图形处理单元）进行管理，目前处于**实验**状态。

本页介绍了用户如何使用 GPU，并描述了实施中的一些限制。
...
来自 GPU 厂商的对应的设备插件。以下是供应商说明的一些链接：
...
-->
安装插件后，你的集群会公开一个自定义的可调度资源，
例如 `amd.com/gpu` 或 `nvidia.com/gpu`。

你可以通过请求 GPU 资源来使用 GPU 设备，就像你为 CPU
和内存所做的那样。
但是，在如何指定自定义设备的资源要求方面存在一些限制。
...
spec:
  ...
  containers:
    - name: example-vector-add
      ...
      resources:
        limits:
          gpu-vendor.example/example-gpu: 1 # 申请 1 个 GPU
```
...
you specified.
-->
这能够确保 Pod 能够被调度到你所指定类型的 GPU 的节点上去。
