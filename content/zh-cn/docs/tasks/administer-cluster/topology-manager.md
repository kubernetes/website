...
* `prefer-closest-numa-nodes` (beta, invisible by default, `TopologyManagerPolicyOptions` and `TopologyManagerPolicyAlphaOptions` feature gates have to be enabled)
...
* `prefer-closest-numa-nodes`（Beta，默认可见，`TopologyManagerPolicyOptions` 和
  ...
  `prefer-closest-numa-nodes` 策略选项在 Kubernetes {{< skew currentVersion >}} 为 Beta 级别特性。
