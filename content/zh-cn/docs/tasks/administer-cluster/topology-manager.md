...
-->
拓扑管理器是一个 kubelet 组件，扮演信息源的角色，以便其他 kubelet
组件可以做出与拓扑结构相对应的资源分配决定。
...
* `TopologyManagerPolicyBetaOptions` default disabled. Enable to show beta-level options.
...
* `TopologyManagerPolicyBetaOptions` 默认禁用。启用以显示 Beta 级别选项。
...
* `prefer-closest-numa-nodes`（Beta，默认可见，`TopologyManagerPolicyOptions` 和
  ...
  `prefer-closest-numa-nodes` 策略选项在 Kubernetes {{< skew currentVersion >}} 是 Beta 级别特性。
...
to `limits`.
-->
此 Pod 独立使用 CPU 请求量，以 `Guaranteed` QoS 类运行，因为其 `requests`
值等于 `limits` 值。
...
to `limits`.
-->
此 Pod 和其他资源共享 CPU 请求量，以 `Guaranteed` QoS 类运行，因为其 `requests`
值等于 `limits` 值。
...
assignments.
-->
基于此信息，拓扑管理器将为 Pod 计算最佳提示并存储该信息，
并且供提示提供程序在进行资源分配时使用。
