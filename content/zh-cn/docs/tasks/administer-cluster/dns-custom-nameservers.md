...
DNS 是使用 **插件管理器** 的[集群插件](http://releases.k8s.io/master/cluster/addons/README.md)自动启动内置的 Kubernetes 服务。
...
-->
CoreDNS 服务在 `metadata.name` 字段中被命名为 `kube-dns`。
目的是确保与依赖于用于解析集群内部地址的旧版“kube-dns”服务名称。
...
-->
如果你在使用 Deployment 运行 CoreDNS，
则该 Deployment 通常会向外暴露为一个具有静态 IP 地址 Kubernetes 服务。
...
CoreDNS 是模块化且可插拔的 DNS 服务器，插件增加了新功能。

可以通过维护 [Corefile](https://coredns.io/2017/07/23/corefile-explained/) 来配置 CoreDNS 服务器，即 CoreDNS 配置文件，来定制其行为。
作为集群管理员，你可以修改 CoreDNS Corefile 的 {{< glossary_tooltip text="ConfigMap" term_id="configmap" >}}，以更改服务发现的工作方式。
...
* [health](https://coredns.io/plugins/health/)：在 `http://localhost:8080/health` 处提供 CoreDNS 的健康报告。
  在这个扩展语法中，`lameduck` 将使进程不健康，然后等待 5 秒钟，然后再关闭该进程。
* [ready](https://coredns.io/plugins/ready/)：在端口 8181 上提供的一个 HTTP 末端，
  当所有能够表达自身就绪的插件都已就绪时，在此末端返回 200 OK。
...
-->
* [kubernetes](https://coredns.io/plugins/kubernetes/)：CoreDNS 会回复 DNS 查询基于服务和 Pod 的 IP。
  你可以找到[更多细节](https://coredns.io/plugins/kubernetes/)关于 CoreDNS 网站上的这个插件。
  - `ttl` 允许你为响应设置自定义 TTL。默认值为 5 秒。
    允许的最小 TTL 为 0 秒，最大值为 3600 秒。
    将 TTL 设置为 0 将防止记录被缓存。
  - 提供了 `pods insecure` 选项以向后兼容 `kube-dns`。
  - 你可以使用 `pods encrypted` 选项，该选项仅在存在 Pod 时，
    才返回 A 记录在具有匹配 IP 的同一命名空间中。
  - 如果你不使用 Pod 记录，可以使用 `pods disabled` 选项。
...
* [prometheus](https://coredns.io/plugins/metrics/)：CoreDNS 的指标可在
  [Prometheus](https://prometheus.io/) 格式的 `http://localhost:9153/metrics`
  （也称为 OpenMetrics）。
* [forward](https://coredns.io/plugins/forward/)：任何不在 Kubernetes 内的查询集群域被转发到预定义的解析器 (/etc/resolv.conf)。
* [cache](https://coredns.io/plugins/cache/)：启用前端缓存。
* [loop](https://coredns.io/plugins/loop/)：检测简单的转发循环和
  如果发现循环，则停止 CoreDNS 进程。
* [reload](https://coredns.io/plugins/reload)：允许自动重新加载已更改的核心文件。
  编辑 ConfigMap 配置后，等待两分钟让你的更改生效。
* [loadbalance](https://coredns.io/plugins/loadbalance)：这是一个轮转式 DNS 负载平衡器随机分配中 A、AAAA 和 MX 记录的顺序。
...
-->
CoreDNS 不支持存根域和名称服务器的 FQDN（例如：“ns.foo.com”）。
在转换期间中，CoreDNS 配置中将省略所有 FQDN 域名服务器。
{{< /note >}}
