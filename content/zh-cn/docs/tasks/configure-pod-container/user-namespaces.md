...
-->
本页展示如何为 Pod 配置用户命名空间。可以将容器内的用户与主机上的用户隔离开来。
...
进程在用户命名空间内部拥有执行操作的全部特权，但在用户命名空间外部并没有执行操作的特权。
...
[有些安全脆弱性问题][KEP-vulns]被评为 **HIGH** 或 **CRITICAL**，但当用户命名空间被启用时，
它们是无法被利用的。相信用户命名空间也能减轻一些未来的漏洞影响。
...
-->
在不使用用户命名空间的情况下，对于以 root 用户运行的容器而言，发生容器逃逸时，
...
-->
在用户命名空间原来仅支持无状态的 Pod 时，启用用户命名空间的特性门控先前被命名为 `UserNamespacesStatelessPodsSupport`。
...
-->
你所使用的集群**必须**包括至少一个符合[要求](/zh-cn/docs/concepts/workloads/pods/user-namespaces/#before-you-begin)的节点，
以便为 Pod 配置用户命名空间。
...
-->
如果你有混合节点，并且只有部分节点支持为 Pod 配置用户命名空间，
你还需要确保配置了用户命名空间的 Pod
...
-->
## 运行一个使用用户命名空间的 Pod {#create-pod}

<!--
A user namespace for a pod is enabled setting the `hostUsers` field of `.spec`
to `false`. For example:
-->
为一个 Pod 启用用户命名空间需要设置 `.spec` 的 `hostUsers` 字段为 `false`。例如：
...
-->
2. 进入 Pod 并运行 `readlink /proc/self/ns/user`：
...
如果你在用户命名空间中运行 kubelet，则需要将在 Pod 中运行命令的输出与在主机中运行的输出进行比较：
