...
当 Pod 的 `Ready` [条件](/zh-cn/docs/concepts/workloads/pods/pod-lifecycle/#pod-conditions) 为
true 时，Pod 被认为是就绪的。若 Pod 尚未就绪，会被从 Service 的负载均衡器中剔除。
当 Pod 的节点的 `Ready` 条件不为 true 时、当 Pod 的某个 `readinessGates` 为 false
时，或者当 Pod 中有任何一个容器未就绪时，Pod 的 `Ready` 条件为 false。
