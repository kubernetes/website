...
1. 当工作负载未如预期启动时，从 Job 到 Pod 再到 ResourceClaim 逐步深入检查，
   并使用 `kubectl describe` 检查每个级别的对象，
   查看是否有任何状态字段或事件可以解释工作负载为何没有启动。
2. 当创建 Pod 失败并显示 `must specify one of：resourceClaimName, resourceClaimTemplateName` 时，
   检查 `pod.spec.resourceClaims` 中的所有条目是否正好设置了一个这些字段。如果是这样，
   那么可能是集群安装了一个与 Kubernetes < 1.32 的 API 构建的 Pod 变更的 Webhook。
   请与你的集群管理员合作检查这个问题。
