...
大多数 kubeadm 命令支持 `--config` 标志，允许从磁盘传递配置文件。
...
-->
你可以运行 [`kubeadm kubeconfig user`](/docs/reference/setup-tools/kubeadm/kubeadm-kubeconfig/#cmd-kubeconfig-user)
...
-->
这分为发现（让该节点信任 Kubernetes 的 API 服务器）和 TLS 引导
（让 Kubernetes 的 API 服务器信任该节点）。
...
-->
1. 如果你要加入一个 Windows 节点，将跳过 Linux 特定的控制。
...
-->
你可以通过在命令行上传递 `--discovery-token-unsafe-skip-ca-verification`
...
`kubeadm upgrade` 包含处理由 kubeadm 创建的 Kubernetes 集群升级的子命令。
你必须在控制平面节点上运行 `kubeadm upgrade apply`（你可以选择其中一个节点），以启动升级过程。
...
[`kubeadm upgrade phase`](/docs/reference/setup-tools/kubeadm/kubeadm-upgrade-phase/)。
...
你可以在运行 `kubeadm upgrade apply` 之前选择运行 `kubeadm upgrade plan`。
`plan` 子命令会检查有哪些版本可以升级，并验证你当前的集群是否可升级。
...
这会显示将对控制平面节点的现有静态 Pod 清单应用哪些差异。
更详细的做法是运行 `kubeadm upgrade apply --dry-run`
...
`kubeadm upgrade apply` 为所有节点的升级做准备，同时也会升级在其上运行的控制平面节点。它执行的步骤包括：
...
-->
- 类似于 `kubeadm init` 和 `kubeadm join`，运行预检检查，确保容器镜像已下载且集群处于可升级的良好状态。
- 升级位于 `/etc/kubernetes/manifests` 磁盘上的控制平面清单文件，并在文件发生更改时等待 kubelet 重启组件。
...
- 配置引导令牌和 `cluster-info` ConfigMap 以用于 RBAC 规则。这与 `kubeadm init`
  阶段相同，确保集群继续支持使用引导令牌加入的节点。
- 如果集群中所有现有的 kube-apiserver 已经升级到目标版本，则有条件地升级 kube-proxy 和 CoreDNS 插件。
- 执行任何升级后的任务，例如清理特定发布版本中废弃的功能。
...
- Performs any post-upgrade tasks, such as cleaning up deprecated features which are release specific.
-->
- 类似于 `kubeadm upgrade apply`，运行预检检查。
- 对于控制平面节点，升级位于 `/etc/kubernetes/manifests` 磁盘上的控制平面清单文件，并在文件发生更改时等待 kubelet 重启组件。
- 在 `/var/lib/kubelet/config.yaml` 中为此节点写入更新的 kubelet 配置。
- （针对控制平面节点）如果集群中所有现有的 API 服务器已经升级到目标版本，则有条件地升级
  kube-proxy 和 CoreDNS {{< glossary_tooltip text="插件" term_id="addons" >}}。
- 执行任何升级后的任务，例如清理特定发布版本中废弃的特性。
