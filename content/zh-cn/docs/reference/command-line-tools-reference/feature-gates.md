...
- `AdvancedAuditing`：启用[高级审计功能](/zh-cn/docs/tasks/debug-application-cluster/audit/#advanced-audit)。
...
- `DynamicKubeletConfig`：启用 kubelet 的动态配置。请参阅
  [重新配置 kubelet](/zh-cn/docs/tasks/administer-cluster/reconfigure-kubelet/)。
...
- `RotateKubeletClientCertificate`：在 kubelet 上启用客户端 TLS 证书的轮换。
  更多详细信息，请参见
  [kubelet 配置](/zh-cn/docs/reference/command-line-tools-reference/kubelet-tls-bootstrapping/#kubelet-configuration)。
