...
- `AdvancedAuditing`：启用[高级审计](/docs/tasks/debug/debug-cluster/audit/#advanced-audit)
...
- `CSIMigrationGCE`: Enables shims and translation logic to route volume
  ...
  enabled.
-->
- `CSIMigrationGCE`：启用填充程序和转换逻辑，将卷操作从 GCE-PD 树内插件路由到 PD CSI 插件。
  支持回退到树内 GCE 插件，以对禁用该特性或未安装和配置 PD CSI 插件的节点进行挂载操作。
  不支持回退配置操作，因为必须安装和配置 CSI 插件。需要启用 CSIMigration 特性参数。
...
-->
- `CSIStorageCapacity`：使 CSI 驱动程序能够发布存储容量信息，并让 Kubernetes
  调度程序在调度 Pod 时使用该信息，参阅[存储容量](/zh-cn/docs/concepts/storage/storage-capacity/)。
  有关更多详细信息，请参阅 [`csi` 卷类型](/zh-cn/docs/concepts/storage/volumes/#csi)文档。
...
- `DelegateFSGroupToCSIDriver`：如果 CSI 驱动程序支持，则通过 NodeStageVolume 和
  NodePublishVolume CSI 调用传递 `fsGroup`，将 `fsGroup` 从 Pod 的 `securityContext`
  应用到驱动程序的角色委派给驱动程序。

- `DevicePlugins`：在节点上启用基于 [Device Plugin](/zh-cn/docs/concepts/extend-kubernetes/compute-storage-net/device-plugins/)
  的资源配置。

- `DisableAcceleratorUsageMetrics`：[禁用 kubelet 收集的加速器指标](/zh-cn/docs/concepts/cluster-administration/system-metrics/#disable-accelerator-metrics)

- `DryRun`：启用服务器端[试运行](/zh-cn/docs/reference/using-api/api-concepts/#dry-run)请求，
  以便无需正式提交即可测试验证、合并和变更。
...
- `EndpointSliceTerminateCondition`：启用 EndpointSlice 的 `termination` 和 `serving` 条件字段。
...
- `MixedProtocolLBService`：允许在同一 `LoadBalancer` 类型 Service 实例中使用不同的协议。
...
- `PodHasNetworkCondition`：使 kubelet 能够在 Pod 上标记 [PodHasNetwork](/zh-cn/docs/concepts/workloads/pods/pod-lifecycle/#pod-has-network)
  条件。在 1.28 中，它被重命名为 `PodReadyToStartContainersCondition`。
...
-->
- `ServiceIPStaticSubrange`：为 Service 启用 ClusterIP 分配策略，从而细分 ClusterIP 范围。
  动态分配的 ClusterIP 地址将优先从较高范围分配，允许用户从较低范围分配静态 ClusterIP，
  降低冲突风险。有关更多详细信息，请参阅[避免冲突](/docs/reference/networking/virtual-ips/#avoiding-collisions)。

- `ServiceInternalTrafficPolicy`：为 Service 启用 `internalTrafficPolicy` 字段。
...
- `UserNamespacesStatelessPodsSupport`：为无状态 Pod 启用用名字空间支持，
  该标志在新版本中已重命名为 `UserNamespacesSupport`。
