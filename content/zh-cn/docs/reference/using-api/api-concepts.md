...
此外，[API 聚合](/zh-cn/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/)
和[定制资源](/zh-cn/docs/concepts/extend-kubernetes/api-extension/custom-resources/)
...
[Server Side Apply](/docs/reference/using-api/server-side-apply/).
-->
从 Kubernetes v1.18 开始，可以启用[服务器端应用](/zh-cn/docs/reference/using-api/server-side-apply/)功能
特性，启用该特性后，控制面会跟踪所有新创建的对象的托管字段。服务器端应用提供了一种简洁的模式来管理字段冲突，提供服务器端的 `Apply` 和 `Update` 操作，并取代了
`kubectl apply` 的客户端功能。有关该特性的详细描述，请参见[服务器端应用](/zh-cn/docs/reference/using-api/server-side-apply/)章节
