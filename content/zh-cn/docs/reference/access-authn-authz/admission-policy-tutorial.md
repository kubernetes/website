...
-->
本页面提供声明式准入策略的概述，此策略允许你使用通用氏表达语言（CEL）
来验证或变更资源。
...
通过使用通用表达氏语言（CEL）来声明策略规则，这些策略直接在 API 服务器内进行评估。
...
-->
`spec.validations` 包含使用[通用表达氏语言（CEL）](https://github.com/google/cel-spec)的 CEL 表达式，
