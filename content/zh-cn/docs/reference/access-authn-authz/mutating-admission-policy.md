...
-->
本页概述了 **MutatingAdmissionPolicy（可变准入策略）**。
...
可变准入策略提供了一种声明式的、进程内的替代方案来变更准入 Webhook。
...
-->
可变准入策略使用通用表达语言（Common Expression Language，CEL）来声明对资源的变更。
...
可变准入策略是高度可配置的，使得策略作者在定义策略时能够对集群管理员所需的资源执行参数化设置并限定作用域。
...
-->
- **参数资源**向 MutatingAdmissionPolicy 提供信息，使其成为一个具体的语句
 （可以理解为：“将 `owner` 标签设置为类似 `company.example.com` 的值”）。
  参数资源指的是 Kubernetes API 中可用的 Kubernetes 资源。这些资源可以是内置类型或扩展，
  例如 {{< glossary_tooltip term_id="CustomResourceDefinition" text="CustomResourceDefinition" >}}（CRD）。
  例如，你可以使用 ConfigMap 作为参数。

- MutatingAdmissionPolicyBinding 将上述（MutatingAdmissionPolicy 和参数）资源关联在一起，并提供作用域。
  如果你只想为 `Pods` 设置 `owner` 标签，而不想为其他 API 类别设置标签，则绑定操作发生在你指定此变更的地方。
...
如果 MutatingAdmissionPolicy 不需要通过参数进行配置，只需在
...
-->
## 开始使用 MutatingAdmissionPolicy   {#getting-started-with-mutating-admission-policies}

可变准入策略是集群控制平面的一部分。你需要非常谨慎地编写和部署这些策略。
下文说明了如何快速体验可变准入策略。
...
-->
### 创建 MutatingAdmissionPolicy   {#create-a-mutating-admission-policy}

以下是一个 MutatingAdmissionPolicy 的示例。
此策略会将新建的 Pod 变更为添加一个边车容器（如果以前没有边车容器的话）。
...
-->
`.spec.mutations` 字段由一系列对资源补丁求值评估的表达式组成。
发出的补丁可以是[应用配置](#patch-type-apply-configuration)或 [JSON 补丁](#patch-type-json-patch)。
你不能指定一个空的变更列表。在所有表达式完成求值后，API 服务器将这些变更应用到正在通过准入传递的资源。
...
-->
要配置可变准入策略用于某个集群中，绑定是必需的。
只有与策略的 `spec.name` 相匹配的 `spec.policyName` 被引用时存在相应绑定时，MutatingAdmissionPolicy 才会生效。

一旦绑定和策略都被创建，任何与策略的 `spec.matchConditions` 相匹配的资源请求都会触发所定义的变更集。
...
参数资源允许策略配置与其定义分隔开。策略可以定义概述参数资源 GVK 的 `paramKind`，
随后策略绑定操作会通过 `paramRef` 将某个策略按名称（通过 `policyName`）绑定到特定参数资源。
...
-->
MutatingAdmissionPolicy 表达式始终是 CEL。
每个应用配置 `expression` 必须求值为（使用 `Object()` 初始化声明的）CEL 对象。

这些应用配置不能修改原子结构、映射或数组，因为这可能导致意外删除未包含在应用配置中的值。
...
-->
CEL 表达式可以访问以 CEL 变量及一些其他有用变量分组的 API 请求的内容：

- `object` - 来自传入请求的对象。对于 DELETE 请求，取值为 null。
- `oldObject` - 现有对象。对于 CREATE 请求，取值为 null。
- `request` - API 请求的属性。
- `params` - 正被评估的策略绑定所引用到的参数资源。仅在策略具有 ParamKind 时填充。
<!--
...
- `variables` - 组合变量的映射，从其名称到其惰性评估值。
  例如，名为 `foo` 的变量可以以 `variables.foo` 进行访问。
- `authorizer` - 一个 CEL 授权器。可用于对请求的主体（用户或服务账户）进行授权检查。
...
每个求值的 `expression` 必须返回 `JSONPatch` 值形成的数组。
`JSONPatch` 类型表示 JSON 补丁中的一个操作。

例如，这个 CEL 表达式返回 JSON 补丁，用于有条件地修改某个值：
...
- `params` - Parameter resource referred to by the policy binding being evaluated. Only populated if the policy has a ParamKind.
-->
CEL 表达式可以访问以 CEL 变量及一些其他有用变量组织的 API 请求的内容：
