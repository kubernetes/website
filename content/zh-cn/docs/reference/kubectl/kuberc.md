...
2. `command` —— 指定别名实际执行的内置命令。
   这包括支持 `create role` 这类子命令。
3. `options` —— 指定选项的默认值。若你在运行 `kubectl` 时显式指定某个选项，
...
2. `options` —— 指定选项的默认值。若你在运行 kubectl 时显式指定某个选项，
   你提供的值要比 kuberc 中定义的默认值优先生效。
