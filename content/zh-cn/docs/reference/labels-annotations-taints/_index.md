...
-->
例子：`kubernetes.io/psp: restricted`

只有当你使用 [PodSecurityPolicies](/zh-cn/docs/concepts/security/pod-security-policy/) 时，此注解才生效。

当 PodSecurityPolicy 准入控制器接入一个 Pod 时，准入控制器为 Pod 添加此注解。
注解的值是用于验证的 PodSecurityPolicy 的名称。
