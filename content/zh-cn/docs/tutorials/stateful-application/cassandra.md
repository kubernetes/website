...
在此示例中，自定义 Cassandra seed provider 使数据库在接入
Cassandra 集群时能够发现新的 Cassandra 实例。
...
当这些 Pod 在你的 Kubernetes 集群中运行时，Kubernetes 控制平面会将这些 Pod
调度到 Kubernetes 的{{< glossary_tooltip text="节点" term_id="node" >}}上。
...
-->
创建一个 Service 来跟踪 `cassandra-service.yaml` 文件中的所有
...
-->
如果没有看到名为 `cassandra` 的 Service，则表示创建失败。
请阅读[调试 Service](/zh-cn/docs/tasks/debug/debug-application/debug-service/)，以解决常见问题。
...
1. 运行以下命令：
   ...
   此命令你的终端中打开一个编辑器。需要更改的是 `replicas` 字段。下面是
   StatefulSet 文件的片段示例：
...
-->
根据存储类和回收策略，删除 **PersistentVolumeClaim** 可能导致关联的卷也被删除。
...
本教程中的 Pod 使用来自 Google [容器镜像库](https://cloud.google.com/container-registry/docs/)的
...
* 了解有关 [**KubernetesSeedProvider**](https://github.com/kubernetes/examples/blob/master/cassandra/java/src/main/java/io/k8s/cassandra/KubernetesSeedProvider.java)
  的更多信息
