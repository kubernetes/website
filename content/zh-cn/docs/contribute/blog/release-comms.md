...
这与[即将发布的变更和新功能的流程](/zh-cn/docs/contribute/new-content/new-features/#open-a-placeholder-pr)不同。
...
-->
你应该对拉取请求中的提交进行[压缩（squash）](https://www.k8s.dev/docs/guide/pull-requests/#squashing)；
如果你不确定如何操作，完全可以向发布沟通团队或博客团队寻求帮助。

只要你的文章在
[前置元数据（front matter）](https://gohugo.io/content-management/front-matter/)中被标记为草稿（`draft: true`），
...
（如果到了截止日期内容仍未准备好，且你没有获得例外批准，那么该公告将不会被包含）。
他们将为即将发布的文章制定一个时间表，并打开新的拉取请求，
以将这些文章从草稿状态变为已发布状态。
...
在发布日前，用于发布这些公告的拉取请求（PR）应已获得 LGTM（“看起来不错”）
...
-->
一旦实际发布完成并且网站 Git 仓库解冻，发布沟通团队或发布团队可以立即取消对该
PR（或一组 PR）的 **hold** 状态。

在每篇文章预定发布的当天，自动化流程将触发网站构建，该文章将会变得可见。
