...
-->
存在两个官方的 Kubernetes 博客，且 CNCF 有自己的博客，在这些博客上你也可以发布关于 Kubernetes 的内容。  
对于主要的 Kubernetes 博客，我们（Kubernetes 项目）喜欢发表具有不同视角和特别关注点的文章，
这些文章与 Kubernetes 有一定的关联。

有些文章会同时出现在两个博客上：一篇文章是主要版本，另一篇是另一个博客上的**镜像文章**。

此页面描述了镜像的标准、镜像的动机，并解释了为确保文章能够在两个博客上都成功发布你需要做些什么。
...
镜像几乎总是从贡献者博客到主博客。项目组会针对那些关于贡献者社区或其部分内容的文章进行镜像，
但这些文章同时也与 Kubernetes 主博客的更广泛读者相关。
...
- 对于在原始文章已合并到贡献者博客**之后**，向主博客添加镜像文章的 PR，请确保满足以下所有条件：
  - 在原始文章发布到贡献者博客之后，主博客上没有发布任何文章。
  - 在原始文章的发布时间和镜像文章的发布时间之间，没有任何主博客文章计划发布。
...
- 镜像文章将在其[前置元数据](https://gohugo.io/content-management/front-matter/)中正确设置
  `canonicalUrl`。
...
- 镜像文章将在其 [前置元数据](https://gohugo.io/content-management/front-matter/) 中正确设置 `canonicalUrl`。
...
作为文章的作者，你应该为镜像文章设置规范 URL（`canonicalUrl`），
并将其指向原始文章的 URL（你可以使用预览功能预测 URL，并在实际发布前填写此内容）。
在[前置元数据](https://gohugo.io/content-management/front-matter/) 中使用
