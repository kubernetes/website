...
应用这种特定于实例的配置的推荐方法是使用 [`KubeletConfiguration` 补丁](/zh-cn/docs/setup/production-environment/tools/kubeadm/control-plane-flags#patches)。
<!--
...
也也可以在基础 `KubeletConfiguration` 上应用实例特定的补丁。
请阅读 [Customizing the kubelet](/zh-cn/docs/setup/production-environment/tools/kubeadm/control-plane-flags#customizing-the-kubelet) 
来获取更多信息。
