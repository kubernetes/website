...
-->
当你创建一个使用 `Gt` 或 `Lt` 容忍运算符的 Pod 时，API
服务器会校验这些容忍值是否为有效整数。节点上的污点值在节点注册时不会被校验。
如果某个节点具有非数值的污点值（例如
`servicelevel.organization.example/agreed-service-level=high:NoSchedule`），
...
-->
此容忍匹配 `node1` 上的污点，因为 `950 > 900`（对于 `Gt` 运算符来说，污点值大于容忍值）。
同样地，你也可以使用 `Lt` 运算符来匹配污点值小于容忍值的情况：
...
* 容忍值和污点值都必须是有效的、带符号的 64 位整数
  （不允许带前导零的数字，例如 `"0550"`）。
* 如果某个值无法被解析为整数，则此容忍不匹配。
* 数值运算符适用于所有污点效果：`NoSchedule`、`PreferNoSchedule` 和 `NoExecute`。
* 对于使用数值运算符的 `PreferNoSchedule`：如果 Pod 的容忍不满足数值比较条件
  （例如在使用 `Gt` 时，污点值 < 容忍值），调度器降低相应节点的优先级，
...
* 你应识别所有使用 `Gt` 或 `Lt` 运算符的工作负载，以避免控制器出现热循环。
