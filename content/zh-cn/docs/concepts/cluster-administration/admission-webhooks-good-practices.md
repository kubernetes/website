...
* [准入 Webhook](/docs/reference/access-authn-authz/extensible-admission-controllers/#what-are-admission-webhooks)
...
常常会导致工作负载中断。与其他 API 扩展机制一样，Webhook 在大规模测试时与所有工作负载、其他
Webhook、插件及附加组件的兼容性是一个挑战。
...
-->
此外，随着每个版本的发布，Kubernetes 会通过新增特性、将特性提升为测试版或稳定版以及弃用旧特性来添加或修改
API。即使是稳定的 Kubernetes API 也可能会发生变化。例如，在 v1.29 中，`Pod` API 发生了变化，
...
-->
本页面描述了常见的 Webhook 失败场景，以及如何通过谨慎和周到的设计与实现你的
Webhook 来避免这些问题。
...
Kubernetes 包含多个准入控制和策略执行选项。知道何时使用特定选项可以帮助你提高延迟和性能，
减少管理开销，并避免版本升级期间的问题。下表描述了允许你在准入时变更或验证资源的机制：
...
<table>
  ...
  <tbody>
    <tr>
      <td><a href="/zh-cn/docs/reference/access-authn-authz/extensible-admission-controllers/">变更准入 Webhook</a></td>
      <td>在准入前拦截 API 请求，并根据自定义逻辑进行必要的修改。</td>
      <td><ul>
        <li>执行资源准入前必须发生的重大修改。</li>
        <li>执行需要高级逻辑的复杂修改，例如调用外部 API。</li>
      </ul></td>
    </tr>
    <tr>
      <td><a href="/zh-cn/docs/reference/access-authn-authz/mutating-admission-policy/">变更准入策略</a></td>
      <td>在准入前拦截 API 请求，并使用通用表达式语言（CEL）表达式进行必要的修改。</td>
      <td><ul>
        <li>执行资源准入前必须发生的重大修改。</li>
        <li>执行简单修改，例如调整标签或副本数量。</li>
      </ul></td>
    </tr>
    <tr>
      <td><a href="/zh-cn/docs/reference/access-authn-authz/extensible-admission-controllers/">验证准入 Webhook</a></td>
      <td>在准入前拦截 API 请求，并根据复杂的策略声明进行验证。</td>
      <td><ul>
        <li>在资源准入前验证关键配置。</li>
        <li>在准入前执行复杂的策略逻辑。</li>
      </ul></td>
    ...
    <tr>
      <td><a href="/zh-cn/docs/reference/access-authn-authz/validating-admission-policy/">验证准入策略</a></td>
      <td>在准入前拦截 API 请求，并根据 CEL 表达式进行验证。</td>
      <td><ul>
        <li>在资源准入前验证关键配置。</li>
        <li>使用 CEL 表达式执行策略逻辑。</li>
      </ul></td>
    </tr>
  </tbody>
</table>
...
-->
一般来说，当你希望以可扩展的方式声明或配置逻辑时，使用 **Webhook** 准入控制。
当你希望声明更简单的逻辑而无需运行 Webhook 服务器的开销时，使用基于 CEL
...
请勿使用准入 Webhook 来验证 CustomResource 规范中的值或设置字段的默认值。
...
-->
### 为低延迟设计准入 Webhook   {#design-admission-webhooks-low-latency}

变更准入 Webhook 是按顺序调用的。根据变更 Webhook 的设置，某些 Webhook
可能会被多次调用。每次变更 Webhook 调用都会增加准入过程的延迟。
这与验证 Webhook 不同，验证 Webhook 是并行调用的。

在设计你的变更 Webhook 时，请考虑你的延迟要求和容忍度。集群中的变更 Webhook 越多，
延迟增加的可能性就越大。
...
-->
### 防止由冲突控制器引起的循环   {#prevent-loops-competing-controllers}

考虑集群中运行的其他可能与你的 Webhook 所做的变更发生冲突的组件。例如，如果你的
Webhook 添加了一个标签，而另一个控制器删除了该标签，那么你的 Webhook
会被再次调用，从而导致循环。
...
2. 检查审计事件，查看是否有 Webhook 被多次重新调用并应用了相同的补丁到同一个对象，
   或者某个对象的字段被多次更新和回滚。
...
准入 Webhook 应尽可能快速评估（通常在毫秒级别），因为它们会增加 API 请求的延迟。
为 Webhook 设置较小的超时值。
...
`ClusterIP`.
-->
### 使用负载均衡确保 Webhook 可用性   {#load-balancer-webhook}

准入 Webhook 应该利用某种形式的负载均衡来提供高可用性和性能优势。如果
Webhook 在集群内运行，你可以在类型为 `ClusterIP` 的服务后面运行多个
Webhook 后端。
...
在设计 Webhook 时，请考虑集群的可用性需求。例如，在节点停机或区域中断期间，
Kubernetes 会将 Pod 标记为 `NotReady`，以便负载均衡器可以将流量重新路由到可用的区域和节点。
这些对 Pod 的更新可能会触发你的变更 Webhook。根据受影响 Pod 的数量，变更 Webhook
服务器有超时或导致 Pod 处理延迟的风险。结果是，流量不会像你需要的那样快速地重新路由。
...
本节提供了关于过滤哪些请求触发特定 Webhook 的建议。总结如下：

* 限制 Webhook 的作用范围，以避免系统组件和只读请求。
* 将 Webhook 限制在特定的命名空间中。
* 使用匹配条件执行细粒度的请求过滤。
* 匹配对象的所有版本。
...
准入 Webhook 仅在 API 请求与相应的 Webhook 配置匹配时才会被调用。
...
准入控制器支持多个字段，你可以使用这些字段来匹配符合特定条件的请求。例如，
...
-->
为了实现更细粒度的请求过滤，请在 Webhook 配置中使用 `matchConditions` 字段。
...
默认情况下，准入 Webhook 会在影响指定资源的任何 API 版本上运行。Webhook
配置中的 `matchPolicy` 字段控制此行为。在 `matchPolicy` 字段中指定值为
`Equivalent` 或省略该字段，以允许 Webhook 在任何 API 版本上运行。
...
变更 Webhook 可以添加具体的字段进行变更，以允许准入。确保你仅修改需要更改的字段。
...
-->
例如，考虑一个配置为确保 `web-server` 部署至少具有三个副本的变更 Webhook。
当创建 Deployment 对象的请求与你的 Webhook 配置匹配时，Webhook
...
Kubernetes 对象规范中的字段可能包含数组。有些数组包含键值对
（如容器规范中的 `envVar` 字段），而其他数组则没有键（如 Pod 规范中的 `readinessGates` 字段）。
在某些情况下，数组字段中值的顺序可能很重要。例如，容器规范中 `args`
字段的参数顺序可能会影响容器。
...
-->
* 尽可能使用 `add` JSONPatch 操作，而不是 `replace`，以避免意外替换所需的值。
* 将不使用键值对的数组视为集合来处理。
* 确保你修改的字段中的值不需要特定的顺序。
* 除非绝对必要，否则不要覆盖现有的键值对。
...
例如，配置了一个变更准入 Webhook，仅当 Pod 中设置了特定标签（如 `env: prod`）时才允许**创建**
Pod 请求。而 Webhook 服务器运行在一个没有设置 `env` 标签的 Deployment 中。
...
变更准入 Webhook 支持 `failurePolicy` 配置字段。此字段指示如果 Webhook
...
-->
通过将 `failurePolicy` 字段设置为 Ignore，允许你的变更 Webhook 在失败时“开放”。
使用验证控制器检查请求的状态，确保它们符合你的策略。

这种方法有以下好处：

* 变更 Webhook 的停机不会影响合规资源的部署。
* 策略执行发生在验证准入控制阶段。
* 变更 Webhooks 不会干扰集群中的其他控制器。
...
响应广泛 API 请求的变更 Webhook 可能会无意中触发自身。例如，考虑一个响应集群内所有请求的
...
-->
### 不要更改不可变对象   {#dont-change-immutable-objects}
...
来跟踪该静态 Pod。然而，对镜像 Pod 的更改不会传播到静态 Pod。
...
的同时降低忽略注解的安全风险，可以允许静态 Pod 仅在特定的命名空间中运行。
...
-->
## 变更 Webhook 的顺序与幂等性 {#ordering-idempotence}

本节提供了关于 Webhook 顺序设计和幂等性 Webhook 的建议。总结如下：
...
* 在准入前验证变更。
* 检查是否存在其他控制器覆盖的变更。
* 确保整个变更 Webhook 集合是幂等的，而不仅仅是单个 Webhook 具有幂等性。
...
被调用的时机。不要依赖你的 Webhook 在准入流程中的某个特定点运行，
因为其他 Webhook 仍可能对你修改的对象进行进一步变更。
...
每个变更准入 Webhook 都应该是**幂等的**。Webhook 应能够在已经修改过的对象上运行，
而不会在原始更改之外产生额外的更改。
...
-->
此外，集群中的所有变更 Webhook 集合也应当是幂等的。在准入控制的变更阶段结束后，
每个单独的变更 Webhook 应能够在对象上运行而不会对该对象产生额外的更改。

根据你的环境，确保大规模幂等性可能会具有挑战性。以下建议可能有所帮助：
...
* 在测试集群中测试你的部署，查看是否有任何对象被同一个 Webhook 多次修改。
* 确保每个变更 Webhook 的作用范围具体且有限。
...
-->
1. 对于 **create**/**update** Pod 请求，如果 Pod 已设置标签 `env`，则拒绝请求；
   否则，向 Pod 添加标签 `env: prod`。

   重新调用 Webhook 将导致 Webhook 在其自身的输出上失败。
...
-->
4. 对于 **create** Pod 请求，在不检查是否已存在名为 `foo-sidecar`
...
-->
## 变更测试与验证 {#mutation-testing-validation}

本节提供了关于测试变更 Webhook 和验证变更对象的建议。总结如下：
...
changes.
-->
### 在测试环境中测试 Webhook   {#test-in-staging-environments}

稳健的测试应该是你发布新 Webhook 或更新 Webhook 的核心部分。如果可能的话，
在一个与生产集群相似的测试环境中测试对集群 Webhook 的任何更改。至少，
...
你的变更 Webhook 不应破坏对象在准入前适用的任何验证规则。例如，考虑一个将
Pod 的默认 CPU 请求设置为特定值的变更 Webhook。如果该 Pod 的 CPU
限制设置为低于变更后的请求值，则该 Pod 将无法通过准入。

针对集群中运行的验证规则测试每个变更 Webhook。
...
在将生产集群升级到新的小版本之前，在测试环境中测试你的 Webhook 和工作负载。
...
变更 Webhook 会在任何验证 Webhook 运行之前完成运行。
变更应用于对象的顺序并不稳定。因此，你的变更可能会被后续运行的变更 Webhook 覆盖。
...
-->
添加如 ValidatingAdmissionWebhook 或 ValidatingAdmissionPolicy
这样的验证准入控制器到你的集群中，以确保你的变更是仍然存在的。例如，
考虑一个变更 Webhook，它将 `restartPolicy: Always` 字段插入特定的初始化容器中，
...
* Limit access to the namespace that runs the webhook server, if the server is
  in-cluster.
-->
## 变更 Webhook 部署   {#mutating-webhook-deployment}

本节提供了关于部署变更准入 Webhook 的建议。总结如下：

* 逐步推出 Webhook 配置，并按命名空间监控问题。
* 限制对编辑 Webhook 配置资源的访问权限。
...
-->
### 安装并启用变更 Webhook {#install-enable-mutating-webhook}

当你准备将变更 Webhook 部署到集群时，请按照以下操作顺序进行：
...
2. 在 MutatingWebhookConfiguration 清单中将 `failurePolicy`
   字段设置为 `Ignore`。这样可以避免因 Webhook 配置错误而导致的中断。
...
entities:
-->
### 限制对变更 Webhook 的编辑访问 {#limit-edit-access}

变更 Webhook 是强大的 Kubernetes 控制器。使用 RBAC
或其他授权机制来限制对你的 Webhook 配置和服务器的访问权限。
对于 RBAC，确保只有受信任的实体才具有以下访问权限：
...
如果你的变更 Webhook 服务器在集群内运行，请限制对该命名空间中任何资源的创建或修改权限。
...
-->
以下项目是“良好”自定义 Webhook 服务器实现的示例。在设计你自己的 Webhook 时，
...
-->
* [使用 Webhook 进行身份验证和授权](/zh-cn/docs/reference/access-authn-authz/webhook/)
* [了解 MutatingAdmissionPolicies](/zh-cn/docs/reference/access-authn-authz/mutating-admission-policy/)
* [了解 ValidatingAdmissionPolicies](/zh-cn/docs/reference/access-authn-authz/validating-admission-policy/)
