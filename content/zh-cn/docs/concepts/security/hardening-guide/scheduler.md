...
在设置身份认证配置时，应确保 kube-scheduler 的身份认证配置与 kube-api-server 的身份认证配置保持一致。
如果任一请求缺少身份认证头，
则应[通过 kube-api-server 进行身份认证以保证集群内的所有身份认证一致](/zh-cn/docs/tasks/extend-kubernetes/configure-aggregation-layer/#original-request-username-and-group)。
...
- `bind-address`：在大多数情况下，kube-scheduler 不需要从外部能够访问。
