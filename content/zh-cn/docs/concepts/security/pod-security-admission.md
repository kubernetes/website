...
在 v1.23 中，`PodSecurity` [特性门控](/zh-cn/docs/reference/command-line-tools-reference/feature-gates/)
是一项 Beta 功能特性，默认被启用。

在 v1.22 中，`PodSecurity` [特性门控](/zh-cn/docs/reference/command-line-tools-reference/feature-gates/)
...
**audit** | 策略违例会触发[审计日志](/zh-cn/docs/tasks/debug-application-cluster/audit/)中记录新事件时添加审计注解；但是 Pod 仍是被接受的。
