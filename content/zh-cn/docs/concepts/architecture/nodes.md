...
- `--node-ip` - 可选的节点 IP 地址的列表（逗号分隔）。
  对于每个地址族，你只能指定一个地址。
  例如，在单栈 IPv4 集群中，你将此值设置为 kubelet 节点使用的 IPv4 地址。
  请参阅 [配置 IPv4/IPv6 双栈](/zh-cn/docs/concepts/services-networking/dual-stack/#configure-ipv4-ipv6-dual-stack)以了解运行双栈集群的详细信息。

  如果你不提供此参数，kubelet 将使用节点的默认 IPv4 地址，如果有 IPv4 地址；
  如果节点没有 IPv4 地址，kubelet 将使用节点的默认 IPv6 地址。
...
例如，当 Ready 条件的 `status` 保持为 `Unknown` 或 `False` 的时间超过了 kube-controller-manager 的 `NodeMonitorGracePeriod`，默认为 40 秒。
这将导致节点被添加 `node.kubernetes.io/unreachable` 污点（针对 `Unknown` 状态）或 `node.kubernetes.io/not-ready` 污点（针对 `False` 状态）。

这些污点会影响待定的 pod，因为调度器在将 pod 分配给节点时会考虑节点的污点。
由于 `NoExecute` 污点，已经调度到该节点的现有 pod 可能会被驱逐。
Pod 还可以具有{{< glossary_tooltip text="容忍度" term_id="toleration" >}}，让它们即使在具有特定污点的节点上仍然可以调度并继续运行。
...
for more details.
-->
进一步的细节可参阅[基于污点驱逐](zh-cn/docs/concepts/scheduling-eviction/taint-and-toleration/#taint-based-evictions)和
