...
Kubernetes 还利用租约确保在任何给定时间某个组件只有一个实例在运行。
...
-->
租约名称中使用的 SHA256 哈希基于该 API 服务器所看到的操作系统主机名生成。
...
-->
你自己的工作负载可以定义自己对 Lease 的使用。
例如，你可以运行一个自定义{{< glossary_tooltip term_id="controller" text="控制器" >}}，
其中主要成员或领导成员执行其对等方不执行的操作。
你定义一个租约，以便控制器副本可以使用 Kubernetes API 进行协调来选择或选举领导者。
<!--
...
`example-foo`.
-->
如果你确实使用租约，最好为租约定义一个明显与产品或组件相关联的名称。
例如，如果你有一个名为 Example Foo 的组件，请使用名为 `example-foo` 的租约。
...
for the Leases.
-->
如果集群操作员或其他终端用户可以部署组件的多个实例，
请选择名称前缀并选择一种机制（例如 Deployment 名称的哈希）来避免租约的命名冲突。

<!--
You can use another approach so long as it achieves the same outcome: different software products do
not conflict with one another.
-->
你可以使用另一种方法，只要它能达到相同的结果：不同的软件产品不会相互冲突。
