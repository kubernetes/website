...
[command line flags](/docs/reference/command-line-tools-reference/kubelet).
-->
当通过 gRPC 连接到容器运行时，kubelet 将充当客户端。
运行时和镜像服务端点必须在容器运行时中可用，可以使用 `--image-service-endpoint`
...
-->
对于 Kubernetes v1.26 及更高版本，kubelet 要求容器运行时支持 `v1` CRI API。
...
当升级节点上的 Kubernetes 版本时，kubelet 会重启。
如果容器运行时不支持 `v1` CRI API，kubelet 将无法注册并报告错误。
如果因为容器运行时已升级而需要进行 gRPC 重拨，则连接要成功，
运行时必须支持 `v1` CRI API。
这可能需要在正确配置容器运行时后重启 kubelet。
