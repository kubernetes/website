---
title: 在 Kubernetes 中运行 Windows 容器的指南
...
-->
本文提供了一些参考演示步骤，方便你使用 Kubernetes 运行 Windows 容器。
本文还重点介绍了 Kubernetes 中专为 Windows 设计的一些特有功能。

需要注意的是，在 Kubernetes 中创建和部署服务与工作负载的行为，对于 Linux 容器和 Windows 容器来说是相同的。
与集群交互的 [kubectl 命令](/zh-cn/docs/reference/kubectl/)也是完全一致的。
本文中的几个例子旨在帮助你快速开始使用 Windows 容器。
...
-->
你应当已经拥有一个 Kubernetes 集群的访问权限，并且该集群中包含一个运行 Windows Server 的工作节点。
...
-->
## 快速开始：部署 Windows 工作负载  {#getting-started-deploying-a-windows-workload}

以下示例 YAML 文件部署了一个在 Windows 容器内运行的简单 Web 服务器的应用。

创建一个名为 `win-webserver.yaml` 的清单（Manifest），其内容如下：
...
3. 检查部署是否成功。请验证：
   ...
   * 当执行 `kubectl get pods` 命令时，能够从 Linux 控制平面所在的节点上列出若干 Pod。
   * 跨网络的节点到 Pod 通信，从 Linux 控制平面所在的节点上执行 `curl` 命令来访问
     Pod IP 的 80 端口以检查 Web 服务器响应。
   * Pod 间通信，使用 `kubectl exec`
...
用户需要使用某种{{<glossary_tooltip text="污点" term_id="taint" >}}和节点选择器的组合，
以便将 Linux 和 Windows 工作负载各自调度到特定操作系统的节点。
下面概述了推荐的方法，其主要目标之一是该方法不应破坏现有 Linux 工作负载的兼容性。

你可以（并且应该）将每个 Pod 的 `.spec.os.name` 设置为 Pod 中的容器设计所用于的操作系统。
...
-->
如果你运行的 Kubernetes 版本低于 1.24，你可能需要启用 `IdentifyPodOS`
...
因此仍然需要污点和容忍（或节点选择器）来确保 Windows Pod 落在适当的 Windows 节点。
...
所有运行 Kubernetes {{< skew currentVersion >}} 的 Kubernetes 节点都有以下默认标签：
...
Kubernetes {{< skew currentVersion >}} 的最佳实践是使用 `nodeSelector`。
...
-->
但是，在许多情况下，用户已经预先存在大量 Linux 容器部署，
以及现成配置的生态系统，例如社区中的 Helm Chart 包和程序化的 Pod 生成案例，例如 Operator。
在这些情况下，你可能不愿更改配置来添加 `nodeSelector` 字段到所有 Pod 或 Pod 模板。
...
如果要在同一个集群中使用多个 Windows Server 版本，则应设置额外的节点标签和 `nodeSelector` 字段。

Kubernetes 自动添加了一个新标签
...
3. 根据情况，向 Pod 规约中添加 `runtimeClassName: windows-2019`
