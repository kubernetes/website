...
-->
请注意：当 Pod 正在被删除时，该Pod会被一些 kubectl 命令显示为 `Terminating` 状态。
此 `Terminating` 状态并不是 Pod 的 `phase` 之一。 
Pod 被授予一个期间来进行优雅退出，默认为 30 秒。
你可以使用标志 --force 来[强制终止 Pod](/docs/concepts/workloads/pods/pod-lifecycle/#pod-termination-forced)。
{{< /note >}}
