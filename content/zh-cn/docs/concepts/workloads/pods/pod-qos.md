...
该 Pod 必须设置 Pod 级别的内存 limit 和内存 request，并且这两个值必须相等。
该 Pod 必须设置 Pod 级别的 CPU limit 和 CPU request，并且这两个值必须相等。
...
* Pod 中至少一个容器有内存或 CPU 的 request 或 limit，或者该 Pod 本身设置了 Pod 级别的内存或 CPU 的 request 或 limit。
...
CPU request，且Pod 本身 也没有设置任何 Pod 级别的内存或 CPU 的 limit 或 request 时，
