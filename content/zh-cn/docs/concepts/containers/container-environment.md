...
这里的服务仅限于新容器的 Pod 所在的名字空间中的服务，以及 Kubernetes 控制平面的服务。
...
if [DNS addon](https://releases.k8s.io/{{< skew currentPatchVersion >}}/cluster/addons/dns/) is enabled. 
-->
服务具有专用的 IP 地址。如果启用了
[DNS 插件](https://releases.k8s.io/{{< skew currentPatchVersion >}}/cluster/addons/dns/)，
可以在容器中通过 DNS 来访问服务。
