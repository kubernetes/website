...
在 Kubernetes 中，[服务]（/doc/concepts/Services-networking/service/）是一种抽象的方式，用于公开在一组 Pod 上运行的应用程序。
...
描述为不小于 16 或不大于 256，它们之间有一个渐进的步长。

默认情况下，动态 IP 分配使用较高频段，
一旦用完，它将使用较低范围。 这将允许用户在冲突风险较低的较低频段上使用静态分配。
