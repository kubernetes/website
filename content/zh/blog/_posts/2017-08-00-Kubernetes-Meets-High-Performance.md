...
-->
任何用过 Docker 的人都能体会到容器在效率方面的巨大提升。虽然 Kubernetes 擅长编排容器，但在 Kubernetes 上部署高性能计算 (HPC) 应用程序可能会非常棘手。
...
-->
在本文中，我将讨论使用 Kubernetes 运行 HPC 工作负载的一些挑战，解释公司如何应对当前的这些挑战，并提出一种在共享 Kubernetes 集群上支持混合工作负载的方法。我们还将提供有关客户 IHME 案例研究的信息和链接，展示如何扩展 Kubernetes 以无缝地为其 HPC 工作负载提供服务，同时保留 HPC 用户熟悉的可扩展性和界面。
...
-->
在 Kubernetes 中，调度的基本单位是 Pod：一个或多个 Docker 容器被调度分配到集群主机上。 Kubernetes 假设工作负载是容器。 虽然 Kubernetes 有 [Cron Jobs](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/) 和 [Jobs](https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/) 会一直运行到完成，但部署在 Kubernetes 上的应用程序通常是长期运行的服务，如 Web 服务器，负载均衡器或数据存储，虽然它们是高度动态的，pod 来来往往，但这些还是与 HPC 应用程序的模式有很大的不同。
...
-->
- 在金融或工程模拟中，作业可能由数万个短期运行任务所组成，要求低延迟和高吞吐量调度，以期在可接受的时间内完成模拟。
- 计算流体力学 (CFD) 问题可以使用消息传递库来同步状态，在数百个甚至数千个节点上并行执行。 这需要专门的调度和作业管理功能来分配和启动这些作业，然后到检查点，暂停/恢复或者回填它们。
...
-->
HPC 用户认可容器价值的理由与其他组织相同。 容器中的封装逻辑使其便于移植，不受环境依赖的影响，并且易于与其他容器进行交换，这些都显然很具价值。 但是，切换到容器这点却很困难。
...
-->
HPC 工作负载流程通常集成到命令行上。作业并不是通过编码，而是通过命令行作为二进制文件或作为包装器的简单 shell 脚本提交给队列。 HPC 站点使用了数百种工程，科学和分析应用程序，这些应用程序采用这种方法，并且与流行的工作负载调度程序进行了成熟且经过认证的集成。
...
-->
虽然将工作负载打包到 Docker 容器，将其发布到镜像仓库，并提交描述工作负载的 YAML 文件的做法是 Kubernetes 用户的第二天性，但这个流程对于大多数 HPC 用户来说却是陌生的。 用 R，MATLAB 或 Stata 编写模型的分析师只想快速提交模拟请求，监控其执行情况，并尽快获得结果。
...
-->
更好的方法是在同一共享环境中原生支持 HPC 和容器工作负载。理想情况下，用户应该看到适合其工作负载或工作流类型的环境。
...
-->
通过这种方法，Kubernetes 充当资源管理器，使资源可用于单独的 HPC 调度程序。群集管理员可以使用可视界面根据策略分配资源，或者只需通过 Web UI 拖动滑块，即可将不同比例的 Kubernetes 环境分配给非容器 (HPC) 工作负载和 Kubernetes 的原生应用程序和服务。
...
-->
一个在混合工作负载方面取得成功的客户是健康指标与评估研究所 (IHME)，华盛顿大学的一所独立的健康研究中心。为支持其全球公认的全球健康数据交换 (GHDx)，IHME 的运行环境规模巨大，包括500个节点和20,000个核，在 Kubernetes 集群上运行分析，HPC 和基于容器的应用程序的混合负载。 [案例研究](http://navops.io/ihme-case-study.html) 详细描述了 IHME 使用 Navops Command 在共享 Kubernetes 集群上托管现有 HPC 工作负载的成功经验。
