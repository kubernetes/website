...
-->
当选择一个 Linux 系统发行版作为初始化系统时，初始化进程会生成一个 root 控制组 （`cgroup`），并使用它充当 cgroup 管理器。
系统具有紧密的 cgroup 集成，并将为每个进程分配 cgroup。
您可以配置容器运行时和 kubelet 去使用 `cgroupf`。
连同系统一起使用 `cgroupf` 意味着将有两个不同的 cgroup 管理器。
...
在此领域我们已经看到案例，其中为 kubelet 和 docker 配置节点来使用 `cgroupf`，在节点上运行的其余进程在资源压力下会变得不稳定。
...
-->
更改设置，以便您的容器运行时和 kubelet 使用 `systemd` 作为 cgroup 驱动来稳定系统。
...
如果 kubelet 已经使用 cgroup 驱动的语义创建了 pod，则当尝试为现有的 pod 重新创建 PodSandbox 时，其他的 cgroup 驱动程序的运行时可能会产生错误。
重启 kubelet 或许不能解决此类问题。
推荐从工作负载中排出节点，将其从集群中删除并重新加入。
{{< /caution >}}
...
在 Kubernetes 发行说明中保持跟随经过验证的 Docker 最新版本。
