...
网络策略帮助您声明性地配置允许哪些 Pod 之间互接、哪些名称空间之间允许进行通信，并具体配置了哪些端口号来执行每个策略。`NetworkPolicy` 资源使用标签来选择 Pod，并定义了所选 Pod 可以接受什么样的流量。网络策略由网络提供商提供的并被 Kubernetes 支持的网络插件实现。请注意，当没有控制器实现网络资源时，创建网络资源将不会生效。
