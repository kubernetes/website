...
1. 如果调用 kubeadm 时启用了 `--feature-gates=DynamicKubeletConfig`，它首先从主机上检索 kubelet 初始化配置并将其写入磁盘。
   ...
   进一步了解动态 kubelet 配置 请参考 [使用配置文件设置 Kubelet 参数](/zh/docs/tasks/administer-cluster/kubelet-config-file/) 和 [重新配置集群中节点的 Kubelet](/zh/docs/tasks/administer-cluster/reconfigure-kubelet/)。
