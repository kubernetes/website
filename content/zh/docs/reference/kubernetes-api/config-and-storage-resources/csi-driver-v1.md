...
- **attachRequired** (boolean)
  ...
  （因为它实现 CSI ControllerPublishVolume() 方法），
...
- **tokenRequests** ([]TokenRequest)
  ...
  *原子：将在合并期间被替换*
  ...
  要在过期后接收新的令牌，requiresRepublish 可用于周期性地触发 NodePublishVolume。
...
- **volumeLifecycleModes** ([]string)
  ...
  **设置：唯一值将在合并期间被保留**
  ...
  驱动必须意识到这一点，因为只会为这种卷获取 NodePublishVolume 调用。
  有关实现此模式的更多信息，请参阅
