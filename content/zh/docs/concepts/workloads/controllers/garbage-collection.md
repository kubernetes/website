...
设计不允许跨 namespace 所有者引用。这意味着：

1）命名空间范围的 dependents 只能指定同一命名空间以及集群范围的 owners。

2）集群范围的 dependents 只能指定集群范围的 owners，不能指定命名空间范围的。
...
在 1.7 之前的版本中，当在 Deployments 中使用级联删除时，您必须 *使用* `propagationPolicy:Foreground`。不仅删除创建的 ReplicaSet，还删除其 Pod。如果不使用这种类型的 `propagationPolicy`，则将只删除 ReplicaSet，而 Pod 被孤立。
