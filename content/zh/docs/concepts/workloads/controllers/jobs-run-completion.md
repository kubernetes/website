...
Job 创建一个或多个 pod，并且确保指定数量的 pod 成功终止。Pod 成功完成后，Job 将跟踪成功完成的情况。当达到指定的成功完成次数时，任务（即 Job）就完成了。删除 Job 将清楚其创建的 pod。
...
下面是一个作业配置样例。它计算 π 到2000位并将其打印出来。大约需要 10 秒钟才能完成。
...
要查看作业的已完成 pod，可以使用 `kubectl get pods` 命令。

<!--
To list all the Pods that belong to a Job in a machine readable form, you can use a command like this:
-->

要以机器可读的形式列出属于作业的所有 pod，可以使用以下命令：
...
这里，选择器与作业的选择器相同。`--output=jsonpath` 选项指定一个表达式来从 pod 返回列表中获取每个 pod 的名称。
...
和其他 Kubernetes 配置一样，作业需要 `apiVersion`，`kind` 和 `metadata` 字段。

<!--
A Job also needs a [`.spec` section](https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status).
-->

作业也需要 [`.spec` 部分](https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status)。
...
除了 pod 的必填字段，作业中的 pod 模板还必须指定合适的标签（参考 [pod 选择器](#pod-selector)）和适当的重启策略。
...
1. 配置了*固定完成计数*的并行作业：
  ...
  - 作业代表整体任务，并且在 1 到 `.spec.completions` 范围内都有一个 pod 成功完成时，作业才算完成。
...
1. 具有*工作队列*的并行作业：
  ...
  - 每个 pod 都可以独立确定其所有对端是否完成，从而确定整个作业完成。
  - 当作业中的_任何_ pod 成功结束后，不会再创建新的 pod。
  - 一旦至少一个 pod 成功终止并且所有的 pod 都终止了，则作业就成功完成了。
...
对于_非并行_作业，可以不设置 `.spec.completions` 和 `.spec.parallelism`。两者均未设置时，默认值为 1。
...
对于一个_固定完成计数_作业，应该将 `.spec.completions` 设置为所需的完成数量。`.spec.parallelism` 可以设置，也可以不设置，默认值为 1。
...
对于一个_工作队列_作业，不要设置 `.spec.completions`，并且将 `.spec.parallelism` 设置一个非负整数。

<!--
For more information about how to make use of the different types of job, see the [job patterns](#job-patterns) section.
-->

有关如何使用不同类型作业的更多信息，请参考[作业模式](#job-patterns)部分。
...
请求的并发数（`.spec.parallelism`）可以设置为任何非负的整数。如果未指定，默认值为 1。如果指定为 0，则作业就被有效地暂停直到该配置值增加。
...
- 对于_固定完成计数_作业，实际上并行运行的 pod 数量不会超过剩余的完成数。更高的 `.spec.parallelism` 值将被优先忽略。
- 对于_工作队列_作业，任何 pod 成功之后都不会启动新的 pod。然而，剩余的 pod 可以完成。
- 如果控制器还没来得及反应。
- 如果控制器由于任何原因（缺少 `ResourceQuota`，缺少权限等）未能创建 pod，则 pod 数量将少于请求数。
- 控制器可能会因为同一作业中有过多之前失败的 pod，从而限制新的 pod 的创建。
...
由于多种原因，整个 pod 也可能失败，比如，当 pod 从节点上被踢走（节点在升级，重启，被删除等），或者 pod 中的容器失败且设置了 `.spec.template.spec.restartPolicy = "Never"`。当 pod 失败后，作业控制器将启动一个新的 pod。这意味着你的应用程序需要处理这种情况（应用程序在新的 pod 中重启）。特别是，它需要处理由先前运行引起的临时文件，锁，不完整输出等等。
...
在某些情况下（配置中的逻辑错误等），你需要在重试一定次数后才使作业失败。为此需要设置 `.spec.backoffLimit` 配置值为判断作业失败前的重试次数。 退避限制默认设置为 6。与作业相关的 pod在失败后，经过一个指数级退避延迟（10 秒，20 秒，40 秒，上限为 6 分钟），由作业控制器重新创建。 如果在作业的下一次状态检查之前未出现新的失败 pod，则会重置退避计数。
...
如果你的作业外配置了 `restartPolicy = "OnFailure"`，请记住，一旦作业达到退避限制，运行在该作业的容器将被终止。这会使得调试作业的可执行将变得更加困难。我们建议在调试作业或使用日志系统时设置 `restartPolicy = "Never"`，以确保失败作业的输出不会在不经意间丢失。
{{< /note >}}
...
当作业完成后，不会再创建 pod，但是 pod 也不会被删除。将这些 pod 保留，使你仍然可以查看已完成 pod 的日志来检查错误，警告或其他诊断输出。作业对象在完成后也同样保留下来，以便你可以查看它的状态。由用户来决定在查看完状态后删除旧的作业。使用 `kubectl`（例如 `kubectl delete jobs/pi` 或 `kubectl delete -f ./job.yaml` ）来删除作业。当你使用 `kubectl` 删除作业后，它所创建的所有 pod 也会被删除。
...
默认情况下，除非 pod 失败（`restartPolicy=Never`）或容器错误退出（`restartPolicy=OnFailure`），否则作业将不会被中断运行。作业中断将遵循上面介绍的 `.spec.backoffLimit`。一旦 `.spec.backoffLimit` 达到，作业将被标记为失败，企鹅别所有正运行的 pod 将被终止。
...
终止作业的另一种方式是设置有限期限。通过设置作业对象的 `.spec.activeDeadlineSeconds` 字段为秒数来达到。`activeDeadlineSeconds` 适用于作业存在期间，不管有多少 pod 被创建。一旦作业达到 `activeDeadlineSeconds`，所有运行中的 pod将被终止，并且作业的状态将变成 `type: Failed` 与 `reason: DeadlineExceeded`。

<!--
Note that a Job's `.spec.activeDeadlineSeconds` takes precedence over its `.spec.backoffLimit`. Therefore, a Job that is retrying one or more failed Pods will not deploy additional Pods once it reaches the time limit specified by `activeDeadlineSeconds`, even if the `backoffLimit` is not yet reached.
-->

请注意，作业的 `.spec.activeDeadlineSeconds` 配置优先于 `.spec.backoffLimit` 配置。因此，重试一个或多个失败 pod 的作业在达到 `activeDeadlineSeconds` 指定的时间限制前，不会部署其他的 pod，即使 `backoffLimit` 尚未达到。
...
请注意，作业对象中的作业规格和 [Pod 模板规格](/docs/concepts/workloads/pods/init-containers/#detailed-behavior)都有 `activeDeadlineSeconds` 字段。确保将此字段设置在适当的层级。
...
系统中已完成的作业通常不再需要。将它们保留在系统中将会给 API 服务器带来压力。如果作业由更高级别的控制器管理，比如 [CronJobs](/docs/concepts/workloads/controllers/cron-jobs/)，则作业可以由 CronJobs 基于指定的基于容量的清理策略来进行清除。
...
清理已完成作业（`Complete` 或 `Failed`）的另一个方式是通过使用 [TTL 控制器](/docs/concepts/workloads/controllers/ttlafterfinished/)提供的 TTL 机制，TTL 通过指定作业的 `.spec.ttlSecondsAfterFinished` 字段来清理已完成资源。
...
当 TTL 控制器清理作业时，它将级联删除作业，即删除它的依赖对象，比如 pod 将和作业一起被删除。请注意，当作业被删除后，它的生命周期保证，如终结者，将被触发。
...
`pi-with-ttl` 作业在完成后 100 秒就会自动删除掉。
...
如果该字段设为为 `0`，那么作业将在完成后立即被自动删除掉。如果该字段未设置，则作业在完成后将不会被 TTL 控制器清理掉。
...
作业对象可用于支持 pod 的可靠并行执行。作业对象并非设计为支持紧密通信的并行过程（常见于科学计算领域）。它被用于支持一组独立但相关的*工作项*的并行处理。这些可能是要发送的电子邮件，要渲染的帧，要编码转换的文件，要扫描的 NoSQL 数据库中一段范围内的键等等。
...
- 一个作业对象处理一个工作项，还是一个作业对象处理所有的工作项。后者适用于有大量工作项的场景。前者在管理大量作业对象时会对用户和系统带来一些开销。
...
|                            模式                                   | 单个作业对象 | 比工作项更少的 pod？ | 使用未修改的 app？ |  Kube 1.1 中生效？ |
| -------------------------------------------------------------------- |:-----------------:|:---------------------------:|:-------------------:|:-------------------:|
| [作业模板扩展](/docs/tasks/job/parallel-processing-expansion/)            |                   |                             |          ✓          |          ✓          |
...
当使用 `.spec.completions` 指定完成数时，由作业控制器创建的每个 pod 都有一个相同的 [`规格`](https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status)。这意味着一个任务的所有 pod 将有相同的命令行和镜像，相同的卷以及（几乎）相同的环境变量。这些模式通过不同的方式安排 pod 去处理不同的任务。
...
| -------------------------------------------------------------------- |:-------------------:|:--------------------:|
| [作业模板扩展](/docs/tasks/job/parallel-processing-expansion/)           |          1          |     应该为 1      |
...
通常，在创建作业对象时，不用指定 `.spec.selector`。系统的默认逻辑会在创建作业时添加这个字段。它会选择一个不会和其他 pod 重叠的选择器。
...
但是，在某些情况下，你可能需要去覆盖默认设置的选择器。为此，你需要指定作业的 `.spec.selector`。
...
当设置时需要特别小心。如果你指定的标签选择器不是作业的 pod 所独有的，并且匹配到了不相关的 pod，那么不相关作业的 pod 可能会被删除，或此作业可能将其他 pod 统计为已完成，或者其中一个或两个作业可能拒绝创建 pod 或运行完成。如果一个选择了非唯一的选择器，则其他的控制器（比如 ReplicationController）和它的 pod 的行为也将是不可预测的。当指定了 `.spec.selector`，Kubernetes将不会阻止你产生错误。
...
假设 `old` 作业已经在运行了。你想要运行中的 pod 保持运行，但是你想作业创建的其他 pod 使用不同的 pod 模板，并使作业具有一个新的名称。你不能更新作业对象，因为这些字段是不可更新的。因此，你需要删除 `old` 但是_让它的 pod 运行_，可以使用 `kubectl delete jobs/old --cascade=false` 命令。在删除之前，你需要记下它所使用的选择器：
...
然后使用新名称 `new` 创建一个新的作业，并且显示指定相同的选择器。由于现有的 pod 具有 `controller-uid=a8f3d00d-c6d2-11e5-9f87-42010af00002` 标签，它们也受 `new` 作业的控制。
...
由于你不再使用系统通常为你自动生成的选择器，你需要在新的作业中指定 `manualSelector: true`。
...
新的作业本身将具有一个与 `a8f3d00d-c6d2-11e5-9f87-42010af00002` 不通的 uid。设置 `manualSelector: true` 来告诉系统你知道自己在做什么，并使其允许这种不匹配。
...
当 pod 所在节点重启或发生故障时，pod 将终止并且不会重启。然后，作业将创建新的 pod 替换终止的 pod。因此，即使你的应用仅需要一个 pod，我们还是推荐使用作业而不是裸 pod。
...
作业是[副本控制器](/docs/user-guide/replication-controller)的补充。副本控制器管理预期不会终止的 pod（比如 web 服务器），而作业管理预期将会终止的 pod（比如批处理任务）。
...
单个作业的另一种模式是创建一个创建其他 pod 的 pod，充当这些 pod 的一种自定义控制器。这提供了更大的灵活性，但是入门起来可能有点复杂，并且与 Kubernetes 的集成较少。
...
这种模式的一个样例是：一个作业启动一个 pod 来运行一个脚本，该脚本依次启动 Spark 主控制器（参阅 [spark 例子](https://github.com/kubernetes/examples/tree/{{< param "githubbranch" >}}/staging/spark/README.md)），并运行 spark 驱动，然后进行清理。
...
这种方法的优点是整个过程可以获取作业对象的完成保证，不是完全控制 pod 的创建以及如何将工作分配给 pod。
...
你可以使用 [`CronJob`](/docs/concepts/workloads/controllers/cron-jobs/) 来创建一个会在指定的时间/日期执行的作业，类似于 Unix 工具 `cron`。
