...
Kubernetes v{{< skew currentVersion >}} 包括集群级 API 支持存储容量跟踪。 
要使用它，你还必须使用支持容量跟踪的 CSI 驱动程序。 
查阅你所使用的 CSI 驱动程序的文档了解此支持是否可用，以及如果可用时如何使用它。 
如果你所运行的 Kubernetes 不是 v{{< skew currentVersion >}}，请检查对应版本 Kubernetes 的文档。
...
- [CSIStorageCapacity](/docs/reference/kubernetes-api/config-and-storage-resources/csi-storage-capacity-v1/)
  对象：这些对象由 CSI 驱动程序在驱动程序所在的命名空间中生成。
