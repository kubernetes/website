...
-->
_标签_ 是附加到 Kubernetes 对象上的键值对，比如 Pods。
...
我们最终将标签索引和反向索引，用于高效查询和监视，使用它们在 UI 和 CLI 中进行排序和分组等。我们不希望污染具有非标识，特别是大型或结构化数据的标签。应使用 [注解](/docs/concepts/overview/working-with-objects/annotations/) 记录非识别信息
...
-->
服务部署和批处理流水线通常是多维实体（例如，多个分区或部署，多个发布追踪，多个层，每层多个微服务）。管理通常需要交叉操作，这打破了严格的层次表示的封装，特别是由基础设施而不是用户确定的严格的层次结构。
...
-->
这些只是常用标签的例子; 您可以自由地制定自己的约定。请记住，对于给定对象标签的键必须是唯一的。
...
-->
有效标签值必须为63个字符或更少，并且必须为空或以字母数字字符（`[a-z0-9A-Z]`）和破折号（`-`），下划线（`_`），点开头和结尾 （`.`）和之间的字母数字。
...
标签选择器可以由逗号分隔的多个 _需求_ 组成。在多个需求的情况下，必须满足所有要求，因此逗号分隔符充当逻辑_AND_（`&&`）运算符。
...
承认三种运算符`=`，`==`，`！=`。 前两个表示 _相等_（并且只是同义词），而后者表示 _不相等_。 例如：
...
-->
前者选择所有资源，其密钥等于 `environment`，值等于 `production`。
后者选择所有资源，其密钥等于 `tier`，值不同于 `frontend`，所有资源都没有带有 `tier` 键的标签。
可以使用逗号运算符来过滤 `production` 中的资源，不包括 `frontend`:`environment = production，tier！= frontend`
...
-->
基于等同的标签要求的一种使用场景是 Pods 要指定节点选择标准。例如，下面的示例 Pod 选择带有的节点
标签 "`accelerator=nvidia-tesla-p100`"。
...
-->
LIST and WATCH 操作可以使用查询参数指定标签选择器过滤一组对象。两个要求都是允许的。（这里显示的是它们出现在 URL 查询字符串中）

<!--
  * _equality-based_ requirements: `?labelSelector=environment%3Dproduction,tier%3Dfrontend`
  * _set-based_ requirements: `?labelSelector=environment+in+%28production%2Cqa%29%2Ctier+in+%28frontend%29`
-->
  * _基于相等_ 的要求: `?labelSelector=environment%3Dproduction,tier%3Dfrontend`
  * _基于集合_ 的要求: `?labelSelector=environment+in+%28production%2Cqa%29%2Ctier+in+%28frontend%29`
...
一个`服务`指向的一组 pods 是由标签选择器定义的。同样，一个`副本控制器`应该管理的 pods 的数量也是由标签选择器定义的。
...
`matchLabels` 是 `{key，value}` 对的映射。`matchLabels` 映射中的单个 `{key，value }` 等同于 `matchExpressions` 的元素，其 `key`字段为 "key"，`operator` 为 "In"，而 `values` 数组仅包含 "value"。`matchExpressions` 是 pod 选择器要求的列表。有效的运算符包括 In，NotIn，Exists 和 DoesNotExist。在 In 和 NotIn 的情况下，设置的值必须是非空的。来自 `matchLabels` 和 `matchExpressions` 的所有要求都是合在一起 -- 它们必须都满足才能匹配。
