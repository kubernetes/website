...
节点控制器包含 kubelet 中云依赖的功能，在引入 CCM 之前，kubelet 负责使用云的规格信息（如 IP 地址，域/区标签和实例类型信息）初始化节点。CCM 的引入已将此初始化操作从 kubelet 转移到 CCM 中。
...
在这个新模型中，kubelet 初始化一个没有云的规格信息的节点。但是，它会为新创建的节点添加污点，使节点不可调度，直到 CCM 使用云的规格信息初始化节点后，才会清除这种污点，便得该节点可被调度。
