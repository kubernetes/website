...
order to safely use Secrets, it is recommend you (at a minimum):
...
的任何人都可以以明文形式读取这些数据。
为了能够安全地使用 Secret，建议你（至少）：

1. 为 Secret [启用静态加密](/zh/docs/tasks/administer-cluster/encrypt-data/)；
2. [启用或配置 RBAC 规则](/zh/docs/reference/access-authn-authz/authorization/)来限制对 Secret 的读写操作。
...
[KubeletConfiguration struct](/docs/reference/config-api/kubelet-config.v1beta1/).
A Secret can be either propagated via watch (default), ttl-based, or by redirecting
...
Secret 可以通过 watch 操作来传播（默认），基于 TTL 来刷新，也可以
将所有请求直接重定向到 API 服务器。
...
-->
Kubernetes 特性 _不可变的 Secret 和 ConfigMap_ 提供了一种可选配置，
