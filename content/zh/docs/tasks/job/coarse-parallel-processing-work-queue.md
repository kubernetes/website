...
1. **启动一个消息队列服务**  本例中，我们使用 RabbitMQ，你也可以用其他的。练习中你可以创建一次消息队列服务然后再多个任务中重复使用。

1. **创建一个队列，放上消息数据**  每个消息表示一个要执行的任务。本例中，每个消息表示我们需要计算长度的一个整数。

1. **启动一个在队列中执行这些任务的 Job **。该 Job 启动了多个 Pod。每个 Pod 从消息队列中取走一个任务，处理它，然后重复执行，直到队列的最后。
...
本例使用了 RabbitMQ，使用其他 AMQP 类型的消息服务也许会容易些。

练习中你可以创建一次消息队列服务,然后在多个任务中重复使用，长期运行的服务也一样。
...
我们仅用到 [celery-rabbitmq example](https://github.com/kubernetes/kubernetes/tree/release-1.3/examples/celery-rabbitmq) 中描述的部分功能。
...
现在，我们可以试着访问消息队列。我们将会创建一个临时的非激活状态的 Pod，在它上面安装一些工具，然后用队列做实验。
...
最后一个命令中， `amqp-consume` 工具从队列中取走了一个消息，并把该消息传递给了随机命令的标准输出。在这种情况下，`cat` 只会打印它从标准输入或得得内容，echo 只会添加回车符以便示例可读。
...
实践中，如果有大量的数据需要被 Job 的所有 Pod 读取，典型的做法是把它们放在一个共享文件系统中，如NFS，并以只读的方式挂载到所有 Pod，或者 Pod 中的程序从类似 HDFS 的集群文件系统中读取。
...
现在我们可以创建一个做为任务来运行的镜像。

我们将用 `amqp-consume` 来从队列中读取消息并运行我们真实的程序。这里给出一个非常简单的示例程序：
...
现在，编译镜像。如果你在用源代码树，那么切换到目录 `examples/job/work-queue-1`。否则的话，创建一个临时目录，切换到这个目录。下载 [Dockerfile](/examples/application/job/rabbitmq/Dockerfile)，和 [worker.py](/examples/application/job/rabbitmq/worker.py)。对每一种情况，用下面的命令编译镜像：
...
本例中，每个 Pod 使用队列中的一个消息然后退出。这样，统计的完成数量就代表了完成的工作项的数量。本例中我们设置 `.spec.completions: 8`，因为我们放了8项内容在队列中。
...
这种处理方法的好处是你不需要修改你的 "worker" 程序使其知道工作队列的存在。

它需要你运行一个消息队列服务。如果不方便运行消息队列服务，你也许会考虑另外一种[job patterns](/docs/concepts/jobs/run-to-completion-finite-workloads/#job-patterns)。
...
这个处理方法为每个工作项创建了一个 Pod。如果你的工作项仅需数秒钟，为每个工作项创建 Pod会增加很多的常规消耗。如果要考虑另外的方案请参考[示例](/docs/tasks/job/fine-parallel-processing-work-queue/)，这种方案可以实现每个 Pod 执行多个工作项。
...
如果设置的完成数量大于队列中的消息数量，当队列中所有的消息都处理完成后，Job 也会显示为未完成。Job 将创建 Pod 并等待消息输入。
