...
weight: 40
description: >-
  为命名口空间定义一个有效的 CPU 资源限制范围，在该命名空间中每个新创建 Pod 的 CPU 资源是在设置的范围内的。
---
...
in the namespace.
-->
 本页介绍如何设置{{< glossary_tooltip text="命名空间" term_id="namespace" >}}中的容器和 Pod 使用的 CPU 资源的最小和最大值。
你可以通过
[LimitRange](/zh/docs/reference/kubernetes-api/policy-resources/limit-range-v1/)
对象声明 CPU 的最小和最大值.
...
以下为只有一个容器的 Pod 的清单。该容器声明了 CPU 请求 100 millicpu 和 CPU 限制 800 millicpu。
...
enforced minimum:
-->
输出结果显示 Pod 没有创建成功，因为容器声明的 CPU 请求小于最小值的限制：
...
以下为只有一个容器的 Pod 清单。该容器没有声明 CPU 请求，也没有声明 CPU 限制。
...
-->
输出结果显示 Pod 的唯一容器 CPU 请求为 800 millicpu， CPU 限制为 800 millicpu。
...
-->
因为你的容器没有声明自己的 CPU 请求和限制，
控制面会根据命名空间中配置 LimitRange 设置
