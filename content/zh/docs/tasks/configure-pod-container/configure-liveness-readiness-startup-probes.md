...
-->
在Kubernetes 1.23之前，常用 [grpc-health-probe](https://github.com/grpc-ecosystem/grpc-health-probe/) 来实现gRPC健康检测，
...
从grpc-health-probe探测器 迁移到 内置探测器时，请记住以下的区别：

- 内置探测器是相对于pod IP地址运行的，而 grpc-health-probe探测器通常是相对于 `127.0.0.1` 运行的。
  确保将gRPC endpoint配置为侦听Pod的IP地址。
- 内置探测器不支持任何身份验证参数（如：`-tls`）。
- 内置探测器中没有代码错误。所有的错误被视为探测失败。
- 如果`ExecProbeTimeout`特性开关设置为`false`，grpc-health-probe将**不**遵循`timeoutSeconds`的设置（默认为1秒），
  而内置探测器会在超时时失败。
