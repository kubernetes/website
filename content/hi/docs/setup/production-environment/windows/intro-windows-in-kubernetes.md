...
Windows पर कुबेरनेट्स सिंगल-स्टैक "IPv6-only" नेटवर्किंग का समर्थन नहीं करता है। हालांकि, एकल परिवार सेवाओं 
...
कुबेरनेट्स एक बहु-आर्किटेक्चर छवि रखता है जिसमें Windows के लिए समर्थन शामिल है। Kubernetes v1.22 के लिए अनुशंसित विराम छवि `k8s.gcr.io/pause:3.5` है। [स्रोत कोड](https://github.com/kubernetes/kubernetes/tree/master/build/pause) गिटहब पर उपलब्ध है।
...
कुबेरनेट्स [संस्करण-तिरछा नीति](/docs/setup/release/version-skew-policy/) भी लागू होता है।
...
1. क्या मैं कुबेरनेट्स नोड प्रक्रियाओं को पृष्ठभूमि में सेवाओं के रूप में चलाने के लिए कॉन्फ़िगर कर सकता हूं?
    ...
    1. `nssm.exe` . का उपयोग करना
       ...
       # NetworkMode      = नेटवर्क मोड l2bridge(flannel host-gw, डिफ़ॉल्ट मान) या नेटवर्क समाधान के रूप में चुना गया ओवरले(flannel vxlan)
       # ManagementIP     = Windows नोड को सौंपा गया आईपी पता। इसे खोजने के लिए आप ipconfig का उपयोग कर सकते हैं
       # ClusterCIDR      = क्लस्टर सबनेट रेंज। (डिफ़ॉल्ट मान 10.244.0.0/16)
       # KubeDnsServiceIP = कुबेरनेट्स DNS सेवा आईपी (डिफ़ॉल्ट मान 10.96.0.10)
       # LogDir           = निर्देशिका जहां kubelet और क्यूब-प्रॉक्सी लॉग को उनकी संबंधित आउटपुट फ़ाइलों में पुनर्निर्देशित किया जाता है (डिफ़ॉल्ट मान C:\k)
       ```
...
2. माई पॉड्स "कंटेनर क्रिएटिंग" पर अटके हुए हैं या बार-बार रीस्टार्ट हो रहे हैं
...
2. मेरे Windows पॉड बाहरी संसाधनों को पिंग नहीं कर सकते हैं
    ...
    यदि आप अभी भी समस्याओं का सामना कर रहे हैं, तो सबसे अधिक संभावना है कि आपका नेटवर्क कॉन्फ़िगरेशन [cni.conf](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/l2bridge/cni/config/cni.conf) कुछ अतिरिक्त ध्यान देने योग्य है। आप इस स्थिर फ़ाइल को कभी भी संपादित कर सकते हैं। कॉन्फ़िगरेशन अद्यतन किसी भी नए कुबेरनेट्स संसाधनों पर लागू होगा।
             
    कुबेरनेट्स नेटवर्किंग आवश्यकताओं में से एक ([कुबेरनेट्स मॉडल](/docs/concepts/cluster-administration/networking/) देखें) यह है कि क्लस्टर संचार आंतरिक रूप से NAT के बिना होता है। इस आवश्यकता का सम्मान करने के लिए, एक है [अपवाद सूची](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/l2bridge/cni/config/cni.conf#L20) उन सभी संचारों के लिए जहां आप नहीं चाहते कि आउटबाउंड NAT हो। तथापि,इसका मतलब यह भी है कि आपको उस बाहरी आईपी को बाहर करना होगा जिसे आप क्वेरी करने का प्रयास कर रहे हैं `ExceptionList` से। तभी आपके Windows से आने वाला ट्रैफिक होगा पॉड्स को बाहरी दुनिया से प्रतिक्रिया प्राप्त करने के लिए सही ढंग से SNAT'ed किया जाना चाहिए। इसमें संबंध में, आपकी `ExceptionList` `cni.conf` में इस प्रकार दिखनी चाहिए:
...
* कुबेरनेट्स आधिकारिक फोरम [discuss.kubernetes.io](https://discuss.kubernetes.io/)
...
* कुबेरनेट्स संस्करण: `kubectl version` से आउटपुट
...
Kubeadm टूल आपको कुबेरनेट्स क्लस्टर को तैनात करने में मदद करता है, क्लस्टर को प्रबंधित करने के लिए कंट्रोल प्लेन प्रदान करता है, और आपके वर्कलोड को चलाने के लिए नोड्स प्रदान करता है। [Windows नोड्स जोड़ना](/docs/tasks/administer-cluster/kubeadm/adding-windows-nodes/) बताता है कि kubeadm का उपयोग करके अपने क्लस्टर में Windows नोड्स को कैसे तैनात किया जाए।
