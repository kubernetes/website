...
title: डिवाइस प्लगइन
description: >
  डिवाइस प्लगइन आपको उन डिवाइस या रिसोर्स के सपोर्ट के साथ अपने क्लस्टर को कॉन्फ़िगर करने देते हैं
...
{{< glossary_tooltip term_id="kubelet" >}} के लिए एडवरटाइज़ करने के लिए कर सकते हैं।

खुद कुबरनेट्स के कोड को कस्टमाइज़ करने के बजाय, वेंडर एक डिवाइस प्लगइन इम्प्लीमेंट कर सकते हैं जिसे आप
...
एक डिवाइस प्लगइन इस gRPC सेवा के माध्यम से kubelet के साथ स्वयं को रजिस्टर कर सकता है। रजिस्ट्रेशन के
दौरान, डिवाइस प्लगइन को यह भेजना होता है:

* इसके Unix सॉकेट का नाम।
* वह Device Plugin API वर्शन जिसके लिए इसे बनाया गया है।
* वह `ResourceName` जिसे यह एडवरटाइज़ करना चाहता है। यहाँ `ResourceName` को
  ...
  (उदाहरण के लिए, एक NVIDIA GPU को `nvidia.com/gpu` के रूप में एडवरटाइज़ किया जाता है।)

सफल रजिस्ट्रेशन के बाद, डिवाइस प्लगइन kubelet को उन डिवाइस की सूची भेजता है जिन्हें यह मैनेज करता है,
और उसके बाद kubelet, नोड स्टेटस अपडेट के हिस्से के रूप में उन रिसोर्स को API सर्वर पर एडवरटाइज़ करने
की ज़िम्मेदारी लेता है। उदाहरण के लिए, किसी डिवाइस प्लगइन द्वारा kubelet के साथ
`hardware-vendor.example/foo` रजिस्टर करने और किसी नोड पर दो हेल्दी डिवाइस रिपोर्ट करने के बाद, नोड
स्टेटस को यह एडवरटाइज़ करने के लिए अपडेट किया जाता है कि नोड पर 2 "Foo" डिवाइस इंस्टॉल्ड और उपलब्ध हैं।
...
एक्सटेंडेड रिसोर्स का अनुरोध करना अन्य रिसोर्स के लिए रिक्वेस्ट और लिमिट मैनेज करने जैसा ही है, इन
अंतरों के साथ:
* एक्सटेंडेड रिसोर्स केवल इंटीजर रिसोर्स के रूप में सपोर्टेड हैं और इन्हें ओवरकमिट नहीं किया जा सकता।
...
रिसोर्स को एडवरटाइज़ करता है। यहाँ एक डेमो वर्कलोड चलाने के लिए इस रिसोर्स का अनुरोध करने वाले पॉड का
उदाहरण है:
...
1. प्लगइन एक gRPC सेवा शुरू करता है, जिसमें होस्ट पाथ `/var/lib/kubelet/device-plugins/` के अंतर्गत
   ...
   से प्रभावित नहीं होता), और यह निम्न इंटरफ़ेस इम्प्लीमेंट करता है:
   ...
   {{< note >}}
   प्लगइन के लिए `GetPreferredAllocation()` या `PreStartContainer()` का उपयोगी इम्प्लीमेंटेशन देना
...
1. प्लगइन होस्ट पाथ `/var/lib/kubelet/device-plugins/kubelet.sock` पर मौजूद Unix सॉकेट के माध्यम से
   kubelet के साथ स्वयं को रजिस्टर करता है।

   {{< note >}}
   वर्कफ़्लो का क्रम महत्वपूर्ण है। सफल रजिस्ट्रेशन के लिए किसी प्लगइन को kubelet के साथ स्वयं को
   रजिस्टर करने से पहले gRPC सेवा को सर्व करना शुरू करना ज़रूरी है।
   {{< /note >}}

1. स्वयं को सफलतापूर्वक रजिस्टर करने के बाद, डिवाइस प्लगइन सर्विंग मोड में चलता है, जिसके दौरान यह
   डिवाइस की हेल्थ मॉनिटर करता रहता है और किसी भी डिवाइस स्टेटस बदलाव पर kubelet को रिपोर्ट करता है।
   यह `Allocate` gRPC रिक्वेस्ट को सर्व करने के लिए भी जिम्मेदार है। `Allocate` के दौरान, डिवाइस
   प्लगइन डिवाइस-विशिष्ट तैयारी कर सकता है; उदाहरण के लिए, GPU क्लीनअप या QRNG इनिशियलाइज़ेशन। अगर
   ऑपरेशन सफल होते हैं, तो डिवाइस प्लगइन एक `AllocateResponse` लौटाता है जिसमें एलोकेटेड डिवाइस को
   एक्सेस करने के लिए कंटेनर रनटाइम कॉन्फ़िगरेशन होते हैं। kubelet यह जानकारी कंटेनर रनटाइम को भेजता है।

   एक `AllocateResponse` में शून्य या अधिक `ContainerAllocateResponse` ऑब्जेक्ट होते हैं। इनमें,
   डिवाइस प्लगइन उन बदलावों को परिभाषित करता है जो डिवाइस तक एक्सेस देने के लिए किसी कंटेनर की
   ...
   * फ़ुली-क्वालिफ़ाइड CDI डिवाइस नाम
   ...
   {{< note >}}
   फ़ुली-क्वालिफ़ाइड CDI डिवाइस नामों को Device Manager द्वारा प्रोसेस करने के लिए यह आवश्यक है कि
...
### kubelet रीस्टार्ट को हैंडल करना

एक डिवाइस प्लगइन से यह अपेक्षा की जाती है कि वह kubelet रीस्टार्ट को डिटेक्ट करे और नए kubelet इंस्टेंस
के साथ स्वयं को फिर से रजिस्टर करे। जब कोई नया kubelet इंस्टेंस शुरू होता है, तो यह
`/var/lib/kubelet/device-plugins` (डिवाइस प्लगइन के लिए हार्डकोडेड पाथ) के अंतर्गत मौजूद सभी Unix
सॉकेट को डिलीट कर देता है। एक डिवाइस प्लगइन अपने Unix सॉकेट के डिलीशन को मॉनिटर कर सकता है और ऐसे इवेंट
पर स्वयं को फिर से रजिस्टर कर सकता है।
...
जो पॉड फ़ेल हो गए डिवाइस को असाइन किए गए थे, वे इस डिवाइस को असाइन रहेंगे। यह सामान्य बात है कि डिवाइस
...
`allocatedResourcesStatus` फ़ील्ड कंटेनर को असाइन किए गए हर डिवाइस के लिए हेल्थ जानकारी रिपोर्ट करती
...
एक्सेलेरेटर ओवर-टेम्परेचर इवेंट रिपोर्ट कर रहा है, तो `allocatedResourcesStatus` फ़ील्ड इसे रिपोर्ट कर
सकती है।
...
रखने के लिए, फ़ेल्योर के बाद डेमन पॉड को रीस्टार्ट करने के लिए, और अपग्रेड को ऑटोमेट करने में मदद करने
के लिए।
...
* बैकवर्ड/फ़ॉरवर्ड कॉम्पेटिबिलिटी के लिए डिवाइस प्लगइन API के कई वर्शन को सपोर्ट करें।

ऐसे नोड पर डिवाइस प्लगइन चलाने के लिए जिन्हें एक नए डिवाइस प्लगइन API वर्शन वाले कुबरनेट्स रिलीज़ में
अपग्रेड करना है, इन नोड को अपग्रेड करने से पहले अपने डिवाइस प्लगइन को दोनों वर्शन को सपोर्ट करने के लिए
अपग्रेड करें। यह तरीका अपनाने से अपग्रेड के दौरान डिवाइस एलोकेशन का निरंतर काम करना सुनिश्चित होगा।

## डिवाइस प्लगइन रिसोर्स की मॉनिटरिंग {#monitoring-device-plugin-resources}

{{< feature-state for_k8s_version="v1.28" state="stable" >}}

डिवाइस प्लगइन द्वारा प्रदान किए गए रिसोर्स को मॉनिटर करने के लिए, मॉनिटरिंग एजेंट को नोड पर उपयोग हो
रहे डिवाइस के सेट का पता लगाने और यह बताने वाला मेटाडेटा प्राप्त करने में सक्षम होना चाहिए कि मेट्रिक
किस कंटेनर से जुड़ा होना चाहिए। डिवाइस मॉनिटरिंग एजेंट द्वारा एक्सपोज़ किए गए
...
CPU की id, डिवाइस प्लगइन द्वारा रिपोर्ट की गई डिवाइस id, और जिन डिवाइस को एलोकेट किया गया है उस NUMA
...
kubelet को एक्सपोज़ किए जा रहे अंतर्निहित रिसोर्स में बदलाव न होने तक, वही रहेगा। ऐसा शायद ही कभी होता
...
`ContainerDevices` टोपोलॉजी जानकारी एक्सपोज़ करते हैं जो यह बताती है कि डिवाइस किस NUMA सेल से जुड़ा है।
NUMA सेल की पहचान एक ऑपेक इंटीजर ID का उपयोग करके की जाती है, जिसकी वैल्यू उससे मेल खाती है जो डिवाइस
प्लगइन
[kubelet के साथ स्वयं को रजिस्टर करते समय](/docs/concepts/extend-kubernetes/compute-storage-net/device-plugins/#device-plugin-integration-with-the-topology-manager)
रिपोर्ट करते हैं।

gRPC सेवा को kubelet की रूट डायरेक्टरी के भीतर `pod-resources/kubelet.sock` पर एक unix सॉकेट के
माध्यम से सर्व किया जाता है (आमतौर पर `/var/lib/kubelet/pod-resources/kubelet.sock`)। डिवाइस प्लगइन
रिसोर्स के लिए मॉनिटरिंग एजेंट को एक डेमन के रूप में, या DaemonSet के रूप में डिप्लॉय किया जा सकता है।
kubelet रूट डायरेक्टरी के अंतर्गत कैनोनिकल डायरेक्टरी `pod-resources` (आमतौर पर
`/var/lib/kubelet/pod-resources`) को प्रिविलेज्ड एक्सेस की आवश्यकता होती है, इसलिए मॉनिटरिंग एजेंट को
प्रिविलेज्ड सिक्योरिटी कॉन्टेक्स्ट में चलना चाहिए। अगर कोई डिवाइस मॉनिटरिंग एजेंट DaemonSet के रूप में
चल रहा है, तो `pod-resources` डायरेक्टरी को डिवाइस मॉनिटरिंग एजेंट के
...
कंटेनर माउंट को सॉकेट या डायरेक्टरी को संदर्भित करने वाले inode द्वारा मैनेज किया जाता है, यह इस पर
निर्भर करता है कि क्या माउंट किया गया था। जब kubelet रीस्टार्ट होता है, तो सॉकेट डिलीट हो जाता है और
...
जानकारी जैसी ही जानकारी एक्सपोज़ करता है। `Get` एंडपॉइंट को चल रहे Pod के `PodName` और `PodNamespace`
की आवश्यकता होती है।
...
## Topology Manager के साथ डिवाइस प्लगइन इंटीग्रेशन {#device-plugin-integration-with-the-topology-manager}

{{< feature-state for_k8s_version="v1.27" state="stable" >}}

Topology Manager एक Kubelet कॉम्पोनेंट है जो रिसोर्स को टोपोलॉजी के अनुसार समन्वित करने की अनुमति देता
है। ऐसा करने के लिए, Device Plugin API को एक `TopologyInfo` स्ट्रक्ट शामिल करने के लिए एक्सटेंड किया
गया।
...
device manager इस जानकारी का उपयोग Topology Manager से सलाह लेने और रिसोर्स असाइनमेंट के निर्णय लेने
के लिए करता है।

`TopologyInfo` `nodes` फ़ील्ड को `nil` या NUMA नोड की सूची पर सेट करने का सपोर्ट देता है। इससे Device
Plugin ऐसे डिवाइस को एडवरटाइज़ कर सकता है जो कई NUMA नोड में फैला हो।
...
डिवाइस प्लगइन इम्प्लीमेंटेशन के कुछ उदाहरण यहाँ दिए गए हैं:

* [Akri](https://github.com/project-akri/akri), जो आपको हेटरोजीनियस लीफ़ डिवाइस (जैसे IP कैमरा और USB डिवाइस) को आसानी से एक्सपोज़ करने देता है।
...
* NVIDIA GPU को एक्सपोज़ करने और GPU हेल्थ मॉनिटर करने के लिए NVIDIA का
...
* डिवाइस प्लगइन का उपयोग करके [GPU रिसोर्स को शेड्यूल करने](/docs/tasks/manage-gpus/scheduling-gpus/) के
  बारे में जानें
* किसी नोड पर [एक्सटेंडेड रिसोर्स एडवरटाइज़ करने](/docs/tasks/administer-cluster/extended-resource-node/) के
  बारे में जानें
