...
[Service](/docs/reference/kubernetes-api/service-resources/service-v1/). This
annotation has no effect in Kubernetes {{< skew currentVersion >}}.
...
The control plane adds this label to EndpointSlice and Endpoints objects when the owning Service is headless
(as a hint to the service proxy that it can ignore these endpoints).
...
API is deprecated in favor of
[EndpointSlice](/docs/reference/kubernetes-api/service-resources/endpoint-slice-v1/).
A Service can have multiple EndpointSlice objects, so EndpointSlices never need to be truncated.
{{< /note >}}
