...
Part of the [ApplySet specification](https://github.com/kubernetes/enhancements/tree/master/keps/sig-cli/3659-kubectl-apply-prune#design-details-applyset-specification). This label can be set on a CRD to identify the custom resource type it defines (not the CRD itself) as an allowed parent for an ApplySet. The value of the label MUST be `"true"`.
