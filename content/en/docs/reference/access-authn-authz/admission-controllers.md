...
control plugins to invoke prior to modifying objects in the cluster.
...
admission control plugins to be disabled, even if they are in the list of plugins enabled by default.
...
Such kubelets will only be allowed to modify their own `Node` API object and `Pod` API objects that are bound to their node.
...
`validatingadmissionpolicy` and `admissionregistration.k8s.io/v1alpha1` group/version are enabled.
