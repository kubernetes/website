...
Kubernetes expects a client certificate with a subject that contains a `commonName` attribute,
...
Kubernetes can parse the user UID from a certificate. UID is different from
user name; it is an opaque value. For example, the UID could be `1042` or `d3f77937-ec82-4f16-8010-61821abe315a`.
To use this feature, the certificate must have the attribute 1.3.6.1.4.1.57683.2 included, and the `AllowParsingUserUIDFromCertAuth` [feature gate](/docs/reference/command-line-tools-reference/feature-gates/)
