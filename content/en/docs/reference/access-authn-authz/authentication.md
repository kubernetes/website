...
Any kind of **authenticated** user can attempt to [impersonate](#user-impersonation) another user.
The other user can be the same kind (for example: a ServiceAccount impersonating a different ServiceAccount),
or a different kind (for example, a normal user impersonating a ServiceAccount).
If either kind of user is a member of `system:masters` (a special privileged group name that is meaningful to
Kubernetes), then that principal can impersonate any other user or group.

Read the section on impersonation to learn more about this, including the checks that Kubernetes does before
allowing impersonation to happen.

## Attributes of authenticated users {#authenticated-user-attributes}
...
{{< note >}}
Username and extra data values are opaque to the authentication system and only hold significance
when interpreted by an [authorizer](/docs/reference/access-authn-authz/authorization/).

Kubernetes does give special treatment to some group names, including `system:authenticated` and `system:masters`,
but this special behavior is part of the authorization mechanisms. The authentication aspect of Kubernetes does
set these group memberships but doesn't otherwise give them any special treatment.
{{< /note >}}
...
Impersonation also enables a user to change what groups they are a member of; if a user
is authorized to impersonate being a member of that group, they can use impersonation to
act as if they are in that group.
