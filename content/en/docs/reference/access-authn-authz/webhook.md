...
- `condition`: Optional if it can be inferred from `id`. The condition expression to evaluate (format depends on `type`, also optional if inferred from `id`)
...
- `admissionControlData`: Contains fields similar to those in [AdmissionReview](/docs/reference/access-authn-authz/extensible-admission-controllers/#request), including:
  ...
  - `dryRun`: Indicates whether modifications will be persisted
