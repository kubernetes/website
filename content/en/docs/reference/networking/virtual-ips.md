...
reserved for your service. For example, you might reserve ports 
31000-31009 for your service.

To define a static subrange, you can set the nodePort field in your 
service's YAML configuration to a specific port number within the 
NodePort range, followed by a hyphen and the upper bound of the 
reserved range. Kubernetes will then ensure that the specified range 
of ports is reserved for your service, and will not assign those 
ports to any other services.
...
the static subrange must be a subset of the default NodePort range, and must 
not overlap with any other static assignment subranges used by other services in the cluster.
