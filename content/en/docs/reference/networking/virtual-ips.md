...
* `trafficDistribution` Influence: For a given Service, if a traffic policy
  ...
  default), or if the fields are not set, then `trafficDistribution: PreferClose`
...
* **Implementation-specific behavior:** Each data plane implementation may handle
  this field slightly differently. If you're using an implementation other than
  kube-proxy, refer to the documentation specific to that implementation to
