...
Resource version strings are comparable within the same resource type for all
types served by kube-apiserver. This includes built-in API types and types
backed by custom resource definitions. Starting with Kubernetes 1.35,
comparability of resource versions for all Kubernetes types is included in
Certified Kubernetes requirements. Both resource versions must be from objects
of the same API group and resource type. For example, two deployments from the
apps API group can have their resource versions compared, but a pod and a
deployment cannot.
