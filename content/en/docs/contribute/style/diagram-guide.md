...
{{< note >}}
You can click the caption for each diagram in this section to view the code and rendered diagram in the Mermaid live editor.
{{< /note >}}
...
{{< note >}}
You can still use Mermaid even if it's not supported in your environment. This method is called __Mermaid+SVG__ and is explained below.
{{< /note >}}
...
4. Add a caption using the `<figcaption>` tag.
...
<figcaption><p style="..">Figure 17. A to B</p></figcaption> 
more text
```
{{< note >}}
You must include the `{{</*  mermaid */>}}`, `{{</* /mermaid */>}}` shortcode tags at the start and end of the Mermaid code block. You should add a diagram caption using the `<figcaption>` tag. 
{{< /note >}}

For more details on diagram captions, see the [How to use captions](#how-to-use-captions).
...
5. Add a caption using the `<figcaption>` tag.
...
Reference the `.svg` image file in the PR `.md` file using the `{{</* figure */>}}` shortcode.
```json
{{</* figure src="/static/images/boxnet.svg" alt="Boxnet figure" class="diagram-large" */>}}
```

Add a caption to the diagram using the `<figcaption>` tag: 

```text
<figcaption><p style="padding-left: 310px; font-weight: bold">Figure 15. A Boxnet diagram</p></figcaption><br>
```

For more details on diagram captions, see [How to use captions](#how-to-use-captions).

{{< note >}}
The `{{</* figure */>}}` shortcode is the preferred method for adding `.svg` image files to your documentation. You can also use the standard markdown image syntax like so: `![my boxnet diagram](static/images/boxnet.svg)`. 
{{< /note >}}
...
Be sure to check the diagram renders properly using the [local](https://kubernetes.io/docs/contribute/new-content/open-a-pr/#preview-locally) and Netlify previews.
...
Here is the `{{</* figure */>}}` shortcode and `<figcaption>` tags for the `images/apple.svg` diagram: 
```text
{{</* figure src="/static/images/apple.svg" alt="red-apple-figure" class="diagram-large" */>}} // our figure
<figcaption><p style="padding-left: 310px; font-weight: bold">Figure 9. A Red Apple</p></figcaption><br> // our figure caption
```
...
Don't forget to check the rendered diagram using the [local](https://kubernetes.io/docs/contribute/new-content/open-a-pr/#preview-locally) and Netlify previews.
...
<figcaption><a href="https://mermaid-js.github.io/mermaid-live-editor/edit/#"><p style="padding-left: 310px; font-weight: bold">Figure 6. Pod Topology Spread Constraints.</p></a></figcaption><br>
...
<figcaption><a href="https://mermaid-js.github.io/mermaid-live-editor/edit/#"><p style="padding-left: 310px; font-weight: bold">Figure 7. Ingress.</p></a></figcaption><br>
...
The combination of some text and a diagram tied together with a caption provide a concise representation of the information you wish to convey to the user. 
...
Proper captioning involves three components: diagram, diagram caption and the diagram referral. As an example, consider a diagram `.svg` image file saved to `/images/docs/components-of-kubernetes.svg`.
...
```
You should only pass the `src`, `alt` and `class` values into the `{{</* figure */>}}` shortcode. You can adjust the size of the diagram using `diagram-large`, `diagram-medium` and `diagram-small` classes. You can't style the `caption` value in this shortcode, and it only supports diagram image files. 

{{< note >}}
Diagrams created using the `Inline` method don't use the `{{</* figure */>}}` shortcode.
{{< /note >}}

**Diagram Caption**

Next, add a diagram caption. You must use the following format:
```text
...
* Prepend your diagram caption with `Figure NUMBER.`. You must use `Figure` and the number must be unique for each diagram in your documentation page. 
* Add your diagram caption text after the `Figure NUMBER.` on the same line. Keep it short.
...
Check out the source code for this page, `diagram-guide.md`, for more examples of diagram captions.

**Diagram Referral**

Finally, you can add a diagram caption referral. This is used inside your text. It allows a user to connect your text with the diagram. The `Figure NUMBER` in your referral and caption must match.   

Here is an example of a caption referral included in your content text:
```text
...
Figure 9 shows the Kubernetes Architecture diagram that includes the diagram, diagram caption and diagram caption referral. The `{{</* figure */>}}` shortcode renders the diagram. The `<figcaption>` defines the caption. The caption referral is contained in this paragraph.

Here are the `{{</* figure */>}}` shortcode and `<figcaption>` statements for this diagram:
```
{{</* figure src="/images/docs/components-of-kubernetes.svg" alt="Kubernetes pod running inside a cluster" class="diagram-large" */>}}
<figcaption><a href="https://kubernetes.io/docs/concepts/overview/components/"><p style="padding-left: 310px; font-weight: bold">Figure x. Kubernetes Architecture Test.</p></a></figcaption><br>
```

{{< figure src="/images/docs/components-of-kubernetes.svg" alt="Kubernetes pod running inside a cluster" class="diagram-large" >}}
<figcaption><p style="padding-left: 310px; font-weight: bold">Figure 9. Kubernetes Architecture.</p></figcaption><br>
...
* Click on live editor links in this section to look over more examples.
