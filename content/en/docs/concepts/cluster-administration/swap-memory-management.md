...
to increase available memory for workloads.
...
shield nodes from system-level memory spikes that might compromise its stability,
allow for more flexible memory management on the node, and much more.
...
## Prerequisites
 ...
 - Swap must be enabled and provisioned on the Linux node.
...
Kubernetes only supports configurable swap for Linux nodes.
...
- Direct the Container Runtime Interface to allocate zero swap memory
...
node3   3m           10%      4Mi             10%         <unkown>      <unkown>   
```
...
In Linux, the `vm.min_free_kbytes` parameter the memory threshold for the kernel
...
- For more information, please see the original KEP, [KEP-2400](https://github.com/kubernetes/enhancements/issues/4128),
