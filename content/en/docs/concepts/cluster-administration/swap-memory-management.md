...
It offers numerous benefits, such as effectively increasing a {{< glossary_tooltip text="node's" term_id="node" >}}
...
The `<unknown>` value indicates that the `node.status.nodeInfo.swap.capacity` field is not set.
...
Currently, Kubernetes does not support scheduling Pods based on swap memory usage.
