...
* `service.beta.kubernetes.io/aws-load-balancer-security-groups`: Used to specify the security groups to be added to the ELB created. This replaces all other security groups previously assigned to the ELB. Security groups defined here should not be shared between services. This annotation also overrides the creation of a uniquely generated security group for this ELB. Only use this annotation when providing a unique list of security groups, as the first security group ID on this list is used as a source in the security group rules to permit traffic to target worker nodes (service traffic and health checks). If a non-unique Security Group ID is used for multiple ELBs, only a single permit line is added to worker node security groups. If you delete any of those ELBs it deletes the single PERMIT line and removes access for all ELBs sharing the same security group ID. This can cause a cross-service outage if not used properly.
* `service.beta.kubernetes.io/aws-load-balancer-extra-security-groups`: Used to specify additional security groups to be added to the created ELB in addition to the generated, unique Security Group. Security groups defined here can be shared between services. Using this annotation by itself leaves the uniquely generated security group in place, which is added to the worker node security groups as source to allow traffic (service traffic and health checks). This ensures that every ELB has a unique security group ID and a matching PERMIT line in the worker node security groups to allow traffic.
