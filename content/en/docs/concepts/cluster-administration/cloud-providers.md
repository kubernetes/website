...
* `service.beta.kubernetes.io/aws-load-balancer-security-groups`: Used to specify the security groups to be added to ELB created unlike `service.beta.kubernetes.io/aws-load-balancer-extra-security-groups`, this replaces all other security groups previously assigned to the ELB. In order to use this you will have to make sure that the specified existing security groups and `ServiceSpec.loadBalancerSourceRanges` have same IP ranges.
