...
cluster (for example, scheduling), and they detect and respond to cluster events (for example, starting up a new [pod](/docs/concepts/workloads/pods/pod) when a replication controller's `replicas` field is unsatisfied).
