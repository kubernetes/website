...
### Standard device attributes {#standard-device-attributes}

Kubernetes defines a set of standard device attributes under the
`resource.kubernetes.io` domain. These attributes have well-defined semantics
so that `matchAttribute` constraints work across devices from different DRA
drivers without requiring driver-specific knowledge.
...
The following standard device attributes are defined:
...
: Identifies the NUMA topology of a device. This attribute can be either:
  ...
  - `GetNUMANodeAttributeByPCIBusID(pciBusID, listEnabled)` — for PCI devices
    (GPUs, NICs, NVMe controllers). Reads `numa_node` from sysfs and, when
    `listEnabled` is true, computes the SLIT-based list.
  - `GetNUMANodeAttribute(numaNode, listEnabled)` — for devices that already
