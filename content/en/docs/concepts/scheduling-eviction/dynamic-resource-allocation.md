...
is added to each entry in the `status.containerStatuses` field of the Pod specification. The `allocatedResourcesStatus`
field reports health information for each device assigned to the container.

For a failed Pod, or where you suspect a fault, you can use the `allocatedResourcesStatus` field to understand
whether the Pod behavior might be associated with device failure. For example, if an accelerator is reporting
an over-temperature event, the `allocatedResourcesStatus` field might help you to identify the event.
