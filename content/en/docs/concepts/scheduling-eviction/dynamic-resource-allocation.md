...
1. Read the status to see pool availability:
   ...
   - `pools`: a list of pool details, each containing:
     ...
     - `validationError`: set when the pool's data could not be fully validated
       ...
       may be unset.
     - `partitionSummary`: for [partitionable](#partitionable-devices) pools,
       per-partition-type allocatability (see
...
For pools that use [partitionable devices](#partitionable-devices) or shareable
devices (devices that allow multiple allocations), a ResourcePoolStatusRequest
can report two additional per-pool views: `partitionSummary` and
`shareableSummary`.
