...
: Defines a category of devices that can be claimed and how to select specific
  device attributes in claims. The DeviceClass parameters can, but don't have to,
  match devices in ResourceSlices. To claim devices from a DeviceClass,
...
{{< glossary_tooltip term_id="device" text="devices" >}} in a pool of
devices. The pool is managed by a device driver, which creates and manages
ResourceSlices. The resources in a pool might be represented by a single
ResourceSlice or span multiple ResourceSlices. 
...
enabling the DynamicResourceAllocation feature gate and the core DRA
