...
each Pod has its own sidecar. A typically sidecar fetches configuration data over the
...
You have two options for configuration using init containers:
...
This is also done using a volume, but only the init container mounts that volume.
