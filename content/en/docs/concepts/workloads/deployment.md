...
You describe a _desired state_ in a Deployment, and the [Deployment controller](/docs/reference/controllers/deployment) changes the actual state to the desired state at a controlled rate. You can define Deployments to create new ReplicaSets, or to remove existing Deployments and adopt all their resources with new Deployments.

{{< note >}}
Do not manage ReplicaSets owned by a Deployment. If your use case is not covered below, consider opening an issue in the main Kubernetes repository.
{{< /note >}}
...
* The `template` field contains the following sub-fields:
  ...
  * Create one container and name it `nginx` using the `name` field.
  ...
  `app: nginx`). Do not overlap labels or selectors with other resources
  (including other Deployments and StatefulSets).
  Kubernetes doesn't stop you from overlapping. If multiple resources have overlapping selectors, they might conflict and behave unexpectedly.
  {{< /note >}}
...
The [Deployment controller](/docs/reference/controllers/deployment) adds a `pod-template-hash` label to every ReplicaSet that a Deployment creates or adopts.
...
* Running `get pods` should now show only the new Pods:
    ...
    The Deployment resource ensures (via associated controllers) that only a certain number of Pods are
    down while they are being updated. By default, at least 25% of the desired number of Pods must be available.
    ...
    The Deployment controller makes sure that at least 2 Pods are available and that there are no more than 4 available Pods.
...
Each time the [Deployment controller](/docs/reference/controllers/deployment) observes a new Deployment
object, the controller creates a new ReplicaSet to bring up the desired Pods if there is no existing
ReplicaSet doing so. Existing ReplicaSet controlling Pods whose labels match `.spec.selector` but whose
template does not match `.spec.template` are scaled down. Eventually, the new
...
* Looking at the Pods created, you see that 1 Pod created by new ReplicaSet is stuck in an image pull loop.
    ...
    {{< note >}}
    The [Deployment controller](/docs/reference/controllers/deployment) stops the bad rollout automatically, and stops scaling up the new
...
or paused), then the [Deployment controller](/docs/reference/controllers/deployment) will balance the additional replicas in the existing active
...
workloads you may be running, or by increasing quota in your namespace. If you satisfy the quota
...
another Deployment, or by creating another resource such as a ReplicaSet. If you do so, the first
Deployment thinks that it created these other Pods. Kubernetes does not stop you from doing this.
{{< /note >}}
...
and `Reason=ProgressDeadlineExceeded` in the status of the resource. The
[Deployment controller](/docs/reference/controllers/deployment) will keep retrying the Deployment.
{{< note >}}
In the future, once automatic rollback is implemented, the Deployment controller will roll back a
...
* Read the [Deployment controller](/docs/reference/controllers/deployment) reference documentation
{{% /capture %}}
