...
The Pod garbage collector (PodGC), which is a controller in control plane, cleans up terminated Pods (with a phase of `Succeeded` or
`Failed`), when the number of Pods exceeds the configured threshold
(determined by `terminated-pod-gc-threshold` in the kube-controller-manager).
This avoids a resource leak as Pods are created and terminated over time.

Additionally, PodGC cleans up any Pods which satisfy any of the following conditions:
1. are terminating pods, bound to a non-ready node tainted with `node.kubernetes.io/out-of-service`
2. are orphan pods - bound to a node which no longer exists,
3. are unscheduled terminating pods.

When the `PodDisruptionConditions` feature gate is enabled PodGC, along with
cleaning up the pods, will also mark them as failed if they are in a non-terminal
