...
File storage is useful when building stateful applications. Use volumes to declaratively manage storage for containers. The Kubernetes `Volume` abstraction facilitates sharing files between containers on the same `Pod`, and allows files to persist when a container is restarted. 
...
{{< note >}}
A Container using Downward API as a [subPath](#using-subpath) volume mount will not receive Downward API updates.
{{< /note >}}
...
other node constraints the Pod may have, such as node resource requirements, node selectors, Pod affinity, and Pod anti-affinity.
