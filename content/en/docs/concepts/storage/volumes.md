...
coincides with the lifetime of its pod. Consequently, a volume outlives any containers
...
The `CSIMigration` feature for `azureDisk`, when enabled, shims all plugin operations
...
The `CSIMigration` feature for `azureFile`, when enabled, shims all plugin operations
...
that data can be transferred between pods. The `cephfs` volume can be mounted by multiple
...
The `CSIMigration` feature for Cinder, when enabled, shims all plugin operations
...
can be transferred between pods as required.
...
pre-populated with data, and that data can be transferred between pods.
...
The `CSIMigration` feature for GCE PD, when enabled, shims all plugin operations
...
[Rados Block Device](https://ceph.com/docs/master/rbd/rbd/) (RBD) volume to be mounted into your
Pod. Unlike `emptyDir`, which is erased when a pod is removed, the contents of
a `rbd` volume are preserved and the volume is merely unmounted. This
means that a RBD volume can be pre-populated with data, and that data can
be transferred between pods.
...
The `CSIMigration` feature for `vsphereVolume`, when enabled, shims all plugin operations
