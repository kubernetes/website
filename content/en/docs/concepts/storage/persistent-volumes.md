...
The following PersistentVolume types are supported through the `CSIMigration` feature gate and require the corresponding CSI driver to be installed in the cluster. Kubernetes offers built-in APIs to facilitate their use.

* [`awsElasticBlockStore`](/docs/concepts/storage/volumes/#awselasticblockstore) - AWS Elastic Block Store (EBS)
  &mdash; but for Kubernetes version 1.27 and onwards, this volume type is **only** available via a separate CSI driver 
...
* [`gcePersistentDisk`](/docs/concepts/storage/volumes/#gcepersistentdisk) - GCE Persistent Disk &mdash; but for Kubernetes version 1.28 and onwards, this volume type is **only** available via a separate CSI driver 
