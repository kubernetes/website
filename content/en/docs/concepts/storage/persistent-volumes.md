...
PersistentVolumeClaims can be expanded to request more storage without recreating them,
provided their StorageClass has `allowVolumeExpansion` set to true.

See [Expanding Persistent Volumes](/docs/concepts/storage/expanding-persistent-volumes/)
for the supported volume types, how to trigger a resize, and how to recover from a failed expansion.
