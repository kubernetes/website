...
Every [`Pod`](/docs/concepts/workloads/pods/) in a cluster gets its own unique cluster-wide IP address(one address per IP address family). This means that you don't need to explicitly create links between `Pods` and you almost never need to deal with mapping container ports to host ports.  
...
{{< /note >}}
   ...
   * host-network pods of a node can connect directly to the pod IP's on all
