...
that Deployment can create and destroy Pods dynamically, as needed - whether that
is to handle failure, or to match against demand (horizontal scaling).
...
A Service in Kubernetes is a REST object, similar to a Pod or a ConfigMap. 
Like all of the REST objects, you can `POST` a Service definition to the API server
...
and contains a label `app.kubernetes.io/name=MyApp`. You can define a Service to publish that TCP
listener:
...
Kubernetes assigns this Service an IP address (sometimes called the _cluster IP_)
which is used by the virtual IP address mechanism. For more details on that mechanism,
read [Virtual IPs and Service Proxies](/docs/reference/networking/virtual-ips/).

The EndpointSlice {{<glossary_tooltip text="controller" term_id="controller">}}
continuously scans for Pods that match the selectors for existing Services.
When there is a change, either a new endpoint or a removal, that controller then
applies those updates to a set of [EndpointSlices](#endpointslices). The EndpointSlices
for a Service together represent the backend Pods.

The control plane also manages an older API, [Endpoints](#endpoints), that is
similar to EndpointSlice (but has some limitations around scalability). The Endpoints
API is widely used and remains a supported Kubernetes API.
...
(or with a single Endpoints)
object and without a selector, the Service can abstract other kinds of backends,
...
In any of these scenarios you can define a Service _without_ any Pod selector.
For example:
...
domain prefixed names such as `mycompany.example/my-custom-protocol`.

{{< note >}}
The `appProtocol` field is not the same as the `protocol` field for a
[port definition](#field-spec-ports).

The `protocol` field typically declares a transport protocol such as TCP
or UDP, whereas an `appProtocol` defines a service running over that port
such as `nfs`.
{{< /note >}}
...
All more advanced Service types build on this basic model for a Service.
...
spec:
  ...
  ports:
    ...
    - port: 80
      targetPort: 80
      # Optional field
      # By default and for convenience, the Kubernetes control plane will allocate a port from a range
...
Some cloud providers allow you to specify the `loadBalancerIP` field. In those
cases, the load-balancer is created with the user-specified `loadBalancerIP`. If the
`loadBalancerIP` field is not specified, the load balancer is set up with an ephemeral
IP address.

If you specify a `loadBalancerIP` but your cloud provider does not support the feature,
the value of the `loadbalancerIP` field that you set is ignored.
...
_For more details on using a Service with your chosen cloud provider, visit and read that
cloud provider's own documentation._

#### Load balancers with mixed protocol types

{{< feature-state for_k8s_version="v1.24" state="beta" >}}

By default, for Services of `type: LoadBalancer` that have more than one port defined,
all ports must have the same protocol, and the protocol must be one which is supported
by the cloud provider.

Provided that the `MixedProtocolLBService` [feature gate](/docs/reference/command-line-tools-reference/feature-gates/)
is enabled, Kubernetes allows the use of different protocols for Services of
`type: LoadBalancer`: when there is more than one port defined. This lets Kubernetes
integrate with cloud providers or other load balancing mechanisms that can work with
multiple protocols.
...
by setting the field `spec.allocateLoadBalancerNodePorts` to `false`. This should
only be used for load balancer implementations that route traffic directly to pods
as opposed to using node ports. By default, `spec.allocateLoadBalancerNodePorts`
is `true` and type LoadBalancer Services will continue to allocate node ports.

This should only be used for load balancer implementations that route traffic directly to
pods as opposed to using node ports. By default, `spec.allocateLoadBalancerNodePorts`
is `true`, which means that `type: LoadBalancer` Services do allocate node ports.

{{< note >}}
If you have the feature enabled and then set `spec.allocateLoadBalancerNodePorts` to
`false` on an existing Service that already has allocated node ports, those node portsi
are **not** be de-allocated automatically.
...
Any built-in or default load balancer implementation (for example, the one provided
by the cloud provider) ignores all Services that have this field set. You can only
set the `.spec.loadBalancerClass` field a Service of type `LoadBalancer`. Once set,
it cannot be changed.

The value of `.spec.loadBalancerClass` must be a label-style identifier,
with an optional prefix such as `"internal-vip"` or `"example.com/internal-vip"`.
Unprefixed names are reserved for end-users; if you are implementing a load
balancer class that you will publish, make sure you use a suitably prefixed
identifier.

Your cluster must have the `ServiceLoadBalancerClass`
[feature gate](/docs/reference/command-line-tools-reference/feature-gates/) enabled to use this field.
For Kubernetes v{{< skew currentVersion >}}, this feature gate is enabled by default. For clusters
running other versions of Kubernetes, check the documentation for that release.

#### Internal load balancer Services {#internal-load-balancer}
...
#### AWS elastic load balancing and TLS {#ssl-support-on-aws}
...
#### AWS load balancing and PROXY protocol {#proxy-protocol-support-on-aws}
...
#### AWS load balancing and access logging {#elb-access-logs-on-aws}
...
#### AWS load balancing and connection draining {#connection-draining-on-aws}
...
#### AWS load balancing and Network Load Balancer support {#aws-nlb-support}
...
#### AWS load balancing and miscellaneous configuration {#other-elb-annotations}
...
{{< caution >}}
You may have trouble using `type: ExternalName` for some common protocols, including HTTP and HTTPS. If you use ExternalName then the hostname used by clients inside your cluster is different from the name that the ExternalName references.
