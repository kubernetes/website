...
capabilities such as node heartbeats and component-level leader election.
...
the `spec.renewTime` field for the Lease. The Kubernetes control plane uses the timestamp of this field
...
This is used by control plane components such as `kube-controller-manager` and `kube-scheduler` in
HA configurations, where only one instance of the component should be actively running while the other
instances are on standby.
...
rest of the system. While this may not be particularly useful on its own, it provides a mechanism for clients to
discover how many instances of `kube-apiserver` are operating the Kubernetes control plane.
The existence of kube-apiserver leases enables future capabilities that may require coordination between
each kube-apiserver.
...
will take over existing Leases using a new holder identity, instead of instantiating new lease objects. You can check the
