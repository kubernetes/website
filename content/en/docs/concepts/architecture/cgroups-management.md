...
- `/sys/fs/cgroup/`:
  ...
  - `system.slice`: Contains system services (e.g., `sshd`, `containerd`, `kubelet`).
  - `user.slice`: Contains user sessions.

Kubelet manages cgroup settings of `kubepods.slice` to define it's relative importance to the system workloads. These settings are designed to ensure the node stability.

### Observed Root Cgroup Settings
...
## QoS Slices under `kubepods.slice`
...
### cgroupv1, cgroupv2 and a new formula
...
So the new non-linear formula is being rolled out as discussed in the [January 2026 Kubernetes Blog](https://kubernetes.io/blog/2026/01/30/new-cgroup-v1-to-v2-cpu-conversion-formula/).
...
As noted in the responsibilities section, the root `kubepods.slice` also has `cpu.max` set to `max`. This ensures the collective set of all pods can utilize the entire node's CPU capacity if it is not being used by the system.
...
Kubernetes manages memory resources in cgroupv2 primarily through the **`memory.max`** setting, which enforces hard limits. If a cgroup exceeds this limit, the kernel will attempt to reclaim memory; if it fails, the processes within that cgroup are subject to being killed by the OOM (Out of Memory) killer.
