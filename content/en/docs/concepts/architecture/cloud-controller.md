...
such as containers in Pods. Each cloud-controller-manager implements
...
1. Initializing a Node object for each server that the controller discovers through the cloud provider API.
...
3. Obtaining the node's hostname and network addresses.
4. Verifying the node's health. If a node becomes unresponsive, then this controller checks with
   your cloud provider's API to see if the server has been deactivated or deleted or terminated.
   If the node has been deleted from the cloud, then the controller deletes the Node object from your Kubernetes
   cluster.
...
The service controller listens to Service object events (Create, Update, and Delete) and then configures Endpoints for those Services appropriately.
...
The implementation of the core of the cloud controller manager requires access to create Event objects. To ensure secure operation, it requires access to create ServiceAccounts.
...
The Kubernetes core includes the implementation of the shared controllers highlighted in this document (Node, Route, and Service) and some scaffolding along with the shared cloudprovider interface. Implementations specific to cloud providers are outside the core of Kubernetes and implement the `CloudProvider` interface.
