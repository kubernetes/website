...
1.  Upgrade the kubelet on the control plane node:
    ...
    {{% tab name="CentOS, RHEL or Fedora" %}}
    # replace x in 1.14.x-0 with the latest patch version
    yum install -y kubelet-1.14.x-0 --disableexcludes=kubernetes
    {{% /tab %}}
    {{< /tabs >}}
...
1.  Upgrade kubectl on all nodes:
    ...
    apt-get update && apt-get install -y kubectl=1.14.x-00 && \
    apt-mark hold kubectl
    {{% /tab %}}
    {{% tab name="CentOS, RHEL or Fedora" %}}
    # replace x in 1.14.x-0 with the latest patch version
    yum install -y kubectl-1.14.x-0 --disableexcludes=kubernetes
    {{% /tab %}}
    {{< /tabs >}}
...
1.  Upgrade the Kubernetes package version on each `$NODE` node by running the Linux package manager for your distribution:
    ...
    # replace x in 1.14.x-00 with the latest patch version
    apt-get update
    apt-get install -y kubelet=1.14.x-00 kubeadm=1.14.x-00
    {{% /tab %}}
    {{% tab name="CentOS, RHEL or Fedora" %}}
    # replace x in 1.14.x-0 with the latest patch version
    yum install -y kubelet-1.14.x-0 kubeadm-1.14.x-0 --disableexcludes=kubernetes
    {{% /tab %}}
    {{< /tabs >}}
