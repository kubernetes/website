...
Open the file in an editor and set `controlPlaneEndpoint` to either the local node's IP address or the load balancer's dns name if in a high availability configuration -- if it's not set already.

Now run the upgrade on each control plane node one at a time.

```
kubeadm upgrade apply v1.13.0 --config kubeadm-config.yaml
```
