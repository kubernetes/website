...
2. The default scheduler is not topology-aware, so it is possible to be scheduled on a node but then fail admission on the node due to the Topology Manager.
However, an [out-of-tree scheduler plugin](https://github.com/kubernetes-sigs/scheduler-plugins/tree/master/pkg/noderesourcetopology) mitigates this problem and allows optimal scheduling by taking resource topology into consideration.

In order to achieve this, the plugin needs to subscribe to a `NodeResourceTopology` Custom Resource endpoint.
These Custom Resources are generated by a Node Resource Topology Exporter which exposes NUMA information about the various resources available on the machine.
[Node Feature Discovery](https://github.com/kubernetes-sigs/node-feature-discovery) implements the Node Resource Topology Exporter in one of its plugins.
