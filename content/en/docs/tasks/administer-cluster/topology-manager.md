...
2. The default scheduler is not topology-aware, it is possible to be scheduled on a node and then fail on the node due to the Topology Manager. However, [out-of-tree scheduler](https://github.com/kubernetes-sigs/scheduler-plugins/tree/master/pkg/noderesourcetopology) scheduler plugin mitigates this problem and allows optimal scheduling by taking resource topology into consideration.
In order to achieve this, the plugin needs to subscribe to NodeResourceTopology Custom Resources generated by Node Resource Topology Exporter which exposes resource information with NUMA node granularity. [Node Feature Discovery](https://github.com/kubernetes-sigs/node-feature-discovery) implements Node Resource Topology Exporter in one of its plugins.
