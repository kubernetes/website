...
You can enable this option using adding `full-pcups-only=true` to the CPUManager policy options.
...
can fulfilled allocating full physical cores.
If the pod does not pass the admission, it will be put in Failed state with the reason `SMTAlignmentError`.
