...
Changes to original:
* An ingress that preserves source IPs
* ConfigMaps for the Apache2 and Mod-Status configs to block the `/server-status` endpoint from outside the internal network
* A `redis.conf` to set the slowlog time criteria.
