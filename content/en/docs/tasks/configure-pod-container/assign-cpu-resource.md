...
The `- CPU s "2"` argument tells the Container to attempt to use 2 CPU s.
...
```
<!-- 
The output shows that the one container in the  Pod  has a CPU  request of 500 milli CPU 
...
```
<!-- 
This example output shows that the  Pod  is using 974 milli CPU , which is
...
```
<!-- 
Recall that by setting `- CPU  "2"`, you configured the Container to attempt to use 2 CPU s, but the Container is only being allowed to use about 1 CPU . The container's CPU  use is being throttled, because the container is attempting to use more CPU  resources than its limit.
-->
...
Fractional values are allowed. A Container that requests 0.5 CPU  is guaranteed half as much
 ...
 100 m CPU , 100 milli CPU , and 0.1 CPU  are all the same. Precision finer than  1 m is not allowed.
