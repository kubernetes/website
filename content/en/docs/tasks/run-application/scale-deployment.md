...
If you do not already have a Deployment, follow the instructions in [Run a Stateless Application Using a Deployment](/content/en/docs/tasks/run-application/run-stateless-application-deployment/) to create one.
...
{{< note >}}
Replace <deployment-name> with the name of your Deployment (for example, nginx).
{{</ note >}}
...
When the replicas is increased from 2 to 4, Kubernetes creates two additional Pods to match the desired state and schedules them to available nodes while keeping the existing Pods unchanged.
...
Scale to zero set the replica count of a workload to 0.
...
If a HPA is enabled for a Deployment, the HPA may override replica counts that you have set manually.

## {{% heading "whatsnext" %}}
- Learn more about [HorizontalPodAutoscaler](/docs/tasks/run-application/horizontal-pod-autoscale/)
- Read about [Pad disruption budgets](/docs/concepts/workloads/pods/disruptions/)