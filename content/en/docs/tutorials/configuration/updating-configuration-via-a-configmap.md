...
The first container runs a webserver (`nginx`). The mount path for the shared volume in the webserver container is `/usr/share/nginx/html`. The second helper container is based on `alpine`, and for this container the `emptyDir` volume is mounted at `/pod-data`. The helper container writes a file in HTML that has its content based on a ConfigMap. The webserver container serves the HTML via HTTP.
...
As a Sidecar Container is conceptually an Init Container, it is guaranteed to start before the main webserver container.  
This ensures that the HTML file is always available when the webserver is ready to serve it.  
...
In order to modify the behavior of the Pods that use this configuration, you will change the name of
the ConfigMap, and edit the Deployment to define a slightly different pod template, referencing the
new ConfigMap.
{{< /note >}}
...
In the editor that appears, perform the following actions:
- change the name of the ConfigMap from `company-name-20150801` to `company-name-20240312`
- change the value of key `company_name` from `ACME, Inc.` to `Fiktivesunternehmen GmbH`.
...
immutable: true
kind: ConfigMap
# Apart from the name field, you can leave the existing metadata as they are.
# The values you'll see won't exactly match these.
metadata:
  annotations:
    kubectl.kubernetes.io/last-applied-configuration: |
      {"apiVersion":"v1","data":{"company_name":"ACME, Inc."},"immutable":true,"kind":"ConfigMap","metadata":{"annotations":{},"name":"company-name-20150801","namespace":"default"}}
  creationTimestamp: "2024-03-20T03:51:47Z"
  name: company-name-20240312 # new ConfigMap name
  namespace: default
  resourceVersion: "854"
  uid: bc6d7c83-1b01-43d2-acb9-c4e1a4e4a8e8
```

Attempt to save the changes. This action is refused as the name of the ConfigMap was changed. Instead, a copy of the changes is stored in a temporary manifest as shown below:

```
A copy of your changes has been stored to "/var/folders/nn/bb6y1j8d69n9lhqppjlrfthm0000gn/T/kubectl-edit-1440698061.yaml"
error: At least one of apiVersion, kind and name was changed
```

Create a new ConfigMap by referencing the temporary manifest.
```shell
kubectl apply -f /var/folders/nn/bb6y1j8d69n9lhqppjlrfthm0000gn/T/kubectl-edit-1440698061.yaml
```
...
kubectl delete configmap sport fruits color company-name-20150801 company-name-20240312
```