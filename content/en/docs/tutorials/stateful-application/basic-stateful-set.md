...
you could also connect to the IP address of a `type: ClusterIP` backed
by the Pods in that StatefulSet. You can use the same Service that
...
Scaling up means adding more replicas. Provided that your app is able to distribute
work across the Pods that make up the StatefulSet, adding Pods to your StatefulSet
means that your app can then perform more work and provide more capacity.
...
Examine the output of the terminal where the `kubectl get` command is running. It may look something likeL
