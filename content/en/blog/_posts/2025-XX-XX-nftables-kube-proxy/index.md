...
nftables becomes GA, iptables will still be the _default_.)
...
appears earlier or later in the `KUBE-SERVICES` chain).

![kube-proxy iptables first packet latency, at various percentiles, in clusters of various sizes](iptables-only.svg)
...
![kube-proxy nftables first packet latency, at various percentiles, in clusters of various sizes (raw data available at link below)](nftables-only.svg)
...
services, the average (p50) latency for nftables is about the same as
the best-case (p01) latency for iptables. In the 30,000 service
cluster, the _worst-case_ latency for nftables manages to beat out the
_best-case_ latency for iptables by a few microseconds! We can combine
...
number of services and endpoints. Originally, the iptables backend would
rewrite every rule on every update, and with tens of thousands of
services, this could grow to be hundreds of thousands of iptables rules.
Starting in Kubernetes 1.26, we began improving kube-proxy so that it
could skip updating _most_ of the unchanged rules in each update, but
the limitations of `iptables-restore` as an API meant that it was still
always necessary to send an update that's **O(n)** in the number of
services (though with a noticeably smaller constant than it used to be).
...
endpoints. The fact that the nftables API allows each nftables-using
component to have its own private table also means that there is no
global lock contention between components like with iptables. As a
...
First, while kube-proxy's nftables mode behaves very similarly to the
iptables mode at a high level (since both APIs speak to the same
underlying kernel subsystem), it's still possible that some existing
network plugins that work with kube-proxy in iptables mode might not
work right with it in nftables mode; you should consult the
documentation (or forums, bug tracker, etc.) for your network plugin to
see if it supports nftables mode. (Note that some plugins may not need
any changes; as long as they don't try to directly interact with or
...
run kube-proxy in nftables mode on nodes that have a pre-"1.0.0" version
of `nft` in the host filesystem (or else kube-proxy's use of nftables
may interfere with other uses of nftables on the system).
...
(In particular, with nftables mode, NodePort services are only reachable
on their nodes' default IPs, as opposed to being reachable on all IPs,
including `127.0.0.1`, with iptables mode.) The [kube-proxy
documentation] has more information about this, including information
about metrics you can look at to determine if you are relying on any of
the changed functionality, and what configuration options are available
to get more backward-compatible behavior.
...
[how to pass a `KubeProxyConfiguration` to `kubeadm init`]: https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/control-plane-flags/#customizing-kube-proxy
...
While we do not have an immediate plan to drop the IPVS backend, it is
probably doomed in the long run, and people who are currently using
IPVS mode should try out the nftables mode instead (and file bugs if
you think there is missing functionality in nftables mode that you
can't work around).
