...
At alpha release, when pods with `SupplementalGroupsPolicy=Strict` are scheduled to the node that does NOT support this feature, the pod's supplemental groups policy got fallback-ed to the `Merge` policy _silently_.

However, since the beta release (i.e. v1.33), to enforce the policy more strictly, if a pod with `supplementalGroupsPolicy=Strict` are scheduled to a node which does not support this feature (i.e. `.status.features.supplementalGroupsPolicy=false`), __a pod creation will be rejected by kubelet because the node can not ensure the specified policy__. When your pod is rejected, you will see an warning events with `reason=SupplementalGroupsPolicyNotSupported` like below:
...
If you already used the feature, particularly `SupplementalGroupsPolicy=Strict` policy, we assume your cluster's CRI runtimes already support this feature. In this case, you will no need to worry about the pod rejections described in the above section.

However, if your cluster:

- use `SupplementalGroupsPolicy=Strict` policy, but
- its CRI runtimes do NOT support this feature yet,

you will need to prepare the behavioral changes (pod rejection) when upgrading your cluster.
...
- put some label to your nodes whose CRI runtime supports this feature and also put label selector to pods with `Strict` policy to select the nodes (you will need to monitor the number of `Pending` pods in this case instead of pod rejections).
