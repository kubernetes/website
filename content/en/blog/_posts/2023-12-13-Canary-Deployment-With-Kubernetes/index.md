...
In modern software development getting the quick feedback form the users
...
But you are not sure if your users are going to like this feature or not.
...
feedback without ruining the user experience you decided to send the 90% of your traffic
...
repository contains two docker images. One is the canary-app:v1 which is the
stable version of the app. And the other image is canary-app:v2 which is the
...
The kubernetes tries to equally distribute the traffic across the pods, and
...
for the canary-app:v1. The 90% of the traffic will go towards the
...
You can see that the 90% of traffic is flowing towards the app-v1 and the
...
seems efficient. That's why Istio comes into the picture. Istio is a more
sophisticated solution to address these types of issues.

Before moving forward with Istio, change back the values of the `replicas` to the `1`
for the canary-app:v1. And change the service type from the `NodePort` to the 
`ClusterIP`. Because you will be exposing the service using Istio instead of using the
node port. Also change the value of service port to the `80` from the `3000` for
the convenience.

## What is Istio?

Istio is an open source service mesh platform that helps you connect, monitor
and secure the communication between your microservices. Istio makes sure
that a microservice have the access to communicate with other microservices.
...
Once you have an active Kubernetes cluster and the `kubectl` is configured to
...
To make Istio service mesh is accessible from outside the cluster, you need to create
an Istio gateway. Create a file named istio.yaml for deploying Istio gateway and configure
...
In the hosts field a wildcard `*` symbol is specified. This allows for the service
...
as a way to define the rules for how request to a particular service should be handled
...
You can see that here the `canary-gateway` is specified in the virtual service. Specifying
...
you to specify a destination for the requests specified via a `DestinationRule` resource.
...
Now apply the istio.yaml using the following command.

```shell
kubectl apply -f istio.yaml
```

After applying the istio.yaml use the following command to get the external IP of the
...
Copy the external IP of the load balancer and paste that into your browser, and specify
the port `80`. You will see the output of the app. And if you refresh the page several
times you'd be able to see that the traffic is distributed in a 9:1 ratio. You can
