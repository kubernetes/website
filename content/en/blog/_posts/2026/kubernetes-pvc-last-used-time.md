...
"orphaned" PVCs accumulate, silently consuming storage capacity and driving up cloud costs.
...
- **Multiple pods**: If several pods reference the same PVC, the condition transitions to
  `Unused=True` only after the _last_ non-terminated pod is removed.
...
5. Delete the pod and wait for the condition to transit back to `Unused=True`:
