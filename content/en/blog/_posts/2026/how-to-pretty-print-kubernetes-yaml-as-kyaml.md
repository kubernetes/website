...
The interesting part is that Kubernetes doesn't actually need most of those features. It only relies on a small subset of YAML.
...
**Whitespace sensitivity.** Indentation defines structure in YAML, which means a wrongly indented file can be syntactically valid but not apply correctly. This gets especially painful with templating tools like Helm, where you are manipulating indentation from outside the YAML context.
...
- Always quotes value strings — no silent type coercion
...
```bash
# Kubernetes 1.35+ (beta, on by default)
