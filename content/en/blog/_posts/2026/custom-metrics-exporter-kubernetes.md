...
author: >
  Victor D (Independent Contributor)
---
...
If you are running the Prometheus Operator (including
kube-prometheus-stack), create a ServiceMonitor. The `release` label
...
If your Prometheus instead uses annotation-based pod discovery, add
these three annotations to the pod template in the Deployment:
...
Annotation-based scraping relies on a matching `scrape_config` rule in
the Prometheus configuration. If you are unsure whether that rule is in
place, the ServiceMonitor approach is more explicit and easier to debug.
...
next step is surfacing these metrics to the Horizontal Pod Autoscaler
