---
layout: blog
title: "How the controller-runtime Cache Actually Works, and Why Your Operator Does Not Crush the API Server"
draft: true
...
write Kubernetes controllers, this article should help you build a coherent picture and avoid
...
In `-w` (watch) mode `kubectl` subscribes to the same event stream that controllers consume.
...
- **GVK (GroupVersionKind)** — the triple that uniquely identifies a resource type in
  ...
  `controller-runtime` works in terms of GVK rather than "the resource name as it appears in
  kubectl".

- **resourceVersion** — an opaque string the API server attaches to every object (under the
  hood it is a monotonically increasing position in `etcd`). It serves two purposes: it powers
  **optimistic concurrency control** (on `Update`, the API server checks that your
  `resourceVersion` matches the one in `etcd`, otherwise it returns `409 Conflict`); and it is
  used to **resume a watch** (with `WATCH?resourceVersion=X`, the API server replays every
  event that happened after version X).
...
{{< figure src="pipeline.svg" alt="Pipeline diagram: API server to Reflector to DeltaFIFO to Indexer to Event handlers" >}}
...
events, but **grouped by key** and in strict order.
...
controller's event handler extracts the `namespace/name` **key** from the object and enqueues
...
{{< figure src="shared-informer.svg" alt="Shared informer diagram: a single LIST/WATCH per GVK feeding multiple subscribers" >}}
...
Server-Side Apply) is also possible, but that is a separate conversation.

The "write → visibility" cycle now looks like this:

{{< figure src="update-visibility.svg" alt="Write visibility diagram: client.Update to API server to watch event to cache" >}}
...
It does not. **Resync does not perform a `LIST`.** It re-emits everything currently in the
...
operator code reviews answer themselves.
