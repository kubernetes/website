...
Teams increasingly run artificial intelligence (AI) and machine learning (ML)
workloads on Kubernetes. These workloads include notebook servers, distributed
training jobs, hyperparameter tuning, and multi-step pipelines.
[Kubeflow](https://www.kubeflow.org/) provides Kubernetes-native components for
these workflows.

Many Kubeflow components expose their state through
[custom resources](/docs/concepts/extend-kubernetes/api-extension/custom-resources/).
Kubernetes uses CustomResourceDefinition (CRD) objects to define those resources.
Custom resources let cluster operators observe and manage ML workloads through the
Kubernetes API.

Purpose-built ML dashboards focus on workflow submission and the needs of data
scientists. They might not show the Kubernetes details behind a failure. When a
Notebook or TrainJob stalls, operators often use `kubectl` to inspect the backing
Pods and identify the cause.

The **Headlamp Kubeflow plugin** surfaces Kubeflow resources in a general-purpose
Kubernetes user interface (UI). It demonstrates a pattern that other platforms can
use when they model their workloads with custom resources. I contribute to this
plugin, including its resource map and inline summaries.

[Headlamp](https://github.com/kubernetes-sigs/headlamp) is an extensible Kubernetes
web UI maintained under
[Kubernetes SIG UI](https://github.com/kubernetes/community/tree/main/sig-ui) and
licensed under Apache 2.0. Headlamp supports desktop and in-cluster deployments,
and its plugin system can add dedicated views for custom resources.
