...
This enhancement allows credential providers to authenticate with registries
using workload identity, removing the need for persistent image pull secrets.
...
1. **Image pull secrets stored in the Kubernetes API**
   - These secrets are long-lived, making them hard to rotate.
...
### 3. Avoiding long-lived secrets

Unlike traditional image pull secrets, this approach:

- Eliminates the need to store secrets in the Kubernetes API.
- Ensures credentials are ephemeral and automatically rotated.
- Provides isolation between workloads, reducing security risks.
