 ...
 * **The "toolbox" interface of kubeadm** --- Also known as **phases**. If you don't want to consume the all of the `kubeadm init` tasks performed, you can perform fine-grained, generic actions with the `kubeadm init phase` command (for example generating certificates or control plane [Static Pod](https://kubernetes.io/docs/tasks/administer-cluster/static-pod/) manifests).
