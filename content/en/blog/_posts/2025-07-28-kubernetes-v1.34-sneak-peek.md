...
You'll then be able to have the `kubelet` use these tokens when pulling container images from registries that require authentication.
...
Kubernetes does not insist you provide input that is specifically formatted as KYAML, and we have no plan to change that.
