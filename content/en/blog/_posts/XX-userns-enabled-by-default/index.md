---
layout: blog
title: "Kubernetes 1.33: User Namespaces enabled by default!"
date: XXXX-XX-XX
...
In Kubernetes 1.33 support for user namespaces is enabled by default. This means
that, when the stack requirements are met, pods can opt-in to use user
namespaces. There is no need to enable any Kubernetes feature flag anymore!
...
identifiers in the container can be mapped to identifiers on the host in a way
where the host UID/GIDs used for different containers never overlap.
Furthermore, the identifiers can be mapped to unprivileged, non-overlapping UIDs
and GIDs on the host. This brings three key benefits:
...
Rodrigo created some demos, to see how some CVEs are mitigated when user
namespaces are used. We showed them here before, but take a look if you haven't:
...
[userns-req]: https://kubernetes.io/docs/concepts/workloads/pods/user-namespaces/#before-you-begin

**2. What is idmap mounts and why the file-systems used need to support it?**

Idmap mounts is a Linux kernel feature that uses a mapping of UID/GIDs when
accessing a mount. When combined with user namespaces, it greatly simplifies the
support for volumes, as you can forget about the host UID/GIDs the user
namespace is using.
...
However, support for idmap mounts in the kernel is not black/white. The support
is per file-system and support was added over time to different file-systems.

You can check which kernel version added support for each file-system in the
`mount_setattr` man page, or checking the online version of it [here][mount_setattr].

Most popular file-systems are supported in some kernel version, the notable
absence that isn't supported yet is NFS.

[mount_setattr]: https://man7.org/linux/man-pages/man2/mount_setattr.2.html#NOTES

**3. Can you clarify exactly which file-systems need to support idmap mounts?**
...
6.3 kernel to use these volumess (note that if you use them as env variables it
is fine).
...
**4. Can I use a kernel older than 6.3?**
...
To avoid using `tmpfs` can be tricky, though, as we just mentioned. But besides
having to avoid those volume types, you will also have to avoid mounting the
service account token. Every pod have it mounted by default, and it is using a
projected volume that, as we mentioned, uses a `tmpfs` file-system.

You can even go lower than 5.19, all the way to 5.12. However, your container
rootfs is probably using an overlayfs file-system, and support for overlayfs was
...
some years ago, about tricks to use user-namespaces when you we can't support
...
**5. If my stack supports user namespaces, do I need to configure anything else?**

No, if your stack supports it and you are using Kubernetes 1.33, there is
nothing you need to configure. You should be able to follow the task: [Use a
user namespace with a pod][task-userns].

However, there are some configuration you can do, you can check them
[here][userns-k8s-conf]. You can also enable a [feature gate to relax the PSS
rules][userns-pss].

[userns-k8s-conf]: https://kubernetes.io/docs/concepts/workloads/pods/user-namespaces/#set-up-a-node-to-support-user-namespaces
[task-userns]: https://kubernetes.io/docs/tasks/configure-pod-container/user-namespaces/
[userns-pss]: https://kubernetes.io/docs/concepts/workloads/pods/user-namespaces/#integration-with-pod-security-admission-checks

**6. The demos are nice, but are there more CVEs that this mitigates?**
...
**7. Can you sum up why user namespaces is important?**
...
you). In the same talk, Greg and Vinayak show how to achieve the same without
user namespaces we went through a quite complex solution. This part [starts in
this minute][kubecon-nonroot-uids].
...
**8. Is there container runtime documentation for user namespaces?**
...
**9. What about the other container runtimes?**
...
**10. I'd like to learn more about it, what would you recommend?**
...
Also, this presentation at Kubecon 2023, that we mentioned earlier, can be
...
User-namespaces is a unique feature to let you have the best of both worlds: run
