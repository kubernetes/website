...
highlighted in the recent [Llama
...
there. This assumption combined with the overall complexity of a set up, leads
...
* **Training:** These workloads are resource-intensive, often consuming entire
...
* **Inference:** These workloads are usually long-running or run indefinitely,
...
| Images are slim and easily available. | Images are big and require special handling. |
...
This article is not going deeper into the question of "why not start fresh" for
...
{{< figure src="k8s-infra-devices.svg" alt="General diagram of how device plugin works" >}}
...
{{< figure src="k8s-infra-failures.svg" alt="General diagram of possible failures in how device plugin works" >}}
...
* Configure and restart kubelet and the container runtime (containerd or CRI-O)
...
There are many problems with the "health controller":
...
policy](https://kubernetes.io/docs/tasks/job/pod-failure-policy/)

There are some problems with the "Pod failure policy":

* There is no "device failed" condition, so this approach does not work for the
  generic Pod case  
...
it can then delete the attached Pod w as a remediation. The replica set will
...
* There is no "unscheduling" for a pod with `restartPolicy=Always`. 
...
money saved by doing a regular JobSet delete/recreate cycle. But we would
definitely benefit from more hooks and extension points in Kubernetes.

### Failure modes: device degradation

Not all device failures are "terminal". As the hardware stack gets more and more
...
### Roadmap for failure modes: k8s infra
...
Basically, for every interaction of Kubernetes components, we ensure this
...
about failed devices available easier. The step 0 here is the work in the KEP:
Device Health in Pod Status: [Add Resource Health Status to the Pod Status for
Device Plugin and DRA · Issue #4680 · kubernetes/enhancements ·
GitHub](https://github.com/kubernetes/enhancements/issues/4680)
...
* Ability to unschedule pod, including with the restartPolicy=Always, so it can
...
{{< figure src="inplace-pod-restarts.svg" alt="Restarting one pod while recycling all others in a large training job" >}}
![](k8s-infra-failures.svg)
...
We expect developments in this area from Device and Cloud vendors and mostly DIY
