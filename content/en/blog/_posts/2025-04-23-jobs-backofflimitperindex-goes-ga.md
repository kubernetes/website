...
This post describes the Job's _Backoff Limit Per Index_ feature, which graduates to
stable in Kubernetes 1.33, and how you can benefit from using it.

## About

Running your workloads on Kubernetes, you need to account for Pod failures due
to variety of reasons. Ideally, your workload should tolerate some transient
...
However, for Indexed Jobs this is often not flexible enough for
[embarassingly parallel](https://en.wikipedia.org/wiki/Embarrassingly_parallel)
workloads, where every index is considered independent. For example, integration
tests. In that scenario, a fast-failing index is likely to consume your entire
budget for tolerating Pod failures, and you may not be able to run the other
indexes.
...
`spec.backoffLimitPerIndex` field. Important to note is that this field will only apply to Indexed Job.
...
using the "FailIndex" action in the
[Pod Failure Policy](/docs/concepts/workloads/controllers/job/#pod-failure-policy)
feature (see the example below).

When the `spec.backoffLimitPerIndex` is specified the Job will execute all
indexes, unless the cap of the total number of failed indexes is specified by
the `spec.maxFailedIndexes` field.
