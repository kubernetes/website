...
as an Alpha feature with the Kubernetes 1.27 release and moved to [Beta](https://kubernetes.io/blog/2024/12/18/kubernetes-1-32-volume-group-snapshot-beta/) in the Kubernetes 1.32 release.
...
While testing the beta version, we encountered an [issue](https://github.com/kubernetes-csi/external-snapshotter/issues/1271) where the restoreSize is not set for individual VolumeSnapshotContents and VolumeSnapshots if CSI driver does not implement ListSnapshots.
We evaluated various options [here](https://docs.google.com/document/d/1LLBSHcnlLTaP6ZKjugtSGQHH2LGZPndyfnNqR1YvzS4/edit?tab=t.0) and decided to make this v1beta2 API change.

Specifically, a VolumeSnapshotInfo struct is added in v1beta2. VolumeSnapshotInfo contains information for an indivivual volume snapshot that is a member of a volume group snapshot.
A VolumeSnapshotInfoList, a list of VolumeSnapshotInfo, is added to VolumeGroupSnapshotContentStatus to replace VolumeSnapshotHandlePairList.
