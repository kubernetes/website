...
As a DRA driver developer writing in Golang, you can make a device within a ResourceSlice allocatable to multiple ResourceClaims (or DeviceRequests) by setting `AllowMultipleAllocations` to `true`.
...
Notably that **any** `resource.example.com` device that has at least 10 GiB of memory can be allocated.
...
One particularly useful use case is for virtual networks, where a driver can include the assigned IP address in the status.
