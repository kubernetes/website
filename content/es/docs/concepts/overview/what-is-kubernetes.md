...
El soporte, las herramientas y los servicios para Kubernetes son ampliamente disponibles.
...
Kubernetes ofrece un ambiente de administración **centrado en contenedores**. Kubernetes
...
A pesar de que Kubernetes ofrece muchas funcionalidades, siempre hay nuevos
escenarios que se benefician de nuevas características. Los flujos de trabajo
de las aplicaciones pueden optimizarse para acelerar el tiempo de desarrollo. La
orquestación ad hoc, aceptable al principio de un proyecto, a menudo requiere una
robusta automatización de gran escala. Es por ello que Kubernetes fue diseñada como
una plataforma para construir un ecosistema de componentes y herramientas que hacen
más fácil el desplegar, escalar y administrar aplicaciones.

Las etiquetas, o [Labels](/docs/concepts/overview/working-with-objects/labels/), le
permiten a los usuarios organizar sus recursos como deseen. Las anotaciones , o [Annotations](/docs/concepts/overview/working-with-objects/annotations/), les permiten asignar información arbitraria a un recurso para
...
Los usuarios pueden escribir sus propios controladores, como por ejemplo un agendador o [scheduler](https://github.com/kubernetes/community/blob/{{< param "githubbranch" >}}/contributors/devel/scheduler.md),
usando [sus propias
...
* No limita el tipo de aplicaciones que soporta. Kubernetes busca dar soporte a un número diverso de cargas de trabajo, que incluyen aplicaciones con y sin estado así como aplicaciones que procesan datos. Si la aplicación puede correr en un contenedor, debe correr bien en Kubernetes.
...
Con contenedores, podemos crear imagenes inmutables al momento de la compilación
en vez del despliegue ya que las aplicaciones no necesitan componerse junto al
resto de la pila ni atarse al ambiente de infraestructura de producción. Generar
una imagen de contenedor al momento de la compilación permite tener un ambiente
...
* **Ágil creación y despliegue de aplicaciones**:
    Mayor facilidad y eficiencia al crear imagenes de contenedor en vez de máquinas virtuales
...
* **Separación de tareas entre Dev y Ops**:
    Puedes crear imagenes de contenedor al momento de compilar y no al desplegar, desacoplando la
...
* **Consistencia entre los ambientes de desarrollo, pruebas y producción**:
...
* **[Microservicios](https://martinfowler.com/articles/microservices.html)** distribuidos, elásticos, liberados y débilmente acoplados:
    Las aplicacionesse separan en piezas pequeñas e independientes que pueden ser desplegadas y administradas de forma dinámica, y no como una pila monolítica que opera en una sola máquina grande
