...
Kubernetes 1.19 y versiones posteriores reciben [aproximadamente 1 año de soporte para parches](/releases/patch-releases/#periodo-de-soporte).
...
{{< note >}}
Si existe un desfase de versiones entre las instancias de `kube-apiserver` en un clúster HA, esto reduce el rango de versiones permitidas para `kubelet`.
{{< /note >}}
...
{{< note >}}
Si existe un desfase de versiones entre las instancias de `kube-apiserver` en un clúster HA, esto reduce el rango de versiones permitidas para `kube-proxy`.
{{</ note >}}
...
* En un clúster HA, todas las instancias de `kube-apiserver` están en las versiones **{{< skew currentVersionAddMinor -1 >}}** o
